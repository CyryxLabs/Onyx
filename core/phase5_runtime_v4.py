"""Host-owned Phase 5 integration core (isolated V4 candidate).

This module is deliberately not imported by a live/startup surface.  V4 keeps
...
PHASE5_RUNTIME_FLAG = "ONYX_PHASE5_RUNTIME_V4"
PHASE5_GRANT_SHADOW_FLAG = "ONYX_PHASE5_GRANT_SHADOW_V4"
PHASE5_APPROVAL_INBOX_FLAG = "ONYX_PHASE5_APPROVAL_INBOX_V4"
PHASE5_LOW_RISK_FLAG = "ONYX_PHASE5_LOW_RISK_V4"
PHASE5_NEXUS_PROJECTION_FLAG = "ONYX_PHASE5_NEXUS_PROJECTION_V4"
PHASE5_LOCAL_CATALOG_READ_FLAG = "ONYX_PHASE5_LOCAL_CATALOG_READ_V4"
PHASE5_DASHBOARD_PROJECTION_FLAG = "ONYX_PHASE5_DASHBOARD_PROJECTION_V4"
...
class Phase5RuntimeV4Error(RuntimeError):
    """Base error for the isolated V4 runtime core."""


class Phase5RuntimeV4ContractError(ValueError):
    """A value violated the closed and bounded V4 host contract."""


class RuntimeDispositionV4(str, Enum):
    FALLBACK = "FALLBACK"
    ALLOW_LOCAL_CATALOG_READ = "ALLOW_LOCAL_CATALOG_READ"
    EXPLICIT = "EXPLICIT"


class RuntimeStateV4(str, Enum):
    DISABLED = "DISABLED"
    READY = "READY"
    DEGRADED = "DEGRADED"
...
class TerminalReasonV4(str, Enum):
    KILL = "kill"
    REVOKE = "revoke"
    ROLLBACK = "rollback"
...
def _exact_bool(value: object, label: str) -> bool:
    if type(value) is not bool:
        raise Phase5RuntimeV4ContractError(f"{label} must be an exact boolean")
    return value


def _safe_id(value: object, label: str) -> str:
    if type(value) is not str or not _SAFE_ID.fullmatch(value):
        raise Phase5RuntimeV4ContractError(f"{label} is invalid")
    if _contains_sensitive_value(value):
        raise Phase5RuntimeV4ContractError(f"{label} contains sensitive data")
    return value
...
class RuntimeFlagsV4:
    ...
    @classmethod
    def from_environ(cls, environ: Mapping[str, str] | None = None) -> RuntimeFlagsV4:
...
def _binding(value: object) -> RuntimeBindingV4:
    if type(value) is not RuntimeBindingV4:
        raise Phase5RuntimeV4ContractError("binding must be exact RuntimeBindingV4")
    return RuntimeBindingV4(
        value.trace_id,
        value.session_id,
        value.workspace_id,
        value.account_id,
...
def _flags(value: object) -> RuntimeFlagsV4:
    if type(value) is not RuntimeFlagsV4:
        raise Phase5RuntimeV4ContractError("flags must be exact RuntimeFlagsV4")
    return RuntimeFlagsV4(**dict(value.payload()))


@dataclass(frozen=True, slots=True)
class ActionRequestV4:
    invocation_ref: str
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.cost_micro) is not int or not 0 <= self.cost_micro <= MAX_INTEGER:
            raise Phase5RuntimeV4ContractError("cost_micro is invalid")
        if self.risk not in _RISKS:
            raise Phase5RuntimeV4ContractError("risk is invalid")
        if type(self.action_classes) is not tuple or len(self.action_classes) > 32:
            raise Phase5RuntimeV4ContractError("action_classes is invalid")
        classes = tuple(
            _safe_id(item, "action_class").casefold() for item in self.action_classes
        )
        if len(classes) != len(set(classes)):
            raise Phase5RuntimeV4ContractError("action_classes contains duplicates")
        object.__setattr__(self, "action_classes", tuple(sorted(classes)))


def _action(value: object) -> ActionRequestV4:
    if type(value) is not ActionRequestV4:
        raise Phase5RuntimeV4ContractError("action must be exact ActionRequestV4")
    return ActionRequestV4(
...
@dataclass(frozen=True, slots=True)
class RuntimeDecisionV4:
    disposition: RuntimeDispositionV4
    reason: str
    trace_id: str
    epoch: int
    advisory_json: str = "{}"
    ...
    def __post_init__(self) -> None:
        if type(self.disposition) is not RuntimeDispositionV4:
            raise Phase5RuntimeV4ContractError("decision disposition is invalid")
        _text(self.reason, "reason", 128, redact=False)
        _safe_id(self.trace_id, "trace_id")
        if type(self.epoch) is not int or self.epoch < 1:
            raise Phase5RuntimeV4ContractError("decision epoch is invalid")
        if (
            type(self.advisory_json) is not str
            or len(self.advisory_json.encode()) > MAX_EVENT_TEXT_BYTES
        ):
            raise Phase5RuntimeV4ContractError("advisory_json is invalid")
        try:
            parsed = json.loads(self.advisory_json)
        except json.JSONDecodeError as exc:
            raise Phase5RuntimeV4ContractError("advisory_json is invalid") from exc
        if type(parsed) is not dict or set(parsed) - {
            ...
            raise Phase5RuntimeV4ContractError("advisory_json schema is invalid")
...
class _Budget:
    ...
    def add(self, *, nodes: int = 1, bytes_: int = 0) -> None:
        ...
        if self.nodes > MAX_JSON_NODES or self.bytes > MAX_JSON_BYTES:
            raise Phase5RuntimeV4ContractError("JSON budget exceeded")
...
def _json_value(
    ...
    if budget is None:
        budget = _Budget()
    if depth > MAX_DEPTH:
        raise Phase5RuntimeV4ContractError("JSON nesting is too deep")
    budget.add()
    if value is None or type(value) is bool:
        return value
    if type(value) is int:
        if not -MAX_INTEGER <= value <= MAX_INTEGER:
            raise Phase5RuntimeV4ContractError("JSON integer is out of bounds")
    ...
    if type(value) in {list, tuple, dict, MappingProxyType}:
        ...
        if identity in budget.seen:
            raise Phase5RuntimeV4ContractError(
                "JSON contains a cycle or shared container"
            )
        budget.seen.add(identity)
    if type(value) in {list, tuple}:
        if len(value) > MAX_FANOUT:
            raise Phase5RuntimeV4ContractError("JSON sequence fanout exceeded")
    ...
    if type(value) is dict or type(value) is MappingProxyType:
        if len(value) > MAX_FANOUT:
            raise Phase5RuntimeV4ContractError("JSON object fanout exceeded")
        ...
        for key, item in value.items():
            if (
                ...
                raise Phase5RuntimeV4ContractError("JSON key is unsafe")
            budget.add(nodes=0, bytes_=len(key.encode()))
            result[key] = _json_value(item, depth=depth + 1, budget=budget)
        return result
    raise Phase5RuntimeV4ContractError("JSON contains a non-JSON value")
...
class ProjectionBatchV4:
    ...
    def __post_init__(self) -> None:
        if type(self.items) is not tuple or len(self.items) > MAX_PAGE_SIZE:
            raise Phase5RuntimeV4ContractError("projection batch is invalid")
        ...
        if self.next_cursor is not None and self.next_cursor == self.source_cursor:
            raise Phase5RuntimeV4ContractError("projection cursor does not advance")


def _batch(value: object) -> ProjectionBatchV4:
    if type(value) is not ProjectionBatchV4:
        raise Phase5RuntimeV4ContractError("component returned the wrong batch type")
    budget = _Budget()
    return ProjectionBatchV4(
...
class ApprovalInboxReaderV4(Protocol):
    def read_page(
        self,
        *,
        binding: RuntimeBindingV4,
...
@dataclass(frozen=True, slots=True)
class RuntimeComponentsV4:
    grant_evaluator: Callable[[str], object] | None = None
    nexus_projector: Callable[[RuntimeBindingV4], ProjectionBatchV4] | None = None
    inbox_factory: Callable[[RuntimeBindingV4], ApprovalInboxReaderV4] | None = None
    ...
    def __post_init__(self) -> None:
        for name in self.__dataclass_fields__:
            ...
            if value is not None and not callable(value):
                raise Phase5RuntimeV4ContractError(f"{name} must be callable")


def _components(value: object) -> RuntimeComponentsV4:
    if type(value) is not RuntimeComponentsV4:
        raise Phase5RuntimeV4ContractError(
            "components must be exact RuntimeComponentsV4"
        )
    return RuntimeComponentsV4(
        **{name: getattr(value, name) for name in value.__dataclass_fields__}
    )


@dataclass(frozen=True, slots=True)
class ProjectionItemV4:
    item_id: str
    payload_json: str
...
@dataclass(frozen=True, slots=True)
class ProjectionPageV4:
    kind: str
    offset: int
    page_size: int
    total: int
    items: tuple[ProjectionItemV4, ...]
...
@dataclass(frozen=True, slots=True)
class _DecisionSealV4:
    decision: RuntimeDecisionV4
    public_digest: str
    request_digest: str
    binding_digest: str
    flags_digest: str
...
def _binding_digest(value: RuntimeBindingV4) -> str:
    return _digest(
        [
            value.trace_id,
            value.session_id,
...
def _flags_digest(value: RuntimeFlagsV4) -> str:
    return _digest(value.payload())


def _action_digest(value: ActionRequestV4) -> str:
    return _digest(
        {
...
def _decision_digest(value: RuntimeDecisionV4) -> str:
    return _digest(
        [
...
def _closed_inbox(
    ...
    if set(raw) != _INBOX_FIELDS:
        raise Phase5RuntimeV4ContractError("inbox schema fields are invalid")
    ...
    if risk not in _RISKS or state not in _INBOX_STATE:
        raise Phase5RuntimeV4ContractError("inbox enum is invalid")
    created_at = _text(raw["created_at"], "created_at", 32, redact=False)
    if not _RFC3339_UTC.fullmatch(created_at):
        raise Phase5RuntimeV4ContractError("created_at is invalid")
    payload: dict[str, object] = {
        "item_id": identifier,
...
def _record(
    surface: str, raw: Mapping[str, object], binding: RuntimeBindingV4
) -> ProjectionItemV4:
    identifier, payload = (
    ...
    if len(encoded.encode()) > MAX_PROJECTION_ROW_BYTES:
        raise Phase5RuntimeV4ContractError("projection row is too large")
    return ProjectionItemV4(identifier, encoded)
...
def _advisory(value: object) -> str:
    if value is None:
        ...
        if set(value) - {
            ...
            raise Phase5RuntimeV4ContractError("grant advisory fields are invalid")
        raw = dict(value)
    else:
        raw = {}
    ...
    if len(encoded.encode()) > MAX_EVENT_TEXT_BYTES:
        raise Phase5RuntimeV4ContractError("grant advisory is too large")
    return encoded
...
class Phase5RuntimeV4:
    ...
    def __init__(
        ...
        *,
        binding: RuntimeBindingV4,
        flags: RuntimeFlagsV4 | None = None,
        components: RuntimeComponentsV4 | None = None,
    ) -> None:
        self._binding = _binding(binding)
        self._flags = _flags(flags or RuntimeFlagsV4.from_environ())
        self._components = _components(components or RuntimeComponentsV4())
        ...
        self._capabilities: OrderedDict[str, ProjectionItemV4] = OrderedDict()
        self._inbox: OrderedDict[str, ProjectionItemV4] = OrderedDict()
        ...
        self._events: deque[RuntimeEventV4] = deque(maxlen=MAX_EVENTS)
        self._event_sequence = 0
        self._decision_seals: OrderedDict[int, _DecisionSealV4] = OrderedDict()
        self._decision_key = secrets.token_bytes(32)

    @property
    def binding(self) -> RuntimeBindingV4:
        with self._lock:
            return _binding(self._binding)

    @property
    def flags(self) -> RuntimeFlagsV4:
        with self._lock:
    ...
    def _event_locked(
        ...
        *,
        ...
        if count is not None and (
            ...
            raise Phase5RuntimeV4ContractError("event count is invalid")
        reason = (
            None
            if reason is None
    ...
    def rebind(self, binding: RuntimeBindingV4) -> RuntimeStatusV4:
    ...
    def configure_flags(self, flags: RuntimeFlagsV4) -> RuntimeStatusV4:
    ...
    def replace_components(self, components: RuntimeComponentsV4) -> RuntimeStatusV4:
    ...
    def _ensure_active_locked(self) -> None:
        if self._terminated:
            raise Phase5RuntimeV4Error("runtime is terminated")
    ...
    def _state_locked(self) -> RuntimeStateV4:
        if self._termination_running:
            return RuntimeStateV4.TERMINATING
        if self._termination_complete:
            return RuntimeStateV4.TERMINATED
        if not self._flags.runtime:
            return RuntimeStateV4.DISABLED
        if any(
            ...
            return RuntimeStateV4.DEGRADED
        return RuntimeStateV4.READY

    def _status_locked(self) -> RuntimeStatusV4:
        ...
        reason = (
            ...
            if self._terminated
                ...
                "feature_flag_off"
                if state is RuntimeStateV4.DISABLED
                else "component_failure"
                if state is RuntimeStateV4.DEGRADED
                else "ready"
            )
        )
        return RuntimeStatusV4(
            state,
    ...
    def status(self) -> RuntimeStatusV4:
        with self._lock:
            return self._status_locked()
    ...
    def _page_bounds(offset: int, page_size: int) -> tuple[int, int]:
        if type(offset) is not int or not 0 <= offset <= MAX_PROJECTION_ITEMS:
            raise Phase5RuntimeV4ContractError("offset is invalid")
        if type(page_size) is not int or not 1 <= page_size <= MAX_PAGE_SIZE:
            raise Phase5RuntimeV4ContractError("page_size is invalid")
        return offset, page_size

    def events(
        self, *, offset: int = 0, page_size: int = MAX_PAGE_SIZE
    ) -> tuple[RuntimeEventV4, ...]:
    ...
    def _set_failure_locked(self, name: str, failed: bool) -> None:
        if name not in _COMPONENTS:
            raise Phase5RuntimeV4ContractError("component is invalid")
    ...
    @staticmethod
    def _contains_explicit(action: ActionRequestV4) -> bool:
        if (
    ...
    def _decision_locked(
        ...
        *,
        ...
        if disposition is RuntimeDispositionV4.ALLOW_LOCAL_CATALOG_READ:
            ...
            self._decision_seals[id(decision)] = _DecisionSealV4(
                decision,
                public,
                request_digest,
                binding,
    ...
    def evaluate(self, action: ActionRequestV4) -> RuntimeDecisionV4:
        with self._lock:
            if self._terminated:
                return self._decision_locked(
                    RuntimeDispositionV4.EXPLICIT, "runtime_terminated"
                )
            if not self._flags.runtime:
                return self._decision_locked(
                    RuntimeDispositionV4.FALLBACK, "feature_flag_off", publish=False
                )
            try:
                snapshot = _action(action)
            except (AttributeError, Phase5RuntimeV4ContractError):
                return self._decision_locked(
                    RuntimeDispositionV4.EXPLICIT, "invalid_action_contract"
                )
            if self._contains_explicit(snapshot):
                return self._decision_locked(
                    RuntimeDispositionV4.EXPLICIT, "explicit_action_class"
                )
            if not self._flags.low_risk or not self._flags.local_catalog_read:
                return self._decision_locked(
                    RuntimeDispositionV4.FALLBACK, "extension_path_off", publish=False
                )
            if snapshot.binding != self._binding:
                return self._decision_locked(
                    RuntimeDispositionV4.EXPLICIT, "scope_mismatch"
                )
            exact = (
            ...
            if not exact:
                return self._decision_locked(
                    RuntimeDispositionV4.EXPLICIT, "not_exact_local_catalog_read"
                )
            if any(
                self._enabled_component_locked(name)
                and (
                ...
                return self._decision_locked(
                    RuntimeDispositionV4.EXPLICIT, "component_unverified_or_failed"
                )
            observer = (
            ...
            if self._flags.grant_shadow and observer is None:
                ...
                return self._decision_locked(
                    RuntimeDispositionV4.EXPLICIT, "grant_shadow_unavailable"
                )
        ...
        if observer is not None:
            try:
                ...
                with self._lock:
                    return self._decision_locked(
                        RuntimeDispositionV4.EXPLICIT, "grant_shadow_failure"
                    )

        with self._lock:
            if self._terminated or config_lease != self._config_generation:
                return self._decision_locked(
                    RuntimeDispositionV4.EXPLICIT, "authority_revoked_during_evaluation"
                )
            if component_lease is not None:
                if not self._lease_current_locked("grant_shadow", component_lease):
                    return self._decision_locked(
                        RuntimeDispositionV4.EXPLICIT, "evaluation_superseded"
                    )
            if any(
                self._enabled_component_locked(name)
                ...
                return self._decision_locked(
                    RuntimeDispositionV4.EXPLICIT, "component_failure_during_evaluation"
                )
            return self._decision_locked(
                RuntimeDispositionV4.ALLOW_LOCAL_CATALOG_READ,
    ...
    def legacy_or_decision(
        self, action: ActionRequestV4, legacy: Callable[[], _T]
    ) -> _T | RuntimeDecisionV4:
        if not callable(legacy):
            raise Phase5RuntimeV4ContractError("legacy fallback must be callable")
        decision = self.evaluate(action)
        return (
            legacy()
            if decision.disposition is RuntimeDispositionV4.FALLBACK
            else decision
        )

    def _current_locked(
        self, decision: RuntimeDecisionV4, action: ActionRequestV4
    ) -> bool:
        if (
            type(decision) is not RuntimeDecisionV4
            or type(action) is not ActionRequestV4
        ):
            return False
        ...
        try:
            ...
            return False
        return (
            decision.disposition is RuntimeDispositionV4.ALLOW_LOCAL_CATALOG_READ
    ...
    def decision_is_current(
        self, decision: RuntimeDecisionV4, action: ActionRequestV4 | None = None
    ) -> bool:
    ...
    def _prepare(
        surface: str,
        batch: ProjectionBatchV4,
        binding: RuntimeBindingV4,
    ) -> tuple[ProjectionBatchV4, tuple[ProjectionItemV4, ...]]:
    ...
    @staticmethod
    def _apply(
        target: OrderedDict[str, ProjectionItemV4],
        records: tuple[ProjectionItemV4, ...],
        *,
        replace: bool,
    ...
    def refresh_capabilities(self) -> ProjectionPageV4:
        with self._lock:
            if (
                self._terminated
    ...
    def refresh_inbox(
        ...
        try:
            ...
            if not callable(read_page):
                raise Phase5RuntimeV4ContractError("inbox reader has no read_page")
            snapshot, records = self._prepare(
                "inbox",
                read_page(
            ...
            if snapshot.source_cursor != cursor:
                raise Phase5RuntimeV4ContractError("inbox source cursor mismatch")
            if cursor is None and not snapshot.replace:
                raise Phase5RuntimeV4ContractError("first inbox page must replace")
            if cursor is not None and snapshot.replace:
                raise Phase5RuntimeV4ContractError("continuation cannot replace")
            if not records and snapshot.next_cursor is not None:
                raise Phase5RuntimeV4ContractError("empty inbox page cannot continue")
        except BaseException:
        ...
        with self._lock:
            ...
            except Phase5RuntimeV4ContractError:
                self._set_failure_locked("inbox", True)
                return self._page_locked("inbox", 0, page_size)
    ...
    def recover_component(self, name: str) -> RuntimeStatusV4:
        ...
        with self._lock:
            self._ensure_active_locked()
            if not self._flags.runtime:
                raise Phase5RuntimeV4Error("runtime is disabled")
            if not self._enabled_component_locked(name):
                raise Phase5RuntimeV4ContractError("component is not enabled")
        ...
        snapshot: ProjectionBatchV4 | None = None
        records: tuple[ProjectionItemV4, ...] = ()
        try:
            if name == "grant_shadow":
                assert callable(component)
                _advisory(component("phase5-v4-probe"))
            elif name == "nexus":
                assert callable(component)
                snapshot, records = self._prepare(
    ...
    def _page_locked(self, kind: str, offset: int, page_size: int) -> ProjectionPageV4:
    ...
    def dashboard_read(
        ...
        *,
    ...
    ) -> RuntimeStatusV4 | ProjectionPageV4 | tuple[RuntimeEventV4, ...]:
        if type(surface) is not str or surface not in {
            ...
            raise Phase5RuntimeV4ContractError("dashboard surface is not allowlisted")
        with self._lock:
            if not self._flags.runtime or not self._flags.dashboard_projection:
                raise Phase5RuntimeV4Error("dashboard projection is disabled")
        if surface == "status":
            return self.status()
        if surface == "capabilities":
    ...
    def terminate(self, reason: TerminalReasonV4) -> RuntimeStatusV4:
        if type(reason) is not TerminalReasonV4:
            raise Phase5RuntimeV4ContractError("terminal reason must be exact")
        with self._terminal_condition:
            if self._termination_complete:
                return self._status_locked()
            if self._termination_running:
                if self._termination_owner_thread == threading.get_ident():
                    raise Phase5RuntimeV4Error("reentrant termination is not allowed")
    ...
    def kill(self) -> RuntimeStatusV4:
        return self.terminate(TerminalReasonV4.KILL)

    def revoke(self) -> RuntimeStatusV4:
        return self.terminate(TerminalReasonV4.REVOKE)

    def rollback(self) -> RuntimeStatusV4:
        return self.terminate(TerminalReasonV4.ROLLBACK)

    def end_session(self) -> RuntimeStatusV4:
        return self.terminate(TerminalReasonV4.END_SESSION)

    def reconnect(self) -> RuntimeStatusV4:
        return self.terminate(TerminalReasonV4.RECONNECT)

    def shutdown(self) -> RuntimeStatusV4:
        return self.terminate(TerminalReasonV4.SHUTDOWN)
...
__all__ = [
    ...
    "MAX_FANOUT",
    "ActionRequestV4",
    "ApprovalInboxReaderV4",
    "Phase5RuntimeV4",
    "Phase5RuntimeV4ContractError",
    "Phase5RuntimeV4Error",
    "ProjectionBatchV4",
    "ProjectionItemV4",
    "ProjectionPageV4",
    "RuntimeBindingV4",
