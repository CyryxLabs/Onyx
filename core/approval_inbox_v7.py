"""Phase 5.2 V7 default-off, non-authoritative approval review projection.
...
APPROVAL_INBOX_FLAG = "ONYX_APPROVAL_INBOX_V7"
INBOX_SCHEMA_VERSION = 7
SNAPSHOT_SCHEMA_VERSION = 7
TOKEN_SCHEMA_VERSION = 7
MAX_ITEMS = 128
MAX_SNAPSHOTS = 8
MAX_VIEWS = 16
MAX_PAGE_SIZE = 50
...
class ApprovalInboxV7Error(RuntimeError):
    pass


class ApprovalInboxV7ContractError(ValueError):
    pass


class ApprovalInboxV7Disabled(ApprovalInboxV7Error):
    pass


class ApprovalInboxV7Stale(ApprovalInboxV7Error):
    pass


class ApprovalInboxV7Unavailable(ApprovalInboxV7Error):
    pass
...
def _bounded_int(value: object, label: str, minimum: int, maximum: int) -> int:
    if type(value) is not int or not minimum <= value <= maximum:
        raise ApprovalInboxV7ContractError(f"{label} is out of bounds")
    return value
...
def _safe_id(value: object, label: str) -> str:
    if (
        ...
        raise ApprovalInboxV7ContractError(f"{label} is invalid")
    return value
...
def _workspace(value: object) -> str:
    if (
        ...
        raise ApprovalInboxV7ContractError("workspace_id is invalid")
    return value
...
def _digest(value: object, label: str) -> str:
    if (
        ...
        raise ApprovalInboxV7ContractError(f"{label} is not a canonical digest")
    return value
...
def _text(value: object, label: str, maximum: int = MAX_TEXT) -> str:
    ...
    if any(ord(character) < 32 or ord(character) == 127 for character in value):
        raise ApprovalInboxV7ContractError(f"{label} contains control characters")
    if contains_secret(value):
        raise ApprovalInboxV7ContractError(f"{label} contains secret-like material")
    return value
    ...
    def now_ms(self) -> int:
        ...
        with self._lock:
            if self._failed or current < self._high_water:
                ...
                raise ApprovalInboxV7Unavailable("local-monotonic-clock-rollback")
    ...
    def advance(self, milliseconds: int) -> None:
        ...
        with self._lock:
            if self._current > MAX_INTEGER - delta:
                raise ApprovalInboxV7ContractError("clock advance overflows")
            self._current += delta
...
class HostInboxSource:
    ...
    def __init__(
        ...
        if not callable(callback):
            raise ApprovalInboxV7ContractError("host source callback is invalid")
    ...
    def capture(self) -> HostInboxSnapshot:
        if not self._semaphore.acquire(blocking=False):
            raise ApprovalInboxV7Unavailable("host-source-busy") from None
        failed = False
        result: object = None
        try:
            try:
        ...
        if failed:
            raise ApprovalInboxV7Unavailable("host-source-failed") from None
        if type(result) is not HostInboxSnapshot:
            raise ApprovalInboxV7ContractError("host snapshot concrete type is invalid")
        items = result.items
        if type(items) is not tuple:
            raise ApprovalInboxV7ContractError("host items must be an exact tuple")
        if len(items) > MAX_ITEMS:
            raise ApprovalInboxV7ContractError("host item collection exceeds its bound")
        if any(type(item) is not HostInboxItem for item in items):
            raise ApprovalInboxV7ContractError("host item concrete type is invalid")
        return result
...
class ApprovalDisplayItem:
    ...
    def _initialize(self, source: HostInboxItem) -> None:
        ...
        if created >= expires:
            raise ApprovalInboxV7ContractError("source display timestamps are invalid")
        if type(source.always_explicit) is not bool or type(source.batch_eligible) is not bool:
            raise ApprovalInboxV7ContractError("host item booleans are invalid")
        if source.batch_eligible and (
            source.always_explicit or risk in {"high", "critical"}
        ):
            raise ApprovalInboxV7ContractError(
        ...
        if type(currency) is not str or not _CURRENCY.fullmatch(currency):
            raise ApprovalInboxV7ContractError("currency is invalid")
        ...
        payload = {
            "contract": "ApprovalDisplayItem.v7",
...
def _review_digest(item: ApprovalDisplayItem | BatchPreviewItem) -> str:
    if type(item) is ApprovalDisplayItem:
        ...
        raise ApprovalInboxV7ContractError("review digest item type is invalid")
    return _sha(
        {
            "contract": "ApprovalHumanReview.v7",
...
def _build_result_factories() -> tuple[Callable[..., object], ...]:
    ...
    def make_review_item(
        ...
        if type(item) is not ApprovalDisplayItem or item.review_token is not None:
            raise ApprovalInboxV7ContractError("review item source is invalid")
        if type(review_token) is not str or not review_token:
            raise ApprovalInboxV7ContractError("review item token is invalid")
        ...
        *,
        ...
        if status == "disabled":
            if any((snapshot, view, items, offset, snapshot_token, next_cursor)):
                raise ApprovalInboxV7ContractError("disabled page is contradictory")
            values: dict[str, object] = {
                "status": "disabled",
                "snapshot_version": 0,
            ...
            if type(snapshot) is not _Snapshot or type(view) is not _View:
                raise ApprovalInboxV7ContractError("ready page lacks exact snapshot/view")
            if view.snapshot_digest != snapshot.snapshot_digest or type(view.query) is not InboxQuery:
                raise ApprovalInboxV7ContractError("ready page snapshot/view relation is invalid")
            if (
                ...
                raise ApprovalInboxV7ContractError("ready page view bounds/set is invalid")
            derived_view_digest = _canonical_view_digest(snapshot, view)
            if not hmac.compare_digest(view.view_digest, derived_view_digest):
                raise ApprovalInboxV7ContractError("ready page view digest is not canonical")
            if type(items) is not tuple or any(type(item) is not ApprovalDisplayItem for item in items):
                raise ApprovalInboxV7ContractError("ready page items are invalid")
            ...
            if not len(items) <= view.page_size or offset + len(items) > total:
                raise ApprovalInboxV7ContractError("page bounds are contradictory")
            expected_ids = view.allowed_item_ids[offset : offset + len(items)]
            if tuple(item.item_id for item in items) != expected_ids:
                raise ApprovalInboxV7ContractError("page items do not match the bound view")
            ...
            if any(item_id not in snapshot_items for item_id in view.allowed_item_ids) or any(
                ...
                raise ApprovalInboxV7ContractError("page item digests do not match the snapshot")
            if type(snapshot_token) is not str or not snapshot_token:
                raise ApprovalInboxV7ContractError("ready page lacks an authenticated token")
            ...
            if projection._decode(snapshot_token, "snapshot") != expected_snapshot_payload:
                raise ApprovalInboxV7ContractError("ready page snapshot token is contradictory")
            next_offset = offset + len(items)
            has_remaining = next_offset < total
            if has_remaining:
                if type(next_cursor) is not str or not next_cursor:
                    raise ApprovalInboxV7ContractError("ready page lacks its next cursor")
                expected_cursor_payload = expected_snapshot_payload | {
                    "k": "cursor",
                    "o": next_offset,
                }
                if projection._decode(next_cursor, "cursor") != expected_cursor_payload:
                    raise ApprovalInboxV7ContractError("ready page cursor is contradictory")
            elif next_cursor is not None:
                raise ApprovalInboxV7ContractError("terminal page exposes a cursor")
            ...
            raise ApprovalInboxV7ContractError("page status is invalid")
    ...
    def make_batch_preview(
        ...
        if type(projection) is not ApprovalInboxProjection:
            raise TypeError("batch previews are created by the projection")
        if type(snapshot) is not _Snapshot or type(view) is not _View:
            raise ApprovalInboxV7ContractError("batch preview lacks exact snapshot/view")
        if view.snapshot_digest != snapshot.snapshot_digest or type(view.query) is not InboxQuery:
            raise ApprovalInboxV7ContractError("batch snapshot/view relation is invalid")
        derived_view_digest = _canonical_view_digest(snapshot, view)
        if not hmac.compare_digest(view.view_digest, derived_view_digest):
            raise ApprovalInboxV7ContractError("batch view digest is not canonical")
        expected_snapshot_payload = {
            "d": snapshot.snapshot_digest,
            "k": "snapshot",
            "q": view.query.payload(),
        ...
        if projection._decode(snapshot_token, "snapshot") != expected_snapshot_payload:
            raise ApprovalInboxV7ContractError("batch snapshot token is contradictory")
        if type(items) is not tuple or not 1 <= len(items) <= MAX_BATCH_ITEMS:
            raise ApprovalInboxV7ContractError("batch preview item count is invalid")
        if any(
            ...
            raise ApprovalInboxV7ContractError("batch preview items are invalid")
        ...
        if ordered != items or len({item.item_id for item, _token in items}) != len(items):
            raise ApprovalInboxV7ContractError("batch preview ordering/set is invalid")
        ...
        for item, review_token in items:
            if item.item_id not in allowed:
                raise ApprovalInboxV7ContractError("batch item is hidden from the bound view")
            if item.item_id not in snapshot_items or not hmac.compare_digest(
                item.item_digest, snapshot_items[item.item_id]
            ):
                raise ApprovalInboxV7ContractError("batch item digest is not in the snapshot")
            if not item.batch_eligible or item.always_explicit or item.risk in {"high", "critical"}:
                raise ApprovalInboxV7ContractError("batch item is ineligible")
            if projection._decode(review_token, "review") != projection._review_payload(
                snapshot, view, item
            ):
                raise ApprovalInboxV7ContractError("batch review token is contradictory")
...
class ApprovalInboxProjection:
    ...
    def __init__(
        ...
        if type(source) is not HostInboxSource:
            raise ApprovalInboxV7ContractError("exact HostInboxSource is required")
        if type(clock) not in {MonotonicInboxClock, DeterministicInboxClock}:
            raise ApprovalInboxV7ContractError("exact concrete inbox clock is required")
        self._source = source
        self._clock = clock
    ...
    def _assert_integrity(self) -> None:
        with self._lock:
            if self._integrity_failed:
                raise ApprovalInboxV7Unavailable("projection-integrity-failed")
    ...
    def _latch_integrity(self, reason: str) -> None:
        ...
        raise ApprovalInboxV7Stale(reason)
    ...
    def _materialize(self, source: HostInboxSnapshot, local_now: int) -> _Snapshot:
        if type(source.schema_version) is not int or source.schema_version != INBOX_SCHEMA_VERSION:
            raise ApprovalInboxV7ContractError("unknown host snapshot schema")
        ...
        if principal != self._source.principal_id or session != self._source.session_id:
            raise ApprovalInboxV7Unavailable("host-principal-session-drift")
        ...
        if any(
            ...
            raise ApprovalInboxV7ContractError("host safety values are invalid")
        if not source.audit_healthy:
            raise ApprovalInboxV7Unavailable("host-audit-unhealthy")
        if not source.session_active:
            raise ApprovalInboxV7Unavailable("host-session-inactive")
        if source.kill_switch:
            raise ApprovalInboxV7Unavailable("host-kill-active")
        ...
        if len({item.item_id for item in items}) != len(items):
            raise ApprovalInboxV7ContractError("host item IDs are duplicated")
        if len({item.item_digest for item in items}) != len(items):
            raise ApprovalInboxV7ContractError("host item digests are duplicated")
        if len({item.idempotency_key for item in items}) != len(items):
            raise ApprovalInboxV7ContractError("host idempotency keys are duplicated")
        semantic = {
            "contract": "ApprovalInboxSnapshot.v7",
            "schema_version": SNAPSHOT_SCHEMA_VERSION,
        ...
        if local_now > MAX_INTEGER - MAX_SNAPSHOT_AGE_MS:
            raise ApprovalInboxV7Unavailable("local-monotonic-deadline-overflow")
        source_digest = _sha(semantic)
        deadline = local_now + MAX_SNAPSHOT_AGE_MS
        snapshot_digest = _sha(
            {
                "contract": "LocalApprovalReviewSnapshot.v7",
    ...
    def _query_from_payload(value: object) -> InboxQuery:
        if type(value) is not dict or frozenset(value) != {
            ...
            raise ApprovalInboxV7ContractError("token query shape is invalid")
        risks = value["risks"]
        if type(risks) is not list:
            raise ApprovalInboxV7ContractError("token risk filter is invalid")
        return InboxQuery(
    ...
    def _validate_token_payload(cls, value: object, kind: str) -> dict[str, object]:
        ...
        if type(value) is not dict or set(value) != expected:
            raise ApprovalInboxV7ContractError("token payload shape is invalid")
        if value["v"] != TOKEN_SCHEMA_VERSION or value["k"] != kind:
            raise ApprovalInboxV7ContractError("token payload schema is invalid")
        ...
        if query.payload() != value["q"]:
            raise ApprovalInboxV7ContractError("token query is noncanonical")
        return value
    ...
    def _encode(self, payload: dict[str, object], kind: str) -> str:
        ...
        if not body or not _TOKEN_BODY.fullmatch(body):
            raise ApprovalInboxV7ContractError("token body encoding is invalid")
        ...
        if len(token) > MAX_TOKEN:
            raise ApprovalInboxV7ContractError("token exceeds its bound")
        return token
    ...
    def _decode(self, token: object, kind: str) -> dict[str, object]:
        ...
        if failed or type(payload) is not dict:
            raise ApprovalInboxV7Stale("invalid-authenticated-review-token") from None
        return payload
    ...
    def _lookup(self, payload: dict[str, object], local_now: int) -> tuple[_Snapshot, _View]:
        ...
        if type(snapshot) is not _Snapshot or type(view) is not _View:
            raise ApprovalInboxV7Stale("review-snapshot-or-view-not-available")
        if view.snapshot_digest != snapshot.snapshot_digest:
            raise ApprovalInboxV7Stale("review-view-snapshot-mismatch")
        if view.page_size != payload["z"] or view.query.payload() != payload["q"]:
            raise ApprovalInboxV7Stale("review-token-view-mismatch")
        if local_now >= snapshot.local_deadline_ms or local_now >= view.local_deadline_ms:
            raise ApprovalInboxV7Stale("local-review-deadline-expired")
    ...
    def _refresh_exact(self, pinned: _Snapshot, local_now: int) -> None:
        ...
        if (
            ...
            raise ApprovalInboxV7Stale("host-source-drifted-from-review-snapshot")
    ...
    def _items_for_view(snapshot: _Snapshot, view: _View) -> tuple[ApprovalDisplayItem, ...]:
        ...
        if any(item_id not in by_id for item_id in view.allowed_item_ids):
            raise ApprovalInboxV7Stale("review-view-item-set-drifted")
    ...
    def _page(self, snapshot: _Snapshot, view: _View, offset: int) -> InboxPage:
        with self._lock:
            ...
            if type(returned) is not dict:
                raise ApprovalInboxV7Stale("review-view-return-state-missing")
            for item in page.items:
                if type(item.review_token) is not str:
                    raise ApprovalInboxV7ContractError("page item lacks review token")
    ...
    def open_page(
        ...
        *,
        ...
        if type(query) is not InboxQuery:
            raise ApprovalInboxV7ContractError("exact InboxQuery is required")
    ...
    def preview_calm_batch(
        ...
        *,
        ...
        if not approval_inbox_enabled(environ):
            raise ApprovalInboxV7Disabled(f"{APPROVAL_INBOX_FLAG} is disabled")
        self._assert_integrity()
        payload = self._decode(snapshot_token, "snapshot")
        if type(selections) not in {tuple, list}:
            raise ApprovalInboxV7ContractError("batch selection must be an exact finite sequence")
        if not 1 <= len(selections) <= MAX_BATCH_ITEMS:
            raise ApprovalInboxV7ContractError("batch selection exceeds its bound")
        ...
        if len({item_id for item_id, _token in selected_pairs}) != len(selected_pairs):
            raise ApprovalInboxV7ContractError("batch selection contains duplicates")
        local_now = self._clock.now_ms()
        snapshot, view = self._lookup(payload, local_now)
        self._refresh_exact(snapshot, local_now)
        allowed = set(view.allowed_item_ids)
        ...
        if selected_ids - allowed:
            raise ApprovalInboxV7ContractError("batch selection is outside the bound view")
        by_id = {item.item_id: item for item in snapshot.items}
        with self._lock:
            if self._integrity_failed:
                raise ApprovalInboxV7Unavailable("projection-integrity-failed")
...
__all__ = [
    ...
    "ApprovalInboxV7ContractError",
    "ApprovalInboxV7Disabled",
    "ApprovalInboxV7Error",
    "ApprovalInboxV7Stale",
    "ApprovalInboxV7Unavailable",
    "BatchPreviewItem",
    "CalmBatchPreview",
