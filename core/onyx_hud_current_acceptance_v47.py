"""Authenticate non-visual shortcut recovery over immutable HUD V46."""
...
PREDECESSOR_ACCEPTANCE: Final = Path("core/onyx_hud_current_acceptance_v46.py")
PREDECESSOR_ACCEPTANCE_SHA256: Final = (
    "e630c76b40860af754307c40a5582edc5a67f72f38affd5e16b99260db9aa85f"
)
...
class CurrentHudAcceptanceV47Error(RuntimeError):
    """The V47 shortcut delta or immutable V46 authority drifted."""
...
def _file(root: Path, relative: object) -> Path:
    ...
    if (
        ...
        raise CurrentHudAcceptanceV47Error("noncanonical V47 input path")
    path = root.joinpath(*pure.parts)
    try:
        resolved = path.resolve(strict=True)
        resolved.relative_to(root)
    except (OSError, ValueError) as error:
        raise CurrentHudAcceptanceV47Error("V47 input unavailable") from error
    if path.is_symlink() or resolved != path.absolute() or not resolved.is_file():
        raise CurrentHudAcceptanceV47Error("V47 input is linked")
    return resolved
...
def verify_current_hud_acceptance(project: Path) -> dict[str, object]:
    ...
    for relative, digest in (
        ...
        if _sha256(_file(root, relative.as_posix())) != digest:
            raise CurrentHudAcceptanceV47Error("immutable V46 predecessor drifted")
    manifest_path = _file(root, MANIFEST_RELATIVE.as_posix())
    if _sha256(manifest_path) != MANIFEST_SHA256:
        raise CurrentHudAcceptanceV47Error("V47 manifest drifted")
    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    except (UnicodeError, json.JSONDecodeError) as error:
        raise CurrentHudAcceptanceV47Error("V47 manifest invalid") from error
    semantics = {
    ...
    if (
        manifest.get("schema") != "onyx.hud.current.v47.acceptance.v1"
        ...
        raise CurrentHudAcceptanceV47Error("V47 manifest contract drifted")
    for entry in entries:
        if type(entry) is not dict or set(entry) != {"path", "sha256"}:
            raise CurrentHudAcceptanceV47Error("V47 input binding drifted")
        if _sha256(_file(root, entry["path"])) != entry["sha256"]:
            raise CurrentHudAcceptanceV47Error(
                f"V47 source input drifted: {entry['path']}"
            )
    return {
