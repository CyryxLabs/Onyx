...
class CurrentHudAcceptanceV29Error(RuntimeError):
    """The V29 source closure or its immutable V28 predecessor drifted."""
...
PREDECESSOR_MANIFEST_SHA256: Final = (
    "64700fc51c5d12bafdaa5d26dcbfad6d2ebf759735f1805776721aa289d7f200"
)
PREDECESSOR_ACCEPTANCE_RELATIVE: Final = Path(
    "core/onyx_hud_current_acceptance_v28.py"
)
PREDECESSOR_ACCEPTANCE_SHA256: Final = (
    "40ea1ec528b5e2cc0711e5f344745155b2db82581606dc85861346cad4aaa170"
)
...
CURRENT_RUNTIME_PATHS: Final = (
    ...
    Path("scripts/generate_hud_v29_manifests.py"),
    ...
    Path("tests/test_onyx_hud_current_acceptance_v29.py"),
)
...
def _canonical_file(root: Path, relative: Path) -> Path:
    ...
    if pure.is_absolute() or ".." in pure.parts or pure.as_posix() != relative.as_posix():
        raise CurrentHudAcceptanceV29Error("noncanonical V29 source path")
    candidate = root.joinpath(*pure.parts)
    if candidate.is_symlink() or not candidate.is_file():
        raise CurrentHudAcceptanceV29Error(f"V29 source input unavailable: {relative}")
    try:
        resolved = candidate.resolve(strict=True)
        resolved.relative_to(root)
    except (OSError, ValueError) as exc:
        raise CurrentHudAcceptanceV29Error(
            f"V29 source input escapes root: {relative}"
        ) from exc
    if resolved != candidate.absolute():
        raise CurrentHudAcceptanceV29Error(f"V29 source input is linked: {relative}")
    return resolved
...
def _strict_json(path: Path) -> dict[str, object]:
    def pairs(values: list[tuple[str, object]]) -> dict[str, object]:
        ...
        for key, value in values:
            if key in result:
                raise CurrentHudAcceptanceV29Error("duplicate V29 manifest key")
            result[key] = value
        return result
    ...
    try:
        ...
        raise CurrentHudAcceptanceV29Error("V29 manifest unreadable") from exc
    if type(value) is not dict:
        raise CurrentHudAcceptanceV29Error("V29 manifest must be an object")
    return value
...
def verify_current_hud_acceptance(project: Path) -> dict[str, object]:
    ...
    if (
        ...
        or manifest.get("schema") != "onyx.hud.current.v29.acceptance.v1"
        or manifest.get("candidate")
        != "onyx-hud-v10-qml-teardown-guard-011"
        ...
        raise CurrentHudAcceptanceV29Error("V29 manifest contract drift")
    entries = manifest["runtime_inputs"]
    if type(entries) is not list or len(entries) != len(CURRENT_RUNTIME_PATHS):
        raise CurrentHudAcceptanceV29Error("V29 source membership drift")
    ...
    for relative, entry in zip(CURRENT_RUNTIME_PATHS, entries, strict=True):
        ...
        if _sha256(_canonical_file(root, relative)) != entry["sha256"]:
            raise CurrentHudAcceptanceV29Error(f"V29 source input drift: {relative}")
        records.append(entry)
    artifact_root = _artifact_root(records)
    if manifest.get("artifact_root_sha256") != artifact_root:
        raise CurrentHudAcceptanceV29Error("V29 artifact root drift")
    for relative, expected in (
        ...
        if _sha256(_canonical_file(root, relative)) != expected:
            raise CurrentHudAcceptanceV29Error(
                f"immutable V28 predecessor drift: {relative}"
            )
    anchors = (
        (Path("ui.py"), "core.onyx_hud_current_acceptance_v29"),
        (Path("ui.py"), "verify_packaged_runtime_hud_contract(root)"),
        (Path("qml/OnyxLiveShellV7.qml"), "readonly property bool projectionAvailable"),
        (Path("qml/OnyxLiveShellV7.qml"), "function requestWithValue(name, value)"),
        (Path("qml/components/OnyxOrbEntityV7.qml"), "opacity: !projection ||"),
    ...
    for relative, anchor in anchors:
        if anchor not in _canonical_file(root, relative).read_text(encoding="utf-8"):
            raise CurrentHudAcceptanceV29Error(f"V29 wiring drift: {relative}")
    return {
