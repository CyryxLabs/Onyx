"""Phase 5.3 Capability Nexus V21: isolated, closed-provenance contracts.

V21 supersedes the rejected V1-V13 candidates without importing them. Descriptor
...
CAPABILITY_SCHEMA_VERSION_V21 = "onyx.capability.v21"
NEXUS_CONTRACT_VERSION_V21 = "onyx.capability-nexus.v21"
CATALOG_CONTRACT_VERSION_V21 = "onyx.local-catalog.v21"
LOCK_ORDER_V21 = ("gate_immutable_state", "registry_lock", "adapter_lock")


class CapabilityV21ContractError(ValueError):
    pass


class CapabilityV21DeniedError(PermissionError):
    pass


class OperationKindV21(str, Enum):
    READ = "read"
    DRAFT = "draft"
    MUTATE = "mutate"
    VERIFY = "verify"
...
class CapabilityStatusV21(str, Enum):
    DISABLED = "disabled"
...
class TransportKindV21(str, Enum):
    LEGACY = "legacy"
    LOCAL = "local"
    API = "api"
    CLI = "cli"
...
class ReadFailureClassV21(str, Enum):
    NONE = "none"
    AUTH = "auth"
    SCOPE = "scope"
    DISABLED = "disabled"
...
def _scan_content_representation(value: str, *, allow_typed_hash: bool = False) -> None:
    ...
    if any(
        ...
        raise CapabilityV21ContractError("content contains unsupported invisible/bidi/combining characters")
    ...
    if (
        ...
        raise CapabilityV21ContractError("content representation contains secret material")
    if _RESIDUAL_UNICODE_ESCAPE.search(normalized):
        raise CapabilityV21ContractError("content contains residual character encoding")
...
def _canonical_percent_decode(value: str) -> str | None:
    ...
    if _PERCENT.sub("", value).find("%") >= 0:
        raise CapabilityV21ContractError("metadata contains malformed percent encoding")
    try:
        decoded = unquote_to_bytes(value).decode("utf-8", errors="strict")
    except (UnicodeDecodeError, ValueError) as exc:
        raise CapabilityV21ContractError("metadata percent encoding is invalid") from exc
    ...
    if normalize_hex(canonical) != normalize_hex(value):
        raise CapabilityV21ContractError("metadata percent encoding is noncanonical")
...
def _canonical_base64_decode(value: str) -> str | None:
    ...
    if ("+" in compact or "/" in compact) and ("-" in compact or "_" in compact):
        raise CapabilityV21ContractError("metadata Base64 alphabet is ambiguous")
    ...
    if canonical.rstrip("=") != compact.rstrip("="):
        raise CapabilityV21ContractError("metadata Base64 encoding is noncanonical")
...
def _validate_content_decoding_closure(value: str, *, allow_typed_hash: bool = False) -> None:
    ...
    if encoded_size > _MAX_METADATA_VALUE_BYTES:
        raise CapabilityV21ContractError("content value exceeds decoding byte budget")
    ...
    while queue:
        ...
        if current in ancestors:
            raise CapabilityV21ContractError("content decoding cycle detected")
        seen.add(current)
        if len(seen) > _MAX_METADATA_REPRESENTATIONS:
            raise CapabilityV21ContractError("content decoding representation budget exceeded")
        total_bytes += len(current.encode("utf-8"))
        if total_bytes > _MAX_METADATA_TOTAL_BYTES:
            raise CapabilityV21ContractError("content decoding expansion budget exceeded")
        ...
        if depth >= _MAX_METADATA_DECODE_DEPTH and candidates:
            raise CapabilityV21ContractError("content decoding depth exceeded")
...
def _text(value: object, label: str, maximum: int, *, empty: bool = False) -> str:
    if type(value) is not str or value != value.strip() or (not value and not empty):
        raise CapabilityV21ContractError(f"{label} must be a canonical exact string")
    if len(value) > maximum or any(
        (ord(ch) < 32 and ch not in {"\t", "\n", "\r"}) or ord(ch) == 127 for ch in value
    ):
        raise CapabilityV21ContractError(f"{label} is out of bounds")
    if _secret_canary(value):
        raise CapabilityV21ContractError(f"{label} resembles secret material")
    return value
...
def _slug(value: object, label: str) -> str:
    ...
    if not _SLUG.fullmatch(result):
        raise CapabilityV21ContractError(f"{label} is not a canonical slug")
    return result
...
def _identifier(value: object, label: str) -> str:
    ...
    if not _IDENT.fullmatch(result):
        raise CapabilityV21ContractError(f"{label} is not a canonical identifier")
    return result
...
def _strings(value: object, label: str, *, count: int = 32, size: int = 128, required: bool = False) -> tuple[str, ...]:
    if type(value) not in {tuple, list} or len(value) > count or (required and not value):
        raise CapabilityV21ContractError(f"{label} cardinality is invalid")
    result = tuple(_text(item, f"{label} item", size) for item in value)
    if len(set(result)) != len(result):
        raise CapabilityV21ContractError(f"{label} contains duplicates")
...
def _reject_json_secrets(
    ...
    *,
    ...
    if _depth > _MAX_JSON_DEPTH:
        raise CapabilityV21ContractError(f"{label} exceeds nesting depth")
    active = set() if _active is None else _active
    budget = [0, 0] if _budget is None else _budget
    budget[0] += 1
    if budget[0] > _MAX_JSON_NODES:
        raise CapabilityV21ContractError(f"{label} exceeds node budget")
    if isinstance(value, Mapping):
        if len(value) > 512:
            raise CapabilityV21ContractError(f"{label} is too large")
        identity = id(value)
        if identity in active:
            raise CapabilityV21ContractError(f"{label} contains a cycle")
        active.add(identity)
        try:
            for key, item in value.items():
                key = _text(key, f"{label} key", 128)
                if not _JSON_FIELD.fullmatch(key) or not key.isascii():
                    raise CapabilityV21ContractError(f"{label} key is not a canonical ASCII identifier")
                if _normalized_name(key) in _SECRET_NAMES:
                    raise CapabilityV21ContractError(f"{label} contains a secret-name field")
        ...
        if budget[1] > _MAX_JSON_SCALAR_BYTES:
            raise CapabilityV21ContractError(f"{label} exceeds scalar byte budget")
        _validate_content_decoding_closure(item)
    elif value is not None:
        if type(value) not in {bool, int, float}:
            raise CapabilityV21ContractError(f"{label} contains an unsupported value")
        if type(value) is float and not math.isfinite(value):
            raise CapabilityV21ContractError(f"{label} contains a non-finite number")
        if type(value) is int and not -(2**63) <= value <= 2**63 - 1:
            raise CapabilityV21ContractError(f"{label} integer is out of bounds")
...
def _json(value: object, label: str, maximum: int = 65536) -> str:
    ...
    try:
        ...
        raise CapabilityV21ContractError(f"{label} is not canonical JSON") from exc
    if len(result.encode("utf-8")) > maximum:
        raise CapabilityV21ContractError(f"{label} exceeds its byte budget")
    return result
...
def _safe_metadata(value: object) -> tuple[tuple[str, str], ...]:
    if type(value) not in {tuple, list} or len(value) > len(_SAFE_METADATA_KEYS):
        raise CapabilityV21ContractError("metadata must be bounded key/value pairs")
    result: list[tuple[str, str]] = []
    for pair in value:
        if type(pair) not in {tuple, list} or len(pair) != 2:
            raise CapabilityV21ContractError("metadata entry is invalid")
        key = pair[0]
        if type(key) is not str or not key.isascii() or key not in _SAFE_METADATA_KEYS:
            raise CapabilityV21ContractError("metadata key is not declared safe")
        item = pair[1]
        if type(item) is not str or not item.isascii() or not item or item != item.strip() or len(item) > 512:
            raise CapabilityV21ContractError(f"metadata {key} is not an exact ASCII atom")
        if key == "declaration_sha256":
            if not _HEX64.fullmatch(item):
                raise CapabilityV21ContractError("declaration_sha256 is invalid")
        elif item not in _METADATA_ENUMS[key]:
            raise CapabilityV21ContractError(f"metadata {key} is outside its closed enum")
        result.append((key, item))
    if len({key for key, _ in result}) != len(result):
        raise CapabilityV21ContractError("metadata key is duplicated")
    return tuple(sorted(result))


@dataclass(frozen=True, slots=True)
class OperationDescriptorV21:
    operation_id: str
    kind: OperationKindV21
    description: str
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.kind) is not OperationKindV21:
            raise CapabilityV21ContractError("operation kind must be exact")
        ...
        if type(self.parameter_schema_json) is not str:
            raise CapabilityV21ContractError("parameter schema must be exact string")
        try:
            parsed = json.loads(self.parameter_schema_json)
        except json.JSONDecodeError as exc:
            raise CapabilityV21ContractError("parameter schema is invalid") from exc
        if _json(parsed, "parameter schema") != self.parameter_schema_json:
            raise CapabilityV21ContractError("parameter schema is noncanonical")
        ...
        if any(not _DOMAIN.fullmatch(item) or ".." in item for item in domains):
            raise CapabilityV21ContractError("allowed domain is invalid")
...
@dataclass(frozen=True, slots=True)
class CapabilityDescriptorV21:
    capability_id: str
    capability_version: str
    provider: str
    transport: TransportKindV21
    api_name: str
    api_version: str
    workspace_id: str
    account_id: str
    ...
    operations: tuple[OperationDescriptorV21, ...]
    status: CapabilityStatusV21 = CapabilityStatusV21.DISABLED
    status_reason: str = "candidate_not_activated"
    limitations: tuple[str, ...] = ()
    license_review: str = "not_required_local_only"
    metadata: tuple[tuple[str, str], ...] = ()
    schema_version: str = CAPABILITY_SCHEMA_VERSION_V21

    def __post_init__(self) -> None:
        if self.schema_version != CAPABILITY_SCHEMA_VERSION_V21:
            raise CapabilityV21ContractError("unknown descriptor schema")
        for name in ("capability_id", "capability_version", "provider", "api_name", "api_version"):
            object.__setattr__(self, name, _slug(getattr(self, name), name))
        if type(self.transport) is not TransportKindV21 or type(self.status) is not CapabilityStatusV21:
            raise CapabilityV21ContractError("descriptor enum type is invalid")
        ...
        if self.credential_alias is not None and (
            ...
            raise CapabilityV21ContractError("credential_alias must be opaque")
        if type(self.operations) not in {tuple, list} or not self.operations or len(self.operations) > 64:
            raise CapabilityV21ContractError("operation cardinality is invalid")
        operations = tuple(self.operations)
        if any(type(item) is not OperationDescriptorV21 for item in operations):
            raise CapabilityV21ContractError("operation descriptor type is invalid")
        if len({item.operation_id for item in operations}) != len(operations):
            raise CapabilityV21ContractError("duplicate operation")
        ...
        if self.status is CapabilityStatusV21.DEGRADED and self.status_reason == "healthy":
            raise CapabilityV21ContractError("degraded descriptor requires a reason")
        if self.transport is TransportKindV21.BROWSER and dict(self.metadata).get("fallback_class") != "explicit_browser_fallback":
            raise CapabilityV21ContractError("browser fallback is not explicit")
...
@dataclass(frozen=True, slots=True)
class NexusFeatureGateV21:
    nexus_enabled: bool = False
    shadow_mode: bool = True
    dispatch_enabled: bool = False
    contract_version: str = NEXUS_CONTRACT_VERSION_V21

    def __post_init__(self) -> None:
        if type(self.nexus_enabled) is not bool or type(self.shadow_mode) is not bool or type(self.dispatch_enabled) is not bool:
            raise CapabilityV21ContractError("gate values must be exact booleans")
        if self.contract_version != NEXUS_CONTRACT_VERSION_V21:
            raise CapabilityV21ContractError("gate contract version is invalid")
        if self.nexus_enabled or not self.shadow_mode or self.dispatch_enabled:
            raise CapabilityV21ContractError("V21 is strictly default-off and shadow-only")


@dataclass(frozen=True, slots=True)
class CapabilityProjectionV21:
    descriptor: CapabilityDescriptorV21
    descriptor_digest: str
    workspace_id: str
    account_id: str
    profile_id: str
    projection_status: CapabilityStatusV21
    projection_reason: str
    runtime_available: bool = False
    authority_granted: bool = False

    def __post_init__(self) -> None:
        if type(self.descriptor) is not CapabilityDescriptorV21 or self.descriptor_digest != self.descriptor.digest:
            raise CapabilityV21ContractError("projection descriptor binding drift")
        if (
            ...
            raise CapabilityV21ContractError("projection identity binding drift")
        if type(self.projection_status) is not CapabilityStatusV21:
            raise CapabilityV21ContractError("projection status is invalid")
        ...
        if type(self.runtime_available) is not bool or type(self.authority_granted) is not bool:
            raise CapabilityV21ContractError("projection booleans must be exact")
        if self.runtime_available or self.authority_granted:
            raise CapabilityV21ContractError("V21 projections cannot convey authority")
...
@dataclass(frozen=True, slots=True)
class CapabilitySnapshotV21:
    workspace_id: str
    account_id: str
    profile_id: str
    entries: tuple[CapabilityProjectionV21, ...]
    snapshot_digest: str
    contract_version: str = NEXUS_CONTRACT_VERSION_V21

    def __post_init__(self) -> None:
        if self.contract_version != NEXUS_CONTRACT_VERSION_V21:
            raise CapabilityV21ContractError("snapshot contract version is invalid")
        for name in ("workspace_id", "account_id", "profile_id"):
            object.__setattr__(self, name, _identifier(getattr(self, name), name))
        if type(self.entries) is not tuple or any(type(item) is not CapabilityProjectionV21 for item in self.entries):
            raise CapabilityV21ContractError("snapshot entries are invalid")
        if any(
            ...
            raise CapabilityV21ContractError("snapshot contains a cross-profile projection")
        expected = _sha(
            {
                "account_id": self.account_id,
        ...
        if self.snapshot_digest != expected:
            raise CapabilityV21ContractError("snapshot digest is invalid")


class CapabilityNexusV21:
    """Descriptor-only registry; lock order is gate immutable -> registry."""

    def __init__(self, gate: NexusFeatureGateV21 | None = None) -> None:
        if gate is None:
            gate = NexusFeatureGateV21()
        if type(gate) is not NexusFeatureGateV21:
            raise CapabilityV21ContractError("registry requires the exact concrete V21 gate")
        self._gate = gate
        self._descriptors: dict[str, CapabilityDescriptorV21] = {}
        self._revoked_aliases: set[str] = set()
        self._kill = False
        self._lock = threading.RLock()

    @property
    def gate(self) -> NexusFeatureGateV21:
        return self._gate

    @property
    def lock_order(self) -> tuple[str, ...]:
        return LOCK_ORDER_V21

    def register(self, descriptor: CapabilityDescriptorV21) -> str:
        if type(descriptor) is not CapabilityDescriptorV21:
            raise CapabilityV21ContractError("registry descriptor type is invalid")
        with self._lock:
            if descriptor.capability_id in self._descriptors:
                raise CapabilityV21ContractError("duplicate capability")
    ...
    def revoke_alias(self, alias: str) -> None:
        if type(alias) is not str or not _ALIAS.fullmatch(alias):
            raise CapabilityV21ContractError("credential alias is invalid")
        with self._lock:
            self._revoked_aliases.add(alias)

    def set_kill(self, active: bool) -> None:
        if type(active) is not bool:
            raise CapabilityV21ContractError("kill state must be exact boolean")
        with self._lock:
            self._kill = active
    ...
    def discover(
        ...
        *,
        ...
        if not _HEX64.fullmatch(expected_digest):
            raise CapabilityV21ContractError("expected digest is invalid")
        with self._lock:
            descriptor = self._descriptors.get(capability_id)
            if descriptor is None:
                raise CapabilityV21DeniedError("unknown capability")
            if (
                descriptor.workspace_id != workspace_id
                or descriptor.account_id != account_id
                or descriptor.profile_id != profile_id
            ):
                raise CapabilityV21DeniedError("workspace/account/profile mismatch")
            if descriptor.capability_version != expected_version or descriptor.digest != expected_digest:
                raise CapabilityV21DeniedError("version or descriptor drift")
            status, reason = self._status(descriptor)
            return CapabilityProjectionV21(
                descriptor, descriptor.digest, workspace_id, account_id, profile_id, status, reason
            )

    def health_projection(self, *args: object, **kwargs: object) -> CapabilityProjectionV21:
        return self.discover(*args, **kwargs)

    def snapshot(self, *, workspace_id: str, account_id: str, profile_id: str) -> CapabilitySnapshotV21:
        workspace_id = _identifier(workspace_id, "workspace_id")
        account_id = _identifier(account_id, "account_id")
        profile_id = _identifier(profile_id, "profile_id")
        with self._lock:
        ...
        payload = {
            ...
            "contract_version": NEXUS_CONTRACT_VERSION_V21,
            "entries": [item.digest_payload() for item in entries],
            "profile_id": profile_id,
            "workspace_id": workspace_id,
        }
        return CapabilitySnapshotV21(workspace_id, account_id, profile_id, entries, _sha(payload))

    def _status(self, descriptor: CapabilityDescriptorV21) -> tuple[CapabilityStatusV21, str]:
        if self._kill:
            return CapabilityStatusV21.DISABLED, "global_kill_active"
        if descriptor.credential_alias is not None and descriptor.credential_alias in self._revoked_aliases:
            return CapabilityStatusV21.BLOCKED_BY_ACCESS, "credential_alias_revoked"
        if descriptor.status is CapabilityStatusV21.AVAILABLE_READ_ONLY:
            return CapabilityStatusV21.DISABLED, "nexus_default_off_shadow_only"
        return descriptor.status, descriptor.status_reason


@dataclass(frozen=True, slots=True)
class LegacyParityRecordV21:
    tool_name: str
    host_policy: str
...
@dataclass(frozen=True, slots=True)
class LegacyDescriptorSetV21:
    descriptors: tuple[CapabilityDescriptorV21, ...]
    parity_records: tuple[LegacyParityRecordV21, ...]
    snapshot_digest: str
...
def build_legacy_descriptors_v21(
    ...
    *,
    ...
    account_id: str,
    profile_id: str,
    operation_kinds: Mapping[str, OperationKindV21] | None = None,
) -> LegacyDescriptorSetV21:
    if type(tool_declarations) not in {tuple, list} or not tool_declarations or len(tool_declarations) > 128:
        raise CapabilityV21ContractError("legacy declarations are invalid")
    if not isinstance(policy_mapping, Mapping):
        raise CapabilityV21ContractError("legacy policy mapping is invalid")
    workspace_id = _identifier(workspace_id, "workspace_id")
    account_id = _identifier(account_id, "account_id")
    profile_id = _identifier(profile_id, "profile_id")
    ...
    for raw in tool_declarations:
        if not isinstance(raw, Mapping) or set(raw) != {"name", "description", "parameters"}:
            raise CapabilityV21ContractError("legacy declaration shape drift")
        name = _slug(raw.get("name"), "tool name")
        if name in seen:
            raise CapabilityV21ContractError("duplicate legacy declaration")
        seen.add(name)
        _text(raw.get("description"), "legacy description", 4096)
        if not isinstance(raw.get("parameters"), Mapping):
            raise CapabilityV21ContractError("legacy parameter schema is invalid")
        encoded = _json(dict(raw), "legacy declaration")
        parsed.append((name, raw, encoded))
    if set(policy_mapping) != seen:
        raise CapabilityV21ContractError("legacy policy coverage is not exact")
    if operation_kinds is not None and set(operation_kinds) != seen:
        raise CapabilityV21ContractError("legacy operation coverage is not exact")
    descriptors: list[CapabilityDescriptorV21] = []
    records: list[LegacyParityRecordV21] = []
    for name, raw, encoded in parsed:
        policy = _slug(policy_mapping[name], "host policy")
        kind = OperationKindV21.MUTATE if operation_kinds is None else operation_kinds[name]
        if type(kind) is not OperationKindV21:
            raise CapabilityV21ContractError("legacy operation kind is invalid")
        digest = hashlib.sha256(encoded.encode()).hexdigest()
        operation = OperationDescriptorV21(
            name,
            kind,
            str(raw["description"]),
        ...
        descriptors.append(
            CapabilityDescriptorV21(
                f"legacy.{name}", "v21", "onyx_legacy", TransportKindV21.LEGACY,
                "legacy_dispatcher", "unchanged", workspace_id, account_id, profile_id, None,
                (operation,), CapabilityStatusV21.DISABLED, "shadow_descriptor_only",
        ...
        records.append(LegacyParityRecordV21(name, policy, encoded, digest))
    ...
    return LegacyDescriptorSetV21(tuple(descriptors), tuple(records), _sha(payload))


@dataclass(frozen=True, slots=True)
class CatalogItemV21:
    item_id: str
    label: str
    category: str
    item_version: str
...
class CatalogReadRequestV21:
    ...
    timeout_ms: int = 1000
    cancellation: CancellationTokenV21 | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.page_size) is not int or not 1 <= self.page_size <= 100:
            raise CapabilityV21ContractError("page_size is invalid")
        ...
        if type(self.timeout_ms) is not int or not 1 <= self.timeout_ms <= 30_000:
            raise CapabilityV21ContractError("timeout_ms is invalid")
        if self.cancellation is not None and type(self.cancellation) is not CancellationTokenV21:
            raise CapabilityV21ContractError("cancellation token type is invalid")


@dataclass(frozen=True, slots=True)
class ConnectorHealthV21:
    status: CapabilityStatusV21
    reason: str
    workspace_id: str
    account_id: str
    profile_id: str
...
@dataclass(frozen=True, slots=True)
class CatalogReadProjectionV21:
    state: ReadStateV21
    failure_class: ReadFailureClassV21
    correlation_id: str
    request_digest: str
    workspace_id: str
    account_id: str
    profile_id: str
    items: tuple[CatalogItemV21, ...]
    ...
    def __post_init__(self) -> None:
        if type(self.state) is not ReadStateV21 or type(self.failure_class) is not ReadFailureClassV21:
            raise CapabilityV21ContractError("read projection enum is invalid")
        if self.authority_granted or type(self.authority_granted) is not bool:
            raise CapabilityV21ContractError("read projection cannot grant authority")
        if self.state is not ReadStateV21.COMPLETED and self.items:
            raise CapabilityV21ContractError("non-completed projection cannot expose items")
        if self.state is ReadStateV21.COMPLETED and (
            self.failure_class is not ReadFailureClassV21.NONE or self.receipt_digest is None
        ):
            raise CapabilityV21ContractError("completed projection is incomplete")


@dataclass(frozen=True, slots=True)
class _ReadLedgerEntryV21:
    request_digest: str
    workspace_id: str
    account_id: str
    profile_id: str
...
class LocalCatalogReadAdapterV21:
    ...
    def __init__(
        ...
        *,
        ...
        if type(items) not in {tuple, list} or len(items) > 1000:
            raise CapabilityV21ContractError("catalog items are invalid")
        copied = tuple(items)
        if any(type(item) is not CatalogItemV21 for item in copied) or len({item.item_id for item in copied}) != len(copied):
            raise CapabilityV21ContractError("catalog items must be exact and unique")
        ...
        if type(cursor_signing_key) is not bytes or not 32 <= len(cursor_signing_key) <= 128:
            raise CapabilityV21ContractError("cursor signing key is invalid")
        self._cursor_key = bytes(cursor_signing_key)
        if not callable(read_hook) or not callable(clock):
            raise CapabilityV21ContractError("hook/clock must be callable")
        ...
        if type(auth_available) is not bool:
            raise CapabilityV21ContractError("auth state must be exact boolean")
        self._auth = auth_available
        self._scopes = frozenset(_strings(granted_scopes, "granted_scopes"))
        if type(status) is not CapabilityStatusV21 or status not in {
            CapabilityStatusV21.DISABLED, CapabilityStatusV21.AVAILABLE_READ_ONLY, CapabilityStatusV21.DEGRADED
        }:
            raise CapabilityV21ContractError("adapter status is invalid")
        self._status = status
        self._status_reason = _slug(status_reason, "status_reason")
        if status is CapabilityStatusV21.DEGRADED and status_reason == "healthy":
            raise CapabilityV21ContractError("degraded status requires reason")
        for name, value, lower, upper in (
            ...
            if type(value) is not int or not lower <= value <= upper:
                raise CapabilityV21ContractError(f"{name} is invalid")
        ...
        if pending_capacity > uncertain_capacity or uncertain_capacity > ledger_capacity:
            raise CapabilityV21ContractError("pending/uncertain capacities exceed their parent bound")
        ...
        self._window_used = 0
        self._ledger: dict[str, _ReadLedgerEntryV21] = {}
        self._kill = False
        self._revoked = False
        self._lock = threading.RLock()
    ...
    @property
    def descriptor(self) -> CapabilityDescriptorV21:
        operation = OperationDescriptorV21(
            "catalog_read", OperationKindV21.READ, "Read allowlisted provider-free catalog metadata.",
        ...
        return CapabilityDescriptorV21(
            self.CAPABILITY_ID, "v21", "onyx_local", TransportKindV21.LOCAL, "local_catalog",
            CATALOG_CONTRACT_VERSION_V21, self._workspace_id, self._account_id, self._profile_id,
    ...
    def health(self) -> ConnectorHealthV21:
        now = self._clock()
        with self._lock:
            self._expire_pending(now)
            status, reason, _ = self._effective_status()
            pending = sum(item.state is ReadStateV21.PENDING for item in self._ledger.values())
            uncertain = sum(item.state is ReadStateV21.UNCERTAIN for item in self._ledger.values())
            return ConnectorHealthV21(
                status, reason, self._workspace_id, self._account_id, self._profile_id,
                CATALOG_CONTRACT_VERSION_V21, (self.REQUIRED_SCOPE,), tuple(sorted(self._scopes)),
                max(0, self._quota_limit - self._quota_used), len(self._ledger), pending, uncertain,
            )

    def read_page(self, request: CatalogReadRequestV21) -> CatalogReadProjectionV21:
        with self._hook_state_lock:
            if self._hook_active:
                raise CapabilityV21DeniedError("catalog read reentrancy is denied")
    ...
    def _read_page_claimed(self, request: CatalogReadRequestV21) -> CatalogReadProjectionV21:
        if type(request) is not CatalogReadRequestV21:
            raise CapabilityV21ContractError("read request type is invalid")
        ...
        with self._lock:
            ...
            if prior is not None:
                if prior.request_digest != digest:
                    raise CapabilityV21DeniedError("correlation request drift")
                return self._projection(request.correlation_id, prior)
            if len(self._ledger) >= self._capacity:
                raise CapabilityV21DeniedError("correlation ledger capacity exhausted")
            if request.cancellation is not None and request.cancellation.cancelled:
                entry = self._terminal_without_reservation(
                    request, digest, ReadStateV21.CANCELLED_BEFORE_HOOK, ReadFailureClassV21.CANCELLED, started
                )
            ...
            if status not in {CapabilityStatusV21.AVAILABLE_READ_ONLY, CapabilityStatusV21.DEGRADED}:
                entry = self._terminal_without_reservation(request, digest, ReadStateV21.DENIED, failure, started)
                self._ledger[request.correlation_id] = entry
                return self._projection(request.correlation_id, entry)
            window = int(started)
            ...
            if self._window_used >= self._rate_limit:
                entry = self._terminal_without_reservation(
                    request, digest, ReadStateV21.RATE_LIMITED, ReadFailureClassV21.RATE_LIMIT, started, retry_after_ms=1000
                )
                self._ledger[request.correlation_id] = entry
                return self._projection(request.correlation_id, entry)
            if self._quota_used >= self._quota_limit:
                entry = self._terminal_without_reservation(
                    request, digest, ReadStateV21.DENIED, ReadFailureClassV21.QUOTA, started
                )
                self._ledger[request.correlation_id] = entry
                return self._projection(request.correlation_id, entry)
            pending_count = sum(item.state is ReadStateV21.PENDING for item in self._ledger.values())
            uncertain_count = sum(item.state is ReadStateV21.UNCERTAIN for item in self._ledger.values())
            if pending_count >= self._pending_capacity or pending_count + uncertain_count >= self._uncertain_capacity:
                raise CapabilityV21DeniedError("pending/uncertain ledger capacity exhausted")
            ...
            self._quota_used += 1
            self._window_used += 1
            entry = _ReadLedgerEntryV21(
                digest, self._workspace_id, self._account_id, self._profile_id,
                ReadStateV21.PENDING, ReadFailureClassV21.NONE, page, next_cursor, None, 1,
        ...
        if request.cancellation is not None and request.cancellation.cancelled:
            return self._finish_before_hook(
                request, digest, ReadStateV21.CANCELLED_BEFORE_HOOK, ReadFailureClassV21.CANCELLED, pre_hook_now
            )
        if (pre_hook_now - started) * 1000 >= request.timeout_ms:
            return self._finish_before_hook(
                request, digest, ReadStateV21.TIMEOUT_BEFORE_HOOK, ReadFailureClassV21.TIMEOUT, pre_hook_now
            )
        with self._lock:
            ...
            if status not in {CapabilityStatusV21.AVAILABLE_READ_ONLY, CapabilityStatusV21.DEGRADED}:
                return self._finish_before_hook_locked(
                    request.correlation_id, current, ReadStateV21.DENIED, failure, pre_hook_now
                )
        ...
        if hook_failed:
            ...
            with self._lock:
                current = self._ledger[request.correlation_id]
                if current.state is not ReadStateV21.PENDING:
                    return self._projection(request.correlation_id, current)
                uncertain = replace(
                    current, state=ReadStateV21.UNCERTAIN, failure_class=ReadFailureClassV21.HOOK_EXCEPTION,
        ...
        with self._lock:
            ...
            status, _, failure = self._effective_status()
            if request.cancellation is not None and request.cancellation.cancelled:
                return self._finish_uncertain_locked(request.correlation_id, current, ReadFailureClassV21.CANCELLED, finished)
            if (finished - started) * 1000 >= request.timeout_ms:
                return self._finish_uncertain_locked(request.correlation_id, current, ReadFailureClassV21.TIMEOUT, finished)
            if status not in {CapabilityStatusV21.AVAILABLE_READ_ONLY, CapabilityStatusV21.DEGRADED}:
            ...
            receipt = _sha(
                {"account_id": current.account_id, "correlation_id": request.correlation_id,
                 ...
                 "state": ReadStateV21.COMPLETED.value, "workspace_id": current.workspace_id}
            )
            completed = replace(
                current, state=ReadStateV21.COMPLETED, failure_class=ReadFailureClassV21.NONE,
    ...
    def reconcile(self, request: CatalogReadRequestV21) -> CatalogReadProjectionV21:
        if type(request) is not CatalogReadRequestV21:
            raise CapabilityV21ContractError("reconcile request type is invalid")
        self._validate_binding(request)
        digest = self._request_digest(request)
        now = self._clock()
        with self._lock:
            ...
            if entry is None:
                return CatalogReadProjectionV21(
                    ReadStateV21.UNKNOWN_CORRELATION, ReadFailureClassV21.UNKNOWN,
    ...
    def draft(self, *_args: object, **_kwargs: object) -> None:
        raise CapabilityV21DeniedError("catalog adapter has no draft operation")

    def mutate(self, *_args: object, **_kwargs: object) -> None:
        raise CapabilityV21DeniedError("catalog adapter has no mutation operation")

    def _validate_binding(self, request: CatalogReadRequestV21) -> None:
        if (
        ...
        if request.target != self.TARGET:
            raise CapabilityV21DeniedError("target is not allowlisted")

    def _effective_status(self) -> tuple[CapabilityStatusV21, str, ReadFailureClassV21]:
        if self._kill:
            return CapabilityStatusV21.DISABLED, "global_kill_active", ReadFailureClassV21.KILL
        if self._revoked:
            return CapabilityStatusV21.BLOCKED_BY_ACCESS, "credential_alias_revoked", ReadFailureClassV21.REVOKED
        if not self._auth:
            return CapabilityStatusV21.BLOCKED_BY_ACCESS, "authentication_unavailable", ReadFailureClassV21.AUTH
        if self.REQUIRED_SCOPE not in self._scopes:
            return CapabilityStatusV21.BLOCKED_BY_SCOPE, "required_scope_missing", ReadFailureClassV21.SCOPE
        if self._status is CapabilityStatusV21.DISABLED:
            return self._status, self._status_reason, ReadFailureClassV21.DISABLED
        return self._status, self._status_reason, ReadFailureClassV21.NONE

    def _request_digest(self, request: CatalogReadRequestV21) -> str:
        return _sha(
    ...
    def _encode_cursor(self, offset: int) -> str:
        payload = {"account_id": self._account_id, "capability_id": self.CAPABILITY_ID,
                   "contract_version": CATALOG_CONTRACT_VERSION_V21, "offset": offset,
    ...
    def _decode_cursor(self, cursor: str) -> int:
        try:
            ...
            if not hmac.compare_digest(signature, expected):
                raise CapabilityV21DeniedError("cursor integrity failure")
            payload = json.loads(body)
        except (ValueError, UnicodeError, json.JSONDecodeError) as exc:
            raise CapabilityV21DeniedError("cursor integrity failure") from exc
        expected_payload = {"account_id": self._account_id, "capability_id": self.CAPABILITY_ID,
                            "contract_version": CATALOG_CONTRACT_VERSION_V21,
                            "profile_id": self._profile_id, "snapshot_digest": self._snapshot_digest,
                            "workspace_id": self._workspace_id}
        if set(payload) != set(expected_payload) | {"offset"} or any(payload.get(k) != v for k, v in expected_payload.items()):
            raise CapabilityV21DeniedError("cursor profile/snapshot binding drift")
        offset = payload.get("offset")
        if type(offset) is not int or not 0 <= offset < len(self._items):
            raise CapabilityV21DeniedError("cursor offset is invalid")
        return offset

    def _terminal_without_reservation(
        self, request: CatalogReadRequestV21, digest: str, state: ReadStateV21,
        failure: ReadFailureClassV21, now: float, *, retry_after_ms: int | None = None,
    ) -> _ReadLedgerEntryV21:
        return _ReadLedgerEntryV21(
    ...
    def _finish_before_hook(
        self, request: CatalogReadRequestV21, digest: str, state: ReadStateV21, failure: ReadFailureClassV21,
        now: float,
    ) -> CatalogReadProjectionV21:
        with self._lock:
            current = self._ledger[request.correlation_id]
    ...
    def _finish_before_hook_locked(
        self, correlation_id: str, current: _ReadLedgerEntryV21, state: ReadStateV21,
        failure: ReadFailureClassV21, now: float,
    ) -> CatalogReadProjectionV21:
    ...
    def _finish_uncertain_locked(
        self, correlation_id: str, current: _ReadLedgerEntryV21,
        failure: ReadFailureClassV21, now: float,
    ) -> CatalogReadProjectionV21:
        uncertain = replace(
            current, state=ReadStateV21.UNCERTAIN, failure_class=failure,
    ...
    def _release_reservation(self, entry: _ReadLedgerEntryV21) -> None:
    ...
    def _expire_pending(self, now: float) -> None:
        for correlation_id, entry in tuple(self._ledger.items()):
            if entry.state is ReadStateV21.PENDING and now >= entry.expires_at:
                uncertain = replace(
                    entry, state=ReadStateV21.UNCERTAIN,
                    failure_class=ReadFailureClassV21.EXPIRED_PENDING,
                    items=(), next_cursor=None, expires_at=now + self._uncertain_ttl,
                )
                self._ledger[correlation_id] = uncertain
            elif entry.state is ReadStateV21.UNCERTAIN and now >= entry.expires_at:
                self._ledger[correlation_id] = replace(
                    entry, state=ReadStateV21.EXPIRED_UNCERTAIN,
                    failure_class=ReadFailureClassV21.UNCERTAIN_EXPIRED,
                    items=(), next_cursor=None, expires_at=now,
                )

    def _projection(self, correlation_id: str, entry: _ReadLedgerEntryV21) -> CatalogReadProjectionV21:
        return CatalogReadProjectionV21(
            entry.state, entry.failure_class, correlation_id, entry.request_digest,
            entry.workspace_id, entry.account_id, entry.profile_id,
            entry.items if entry.state is ReadStateV21.COMPLETED else (),
            entry.next_cursor if entry.state is ReadStateV21.COMPLETED else None,
            entry.receipt_digest if entry.state is ReadStateV21.COMPLETED else None,
