"""Phase 5.2 V15 host-owned, read-only approval inbox projection.

V15 produces authenticated *review evidence*, never authority.  The host owns
...
class ApprovalInboxV15Error(RuntimeError):
    """Base V15 failure."""


class ApprovalInboxV15ContractError(ValueError):
    """Caller or host data violated the bounded V15 contract."""


class ApprovalInboxV15Unavailable(ApprovalInboxV15Error):
    """Projection cannot safely produce a result."""


class ApprovalInboxV15State(str, Enum):
    DISABLED = "DISABLED"
    READY = "READY"
    STALE = "STALE"
    REVOKED = "REVOKED"
...
def _bounded_int(value: object, label: str, minimum: int, maximum: int) -> int:
    if type(value) is not int or not minimum <= value <= maximum:
        raise ApprovalInboxV15ContractError(f"{label} is out of bounds")
    return value


def _bool(value: object, label: str) -> bool:
    if type(value) is not bool:
        raise ApprovalInboxV15ContractError(f"{label} is not boolean")
    return value
...
def _safe_text(value: object, label: str, maximum: int = MAX_TEXT) -> str:
    if (
        ...
        raise ApprovalInboxV15ContractError(f"{label} is invalid")
    if any(ord(character) < 32 or ord(character) == 127 for character in value):
        raise ApprovalInboxV15ContractError(f"{label} contains control characters")
    if _SECRET.search(value):
        raise ApprovalInboxV15ContractError(f"{label} contains secret-like material")
    return value
...
def _safe_id(value: object, label: str) -> str:
    ...
    if not _ID.fullmatch(text):
        raise ApprovalInboxV15ContractError(f"{label} is not canonical")
    return text
...
def _digest(value: object, label: str) -> str:
    if (
        ...
        raise ApprovalInboxV15ContractError(f"{label} is not a canonical digest")
    return value
...
class ApprovalInboxFeatureGateV15:
    ...
    def __init__(
        ...
        *,
        ...
        if not callable(epoch_reader):
            raise ApprovalInboxV15ContractError("epoch_reader is required")
        source = os.environ if environ is None else environ
        raw = source.get(APPROVAL_INBOX_V15_FLAG, "")
        if type(raw) is not str:
            raise ApprovalInboxV15ContractError("feature flag is not text")
    ...
    def _read_epoch(self) -> int:
        try:
            ...
            raise ApprovalInboxV15Unavailable("feature-epoch-unavailable") from None
    ...
    def state(self) -> ApprovalInboxV15State:
        with self._lock:
            if self._revoked:
                return ApprovalInboxV15State.REVOKED
            if not self._enabled:
                return ApprovalInboxV15State.DISABLED
        ...
        with self._lock:
            ...
            if current != self._boot_epoch:
                self._revoked = True
                return ApprovalInboxV15State.REVOKED
            return ApprovalInboxV15State.READY
    ...
    def _publication_barrier(self):
        ...
        with self._lock:
            if self._revoked:
                raise ApprovalInboxV15Unavailable("feature-epoch-revoked")
            if not self._enabled:
                raise ApprovalInboxV15Unavailable("disabled")
        current = self._read_epoch()
        with self._lock:
    ...
    def now_ms(self) -> int:
        ...
        with self._lock:
            if self._failed or current < self._high_water:
                self._failed = True
                raise ApprovalInboxV15Unavailable("monotonic-clock-rollback")
            self._high_water = current
            return current


class DeterministicInboxClockV15:
    ...
    def advance(self, milliseconds: int) -> None:
        ...
        with self._lock:
            if self._current > MAX_INTEGER - delta:
                raise ApprovalInboxV15ContractError("clock overflow")
            self._current += delta
...
@dataclass(frozen=True, slots=True)
class HostInboxSnapshotV15:
    source_epoch: object
    captured_at_ms: object
    items: object


class HostInboxSourceV15:
    ...
    def __init__(
        ...
        *,
        principal_id: str,
        session_id: str,
        capture_callback: Callable[[], HostInboxSnapshotV15],
    ) -> None:
        ...
        if not callable(capture_callback):
            raise ApprovalInboxV15ContractError("capture_callback is required")
    ...
    def capture(self) -> HostInboxSnapshotV15:
        if not self._semaphore.acquire(blocking=False):
            raise ApprovalInboxV15Unavailable("source-single-flight-busy")
        try:
            failed = False
            try:
            ...
            if failed:
                raise ApprovalInboxV15Unavailable("source-callback-failed")
            if type(result) is not HostInboxSnapshotV15:
                raise ApprovalInboxV15ContractError(
...
class InboxPageV15:
    ...
    page_size: int
    offset: int
    items: tuple[ApprovalReviewItemV15, ...]
...
@dataclass(frozen=True, slots=True)
class ReviewSelectionHandoffV15:
...
class ApprovalInboxProjectionV15:
    ...
    def __init__(
        ...
        *,
        ...
        clock: MonotonicInboxClockV15 | DeterministicInboxClockV15,
    ) -> None:
        if type(gate) is not ApprovalInboxFeatureGateV15:
            raise ApprovalInboxV15ContractError("exact feature gate required")
        if type(source) is not HostInboxSourceV15:
            raise ApprovalInboxV15ContractError("exact source required")
        if type(clock) not in {MonotonicInboxClockV15, DeterministicInboxClockV15}:
            raise ApprovalInboxV15ContractError("exact monotonic clock required")
        self._gate = gate
        self._source = source
        self._clock = clock
    ...
    def state(self) -> ApprovalInboxV15State:
        with self._lock:
            if self._state in {
                ApprovalInboxV15State.STALE,
                ApprovalInboxV15State.REVOKED,
                ApprovalInboxV15State.INTEGRITY_LATCHED,
            }:
                return self._state
        try:
            with self._gate._publication_barrier():
                with self._lock:
                    ...
                    try:
                        self._clock.now_ms()
                    except ApprovalInboxV15Error:
                        self._state = ApprovalInboxV15State.INTEGRITY_LATCHED
                        self._source_generation += 1
                        self._snapshots.clear()
                        self._views.clear()
                        return self._state
                    self._state = ApprovalInboxV15State.READY
                    return self._state
        except ApprovalInboxV15Unavailable as error:
            with self._lock:
                if str(error) == "disabled":
                    self._state = ApprovalInboxV15State.DISABLED
                elif self._state not in {
                    ApprovalInboxV15State.STALE,
                    ApprovalInboxV15State.REVOKED,
                    ApprovalInboxV15State.INTEGRITY_LATCHED,
                }:
                    self._state = ApprovalInboxV15State.REVOKED
                    self._source_generation += 1
                self._snapshots.clear()
                self._views.clear()
                return self._state

    def _terminal(self, state: ApprovalInboxV15State, code: str) -> None:
        with self._lock:
            self._state = state
            self._source_generation += 1
            self._snapshots.clear()
            self._views.clear()
        raise ApprovalInboxV15Unavailable(code)
            ...
            with self._gate._publication_barrier():
                with self._lock:
                    if self._state in {
                        ...
                        raise ApprovalInboxV15Unavailable(self._state.value.casefold())
                    try:
                    ...
                    if record is not None:
                        if (
                            ...
                            raise ApprovalInboxV15Unavailable("source-epoch-superseded")
                        if now > record.deadline_ms:
                            self._terminal(
                                ApprovalInboxV15State.STALE, "snapshot-stale"
                            )
                    if view is not None and (
                        ...
                        raise ApprovalInboxV15Unavailable("view-state-unavailable")
                    self._state = ApprovalInboxV15State.READY
                    yield now
        except ApprovalInboxV15Unavailable as error:
            code = str(error)
    ...
    def _normalize_item(self, raw: HostInboxItemV15) -> dict[str, object]:
        if type(raw) is not HostInboxItemV15:
            raise ApprovalInboxV15ContractError("source item has wrong type")
        ...
        if (
            ...
            raise ApprovalInboxV15ContractError("source identity mismatch")
        ...
        if not _CURRENCY.fullmatch(currency):
            raise ApprovalInboxV15ContractError("currency is invalid")
        ...
        if expires <= created:
            raise ApprovalInboxV15ContractError("item expiry is not after creation")
        ...
        item["action_fingerprint"] = _sha(
            {
                "contract": "OnyxActionBinding.v15",
    ...
    def _capture(self, now: int) -> _SnapshotRecord:
        ...
        if type(raw.items) is not tuple or len(raw.items) > MAX_ITEMS:
            raise ApprovalInboxV15ContractError("snapshot items are invalid")
        items = tuple(self._normalize_item(item) for item in raw.items)
        ids = [str(item["item_id"]) for item in items]
        if len(ids) != len(set(ids)):
            raise ApprovalInboxV15ContractError("duplicate item_id")
        semantic = _sha({"contract": "HostInboxSemantic.v15", "items": items})
        ...
        with self._publication() as published_now:
            ...
            if epoch < self._highest_source_epoch:
                ...
                self._state = ApprovalInboxV15State.INTEGRITY_LATCHED
            if self._state is ApprovalInboxV15State.INTEGRITY_LATCHED:
                self._snapshots.clear()
                self._views.clear()
                raise ApprovalInboxV15Unavailable("source-integrity-latched")
            ...
            for record in self._snapshots.values():
                if record.source_epoch == epoch and hmac.compare_digest(
                    ...
                    if now > record.deadline_ms:
                        self._state = ApprovalInboxV15State.STALE
                        self._snapshots.clear()
                        self._views.clear()
                        raise ApprovalInboxV15Unavailable("snapshot-stale")
                    return record
            if now > MAX_INTEGER - MAX_SNAPSHOT_AGE_MS:
                self._state = ApprovalInboxV15State.INTEGRITY_LATCHED
                raise ApprovalInboxV15Unavailable("deadline-overflow")
            snapshot_id = _sha(
                {
                    "contract": "ApprovalInboxSnapshot.v15",
                    "epoch": epoch,
    ...
    def _token(self, kind: str, payload: dict[str, object]) -> str:
        body = _canonical({"kind": kind, "schema": INBOX_V15_SCHEMA_VERSION, **payload})
    ...
    def _untoken(self, token: object, kind: str) -> dict[str, object]:
        if (
            ...
            raise ApprovalInboxV15ContractError("token is malformed")
        ...
        try:
            ...
            raise ApprovalInboxV15ContractError("token is malformed") from None
        expected = hmac.new(self._hmac_key, body, hashlib.sha256).hexdigest()
        if not hmac.compare_digest(mac, expected):
            raise ApprovalInboxV15ContractError("token authentication failed")
        if (
            type(decoded) is not dict
            or decoded.get("kind") != kind
            or decoded.get("schema") != 10
        ):
            raise ApprovalInboxV15ContractError("token relation is invalid")
        return decoded
    ...
    def _review_item(
        ...
        if prior is not None and not hmac.compare_digest(prior, proof):
            self._terminal(
                ApprovalInboxV15State.INTEGRITY_LATCHED, "review-proof-equivocation"
            )
        view.returned_proofs[item_id] = proof
        return ApprovalReviewItemV15(
            item_id=item_id,
    ...
    def _page(
        ...
        with self._publication(record, view):
            ...
            if not 0 <= offset <= len(ordered) or (offset and offset % view.page_size):
                raise ApprovalInboxV15ContractError("cursor offset is invalid")
            ...
            return InboxPageV15(
                state=ApprovalInboxV15State.READY,
                snapshot_id=record.snapshot_id,
                snapshot_digest=record.semantic_digest,
                view_digest=view.view_digest,
    ...
    def open_page(
        self, *, query: InboxQueryV15 | None = None, page_size: int = 20
    ) -> InboxPageV15:
        now = self._guard()
        size = _bounded_int(page_size, "page_size", 1, MAX_PAGE_SIZE)
        if query is None:
            query = InboxQueryV15()
        if type(query) is not InboxQueryV15:
            raise ApprovalInboxV15ContractError("exact query type required")
        ...
        view_digest = _sha(
            {
                "contract": "ApprovalInboxView.v15",
                "snapshot_id": record.snapshot_id,
        ...
        with self._publication(record) as now:
            ...
            if view is None:
                ...
                self._terminal(
                    ApprovalInboxV15State.INTEGRITY_LATCHED, "view-digest-collision"
                )
        return self._page(record, view, 0, now)

    def continue_page(self, cursor: object) -> InboxPageV15:
        now = self._guard()
        ...
        if type(snapshot_id) is not str or type(view_digest) is not str:
            raise ApprovalInboxV15ContractError("cursor relation is invalid")
        with self._lock:
            record = self._snapshots.get(snapshot_id)
            view = self._views.get(view_digest)
            if record is None or view is None or view.snapshot_id != snapshot_id:
                raise ApprovalInboxV15Unavailable("cursor-state-unavailable")
            if decoded.get("page_size") != view.page_size:
                raise ApprovalInboxV15ContractError("cursor page size mismatch")
            if decoded.get("item_ids_digest") != _sha(view.item_ids):
                raise ApprovalInboxV15ContractError("cursor item set mismatch")
            if decoded.get("deadline_ms") != record.deadline_ms:
                raise ApprovalInboxV15ContractError("cursor deadline mismatch")
            offset = _bounded_int(
    ...
    def _resolve_selections(
        ...
        now: int,
    ) -> tuple[_SnapshotRecord, _ViewRecord, tuple[ApprovalReviewItemV15, ...]]:
        ...
        if record is None or view is None or view.snapshot_id != snapshot_key:
            raise ApprovalInboxV15Unavailable("selection-state-unavailable")
        with self._publication(record, view):
            item_by_id = {str(item["item_id"]): item for item in record.items}
            resolved: list[ApprovalReviewItemV15] = []
            for item_id, proof in normalized:
                ...
                if item is None or item_id not in view.item_ids:
                    raise ApprovalInboxV15ContractError("selected item is unavailable")
                if (
        ...
        for label, values in {
            ...
            if len(values) != len(set(values)):
                raise ApprovalInboxV15ContractError(f"duplicate {label}")
    ...
    def preview_calm_batch(
        ...
        payload = {
            "contract": "CalmBatchReviewPreview.v15",
            "snapshot_id": record.snapshot_id,
            "view_digest": view.view_digest,
    ...
    def handoff_calm_batch(
        ...
        payload = {
            "contract": "ReviewSelectionHandoff.v15",
            "snapshot_id": record.snapshot_id,
            "view_digest": view.view_digest,
            "item_ids": item_ids,
        ...
        with self._publication(record, view):
            return ReviewSelectionHandoffV15(
                snapshot_id=record.snapshot_id,
                view_digest=view.view_digest,
