"""Phase 5.2 V6 default-off, non-authoritative approval review projection.
...
APPROVAL_INBOX_FLAG = "ONYX_APPROVAL_INBOX_V6"
INBOX_SCHEMA_VERSION = 6
...
class ApprovalInboxV6Error(RuntimeError):
    pass


class ApprovalInboxV6ContractError(ValueError):
    pass


class ApprovalInboxV6Disabled(ApprovalInboxV6Error):
    pass


class ApprovalInboxV6Stale(ApprovalInboxV6Error):
    pass


class ApprovalInboxV6Unavailable(ApprovalInboxV6Error):
    pass
...
def _bounded_int(value: object, label: str, minimum: int, maximum: int) -> int:
    if type(value) is not int or not minimum <= value <= maximum:
        raise ApprovalInboxV6ContractError(f"{label} is out of bounds")
    return value
...
def _safe_id(value: object, label: str) -> str:
    if (
        ...
        raise ApprovalInboxV6ContractError(f"{label} is invalid")
    return value
...
def _workspace(value: object) -> str:
    if (
        ...
        raise ApprovalInboxV6ContractError("workspace_id is invalid")
    return value
...
def _digest(value: object, label: str) -> str:
    if (
        ...
        raise ApprovalInboxV6ContractError(f"{label} is not a canonical digest")
    return value
...
def _text(value: object, label: str, maximum: int = MAX_TEXT) -> str:
    ...
    if any(ord(character) < 32 or ord(character) == 127 for character in value):
        raise ApprovalInboxV6ContractError(f"{label} contains control characters")
    if contains_secret(value):
        raise ApprovalInboxV6ContractError(f"{label} contains secret-like material")
    return value
    ...
    def now_ms(self) -> int:
        ...
        with self._lock:
            if self._failed or current < self._high_water:
                ...
                raise ApprovalInboxV6Unavailable("local-monotonic-clock-rollback")
    ...
    def advance(self, milliseconds: int) -> None:
        ...
        with self._lock:
            if self._current > MAX_INTEGER - delta:
                raise ApprovalInboxV6ContractError("clock advance overflows")
            self._current += delta
...
class HostInboxSource:
    ...
    def __init__(
        ...
        if not callable(callback):
            raise ApprovalInboxV6ContractError("host source callback is invalid")
    ...
    def capture(self) -> HostInboxSnapshot:
        if not self._semaphore.acquire(blocking=False):
            raise ApprovalInboxV6Unavailable("host-source-busy") from None
        failed = False
        result: object = None
        try:
            try:
        ...
        if failed:
            raise ApprovalInboxV6Unavailable("host-source-failed") from None
        if type(result) is not HostInboxSnapshot:
            raise ApprovalInboxV6ContractError("host snapshot concrete type is invalid")
        items = result.items
        if type(items) is not tuple:
            raise ApprovalInboxV6ContractError("host items must be an exact tuple")
        if len(items) > MAX_ITEMS:
            raise ApprovalInboxV6ContractError("host item collection exceeds its bound")
        if any(type(item) is not HostInboxItem for item in items):
            raise ApprovalInboxV6ContractError("host item concrete type is invalid")
        return result
...
class ApprovalDisplayItem:
    ...
    def _initialize(self, source: HostInboxItem) -> None:
        ...
        if created >= expires:
            raise ApprovalInboxV6ContractError("source display timestamps are invalid")
        if type(source.always_explicit) is not bool or type(source.batch_eligible) is not bool:
            raise ApprovalInboxV6ContractError("host item booleans are invalid")
        if source.batch_eligible and (
            source.always_explicit or risk in {"high", "critical"}
        ):
            raise ApprovalInboxV6ContractError(
        ...
        if type(currency) is not str or not _CURRENCY.fullmatch(currency):
            raise ApprovalInboxV6ContractError("currency is invalid")
        ...
        payload = {
            "contract": "ApprovalDisplayItem.v6",
...
def _review_digest(item: ApprovalDisplayItem | BatchPreviewItem) -> str:
    if type(item) is ApprovalDisplayItem:
        ...
        raise ApprovalInboxV6ContractError("review digest item type is invalid")
    return _sha(
        {
            "contract": "ApprovalHumanReview.v6",
...
def _build_result_factories() -> tuple[Callable[..., object], ...]:
    ...
    def make_review_item(
        ...
        if type(item) is not ApprovalDisplayItem or item.review_token is not None:
            raise ApprovalInboxV6ContractError("review item source is invalid")
        if type(review_token) is not str or not review_token:
            raise ApprovalInboxV6ContractError("review item token is invalid")
        ...
        *,
        ...
        if status == "disabled":
            if any((snapshot, view, items, offset, snapshot_token, next_cursor)):
                raise ApprovalInboxV6ContractError("disabled page is contradictory")
            values: dict[str, object] = {
                "status": "disabled",
                "snapshot_version": 0,
            ...
            if type(snapshot) is not _Snapshot or type(view) is not _View:
                raise ApprovalInboxV6ContractError("ready page lacks exact snapshot/view")
            if view.snapshot_digest != snapshot.snapshot_digest or type(view.query) is not InboxQuery:
                raise ApprovalInboxV6ContractError("ready page snapshot/view relation is invalid")
            if (
                ...
                raise ApprovalInboxV6ContractError("ready page view bounds/set is invalid")
            derived_view_digest = _canonical_view_digest(snapshot, view)
            if not hmac.compare_digest(view.view_digest, derived_view_digest):
                raise ApprovalInboxV6ContractError("ready page view digest is not canonical")
            if type(items) is not tuple or any(type(item) is not ApprovalDisplayItem for item in items):
                raise ApprovalInboxV6ContractError("ready page items are invalid")
            ...
            if not len(items) <= view.page_size or offset + len(items) > total:
                raise ApprovalInboxV6ContractError("page bounds are contradictory")
            expected_ids = view.allowed_item_ids[offset : offset + len(items)]
            if tuple(item.item_id for item in items) != expected_ids:
                raise ApprovalInboxV6ContractError("page items do not match the bound view")
            ...
            if any(item_id not in snapshot_items for item_id in view.allowed_item_ids) or any(
                ...
                raise ApprovalInboxV6ContractError("page item digests do not match the snapshot")
            if type(snapshot_token) is not str or not snapshot_token:
                raise ApprovalInboxV6ContractError("ready page lacks an authenticated token")
            ...
            if projection._decode(snapshot_token, "snapshot") != expected_snapshot_payload:
                raise ApprovalInboxV6ContractError("ready page snapshot token is contradictory")
            next_offset = offset + len(items)
            has_remaining = next_offset < total
            if has_remaining:
                if type(next_cursor) is not str or not next_cursor:
                    raise ApprovalInboxV6ContractError("ready page lacks its next cursor")
                expected_cursor_payload = expected_snapshot_payload | {
                    "k": "cursor",
                    "o": next_offset,
                }
                if projection._decode(next_cursor, "cursor") != expected_cursor_payload:
                    raise ApprovalInboxV6ContractError("ready page cursor is contradictory")
            elif next_cursor is not None:
                raise ApprovalInboxV6ContractError("terminal page exposes a cursor")
            ...
            raise ApprovalInboxV6ContractError("page status is invalid")
    ...
    def make_batch_preview(
        ...
        if type(projection) is not ApprovalInboxProjection:
            raise TypeError("batch previews are created by the projection")
        if type(snapshot) is not _Snapshot or type(view) is not _View:
            raise ApprovalInboxV6ContractError("batch preview lacks exact snapshot/view")
        if view.snapshot_digest != snapshot.snapshot_digest or type(view.query) is not InboxQuery:
            raise ApprovalInboxV6ContractError("batch snapshot/view relation is invalid")
        derived_view_digest = _canonical_view_digest(snapshot, view)
        if not hmac.compare_digest(view.view_digest, derived_view_digest):
            raise ApprovalInboxV6ContractError("batch view digest is not canonical")
        expected_snapshot_payload = {
            "d": snapshot.snapshot_digest,
            "k": "snapshot",
            "q": view.query.payload(),
        ...
        if projection._decode(snapshot_token, "snapshot") != expected_snapshot_payload:
            raise ApprovalInboxV6ContractError("batch snapshot token is contradictory")
        if type(items) is not tuple or not 1 <= len(items) <= MAX_BATCH_ITEMS:
            raise ApprovalInboxV6ContractError("batch preview item count is invalid")
        if any(
            ...
            raise ApprovalInboxV6ContractError("batch preview items are invalid")
        ...
        if ordered != items or len({item.item_id for item, _token in items}) != len(items):
            raise ApprovalInboxV6ContractError("batch preview ordering/set is invalid")
        ...
        for item, review_token in items:
            if item.item_id not in allowed:
                raise ApprovalInboxV6ContractError("batch item is hidden from the bound view")
            if item.item_id not in snapshot_items or not hmac.compare_digest(
                item.item_digest, snapshot_items[item.item_id]
            ):
                raise ApprovalInboxV6ContractError("batch item digest is not in the snapshot")
            if not item.batch_eligible or item.always_explicit or item.risk in {"high", "critical"}:
                raise ApprovalInboxV6ContractError("batch item is ineligible")
            if projection._decode(review_token, "review") != projection._review_payload(
                snapshot, view, item
            ):
                raise ApprovalInboxV6ContractError("batch review token is contradictory")
...
class ApprovalInboxProjection:
    ...
    def __init__(
        ...
        if type(source) is not HostInboxSource:
            raise ApprovalInboxV6ContractError("exact HostInboxSource is required")
        if type(clock) not in {MonotonicInboxClock, DeterministicInboxClock}:
            raise ApprovalInboxV6ContractError("exact concrete inbox clock is required")
        self._source = source
        self._clock = clock
    ...
    def _assert_integrity(self) -> None:
        with self._lock:
            if self._integrity_failed:
                raise ApprovalInboxV6Unavailable("projection-integrity-failed")
    ...
    def _latch_integrity(self, reason: str) -> None:
        ...
        raise ApprovalInboxV6Stale(reason)
    ...
    def _materialize(self, source: HostInboxSnapshot, local_now: int) -> _Snapshot:
        if type(source.schema_version) is not int or source.schema_version != INBOX_SCHEMA_VERSION:
            raise ApprovalInboxV6ContractError("unknown host snapshot schema")
        ...
        if principal != self._source.principal_id or session != self._source.session_id:
            raise ApprovalInboxV6Unavailable("host-principal-session-drift")
        ...
        if any(
            ...
            raise ApprovalInboxV6ContractError("host safety values are invalid")
        if not source.audit_healthy:
            raise ApprovalInboxV6Unavailable("host-audit-unhealthy")
        if not source.session_active:
            raise ApprovalInboxV6Unavailable("host-session-inactive")
        if source.kill_switch:
            raise ApprovalInboxV6Unavailable("host-kill-active")
        ...
        if len({item.item_id for item in items}) != len(items):
            raise ApprovalInboxV6ContractError("host item IDs are duplicated")
        if len({item.item_digest for item in items}) != len(items):
            raise ApprovalInboxV6ContractError("host item digests are duplicated")
        if len({item.idempotency_key for item in items}) != len(items):
            raise ApprovalInboxV6ContractError("host idempotency keys are duplicated")
        semantic = {
            "contract": "ApprovalInboxSnapshot.v6",
            "schema_version": SNAPSHOT_SCHEMA_VERSION,
        ...
        if local_now > MAX_INTEGER - MAX_SNAPSHOT_AGE_MS:
            raise ApprovalInboxV6Unavailable("local-monotonic-deadline-overflow")
        source_digest = _sha(semantic)
        deadline = local_now + MAX_SNAPSHOT_AGE_MS
        snapshot_digest = _sha(
            {
                "contract": "LocalApprovalReviewSnapshot.v6",
    ...
    def _register_view(self, snapshot: _Snapshot, query: InboxQuery, page_size: int) -> _View:
        ...
        with self._lock:
            if self._integrity_failed:
                raise ApprovalInboxV6Unavailable("projection-integrity-failed")
            registered = self._views.get(digest)
            returned = self._returned_reviews.get(digest)
            if registered is None:
                if returned is not None:
                    self._latch_integrity("view-proof-without-registered-view")
                self._views[digest] = view
                self._returned_reviews[digest] = {}
            elif registered != view or returned is None:
                self._latch_integrity("registered-view-proof-conflict")
    ...
    def _query_from_payload(value: object) -> InboxQuery:
        if type(value) is not dict or frozenset(value) != {
            ...
            raise ApprovalInboxV6ContractError("token query shape is invalid")
        risks = value["risks"]
        if type(risks) is not list:
            raise ApprovalInboxV6ContractError("token risk filter is invalid")
        return InboxQuery(
    ...
    def _validate_token_payload(cls, value: object, kind: str) -> dict[str, object]:
        ...
        if type(value) is not dict or set(value) != expected:
            raise ApprovalInboxV6ContractError("token payload shape is invalid")
        if value["v"] != TOKEN_SCHEMA_VERSION or value["k"] != kind:
            raise ApprovalInboxV6ContractError("token payload schema is invalid")
        ...
        if query.payload() != value["q"]:
            raise ApprovalInboxV6ContractError("token query is noncanonical")
        return value
    ...
    def _encode(self, payload: dict[str, object], kind: str) -> str:
        ...
        if not body or not _TOKEN_BODY.fullmatch(body):
            raise ApprovalInboxV6ContractError("token body encoding is invalid")
        ...
        if len(token) > MAX_TOKEN:
            raise ApprovalInboxV6ContractError("token exceeds its bound")
        return token
    ...
    def _decode(self, token: object, kind: str) -> dict[str, object]:
        ...
        if failed or type(payload) is not dict:
            raise ApprovalInboxV6Stale("invalid-authenticated-review-token") from None
        return payload
    ...
    def _lookup(self, payload: dict[str, object], local_now: int) -> tuple[_Snapshot, _View]:
        ...
        if type(snapshot) is not _Snapshot or type(view) is not _View:
            raise ApprovalInboxV6Stale("review-snapshot-or-view-not-available")
        if view.snapshot_digest != snapshot.snapshot_digest:
            raise ApprovalInboxV6Stale("review-view-snapshot-mismatch")
        if view.page_size != payload["z"] or view.query.payload() != payload["q"]:
            raise ApprovalInboxV6Stale("review-token-view-mismatch")
        if local_now >= snapshot.local_deadline_ms or local_now >= view.local_deadline_ms:
            raise ApprovalInboxV6Stale("local-review-deadline-expired")
    ...
    def _refresh_exact(self, pinned: _Snapshot, local_now: int) -> None:
        ...
        if (
            ...
            raise ApprovalInboxV6Stale("host-source-drifted-from-review-snapshot")
    ...
    def _items_for_view(snapshot: _Snapshot, view: _View) -> tuple[ApprovalDisplayItem, ...]:
        ...
        if any(item_id not in by_id for item_id in view.allowed_item_ids):
            raise ApprovalInboxV6Stale("review-view-item-set-drifted")
    ...
    def _page(self, snapshot: _Snapshot, view: _View, offset: int) -> InboxPage:
        with self._lock:
            ...
            if type(returned) is not dict:
                raise ApprovalInboxV6Stale("review-view-return-state-missing")
            for item in page.items:
                if type(item.review_token) is not str:
                    raise ApprovalInboxV6ContractError("page item lacks review token")
    ...
    def open_page(
        ...
        *,
        ...
        if type(query) is not InboxQuery:
            raise ApprovalInboxV6ContractError("exact InboxQuery is required")
    ...
    def preview_calm_batch(
        ...
        *,
        ...
        if not approval_inbox_enabled(environ):
            raise ApprovalInboxV6Disabled(f"{APPROVAL_INBOX_FLAG} is disabled")
        self._assert_integrity()
        payload = self._decode(snapshot_token, "snapshot")
        if type(selections) not in {tuple, list}:
            raise ApprovalInboxV6ContractError("batch selection must be an exact finite sequence")
        if not 1 <= len(selections) <= MAX_BATCH_ITEMS:
            raise ApprovalInboxV6ContractError("batch selection exceeds its bound")
        ...
        if len({item_id for item_id, _token in selected_pairs}) != len(selected_pairs):
            raise ApprovalInboxV6ContractError("batch selection contains duplicates")
        local_now = self._clock.now_ms()
        snapshot, view = self._lookup(payload, local_now)
        self._refresh_exact(snapshot, local_now)
        allowed = set(view.allowed_item_ids)
        ...
        if selected_ids - allowed:
            raise ApprovalInboxV6ContractError("batch selection is outside the bound view")
        by_id = {item.item_id: item for item in snapshot.items}
        with self._lock:
            if self._integrity_failed:
                raise ApprovalInboxV6Unavailable("projection-integrity-failed")
...
__all__ = [
    ...
    "ApprovalInboxV6ContractError",
    "ApprovalInboxV6Disabled",
    "ApprovalInboxV6Error",
    "ApprovalInboxV6Stale",
    "ApprovalInboxV6Unavailable",
    "BatchPreviewItem",
    "CalmBatchPreview",
