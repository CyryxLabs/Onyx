"""Host-owned Phase 5 integration core (isolated V7 candidate).

This module is deliberately not imported by a live/startup surface.  V7 keeps
...
bounded :class:`TerminationPlanV7` and acknowledges its opaque single-use
actions explicitly.
"""
...
PHASE5_RUNTIME_FLAG = "ONYX_PHASE5_RUNTIME_V7"
PHASE5_GRANT_SHADOW_FLAG = "ONYX_PHASE5_GRANT_SHADOW_V7"
PHASE5_APPROVAL_INBOX_FLAG = "ONYX_PHASE5_APPROVAL_INBOX_V7"
PHASE5_LOW_RISK_FLAG = "ONYX_PHASE5_LOW_RISK_V7"
PHASE5_NEXUS_PROJECTION_FLAG = "ONYX_PHASE5_NEXUS_PROJECTION_V7"
PHASE5_LOCAL_CATALOG_READ_FLAG = "ONYX_PHASE5_LOCAL_CATALOG_READ_V7"
PHASE5_DASHBOARD_PROJECTION_FLAG = "ONYX_PHASE5_DASHBOARD_PROJECTION_V7"
...
class Phase5RuntimeV7Error(RuntimeError):
    """Base error for the isolated V7 runtime core."""


class Phase5RuntimeV7ContractError(ValueError):
    """A value violated the closed and bounded V7 host contract."""


class RuntimeDispositionV7(str, Enum):
    FALLBACK = "FALLBACK"
    ALLOW_LOCAL_CATALOG_READ = "ALLOW_LOCAL_CATALOG_READ"
    EXPLICIT = "EXPLICIT"


class RuntimeStateV7(str, Enum):
    DISABLED = "DISABLED"
    READY = "READY"
    DEGRADED = "DEGRADED"
...
class TerminalReasonV7(str, Enum):
    KILL = "kill"
    REVOKE = "revoke"
    ROLLBACK = "rollback"
...
def _exact_bool(value: object, label: str) -> bool:
    if type(value) is not bool:
        raise Phase5RuntimeV7ContractError(f"{label} must be an exact boolean")
    return value
...
def _safe_id(value: object, label: str) -> str:
    ...
    if type(value) is not str or not value:
        raise Phase5RuntimeV7ContractError(f"{label} is invalid")
    normalized = unicodedata.normalize("NFKC", value)
    if _has_unsafe_unicode(value) or _has_unsafe_unicode(normalized):
        raise Phase5RuntimeV7ContractError(f"{label} contains unsafe Unicode")
    ...
    if _contains_sensitive_value(value):
        raise Phase5RuntimeV7ContractError(f"{label} contains sensitive data")
    return normalized
...
def _scan_shadows(value: str) -> tuple[str, str, str]:
    """Return original, NFKC/casefold and separator-collapsed DLP views."""
    ...
    return value, normalized, collapsed
...
class RuntimeFlagsV7:
    ...
    @classmethod
    def from_environ(cls, environ: Mapping[str, str] | None = None) -> RuntimeFlagsV7:
...
def _binding(value: object) -> RuntimeBindingV7:
    if type(value) is not RuntimeBindingV7:
        raise Phase5RuntimeV7ContractError("binding must be exact RuntimeBindingV7")
...
def _flags(value: object) -> RuntimeFlagsV7:
    if type(value) is not RuntimeFlagsV7:
        raise Phase5RuntimeV7ContractError("flags must be exact RuntimeFlagsV7")
    return RuntimeFlagsV7(**dict(value.payload()))


@dataclass(frozen=True, slots=True)
class ActionRequestV7:
    invocation_ref: str
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.cost_micro) is not int or not 0 <= self.cost_micro <= MAX_INTEGER:
            raise Phase5RuntimeV7ContractError("cost_micro is invalid")
        if self.risk not in _RISKS:
            raise Phase5RuntimeV7ContractError("risk is invalid")
        if type(self.action_classes) is not tuple or len(self.action_classes) > 32:
            raise Phase5RuntimeV7ContractError("action_classes is invalid")
        classes = tuple(
            _safe_id(item, "action_class").casefold() for item in self.action_classes
        )
        if len(classes) != len(set(classes)):
            raise Phase5RuntimeV7ContractError("action_classes contains duplicates")
        object.__setattr__(self, "action_classes", tuple(sorted(classes)))


def _action(value: object) -> ActionRequestV7:
    if type(value) is not ActionRequestV7:
        raise Phase5RuntimeV7ContractError("action must be exact ActionRequestV7")
    return ActionRequestV7(
...
@dataclass(frozen=True, slots=True)
class RuntimeDecisionV7:
    disposition: RuntimeDispositionV7
    reason: str
    trace_id: str
    epoch: int
    advisory_json: str = "{}"
    ...
    def __post_init__(self) -> None:
        if type(self.disposition) is not RuntimeDispositionV7:
            raise Phase5RuntimeV7ContractError("decision disposition is invalid")
        _text(self.reason, "reason", 128, redact=False)
        _safe_id(self.trace_id, "trace_id")
        if type(self.epoch) is not int or self.epoch < 1:
            raise Phase5RuntimeV7ContractError("decision epoch is invalid")
        if (
            type(self.advisory_json) is not str
            or len(self.advisory_json.encode()) > MAX_EVENT_TEXT_BYTES
        ):
            raise Phase5RuntimeV7ContractError("advisory_json is invalid")
        try:
            parsed = json.loads(self.advisory_json)
        except json.JSONDecodeError as exc:
            raise Phase5RuntimeV7ContractError("advisory_json is invalid") from exc
        if type(parsed) is not dict or set(parsed) - {
            ...
            raise Phase5RuntimeV7ContractError("advisory_json schema is invalid")
...
class _Budget:
    ...
    def add(self, *, nodes: int = 1, bytes_: int = 0) -> None:
        ...
        if self.nodes > MAX_JSON_NODES or self.bytes > MAX_JSON_BYTES:
            raise Phase5RuntimeV7ContractError("JSON budget exceeded")
...
def _json_value(
    ...
    *,
    ...
    if budget is None:
        budget = _Budget()
    if depth > MAX_DEPTH:
        raise Phase5RuntimeV7ContractError("JSON nesting is too deep")
    budget.add()
    if value is None or type(value) is bool:
        return value
    if type(value) is int:
        if not -MAX_INTEGER <= value <= MAX_INTEGER:
            raise Phase5RuntimeV7ContractError("JSON integer is out of bounds")
    ...
    if type(value) in {list, tuple, dict, MappingProxyType}:
        ...
        if identity in budget.seen:
            raise Phase5RuntimeV7ContractError(
                "JSON contains a cycle or shared container"
            )
        budget.seen.add(identity)
    if type(value) in {list, tuple}:
        if len(value) > MAX_FANOUT:
            raise Phase5RuntimeV7ContractError("JSON sequence fanout exceeded")
        return [
            _json_value(
                item,
                depth=depth + 1,
    ...
    if type(value) is dict or type(value) is MappingProxyType:
        if len(value) > MAX_FANOUT:
            raise Phase5RuntimeV7ContractError("JSON object fanout exceeded")
        ...
        for key, item in value.items():
            ...
            if (
                ...
                raise Phase5RuntimeV7ContractError("JSON key is unsafe")
            budget.add(nodes=0, bytes_=len(normalized_key.encode()))
            if normalized_key in result:
                raise Phase5RuntimeV7ContractError("JSON keys collide after NFKC")
        ...
        return result
    raise Phase5RuntimeV7ContractError("JSON contains a non-JSON value")
...
class ProjectionBatchV7:
    ...
    def __post_init__(self) -> None:
        if type(self.items) is not tuple or len(self.items) > MAX_PAGE_SIZE:
            raise Phase5RuntimeV7ContractError("projection batch is invalid")
        ...
        if self.next_cursor is not None and self.next_cursor == self.source_cursor:
            raise Phase5RuntimeV7ContractError("projection cursor does not advance")


def _batch(value: object) -> ProjectionBatchV7:
    if type(value) is not ProjectionBatchV7:
        raise Phase5RuntimeV7ContractError("component returned the wrong batch type")
    budget = _Budget()
    return ProjectionBatchV7(
        tuple(
...
class ApprovalInboxReaderV7(Protocol):
    def read_page(
        self,
        *,
        binding: RuntimeBindingV7,
...
@dataclass(frozen=True, slots=True)
class RuntimeComponentsV7:
    grant_evaluator: Callable[[str], object] | None = None
    nexus_projector: Callable[[RuntimeBindingV7], ProjectionBatchV7] | None = None
    inbox_factory: Callable[[RuntimeBindingV7], ApprovalInboxReaderV7] | None = None
    ...
    def __post_init__(self) -> None:
        for name in ("grant_evaluator", "nexus_projector", "inbox_factory"):
            ...
            if value is not None and not callable(value):
                raise Phase5RuntimeV7ContractError(f"{name} must be callable")
        if (
            ...
            raise Phase5RuntimeV7ContractError("termination_actions is invalid")


def _components(value: object) -> RuntimeComponentsV7:
    if type(value) is not RuntimeComponentsV7:
        raise Phase5RuntimeV7ContractError(
            "components must be exact RuntimeComponentsV7"
        )
    return RuntimeComponentsV7(
        **{name: getattr(value, name) for name in value.__dataclass_fields__}
    )


@dataclass(frozen=True, slots=True)
class ProjectionItemV7:
    item_id: str
    payload_json: str
...
@dataclass(frozen=True, slots=True)
class ProjectionPageV7:
    kind: str
    offset: int
    page_size: int
    total: int
    items: tuple[ProjectionItemV7, ...]
...
@dataclass(frozen=True, slots=True)
class RuntimeStatusV7:
    state: RuntimeStateV7
    reason: str
    epoch: int
    binding: RuntimeBindingV7
...
@dataclass(frozen=True, slots=True)
class TerminationPlanV7:
    """Immutable, bounded cleanup plan issued once per runtime."""

    runtime_id: str
    epoch: int
    reason: str
    actions: tuple[TerminationActionV7, ...]


@dataclass(frozen=True, slots=True)
class _DecisionSealV7:
    decision: RuntimeDecisionV7
    public_digest: str
    request_digest: str
    binding_digest: str
    flags_digest: str
...
def _binding_digest(value: RuntimeBindingV7) -> str:
    return _digest(
        [
            value.trace_id,
            value.session_id,
...
def _flags_digest(value: RuntimeFlagsV7) -> str:
    return _digest(value.payload())


def _action_digest(value: ActionRequestV7) -> str:
    return _digest(
        {
...
def _decision_digest(value: RuntimeDecisionV7) -> str:
    return _digest(
        [
...
def _closed_inbox(
    ...
    if set(raw) != _INBOX_FIELDS:
        raise Phase5RuntimeV7ContractError("inbox schema fields are invalid")
    ...
    if risk not in _RISKS or state not in _INBOX_STATE:
        raise Phase5RuntimeV7ContractError("inbox enum is invalid")
    created_at = _text(raw["created_at"], "created_at", 32, redact=False)
    if not _RFC3339_UTC.fullmatch(created_at):
        raise Phase5RuntimeV7ContractError("created_at is invalid")
    payload: dict[str, object] = {
        "item_id": identifier,
...
def _record(
    surface: str, raw: Mapping[str, object], binding: RuntimeBindingV7
) -> ProjectionItemV7:
    identifier, payload = (
    ...
    if len(encoded.encode()) > MAX_PROJECTION_ROW_BYTES:
        raise Phase5RuntimeV7ContractError("projection row is too large")
    return ProjectionItemV7(identifier, encoded)
...
def _normalize_advisory(raw: Mapping[str, object]) -> dict[str, object]:
    ...
    if set(raw) - {
        ...
        raise Phase5RuntimeV7ContractError("grant advisory fields are invalid")
    ...
    for name, item in raw.items():
        if name in {"grant_id", "scope_digest", "action_digest"}:
            ...
            raise Phase5RuntimeV7ContractError("grant advisory field is invalid")
    return normalized
...
def _advisory(value: object) -> str:
    ...
    if len(encoded.encode()) > MAX_EVENT_TEXT_BYTES:
        raise Phase5RuntimeV7ContractError("grant advisory is too large")
    return encoded
...
class Phase5RuntimeV7:
    ...
    def __init__(
        ...
        *,
        binding: RuntimeBindingV7,
        flags: RuntimeFlagsV7 | None = None,
        components: RuntimeComponentsV7 | None = None,
        ...
        self._flags = _flags(flags or RuntimeFlagsV7.from_environ())
        self._components = _components(components or RuntimeComponentsV7())
        ...
        self._termination_plan: TerminationPlanV7 | None = None
        self._termination_pending: OrderedDict[str, TerminationActionV7] = OrderedDict()
        ...
        self._capabilities: OrderedDict[str, ProjectionItemV7] = OrderedDict()
        self._inbox: OrderedDict[str, ProjectionItemV7] = OrderedDict()
        ...
        self._events: deque[RuntimeEventV7] = deque(maxlen=MAX_EVENTS)
        self._event_sequence = 0
        self._decision_seals: OrderedDict[int, _DecisionSealV7] = OrderedDict()
        self._decision_key = secrets.token_bytes(32)

    @property
    def binding(self) -> RuntimeBindingV7:
        with self._lock:
            return _binding(self._binding)

    @property
    def flags(self) -> RuntimeFlagsV7:
        with self._lock:
    ...
    def _event_locked(
        ...
        *,
        ...
        if count is not None and (
            ...
            raise Phase5RuntimeV7ContractError("event count is invalid")
        reason = (
            None
            if reason is None
    ...
    def rebind(self, binding: RuntimeBindingV7) -> RuntimeStatusV7:
    ...
    def configure_flags(self, flags: RuntimeFlagsV7) -> RuntimeStatusV7:
    ...
    def replace_components(self, components: RuntimeComponentsV7) -> RuntimeStatusV7:
    ...
    def _ensure_active_locked(self) -> None:
        if self._terminated:
            raise Phase5RuntimeV7Error("runtime is terminated")
    ...
    def _state_locked(self) -> RuntimeStateV7:
        if self._terminated and not self._termination_complete:
            return RuntimeStateV7.TERMINATION_PENDING
        if self._termination_complete:
            return RuntimeStateV7.TERMINATED
        if not self._flags.runtime:
            return RuntimeStateV7.DISABLED
        if any(
            ...
            return RuntimeStateV7.DEGRADED
        return RuntimeStateV7.READY

    def _status_locked(self) -> RuntimeStatusV7:
        ...
        reason = (
            ...
            if self._terminated
                ...
                "feature_flag_off"
                if state is RuntimeStateV7.DISABLED
                else "component_failure"
                if state is RuntimeStateV7.DEGRADED
                else "ready"
            )
        )
        return RuntimeStatusV7(
            state,
    ...
    def status(self) -> RuntimeStatusV7:
        with self._lock:
            return self._status_locked()
    ...
    def _page_bounds(offset: int, page_size: int) -> tuple[int, int]:
        if type(offset) is not int or not 0 <= offset <= MAX_PROJECTION_ITEMS:
            raise Phase5RuntimeV7ContractError("offset is invalid")
        if type(page_size) is not int or not 1 <= page_size <= MAX_PAGE_SIZE:
            raise Phase5RuntimeV7ContractError("page_size is invalid")
        return offset, page_size

    def events(
        self, *, offset: int = 0, page_size: int = MAX_PAGE_SIZE
    ) -> tuple[RuntimeEventV7, ...]:
    ...
    def _set_failure_locked(self, name: str, failed: bool) -> None:
        if name not in _COMPONENTS:
            raise Phase5RuntimeV7ContractError("component is invalid")
    ...
    @staticmethod
    def _contains_explicit(action: ActionRequestV7) -> bool:
        if (
    ...
    def _decision_locked(
        ...
        *,
        ...
        if disposition is RuntimeDispositionV7.ALLOW_LOCAL_CATALOG_READ:
            ...
            self._decision_seals[id(decision)] = _DecisionSealV7(
                decision,
                public,
                request_digest,
                binding,
    ...
    def evaluate(self, action: ActionRequestV7) -> RuntimeDecisionV7:
        with self._lock:
            if self._terminated:
                return self._decision_locked(
                    RuntimeDispositionV7.EXPLICIT, "runtime_terminated"
                )
            if not self._flags.runtime:
                return self._decision_locked(
                    RuntimeDispositionV7.FALLBACK, "feature_flag_off", publish=False
                )
            try:
                snapshot = _action(action)
            except (AttributeError, Phase5RuntimeV7ContractError):
                return self._decision_locked(
                    RuntimeDispositionV7.EXPLICIT, "invalid_action_contract"
                )
            if self._contains_explicit(snapshot):
                return self._decision_locked(
                    RuntimeDispositionV7.EXPLICIT, "explicit_action_class"
                )
            if not self._flags.low_risk or not self._flags.local_catalog_read:
                return self._decision_locked(
                    RuntimeDispositionV7.FALLBACK, "extension_path_off", publish=False
                )
            if snapshot.binding != self._binding:
                return self._decision_locked(
                    RuntimeDispositionV7.EXPLICIT, "scope_mismatch"
                )
            exact = (
            ...
            if not exact:
                return self._decision_locked(
                    RuntimeDispositionV7.EXPLICIT, "not_exact_local_catalog_read"
                )
            if any(
                self._enabled_component_locked(name)
                and (
                ...
                return self._decision_locked(
                    RuntimeDispositionV7.EXPLICIT, "component_unverified_or_failed"
                )
            observer = (
            ...
            if self._flags.grant_shadow and observer is None:
                ...
                return self._decision_locked(
                    RuntimeDispositionV7.EXPLICIT, "grant_shadow_unavailable"
                )
        ...
        if observer is not None:
            try:
                ...
                with self._lock:
                    return self._decision_locked(
                        RuntimeDispositionV7.EXPLICIT, "grant_shadow_failure"
                    )

        with self._lock:
            if self._terminated or config_lease != self._config_generation:
                return self._decision_locked(
                    RuntimeDispositionV7.EXPLICIT, "authority_revoked_during_evaluation"
                )
            if component_lease is not None:
                if not self._lease_current_locked("grant_shadow", component_lease):
                    return self._decision_locked(
                        RuntimeDispositionV7.EXPLICIT, "evaluation_superseded"
                    )
                assert operation_sequence is not None
            ...
            if any(
                ...
                return self._decision_locked(
                    RuntimeDispositionV7.EXPLICIT, "component_failure_during_evaluation"
                )
            return self._decision_locked(
                RuntimeDispositionV7.ALLOW_LOCAL_CATALOG_READ,
    ...
    def legacy_or_decision(
        self, action: ActionRequestV7, legacy: Callable[[], _T]
    ) -> _T | RuntimeDecisionV7:
        if not callable(legacy):
            raise Phase5RuntimeV7ContractError("legacy fallback must be callable")
        decision = self.evaluate(action)
        return (
            legacy()
            if decision.disposition is RuntimeDispositionV7.FALLBACK
            else decision
        )

    def _current_locked(
        self, decision: RuntimeDecisionV7, action: ActionRequestV7
    ) -> bool:
        if (
            type(decision) is not RuntimeDecisionV7
            or type(action) is not ActionRequestV7
        ):
            return False
        ...
        try:
            ...
            return False
        return (
            decision.disposition is RuntimeDispositionV7.ALLOW_LOCAL_CATALOG_READ
    ...
    def decision_is_current(
        self, decision: RuntimeDecisionV7, action: ActionRequestV7 | None = None
    ) -> bool:
    ...
    def _prepare(
        surface: str,
        batch: ProjectionBatchV7,
        binding: RuntimeBindingV7,
    ) -> tuple[ProjectionBatchV7, tuple[ProjectionItemV7, ...]]:
    ...
    @staticmethod
    def _apply(
        target: OrderedDict[str, ProjectionItemV7],
        records: tuple[ProjectionItemV7, ...],
        *,
        replace: bool,
    ...
    def refresh_capabilities(self) -> ProjectionPageV7:
        with self._lock:
            if (
                self._terminated
    ...
    def refresh_inbox(
        ...
        except Phase5RuntimeV7ContractError:
            with self._lock:
        ...
        try:
            ...
            if not callable(read_page):
                raise Phase5RuntimeV7ContractError("inbox reader has no read_page")
            snapshot, records = self._prepare(
                "inbox",
                read_page(
            ...
            if snapshot.source_cursor != cursor:
                raise Phase5RuntimeV7ContractError("inbox source cursor mismatch")
            if cursor is None and not snapshot.replace:
                raise Phase5RuntimeV7ContractError("first inbox page must replace")
            if cursor is not None and snapshot.replace:
                raise Phase5RuntimeV7ContractError("continuation cannot replace")
            if not records and snapshot.next_cursor is not None:
                raise Phase5RuntimeV7ContractError("empty inbox page cannot continue")
        except BaseException:
    ...
    def recover_component(self, name: str) -> RuntimeStatusV7:
        ...
        with self._lock:
            self._ensure_active_locked()
            if not self._flags.runtime:
                raise Phase5RuntimeV7Error("runtime is disabled")
            if not self._enabled_component_locked(name):
                raise Phase5RuntimeV7ContractError("component is not enabled")
        ...
        snapshot: ProjectionBatchV7 | None = None
        records: tuple[ProjectionItemV7, ...] = ()
        try:
            if name == "grant_shadow":
                assert callable(component)
                _advisory(component("phase5-v7-probe"))
            elif name == "nexus":
                assert callable(component)
                snapshot, records = self._prepare(
    ...
    def _page_locked(self, kind: str, offset: int, page_size: int) -> ProjectionPageV7:
    ...
    def dashboard_read(
        ...
        *,
    ...
    ) -> RuntimeStatusV7 | ProjectionPageV7 | tuple[RuntimeEventV7, ...]:
        if type(surface) is not str or surface not in {
            ...
            raise Phase5RuntimeV7ContractError("dashboard surface is not allowlisted")
        with self._lock:
            if not self._flags.runtime or not self._flags.dashboard_projection:
                raise Phase5RuntimeV7Error("dashboard projection is disabled")
        if surface == "status":
            return self.status()
        if surface == "capabilities":
    ...
    def terminate(self, reason: TerminalReasonV7) -> RuntimeStatusV7:
        ...
        """
        if type(reason) is not TerminalReasonV7:
            raise Phase5RuntimeV7ContractError("terminal reason must be exact")
        with self._terminal_condition:
            ...
            actions: list[TerminationActionV7] = []
            for action in requested:
                nonce = secrets.token_hex(16)
                payload = "\0".join(
                    (
                        self._runtime_id,
                        str(self._authority_epoch),
                        action,
                        nonce,
                    )
                )
                ...
                actions.append(item)
                self._termination_pending[token] = item
            self._termination_plan = TerminationPlanV7(
                self._runtime_id,
                self._authority_epoch,
                reason.value,
                tuple(actions),
    ...
    def termination_plan(self) -> TerminationPlanV7 | None:
        """Return the original immutable plan, including acknowledged actions."""
        with self._lock:
            return self._termination_plan

    def pending_termination_actions(self) -> tuple[TerminationActionV7, ...]:
        """Return the bounded outstanding host-owned actions in plan order."""
        with self._lock:
            return tuple(self._termination_pending.values())

    def _validate_termination_token_locked(self, token: object) -> TerminationActionV7:
        if type(token) is not str or not 1 <= len(token) <= 256:
            raise Phase5RuntimeV7ContractError("termination token is invalid")
        if token in self._termination_acknowledged:
            raise Phase5RuntimeV7ContractError("termination token was already used")
        item = self._termination_pending.get(token)
        if item is None:
            raise Phase5RuntimeV7ContractError("termination token is unknown")
        parts = token.split(".")
        if len(parts) != 5:
            raise Phase5RuntimeV7ContractError("termination token is malformed")
        ...
        if (
            ...
            raise Phase5RuntimeV7ContractError("termination token binding is invalid")
        return item
    ...
    def await_termination(self, timeout: float | None = None) -> RuntimeStatusV7:
        """Optionally wait once on the host acknowledgement condition; no poll."""
        if timeout is None:
        ...
        if (
            ...
            raise Phase5RuntimeV7ContractError("termination timeout is invalid")
        timeout = float(timeout)
        with self._terminal_condition:
            if not self._terminated or self._termination_complete:
                return self._status_locked()
            self._terminal_condition.wait(timeout)
    ...
    def kill(self) -> RuntimeStatusV7:
        return self.terminate(TerminalReasonV7.KILL)

    def revoke(self) -> RuntimeStatusV7:
        return self.terminate(TerminalReasonV7.REVOKE)

    def rollback(self) -> RuntimeStatusV7:
        return self.terminate(TerminalReasonV7.ROLLBACK)

    def end_session(self) -> RuntimeStatusV7:
        return self.terminate(TerminalReasonV7.END_SESSION)

    def reconnect(self) -> RuntimeStatusV7:
        return self.terminate(TerminalReasonV7.RECONNECT)

    def shutdown(self) -> RuntimeStatusV7:
        return self.terminate(TerminalReasonV7.SHUTDOWN)
...
__all__ = [
    ...
    "MAX_FANOUT",
    "ActionRequestV7",
    "ApprovalInboxReaderV7",
    "Phase5RuntimeV7",
    "Phase5RuntimeV7ContractError",
    "Phase5RuntimeV7Error",
    "ProjectionBatchV7",
    "ProjectionItemV7",
    "ProjectionPageV7",
    "RuntimeBindingV7",
