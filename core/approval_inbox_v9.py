"""Phase 5.2 V9 host-owned, read-only approval inbox projection.

V9 produces authenticated *review evidence*, never authority.  The host owns
...
class ApprovalInboxV9Error(RuntimeError):
    """Base V9 failure."""


class ApprovalInboxV9ContractError(ValueError):
    """Caller or host data violated the bounded V9 contract."""


class ApprovalInboxV9Unavailable(ApprovalInboxV9Error):
    """Projection cannot safely produce a result."""


class ApprovalInboxV9State(str, Enum):
    DISABLED = "DISABLED"
    READY = "READY"
    STALE = "STALE"
    REVOKED = "REVOKED"
...
def _bounded_int(value: object, label: str, minimum: int, maximum: int) -> int:
    if type(value) is not int or not minimum <= value <= maximum:
        raise ApprovalInboxV9ContractError(f"{label} is out of bounds")
    return value


def _bool(value: object, label: str) -> bool:
    if type(value) is not bool:
        raise ApprovalInboxV9ContractError(f"{label} is not boolean")
    return value
...
def _safe_text(value: object, label: str, maximum: int = MAX_TEXT) -> str:
    if (
        ...
        raise ApprovalInboxV9ContractError(f"{label} is invalid")
    if any(ord(character) < 32 or ord(character) == 127 for character in value):
        raise ApprovalInboxV9ContractError(f"{label} contains control characters")
    if _SECRET.search(value):
        raise ApprovalInboxV9ContractError(f"{label} contains secret-like material")
    return value
...
def _safe_id(value: object, label: str) -> str:
    ...
    if not _ID.fullmatch(text):
        raise ApprovalInboxV9ContractError(f"{label} is not canonical")
    return text
...
def _digest(value: object, label: str) -> str:
    if (
        ...
        raise ApprovalInboxV9ContractError(f"{label} is not a canonical digest")
    return value
...
class ApprovalInboxFeatureGateV9:
    ...
    def __init__(
        ...
        *,
        ...
        if not callable(epoch_reader):
            raise ApprovalInboxV9ContractError("epoch_reader is required")
        source = os.environ if environ is None else environ
        raw = source.get(APPROVAL_INBOX_V9_FLAG, "")
        if type(raw) is not str:
            raise ApprovalInboxV9ContractError("feature flag is not text")
    ...
    def _read_epoch(self) -> int:
        try:
            ...
            raise ApprovalInboxV9Unavailable("feature-epoch-unavailable") from None
    ...
    def state(self) -> ApprovalInboxV9State:
        with self._lock:
            if self._revoked:
                return ApprovalInboxV9State.REVOKED
            if not self._enabled:
                return ApprovalInboxV9State.DISABLED
        ...
        with self._lock:
            ...
            if current != self._boot_epoch:
                self._revoked = True
                return ApprovalInboxV9State.REVOKED
            return ApprovalInboxV9State.READY
    ...
    def _publication_barrier(self):
        ...
        with self._lock:
            if self._revoked:
                raise ApprovalInboxV9Unavailable("feature-epoch-revoked")
            if not self._enabled:
                raise ApprovalInboxV9Unavailable("disabled")
        current = self._read_epoch()
        with self._lock:
    ...
    def now_ms(self) -> int:
        ...
        with self._lock:
            if self._failed or current < self._high_water:
                self._failed = True
                raise ApprovalInboxV9Unavailable("monotonic-clock-rollback")
            self._high_water = current
            return current


class DeterministicInboxClockV9:
    ...
    def advance(self, milliseconds: int) -> None:
        ...
        with self._lock:
            if self._current > MAX_INTEGER - delta:
                raise ApprovalInboxV9ContractError("clock overflow")
            self._current += delta
...
@dataclass(frozen=True, slots=True)
class HostInboxSnapshotV9:
    source_epoch: object
    captured_at_ms: object
    items: object


class HostInboxSourceV9:
    ...
    def __init__(
        ...
        *,
        principal_id: str,
        session_id: str,
        capture_callback: Callable[[], HostInboxSnapshotV9],
    ) -> None:
        ...
        if not callable(capture_callback):
            raise ApprovalInboxV9ContractError("capture_callback is required")
    ...
    def capture(self) -> HostInboxSnapshotV9:
        if not self._semaphore.acquire(blocking=False):
            raise ApprovalInboxV9Unavailable("source-single-flight-busy")
        try:
            failed = False
            try:
            ...
            if failed:
                raise ApprovalInboxV9Unavailable("source-callback-failed")
            if type(result) is not HostInboxSnapshotV9:
                raise ApprovalInboxV9ContractError(
...
class InboxPageV9:
    ...
    page_size: int
    offset: int
    items: tuple[ApprovalReviewItemV9, ...]
...
@dataclass(frozen=True, slots=True)
class ReviewSelectionHandoffV9:
...
class ApprovalInboxProjectionV9:
    ...
    def __init__(
        ...
        *,
        ...
        clock: MonotonicInboxClockV9 | DeterministicInboxClockV9,
    ) -> None:
        if type(gate) is not ApprovalInboxFeatureGateV9:
            raise ApprovalInboxV9ContractError("exact feature gate required")
        if type(source) is not HostInboxSourceV9:
            raise ApprovalInboxV9ContractError("exact source required")
        if type(clock) not in {MonotonicInboxClockV9, DeterministicInboxClockV9}:
            raise ApprovalInboxV9ContractError("exact monotonic clock required")
        self._gate = gate
        self._source = source
        self._clock = clock
    ...
    def state(self) -> ApprovalInboxV9State:
        with self._lock:
            if self._state in {
                ApprovalInboxV9State.STALE,
                ApprovalInboxV9State.REVOKED,
                ApprovalInboxV9State.INTEGRITY_LATCHED,
            }:
                return self._state
        try:
            with self._gate._publication_barrier():
                with self._lock:
                    ...
                    try:
                        self._clock.now_ms()
                    except ApprovalInboxV9Error:
                        self._state = ApprovalInboxV9State.INTEGRITY_LATCHED
                        self._source_generation += 1
                        self._snapshots.clear()
                        self._views.clear()
                        return self._state
                    self._state = ApprovalInboxV9State.READY
                    return self._state
        except ApprovalInboxV9Unavailable as error:
            with self._lock:
                if str(error) == "disabled":
                    self._state = ApprovalInboxV9State.DISABLED
                elif self._state not in {
                    ApprovalInboxV9State.STALE,
                    ApprovalInboxV9State.REVOKED,
                    ApprovalInboxV9State.INTEGRITY_LATCHED,
                }:
                    self._state = ApprovalInboxV9State.REVOKED
                    self._source_generation += 1
                self._snapshots.clear()
                self._views.clear()
                return self._state

    def _terminal(self, state: ApprovalInboxV9State, code: str) -> None:
        with self._lock:
            self._state = state
            self._source_generation += 1
            self._snapshots.clear()
            self._views.clear()
        raise ApprovalInboxV9Unavailable(code)
            ...
            with self._gate._publication_barrier():
                with self._lock:
                    if self._state in {
                        ...
                        raise ApprovalInboxV9Unavailable(self._state.value.casefold())
                    try:
                    ...
                    if record is not None:
                        if (
                            ...
                            raise ApprovalInboxV9Unavailable("source-epoch-superseded")
                        if now > record.deadline_ms:
                            self._terminal(ApprovalInboxV9State.STALE, "snapshot-stale")
                    if view is not None and (
                        self._views.get(view.view_digest) is not view
                        or record is None
                        or view.snapshot_id != record.snapshot_id
                    ):
                        raise ApprovalInboxV9Unavailable("view-state-unavailable")
                    self._state = ApprovalInboxV9State.READY
                    yield now
        except ApprovalInboxV9Unavailable as error:
            code = str(error)
    ...
    def _normalize_item(self, raw: HostInboxItemV9) -> dict[str, object]:
        if type(raw) is not HostInboxItemV9:
            raise ApprovalInboxV9ContractError("source item has wrong type")
        ...
        if (
            ...
            raise ApprovalInboxV9ContractError("source identity mismatch")
        ...
        if not _CURRENCY.fullmatch(currency):
            raise ApprovalInboxV9ContractError("currency is invalid")
        ...
        if expires <= created:
            raise ApprovalInboxV9ContractError("item expiry is not after creation")
    ...
    def _capture(self, now: int) -> _SnapshotRecord:
        ...
        if type(raw.items) is not tuple or len(raw.items) > MAX_ITEMS:
            raise ApprovalInboxV9ContractError("snapshot items are invalid")
        items = tuple(self._normalize_item(item) for item in raw.items)
        ids = [str(item["item_id"]) for item in items]
        if len(ids) != len(set(ids)):
            raise ApprovalInboxV9ContractError("duplicate item_id")
        semantic = _sha({"contract": "HostInboxSemantic.v9", "items": items})
        ...
        with self._publication() as published_now:
            ...
            if previous is not None and not hmac.compare_digest(previous, semantic):
                self._state = ApprovalInboxV9State.INTEGRITY_LATCHED
            if self._state is ApprovalInboxV9State.INTEGRITY_LATCHED:
                self._snapshots.clear()
                self._views.clear()
                raise ApprovalInboxV9Unavailable("source-integrity-latched")
            ...
            for record in self._snapshots.values():
                if record.source_epoch == epoch and hmac.compare_digest(
                    ...
                    if now > record.deadline_ms:
                        self._state = ApprovalInboxV9State.STALE
                        self._snapshots.clear()
                        self._views.clear()
                        raise ApprovalInboxV9Unavailable("snapshot-stale")
                    return record
            if now > MAX_INTEGER - MAX_SNAPSHOT_AGE_MS:
                self._state = ApprovalInboxV9State.INTEGRITY_LATCHED
                raise ApprovalInboxV9Unavailable("deadline-overflow")
            snapshot_id = _sha(
                {
                    "contract": "ApprovalInboxSnapshot.v9",
                    "epoch": epoch,
    ...
    def _token(self, kind: str, payload: dict[str, object]) -> str:
        body = _canonical({"kind": kind, "schema": INBOX_V9_SCHEMA_VERSION, **payload})
    ...
    def _untoken(self, token: object, kind: str) -> dict[str, object]:
        if (
            ...
            raise ApprovalInboxV9ContractError("token is malformed")
        ...
        try:
            ...
            raise ApprovalInboxV9ContractError("token is malformed") from None
        expected = hmac.new(self._hmac_key, body, hashlib.sha256).hexdigest()
        if not hmac.compare_digest(mac, expected):
            raise ApprovalInboxV9ContractError("token authentication failed")
        if (
            type(decoded) is not dict
            or decoded.get("kind") != kind
            or decoded.get("schema") != 9
        ):
            raise ApprovalInboxV9ContractError("token relation is invalid")
        return decoded
    ...
    def _review_item(
        ...
        if prior is not None and not hmac.compare_digest(prior, proof):
            self._terminal(
                ApprovalInboxV9State.INTEGRITY_LATCHED, "review-proof-equivocation"
            )
        view.returned_proofs[item_id] = proof
        return ApprovalReviewItemV9(
            item_id=item_id,
    ...
    def _page(
        ...
        with self._publication(record, view):
            ...
            if not 0 <= offset <= len(ordered) or (offset and offset % view.page_size):
                raise ApprovalInboxV9ContractError("cursor offset is invalid")
            ...
            return InboxPageV9(
                state=ApprovalInboxV9State.READY,
                snapshot_id=record.snapshot_id,
                snapshot_digest=record.semantic_digest,
                view_digest=view.view_digest,
    ...
    def open_page(
        self, *, query: InboxQueryV9 | None = None, page_size: int = 20
    ) -> InboxPageV9:
        now = self._guard()
        size = _bounded_int(page_size, "page_size", 1, MAX_PAGE_SIZE)
        if query is None:
            query = InboxQueryV9()
        if type(query) is not InboxQueryV9:
            raise ApprovalInboxV9ContractError("exact query type required")
        ...
        with self._publication(record) as now:
            ...
            if view is None:
                ...
                self._terminal(
                    ApprovalInboxV9State.INTEGRITY_LATCHED, "view-digest-collision"
                )
        return self._page(record, view, 0, now)

    def continue_page(self, cursor: object) -> InboxPageV9:
        now = self._guard()
        ...
        if type(snapshot_id) is not str or type(view_digest) is not str:
            raise ApprovalInboxV9ContractError("cursor relation is invalid")
        with self._lock:
            record = self._snapshots.get(snapshot_id)
            view = self._views.get(view_digest)
            if record is None or view is None or view.snapshot_id != snapshot_id:
                raise ApprovalInboxV9Unavailable("cursor-state-unavailable")
            if decoded.get("page_size") != view.page_size:
                raise ApprovalInboxV9ContractError("cursor page size mismatch")
            if decoded.get("item_ids_digest") != _sha(view.item_ids):
                raise ApprovalInboxV9ContractError("cursor item set mismatch")
            if decoded.get("deadline_ms") != record.deadline_ms:
                raise ApprovalInboxV9ContractError("cursor deadline mismatch")
            offset = _bounded_int(
    ...
    def _resolve_selections(
        ...
        now: int,
    ) -> tuple[_SnapshotRecord, _ViewRecord, tuple[ApprovalReviewItemV9, ...]]:
        ...
        if record is None or view is None or view.snapshot_id != snapshot_key:
            raise ApprovalInboxV9Unavailable("selection-state-unavailable")
        with self._publication(record, view):
            item_by_id = {str(item["item_id"]): item for item in record.items}
            resolved: list[ApprovalReviewItemV9] = []
            for item_id, proof in normalized:
                ...
                if item is None or item_id not in view.item_ids:
                    raise ApprovalInboxV9ContractError("selected item is unavailable")
                if (
        ...
        for label, values in {
            ...
            if len(values) != len(set(values)):
                raise ApprovalInboxV9ContractError(f"duplicate {label}")
    ...
    def preview_calm_batch(
        ...
        payload = {
            "contract": "CalmBatchReviewPreview.v9",
            "snapshot_id": record.snapshot_id,
            "view_digest": view.view_digest,
    ...
    def handoff_calm_batch(
        ...
        payload = {
            "contract": "ReviewSelectionHandoff.v9",
            "snapshot_id": record.snapshot_id,
            "view_digest": view.view_digest,
            "item_ids": item_ids,
        ...
        with self._publication(record, view):
            return ReviewSelectionHandoffV9(
                snapshot_id=record.snapshot_id,
                view_digest=view.view_digest,
