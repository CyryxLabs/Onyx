"""
Onyx — optional engine dependency installer.

Call ``install_for_config`` from a setup or reconfiguration flow when enabling
optional local STT modules. Speech output is exclusively Gemini Live; local or
system TTS packages are never installed as a fallback. The primary Gemini Live
runtime uses the packages declared in ``requirements.txt``.
"""
...
def install_for_config(config: dict, log: Callable | None = None) -> None:
    ...
    tts = str(config.get("tts_engine", "gemini-live")).strip().lower()
    if tts not in {"", "gemini-live", "gemini_live"}:
        raise ValueError(
            "Local/system TTS is disabled; Onyx speech output is Gemini Live only"
        )
