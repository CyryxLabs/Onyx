"""Phase 5.3 Capability Nexus V5: isolated, default-off contracts.

V5 supersedes the rejected V1 and V2 candidates without importing them. The module has
...
CAPABILITY_SCHEMA_VERSION_V5 = "onyx.capability.v5"
NEXUS_CONTRACT_VERSION_V5 = "onyx.capability-nexus.v5"
CATALOG_CONTRACT_VERSION_V5 = "onyx.local-catalog.v5"
LOCK_ORDER_V5 = ("gate_immutable_state", "registry_lock", "adapter_lock")


class CapabilityV5ContractError(ValueError):
    pass


class CapabilityV5DeniedError(PermissionError):
    pass


class OperationKindV5(str, Enum):
    READ = "read"
    DRAFT = "draft"
    MUTATE = "mutate"
    VERIFY = "verify"
...
class CapabilityStatusV5(str, Enum):
    DISABLED = "disabled"
...
class TransportKindV5(str, Enum):
    LEGACY = "legacy"
    LOCAL = "local"
    API = "api"
    CLI = "cli"
...
class ReadFailureClassV5(str, Enum):
    NONE = "none"
    AUTH = "auth"
    SCOPE = "scope"
    DISABLED = "disabled"
...
def _raw_credential_canary(value: str) -> bool:
    ...
    for match in _BASIC_AUTH.finditer(value):
        ...
        if 3 <= len(decoded) <= 1024 and b":" in decoded and all(32 <= byte < 127 for byte in decoded):
            return True
    opaque = re.fullmatch(r"[A-Za-z0-9_-]{40,512}", value)
    if opaque is None or re.fullmatch(r"[0-9A-Fa-f]{64}", value):
        return False
    classes = sum(
        any(predicate(ch) for ch in value)
        for predicate in (str.islower, str.isupper, str.isdigit, lambda ch: ch in "_-")
    )
    if classes < 4:
        return False
...
def _scan_metadata_representation(value: str) -> None:
    ...
    if any(
        ...
        raise CapabilityV5ContractError("metadata contains unsupported invisible/bidi/combining characters")
    if any(ch in _STRUCTURED_METADATA for ch in normalized):
        scripts = {_letter_script(ch) for ch in normalized}
        scripts.discard(None)
        if len(scripts) > 1:
            raise CapabilityV5ContractError("structured metadata mixes Unicode scripts")
    ...
    if (
        ...
        or _raw_credential_canary(normalized)
        or _raw_credential_canary(skeleton)
    ):
        raise CapabilityV5ContractError("metadata representation contains secret material")
    if _RESIDUAL_UNICODE_ESCAPE.search(normalized):
        raise CapabilityV5ContractError("metadata contains residual character encoding")
...
def _canonical_percent_decode(value: str) -> str | None:
    ...
    if _PERCENT.sub("", value).find("%") >= 0:
        raise CapabilityV5ContractError("metadata contains malformed percent encoding")
    try:
        decoded = unquote_to_bytes(value).decode("utf-8", errors="strict")
    except (UnicodeDecodeError, ValueError) as exc:
        raise CapabilityV5ContractError("metadata percent encoding is invalid") from exc
    ...
    if normalize_hex(canonical) != normalize_hex(value):
        raise CapabilityV5ContractError("metadata percent encoding is noncanonical")
...
def _canonical_base64_decode(value: str) -> str | None:
    ...
    if ("+" in compact or "/" in compact) and ("-" in compact or "_" in compact):
        raise CapabilityV5ContractError("metadata Base64 alphabet is ambiguous")
    ...
    if canonical.rstrip("=") != compact.rstrip("="):
        raise CapabilityV5ContractError("metadata Base64 encoding is noncanonical")
...
def _validate_metadata_decoding_closure(value: str) -> None:
    ...
    if encoded_size > _MAX_METADATA_VALUE_BYTES:
        raise CapabilityV5ContractError("metadata value exceeds decoding byte budget")
    ...
    while queue:
        ...
        if current in ancestors:
            raise CapabilityV5ContractError("metadata decoding cycle detected")
        seen.add(current)
        if len(seen) > _MAX_METADATA_REPRESENTATIONS:
            raise CapabilityV5ContractError("metadata decoding representation budget exceeded")
        total_bytes += len(current.encode("utf-8"))
        if total_bytes > _MAX_METADATA_TOTAL_BYTES:
            raise CapabilityV5ContractError("metadata decoding expansion budget exceeded")
        ...
        if depth >= _MAX_METADATA_DECODE_DEPTH and candidates:
            raise CapabilityV5ContractError("metadata decoding depth exceeded")
...
def _text(value: object, label: str, maximum: int, *, empty: bool = False) -> str:
    if type(value) is not str or value != value.strip() or (not value and not empty):
        raise CapabilityV5ContractError(f"{label} must be a canonical exact string")
    if len(value) > maximum or any(
        (ord(ch) < 32 and ch not in {"\t", "\n", "\r"}) or ord(ch) == 127 for ch in value
    ):
        raise CapabilityV5ContractError(f"{label} is out of bounds")
    if _secret_canary(value):
        raise CapabilityV5ContractError(f"{label} resembles secret material")
    return value
...
def _slug(value: object, label: str) -> str:
    ...
    if not _SLUG.fullmatch(result):
        raise CapabilityV5ContractError(f"{label} is not a canonical slug")
    return result
...
def _identifier(value: object, label: str) -> str:
    ...
    if not _IDENT.fullmatch(result):
        raise CapabilityV5ContractError(f"{label} is not a canonical identifier")
    return result
...
def _strings(value: object, label: str, *, count: int = 32, size: int = 128, required: bool = False) -> tuple[str, ...]:
    if type(value) not in {tuple, list} or len(value) > count or (required and not value):
        raise CapabilityV5ContractError(f"{label} cardinality is invalid")
    result = tuple(_text(item, f"{label} item", size) for item in value)
    if len(set(result)) != len(result):
        raise CapabilityV5ContractError(f"{label} contains duplicates")
...
def _reject_json_secrets(
    ...
    *,
    ...
    if _depth > _MAX_JSON_DEPTH:
        raise CapabilityV5ContractError(f"{label} exceeds nesting depth")
    active = set() if _active is None else _active
    budget = [0, 0] if _budget is None else _budget
    budget[0] += 1
    if budget[0] > _MAX_JSON_NODES:
        raise CapabilityV5ContractError(f"{label} exceeds node budget")
    if isinstance(value, Mapping):
        if len(value) > 512:
            raise CapabilityV5ContractError(f"{label} is too large")
        identity = id(value)
        if identity in active:
            raise CapabilityV5ContractError(f"{label} contains a cycle")
        active.add(identity)
        try:
            for key, item in value.items():
                key = _text(key, f"{label} key", 128)
                if not _JSON_FIELD.fullmatch(key) or not key.isascii():
                    raise CapabilityV5ContractError(f"{label} key is not a canonical ASCII identifier")
                if _normalized_name(key) in _SECRET_NAMES:
                    raise CapabilityV5ContractError(f"{label} contains a secret-name field")
        ...
        if budget[1] > _MAX_JSON_SCALAR_BYTES:
            raise CapabilityV5ContractError(f"{label} exceeds scalar byte budget")
        _validate_metadata_decoding_closure(item)
    elif value is not None:
        if type(value) not in {bool, int, float}:
            raise CapabilityV5ContractError(f"{label} contains an unsupported value")
        if type(value) is float and not math.isfinite(value):
            raise CapabilityV5ContractError(f"{label} contains a non-finite number")
        if type(value) is int and not -(2**63) <= value <= 2**63 - 1:
            raise CapabilityV5ContractError(f"{label} integer is out of bounds")
...
def _json(value: object, label: str, maximum: int = 65536) -> str:
    ...
    try:
        ...
        raise CapabilityV5ContractError(f"{label} is not canonical JSON") from exc
    if len(result.encode("utf-8")) > maximum:
        raise CapabilityV5ContractError(f"{label} exceeds its byte budget")
    return result
...
def _safe_metadata(value: object) -> tuple[tuple[str, str], ...]:
    if type(value) not in {tuple, list} or len(value) > len(_SAFE_METADATA_KEYS):
        raise CapabilityV5ContractError("metadata must be bounded key/value pairs")
    result: list[tuple[str, str]] = []
    for pair in value:
        if type(pair) not in {tuple, list} or len(pair) != 2:
            raise CapabilityV5ContractError("metadata entry is invalid")
        key = _text(pair[0], "metadata key", 64)
        if _normalized_name(key) in _SECRET_NAMES or key not in _SAFE_METADATA_KEYS:
            raise CapabilityV5ContractError("metadata key is not declared safe")
        item = _text(pair[1], f"metadata {key}", 512)
        _validate_metadata_decoding_closure(item)
        if key == "declaration_sha256" and not _HEX64.fullmatch(item):
            raise CapabilityV5ContractError("declaration_sha256 is invalid")
        result.append((key, item))
    if len({key for key, _ in result}) != len(result):
        raise CapabilityV5ContractError("metadata key is duplicated")
    return tuple(sorted(result))


@dataclass(frozen=True, slots=True)
class OperationDescriptorV5:
    operation_id: str
    kind: OperationKindV5
    description: str
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.kind) is not OperationKindV5:
            raise CapabilityV5ContractError("operation kind must be exact")
        object.__setattr__(self, "description", _text(self.description, "description", 4096))
        if type(self.parameter_schema_json) is not str:
            raise CapabilityV5ContractError("parameter schema must be exact string")
        try:
            parsed = json.loads(self.parameter_schema_json)
        except json.JSONDecodeError as exc:
            raise CapabilityV5ContractError("parameter schema is invalid") from exc
        if _json(parsed, "parameter schema") != self.parameter_schema_json:
            raise CapabilityV5ContractError("parameter schema is noncanonical")
        ...
        if any(not _DOMAIN.fullmatch(item) or ".." in item for item in domains):
            raise CapabilityV5ContractError("allowed domain is invalid")
...
@dataclass(frozen=True, slots=True)
class CapabilityDescriptorV5:
    capability_id: str
    capability_version: str
    provider: str
    transport: TransportKindV5
    api_name: str
    api_version: str
    workspace_id: str
    account_id: str
    ...
    operations: tuple[OperationDescriptorV5, ...]
    status: CapabilityStatusV5 = CapabilityStatusV5.DISABLED
    status_reason: str = "candidate_not_activated"
    limitations: tuple[str, ...] = ()
    license_review: str = "not_required_local_only"
    metadata: tuple[tuple[str, str], ...] = ()
    schema_version: str = CAPABILITY_SCHEMA_VERSION_V5

    def __post_init__(self) -> None:
        if self.schema_version != CAPABILITY_SCHEMA_VERSION_V5:
            raise CapabilityV5ContractError("unknown descriptor schema")
        for name in ("capability_id", "capability_version", "provider", "api_name", "api_version"):
            object.__setattr__(self, name, _slug(getattr(self, name), name))
        if type(self.transport) is not TransportKindV5 or type(self.status) is not CapabilityStatusV5:
            raise CapabilityV5ContractError("descriptor enum type is invalid")
        ...
        if self.credential_alias is not None and (
            ...
            raise CapabilityV5ContractError("credential_alias must be opaque")
        if type(self.operations) not in {tuple, list} or not self.operations or len(self.operations) > 64:
            raise CapabilityV5ContractError("operation cardinality is invalid")
        operations = tuple(self.operations)
        if any(type(item) is not OperationDescriptorV5 for item in operations):
            raise CapabilityV5ContractError("operation descriptor type is invalid")
        if len({item.operation_id for item in operations}) != len(operations):
            raise CapabilityV5ContractError("duplicate operation")
        ...
        if self.status is CapabilityStatusV5.DEGRADED and self.status_reason == "healthy":
            raise CapabilityV5ContractError("degraded descriptor requires a reason")
        if self.transport is TransportKindV5.BROWSER and dict(self.metadata).get("fallback_class") != "explicit_browser_fallback":
            raise CapabilityV5ContractError("browser fallback is not explicit")
...
@dataclass(frozen=True, slots=True)
class NexusFeatureGateV5:
    nexus_enabled: bool = False
    shadow_mode: bool = True
    dispatch_enabled: bool = False
    contract_version: str = NEXUS_CONTRACT_VERSION_V5

    def __post_init__(self) -> None:
        if type(self.nexus_enabled) is not bool or type(self.shadow_mode) is not bool or type(self.dispatch_enabled) is not bool:
            raise CapabilityV5ContractError("gate values must be exact booleans")
        if self.contract_version != NEXUS_CONTRACT_VERSION_V5:
            raise CapabilityV5ContractError("gate contract version is invalid")
        if self.nexus_enabled or not self.shadow_mode or self.dispatch_enabled:
            raise CapabilityV5ContractError("V5 is strictly default-off and shadow-only")


@dataclass(frozen=True, slots=True)
class CapabilityProjectionV5:
    descriptor: CapabilityDescriptorV5
    descriptor_digest: str
    workspace_id: str
    account_id: str
    profile_id: str
    projection_status: CapabilityStatusV5
    projection_reason: str
    runtime_available: bool = False
    authority_granted: bool = False

    def __post_init__(self) -> None:
        if type(self.descriptor) is not CapabilityDescriptorV5 or self.descriptor_digest != self.descriptor.digest:
            raise CapabilityV5ContractError("projection descriptor binding drift")
        if (
            ...
            raise CapabilityV5ContractError("projection identity binding drift")
        if type(self.projection_status) is not CapabilityStatusV5:
            raise CapabilityV5ContractError("projection status is invalid")
        ...
        if type(self.runtime_available) is not bool or type(self.authority_granted) is not bool:
            raise CapabilityV5ContractError("projection booleans must be exact")
        if self.runtime_available or self.authority_granted:
            raise CapabilityV5ContractError("V5 projections cannot convey authority")
...
@dataclass(frozen=True, slots=True)
class CapabilitySnapshotV5:
    workspace_id: str
    account_id: str
    profile_id: str
    entries: tuple[CapabilityProjectionV5, ...]
    snapshot_digest: str
    contract_version: str = NEXUS_CONTRACT_VERSION_V5

    def __post_init__(self) -> None:
        if self.contract_version != NEXUS_CONTRACT_VERSION_V5:
            raise CapabilityV5ContractError("snapshot contract version is invalid")
        for name in ("workspace_id", "account_id", "profile_id"):
            object.__setattr__(self, name, _identifier(getattr(self, name), name))
        if type(self.entries) is not tuple or any(type(item) is not CapabilityProjectionV5 for item in self.entries):
            raise CapabilityV5ContractError("snapshot entries are invalid")
        if any(
            ...
            raise CapabilityV5ContractError("snapshot contains a cross-profile projection")
        expected = _sha(
            {
                "account_id": self.account_id,
        ...
        if self.snapshot_digest != expected:
            raise CapabilityV5ContractError("snapshot digest is invalid")


class CapabilityNexusV5:
    """Descriptor-only registry; lock order is gate immutable -> registry."""

    def __init__(self, gate: NexusFeatureGateV5 | None = None) -> None:
        if gate is None:
            gate = NexusFeatureGateV5()
        if type(gate) is not NexusFeatureGateV5:
            raise CapabilityV5ContractError("registry requires the exact concrete V5 gate")
        self._gate = gate
        self._descriptors: dict[str, CapabilityDescriptorV5] = {}
        self._revoked_aliases: set[str] = set()
        self._kill = False
        self._lock = threading.RLock()

    @property
    def gate(self) -> NexusFeatureGateV5:
        return self._gate

    @property
    def lock_order(self) -> tuple[str, ...]:
        return LOCK_ORDER_V5

    def register(self, descriptor: CapabilityDescriptorV5) -> str:
        if type(descriptor) is not CapabilityDescriptorV5:
            raise CapabilityV5ContractError("registry descriptor type is invalid")
        with self._lock:
            if descriptor.capability_id in self._descriptors:
                raise CapabilityV5ContractError("duplicate capability")
    ...
    def revoke_alias(self, alias: str) -> None:
        if type(alias) is not str or not _ALIAS.fullmatch(alias):
            raise CapabilityV5ContractError("credential alias is invalid")
        with self._lock:
            self._revoked_aliases.add(alias)

    def set_kill(self, active: bool) -> None:
        if type(active) is not bool:
            raise CapabilityV5ContractError("kill state must be exact boolean")
        with self._lock:
            self._kill = active
    ...
    def discover(
        ...
        *,
        ...
        if not _HEX64.fullmatch(expected_digest):
            raise CapabilityV5ContractError("expected digest is invalid")
        with self._lock:
            descriptor = self._descriptors.get(capability_id)
            if descriptor is None:
                raise CapabilityV5DeniedError("unknown capability")
            if (
                descriptor.workspace_id != workspace_id
                or descriptor.account_id != account_id
                or descriptor.profile_id != profile_id
            ):
                raise CapabilityV5DeniedError("workspace/account/profile mismatch")
            if descriptor.capability_version != expected_version or descriptor.digest != expected_digest:
                raise CapabilityV5DeniedError("version or descriptor drift")
            status, reason = self._status(descriptor)
            return CapabilityProjectionV5(
                descriptor, descriptor.digest, workspace_id, account_id, profile_id, status, reason
            )

    def health_projection(self, *args: object, **kwargs: object) -> CapabilityProjectionV5:
        return self.discover(*args, **kwargs)

    def snapshot(self, *, workspace_id: str, account_id: str, profile_id: str) -> CapabilitySnapshotV5:
        workspace_id = _identifier(workspace_id, "workspace_id")
        account_id = _identifier(account_id, "account_id")
        profile_id = _identifier(profile_id, "profile_id")
        with self._lock:
        ...
        payload = {
            ...
            "contract_version": NEXUS_CONTRACT_VERSION_V5,
            "entries": [item.digest_payload() for item in entries],
            "profile_id": profile_id,
            "workspace_id": workspace_id,
        }
        return CapabilitySnapshotV5(workspace_id, account_id, profile_id, entries, _sha(payload))

    def _status(self, descriptor: CapabilityDescriptorV5) -> tuple[CapabilityStatusV5, str]:
        if self._kill:
            return CapabilityStatusV5.DISABLED, "global_kill_active"
        if descriptor.credential_alias is not None and descriptor.credential_alias in self._revoked_aliases:
            return CapabilityStatusV5.BLOCKED_BY_ACCESS, "credential_alias_revoked"
        if descriptor.status is CapabilityStatusV5.AVAILABLE_READ_ONLY:
            return CapabilityStatusV5.DISABLED, "nexus_default_off_shadow_only"
        return descriptor.status, descriptor.status_reason


@dataclass(frozen=True, slots=True)
class LegacyParityRecordV5:
    tool_name: str
    host_policy: str
...
@dataclass(frozen=True, slots=True)
class LegacyDescriptorSetV5:
    descriptors: tuple[CapabilityDescriptorV5, ...]
    parity_records: tuple[LegacyParityRecordV5, ...]
    snapshot_digest: str
...
def build_legacy_descriptors_v5(
    ...
    *,
    ...
    account_id: str,
    profile_id: str,
    operation_kinds: Mapping[str, OperationKindV5] | None = None,
) -> LegacyDescriptorSetV5:
    if type(tool_declarations) not in {tuple, list} or not tool_declarations or len(tool_declarations) > 128:
        raise CapabilityV5ContractError("legacy declarations are invalid")
    if not isinstance(policy_mapping, Mapping):
        raise CapabilityV5ContractError("legacy policy mapping is invalid")
    workspace_id = _identifier(workspace_id, "workspace_id")
    account_id = _identifier(account_id, "account_id")
    profile_id = _identifier(profile_id, "profile_id")
    ...
    for raw in tool_declarations:
        if not isinstance(raw, Mapping) or set(raw) != {"name", "description", "parameters"}:
            raise CapabilityV5ContractError("legacy declaration shape drift")
        name = _slug(raw.get("name"), "tool name")
        if name in seen:
            raise CapabilityV5ContractError("duplicate legacy declaration")
        seen.add(name)
        _text(raw.get("description"), "legacy description", 4096)
        if not isinstance(raw.get("parameters"), Mapping):
            raise CapabilityV5ContractError("legacy parameter schema is invalid")
        encoded = _json(dict(raw), "legacy declaration")
        parsed.append((name, raw, encoded))
    if set(policy_mapping) != seen:
        raise CapabilityV5ContractError("legacy policy coverage is not exact")
    if operation_kinds is not None and set(operation_kinds) != seen:
        raise CapabilityV5ContractError("legacy operation coverage is not exact")
    descriptors: list[CapabilityDescriptorV5] = []
    records: list[LegacyParityRecordV5] = []
    for name, raw, encoded in parsed:
        policy = _slug(policy_mapping[name], "host policy")
        kind = OperationKindV5.MUTATE if operation_kinds is None else operation_kinds[name]
        if type(kind) is not OperationKindV5:
            raise CapabilityV5ContractError("legacy operation kind is invalid")
        digest = hashlib.sha256(encoded.encode()).hexdigest()
        operation = OperationDescriptorV5(
            name,
            kind,
            str(raw["description"]),
        ...
        descriptors.append(
            CapabilityDescriptorV5(
                f"legacy.{name}", "v5", "onyx_legacy", TransportKindV5.LEGACY,
                "legacy_dispatcher", "unchanged", workspace_id, account_id, profile_id, None,
                (operation,), CapabilityStatusV5.DISABLED, "shadow_descriptor_only",
        ...
        records.append(LegacyParityRecordV5(name, policy, encoded, digest))
    ...
    return LegacyDescriptorSetV5(tuple(descriptors), tuple(records), _sha(payload))
...
class CatalogReadRequestV5:
    ...
    timeout_ms: int = 1000
    cancellation: CancellationTokenV5 | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.page_size) is not int or not 1 <= self.page_size <= 100:
            raise CapabilityV5ContractError("page_size is invalid")
        ...
        if type(self.timeout_ms) is not int or not 1 <= self.timeout_ms <= 30_000:
            raise CapabilityV5ContractError("timeout_ms is invalid")
        if self.cancellation is not None and type(self.cancellation) is not CancellationTokenV5:
            raise CapabilityV5ContractError("cancellation token type is invalid")


@dataclass(frozen=True, slots=True)
class ConnectorHealthV5:
    status: CapabilityStatusV5
    reason: str
    workspace_id: str
    account_id: str
    profile_id: str
...
@dataclass(frozen=True, slots=True)
class CatalogReadProjectionV5:
    state: ReadStateV5
    failure_class: ReadFailureClassV5
    correlation_id: str
    request_digest: str
    workspace_id: str
    account_id: str
    profile_id: str
    items: tuple[CatalogItemV5, ...]
    ...
    def __post_init__(self) -> None:
        if type(self.state) is not ReadStateV5 or type(self.failure_class) is not ReadFailureClassV5:
            raise CapabilityV5ContractError("read projection enum is invalid")
        if self.authority_granted or type(self.authority_granted) is not bool:
            raise CapabilityV5ContractError("read projection cannot grant authority")
        if self.state is not ReadStateV5.COMPLETED and self.items:
            raise CapabilityV5ContractError("non-completed projection cannot expose items")
        if self.state is ReadStateV5.COMPLETED and (
            self.failure_class is not ReadFailureClassV5.NONE or self.receipt_digest is None
        ):
            raise CapabilityV5ContractError("completed projection is incomplete")


@dataclass(frozen=True, slots=True)
class _ReadLedgerEntryV5:
    request_digest: str
    workspace_id: str
    account_id: str
    profile_id: str
...
class LocalCatalogReadAdapterV5:
    ...
    def __init__(
        ...
        *,
        ...
        if type(items) not in {tuple, list} or len(items) > 1000:
            raise CapabilityV5ContractError("catalog items are invalid")
        copied = tuple(items)
        if any(type(item) is not CatalogItemV5 for item in copied) or len({item.item_id for item in copied}) != len(copied):
            raise CapabilityV5ContractError("catalog items must be exact and unique")
        ...
        if type(cursor_signing_key) is not bytes or not 32 <= len(cursor_signing_key) <= 128:
            raise CapabilityV5ContractError("cursor signing key is invalid")
        self._cursor_key = bytes(cursor_signing_key)
        if not callable(read_hook) or not callable(clock):
            raise CapabilityV5ContractError("hook/clock must be callable")
        ...
        if type(auth_available) is not bool:
            raise CapabilityV5ContractError("auth state must be exact boolean")
        self._auth = auth_available
        self._scopes = frozenset(_strings(granted_scopes, "granted_scopes"))
        if type(status) is not CapabilityStatusV5 or status not in {
            CapabilityStatusV5.DISABLED, CapabilityStatusV5.AVAILABLE_READ_ONLY, CapabilityStatusV5.DEGRADED
        }:
            raise CapabilityV5ContractError("adapter status is invalid")
        self._status = status
        self._status_reason = _slug(status_reason, "status_reason")
        if status is CapabilityStatusV5.DEGRADED and status_reason == "healthy":
            raise CapabilityV5ContractError("degraded status requires reason")
        for name, value, lower, upper in (
            ...
            if type(value) is not int or not lower <= value <= upper:
                raise CapabilityV5ContractError(f"{name} is invalid")
        ...
        if pending_capacity > uncertain_capacity or uncertain_capacity > ledger_capacity:
            raise CapabilityV5ContractError("pending/uncertain capacities exceed their parent bound")
        ...
        self._window_used = 0
        self._ledger: dict[str, _ReadLedgerEntryV5] = {}
        self._kill = False
        self._revoked = False
        self._lock = threading.RLock()
    ...
    @property
    def descriptor(self) -> CapabilityDescriptorV5:
        operation = OperationDescriptorV5(
            "catalog_read", OperationKindV5.READ, "Read allowlisted provider-free catalog metadata.",
        ...
        return CapabilityDescriptorV5(
            self.CAPABILITY_ID, "v5", "onyx_local", TransportKindV5.LOCAL, "local_catalog",
            CATALOG_CONTRACT_VERSION_V5, self._workspace_id, self._account_id, self._profile_id,
    ...
    def health(self) -> ConnectorHealthV5:
        now = self._clock()
        with self._lock:
            self._expire_pending(now)
            status, reason, _ = self._effective_status()
            pending = sum(item.state is ReadStateV5.PENDING for item in self._ledger.values())
            uncertain = sum(item.state is ReadStateV5.UNCERTAIN for item in self._ledger.values())
            return ConnectorHealthV5(
                status, reason, self._workspace_id, self._account_id, self._profile_id,
                CATALOG_CONTRACT_VERSION_V5, (self.REQUIRED_SCOPE,), tuple(sorted(self._scopes)),
                max(0, self._quota_limit - self._quota_used), len(self._ledger), pending, uncertain,
            )

    def read_page(self, request: CatalogReadRequestV5) -> CatalogReadProjectionV5:
        with self._hook_state_lock:
            if self._hook_active:
                raise CapabilityV5DeniedError("catalog read reentrancy is denied")
        if type(request) is not CatalogReadRequestV5:
            raise CapabilityV5ContractError("read request type is invalid")
        ...
        with self._lock:
            ...
            if prior is not None:
                if prior.request_digest != digest:
                    raise CapabilityV5DeniedError("correlation request drift")
                return self._projection(request.correlation_id, prior)
            if len(self._ledger) >= self._capacity:
                raise CapabilityV5DeniedError("correlation ledger capacity exhausted")
            if request.cancellation is not None and request.cancellation.cancelled:
                entry = self._terminal_without_reservation(
                    request, digest, ReadStateV5.CANCELLED_BEFORE_HOOK, ReadFailureClassV5.CANCELLED, started
                )
            ...
            if status not in {CapabilityStatusV5.AVAILABLE_READ_ONLY, CapabilityStatusV5.DEGRADED}:
                entry = self._terminal_without_reservation(request, digest, ReadStateV5.DENIED, failure, started)
                self._ledger[request.correlation_id] = entry
                return self._projection(request.correlation_id, entry)
            window = int(started)
            ...
            if self._window_used >= self._rate_limit:
                entry = self._terminal_without_reservation(
                    request, digest, ReadStateV5.RATE_LIMITED, ReadFailureClassV5.RATE_LIMIT, started, retry_after_ms=1000
                )
                self._ledger[request.correlation_id] = entry
                return self._projection(request.correlation_id, entry)
            if self._quota_used >= self._quota_limit:
                entry = self._terminal_without_reservation(
                    request, digest, ReadStateV5.DENIED, ReadFailureClassV5.QUOTA, started
                )
                self._ledger[request.correlation_id] = entry
                return self._projection(request.correlation_id, entry)
            pending_count = sum(item.state is ReadStateV5.PENDING for item in self._ledger.values())
            uncertain_count = sum(item.state is ReadStateV5.UNCERTAIN for item in self._ledger.values())
            if pending_count >= self._pending_capacity or pending_count + uncertain_count >= self._uncertain_capacity:
                raise CapabilityV5DeniedError("pending/uncertain ledger capacity exhausted")
            ...
            self._quota_used += 1
            self._window_used += 1
            entry = _ReadLedgerEntryV5(
                digest, self._workspace_id, self._account_id, self._profile_id,
                ReadStateV5.PENDING, ReadFailureClassV5.NONE, page, next_cursor, None, 1,
        ...
        if request.cancellation is not None and request.cancellation.cancelled:
            return self._finish_before_hook(
                request, digest, ReadStateV5.CANCELLED_BEFORE_HOOK, ReadFailureClassV5.CANCELLED, pre_hook_now
            )
        if (pre_hook_now - started) * 1000 >= request.timeout_ms:
            return self._finish_before_hook(
                request, digest, ReadStateV5.TIMEOUT_BEFORE_HOOK, ReadFailureClassV5.TIMEOUT, pre_hook_now
            )
        with self._lock:
            ...
            if status not in {CapabilityStatusV5.AVAILABLE_READ_ONLY, CapabilityStatusV5.DEGRADED}:
                return self._finish_before_hook_locked(
                    request.correlation_id, current, ReadStateV5.DENIED, failure, pre_hook_now
                )
            current = replace(current, hook_started=True)
            self._ledger[request.correlation_id] = current
        hook_failed = False
        with self._hook_state_lock:
            hook_busy = self._hook_active
            if not hook_busy:
                self._hook_active = True
        if hook_busy:
            return self._finish_before_hook(
                request, digest, ReadStateV5.DENIED, ReadFailureClassV5.UNKNOWN, self._clock()
            )
        try:
            self._read_hook()
        except Exception:
        ...
        if hook_failed:
            ...
            with self._lock:
                current = self._ledger[request.correlation_id]
                if current.state is not ReadStateV5.PENDING:
                    return self._projection(request.correlation_id, current)
                uncertain = replace(
                    current, state=ReadStateV5.UNCERTAIN, failure_class=ReadFailureClassV5.HOOK_EXCEPTION,
        ...
        with self._lock:
            ...
            status, _, failure = self._effective_status()
            if request.cancellation is not None and request.cancellation.cancelled:
                return self._finish_uncertain_locked(request.correlation_id, current, ReadFailureClassV5.CANCELLED, finished)
            if (finished - started) * 1000 >= request.timeout_ms:
                return self._finish_uncertain_locked(request.correlation_id, current, ReadFailureClassV5.TIMEOUT, finished)
            if status not in {CapabilityStatusV5.AVAILABLE_READ_ONLY, CapabilityStatusV5.DEGRADED}:
            ...
            receipt = _sha(
                {"account_id": current.account_id, "correlation_id": request.correlation_id,
                 ...
                 "state": ReadStateV5.COMPLETED.value, "workspace_id": current.workspace_id}
            )
            completed = replace(
                current, state=ReadStateV5.COMPLETED, failure_class=ReadFailureClassV5.NONE,
    ...
    def reconcile(self, request: CatalogReadRequestV5) -> CatalogReadProjectionV5:
        if type(request) is not CatalogReadRequestV5:
            raise CapabilityV5ContractError("reconcile request type is invalid")
        self._validate_binding(request)
        digest = self._request_digest(request)
        now = self._clock()
        with self._lock:
            ...
            if entry is None:
                return CatalogReadProjectionV5(
                    ReadStateV5.UNKNOWN_CORRELATION, ReadFailureClassV5.UNKNOWN,
    ...
    def draft(self, *_args: object, **_kwargs: object) -> None:
        raise CapabilityV5DeniedError("catalog adapter has no draft operation")

    def mutate(self, *_args: object, **_kwargs: object) -> None:
        raise CapabilityV5DeniedError("catalog adapter has no mutation operation")

    def _validate_binding(self, request: CatalogReadRequestV5) -> None:
        if (
        ...
        if request.target != self.TARGET:
            raise CapabilityV5DeniedError("target is not allowlisted")

    def _effective_status(self) -> tuple[CapabilityStatusV5, str, ReadFailureClassV5]:
        if self._kill:
            return CapabilityStatusV5.DISABLED, "global_kill_active", ReadFailureClassV5.KILL
        if self._revoked:
            return CapabilityStatusV5.BLOCKED_BY_ACCESS, "credential_alias_revoked", ReadFailureClassV5.REVOKED
        if not self._auth:
            return CapabilityStatusV5.BLOCKED_BY_ACCESS, "authentication_unavailable", ReadFailureClassV5.AUTH
        if self.REQUIRED_SCOPE not in self._scopes:
            return CapabilityStatusV5.BLOCKED_BY_SCOPE, "required_scope_missing", ReadFailureClassV5.SCOPE
        if self._status is CapabilityStatusV5.DISABLED:
            return self._status, self._status_reason, ReadFailureClassV5.DISABLED
        return self._status, self._status_reason, ReadFailureClassV5.NONE

    def _request_digest(self, request: CatalogReadRequestV5) -> str:
        return _sha(
    ...
    def _encode_cursor(self, offset: int) -> str:
        payload = {"account_id": self._account_id, "capability_id": self.CAPABILITY_ID,
                   "contract_version": CATALOG_CONTRACT_VERSION_V5, "offset": offset,
    ...
    def _decode_cursor(self, cursor: str) -> int:
        try:
            ...
            if not hmac.compare_digest(signature, expected):
                raise CapabilityV5DeniedError("cursor integrity failure")
            payload = json.loads(body)
        except (ValueError, UnicodeError, json.JSONDecodeError) as exc:
            raise CapabilityV5DeniedError("cursor integrity failure") from exc
        expected_payload = {"account_id": self._account_id, "capability_id": self.CAPABILITY_ID,
                            "contract_version": CATALOG_CONTRACT_VERSION_V5,
                            "profile_id": self._profile_id, "snapshot_digest": self._snapshot_digest,
                            "workspace_id": self._workspace_id}
        if set(payload) != set(expected_payload) | {"offset"} or any(payload.get(k) != v for k, v in expected_payload.items()):
            raise CapabilityV5DeniedError("cursor profile/snapshot binding drift")
        offset = payload.get("offset")
        if type(offset) is not int or not 0 <= offset < len(self._items):
            raise CapabilityV5DeniedError("cursor offset is invalid")
        return offset

    def _terminal_without_reservation(
        self, request: CatalogReadRequestV5, digest: str, state: ReadStateV5,
        failure: ReadFailureClassV5, now: float, *, retry_after_ms: int | None = None,
    ) -> _ReadLedgerEntryV5:
        return _ReadLedgerEntryV5(
    ...
    def _finish_before_hook(
        self, request: CatalogReadRequestV5, digest: str, state: ReadStateV5, failure: ReadFailureClassV5,
        now: float,
    ) -> CatalogReadProjectionV5:
        with self._lock:
            current = self._ledger[request.correlation_id]
    ...
    def _finish_before_hook_locked(
        self, correlation_id: str, current: _ReadLedgerEntryV5, state: ReadStateV5,
        failure: ReadFailureClassV5, now: float,
    ) -> CatalogReadProjectionV5:
    ...
    def _finish_uncertain_locked(
        self, correlation_id: str, current: _ReadLedgerEntryV5,
        failure: ReadFailureClassV5, now: float,
    ) -> CatalogReadProjectionV5:
        uncertain = replace(
            current, state=ReadStateV5.UNCERTAIN, failure_class=failure,
    ...
    def _release_reservation(self, entry: _ReadLedgerEntryV5) -> None:
    ...
    def _expire_pending(self, now: float) -> None:
        for correlation_id, entry in tuple(self._ledger.items()):
            if entry.state is ReadStateV5.PENDING and now >= entry.expires_at:
                uncertain = replace(
                    entry, state=ReadStateV5.UNCERTAIN,
                    failure_class=ReadFailureClassV5.EXPIRED_PENDING,
                    items=(), next_cursor=None, expires_at=now + self._uncertain_ttl,
                )
                self._ledger[correlation_id] = uncertain
            elif entry.state is ReadStateV5.UNCERTAIN and now >= entry.expires_at:
                self._ledger[correlation_id] = replace(
                    entry, state=ReadStateV5.EXPIRED_UNCERTAIN,
                    failure_class=ReadFailureClassV5.UNCERTAIN_EXPIRED,
                    items=(), next_cursor=None, expires_at=now,
                )

    def _projection(self, correlation_id: str, entry: _ReadLedgerEntryV5) -> CatalogReadProjectionV5:
        return CatalogReadProjectionV5(
            entry.state, entry.failure_class, correlation_id, entry.request_digest,
            entry.workspace_id, entry.account_id, entry.profile_id,
            entry.items if entry.state is ReadStateV5.COMPLETED else (),
            entry.next_cursor if entry.state is ReadStateV5.COMPLETED else None,
            entry.receipt_digest if entry.state is ReadStateV5.COMPLETED else None,
