"""Phase 5.3 Capability Nexus V12: isolated, default-off contracts.

V12 supersedes the rejected V1 and V2 candidates without importing them. The module has
...
CAPABILITY_SCHEMA_VERSION_V12 = "onyx.capability.v12"
NEXUS_CONTRACT_VERSION_V12 = "onyx.capability-nexus.v12"
CATALOG_CONTRACT_VERSION_V12 = "onyx.local-catalog.v12"
LOCK_ORDER_V12 = ("gate_immutable_state", "registry_lock", "adapter_lock")


class CapabilityV12ContractError(ValueError):
    pass


class CapabilityV12DeniedError(PermissionError):
    pass


class OperationKindV12(str, Enum):
    READ = "read"
    DRAFT = "draft"
    MUTATE = "mutate"
    VERIFY = "verify"
...
class CapabilityStatusV12(str, Enum):
    DISABLED = "disabled"
...
class TransportKindV12(str, Enum):
    LEGACY = "legacy"
    LOCAL = "local"
    API = "api"
    CLI = "cli"
...
class ReadFailureClassV12(str, Enum):
    NONE = "none"
    AUTH = "auth"
    SCOPE = "scope"
    DISABLED = "disabled"
...
_NATURAL_PREFIXES = (
    "auto", "character", "counter", "document", "govern", "inter", "intra",
    "international", "macro", "metadata", "micro", "multi", "network",
    "respons", "tele", "trans",
)
_NATURAL_SUFFIXES = (
    "abilities", "ability", "ibility", "ibilities", "alization", "arization",
    "ization", "isation", "ations", "ation", "itions", "ition", "ology",
    "ological", "ments", "ment", "ness", "ships", "ship", "ities", "ity",
    "ivity", "ances", "ance", "ences", "ence", "architecture", "disciplinary",
    "communications", "communication", "responsibilities", "responsibility",
    "tures", "ture", "aries", "ary", "ories", "ory", "ions", "ion", "ives",
    "ive", "als", "al", "ings", "ing", "ers", "er", "ly", "ous", "ful",
    "less", "ics", "ic", "ants", "ant", "ies",
)
_NATURAL_MORPHEMES = (
    "architect", "catalog", "character", "collabor", "configur", "document",
    "govern", "implement", "interoper", "metadata", "multidisciplin",
    "operat", "responsib", "standard", "telecommun", "workflow",
)
...
def _protected_name_canary(value: str) -> bool:
    ...
    def base_natural_word(part: str) -> bool:
        if not part.isalpha():
            return False
        if part in _NATURAL_SHORT_WORDS or part in _SAFE_SEMANTIC_PREFIXES:
            return True
        if len(part) < 7 or re.search(r"(.)\1{2}", part):
            return False
        vowel_groups = re.findall(r"[aeiouy]+", part)
        consonant_groups = re.findall(r"[^aeiouy]+", part)
        if len(vowel_groups) < 2 or max(map(len, consonant_groups), default=0) > 5:
            return False
        return (
            part.startswith(_NATURAL_PREFIXES)
            or part.endswith(_NATURAL_SUFFIXES)
            or any(morpheme in part for morpheme in _NATURAL_MORPHEMES)
        )

    def natural_word(part: str) -> bool:
        if base_natural_word(part):
            return True
        return any(
            base_natural_word(part[:split]) and base_natural_word(part[split:])
            for split in range(3, len(part) - 2)
        )

    def opaque_remainder(parts: tuple[str, ...]) -> bool:
        if identifier_context:
            opaque_parts = tuple(part for part in parts if part and not natural_word(part))
            return sum(map(len, opaque_parts)) >= 16
        # Whitespace-only text is prose-shaped. It is rejected only with one
        # long opaque lexical unit, multiple adjacent opaque chunks, or numeric
        # evidence. Natural words terminate a candidate run.
        run: list[str] = []
        for part in (*parts, ""):
            if part and not natural_word(part):
                run.append(part)
                continue
            if sum(map(len, run)) >= 16 and (
                len(run) >= 2
                or any(len(item) >= 16 for item in run)
                or any(any(character.isdigit() for character in item) for item in run)
            ):
                return True
            run = []
        return False
    ...
    for start in range(len(segments)):
        for end in range(start + 1, len(segments) + 1):
            ...
            if name in protected:
                if opaque_remainder(segments[:start]) or opaque_remainder(segments[end:]):
                    return True
    ...
    for segment_index, segment in enumerate(segments):
        for name in protected:
            ...
            while position >= 0:
                ...
                left = (*segments[:segment_index], local_remainder)
                right = (local_remainder, *segments[segment_index + 1:])
                if opaque_remainder(left) or opaque_remainder(right):
                    return True
...
def _scan_metadata_representation(value: str, *, allow_typed_hash: bool = False) -> None:
    ...
    if any(
        ...
        raise CapabilityV12ContractError("metadata contains unsupported invisible/bidi/combining characters")
    ...
    if (
        ...
        raise CapabilityV12ContractError("metadata representation contains secret material")
    if _RESIDUAL_UNICODE_ESCAPE.search(normalized):
        raise CapabilityV12ContractError("metadata contains residual character encoding")
...
def _canonical_percent_decode(value: str) -> str | None:
    ...
    if _PERCENT.sub("", value).find("%") >= 0:
        raise CapabilityV12ContractError("metadata contains malformed percent encoding")
    try:
        decoded = unquote_to_bytes(value).decode("utf-8", errors="strict")
    except (UnicodeDecodeError, ValueError) as exc:
        raise CapabilityV12ContractError("metadata percent encoding is invalid") from exc
    ...
    if normalize_hex(canonical) != normalize_hex(value):
        raise CapabilityV12ContractError("metadata percent encoding is noncanonical")
...
def _canonical_base64_decode(value: str) -> str | None:
    ...
    if ("+" in compact or "/" in compact) and ("-" in compact or "_" in compact):
        raise CapabilityV12ContractError("metadata Base64 alphabet is ambiguous")
    ...
    if canonical.rstrip("=") != compact.rstrip("="):
        raise CapabilityV12ContractError("metadata Base64 encoding is noncanonical")
    if not decoded or any(unicodedata.category(ch) in {"Cc", "Cf", "Cs"} for ch in decoded):
        return None
    return decoded if decoded != value else None


def _validate_metadata_decoding_closure(value: str, *, allow_typed_hash: bool = False) -> None:
    encoded_size = len(value.encode("utf-8"))
    if encoded_size > _MAX_METADATA_VALUE_BYTES:
        raise CapabilityV12ContractError("metadata value exceeds decoding byte budget")
    ...
    while queue:
        ...
        if current in ancestors:
            raise CapabilityV12ContractError("metadata decoding cycle detected")
        seen.add(current)
        if len(seen) > _MAX_METADATA_REPRESENTATIONS:
            raise CapabilityV12ContractError("metadata decoding representation budget exceeded")
        total_bytes += len(current.encode("utf-8"))
        if total_bytes > _MAX_METADATA_TOTAL_BYTES:
            raise CapabilityV12ContractError("metadata decoding expansion budget exceeded")
        _scan_metadata_representation(current, allow_typed_hash=allow_typed_hash)
        ...
        if depth >= _MAX_METADATA_DECODE_DEPTH and candidates:
            raise CapabilityV12ContractError("metadata decoding depth exceeded")
...
def _text(value: object, label: str, maximum: int, *, empty: bool = False) -> str:
    if type(value) is not str or value != value.strip() or (not value and not empty):
        raise CapabilityV12ContractError(f"{label} must be a canonical exact string")
    if len(value) > maximum or any(
        (ord(ch) < 32 and ch not in {"\t", "\n", "\r"}) or ord(ch) == 127 for ch in value
    ):
        raise CapabilityV12ContractError(f"{label} is out of bounds")
    if _secret_canary(value):
        raise CapabilityV12ContractError(f"{label} resembles secret material")
    return value
...
def _slug(value: object, label: str) -> str:
    ...
    if not _SLUG.fullmatch(result):
        raise CapabilityV12ContractError(f"{label} is not a canonical slug")
    return result
...
def _identifier(value: object, label: str) -> str:
    ...
    if not _IDENT.fullmatch(result):
        raise CapabilityV12ContractError(f"{label} is not a canonical identifier")
    return result
...
def _strings(value: object, label: str, *, count: int = 32, size: int = 128, required: bool = False) -> tuple[str, ...]:
    if type(value) not in {tuple, list} or len(value) > count or (required and not value):
        raise CapabilityV12ContractError(f"{label} cardinality is invalid")
    result = tuple(_text(item, f"{label} item", size) for item in value)
    if len(set(result)) != len(result):
        raise CapabilityV12ContractError(f"{label} contains duplicates")
...
def _reject_json_secrets(
    ...
    *,
    ...
    if _depth > _MAX_JSON_DEPTH:
        raise CapabilityV12ContractError(f"{label} exceeds nesting depth")
    active = set() if _active is None else _active
    budget = [0, 0] if _budget is None else _budget
    budget[0] += 1
    if budget[0] > _MAX_JSON_NODES:
        raise CapabilityV12ContractError(f"{label} exceeds node budget")
    if isinstance(value, Mapping):
        if len(value) > 512:
            raise CapabilityV12ContractError(f"{label} is too large")
        identity = id(value)
        if identity in active:
            raise CapabilityV12ContractError(f"{label} contains a cycle")
        active.add(identity)
        try:
            for key, item in value.items():
                key = _text(key, f"{label} key", 128)
                if not _JSON_FIELD.fullmatch(key) or not key.isascii():
                    raise CapabilityV12ContractError(f"{label} key is not a canonical ASCII identifier")
                if _normalized_name(key) in _SECRET_NAMES:
                    raise CapabilityV12ContractError(f"{label} contains a secret-name field")
        ...
        if budget[1] > _MAX_JSON_SCALAR_BYTES:
            raise CapabilityV12ContractError(f"{label} exceeds scalar byte budget")
        _validate_metadata_decoding_closure(item)
    elif value is not None:
        if type(value) not in {bool, int, float}:
            raise CapabilityV12ContractError(f"{label} contains an unsupported value")
        if type(value) is float and not math.isfinite(value):
            raise CapabilityV12ContractError(f"{label} contains a non-finite number")
        if type(value) is int and not -(2**63) <= value <= 2**63 - 1:
            raise CapabilityV12ContractError(f"{label} integer is out of bounds")
...
def _json(value: object, label: str, maximum: int = 65536) -> str:
    ...
    try:
        ...
        raise CapabilityV12ContractError(f"{label} is not canonical JSON") from exc
    if len(result.encode("utf-8")) > maximum:
        raise CapabilityV12ContractError(f"{label} exceeds its byte budget")
    return result
...
def _safe_metadata(value: object) -> tuple[tuple[str, str], ...]:
    if type(value) not in {tuple, list} or len(value) > len(_SAFE_METADATA_KEYS):
        raise CapabilityV12ContractError("metadata must be bounded key/value pairs")
    result: list[tuple[str, str]] = []
    for pair in value:
        if type(pair) not in {tuple, list} or len(pair) != 2:
            raise CapabilityV12ContractError("metadata entry is invalid")
        key = _text(pair[0], "metadata key", 64)
        if _normalized_name(key) in _SECRET_NAMES or key not in _SAFE_METADATA_KEYS:
            raise CapabilityV12ContractError("metadata key is not declared safe")
        item = _text(pair[1], f"metadata {key}", 512)
        if key == "declaration_sha256" and not _HEX64.fullmatch(item):
            raise CapabilityV12ContractError("declaration_sha256 is invalid")
        _validate_metadata_decoding_closure(item, allow_typed_hash=key == "declaration_sha256")
        result.append((key, item))
    if len({key for key, _ in result}) != len(result):
        raise CapabilityV12ContractError("metadata key is duplicated")
    return tuple(sorted(result))


@dataclass(frozen=True, slots=True)
class OperationDescriptorV12:
    operation_id: str
    kind: OperationKindV12
    description: str
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.kind) is not OperationKindV12:
            raise CapabilityV12ContractError("operation kind must be exact")
        object.__setattr__(self, "description", _text(self.description, "description", 4096))
        if type(self.parameter_schema_json) is not str:
            raise CapabilityV12ContractError("parameter schema must be exact string")
        try:
            parsed = json.loads(self.parameter_schema_json)
        except json.JSONDecodeError as exc:
            raise CapabilityV12ContractError("parameter schema is invalid") from exc
        if _json(parsed, "parameter schema") != self.parameter_schema_json:
            raise CapabilityV12ContractError("parameter schema is noncanonical")
        ...
        if any(not _DOMAIN.fullmatch(item) or ".." in item for item in domains):
            raise CapabilityV12ContractError("allowed domain is invalid")
...
@dataclass(frozen=True, slots=True)
class CapabilityDescriptorV12:
    capability_id: str
    capability_version: str
    provider: str
    transport: TransportKindV12
    api_name: str
    api_version: str
    workspace_id: str
    account_id: str
    ...
    operations: tuple[OperationDescriptorV12, ...]
    status: CapabilityStatusV12 = CapabilityStatusV12.DISABLED
    status_reason: str = "candidate_not_activated"
    limitations: tuple[str, ...] = ()
    license_review: str = "not_required_local_only"
    metadata: tuple[tuple[str, str], ...] = ()
    schema_version: str = CAPABILITY_SCHEMA_VERSION_V12

    def __post_init__(self) -> None:
        if self.schema_version != CAPABILITY_SCHEMA_VERSION_V12:
            raise CapabilityV12ContractError("unknown descriptor schema")
        for name in ("capability_id", "capability_version", "provider", "api_name", "api_version"):
            object.__setattr__(self, name, _slug(getattr(self, name), name))
        if type(self.transport) is not TransportKindV12 or type(self.status) is not CapabilityStatusV12:
            raise CapabilityV12ContractError("descriptor enum type is invalid")
        ...
        if self.credential_alias is not None and (
            ...
            raise CapabilityV12ContractError("credential_alias must be opaque")
        if type(self.operations) not in {tuple, list} or not self.operations or len(self.operations) > 64:
            raise CapabilityV12ContractError("operation cardinality is invalid")
        operations = tuple(self.operations)
        if any(type(item) is not OperationDescriptorV12 for item in operations):
            raise CapabilityV12ContractError("operation descriptor type is invalid")
        if len({item.operation_id for item in operations}) != len(operations):
            raise CapabilityV12ContractError("duplicate operation")
        ...
        if self.status is CapabilityStatusV12.DEGRADED and self.status_reason == "healthy":
            raise CapabilityV12ContractError("degraded descriptor requires a reason")
        if self.transport is TransportKindV12.BROWSER and dict(self.metadata).get("fallback_class") != "explicit_browser_fallback":
            raise CapabilityV12ContractError("browser fallback is not explicit")
...
@dataclass(frozen=True, slots=True)
class NexusFeatureGateV12:
    nexus_enabled: bool = False
    shadow_mode: bool = True
    dispatch_enabled: bool = False
    contract_version: str = NEXUS_CONTRACT_VERSION_V12

    def __post_init__(self) -> None:
        if type(self.nexus_enabled) is not bool or type(self.shadow_mode) is not bool or type(self.dispatch_enabled) is not bool:
            raise CapabilityV12ContractError("gate values must be exact booleans")
        if self.contract_version != NEXUS_CONTRACT_VERSION_V12:
            raise CapabilityV12ContractError("gate contract version is invalid")
        if self.nexus_enabled or not self.shadow_mode or self.dispatch_enabled:
            raise CapabilityV12ContractError("V12 is strictly default-off and shadow-only")


@dataclass(frozen=True, slots=True)
class CapabilityProjectionV12:
    descriptor: CapabilityDescriptorV12
    descriptor_digest: str
    workspace_id: str
    account_id: str
    profile_id: str
    projection_status: CapabilityStatusV12
    projection_reason: str
    runtime_available: bool = False
    authority_granted: bool = False

    def __post_init__(self) -> None:
        if type(self.descriptor) is not CapabilityDescriptorV12 or self.descriptor_digest != self.descriptor.digest:
            raise CapabilityV12ContractError("projection descriptor binding drift")
        if (
            ...
            raise CapabilityV12ContractError("projection identity binding drift")
        if type(self.projection_status) is not CapabilityStatusV12:
            raise CapabilityV12ContractError("projection status is invalid")
        ...
        if type(self.runtime_available) is not bool or type(self.authority_granted) is not bool:
            raise CapabilityV12ContractError("projection booleans must be exact")
        if self.runtime_available or self.authority_granted:
            raise CapabilityV12ContractError("V12 projections cannot convey authority")
...
@dataclass(frozen=True, slots=True)
class CapabilitySnapshotV12:
    workspace_id: str
    account_id: str
    profile_id: str
    entries: tuple[CapabilityProjectionV12, ...]
    snapshot_digest: str
    contract_version: str = NEXUS_CONTRACT_VERSION_V12

    def __post_init__(self) -> None:
        if self.contract_version != NEXUS_CONTRACT_VERSION_V12:
            raise CapabilityV12ContractError("snapshot contract version is invalid")
        for name in ("workspace_id", "account_id", "profile_id"):
            object.__setattr__(self, name, _identifier(getattr(self, name), name))
        if type(self.entries) is not tuple or any(type(item) is not CapabilityProjectionV12 for item in self.entries):
            raise CapabilityV12ContractError("snapshot entries are invalid")
        if any(
            ...
            raise CapabilityV12ContractError("snapshot contains a cross-profile projection")
        expected = _sha(
            {
                "account_id": self.account_id,
        ...
        if self.snapshot_digest != expected:
            raise CapabilityV12ContractError("snapshot digest is invalid")


class CapabilityNexusV12:
    """Descriptor-only registry; lock order is gate immutable -> registry."""

    def __init__(self, gate: NexusFeatureGateV12 | None = None) -> None:
        if gate is None:
            gate = NexusFeatureGateV12()
        if type(gate) is not NexusFeatureGateV12:
            raise CapabilityV12ContractError("registry requires the exact concrete V12 gate")
        self._gate = gate
        self._descriptors: dict[str, CapabilityDescriptorV12] = {}
        self._revoked_aliases: set[str] = set()
        self._kill = False
        self._lock = threading.RLock()

    @property
    def gate(self) -> NexusFeatureGateV12:
        return self._gate

    @property
    def lock_order(self) -> tuple[str, ...]:
        return LOCK_ORDER_V12

    def register(self, descriptor: CapabilityDescriptorV12) -> str:
        if type(descriptor) is not CapabilityDescriptorV12:
            raise CapabilityV12ContractError("registry descriptor type is invalid")
        with self._lock:
            if descriptor.capability_id in self._descriptors:
                raise CapabilityV12ContractError("duplicate capability")
    ...
    def revoke_alias(self, alias: str) -> None:
        if type(alias) is not str or not _ALIAS.fullmatch(alias):
            raise CapabilityV12ContractError("credential alias is invalid")
        with self._lock:
            self._revoked_aliases.add(alias)

    def set_kill(self, active: bool) -> None:
        if type(active) is not bool:
            raise CapabilityV12ContractError("kill state must be exact boolean")
        with self._lock:
            self._kill = active
    ...
    def discover(
        ...
        *,
        ...
        if not _HEX64.fullmatch(expected_digest):
            raise CapabilityV12ContractError("expected digest is invalid")
        with self._lock:
            descriptor = self._descriptors.get(capability_id)
            if descriptor is None:
                raise CapabilityV12DeniedError("unknown capability")
            if (
                descriptor.workspace_id != workspace_id
                or descriptor.account_id != account_id
                or descriptor.profile_id != profile_id
            ):
                raise CapabilityV12DeniedError("workspace/account/profile mismatch")
            if descriptor.capability_version != expected_version or descriptor.digest != expected_digest:
                raise CapabilityV12DeniedError("version or descriptor drift")
            status, reason = self._status(descriptor)
            return CapabilityProjectionV12(
                descriptor, descriptor.digest, workspace_id, account_id, profile_id, status, reason
            )

    def health_projection(self, *args: object, **kwargs: object) -> CapabilityProjectionV12:
        return self.discover(*args, **kwargs)

    def snapshot(self, *, workspace_id: str, account_id: str, profile_id: str) -> CapabilitySnapshotV12:
        workspace_id = _identifier(workspace_id, "workspace_id")
        account_id = _identifier(account_id, "account_id")
        profile_id = _identifier(profile_id, "profile_id")
        with self._lock:
        ...
        payload = {
            ...
            "contract_version": NEXUS_CONTRACT_VERSION_V12,
            "entries": [item.digest_payload() for item in entries],
            "profile_id": profile_id,
            "workspace_id": workspace_id,
        }
        return CapabilitySnapshotV12(workspace_id, account_id, profile_id, entries, _sha(payload))

    def _status(self, descriptor: CapabilityDescriptorV12) -> tuple[CapabilityStatusV12, str]:
        if self._kill:
            return CapabilityStatusV12.DISABLED, "global_kill_active"
        if descriptor.credential_alias is not None and descriptor.credential_alias in self._revoked_aliases:
            return CapabilityStatusV12.BLOCKED_BY_ACCESS, "credential_alias_revoked"
        if descriptor.status is CapabilityStatusV12.AVAILABLE_READ_ONLY:
            return CapabilityStatusV12.DISABLED, "nexus_default_off_shadow_only"
        return descriptor.status, descriptor.status_reason


@dataclass(frozen=True, slots=True)
class LegacyParityRecordV12:
    tool_name: str
    host_policy: str
...
@dataclass(frozen=True, slots=True)
class LegacyDescriptorSetV12:
    descriptors: tuple[CapabilityDescriptorV12, ...]
    parity_records: tuple[LegacyParityRecordV12, ...]
    snapshot_digest: str
...
def build_legacy_descriptors_v12(
    ...
    *,
    ...
    account_id: str,
    profile_id: str,
    operation_kinds: Mapping[str, OperationKindV12] | None = None,
) -> LegacyDescriptorSetV12:
    if type(tool_declarations) not in {tuple, list} or not tool_declarations or len(tool_declarations) > 128:
        raise CapabilityV12ContractError("legacy declarations are invalid")
    if not isinstance(policy_mapping, Mapping):
        raise CapabilityV12ContractError("legacy policy mapping is invalid")
    workspace_id = _identifier(workspace_id, "workspace_id")
    account_id = _identifier(account_id, "account_id")
    profile_id = _identifier(profile_id, "profile_id")
    ...
    for raw in tool_declarations:
        if not isinstance(raw, Mapping) or set(raw) != {"name", "description", "parameters"}:
            raise CapabilityV12ContractError("legacy declaration shape drift")
        name = _slug(raw.get("name"), "tool name")
        if name in seen:
            raise CapabilityV12ContractError("duplicate legacy declaration")
        seen.add(name)
        _text(raw.get("description"), "legacy description", 4096)
        if not isinstance(raw.get("parameters"), Mapping):
            raise CapabilityV12ContractError("legacy parameter schema is invalid")
        encoded = _json(dict(raw), "legacy declaration")
        parsed.append((name, raw, encoded))
    if set(policy_mapping) != seen:
        raise CapabilityV12ContractError("legacy policy coverage is not exact")
    if operation_kinds is not None and set(operation_kinds) != seen:
        raise CapabilityV12ContractError("legacy operation coverage is not exact")
    descriptors: list[CapabilityDescriptorV12] = []
    records: list[LegacyParityRecordV12] = []
    for name, raw, encoded in parsed:
        policy = _slug(policy_mapping[name], "host policy")
        kind = OperationKindV12.MUTATE if operation_kinds is None else operation_kinds[name]
        if type(kind) is not OperationKindV12:
            raise CapabilityV12ContractError("legacy operation kind is invalid")
        digest = hashlib.sha256(encoded.encode()).hexdigest()
        operation = OperationDescriptorV12(
            name,
            kind,
            str(raw["description"]),
        ...
        descriptors.append(
            CapabilityDescriptorV12(
                f"legacy.{name}", "v12", "onyx_legacy", TransportKindV12.LEGACY,
                "legacy_dispatcher", "unchanged", workspace_id, account_id, profile_id, None,
                (operation,), CapabilityStatusV12.DISABLED, "shadow_descriptor_only",
        ...
        records.append(LegacyParityRecordV12(name, policy, encoded, digest))
    ...
    return LegacyDescriptorSetV12(tuple(descriptors), tuple(records), _sha(payload))


@dataclass(frozen=True, slots=True)
class CatalogItemV12:
    item_id: str
    label: str
    category: str
    item_version: str
...
class CatalogReadRequestV12:
    ...
    timeout_ms: int = 1000
    cancellation: CancellationTokenV12 | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.page_size) is not int or not 1 <= self.page_size <= 100:
            raise CapabilityV12ContractError("page_size is invalid")
        ...
        if type(self.timeout_ms) is not int or not 1 <= self.timeout_ms <= 30_000:
            raise CapabilityV12ContractError("timeout_ms is invalid")
        if self.cancellation is not None and type(self.cancellation) is not CancellationTokenV12:
            raise CapabilityV12ContractError("cancellation token type is invalid")


@dataclass(frozen=True, slots=True)
class ConnectorHealthV12:
    status: CapabilityStatusV12
    reason: str
    workspace_id: str
    account_id: str
    profile_id: str
...
@dataclass(frozen=True, slots=True)
class CatalogReadProjectionV12:
    state: ReadStateV12
    failure_class: ReadFailureClassV12
    correlation_id: str
    request_digest: str
    workspace_id: str
    account_id: str
    profile_id: str
    items: tuple[CatalogItemV12, ...]
    ...
    def __post_init__(self) -> None:
        if type(self.state) is not ReadStateV12 or type(self.failure_class) is not ReadFailureClassV12:
            raise CapabilityV12ContractError("read projection enum is invalid")
        if self.authority_granted or type(self.authority_granted) is not bool:
            raise CapabilityV12ContractError("read projection cannot grant authority")
        if self.state is not ReadStateV12.COMPLETED and self.items:
            raise CapabilityV12ContractError("non-completed projection cannot expose items")
        if self.state is ReadStateV12.COMPLETED and (
            self.failure_class is not ReadFailureClassV12.NONE or self.receipt_digest is None
        ):
            raise CapabilityV12ContractError("completed projection is incomplete")


@dataclass(frozen=True, slots=True)
class _ReadLedgerEntryV12:
    request_digest: str
    workspace_id: str
    account_id: str
    profile_id: str
...
class LocalCatalogReadAdapterV12:
    ...
    def __init__(
        ...
        *,
        ...
        if type(items) not in {tuple, list} or len(items) > 1000:
            raise CapabilityV12ContractError("catalog items are invalid")
        copied = tuple(items)
        if any(type(item) is not CatalogItemV12 for item in copied) or len({item.item_id for item in copied}) != len(copied):
            raise CapabilityV12ContractError("catalog items must be exact and unique")
        ...
        if type(cursor_signing_key) is not bytes or not 32 <= len(cursor_signing_key) <= 128:
            raise CapabilityV12ContractError("cursor signing key is invalid")
        self._cursor_key = bytes(cursor_signing_key)
        if not callable(read_hook) or not callable(clock):
            raise CapabilityV12ContractError("hook/clock must be callable")
        ...
        if type(auth_available) is not bool:
            raise CapabilityV12ContractError("auth state must be exact boolean")
        self._auth = auth_available
        self._scopes = frozenset(_strings(granted_scopes, "granted_scopes"))
        if type(status) is not CapabilityStatusV12 or status not in {
            CapabilityStatusV12.DISABLED, CapabilityStatusV12.AVAILABLE_READ_ONLY, CapabilityStatusV12.DEGRADED
        }:
            raise CapabilityV12ContractError("adapter status is invalid")
        self._status = status
        self._status_reason = _slug(status_reason, "status_reason")
        if status is CapabilityStatusV12.DEGRADED and status_reason == "healthy":
            raise CapabilityV12ContractError("degraded status requires reason")
        for name, value, lower, upper in (
            ...
            if type(value) is not int or not lower <= value <= upper:
                raise CapabilityV12ContractError(f"{name} is invalid")
        ...
        if pending_capacity > uncertain_capacity or uncertain_capacity > ledger_capacity:
            raise CapabilityV12ContractError("pending/uncertain capacities exceed their parent bound")
        ...
        self._window_used = 0
        self._ledger: dict[str, _ReadLedgerEntryV12] = {}
        self._kill = False
        self._revoked = False
        self._lock = threading.RLock()
    ...
    @property
    def descriptor(self) -> CapabilityDescriptorV12:
        operation = OperationDescriptorV12(
            "catalog_read", OperationKindV12.READ, "Read allowlisted provider-free catalog metadata.",
        ...
        return CapabilityDescriptorV12(
            self.CAPABILITY_ID, "v12", "onyx_local", TransportKindV12.LOCAL, "local_catalog",
            CATALOG_CONTRACT_VERSION_V12, self._workspace_id, self._account_id, self._profile_id,
    ...
    def health(self) -> ConnectorHealthV12:
        now = self._clock()
        with self._lock:
            self._expire_pending(now)
            status, reason, _ = self._effective_status()
            pending = sum(item.state is ReadStateV12.PENDING for item in self._ledger.values())
            uncertain = sum(item.state is ReadStateV12.UNCERTAIN for item in self._ledger.values())
            return ConnectorHealthV12(
                status, reason, self._workspace_id, self._account_id, self._profile_id,
                CATALOG_CONTRACT_VERSION_V12, (self.REQUIRED_SCOPE,), tuple(sorted(self._scopes)),
                max(0, self._quota_limit - self._quota_used), len(self._ledger), pending, uncertain,
            )

    def read_page(self, request: CatalogReadRequestV12) -> CatalogReadProjectionV12:
        with self._hook_state_lock:
            if self._hook_active:
                raise CapabilityV12DeniedError("catalog read reentrancy is denied")
    ...
    def _read_page_claimed(self, request: CatalogReadRequestV12) -> CatalogReadProjectionV12:
        if type(request) is not CatalogReadRequestV12:
            raise CapabilityV12ContractError("read request type is invalid")
        ...
        with self._lock:
            ...
            if prior is not None:
                if prior.request_digest != digest:
                    raise CapabilityV12DeniedError("correlation request drift")
                return self._projection(request.correlation_id, prior)
            if len(self._ledger) >= self._capacity:
                raise CapabilityV12DeniedError("correlation ledger capacity exhausted")
            if request.cancellation is not None and request.cancellation.cancelled:
                entry = self._terminal_without_reservation(
                    request, digest, ReadStateV12.CANCELLED_BEFORE_HOOK, ReadFailureClassV12.CANCELLED, started
                )
            ...
            if status not in {CapabilityStatusV12.AVAILABLE_READ_ONLY, CapabilityStatusV12.DEGRADED}:
                entry = self._terminal_without_reservation(request, digest, ReadStateV12.DENIED, failure, started)
                self._ledger[request.correlation_id] = entry
                return self._projection(request.correlation_id, entry)
            window = int(started)
            ...
            if self._window_used >= self._rate_limit:
                entry = self._terminal_without_reservation(
                    request, digest, ReadStateV12.RATE_LIMITED, ReadFailureClassV12.RATE_LIMIT, started, retry_after_ms=1000
                )
                self._ledger[request.correlation_id] = entry
                return self._projection(request.correlation_id, entry)
            if self._quota_used >= self._quota_limit:
                entry = self._terminal_without_reservation(
                    request, digest, ReadStateV12.DENIED, ReadFailureClassV12.QUOTA, started
                )
                self._ledger[request.correlation_id] = entry
                return self._projection(request.correlation_id, entry)
            pending_count = sum(item.state is ReadStateV12.PENDING for item in self._ledger.values())
            uncertain_count = sum(item.state is ReadStateV12.UNCERTAIN for item in self._ledger.values())
            if pending_count >= self._pending_capacity or pending_count + uncertain_count >= self._uncertain_capacity:
                raise CapabilityV12DeniedError("pending/uncertain ledger capacity exhausted")
            ...
            self._quota_used += 1
            self._window_used += 1
            entry = _ReadLedgerEntryV12(
                digest, self._workspace_id, self._account_id, self._profile_id,
                ReadStateV12.PENDING, ReadFailureClassV12.NONE, page, next_cursor, None, 1,
        ...
        if request.cancellation is not None and request.cancellation.cancelled:
            return self._finish_before_hook(
                request, digest, ReadStateV12.CANCELLED_BEFORE_HOOK, ReadFailureClassV12.CANCELLED, pre_hook_now
            )
        if (pre_hook_now - started) * 1000 >= request.timeout_ms:
            return self._finish_before_hook(
                request, digest, ReadStateV12.TIMEOUT_BEFORE_HOOK, ReadFailureClassV12.TIMEOUT, pre_hook_now
            )
        with self._lock:
            ...
            if status not in {CapabilityStatusV12.AVAILABLE_READ_ONLY, CapabilityStatusV12.DEGRADED}:
                return self._finish_before_hook_locked(
                    request.correlation_id, current, ReadStateV12.DENIED, failure, pre_hook_now
                )
        ...
        if hook_failed:
            ...
            with self._lock:
                current = self._ledger[request.correlation_id]
                if current.state is not ReadStateV12.PENDING:
                    return self._projection(request.correlation_id, current)
                uncertain = replace(
                    current, state=ReadStateV12.UNCERTAIN, failure_class=ReadFailureClassV12.HOOK_EXCEPTION,
        ...
        with self._lock:
            ...
            status, _, failure = self._effective_status()
            if request.cancellation is not None and request.cancellation.cancelled:
                return self._finish_uncertain_locked(request.correlation_id, current, ReadFailureClassV12.CANCELLED, finished)
            if (finished - started) * 1000 >= request.timeout_ms:
                return self._finish_uncertain_locked(request.correlation_id, current, ReadFailureClassV12.TIMEOUT, finished)
            if status not in {CapabilityStatusV12.AVAILABLE_READ_ONLY, CapabilityStatusV12.DEGRADED}:
            ...
            receipt = _sha(
                {"account_id": current.account_id, "correlation_id": request.correlation_id,
                 ...
                 "state": ReadStateV12.COMPLETED.value, "workspace_id": current.workspace_id}
            )
            completed = replace(
                current, state=ReadStateV12.COMPLETED, failure_class=ReadFailureClassV12.NONE,
    ...
    def reconcile(self, request: CatalogReadRequestV12) -> CatalogReadProjectionV12:
        if type(request) is not CatalogReadRequestV12:
            raise CapabilityV12ContractError("reconcile request type is invalid")
        self._validate_binding(request)
        digest = self._request_digest(request)
        now = self._clock()
        with self._lock:
            ...
            if entry is None:
                return CatalogReadProjectionV12(
                    ReadStateV12.UNKNOWN_CORRELATION, ReadFailureClassV12.UNKNOWN,
    ...
    def draft(self, *_args: object, **_kwargs: object) -> None:
        raise CapabilityV12DeniedError("catalog adapter has no draft operation")

    def mutate(self, *_args: object, **_kwargs: object) -> None:
        raise CapabilityV12DeniedError("catalog adapter has no mutation operation")

    def _validate_binding(self, request: CatalogReadRequestV12) -> None:
        if (
        ...
        if request.target != self.TARGET:
            raise CapabilityV12DeniedError("target is not allowlisted")

    def _effective_status(self) -> tuple[CapabilityStatusV12, str, ReadFailureClassV12]:
        if self._kill:
            return CapabilityStatusV12.DISABLED, "global_kill_active", ReadFailureClassV12.KILL
        if self._revoked:
            return CapabilityStatusV12.BLOCKED_BY_ACCESS, "credential_alias_revoked", ReadFailureClassV12.REVOKED
        if not self._auth:
            return CapabilityStatusV12.BLOCKED_BY_ACCESS, "authentication_unavailable", ReadFailureClassV12.AUTH
        if self.REQUIRED_SCOPE not in self._scopes:
            return CapabilityStatusV12.BLOCKED_BY_SCOPE, "required_scope_missing", ReadFailureClassV12.SCOPE
        if self._status is CapabilityStatusV12.DISABLED:
            return self._status, self._status_reason, ReadFailureClassV12.DISABLED
        return self._status, self._status_reason, ReadFailureClassV12.NONE

    def _request_digest(self, request: CatalogReadRequestV12) -> str:
        return _sha(
    ...
    def _encode_cursor(self, offset: int) -> str:
        payload = {"account_id": self._account_id, "capability_id": self.CAPABILITY_ID,
                   "contract_version": CATALOG_CONTRACT_VERSION_V12, "offset": offset,
    ...
    def _decode_cursor(self, cursor: str) -> int:
        try:
            ...
            if not hmac.compare_digest(signature, expected):
                raise CapabilityV12DeniedError("cursor integrity failure")
            payload = json.loads(body)
        except (ValueError, UnicodeError, json.JSONDecodeError) as exc:
            raise CapabilityV12DeniedError("cursor integrity failure") from exc
        expected_payload = {"account_id": self._account_id, "capability_id": self.CAPABILITY_ID,
                            "contract_version": CATALOG_CONTRACT_VERSION_V12,
                            "profile_id": self._profile_id, "snapshot_digest": self._snapshot_digest,
                            "workspace_id": self._workspace_id}
        if set(payload) != set(expected_payload) | {"offset"} or any(payload.get(k) != v for k, v in expected_payload.items()):
            raise CapabilityV12DeniedError("cursor profile/snapshot binding drift")
        offset = payload.get("offset")
        if type(offset) is not int or not 0 <= offset < len(self._items):
            raise CapabilityV12DeniedError("cursor offset is invalid")
        return offset

    def _terminal_without_reservation(
        self, request: CatalogReadRequestV12, digest: str, state: ReadStateV12,
        failure: ReadFailureClassV12, now: float, *, retry_after_ms: int | None = None,
    ) -> _ReadLedgerEntryV12:
        return _ReadLedgerEntryV12(
    ...
    def _finish_before_hook(
        self, request: CatalogReadRequestV12, digest: str, state: ReadStateV12, failure: ReadFailureClassV12,
        now: float,
    ) -> CatalogReadProjectionV12:
        with self._lock:
            current = self._ledger[request.correlation_id]
    ...
    def _finish_before_hook_locked(
        self, correlation_id: str, current: _ReadLedgerEntryV12, state: ReadStateV12,
        failure: ReadFailureClassV12, now: float,
    ) -> CatalogReadProjectionV12:
    ...
    def _finish_uncertain_locked(
        self, correlation_id: str, current: _ReadLedgerEntryV12,
        failure: ReadFailureClassV12, now: float,
    ) -> CatalogReadProjectionV12:
        uncertain = replace(
            current, state=ReadStateV12.UNCERTAIN, failure_class=failure,
    ...
    def _release_reservation(self, entry: _ReadLedgerEntryV12) -> None:
    ...
    def _expire_pending(self, now: float) -> None:
        for correlation_id, entry in tuple(self._ledger.items()):
            if entry.state is ReadStateV12.PENDING and now >= entry.expires_at:
                uncertain = replace(
                    entry, state=ReadStateV12.UNCERTAIN,
                    failure_class=ReadFailureClassV12.EXPIRED_PENDING,
                    items=(), next_cursor=None, expires_at=now + self._uncertain_ttl,
                )
                self._ledger[correlation_id] = uncertain
            elif entry.state is ReadStateV12.UNCERTAIN and now >= entry.expires_at:
                self._ledger[correlation_id] = replace(
                    entry, state=ReadStateV12.EXPIRED_UNCERTAIN,
                    failure_class=ReadFailureClassV12.UNCERTAIN_EXPIRED,
                    items=(), next_cursor=None, expires_at=now,
                )

    def _projection(self, correlation_id: str, entry: _ReadLedgerEntryV12) -> CatalogReadProjectionV12:
        return CatalogReadProjectionV12(
            entry.state, entry.failure_class, correlation_id, entry.request_digest,
            entry.workspace_id, entry.account_id, entry.profile_id,
            entry.items if entry.state is ReadStateV12.COMPLETED else (),
            entry.next_cursor if entry.state is ReadStateV12.COMPLETED else None,
            entry.receipt_digest if entry.state is ReadStateV12.COMPLETED else None,
