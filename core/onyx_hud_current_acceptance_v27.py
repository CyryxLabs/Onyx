"""Current HUD acceptance for the V40 single-root stability correction."""
...
class CurrentHudAcceptanceV27Error(RuntimeError):
    """The V27 source closure or its immutable V26 predecessor drifted."""
...
PREDECESSOR_MANIFEST_SHA256: Final = (
    "9fdd2aa53fe7879c53b117f520f8704c37fdfc310bb173a5a2660f23fec77e92"
)
PREDECESSOR_ACCEPTANCE_RELATIVE: Final = Path(
    "core/onyx_hud_current_acceptance_v26.py"
)
PREDECESSOR_ACCEPTANCE_SHA256: Final = (
    "531174bd0cd847345c417a7a69835038c4bd11907f02167427ecb497a79e6027"
)
...
CURRENT_RUNTIME_PATHS: Final = (
    ...
    Path("scripts/generate_hud_v27_manifests.py"),
...
def _canonical_file(root: Path, relative: Path) -> Path:
    ...
    if pure.is_absolute() or ".." in pure.parts or pure.as_posix() != relative.as_posix():
        raise CurrentHudAcceptanceV27Error("noncanonical V27 source path")
    candidate = root.joinpath(*pure.parts)
    if candidate.is_symlink() or not candidate.is_file():
        raise CurrentHudAcceptanceV27Error(f"V27 source input unavailable: {relative}")
    try:
        resolved = candidate.resolve(strict=True)
        resolved.relative_to(root)
    except (OSError, ValueError) as exc:
        raise CurrentHudAcceptanceV27Error(
            f"V27 source input escapes root: {relative}"
        ) from exc
    if resolved != candidate.absolute():
        raise CurrentHudAcceptanceV27Error(f"V27 source input is linked: {relative}")
    return resolved
...
def _strict_json(path: Path) -> dict[str, object]:
    def pairs(values: list[tuple[str, object]]) -> dict[str, object]:
        ...
        for key, value in values:
            if key in result:
                raise CurrentHudAcceptanceV27Error("duplicate V27 manifest key")
            result[key] = value
        return result
    ...
    try:
        ...
        raise CurrentHudAcceptanceV27Error("V27 manifest unreadable") from exc
    if type(value) is not dict:
        raise CurrentHudAcceptanceV27Error("V27 manifest must be an object")
    return value
...
def verify_current_hud_acceptance(project: Path) -> dict[str, object]:
    """Authenticate the V40 single-root source authority."""
    ...
    if (
        ...
        or manifest.get("schema") != "onyx.hud.current.v27.acceptance.v1"
        ...
        raise CurrentHudAcceptanceV27Error("V27 manifest contract drift")
    entries = manifest["runtime_inputs"]
    if type(entries) is not list or len(entries) != len(CURRENT_RUNTIME_PATHS):
        raise CurrentHudAcceptanceV27Error("V27 source membership drift")
    ...
    for relative, entry in zip(CURRENT_RUNTIME_PATHS, entries, strict=True):
        ...
        if _sha256(_canonical_file(root, relative)) != entry["sha256"]:
            raise CurrentHudAcceptanceV27Error(f"V27 source input drift: {relative}")
        records.append(entry)
    artifact_root = _artifact_root(records)
    if manifest.get("artifact_root_sha256") != artifact_root:
        raise CurrentHudAcceptanceV27Error("V27 artifact root drift")
    for relative, expected in (
        ...
        if _sha256(_canonical_file(root, relative)) != expected:
            raise CurrentHudAcceptanceV27Error(
                f"immutable V26 predecessor drift: {relative}"
            )
    anchors = (
        (Path("ui.py"), "core.onyx_hud_current_acceptance_v27"),
    ...
    for relative, anchor in anchors:
        if anchor not in _canonical_file(root, relative).read_text(encoding="utf-8"):
            raise CurrentHudAcceptanceV27Error(f"V27 wiring drift: {relative}")
    return {
