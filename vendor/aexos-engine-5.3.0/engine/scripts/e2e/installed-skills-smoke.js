 ...
 *
 * Packs the local aexos-core package, installs it into a temporary brownfield
...
const { getSkillId } = require(
  path.join(repoRoot, '.aexos-core', 'infrastructure', 'scripts', 'codex-skills-sync', 'index'),
);
...
const verbose = process.env.AEXOS_E2E_VERBOSE === '1';
const keepTemp = process.env.AEXOS_E2E_KEEP_TEMP === '1';
const agentSet = (process.env.AEXOS_E2E_AGENT_SET || 'dev,qa,aexos-master')
  .split(',')
...
async function main() {
  ...
  log('Packing local aexos-core package');
  ...
  log('Installing packed aexos-core tarball');
  ...
  const cliPath = path.join(
    ...
    'node_modules',
    '.bin',
    `aexos-core${process.platform === 'win32' ? '.cmd' : ''}`,
  );
  const packagedCoreDir = path.join(projectRoot, packageInstallRelativePath, '.aexos-core');
  assertPathExists(path.join(packageInstallRelativePath, 'bin', 'aexos.js'), 'file');
  assertPathExists(path.join('node_modules', '.bin', 'aexos-core'), 'any');
  if (process.platform === 'win32') {
    assertPathExists(path.join('node_modules', '.bin', path.basename(cliPath)), 'file');
  }

  log('Validating packaged .aexos-core dependencies');
  ...
  log('Running installed aexos install in CI mode');
  ...
  assertPathExists('.aexos-core', 'dir');
  assertPathExists('.aexos-core/development/agents', 'dir');
  assertPathExists('.claude/skills/AEXOS/agents', 'dir');
  assertPathExists('.codex/agents', 'dir');
  assertPathExists('.codex/skills', 'dir');

  for (const agent of agentSet) {
    const claudeSkill = `.claude/skills/AEXOS/agents/${agent}/SKILL.md`;
    ...
    const sourceAgent = `.aexos-core/development/agents/${agent}.md`;
    ...
    assertContains(
      claudeSkillContent,
      `Source: .aexos-core/development/agents/${agent}.md`,
      claudeSkill,
    );
    ...
    assertContains(codexSkillContent, `.aexos-core/development/agents/${agent}.md`, codexSkill);
  }
  ...
  assertPathExists('.aexos-core/development/scripts/generate-greeting.js', 'file');
...
main()
  .catch((error) => {
    ...
    if (!keepTemp) {
      console.error(
        '[installed-skills-e2e] Re-run with AEXOS_E2E_KEEP_TEMP=1 to preserve temp files.',
      );
    } else {
