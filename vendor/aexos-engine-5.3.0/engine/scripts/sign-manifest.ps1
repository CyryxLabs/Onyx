...
#
# Usage:
#   .\scripts\sign-manifest.ps1 -SecretKey C:\path\to\aexos-core.key
#
# Prerequisites:
...
$ManifestPath = Join-Path $ProjectRoot ".aexos-core\install-manifest.yaml"
$SignaturePath = "$ManifestPath.minisig"

Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Yellow
Write-Host "  AEXOS Core Manifest Signing Tool (Windows)" -ForegroundColor Yellow
Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Yellow
Write-Host ""
...
if (-not (Test-Path $ManifestPath)) {
    ...
    Write-Host "  node bin\cyryx.js manifest:generate"
    exit 1
}
...
Push-Location (Join-Path $ProjectRoot ".aexos-core")
try {
...
Write-Host "  1. git add .aexos-core\install-manifest.yaml.minisig"
