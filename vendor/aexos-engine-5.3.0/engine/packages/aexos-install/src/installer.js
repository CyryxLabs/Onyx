...
async function runDoctor(projectRoot, logger, dryRun) {
  ...
  try {
    const { stdout } = await execa('npx', ['aexos-core', 'doctor'], {
      cwd: projectRoot,
      timeout: 60000,
    });
    ...
    spinner.warn('AEXOS doctor had warnings');
    if (logger.verbose) {
 ...
 * @param {boolean} dryRun - Whether this is a dry run
 */
async function installCyryxCore(projectRoot, logger, dryRun) {
  if (dryRun) {
    logger.action('Run: npm install aexos-core --save-dev');
    return;
  }
  ...
  try {
    await execa('npm', ['install', 'aexos-core', '--save-dev'], {
      cwd: projectRoot,
...
async function initializeCyryx(projectRoot, logger, dryRun) {
  ...
  try {
    await execa('npx', ['aexos-core', 'install'], {
      cwd: projectRoot,
  ...
  if (brownfield.isBrownfield) {
    ...
    if (brownfield.hasLegacyConfig && !brownfield.hasLayeredConfig) {
      ...
      if (shouldMigrate && !options.dryRun) {
        ...
        try {
          await execa('npx', ['aexos-core', 'config', 'migrate'], {
            cwd: projectRoot,
            timeout: 60000,
          });
    ...
    // Install aexos-core
    await installCyryxCore(projectRoot, logger, options.dryRun);

    // Initialize AEXOS
    await initializeCyryx(projectRoot, logger, options.dryRun);
  }
