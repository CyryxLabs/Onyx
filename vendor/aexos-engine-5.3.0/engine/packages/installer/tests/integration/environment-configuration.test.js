...
describe('Environment Configuration Integration', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `aexos-env-test-${testId}`);
  ...
  describe('configureEnvironment', () => {
    it('should create .env file with skip prompts', async () => {
      ...
      // Version-agnostic: check AEXOS_VERSION exists with valid semver format
      expect(content).toMatch(/AEXOS_VERSION=\d+\.\d+\.\d+/);
    });
  ...
  describe('File Structure', () => {
    ...
    it('should create .aexos-core directory if missing', async () => {
      const cyryxcoreDir = path.join(testDir, '.aexos-core');
      expect(await fs.pathExists(cyryxcoreDir)).toBe(false);
      ...
      expect(await fs.pathExists(cyryxcoreDir)).toBe(true);
    });
  });
});
