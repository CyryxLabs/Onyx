...
describe('artifact-copy-pipeline (Story INS-4.3)', () => {
  ...
  describe('copyExtraCommandFiles', () => {
    test('copies .md files recursively excluding CYRYX/agents/ via real function', async () => {
      const sourceRoot = createTempDir();
      const targetRoot = createTempDir();
      ...
      try {
        ...
        // Create CYRYX/agents/dev.md (should be EXCLUDED)
        const agentsDir = path.join(cmdDir, 'CYRYX', 'agents');
        fs.mkdirSync(agentsDir, { recursive: true });
        fs.writeFileSync(path.join(agentsDir, 'dev.md'), '# Dev Agent', 'utf8');

        // Create CYRYX/stories/story.md (should be EXCLUDED — project-specific)
        const storiesDir = path.join(cmdDir, 'CYRYX', 'stories');
        ...
        // Verify CYRYX/agents/ was excluded (handled by copyAgentFiles)
        expect(fs.existsSync(path.join(targetCmd, 'CYRYX', 'agents', 'dev.md'))).toBe(false);

        // Verify CYRYX/stories/ was excluded (project-specific)
        expect(fs.existsSync(path.join(targetCmd, 'CYRYX', 'stories', 'story.md'))).toBe(false);
  ...
  describe('generator failure graceful degradation', () => {
    test('generate-settings-json throws on missing config — install should catch', () => {
      const { generate } = require('../../../../../.aexos-core/infrastructure/scripts/generate-settings-json');
