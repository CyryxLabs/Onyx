/**
 * Brownfield Upgrader
 * Handles incremental upgrades for existing AEXOS Core installations
 *
...
function loadInstalledManifest(targetDir) {
  const installedManifest = loadManifest(
    path.join(targetDir, '.aexos-core'),
  ...
  const versionJsonPath = path.join(targetDir, '.aexos-core', 'version.json');
 ...
 * @param {string} sourceDir - Source package directory (.aexos-core from npm)
...
function generateUpgradeReport(sourceManifest, installedManifest, targetDir) {
  ...
  const cyryxCoreDir = path.join(targetDir, '.aexos-core');
  ...
  for (const [filePath, sourceEntry] of sourceMap) {
    ...
    const absolutePath = path.join(cyryxCoreDir, filePath);
 ...
 * @param {string} sourceDir - Source directory (npm package .aexos-core)
 * @param {string} targetDir - Target project directory
...
async function applyUpgrade(report, sourceDir, targetDir, options = {}) {
  ...
  // Ensure .aexos-core directory exists
  if (!dryRun) {
    fs.ensureDirSync(cyryxCoreDir);
  }

  // Install new files
  for (const file of report.newFiles) {
    const sourcePath = path.join(sourceDir, file.path);
    const targetPath = path.join(cyryxCoreDir, file.path);
...
function updateInstalledManifest(targetDir, sourceManifest, sourcePackage) {
  const installedManifestPath = path.join(targetDir, '.aexos-core', '.installed-manifest.yaml');
  ...
  const header = `# AEXOS Core Installed Manifest
...
function formatUpgradeReport(report) {
  ...
  lines.push('AEXOS Core Upgrade Report');
  lines.push('═'.repeat(60));
  lines.push('');
