...
function isBrownfieldProjectType(projectType) {
  ...
  return normalized === 'brownfield' || normalized === 'existing_cyryx' || normalized === 'existing-cyryx';
}
...
async function configureEnvironment(options = {}) {
  ...
  try {
    ...
    // Step 2: API keys are configured later via .env or aexos-master
    // Skipping prompts during installation for better UX
    const apiKeys = {};

    if (!skipPrompts) {
      console.log('\n💡 API keys can be configured later in .env file or via aexos-master');
    }
    ...
    const coreConfigDir = path.join(targetDir, '.aexos-core');
    ...
    const coreConfigAction = await resolveFileAction(coreConfigPath, {
      ...
      noMerge,
      projectType,
      message: 'Found existing .aexos-core/core-config.yaml. What would you like to do?',
    });

    if (coreConfigAction === 'skip') {
      console.log('⏭️  Skipped .aexos-core/core-config.yaml (keeping existing)');
      ...
      results.coreConfigCreated = true;
      console.log('✅ Created .aexos-core/core-config.yaml');
    }
...
/**
 * Update .gitignore to include critical entries for AEXOS projects
 *
...
async function updateGitignore(targetDir) {
  ...
  const criticalEntries = {
    ...
    'CYRYX Local': ['.aexos-core/local/', '.claude/settings.local.json', '.aexos/install-log.txt'],
  };
