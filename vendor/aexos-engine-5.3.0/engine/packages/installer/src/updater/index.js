...
class CYRYXUpdater {
   ...
   */
  async createBackup() {
    this.backupDir = path.join(this.cyryxConfigDir, 'backup', `pre-update-${Date.now()}`);
    ...
    for (const file of filesToBackup) {
      const src = path.join(this.cyryxCoreDir, file);
  ...
  async rollback() {
    ...
    for (const file of backupFiles) {
      ...
      const dest = path.join(this.cyryxCoreDir, file);
  ...
  async applyUpdate(targetVersion, _customizedFiles = []) {
    ...
    try {
      ...
      await fs.copy(
        path.join(sourceCyryxCore, 'install-manifest.yaml'),
