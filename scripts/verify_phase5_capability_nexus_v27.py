"""Run the phase-only, fail-closed Capability Nexus V27 verifier."""
...
PRIVATE_BASE = Path(tempfile.gettempdir()) / "onyx-p53-v27-private"
SENTINEL = ".phase53-v27-private"
TOTAL_TIMEOUT_SECONDS = 180
POST_KILL_DRAIN_SECONDS = 2
MARKER = "P53_CAPABILITY_NEXUS_V27_EVIDENCE_OK"
...
class V27ParentError(RuntimeError):
    """Trusted fail-closed wrapper for authoritative cleanup evidence.

    Ordinary instances remain compatible with the historical parent-error
    surface.  ``_install_cleanup_evidence`` locks only wrappers created after a
    hostile primary rejected one or more attachment operations.
    """

    __slots__ = (
        "_v27_attachment_failures",
        "_v27_cleanup_failures",
        "_v27_cleanup_overflow",
        "_v27_evidence_locked",
        "_v27_primary_failure",
    )
    ...
    def __init__(self, message: str) -> None:
        ...
        object.__setattr__(self, "_v27_evidence_locked", False)
        object.__setattr__(self, "_v27_attachment_failures", ())
        object.__setattr__(self, "_v27_cleanup_failures", ())
        object.__setattr__(self, "_v27_cleanup_overflow", None)
        object.__setattr__(self, "_v27_primary_failure", None)

    @staticmethod
    def _immutable_record(record: CleanupFailureRecord) -> MappingProxyType:
    ...
    def _install_cleanup_evidence(
        self,
        *,
        cleanup_records: tuple[CleanupFailureRecord, ...],
        overflow_record: CleanupFailureRecord | None,
        primary_record: CleanupFailureRecord | None,
        attachment_records: tuple[CleanupFailureRecord, ...],
    ) -> None:
        if object.__getattribute__(self, "_v27_evidence_locked"):
            raise AttributeError("V27 cleanup evidence is immutable")
        if (
            type(cleanup_records) is not tuple
            or type(attachment_records) is not tuple
            or any(type(item) is not CleanupFailureRecord for item in cleanup_records)
            or any(type(item) is not CleanupFailureRecord for item in attachment_records)
            or (
                overflow_record is not None
                and type(overflow_record) is not CleanupFailureRecord
            )
            or (
                primary_record is not None
                and type(primary_record) is not CleanupFailureRecord
            )
        ):
            raise TypeError("V27 cleanup evidence is not canonical")
        object.__setattr__(
            self,
            "_v27_cleanup_failures",
            tuple(self._immutable_record(item) for item in cleanup_records),
        )
        object.__setattr__(
            self,
            "_v27_cleanup_overflow",
            None if overflow_record is None else self._immutable_record(overflow_record),
        )
        object.__setattr__(
            self,
            "_v27_primary_failure",
            None if primary_record is None else self._immutable_record(primary_record),
        )
        object.__setattr__(
            self,
            "_v27_attachment_failures",
            tuple(self._immutable_record(item) for item in attachment_records),
        )
        object.__setattr__(self, "_v27_evidence_locked", True)

    def _trusted_cleanup_payload(self) -> tuple[dict[str, str], ...] | None:
        if not object.__getattribute__(self, "_v27_evidence_locked"):
            return None
        records = object.__getattribute__(self, "_v27_cleanup_failures")
        return tuple(
            {
    ...
    def cleanup_failures(self):
        if object.__getattribute__(self, "_v27_evidence_locked"):
            return object.__getattribute__(self, "_v27_cleanup_failures")
        values = BaseException.__getattribute__(self, "__dict__")
        if type(values) is dict and "cleanup_failures" in values:
            return dict.__getitem__(values, "cleanup_failures")
        raise AttributeError("cleanup_failures")

    @cleanup_failures.setter
    def cleanup_failures(self, value) -> None:
        if object.__getattribute__(self, "_v27_evidence_locked"):
            raise AttributeError("V27 cleanup evidence is immutable")
        values = BaseException.__getattribute__(self, "__dict__")
        dict.__setitem__(values, "cleanup_failures", value)

    @property
    def cleanup_overflow(self):
        if object.__getattribute__(self, "_v27_evidence_locked"):
            value = object.__getattribute__(self, "_v27_cleanup_overflow")
            if value is None:
                raise AttributeError("cleanup_overflow")
            return value
        values = BaseException.__getattribute__(self, "__dict__")
        if type(values) is dict and "cleanup_overflow" in values:
            return dict.__getitem__(values, "cleanup_overflow")
        raise AttributeError("cleanup_overflow")

    @cleanup_overflow.setter
    def cleanup_overflow(self, value) -> None:
        if object.__getattribute__(self, "_v27_evidence_locked"):
            raise AttributeError("V27 cleanup evidence is immutable")
        values = BaseException.__getattribute__(self, "__dict__")
        dict.__setitem__(values, "cleanup_overflow", value)

    @property
    def primary_failure(self):
        return object.__getattribute__(self, "_v27_primary_failure")

    @property
    def attachment_failures(self):
        return object.__getattribute__(self, "_v27_attachment_failures")

    def __setattr__(self, name: str, value: object) -> None:
        if (
            type(name) is str
            and name in self._PROTECTED_EVIDENCE_FIELDS
            and object.__getattribute__(self, "_v27_evidence_locked")
        ):
            raise AttributeError("V27 cleanup evidence is immutable")
...
def _kernel32():
    if os.name != "nt":
        raise V27ParentError("Windows Job APIs requested on a non-Windows host")
    if ctypes.sizeof(wintypes.HANDLE) != ctypes.sizeof(ctypes.c_void_p):
        raise V27ParentError("Windows HANDLE width does not match pointer width")
...
def _validated_handle(raw: object) -> wintypes.HANDLE:
    ...
    if (
        ...
        raise V27ParentError("Windows handle is invalid or truncated")
...
def _handle_value(handle: wintypes.HANDLE) -> int:
    ...
    if not isinstance(value, int) or value <= 0:
        raise V27ParentError("Windows handle ownership record is invalid")
    return value


def _win_failure(operation: str) -> V27ParentError:
    return V27ParentError(
...
class _WindowsJob:
    def __init__(self):
        ...
        if not self.api.SetInformationJobObject(
            ...
            try:
                self.close()
            except V27ParentError as close_failure:
                raise V27ParentError(
...
def _wait_job_empty(job: _WindowsJob, label: str) -> None:
    ...
    while job.active() and time.monotonic() < deadline:
        time.sleep(0.01)
    if job.active():
        raise V27ParentError(f"{label} process tree survived termination")


def _phase_command(phase: str, junit: Path) -> list[str]:
    if not isinstance(phase, str):
        raise V27ParentError("phase id must be focused or worker")
    if phase == "focused":
        return [
            sys.executable,
            "-m",
...
def _bounded_output(path: Path, label: str) -> str:
    ...
    if size > _OUTPUT_LIMIT_BYTES:
        raise V27ParentError(f"{label} output exceeds bounded evidence limit")
    ...
    *,
    ...
    try:
        ...
        if wait == _WAIT_TIMEOUT:
            raise V27ParentError(f"{label} timed out after {timeout:.3f}s")
    ...
    if not resumed or result is None:
        raise V27ParentError(f"{label} suspended process was not safely completed")
    return result
...
def _wait_posix_group_empty(pgid: int, label: str) -> None:
    ...
    if _posix_group_exists(pgid):
        raise V27ParentError(f"{label} POSIX process group survived cleanup")
...
class _CleanupFailures:
    ...
    def _read_accessor(
        ...
        *,
        ...
        if type(failure) is V27ParentError and field == "cleanup_failures":
            try:
                trusted = V27ParentError._trusted_cleanup_payload(failure)
    ...
    def finish(self, primary: BaseException | None) -> BaseException | None:
        ...
        overflow = (
            None
            if overflow_record is None
            else {
                "operation": overflow_record.operation,
                "type": overflow_record.exception_type,
                "message": overflow_record.message,
            }
        )
        if primary is None:
        ...
        attachment_failures: list[CleanupFailureRecord] = []

        def failed(operation: str, failure: BaseException) -> None:
            message, _render_failure = self._render_message(failure)
        ...
        def attach_attribute(operation: str, field: str, value: object) -> None:
            try:
                setattr(primary, field, value)
                attached = BaseException.__getattribute__(primary, field)
                if attached is not value:
                    raise RuntimeError("cleanup evidence attribute was not retained exactly")
            except BaseException as attach_failure:
                failed(operation, attach_failure)

        def attach_note(operation: str, note: str) -> None:
            try:
                method = getattr(primary, "add_note")
                method(note)
                notes = BaseException.__getattribute__(primary, "__notes__")
                if type(notes) is not list or not notes or list.__getitem__(notes, -1) != note:
                    raise RuntimeError("cleanup evidence note was not retained exactly")
            except BaseException as attach_failure:
                failed(operation, attach_failure)

        attach_attribute("attachment.cleanup_failures", "cleanup_failures", payload)
        for index, record in enumerate(cleanup_records):
            attach_note(f"attachment.note.{index}", self.note(record))
        if overflow_record is not None:
            attach_attribute("attachment.cleanup_overflow", "cleanup_overflow", overflow)
            attach_note(
        ...
        return self._trusted_wrapper(
            ...
            cleanup_records,
            overflow_record,
            tuple(attachment_failures),
            "cleanup failed closed; evidence attachment failed",
        )
    ...
    def _trusted_wrapper(
        ...
        wrapper = V27ParentError(message)
        if primary is not None:
            try:
        ...
        wrapper._install_cleanup_evidence(
        ...
        for record in cleanup_records:
            BaseException.add_note(wrapper, self.note(record))
        if overflow_record is not None:
        ...
        for record in attachment_records:
            BaseException.add_note(
                wrapper,
                "attachment_failure " + self.note(record),
            )
        return wrapper
...
def _direct_child_cleanup(
    ...
    try:
        if process.poll() is None:
            failures.add(
                "direct.poll.reap",
                V27ParentError(f"{label} direct child was not reaped"),
            )
    except BaseException as exc:
    ...
    try:
        if _pid_exists(process.pid):
            failures.add(
                "direct.pid.proof",
                V27ParentError(f"{label} direct child PID survived cleanup"),
            )
    except BaseException as exc:
    ...
    try:
        ...
        if failure is None:
            try:
                ...
                failure = V27ParentError(f"{label} timed out after {timeout:.3f}s")
...
def _validated_junit_path(evidence_nonce: str, *, require_file: bool) -> Path:
    ...
    if (
        ...
        raise V27ParentError("private evidence sentinel is invalid")
    ...
    if require_file:
        ...
        raise V27ParentError("fresh focused JUnit already exists")
    return junit
...
def _run_phase(
    ...
    *,
    ...
    if (
        ...
        raise V27ParentError(f"{label} timeout is invalid")
    if stdout_mode not in {"pipe", "devnull"}:
        raise V27ParentError(f"{label} stdout mode is invalid")
...
def _remaining(started: float, total_timeout: int) -> float:
    if type(total_timeout) is not int or total_timeout <= POST_KILL_DRAIN_SECONDS + 1:
        raise V27ParentError("total timeout is invalid")
    remaining = total_timeout - (time.monotonic() - started)
    if remaining <= POST_KILL_DRAIN_SECONDS + 1:
        raise V27ParentError(f"total verifier deadline exceeded after {total_timeout}s")
    return remaining
...
def _parse_worker_marker(stdout: str) -> dict[str, object]:
    ...
    try:
        ...
        raise V27ParentError("pure worker marker payload is invalid") from exc
    if not isinstance(payload, dict) or set(payload) != {
        ...
        raise V27ParentError("pure worker marker payload schema is invalid")
    return payload
...
def _run_parent(*, total_timeout: int = TOTAL_TIMEOUT_SECONDS) -> dict[str, object]:
    ...
    with _PrivateEvidence() as evidence_nonce:
        ...
        if focused.returncode:
            raise V27ParentError("fixed focused pytest failed")
        worker = _run_phase(
            "worker",
            evidence_nonce=evidence_nonce,
        ...
        if worker.returncode:
            raise V27ParentError("fixed pure artifact worker failed")
