"""Reproduce the Phase 9 Opportunity Scoring V1 candidate."""
...
from core import phase9_opportunity_scoring_v1 as candidate  # noqa: E402
from scripts import (  # noqa: E402
    verify_phase9_intelligence_ingestion_v1_acceptance as phase9_ingestion,
)

BASE = PROJECT / "docs/onyx/checkpoints/phase9-opportunity-scoring-v1"
MANIFEST = BASE / "manifest.json"
SELECTION = BASE / "cumulative-selection.json"
MARKER = "P9_OPPORTUNITY_SCORING_V1_OK"
ARTIFACT_PATHS = (
    "core/phase9_opportunity_scoring_v1.py",
    "tests/test_phase9_opportunity_scoring_v1.py",
    "scripts/verify_phase9_opportunity_scoring_v1.py",
    "docs/onyx/adrs/ADR-0046-phase9-opportunity-scoring-v1.md",
    "docs/onyx/research/PHASE9_OPPORTUNITY_SCORING_V1_SOURCES.md",
    "docs/onyx/checkpoints/phase9-opportunity-scoring-v1/"
    "PHASE9_OPPORTUNITY_SCORING_V1_CHECKPOINT.md",
    "docs/onyx/checkpoints/phase9-opportunity-scoring-v1/"
...
def verify(*, run_tests: bool = True) -> dict[str, object]:
    ...
    if (
        manifest.get("schema") != "onyx.phase9.opportunity-scoring.v1"
        or manifest.get("candidate") != "phase9-opportunity-scoring-v1"
    ...
    ingestion = phase9_ingestion.verify(run_tests=False)
    if (
        ingestion.get("decision") != "accepted"
        or ingestion.get("candidate_artifact_root_sha256")
        != "977132cea3f9316bb1b7a62ecc4c8368da6f2a12a7e65a62b9d5511a833cbb6a"
    ):
        raise VerificationError("accepted ingestion entry drift")
    candidate._verify_entry(PROJECT)
    if (
        candidate.create_opportunity_scoring_v1(
            gate=candidate.OpportunityScoringFeatureGateV1(False),
    ...
    source = (PROJECT / "core/phase9_opportunity_scoring_v1.py").read_text(
        encoding="utf-8"
    )
    for required in (
    ...
    if run_tests:
        temporary = (
            PROJECT
            / "runtime/test-tmp/phase9-opportunity-scoring-v1-verify"
