"""Canonical pre-import launcher for Onyx Live Activation V5."""
...
LOG_PATH = ROOT / "runtime" / "logs" / "onyx-live-v5-startup.log"
MASTER = "ONYX_LIVE_ACTIVATION_V5"
ROLLBACK = "ONYX_LIVE_ROLLBACK_V5"
CHILDREN = (
...
def run() -> None:
    ...
    if mode == "refuse":
        ...
        raise RuntimeError(
            "ONYX_LIVE_V5_PREIMPORT_REFUSAL " + ("DIRTY" if imported else "CLEAN")
        )
    if mode == "legacy":
        _legacy()
        return
    ...
    from core.onyx_live_activation_v5 import activate_main, preflight_host

    if sys.argv[1:] == ["--preflight-only"]:
        preflight_host(onyx_main)
        print("ONYX_LIVE_V5_HOST_PREFLIGHT_OK")
        return
    if sys.argv[1:]:
        raise RuntimeError("Onyx Live V5 launcher arguments are invalid")

    LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    with LOG_PATH.open("a", encoding="utf-8", buffering=1) as log:
        sys.stdout = log
        sys.stderr = log
        print(f"\n[{datetime.now(timezone.utc).isoformat()}] Onyx Live V5 starting")
        try:
            ...
            print(f"[{datetime.now(timezone.utc).isoformat()}] Onyx Live V5 stopped")
