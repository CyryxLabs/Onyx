"""Generate the additive Phase 5 V55 current-successor transition over V54.

V54 tombstoned `ui.py`, `main.py` and `dashboard/server.py` as historical
...
transition exists to record.  The new root is bound under its own V55 domain so
...
TARGET = PROJECT / "tests/fixtures/phase5_current_successor_transition_v55.json"
SCHEMA = "onyx.phase5-current-successor-transition.v55"
ISSUED_AT = "2026-08-22T00:30:00-04:00"
DOMAIN = b"ONYX-PHASE5-CURRENT-SUCCESSOR-TRANSITION-V55\0"
...
def build() -> dict[str, object]:
    ...
    if _sha(raw) != CURRENT_TRANSITION_V54_SHA256:
        raise SystemExit("refusing to build on a drifted V54 predecessor")
