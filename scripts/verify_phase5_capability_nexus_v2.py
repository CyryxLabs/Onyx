"""Verify the frozen Phase 5.3 Capability Nexus V2 candidate."""
...
ARTIFACT_MANIFEST = "docs/onyx/VE-ARTIFACTS-P53-CAPABILITY-NEXUS-V2-001.sha256"
ROOT_MANIFEST = "docs/onyx/VE-SOURCE-P53-CAPABILITY-NEXUS-V2-001.sha256"
EVIDENCE = "docs/onyx/checkpoints/phase5-capability-nexus-v2"
BUNDLE = f"{EVIDENCE}/phase5-capability-nexus-v2.bundle.json"
JUNIT = f"{EVIDENCE}/phase5-capability-nexus-v2.junit.xml"
RAW = f"{EVIDENCE}/phase5-capability-nexus-v2.raw.log"
STATIC = f"{EVIDENCE}/phase5-capability-nexus-v2.static.log"
LIVE = f"{EVIDENCE}/phase5-capability-nexus-v2.live-scan.sha256"
V1_LIVE = "docs/onyx/checkpoints/phase5-capability-nexus-v1/phase5-capability-nexus-v1.live-scan.sha256"
CHECKPOINT = f"{EVIDENCE}/PHASE5_3_CAPABILITY_NEXUS_V2_CHECKPOINT.md"
CORE = "core/capability_nexus_v2.py"
TESTS = "tests/test_capability_nexus_v2.py"
WHITESPACE = "scripts/check_phase5_capability_nexus_v2_whitespace.py"
SELF = "scripts/verify_phase5_capability_nexus_v2.py"
V1_HISTORY = {
...
class V2EvidenceError(RuntimeError):
    pass
...
def _manifest(relative: str) -> tuple[tuple[str, str], ...]:
    ...
    if b"\r" in raw or not raw.endswith(b"\n"):
        raise V2EvidenceError(f"manifest line endings are invalid: {relative}")
    entries = []
    for line in raw.decode().splitlines():
        match = _LINE.fullmatch(line)
        if match is None:
            raise V2EvidenceError(f"manifest line is invalid: {relative}")
        digest, path = match.groups()
        if path.startswith("/") or ".." in Path(path).parts or "\\" in path:
            raise V2EvidenceError("manifest path is unsafe")
        entries.append((digest, path))
    paths = [path for _, path in entries]
    if not entries or paths != sorted(paths) or len(paths) != len(set(paths)):
        raise V2EvidenceError(f"manifest set is invalid: {relative}")
...
def _bundle(paths: tuple[str, ...]) -> dict[str, object]:
    ...
    try:
        ...
        raise V2EvidenceError("bundle JSON is invalid") from exc
    if raw != json.dumps(bundle, sort_keys=True, separators=(",", ":")) + "\n":
        raise V2EvidenceError("bundle JSON is not canonical")
    if bundle.get("contract") != "Phase53CapabilityNexusEvidence.v2" or bundle.get("status") != "candidate-default-off-shadow-only-not-accepted":
        raise V2EvidenceError("bundle contract/status is invalid")
    if bundle.get("external_e6") != {"accepted": False, "required_before_acceptance": True, "status": "pending-external-review"}:
        raise V2EvidenceError("bundle overstates acceptance")
    if bundle.get("feature_gate") != {"dispatch_enabled": False, "nexus_enabled": False, "shadow_mode": True}:
        raise V2EvidenceError("bundle feature gate is invalid")
    files = bundle.get("files")
    expected = set(paths) - {BUNDLE}
    if not isinstance(files, dict) or set(files) != expected:
        raise V2EvidenceError("bundle file set is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V2EvidenceError(f"bundle file hash mismatch: {relative}")
    return bundle
...
def _static() -> dict[str, int]:
    ...
    for line in (PROJECT / STATIC).read_text(encoding="utf-8").splitlines():
        ...
        if not separator or not value.isdigit():
            raise V2EvidenceError("static log is invalid")
        result[key] = int(value)
    expected = {"COMPILE", "DIFF_CHECK", "FOCUSED", "REGRESSIONS", "RUFF", "V1_TAMPER", "WHITESPACE"}
    if set(result) != expected or any(result.values()):
        raise V2EvidenceError("static gate failed")
    return result


def _history_and_tamper(bundle: dict[str, object]) -> None:
    if bundle.get("history") != {"phase53_v1": {"decision": "rejected", "hashes": V1_HISTORY}}:
        raise V2EvidenceError("V1 history is not exact or not rejected")
    for relative, digest in V1_HISTORY.items():
        if _sha(PROJECT / relative) != digest:
            raise V2EvidenceError(f"V1 historical byte drift: {relative}")
    source = (PROJECT / "core/capability_nexus_v1.py").read_bytes()
    with tempfile.TemporaryDirectory(prefix="onyx-p53-v1-tamper-") as directory:
        tampered = Path(directory) / "capability_nexus_v1.py"
        tampered.write_bytes(source + b"\n# tamper fixture\n")
        if _sha(tampered) == V1_HISTORY["core/capability_nexus_v1.py"]:
            raise V2EvidenceError("V1 tamper fixture was not rejected")


def _live(bundle: dict[str, object]) -> None:
    entries = _manifest(LIVE)
    if tuple(path for _, path in entries) != ("core/capability_nexus_v1.py", V1_LIVE):
        raise V2EvidenceError("V2 live scan anchors are invalid")
    ...
    if bundle.get("live_scan") != {"count": len(expanded), "manifest_sha256": _sha(PROJECT / LIVE)}:
        raise V2EvidenceError("live scan summary drift")
    for digest, relative in expanded:
        path = PROJECT / relative
        if _sha(path) != digest or "capability_nexus_v2" in path.read_text(encoding="utf-8"):
            raise V2EvidenceError(f"live boundary drift/wiring: {relative}")
...
def _assignment(path: Path, name: str) -> object:
    ...
    raise V2EvidenceError(f"missing assignment: {name}")
...
def _semantics(bundle: dict[str, object]) -> None:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import) and {item.name.split(".", 1)[0] for item in node.names} & forbidden:
            raise V2EvidenceError("candidate imports a forbidden runtime/provider surface")
        if isinstance(node, ast.ImportFrom) and (node.module or "").split(".", 1)[0] in forbidden:
            raise V2EvidenceError("candidate imports a forbidden runtime/provider surface")
    sys.path.insert(0, str(PROJECT))
    try:
        from core.capability_nexus_v2 import (  # noqa: PLC0415
            CapabilityNexusV2,
            NexusFeatureGateV2,
        ...
        sys.path.pop(0)
    nexus = CapabilityNexusV2(NexusFeatureGateV2())
    if hasattr(nexus, "dispatch") or hasattr(nexus, "execute"):
        raise V2EvidenceError("registry exposes dispatch")
    ...
    if len(built.descriptors) != 26 or dict(built.policy_mapping()) != policies:
        raise V2EvidenceError("legacy parity failed")
    ...
    expected = {
        ...
        "bounded_pending_uncertain_no_replay", "v1_exact_rejected_history_and_tamper",
    }
    if not isinstance(claims, dict) or set(claims) != expected or not all(claims.values()):
        raise V2EvidenceError("semantic claim set is invalid")
...
def _environment(bundle: dict[str, object]) -> None:
    ...
    if not isinstance(recorded, dict) or any(recorded.get(key) != value for key, value in expected.items()):
        raise V2EvidenceError("environment drift")
    ...
    if bundle.get("base_commit") != commit:
        raise V2EvidenceError("base commit drift")
...
def _fresh() -> None:
    ...
    if completed.returncode or "47 passed" not in completed.stdout:
        raise V2EvidenceError("fresh focused suite failed")
...
def verify() -> dict[str, object]:
    ...
    counts = _junit()
    if counts != {"errors": 0, "failed": 0, "passed": 47, "skipped": 0} or bundle.get("counts") != counts:
        raise V2EvidenceError("JUnit counts drift")
    if bundle.get("static_gates") != _static():
        raise V2EvidenceError("static evidence drift")
    _history_and_tamper(bundle)
    _live(bundle)
    _semantics(bundle)
    _environment(bundle)
    _fresh()
    return {"artifacts": len(paths), "focused_passed": 47, "live_scan_files": 81,
            "root_sha256": _sha(PROJECT / ROOT_MANIFEST)}


def main() -> int:
    print("P53_CAPABILITY_NEXUS_V2_EVIDENCE_OK " + json.dumps(verify(), sort_keys=True))
    return 0
