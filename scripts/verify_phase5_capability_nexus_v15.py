"""Verify the frozen Phase 5.3 Capability Nexus V15 candidate."""
...
ARTIFACT_MANIFEST = "docs/onyx/VE-ARTIFACTS-P53-CAPABILITY-NEXUS-V15-001.sha256"
ROOT_MANIFEST = "docs/onyx/VE-SOURCE-P53-CAPABILITY-NEXUS-V15-001.sha256"
EVIDENCE = "docs/onyx/checkpoints/phase5-capability-nexus-v15"
BUNDLE = f"{EVIDENCE}/phase5-capability-nexus-v15.bundle.json"
JUNIT = f"{EVIDENCE}/phase5-capability-nexus-v15.junit.xml"
RAW = f"{EVIDENCE}/phase5-capability-nexus-v15.raw.log"
STATIC = f"{EVIDENCE}/phase5-capability-nexus-v15.static.log"
LIVE = f"{EVIDENCE}/phase5-capability-nexus-v15.live-scan.sha256"
METRICS = f"{EVIDENCE}/phase5-capability-nexus-v15.metrics.json"
COMBINED_JUNIT = f"{EVIDENCE}/phase5-capability-nexus-v15.combined.junit.xml"
REGRESSION_JUNIT = f"{EVIDENCE}/phase5-capability-nexus-v15.regressions.junit.xml"
...
CHECKPOINT = f"{EVIDENCE}/PHASE5_3_CAPABILITY_NEXUS_V15_CHECKPOINT.md"
CORE = "core/capability_nexus_v15.py"
TESTS = "tests/test_capability_nexus_v15.py"
WHITESPACE = "scripts/check_phase5_capability_nexus_v15_whitespace.py"
SELF = "scripts/verify_phase5_capability_nexus_v15.py"
COMBINED_SUITES = tuple(f"tests/test_capability_nexus_v{version}.py" for version in range(1, 16))
REGRESSION_SUITES = ("tests/test_regressions.py",)
TOTAL_VERIFY_TIMEOUT_SECONDS = 180
...
class V15EvidenceError(RuntimeError):
    pass
...
def _run_bounded(
    ...
    *,
    ...
    if type(timeout) is not int or timeout <= 0:
        raise V15EvidenceError(f"{label} timeout is invalid")
    try:
        ...
        raise V15EvidenceError(f"{label} timed out after {timeout}s") from exc


def _remaining_timeout(started: float, per_command: int) -> int:
    if type(started) not in {int, float} or type(per_command) is not int or per_command <= 0:
        raise V15EvidenceError("verifier deadline input is invalid")
    remaining = TOTAL_VERIFY_TIMEOUT_SECONDS - (time.monotonic() - started)
    if remaining < 1:
        raise V15EvidenceError(f"total verifier deadline exceeded after {TOTAL_VERIFY_TIMEOUT_SECONDS}s")
    return max(1, min(per_command, int(remaining)))
...
def _manifest(relative: str) -> tuple[tuple[str, str], ...]:
    ...
    if b"\r" in raw or not raw.endswith(b"\n"):
        raise V15EvidenceError(f"manifest line endings are invalid: {relative}")
    entries = []
    for line in raw.decode().splitlines():
        match = _LINE.fullmatch(line)
        if match is None:
            raise V15EvidenceError(f"manifest line is invalid: {relative}")
        digest, path = match.groups()
        if path.startswith("/") or ".." in Path(path).parts or "\\" in path:
            raise V15EvidenceError("manifest path is unsafe")
        entries.append((digest, path))
    paths = [path for _, path in entries]
    if not entries or paths != sorted(paths) or len(paths) != len(set(paths)):
        raise V15EvidenceError(f"manifest set is invalid: {relative}")
...
def _historical_manifest(relative: str) -> tuple[tuple[str, str], ...]:
    ...
    try:
        ...
        raise V15EvidenceError(f"historical manifest encoding is invalid: {relative}") from exc
    entries = []
    for line in lines:
        match = _LINE.fullmatch(line)
        if match is None:
            raise V15EvidenceError(f"historical manifest line is invalid: {relative}")
        digest, path = match.groups()
        if path.startswith("/") or ".." in Path(path).parts or "\\" in path:
            raise V15EvidenceError("historical manifest path is unsafe")
        entries.append((digest, path))
    paths = [path for _, path in entries]
    if not entries or len(paths) != len(set(paths)):
        raise V15EvidenceError(f"historical manifest set is invalid: {relative}")
    return tuple(entries)
...
def _dag() -> tuple[str, ...]:
    ...
    if root != ((_sha(PROJECT / ARTIFACT_MANIFEST), ARTIFACT_MANIFEST),):
        raise V15EvidenceError("root manifest edge is invalid")
...
def _bundle(paths: tuple[str, ...]) -> dict[str, object]:
    ...
    try:
        ...
        raise V15EvidenceError("bundle JSON is invalid") from exc
    if raw != json.dumps(bundle, sort_keys=True, separators=(",", ":")) + "\n":
        raise V15EvidenceError("bundle JSON is not canonical")
    if bundle.get("contract") != "Phase53CapabilityNexusEvidence.v15" or bundle.get("status") != "candidate-default-off-shadow-only-not-accepted":
        raise V15EvidenceError("bundle contract/status is invalid")
    if bundle.get("external_e6") != {"accepted": False, "required_before_acceptance": True, "status": "pending-external-review"}:
        raise V15EvidenceError("bundle overstates acceptance")
    if bundle.get("feature_gate") != {"dispatch_enabled": False, "nexus_enabled": False, "shadow_mode": True}:
        raise V15EvidenceError("bundle feature gate is invalid")
    files = bundle.get("files")
    expected = set(paths) - {BUNDLE}
    if not isinstance(files, dict) or set(files) != expected:
        raise V15EvidenceError("bundle file set is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V15EvidenceError(f"bundle file hash mismatch: {relative}")
    return bundle


def _junit_path(path: Path) -> dict[str, int]:
...
def _static_path(path: Path, *, frozen_gate: str = "FROZEN_PROOF") -> dict[str, int]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if not separator or not value.isdigit():
            raise V15EvidenceError("static log is invalid")
    ...
    if set(result) != expected or any(result.values()):
        raise V15EvidenceError("static gate failed")
    return result
...
def _canonical_json(relative: str) -> dict[str, object]:
    ...
    try:
        ...
        raise V15EvidenceError(f"structured evidence JSON is invalid: {relative}") from exc
    if not isinstance(value, dict) or raw != json.dumps(value, sort_keys=True, separators=(",", ":")) + "\n":
        raise V15EvidenceError(f"structured evidence JSON is not canonical: {relative}")
    return value
...
def _sum_counts(items: list[dict[str, int]]) -> dict[str, int]:
    ...
    if not items or any(set(item) != keys for item in items):
        raise V15EvidenceError("JUnit count schema is invalid")
...
def _assert_exact_evidence(label: str, actual: object, expected: object) -> None:
    if type(actual) is not type(expected):
        raise V15EvidenceError(f"{label} evidence type drift")
    if isinstance(expected, dict):
        if set(actual) != set(expected):  # type: ignore[arg-type]
            raise V15EvidenceError(f"{label} evidence field drift")
    ...
    if isinstance(expected, list):
        if len(actual) != len(expected):  # type: ignore[arg-type]
            raise V15EvidenceError(f"{label} evidence list drift")
        for index, value in enumerate(expected):
            _assert_exact_evidence(f"{label}[{index}]", actual[index], value)  # type: ignore[index]
        return
    if actual != expected:
        raise V15EvidenceError(f"{label} evidence value drift")
...
def _checkpoint() -> None:
    ...
    if not required.issubset(set(text.splitlines())):
        raise V15EvidenceError("checkpoint status or structured-evidence claim is invalid")
    ...
    if unchecked.search(text):
        raise V15EvidenceError("checkpoint contains an unchecked quantitative claim")
...
def _validate_metrics(
    ...
    if set(metrics) != expected_keys or metrics.get("contract") != "Phase53CapabilityNexusMetrics.v15":
        raise V15EvidenceError("structured evidence top-level schema is invalid")
    ...
    regression_junit = _junit_path(PROJECT / REGRESSION_JUNIT)
    ...
    if regression_junit.get("passed") != 323:
        raise V15EvidenceError("structured regression JUnit semantic count drift")
    ...
    if not isinstance(runtime, dict) or set(runtime) != {
        "fresh_focused_timeout_seconds", "fresh_regression_timeout_seconds",
        "git_timeout_seconds", "measured_full_verifier_seconds", "total_timeout_seconds",
    }:
        raise V15EvidenceError("structured runtime evidence schema is invalid")
    if (
        runtime.get("fresh_focused_timeout_seconds") != 120
        or runtime.get("fresh_regression_timeout_seconds") != 120
        or runtime.get("git_timeout_seconds") != 10
        or runtime.get("total_timeout_seconds") != TOTAL_VERIFY_TIMEOUT_SECONDS
    ):
        raise V15EvidenceError("structured runtime timeout evidence drift")
    measured = runtime.get("measured_full_verifier_seconds")
    if type(measured) not in {int, float} or not 0 < measured <= TOTAL_VERIFY_TIMEOUT_SECONDS:
        raise V15EvidenceError("structured measured verifier runtime is invalid")
    return metrics
...
def _validate_frozen_v14_proof() -> dict[str, object]:
    ...
    if root != expected_root:
        raise V15EvidenceError("frozen V14 root manifest edge is invalid")
    ...
    if (
        ...
        raise V15EvidenceError("frozen V14 artifact manifest closure is invalid")
    for digest, relative in artifacts:
    ...
    if (
        ...
        raise V15EvidenceError("frozen V14 bundle contract/status is invalid")
    files = bundle.get("files")
    expected_files = set(artifact_paths) - {V14_BUNDLE}
    if not isinstance(files, dict) or set(files) != expected_files:
        raise V15EvidenceError("frozen V14 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V15EvidenceError(f"frozen V14 bundle file hash mismatch: {relative}")
    ...
    if live_entries != (
        ...
        raise V15EvidenceError("frozen V14 live manifest anchors are invalid")
    for digest, relative in live_entries:
        _verify_historical_path(relative, digest)
    if bundle.get("live_scan") != {"count": 93, "manifest_sha256": _sha(PROJECT / V14_LIVE)}:
        raise V15EvidenceError("frozen V14 live evidence is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "external" not in checkpoint.lower():
        raise V15EvidenceError("frozen V14 checkpoint status is invalid")
    verifier = (PROJECT / V14_VERIFIER).read_text(encoding="utf-8")
    if "P53_CAPABILITY_NEXUS_V14_EVIDENCE_OK" not in verifier:
        raise V15EvidenceError("frozen V14 verifier marker contract is missing")
    return {
...
def _history_and_tamper(bundle: dict[str, object]) -> tuple[dict[str, object], dict[str, object]]:
    ...
    if bundle.get("history") != expected:
        raise V15EvidenceError("V1/V2/V3 history is not exact or not rejected")
    for version, history in (
        ...
        for relative, digest in history.items():
            if _sha(PROJECT / relative) != digest:
                raise V15EvidenceError(f"{version} historical byte drift: {relative}")
    proof = _validate_frozen_v14_proof()
    ...
    if not set(HISTORICAL_ROOTS).issubset(historical_manifests):
        raise V15EvidenceError("frozen V1-V14 root closure is incomplete")
    targets = [
    ...
    with tempfile.TemporaryDirectory(prefix="onyx-p53-history-tamper-") as directory:
        ...
        for relative in targets:
            ...
            if expected_digest is None:
                raise V15EvidenceError(f"historical tamper leaf is unbound: {relative}")
            copied = base / relative
            copied.parent.mkdir(parents=True, exist_ok=True)
            copied.write_bytes((PROJECT / relative).read_bytes())
            if _sha(copied) != expected_digest:
                raise V15EvidenceError(f"historical pristine copy mismatch: {relative}")
            copied.write_bytes(copied.read_bytes() + b"\n# disposable tamper\n")
            if _sha(copied) == expected_digest:
                raise V15EvidenceError(f"historical tamper fixture was not rejected: {relative}")
    history_metrics = {
        "frozen_candidates_cryptographically_validated": 14,
...
def _verify_manifest_tree(
    ...
    if depth > 32 or len(verified) >= 256 or len(leaves) > 4096:
        raise V15EvidenceError("historical manifest traversal budget exceeded")
    if relative in active:
        raise V15EvidenceError(f"historical manifest cycle detected: {relative}")
    if relative in verified:
        return
    active.add(relative)
    entries = _historical_manifest(relative)
    if len(entries) > 4096:
        raise V15EvidenceError("historical manifest leaf budget exceeded")
...
def _record_historical_leaf(leaves: dict[str, str], relative: str, digest: str) -> None:
    ...
    if prior is None:
        ...
        raise V15EvidenceError(f"historical recursive digest conflict: {relative}")
    if len(leaves) > 4096:
        raise V15EvidenceError("historical manifest leaf budget exceeded")
...
def _verify_historical_path(relative: str, digest: str) -> None:
    ...
    try:
        ...
        raise V15EvidenceError(f"historical path escapes or is missing: {relative}") from exc
    if not path.is_file() or _sha(path) != digest:
        raise V15EvidenceError(f"historical recursive leaf mismatch: {relative}")


def _assert_no_reparse(relative: str) -> None:
    parts = Path(relative).parts
    if not parts or Path(relative).is_absolute() or ".." in parts:
        raise V15EvidenceError(f"historical path is unsafe: {relative}")
    current = PROJECT
...
def _live(bundle: dict[str, object]) -> dict[str, object]:
    ...
    if bundle.get("live_scan") != {"count": len(expanded), "manifest_sha256": _sha(PROJECT / LIVE)}:
        raise V15EvidenceError("live scan summary drift")
    for digest, relative in expanded:
        path = PROJECT / relative
        if _sha(path) != digest or "capability_nexus_v15" in path.read_text(encoding="utf-8"):
            raise V15EvidenceError(f"live boundary drift/wiring: {relative}")
    return {
...
def _assignment(path: Path, name: str) -> object:
    ...
    raise V15EvidenceError(f"missing assignment: {name}")
...
def _semantics(bundle: dict[str, object]) -> None:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import) and {item.name.split(".", 1)[0] for item in node.names} & forbidden:
            raise V15EvidenceError("candidate imports a forbidden runtime/provider surface")
        if isinstance(node, ast.ImportFrom) and (node.module or "").split(".", 1)[0] in forbidden:
            raise V15EvidenceError("candidate imports a forbidden runtime/provider surface")
    sys.path.insert(0, str(PROJECT))
    try:
        from core.capability_nexus_v15 import (  # noqa: PLC0415
            CapabilityNexusV15,
            NexusFeatureGateV15,
            _METADATA_ENUMS,
            _safe_metadata,
        ...
        sys.path.pop(0)
    nexus = CapabilityNexusV15(NexusFeatureGateV15())
    if hasattr(nexus, "dispatch") or hasattr(nexus, "execute"):
        raise V15EvidenceError("registry exposes dispatch")
    ...
    if len(built.descriptors) != 26 or dict(built.policy_mapping()) != policies:
        raise V15EvidenceError("legacy parity failed")
    ...
    if dict(_METADATA_ENUMS) != expected_metadata_enums:
        raise V15EvidenceError("closed metadata provenance contract drift")
    for key, value in (
        ...
        try:
            ...
            raise V15EvidenceError("closed metadata provenance contract accepted an unknown value")
    ...
    if called & {"_secret_canary", "_raw_credential_canary", "_validate_content_decoding_closure"}:
        raise V15EvidenceError("typed metadata still uses free-text heuristic inference")
    ...
    expected = {
        ...
        "bounded_all_subprocesses_fail_closed",
        ...
        "recursive_v1_to_v14_history_and_component_reparse_rejection",
    }
    if not isinstance(claims, dict) or set(claims) != expected or not all(claims.values()):
        raise V15EvidenceError("semantic claim set is invalid")


def _environment(bundle: dict[str, object], started: float) -> None:
    ...
    if not isinstance(recorded, dict) or any(recorded.get(key) != value for key, value in expected.items()):
        raise V15EvidenceError("environment drift")
    completed = _run_bounded(
        ["git", "rev-parse", "HEAD"],
        label="git base commit",
        timeout=_remaining_timeout(started, 10),
    )
    if completed.returncode:
        raise V15EvidenceError("git base commit failed")
    commit = completed.stdout.strip()
    if bundle.get("base_commit") != commit:
        raise V15EvidenceError("base commit drift")


def _fresh(started: float) -> None:
    env = os.environ.copy()
    env["PYTHONDONTWRITEBYTECODE"] = "1"
    with tempfile.TemporaryDirectory(prefix="onyx-p53-v15-fresh-") as directory:
        junit = Path(directory) / "focused.junit.xml"
        completed = _run_bounded(
            [sys.executable, "-m", "pytest", "-q", TESTS, "--disable-warnings", "--maxfail=1", f"--junitxml={junit}"],
            label="fresh focused suite", timeout=_remaining_timeout(started, 120), env=env,
        )
        counts = _junit_path(junit) if completed.returncode == 0 and junit.is_file() else None
    if completed.returncode or counts != _junit():
        raise V15EvidenceError("fresh focused suite failed")


def _fresh_regressions(started: float) -> None:
    env = os.environ.copy()
    env["PYTHONDONTWRITEBYTECODE"] = "1"
    with tempfile.TemporaryDirectory(prefix="onyx-p53-v15-regressions-") as directory:
        junit = Path(directory) / "regressions.junit.xml"
        completed = _run_bounded(
            [
                sys.executable, "-m", "pytest", "-q", *REGRESSION_SUITES,
                "--disable-warnings", "--maxfail=1", f"--junitxml={junit}",
            ],
            label="fresh stable regressions", timeout=_remaining_timeout(started, 120), env=env,
        )
        counts = _junit_path(junit) if completed.returncode == 0 and junit.is_file() else None
    if completed.returncode or counts != _junit_path(PROJECT / REGRESSION_JUNIT):
        raise V15EvidenceError("fresh stable regressions failed")
...
def verify() -> dict[str, object]:
    ...
    if focused["errors"] or focused["failed"] or focused["skipped"] or bundle.get("counts") != focused:
        raise V15EvidenceError("JUnit counts drift")
    static = _static()
    if bundle.get("static_gates") != static:
        raise V15EvidenceError("static evidence drift")
    ...
    if bundle.get("historical_recursive_evidence") != history:
        raise V15EvidenceError("bundle recursive history evidence drift")
    if bundle.get("regression_evidence") != metrics["regressions"]:
        raise V15EvidenceError("bundle regression evidence drift")
    if bundle.get("artifact_entries") != metrics["artifact"]:
        raise V15EvidenceError("bundle artifact entry evidence drift")
    if bundle.get("structured_evidence") != {
        ...
        raise V15EvidenceError("bundle structured evidence binding drift")
    _checkpoint()
    _semantics(bundle)
    _remaining_timeout(started, TOTAL_VERIFY_TIMEOUT_SECONDS)
    _environment(bundle, started)
    _fresh(started)
    _fresh_regressions(started)
    _remaining_timeout(started, TOTAL_VERIFY_TIMEOUT_SECONDS)
...
def main() -> int:
    print("P53_CAPABILITY_NEXUS_V15_EVIDENCE_OK " + json.dumps(verify(), sort_keys=True))
    return 0
