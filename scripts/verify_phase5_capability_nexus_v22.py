"""Run the phase-only, fail-closed Capability Nexus V22 verifier."""
...
PRIVATE_BASE = Path(tempfile.gettempdir()) / "onyx-p53-v22-private"
SENTINEL = ".phase53-v22-private"
TOTAL_TIMEOUT_SECONDS = 180
POST_KILL_DRAIN_SECONDS = 2
MARKER = "P53_CAPABILITY_NEXUS_V22_EVIDENCE_OK"
...
class V22ParentError(RuntimeError):
    pass
...
def _kernel32():
    if os.name != "nt":
        raise V22ParentError("Windows Job APIs requested on a non-Windows host")
    if ctypes.sizeof(wintypes.HANDLE) != ctypes.sizeof(ctypes.c_void_p):
        raise V22ParentError("Windows HANDLE width does not match pointer width")
...
def _validated_handle(raw: object) -> wintypes.HANDLE:
    ...
    if (
        ...
        raise V22ParentError("Windows handle is invalid or truncated")
...
def _handle_value(handle: wintypes.HANDLE) -> int:
    ...
    if not isinstance(value, int) or value <= 0:
        raise V22ParentError("Windows handle ownership record is invalid")
    return value


def _win_failure(operation: str) -> V22ParentError:
    return V22ParentError(f"{operation} failed with Windows error {ctypes.get_last_error()}")
...
def _cleanup_retained_handles() -> None:
    ...
    if failures:
        raise V22ParentError("retained Windows handle cleanup failed: " + ",".join(sorted(failures)))
...
class _WindowsJob:
    def __init__(self):
        ...
        if not self.api.SetInformationJobObject(
            ...
            try:
                self.close()
            except V22ParentError as close_failure:
                raise V22ParentError(
...
def _wait_job_empty(job: _WindowsJob, label: str) -> None:
    ...
    while job.active() and time.monotonic() < deadline:
        time.sleep(0.01)
    if job.active():
        raise V22ParentError(f"{label} process tree survived termination")


def _phase_command(phase: str, junit: Path) -> list[str]:
    if not isinstance(phase, str):
        raise V22ParentError("phase id must be focused or worker")
    if phase == "focused":
        return [
...
def _bounded_output(path: Path, label: str) -> str:
    ...
    if size > _OUTPUT_LIMIT_BYTES:
        raise V22ParentError(f"{label} output exceeds bounded evidence limit")
...
def _windows_phase(
    ...
    try:
        ...
        if wait == _WAIT_TIMEOUT:
            raise V22ParentError(f"{label} timed out after {timeout:.3f}s")
        ...
        except V22ParentError as exc:
            raise V22ParentError(f"{label} left residual descendants after normal completion") from exc
        for _ in range(3):
    ...
    if not resumed or result is None:
        raise V22ParentError(f"{label} suspended process was not safely completed")
    return result
...
def _wait_posix_group_empty(pgid: int, label: str) -> None:
    ...
    if _posix_group_exists(pgid):
        raise V22ParentError(f"{label} POSIX process group survived cleanup")
...
class _CleanupFailures:
    ...
    def finish(self, primary: BaseException | None) -> BaseException | None:
        if not self.records:
            return primary
        if primary is None:
            primary = V22ParentError("cleanup failed closed")
        payload = tuple(
            {"operation": item.operation, "type": item.exception_type, "message": item.message}
...
def _direct_child_cleanup(
    process: subprocess.Popen[str], label: str, failures: _CleanupFailures,
) -> None:
    active = True
    try:
    ...
    try:
        if process.poll() is None:
            failures.add("direct.poll.reap", V22ParentError(f"{label} direct child was not reaped"))
    except BaseException as exc:
        failures.add("direct.poll.reap", exc)
    try:
        if _pid_exists(process.pid):
            failures.add("direct.pid.proof", V22ParentError(f"{label} direct child PID survived cleanup"))
    except BaseException as exc:
    ...
    try:
        ...
        if failure is None:
            try:
                ...
                failure = V22ParentError(f"{label} timed out after {timeout:.3f}s")
...
def _validated_junit_path(evidence_nonce: str, *, require_file: bool) -> Path:
    ...
    if sentinel.read_text(encoding="ascii") != f"{_PRIVATE_CONTRACT}:{evidence_nonce}\n":
        raise V22ParentError("private evidence sentinel is invalid")
    junit = directory / "focused.junit.xml"
    if require_file:
        _plain_component(junit)
        if not junit.is_file() or junit.resolve(strict=True).parent != resolved:
            raise V22ParentError("fresh focused JUnit is outside private evidence scope")
    elif junit.exists():
        raise V22ParentError("fresh focused JUnit already exists")
    return junit
...
def _run_phase(
    ...
    if not isinstance(timeout, (int, float)) or isinstance(timeout, bool) or not math.isfinite(timeout) or timeout <= 0:
        raise V22ParentError(f"{label} timeout is invalid")
    if stdout_mode not in {"pipe", "devnull"}:
        raise V22ParentError(f"{label} stdout mode is invalid")
...
def _remaining(started: float, total_timeout: int) -> float:
    if type(total_timeout) is not int or total_timeout <= POST_KILL_DRAIN_SECONDS + 1:
        raise V22ParentError("total timeout is invalid")
    remaining = total_timeout - (time.monotonic() - started)
    if remaining <= POST_KILL_DRAIN_SECONDS + 1:
        raise V22ParentError(f"total verifier deadline exceeded after {total_timeout}s")
    return remaining
...
def _parse_worker_marker(stdout: str) -> dict[str, object]:
    ...
    if len(matching) != 1 or len(lines) != 1:
        raise V22ParentError("pure worker marker is missing, duplicated, or contaminated")
    import json  # noqa: PLC0415
    try:
        payload = json.loads(matching[0][len(prefix):])
    except json.JSONDecodeError as exc:
        raise V22ParentError("pure worker marker payload is invalid") from exc
    if not isinstance(payload, dict) or set(payload) != {"artifacts", "focused_passed", "live_scan_files", "root_sha256"}:
        raise V22ParentError("pure worker marker payload schema is invalid")
    return payload
...
def _run_parent(*, total_timeout: int = TOTAL_TIMEOUT_SECONDS) -> dict[str, object]:
    ...
    with _PrivateEvidence() as evidence_nonce:
        ...
        if focused.returncode:
            raise V22ParentError("fixed focused pytest failed")
        worker = _run_phase(
            "worker", evidence_nonce=evidence_nonce, label="fixed pure artifact worker",
            timeout=_phase_timeout(started, total_timeout), env=env,
        )
        if worker.returncode:
            raise V22ParentError("fixed pure artifact worker failed")
