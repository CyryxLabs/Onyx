"""Run the phase-only, fail-closed Capability Nexus V30 verifier."""
...
PRIVATE_BASE = Path(tempfile.gettempdir()) / "onyx-p53-v30-private"
SENTINEL = ".phase53-v30-private"
TOTAL_TIMEOUT_SECONDS = 180
POST_KILL_DRAIN_SECONDS = 2
MARKER = "P53_CAPABILITY_NEXUS_V30_EVIDENCE_OK"
...
class V30ParentError(RuntimeError):
    ...
    __slots__ = (
        "_v30_attachment_failures",
        "_v30_cleanup_failures",
        "_v30_cleanup_overflow",
        "_v30_primary_failure",
    )
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            raise TypeError("V30 cleanup snapshot is not canonical")
        object.__setattr__(
            self,
            "_v30_cleanup_failures",
            None
    ...
    def _trusted_cleanup_payload(self) -> tuple[dict[str, str], ...] | None:
        records = object.__getattribute__(self, "_v30_cleanup_failures")
        if records is None:
            return None
        return tuple(
            {
    ...
    def cleanup_failures(self):
        value = object.__getattribute__(self, "_v30_cleanup_failures")
        if value is None:
    ...
    def cleanup_overflow(self):
        value = object.__getattribute__(self, "_v30_cleanup_overflow")
        if value is None:
    ...
    def primary_failure(self):
        return object.__getattribute__(self, "_v30_primary_failure")

    @property
    def attachment_failures(self):
        return object.__getattribute__(self, "_v30_attachment_failures")

    def __setattr__(self, name: str, value: object) -> None:
        if type(name) is not str:
            raise TypeError("V30 attribute name must be exact built-in str")
        raise AttributeError("V30 parent error is API read-only after construction")
...
def _kernel32():
    if os.name != "nt":
        raise V30ParentError("Windows Job APIs requested on a non-Windows host")
    if ctypes.sizeof(wintypes.HANDLE) != ctypes.sizeof(ctypes.c_void_p):
        raise V30ParentError("Windows HANDLE width does not match pointer width")
...
def _validated_handle(raw: object) -> wintypes.HANDLE:
    ...
    if (
        ...
        raise V30ParentError("Windows handle is invalid or truncated")
...
def _handle_value(handle: wintypes.HANDLE) -> int:
    ...
    if not isinstance(value, int) or value <= 0:
        raise V30ParentError("Windows handle ownership record is invalid")
    return value


def _win_failure(operation: str) -> V30ParentError:
    return V30ParentError(
...
class _WindowsJob:
    def __init__(self):
        ...
        if not self.api.SetInformationJobObject(
            ...
            try:
                self.close()
            except V30ParentError as close_failure:
                raise V30ParentError(
...
def _wait_job_empty(job: _WindowsJob, label: str) -> None:
    ...
    while job.active() and time.monotonic() < deadline:
        time.sleep(0.01)
    if job.active():
        raise V30ParentError(f"{label} process tree survived termination")


def _phase_command(phase: str, junit: Path) -> list[str]:
    if not isinstance(phase, str):
        raise V30ParentError("phase id must be focused or worker")
    if phase == "focused":
        return [
            sys.executable,
            "-m",
...
def _bounded_output(path: Path, label: str) -> str:
    ...
    if size > _OUTPUT_LIMIT_BYTES:
        raise V30ParentError(f"{label} output exceeds bounded evidence limit")
    ...
    *,
    ...
    try:
        ...
        if wait == _WAIT_TIMEOUT:
            raise V30ParentError(f"{label} timed out after {timeout:.3f}s")
    ...
    if not resumed or result is None:
        raise V30ParentError(f"{label} suspended process was not safely completed")
    return result
...
def _wait_posix_group_empty(pgid: int, label: str) -> None:
    ...
    if _posix_group_exists(pgid):
        raise V30ParentError(f"{label} POSIX process group survived cleanup")
...
class _CleanupFailures:
    ...
    def _read_accessor(
        ...
        *,
        ...
        if type(failure) is V30ParentError and field == "cleanup_failures":
            try:
                trusted = V30ParentError._trusted_cleanup_payload(failure)
...
def _direct_child_cleanup(
    ...
    try:
        if process.poll() is None:
            failures.add(
                "direct.poll.reap",
                V30ParentError(f"{label} direct child was not reaped"),
            )
    except BaseException as exc:
    ...
    try:
        if _pid_exists(process.pid):
            failures.add(
                "direct.pid.proof",
                V30ParentError(f"{label} direct child PID survived cleanup"),
            )
    except BaseException as exc:
    ...
    try:
        ...
        if failure is None:
            try:
                ...
                failure = V30ParentError(f"{label} timed out after {timeout:.3f}s")
...
def _validated_junit_path(evidence_nonce: str, *, require_file: bool) -> Path:
    ...
    if (
        ...
        raise V30ParentError("private evidence sentinel is invalid")
    ...
    if require_file:
        ...
        raise V30ParentError("fresh focused JUnit already exists")
    return junit
...
def _run_phase(
    ...
    *,
    ...
    if (
        ...
        raise V30ParentError(f"{label} timeout is invalid")
    if stdout_mode not in {"pipe", "devnull"}:
        raise V30ParentError(f"{label} stdout mode is invalid")
...
def _remaining(started: float, total_timeout: int) -> float:
    if type(total_timeout) is not int or total_timeout <= POST_KILL_DRAIN_SECONDS + 1:
        raise V30ParentError("total timeout is invalid")
    remaining = total_timeout - (time.monotonic() - started)
    if remaining <= POST_KILL_DRAIN_SECONDS + 1:
        raise V30ParentError(f"total verifier deadline exceeded after {total_timeout}s")
    return remaining
...
def _parse_worker_marker(stdout: str) -> dict[str, object]:
    ...
    try:
        ...
        raise V30ParentError("pure worker marker payload is invalid") from exc
    if not isinstance(payload, dict) or set(payload) != {
        ...
        raise V30ParentError("pure worker marker payload schema is invalid")
    return payload
...
def _run_parent(*, total_timeout: int = TOTAL_TIMEOUT_SECONDS) -> dict[str, object]:
    ...
    with _PrivateEvidence() as evidence_nonce:
        ...
        if focused.returncode:
            raise V30ParentError("fixed focused pytest failed")
        worker = _run_phase(
            "worker",
            evidence_nonce=evidence_nonce,
        ...
        if worker.returncode:
            raise V30ParentError("fixed pure artifact worker failed")
