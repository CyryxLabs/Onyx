"""V100 SOURCE successor; generation requires the parent's final seal authority.

All V99 paths are inherited. Historical receipt tests are hash-bound file data,
...
TARGET = ROOT / "tests/fixtures/release_workflow_transition_v100.json"
FINAL_FILE_LIST_CONFIRMED = True
ISSUED_AT = "2026-09-05T23:44:32+00:00"
REGISTRY_SOURCES = (
    "scripts/source_test_succession_v2.py",
    "tests/fixtures/source_test_succession_v2.json",
    "tests/test_source_test_succession_v2.py",
)
HISTORICAL_FIXTURES = tuple(
    f"tests/fixtures/release_workflow_transition_v{sequence}.json"
    for sequence in range(71, 100)
)
HISTORICAL_RECEIPTS = (
...
ADDITIONS = (
    ...
    "docs/onyx/V100_BLOCKER_SUCCESSOR_20260905.md",
    *HISTORICAL_RECEIPTS,
    *REGISTRY_SOURCES,
    *HISTORICAL_FIXTURES,
)
...
def load_record(root):
    ...
    # files. Only the finalized helper may authenticate registry V2 at seal.
    from scripts.source_test_succession_v2 import load_record as authenticate
...
def generate():
    if not FINAL_FILE_LIST_CONFIRMED:
        raise RuntimeError("V100 unsealed: awaiting parent 'seal now' and final file list")
    succession = load_record(ROOT)
    # Never run verify_and_run, staged validation, or current V100 verification
    # from generation: those depend on the fixture we have not sealed yet.
    if _sha256(_file(ROOT, V99)) != V99_SHA256:
        raise RuntimeError("V99 predecessor changed")
    old = _json(ROOT / V99)
    if (
        old.get("current_root_sha256") != V99_ROOT_SHA256
        or _root(old, 99) != V99_ROOT_SHA256
        or old.get("policy") != POLICY
        or len(_entries(old)) != 669
    ):
        raise RuntimeError("V99 predecessor contract changed")
    if not isinstance(ISSUED_AT, str):
        raise RuntimeError("V100 unsealed: final issue time is required")
    if datetime.fromisoformat(ISSUED_AT) <= datetime.fromisoformat(old["issued_at"]):
        raise RuntimeError("V100 chronology must follow V99")
    for sequence, relative in zip(range(71, 100), HISTORICAL_FIXTURES, strict=True):
    ...
    if {"scripts/verify_release_workflow_v100.py", TARGET.relative_to(ROOT).as_posix()}.intersection(paths):
        raise RuntimeError("V100 cannot include its own verifier or fixture digest")
    record = {
        "schema": "onyx.release-workflow-transition.v100",
        "issued_at": ISSUED_AT,
        "logical_sequence": 100,
        "predecessor": {"path": V99.as_posix(), "sha256": V99_SHA256},
        "policy": old["policy"],
    ...
    record["current_root_sha256"] = _root(record, 100)
    encoded = (json.dumps(record, separators=(",", ":")) + "\n").encode("utf-8")
    if TARGET.exists():
        if TARGET.is_symlink() or TARGET.read_bytes() != encoded:
            raise RuntimeError("V100 already exists with different bytes; create a successor")
    else:
