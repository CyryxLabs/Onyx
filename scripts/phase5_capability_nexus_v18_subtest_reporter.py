"""Deterministic pytest reporter for the frozen V18 regression evidence."""
...
def pytest_sessionfinish(session: Any, exitstatus: int) -> None:
    destination = os.environ.get("ONYX_P53_V18_SUBTEST_REPORT")
    if not destination:
        raise RuntimeError("ONYX_P53_V18_SUBTEST_REPORT is required")
    payload = {
        "base_tests": sorted(_BASE.values(), key=lambda item: item["nodeid"]),
        "contract": "Phase53CapabilityNexusRegressionReport.v18",
