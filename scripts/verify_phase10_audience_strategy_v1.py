"""Reproduce the Phase 10 Audience/Strategy V1 candidate."""
...
from core import phase10_audience_strategy_v1 as candidate  # noqa: E402
from scripts import (  # noqa: E402
    verify_phase10_content_draft_current_v1_acceptance as content_draft,
)

BASE = PROJECT / "docs/onyx/checkpoints/phase10-audience-strategy-v1"
MANIFEST = BASE / "manifest.json"
SELECTION = BASE / "cumulative-selection.json"
MARKER = "P10_AUDIENCE_STRATEGY_V1_OK"
...
ARTIFACT_PATHS = (
    ...
    "tests/test_phase10_audience_strategy_v1.py",
    "scripts/verify_phase10_audience_strategy_v1.py",
    "docs/onyx/adrs/ADR-0058-phase10-audience-strategy-v1.md",
    "docs/onyx/research/PHASE10_AUDIENCE_STRATEGY_V1_SOURCES.md",
    "docs/onyx/checkpoints/phase10-audience-strategy-v1/"
    "PHASE10_AUDIENCE_STRATEGY_V1_CHECKPOINT.md",
    "docs/onyx/checkpoints/phase10-audience-strategy-v1/CAPABILITY_DELTA_SNAPSHOT.md",
    "docs/onyx/checkpoints/phase10-audience-strategy-v1/cumulative-selection.json",
)
...
def verify(*, run_tests: bool = True) -> dict[str, object]:
    ...
    if (
        manifest.get("schema") != "onyx.phase10.audience-strategy.v1"
        or manifest.get("candidate") != "phase10-audience-strategy-v1"
    ...
    if candidate.AudienceStrategyFeatureGateV1.from_environ({}).enabled is not False:
        raise VerificationError("feature gate is not default-off")
    candidate._verify_entry(PROJECT)
    try:
        candidate.create_audience_strategy_set_v1(PROJECT / "missing")
    except candidate.AudienceStrategyV1Denied:
        pass
    else:
        raise VerificationError("entry-bind did not deny missing evidence")

    if candidate.STRATEGY_HORIZONS != (30, 60, 90):
        raise VerificationError("strategy cadence drift")
    if candidate.EXPERIMENT_MIN_SAMPLE_FLOOR != 100:
        raise VerificationError("experiment sample floor drift")
    ...
    source = (PROJECT / "core/phase10_audience_strategy_v1.py").read_text(
        encoding="utf-8"
    )
    for required in (
    ...
    if run_tests:
        temporary = (
            PROJECT
            / "runtime/test-tmp/p10-audience-strategy-verify"
    ...
    return {
        "candidate": "phase10-audience-strategy-v1",
