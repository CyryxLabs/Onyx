"""Semantic, live-surface, and byte-exact verifier for Phase 5.1 R10."""
...
from scripts import verify_phase5_grants_r9 as r9


TOP_MANIFEST = "docs/onyx/VE-SOURCE-P51-GRANTS-R10-001.sha256"
ARTIFACT_MANIFEST = "docs/onyx/VE-ARTIFACTS-P51-GRANTS-R10-001.sha256"
EVIDENCE_DIR = "docs/onyx/checkpoints/phase5-grants-r10"
BUNDLE = f"{EVIDENCE_DIR}/phase5-grants-r10.bundle.json"
JUNIT = f"{EVIDENCE_DIR}/phase5-grants-r10.junit.xml"
RAW_LOG = f"{EVIDENCE_DIR}/phase5-grants-r10.raw.log"
STATIC_LOG = f"{EVIDENCE_DIR}/phase5-grants-r10.static.log"
LIVE_SCAN_MANIFEST = f"{EVIDENCE_DIR}/phase5-grants-r10.live-scan.sha256"
CHECKPOINT = f"{EVIDENCE_DIR}/PHASE5_1_GRANTS_SHADOW_R10_CHECKPOINT.md"
R9_ROOT = r9.TOP_MANIFEST
R9_ARTIFACT = r9.ARTIFACT_MANIFEST

SOURCE_SEEDS = (
    "core/session_grants_v10.py",
    "scripts/check_phase5_grants_r10_whitespace.py",
    "scripts/verify_phase5_grants_r10.py",
    "tests/test_session_grants_v10.py",
)
WHITESPACE_SCOPE = list(SOURCE_SEEDS)
NORMATIVE_SEEDS = r9.NORMATIVE_SEEDS
...
LIVE_FORBIDDEN_TOKENS = ("ONYX_GRANT_EVALUATOR", "SessionGrantShadowStore", "session_grants_v10")
...
_REQUIRED_PREFIX_COUNTS = {
    ...
    "test_public_finalization_epoch_change_applies_only_mandatory_revocation[": 5,
...
class Phase5GrantR10EvidenceError(RuntimeError):
    pass
...
def _path(root: Path, relative: str) -> Path:
    ...
    if pure.is_absolute() or "." in pure.parts or ".." in pure.parts:
        raise Phase5GrantR10EvidenceError(f"unsafe evidence path: {relative}")
...
def derive_normative_graph(root: Path) -> dict[str, list[str]]:
    ...
    while queue:
        ...
        if not _path(root, relative).is_file():
            raise Phase5GrantR10EvidenceError(f"normative node missing: {relative}")
...
def derive_live_scan_paths(root: Path) -> tuple[str, ...]:
    ...
    if "scripts/launch_onyx.pyw" not in paths:
        raise Phase5GrantR10EvidenceError("operational launcher is absent from live scan")
    return paths


def local_import_closure(root: Path) -> tuple[str, ...]:
    try:
        return r9.r8.r7.r6.local_import_closure(root, SOURCE_SEEDS)
    except Exception as exc:
        raise Phase5GrantR10EvidenceError(str(exc)) from exc
...
def artifact_paths(root: Path) -> tuple[str, ...]:
    ...
    return tuple(sorted(set((
        *closure, *graph, R9_ARTIFACT, R9_ROOT, CHECKPOINT, BUNDLE, JUNIT, RAW_LOG,
...
def _parse_manifest(path: Path, expected: tuple[str, ...] | None = None):
    try:
        return r9._parse_manifest(path, expected)
    except Exception as exc:
        raise Phase5GrantR10EvidenceError(str(exc)) from exc
...
def _verify_entries(root: Path, entries) -> None:
    try:
        r9._verify_entries(root, entries)
    except Exception as exc:
        raise Phase5GrantR10EvidenceError(str(exc)) from exc
...
def _read_bundle(root: Path) -> dict[str, object]:
    ...
    try:
        ...
        raise Phase5GrantR10EvidenceError("bundle is not valid JSON") from exc
    canonical = (json.dumps(value, sort_keys=True, separators=(",", ":"), ensure_ascii=True) + "\n").encode()
    if raw != canonical or not isinstance(value, dict) or frozenset(value) != _BUNDLE_KEYS:
        raise Phase5GrantR10EvidenceError("bundle shape/serialization is noncanonical")
    return value
...
def _junit(root: Path) -> tuple[dict[str, int], str, set[str]]:
    try:
        ...
        raise Phase5GrantR10EvidenceError("JUnit is invalid") from exc
    suites = [document] if document.tag == "testsuite" else document.findall("testsuite")
    if len(suites) != 1:
        raise Phase5GrantR10EvidenceError("JUnit must contain exactly one suite")
    suite = suites[0]
    ...
    if actual != declared or not names or "" in names:
        raise Phase5GrantR10EvidenceError("JUnit declarations or names are invalid")
...
def _kv_log(root: Path, relative: str) -> dict[str, str]:
    ...
    if b"\r" in raw or not raw.endswith(b"\n"):
        raise Phase5GrantR10EvidenceError(f"log is noncanonical: {relative}")
    values: dict[str, str] = {}
    for line in raw.decode().splitlines():
        if not line or line.startswith("#"):
            continue
        if "=" not in line:
            raise Phase5GrantR10EvidenceError(f"log line is invalid: {relative}")
        key, value = line.split("=", 1)
        if not re.fullmatch(r"[a-z][a-z0-9_]*", key) or key in values:
            raise Phase5GrantR10EvidenceError(f"log key is invalid: {relative}")
        values[key] = value
    return values
        ...
        if isinstance(node, ast.ClassDef) and node.name == "SessionGrantShadowStore":
            for item in node.body:
                if isinstance(item, ast.FunctionDef) and item.name == name:
                    return item
    raise Phase5GrantR10EvidenceError(f"required method is missing: {name}")


def _validate_code_claims(root: Path) -> None:
    source = _path(root, "core/session_grants_v10.py").read_text(encoding="utf-8")
    ...
    if identity is None:
        raise Phase5GrantR10EvidenceError("external mutation identity function is missing")
    identity_literals = {node.value for node in ast.walk(identity) if isinstance(node, ast.Constant) and isinstance(node.value, str)}
    required_identity = {"session_id", "workspace_id", "provider", "provider_namespace", "account", "tool", "operation", "idempotency_key", "ExternalMutationIdentity.v10"}
    ...
    if not required_identity <= identity_literals or identity_literals & forbidden_identity:
        raise Phase5GrantR10EvidenceError("external mutation identity namespace is unsafe")
    ...
    if action_fingerprint is None:
        raise Phase5GrantR10EvidenceError("immutable external action fingerprint is missing")
    ...
    required_action = {"provider", "provider_namespace", "account", "operation", "tool", "target_identity", "payload_digest", "binding_digest", "action_contract_digest", "ImmutableExternalActionFingerprint.v10"}
    if not required_action <= action_literals:
        raise Phase5GrantR10EvidenceError("external action fingerprint omits material action context")
    ...
    if not required_services <= _class_fields(tree, "HostServices"):
        raise Phase5GrantR10EvidenceError("pinned outcome/receipt/reconciliation services are incomplete")
    ...
    if not context <= uncertain or not context <= receipt:
        raise Phase5GrantR10EvidenceError("immutable mutation records are not self-sufficient")
    ...
    if not {"identity_digest", "action_fingerprint", "state", "reconciliation_count", "reconciliation_history_root", "latest_reconciliation_sequence"} <= ledger_fields:
        raise Phase5GrantR10EvidenceError("mutation ledger omits immutable binding or compact history")
    if not {"revision", "reconciliation_count", "reconciliation_history_root", "latest_reconciliation_sequence"} <= uncertain:
        raise Phase5GrantR10EvidenceError("uncertain record omits ABA revision history")
    verification_audit = {
    ...
    if not verification_audit <= uncertain:
        raise Phase5GrantR10EvidenceError("uncertainty omits self-sufficient receipt-verification audit")
    ...
    if not {"_call_outcome_resolver", "_quarantine_reservation_locked", "_call_receipt_verifier", "_verification_is_exact", "_persist_verification_audit_locked"} <= outcome_calls:
        raise Phase5GrantR10EvidenceError("dispatch uncertainty path is incomplete")
    ...
    if not quarantine_lines or not snapshot_lines or min(quarantine_lines) >= min(snapshot_lines):
        raise Phase5GrantR10EvidenceError("returned outcome material is not quarantined before the fallible post-snapshot")
    ...
    if not verifier_lines or not audit_lines or not verifier_lines[0] < audit_lines[0] < max(snapshot_lines):
        raise Phase5GrantR10EvidenceError("verified receipt audit is not persisted before commit snapshot")
    ...
    if "_quarantine_reservation_locked" not in dispatch_calls:
        raise Phase5GrantR10EvidenceError("pre-dispatch uncertainty transition is missing")
    ...
    if not {"_call_reconciliation", "_verification_is_exact", "_receipt_replayed_locked", "_set_uncertain_ledger_state_locked", "_advance_reconciliation_locked"} <= reconcile_calls:
        raise Phase5GrantR10EvidenceError("reconciliation path is incomplete")
    ...
    if "MAX_RECONCILIATIONS" in reconcile_names:
        raise Phase5GrantR10EvidenceError("reconciliation is capacity-blocked")
    ...
    if not {"_roll_reconciliation_history", "replace"} <= advance_calls:
        raise Phase5GrantR10EvidenceError("reconciliation revision does not roll immutable history")
    ...
    init_source = ast.get_source_segment(source, _method(tree, "__init__"))
    if not raw_source or "type(raw_value) is str" not in raw_source or '.strip().casefold() in {"1", "true"}' not in raw_source:
        raise Phase5GrantR10EvidenceError("raw feature flag parser is not exact and closed")
    if not init_source or "self._bootstrap_enabled = grant_evaluator_enabled()" not in init_source:
        raise Phase5GrantR10EvidenceError("raw environment is not a strict default-off bootstrap")
    if not validate_source or "snapshot.epoch != lease" not in validate_source or "not snapshot.enabled" not in validate_source:
        raise Phase5GrantR10EvidenceError("host-owned feature epoch is not the operational lease")
    if not observe_source or "grant.issue_epoch < snapshot.epoch" not in observe_source or '"feature-gate-epoch"' not in observe_source:
        raise Phase5GrantR10EvidenceError("new feature epochs do not permanently revoke old grants")
    session_grant_fields = _class_fields(tree, "SessionGrant")
    if "issue_epoch" not in session_grant_fields:
        raise Phase5GrantR10EvidenceError("grant does not bind its issue epoch")
    gate_snapshot = _class_fields(tree, "FeatureGateSnapshot")
    if gate_snapshot != {"enabled", "epoch"}:
        raise Phase5GrantR10EvidenceError("feature gate snapshot contract is incomplete")
    ...
    if not tracker_source or "self._epoch += 1" not in tracker_source or "enabled is not self._enabled" not in tracker_source:
        raise Phase5GrantR10EvidenceError("host feature gate tracker is not monotonic across off/on transitions")
    ...
    gate_lines = [node.lineno for node in snapshot_calls if node.func.attr == "_gate_validate_locked"]
    if not clock_lines or not state_lines or not any(clock_lines[0] < line < state_lines[0] for line in gate_lines) or not any(line > state_lines[0] for line in gate_lines):
        raise Phase5GrantR10EvidenceError("snapshot callbacks are not separated by feature epoch validation")
    for helper_name in ("_call_resolver", "_call_outcome_resolver", "_call_receipt_verifier", "_call_reconciliation"):
        ...
        if "_gate_validate_locked" not in helper_attrs:
            raise Phase5GrantR10EvidenceError(f"host callback helper lacks epoch revalidation: {helper_name}")
    ...
    if not provisional_source or "self._uncertain_records.values()" not in provisional_source:
        raise Phase5GrantR10EvidenceError("uncertain actions are absent from provisional capacity")
    if not decision_source or "provisional_uses" not in decision_source or "provisional_cost" not in decision_source:
        raise Phase5GrantR10EvidenceError("evaluation ignores provisional limits")
    if not reserve_source or "_provisional_for_grant_locked" not in reserve_source:
        raise Phase5GrantR10EvidenceError("reservation ignores provisional limits")
    ...
    if not reconcile_source or "record.grant_max_uses" not in reconcile_source or "record.grant_max_cost_aggregate_micro" not in reconcile_source:
        raise Phase5GrantR10EvidenceError("reconciliation can overrun original grant limits")
    public_methods = (
    ...
    for method_name in public_methods:
        ...
        if "_gate_entry_locked" not in attrs or "_gate_validate_locked" not in attrs:
            raise Phase5GrantR10EvidenceError(f"public method lacks entry/final gate linearization: {method_name}")
        ...
        if any(not any(line > callback_line for line in validation_lines) for callback_line in callback_lines):
            raise Phase5GrantR10EvidenceError(f"callback lacks a later feature gate sample: {method_name}")
    if any(token in source for token in ("MAX_RECONCILIATIONS", "ReconciliationRecord", "self._reconciliations")):
        raise Phase5GrantR10EvidenceError("false reconciliation cardinality remains in the implementation")
    ...
    if "_enum" not in revoke_calls or '"reconciled-effect"' in source:
        raise Phase5GrantR10EvidenceError("revocation reasons are not closed and validated")
    ...
    if cleanup_attrs & {"_mutation_ledger", "_uncertain_records", "_receipts"}:
        raise Phase5GrantR10EvidenceError("grant cleanup erases durable mutation evidence")
    if "_global_generation" not in source or "_grant_generations" not in source:
        raise Phase5GrantR10EvidenceError("scoped generation barriers are missing")
    if "uncertain_needs_reconciliation" not in source or "consumed_verified" not in source:
        raise Phase5GrantR10EvidenceError("required mutation lifecycle states are missing")


def _constants(root: Path) -> dict[str, int]:
    tree = ast.parse(_path(root, "core/session_grants_v10.py").read_text(encoding="utf-8"))
    found: dict[str, int] = {}
    ...
    for node in tree.body:
        if isinstance(node, ast.Assign) and len(node.targets) == 1 and isinstance(node.targets[0], ast.Name) and node.targets[0].id in wanted:
            ...
            if type(value) is not int:
                raise Phase5GrantR10EvidenceError(f"limit is invalid: {node.targets[0].id}")
            found[node.targets[0].id] = value
    if set(found) != wanted:
        raise Phase5GrantR10EvidenceError("material limit set is incomplete")
    return {key: found[name] for key, name in _LIMIT_CONSTANTS.items()}


def _run_history(root: Path) -> dict[str, object]:
    try:
        history = r9._run_history(root)
        top = _parse_manifest(_path(root, R9_ROOT), (R9_ARTIFACT,))
        artifacts = _parse_manifest(_path(root, R9_ARTIFACT))
        _verify_entries(root, top + artifacts)
        result = r9.verify_evidence(root, current_base_commit=_current_commit(root), current_environment=_environment())
    except Exception as exc:
        raise Phase5GrantR10EvidenceError("V1-R9 historical verification failed") from exc
    expected = {"artifact_files": 40, "dependencies": 12, "live_scan": 76, "normative": 20, "root_files": 1, "tests": 40}
    if any(result.get(key) != value for key, value in expected.items()):
        raise Phase5GrantR10EvidenceError("V9 historical result is incomplete")
    history["v9"] = {
        "artifact_manifest": R9_ARTIFACT,
        "artifact_sha256": _sha(_path(root, R9_ARTIFACT)),
        "artifacts": len(artifacts),
        "root": R9_ROOT,
        "root_sha256": _sha(_path(root, R9_ROOT)),
    }
    return history
...
def _current_commit(root: Path) -> str:
    ...
    if result.returncode or not _HEX40.fullmatch(value):
        raise Phase5GrantR10EvidenceError("current git base commit is unavailable")
    return value
...
def _run_static_gates(root: Path, live_paths: tuple[str, ...]) -> dict[str, object]:
    ...
    for relative in compile_scope:
        try:
            ...
            raise Phase5GrantR10EvidenceError(f"live compile gate failed: {relative}") from exc
    ...
    if not ruff:
        raise Phase5GrantR10EvidenceError("live Ruff gate is unavailable")
    ...
    if ruff_result.returncode:
        raise Phase5GrantR10EvidenceError(f"live Ruff gate failed: {ruff_result.stdout}{ruff_result.stderr}")
    whitespace = subprocess.run([sys.executable, str(_path(root, "scripts/check_phase5_grants_r10_whitespace.py"))], cwd=root, capture_output=True, text=True, timeout=60, check=False, env={**os.environ, "PYTHONDONTWRITEBYTECODE": "1"})
    if whitespace.returncode or whitespace.stdout.strip() != "P51_GRANTS_R10_WHITESPACE_OK files=4":
        raise Phase5GrantR10EvidenceError("live whitespace gate failed")
    for relative in live_paths:
        ...
        if any(token in text for token in LIVE_FORBIDDEN_TOKENS):
            raise Phase5GrantR10EvidenceError(f"R10 is wired into live surface: {relative}")
    if _sha(_path(root, "scripts/launch_onyx.pyw")) == "0" * 64:
        raise Phase5GrantR10EvidenceError("operational launcher hash is invalid")
    return {
        "compile": 0,
...
def _require_equal(claim: object, actual: object, label: str) -> None:
    if claim != actual:
        raise Phase5GrantR10EvidenceError(f"{label} binding is false or incomplete")
...
def verify_evidence(root: Path = PROJECT, *, current_base_commit: str | None = None, current_environment: dict[str, str] | None = None) -> dict[str, object]:
    ...
    if bundle.get("contract") != "Phase5SessionGrantShadowEvidence.v10" or bundle.get("status") != "candidate-default-off-not-accepted":
        raise Phase5GrantR10EvidenceError("bundle contract/status is invalid")
    ...
    if not isinstance(files, dict) or tuple(sorted(files)) != tuple(sorted(leaf_paths)):
        raise Phase5GrantR10EvidenceError("bundle leaf set is not exact")
    for relative, digest in files.items():
        if type(digest) is not str or not _HEX64.fullmatch(digest) or _sha(_path(root, relative)) != digest:
            raise Phase5GrantR10EvidenceError(f"bundle leaf hash mismatch: {relative}")
    closure = local_import_closure(root)
    _require_equal(bundle.get("dependency_closure"), list(closure), "dependency closure")
    graph = derive_normative_graph(root)
    if "readme.md" not in graph:
        raise Phase5GrantR10EvidenceError("bare root readme normative fallback was not resolved recursively")
    ...
    if counts["failed"] or counts["errors"] or not _REQUIRED_TESTS <= names:
        raise Phase5GrantR10EvidenceError("focused semantic JUnit evidence is incomplete")
    for prefix, expected in _REQUIRED_PREFIX_COUNTS.items():
        if sum(name.startswith(prefix) for name in names) != expected:
            raise Phase5GrantR10EvidenceError(f"focused parametrized JUnit evidence differs: {prefix}")
    ...
    if raw.get("timestamp") != junit_timestamp or raw.get("exit_code") != "0" or raw.get("result") != f"{counts['passed']} passed" or any(raw.get(key) != str(counts[key]) for key in ("passed", "failed", "errors", "skipped")):
        raise Phase5GrantR10EvidenceError("raw/JUnit evidence differs")
    static = _kv_log(root, STATIC_LOG)
    expected_static = {
        "compile_mode": "recursive-live-in-memory-built-in-compile",
        "compile_exit_code": "0",
        "diff_check_command": "python scripts/check_phase5_grants_r10_whitespace.py",
    ...
    if not _HEX40.fullmatch(base) or bundle.get("base_commit") != base:
        raise Phase5GrantR10EvidenceError("bundle base commit is invalid or stale")
    ...
    if type(timestamp) is not str or timestamp != junit_timestamp:
        raise Phase5GrantR10EvidenceError("bundle timestamp differs from JUnit")
    try:
        parsed = datetime.fromisoformat(timestamp)
    except ValueError as exc:
        raise Phase5GrantR10EvidenceError("bundle timestamp is not ISO-8601") from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise Phase5GrantR10EvidenceError("bundle timestamp lacks timezone")
    return {
...
def main() -> int:
    ...
    print(
        f"P51_GRANTS_R10_EVIDENCE_OK root={result['root_files']} artifacts={result['artifact_files']} "
