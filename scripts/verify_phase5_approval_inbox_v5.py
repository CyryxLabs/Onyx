"""Freeze and verify Phase 5.2 Approval Inbox V5 candidate evidence."""
...
TOP_MANIFEST = "docs/onyx/VE-SOURCE-P52-APPROVAL-INBOX-V5-001.sha256"
ARTIFACT_MANIFEST = "docs/onyx/VE-ARTIFACTS-P52-APPROVAL-INBOX-V5-001.sha256"
EVIDENCE_DIR = "docs/onyx/checkpoints/phase5-approval-inbox-v5"
CHECKPOINT = f"{EVIDENCE_DIR}/PHASE5_2_APPROVAL_INBOX_V5_CHECKPOINT.md"
BUNDLE = f"{EVIDENCE_DIR}/phase5-approval-inbox-v5.bundle.json"
JUNIT = f"{EVIDENCE_DIR}/phase5-approval-inbox-v5.junit.xml"
RAW_LOG = f"{EVIDENCE_DIR}/phase5-approval-inbox-v5.raw.log"
STATIC_LOG = f"{EVIDENCE_DIR}/phase5-approval-inbox-v5.static.log"
LIVE_SCAN_MANIFEST = f"{EVIDENCE_DIR}/phase5-approval-inbox-v5.live-scan.sha256"
...
FROZEN_APPROVAL_PATHS = frozenset(
    {f"core/approval_inbox_v{version}.py" for version in range(1, 6)}
)
...
REQUIRED_TESTS = frozenset(
    {
        ...
        "test_v1_v2_v3_v4_and_r11_frozen_anchor_bytes_remain_exact",
...
class Phase52V5EvidenceError(RuntimeError):
    pass
...
def _path(relative: str, root: Path = PROJECT) -> Path:
    ...
    if pure.is_absolute() or not pure.parts or any(part in {"", ".", ".."} for part in pure.parts):
        raise Phase52V5EvidenceError(f"unsafe evidence path: {relative}")
...
def derive_limits(root: Path = PROJECT) -> dict[str, int]:
    tree = ast.parse(_path("core/approval_inbox_v5.py", root).read_text(encoding="utf-8"))
    limits: dict[str, int] = {}
    for node in tree.body:
        if (
            ...
            if type(node.value) is not ast.Constant or type(node.value.value) is not int:
                raise Phase52V5EvidenceError(f"limit is not a literal integer: {name}")
            if node.value.value <= 0 or name.casefold() in limits:
                raise Phase52V5EvidenceError(f"limit declaration is invalid: {name}")
            limits[name.casefold()] = node.value.value
    if not limits:
        raise Phase52V5EvidenceError("V5 declares no bounded limits")
...
def _identity(command: list[str], root: Path = PROJECT) -> str:
    ...
    if result.returncode or not value or "\n" in value or "\r" in value:
        raise Phase52V5EvidenceError(f"static tool identity unavailable: {command[0]}")
    return value
...
def current_base_commit(root: Path = PROJECT) -> str:
    ...
    if not _HEX40.fullmatch(value):
        raise Phase52V5EvidenceError("current git base commit is invalid")
    return value
    ...
    return {
        ...
        "static_tools": {
            ...
            "whitespace_checker_sha256": _sha(
                _path("scripts/check_phase5_approval_inbox_v5_whitespace.py", root)
            ),
        },
    }
...
def _parse_iso_timestamp(value: object, label: str) -> datetime:
    if type(value) is not str or not value or value.endswith("Z"):
        raise Phase52V5EvidenceError(f"{label} is not canonical ISO-8601")
    try:
        parsed = datetime.fromisoformat(value)
    except ValueError as exc:
        raise Phase52V5EvidenceError(f"{label} is not canonical ISO-8601") from exc
...
def validate_semantic_metadata(
    ...
    if bundle.get("limits") != derive_limits(root):
        raise Phase52V5EvidenceError("bundle limits do not match V5 code constants")
    if bundle.get("base_commit") != current_base_commit(root):
        raise Phase52V5EvidenceError("bundle base commit does not match current git HEAD")
    if bundle.get("environment") != actual_environment(root):
        raise Phase52V5EvidenceError("bundle environment/static tool identity drifted")
    expected_timestamp = canonical_evidence_timestamp(junit_timestamp)
    if bundle.get("timestamp") != expected_timestamp:
        raise Phase52V5EvidenceError("bundle timestamp is not derived from focused JUnit")
...
def _parse_manifest(
    ...
    if b"\r" in raw or not raw.endswith(b"\n"):
        raise Phase52V5EvidenceError(f"manifest is noncanonical: {relative}")
    entries: list[tuple[str, str]] = []
    for line in raw.decode("utf-8").splitlines():
        digest, separator, path = line.partition("  ")
        if not separator or not _HEX64.fullmatch(digest):
            raise Phase52V5EvidenceError(f"manifest line is malformed: {relative}")
        _path(path, root)
    ...
    if not entries or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)):
        raise Phase52V5EvidenceError(f"manifest order/set is invalid: {relative}")
    if expected is not None and paths != expected:
        raise Phase52V5EvidenceError(f"manifest exact set drifted: {relative}")
...
def _verify_entries(entries: tuple[tuple[str, str], ...], root: Path = PROJECT) -> None:
    for expected, relative in entries:
        ...
        if not path.is_file() or _sha(path) != expected:
            raise Phase52V5EvidenceError(f"artifact digest drifted: {relative}")
...
def _historical_manifest_entries(
    ...
    """Read legacy ordering without weakening V5's own strict manifests."""
    raw = _path(relative, root).read_bytes()
    if b"\r" in raw or not raw.endswith(b"\n"):
        raise Phase52V5EvidenceError(f"historical manifest is malformed: {relative}")
    entries: list[tuple[str, str]] = []
    for line in raw.decode("utf-8").splitlines():
        digest, separator, referenced = line.partition("  ")
        if not separator or not _HEX64.fullmatch(digest):
            raise Phase52V5EvidenceError(f"historical manifest line is malformed: {relative}")
    ...
    if not references or len(references) != len(set(references)):
        raise Phase52V5EvidenceError(f"historical manifest set is invalid: {relative}")
    return tuple(entries)
...
def verify_historical_manifest_tree(
    ...
    while queue:
        ...
        path = _path(relative, root)
        if not path.is_file():
            raise Phase52V5EvidenceError(f"historical artifact missing: {relative}")
        if expected is not None and _sha(path) != expected:
            raise Phase52V5EvidenceError(f"historical artifact digest drifted: {relative}")
...
def materialize_manifest_tree(
    ...
    while queue:
        ...
        if not source.is_file():
            raise Phase52V5EvidenceError(f"frozen materialization source missing: {relative}")
...
def execute_frozen_verifier(root: Path, relative: str) -> subprocess.CompletedProcess[str]:
    ...
    if bundle_relative is None:
        raise Phase52V5EvidenceError("unknown frozen verifier")
    ...
    if type(executable) is not str or not Path(executable).is_file():
        raise Phase52V5EvidenceError("frozen verifier interpreter is unavailable")
...
def verify_r11_recursively() -> str:
    with tempfile.TemporaryDirectory(
        prefix=".onyx-p52-v5-r11-", dir=PROJECT
    ) as directory:
        root = Path(directory)
...
def verify_v1_recursively() -> str:
    with tempfile.TemporaryDirectory(
        prefix=".onyx-p52-v5-v1-", dir=PROJECT
    ) as directory:
        root = Path(directory)
        materialize_v1(root)
...
def verify_v2_recursively() -> str:
    with tempfile.TemporaryDirectory(
        prefix=".onyx-p52-v5-v2-", dir=PROJECT
    ) as directory:
        root = Path(directory)
        materialize_v2(root)
...
def verify_v3_recursively() -> str:
    with tempfile.TemporaryDirectory(
        prefix=".onyx-p52-v5-v3-", dir=PROJECT
    ) as directory:
        root = Path(directory)
        materialize_v3(root)
...
def local_import_closure() -> tuple[str, ...]:
    ...
    while queue:
        ...
        try:
            ...
            raise Phase52V5EvidenceError(f"source dependency cannot be parsed: {relative}") from exc
...
def _import_targets(
    ...
    try:
        ...
        raise Phase52V5EvidenceError(f"startup source is invalid: {relative}") from exc
    current = PurePosixPath(relative).with_suffix("").parts
    package = list(current[:-1])
    targets: set[str] = set()
    module_aliases = {"importlib": "importlib", "runpy": "runpy"}
    function_aliases = {"__import__": "__import__"}
    for node in ast.walk(tree):
        if type(node) is ast.Import:
            for alias in node.names:
                if alias.name in {"importlib", "runpy"}:
                    module_aliases[alias.asname or alias.name] = alias.name
        elif type(node) is ast.ImportFrom and node.module in {"importlib", "runpy"}:
            expected = "import_module" if node.module == "importlib" else "run_module"
            for alias in node.names:
                if alias.name == expected:
                    function_aliases[alias.asname or alias.name] = (
                        f"{node.module}.{expected}"
                    )
    for node in ast.walk(tree):
        names: list[str] = []
        if type(node) is ast.Import:
            ...
            if type(node.func) is ast.Name:
                loader = function_aliases.get(node.func.id)
            elif (
                ...
                expected = "import_module" if module == "importlib" else "run_module"
                if node.func.attr == expected:
                    loader = f"{module}.{expected}"
            if loader is not None:
                if (
                    not node.args
                    or type(node.args[0]) is not ast.Constant
...
def live_scan_paths(root: Path = PROJECT) -> tuple[str, ...]:
    ...
    if "scripts/launch_onyx.pyw" not in paths:
        raise Phase52V5EvidenceError("operational launcher is absent from live scan")
...
def _junit() -> tuple[dict[str, int], str, set[str]]:
    try:
        ...
        raise Phase52V5EvidenceError("JUnit is invalid") from exc
    suites = [document] if document.tag == "testsuite" else document.findall("testsuite")
    if len(suites) != 1:
        raise Phase52V5EvidenceError("JUnit must contain exactly one suite")
    suite = suites[0]
    ...
    if len(cases) != EXPECTED_TEST_COUNT or counts != {"errors": 0, "failed": 0, "passed": EXPECTED_TEST_COUNT, "skipped": 0}:
        raise Phase52V5EvidenceError("focused JUnit count/result is invalid")
    if not REQUIRED_TESTS <= names:
        raise Phase52V5EvidenceError("focused JUnit lacks a required test")
    for prefix, expected in REQUIRED_PREFIX_COUNTS.items():
        if sum(name.startswith(prefix) for name in names) != expected:
            raise Phase52V5EvidenceError(f"focused parameter coverage drifted: {prefix}")
...
def _verify_static() -> dict[str, int]:
    ...
    if b"\r" in raw or not raw.endswith(b"\n"):
        raise Phase52V5EvidenceError("static log is noncanonical")
    values: dict[str, str] = {}
    for line in raw.decode().splitlines():
        key, separator, value = line.partition("=")
        if not separator or key in values:
            raise Phase52V5EvidenceError("static log shape is invalid")
        values[key] = value
    if values != expected:
        raise Phase52V5EvidenceError("static gates are not all green")
...
def _verify_raw_log() -> dict[str, object]:
    ...
    if b"\r" in raw or not raw.endswith(b"\n"):
        raise Phase52V5EvidenceError("focused raw log is noncanonical")
    text = raw.decode("utf-8")
    matches = _RAW_PASS.findall(text)
    if len(matches) != 1 or int(matches[0][0]) != EXPECTED_TEST_COUNT:
        raise Phase52V5EvidenceError("focused raw log pass claim is invalid")
    if " failed" in text.casefold() or " error" in text.casefold():
        raise Phase52V5EvidenceError("focused raw log contains a failure claim")
    return {
    ...
    for relative, expected in {
        ...
        **R11_SHA256,
        ...
        if _sha(_path(relative)) != expected:
            raise Phase52V5EvidenceError(f"historical/accepted anchor drifted: {relative}")
...
def _read_bundle() -> dict[str, object]:
    ...
    try:
        ...
        raise Phase52V5EvidenceError("bundle is invalid JSON") from exc
    if type(value) is not dict or raw != _canonical(value):
        raise Phase52V5EvidenceError("bundle is not canonical")
    return value
...
def verify(*, recursive: bool = True) -> dict[str, object]:
    ...
    if recursive:
        ...
        v3_output = verify_v3_recursively()
        if "P51_GRANTS_R11_EVIDENCE_OK" not in r11_output:
            raise Phase52V5EvidenceError("recursive R11 verifier did not report success")
        if "P52_APPROVAL_INBOX_V1_FROZEN_OK" not in v1_output:
            raise Phase52V5EvidenceError("recursive V1 verifier did not report success")
        if "P52_APPROVAL_INBOX_V2_FROZEN_OK" not in v2_output:
            raise Phase52V5EvidenceError("recursive V2 verifier did not report success")
        if "P52_APPROVAL_INBOX_V3_FROZEN_OK" not in v3_output:
            raise Phase52V5EvidenceError("recursive V3 verifier did not report success")
    ...
    if bundle.get("contract") != "Phase52ApprovalInboxEvidence.v5" or bundle.get("status") != "candidate-default-off-not-accepted":
        raise Phase52V5EvidenceError("bundle contract/status is invalid")
    if bundle.get("counts") != counts or bundle.get("junit_timestamp") != timestamp:
        raise Phase52V5EvidenceError("bundle test evidence drifted")
    ...
    if bundle.get("test_evidence") != expected_test_evidence:
        raise Phase52V5EvidenceError("bundle JUnit/raw/static claims drifted")
    if bundle.get("claims") != claims or bundle.get("files") != files:
        raise Phase52V5EvidenceError("bundle claims/files drifted")
    if bundle.get("dependency_closure") != list(closure) or bundle.get("normative_graph") != graph:
        raise Phase52V5EvidenceError("bundle dependency/normative closure drifted")
    if bundle.get("live_scan_paths") != list(live) or bundle.get("dag") != expected_dag:
        raise Phase52V5EvidenceError("bundle live/DAG closure drifted")
    if bundle.get("history") != {
        ...
        raise Phase52V5EvidenceError("bundle history claim drifted")
    if bundle.get("external_e6") != {"accepted": False, "required_before_acceptance": True, "status": "pending-external-review"}:
        raise Phase52V5EvidenceError("bundle external acceptance status is invalid")
    if bundle.get("feature_flag") != {"default": False, "name": "ONYX_APPROVAL_INBOX_V5"}:
        raise Phase52V5EvidenceError("bundle feature flag claim is invalid")
    validate_semantic_metadata(bundle, timestamp)
    print(
        "P52_APPROVAL_INBOX_V5_FROZEN_OK "
        f"tests={counts['passed']} artifacts={len(expected_artifacts)} "
        f"dependencies={len(closure)} normative={len(graph)} live={len(live)} "
        "r11_recursive=ok v3_recursive=ok v2_recursive=ok v1_recursive=ok"
    )
    return bundle
...
def freeze() -> None:
    ...
    if focused.returncode:
        raise Phase52V5EvidenceError("focused tests failed during freeze")
    commands = {
        "RUFF_F_E9": [
            sys.executable,
            "-m",
        ...
        "WHITESPACE": [sys.executable, "scripts/check_phase5_approval_inbox_v5_whitespace.py"],
    ...
    except Phase52V5EvidenceError as exc:
    ...
    try:
        ...
        detail += str(exc)
    try:
        verify_v3_recursively()
        gates["V3_RECURSIVE"] = 0
    except Phase52V5EvidenceError as exc:
        gates["V3_RECURSIVE"] = 1
        detail += str(exc)
    if any(gates.values()):
        raise Phase52V5EvidenceError(f"static gate failed: {gates}\n{detail}")
    ...
    bundle = {
        ...
        "claims": claims,
        "contract": "Phase52ApprovalInboxEvidence.v5",
        "counts": counts,
        "dag": {
        ...
        "feature_flag": {"default": False, "name": "ONYX_APPROVAL_INBOX_V5"},
        "files": files,
        "history": {
            "phase51_r11": {"decision": "accepted-default-off", "anchors": R11_SHA256},
            "phase52_v1": {"decision": "historical-rejected", "anchors": V1_SHA256},
