"""Verify the frozen Phase 5.3 Capability Nexus V32 candidate."""
...
EVIDENCE = "docs/onyx/checkpoints/phase5-capability-nexus-v32"
BUNDLE = f"{EVIDENCE}/phase5-capability-nexus-v32.bundle.json"
JUNIT = f"{EVIDENCE}/phase5-capability-nexus-v32.junit.xml"
RAW = f"{EVIDENCE}/phase5-capability-nexus-v32.raw.log"
STATIC = f"{EVIDENCE}/phase5-capability-nexus-v32.static.log"
LIVE = f"{EVIDENCE}/phase5-capability-nexus-v32.live-scan.sha256"
METRICS = f"{EVIDENCE}/phase5-capability-nexus-v32.metrics.json"
COMBINED_JUNIT = f"{EVIDENCE}/phase5-capability-nexus-v32.combined.junit.xml"
REGRESSION_JUNIT = f"{EVIDENCE}/phase5-capability-nexus-v32.regressions.junit.xml"
REGRESSION_REPORT = f"{EVIDENCE}/phase5-capability-nexus-v32.regressions.json"
...
CHECKPOINT = f"{EVIDENCE}/PHASE5_3_CAPABILITY_NEXUS_V32_CHECKPOINT.md"
CORE = "core/capability_nexus_v32.py"
TESTS = "tests/test_capability_nexus_v32.py"
WHITESPACE = "scripts/check_phase5_capability_nexus_v32_whitespace.py"
SELF = "scripts/verify_phase5_capability_nexus_v32.py"
WORKER = "scripts/verify_phase5_capability_nexus_v32_worker.py"
REPORTER = "scripts/phase5_capability_nexus_v32_subtest_reporter.py"
MUTABLE_PROJECTION_MAP = f"{EVIDENCE}/phase5-capability-nexus-v32.mutable-projections.json"
MUTABLE_PROJECTIONS = (
...
COMBINED_SUITES = tuple(f"tests/test_capability_nexus_v{version}.py" for version in range(1, 33))
...
V31_EVIDENCE = "docs/onyx/checkpoints/phase5-capability-nexus-v31"
V31_BUNDLE = f"{V31_EVIDENCE}/phase5-capability-nexus-v31.bundle.json"
V31_JUNIT = f"{V31_EVIDENCE}/phase5-capability-nexus-v31.junit.xml"
V31_STATIC = f"{V31_EVIDENCE}/phase5-capability-nexus-v31.static.log"
V31_LIVE = "docs/onyx/checkpoints/phase5-capability-nexus-v31/phase5-capability-nexus-v31.live-scan.sha256"
V31_CHECKPOINT = f"{V31_EVIDENCE}/PHASE5_3_CAPABILITY_NEXUS_V31_CHECKPOINT.md"
V31_VERIFIER = "scripts/verify_phase5_capability_nexus_v31.py"
V1_HISTORY = {
...
V31_HISTORY = {
    ...
    "tests/test_capability_nexus_v31.py": "3c32ad903c0177d2924913886247dccc8e1f4f8d3111e576bd42b6e1fda4b671",
    ...
    V31_ARTIFACT_MANIFEST: "e332c904fac7fe9f53cd976210637fbb1a3b8aa74d55e828c95aa9ca448bc3ed",
    V31_ROOT_MANIFEST: "1fe65cb6c5e238c7b1066c72dbcab9d5967e3c8fc9d9055aa56cc30d0e4b888d",
}
HISTORICAL_ROOTS = (
    ...
    "docs/onyx/VE-SOURCE-P53-CAPABILITY-NEXUS-V31-001.sha256",
)
_LINE = re.compile(r"^([0-9a-f]{64})  ([A-Za-z0-9_./-]+)$")
_UINT_LEXICAL = re.compile(r"(?:0|[1-9][0-9]*)")
_TIME_LEXICAL = re.compile(r"(?:0|[1-9][0-9]*)\.[0-9]{3}")
_PRIVATE_BASE = Path(tempfile.gettempdir()) / "onyx-p53-v32-private"
_PRIVATE_SENTINEL = ".phase53-v32-private"
_PRIVATE_CONTRACT = "Phase53CapabilityNexusPrivate.v32"
PATH_INTEGRITY_CONTRACT = {
    "atomic_with_following_read_hash_or_launch": False,
    "concurrent_writer_toctou_security_boundary": False,
    "mode": "stable_filesystem_state_metadata_precheck",
    "race_reproduction": "tests/test_capability_nexus_v32.py::test_path_metadata_precheck_race_window_is_deterministically_disclosed",
    "scope": "worker_and_parent_authoritative_path_gates",
    "trusted_state_requirement": "authoritative_tree_not_concurrently_mutated",
}


class V32EvidenceError(RuntimeError):
    pass
...
def _canonical_authoritative_parts(relative: str) -> tuple[str, ...]:
    if type(relative) is not str or not relative or "\\" in relative or "\x00" in relative:
        raise V32EvidenceError(f"authoritative path is not canonical: {relative!r}")
    ...
    if (
        ...
        raise V32EvidenceError(f"authoritative path is not canonical: {relative}")
    return parts


def _assert_no_reparse(relative: str) -> None:
    """Validate metadata in a stable filesystem state before separate content access.

    This is deliberately not an atomic concurrent-writer or TOCTOU security boundary:
    callers read or hash only after this metadata-only precheck returns.
    """
    ...
    try:
        ...
        raise V32EvidenceError(f"authoritative workspace root is missing: {relative}") from exc
    if stat.S_ISLNK(root_info.st_mode) or getattr(root_info, "st_file_attributes", 0) & reparse_flag:
        raise V32EvidenceError(f"authoritative workspace root is a reparse point: {relative}")
    if str(PROJECT.absolute()) != str(root_resolved):
        raise V32EvidenceError(f"authoritative workspace root uses a path alias: {relative}")
    current = PROJECT
    for component in parts:
        ...
        try:
            ...
            raise V32EvidenceError(f"authoritative path is missing: {relative}") from exc
        if stat.S_ISLNK(info.st_mode) or getattr(info, "st_file_attributes", 0) & reparse_flag:
            raise V32EvidenceError(f"authoritative path contains a reparse component: {relative}")
        try:
            resolved_component = current.resolve(strict=True)
        except OSError as exc:
            raise V32EvidenceError(f"authoritative path is missing: {relative}") from exc
        if resolved_component.name != component:
            raise V32EvidenceError(f"authoritative path uses a case or name alias: {relative}")
    try:
        current.resolve(strict=True).relative_to(root_resolved)
    except (OSError, ValueError) as exc:
        raise V32EvidenceError(f"authoritative path escapes workspace: {relative}") from exc
...
def _manifest(relative: str) -> tuple[tuple[str, str], ...]:
    ...
    if b"\r" in raw or not raw.endswith(b"\n"):
        raise V32EvidenceError(f"manifest line endings are invalid: {relative}")
    entries = []
    for line in raw.decode().splitlines():
        match = _LINE.fullmatch(line)
        if match is None:
            raise V32EvidenceError(f"manifest line is invalid: {relative}")
        digest, path = match.groups()
        if path.startswith("/") or ".." in Path(path).parts or "\\" in path:
            raise V32EvidenceError("manifest path is unsafe")
        entries.append((digest, path))
    paths = [path for _, path in entries]
    if not entries or paths != sorted(paths) or len(paths) != len(set(paths)):
        raise V32EvidenceError(f"manifest set is invalid: {relative}")
...
def _historical_manifest(relative: str) -> tuple[tuple[str, str], ...]:
    ...
    try:
        ...
        raise V32EvidenceError(f"historical manifest encoding is invalid: {relative}") from exc
    entries = []
    for line in lines:
        match = _LINE.fullmatch(line)
        if match is None:
            raise V32EvidenceError(f"historical manifest line is invalid: {relative}")
        digest, path = match.groups()
        if path.startswith("/") or ".." in Path(path).parts or "\\" in path:
            raise V32EvidenceError("historical manifest path is unsafe")
        entries.append((digest, path))
    paths = [path for _, path in entries]
    if not entries or len(paths) != len(set(paths)):
        raise V32EvidenceError(f"historical manifest set is invalid: {relative}")
    return tuple(entries)
...
def _dag() -> tuple[str, ...]:
    ...
    if root != ((_sha(PROJECT / ARTIFACT_MANIFEST), ARTIFACT_MANIFEST),):
        raise V32EvidenceError("root manifest edge is invalid")
    ...
    required = {
        ...
        *V11_HISTORY, *V12_HISTORY, *V13_HISTORY, *V14_HISTORY, *V15_HISTORY, *V16_HISTORY, *V17_HISTORY, *V18_HISTORY, *V19_HISTORY, *V20_HISTORY, *V21_HISTORY, *V22_HISTORY, *V23_HISTORY, *V24_HISTORY, *V25_HISTORY, *V26_HISTORY, *V27_HISTORY, *V28_HISTORY, *V29_HISTORY, *V30_HISTORY, *V31_HISTORY,
    }
    if not required.issubset(paths) or ARTIFACT_MANIFEST in paths or ROOT_MANIFEST in paths:
        raise V32EvidenceError("artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _assert_no_reparse(relative)
        if not (PROJECT / relative).is_file() or _sha(PROJECT / relative) != digest:
            raise V32EvidenceError(f"artifact hash mismatch: {relative}")
    return paths
...
def _bundle(paths: tuple[str, ...]) -> dict[str, object]:
    ...
    try:
        ...
        raise V32EvidenceError("bundle JSON is invalid") from exc
    if raw != json.dumps(bundle, sort_keys=True, separators=(",", ":")) + "\n":
        raise V32EvidenceError("bundle JSON is not canonical")
    if bundle.get("contract") != "Phase53CapabilityNexusEvidence.v32" or bundle.get("status") != "candidate-default-off-shadow-only-not-accepted":
        raise V32EvidenceError("bundle contract/status is invalid")
    if bundle.get("external_e6") != {"accepted": False, "required_before_acceptance": True, "status": "pending-external-review"}:
        raise V32EvidenceError("bundle overstates acceptance")
    if bundle.get("feature_gate") != {"dispatch_enabled": False, "nexus_enabled": False, "shadow_mode": True}:
        raise V32EvidenceError("bundle feature gate is invalid")
    if bundle.get("path_integrity_contract") != PATH_INTEGRITY_CONTRACT:
        raise V32EvidenceError("bundle path integrity contract is invalid")
    limitations = bundle.get("limitations")
    if not isinstance(limitations, list) or set(limitations) != {
        "api_read_only_snapshot_not_reflection_boundary",
        "candidate_not_live",
        "external_e6_pending",
        "metadata_precheck_not_atomic_concurrent_writer_boundary",
        "mutable_projection_paths_not_historical_authority",
        "toctou_requires_stable_authoritative_tree",
    } or len(limitations) != 6:
        raise V32EvidenceError("bundle limitations are incomplete or duplicated")
    files = bundle.get("files")
    expected = set(paths) - {BUNDLE}
    if not isinstance(files, dict) or set(files) != expected:
        raise V32EvidenceError("bundle file set is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V32EvidenceError(f"bundle file hash mismatch: {relative}")
    return bundle
...
def _validate_regression_report(report: dict[str, object]) -> None:
    if set(report) != {"base_tests", "contract", "exit_status", "subtests"}:
        raise V32EvidenceError("regression report schema is invalid")
    if report.get("contract") != "Phase53CapabilityNexusRegressionReport.v32" or report.get("exit_status") != 0:
        raise V32EvidenceError("regression report contract/status is invalid")
    ...
    if not isinstance(bases, list) or not isinstance(subtests, list) or len(bases) != 102 or len(subtests) != 221:
        raise V32EvidenceError("regression report semantic counts are invalid")
    ...
    for base in bases:
        if not isinstance(base, dict) or set(base) != {"identity", "nodeid", "outcome"} or base.get("outcome") != "passed":
            raise V32EvidenceError("regression base record is invalid")
        ...
        if not isinstance(nodeid, str) or identity != expected or nodeid in base_nodeids or identity in base_identities:
            raise V32EvidenceError("regression base identity is invalid or duplicated")
    ...
    for subtest in subtests:
        ...
        if not isinstance(subtest, dict) or set(subtest) != expected_keys or subtest.get("outcome") != "passed":
            raise V32EvidenceError("regression subtest record is invalid")
        ...
        if (
            ...
            raise V32EvidenceError("regression subtest parent/context is invalid")
        ...
        if (
            ...
            raise V32EvidenceError("regression subtest identity is invalid or duplicated")
    ...
    if any(sorted(values) != list(range(len(values))) for values in ordinals.values()):
        raise V32EvidenceError("regression subtest ordinals are not contiguous")
...
def _junit_nodeid(testcase: ET.Element) -> str:
    ...
    if not classname.startswith(prefix) or not name:
        raise V32EvidenceError("regression JUnit testcase identity is invalid")
...
def _require_whitespace_only(value: str | None, label: str) -> None:
    if value is not None and value.strip():
        raise V32EvidenceError(f"JUnit {label} contains unexpected text")


def _canonical_counter(value: str, label: str) -> int:
    if _UINT_LEXICAL.fullmatch(value) is None:
        raise V32EvidenceError(f"JUnit {label} counter lexical form is invalid")
    return int(value)


def _canonical_time(value: str, label: str) -> float:
    if _TIME_LEXICAL.fullmatch(value) is None:
        raise V32EvidenceError(f"JUnit {label} duration lexical form is invalid")
    duration = float(value)
    if not math.isfinite(duration) or duration < 0:
        raise V32EvidenceError(f"JUnit {label} duration is invalid")
    return duration
...
def _junit_path(path: Path, *, subtest_report: dict[str, object] | None = None) -> dict[str, int]:
    ...
    if document_body.startswith(b"<?xml"):
        ...
        if declaration_end < 0:
            raise V32EvidenceError("JUnit XML declaration is malformed")
        document_body = document_body[declaration_end + 2:]
    if b"<!--" in raw or b"<?" in document_body:
        raise V32EvidenceError("JUnit comments and processing instructions are forbidden")
    try:
        root = ET.fromstring(raw)
    except ET.ParseError as exc:
        raise V32EvidenceError("JUnit XML is malformed") from exc
    if root.tag != "testsuites" or root.attrib != {"name": "pytest tests"}:
        raise V32EvidenceError("JUnit root schema is invalid")
    ...
    if len(suites) != 1 or suites[0].tag != "testsuite":
        raise V32EvidenceError("JUnit must contain exactly one suite")
    ...
    for suite in suites:
        ...
        if set(suite.attrib) != allowed_suite or suite.attrib.get("name") != "pytest":
            raise V32EvidenceError("JUnit suite attribute schema is invalid")
        _canonical_time(suite.attrib["time"], "suite")
        try:
            datetime.fromisoformat(suite.attrib["timestamp"])
        except (ValueError, TypeError) as exc:
            raise V32EvidenceError("JUnit suite timestamp is invalid") from exc
        if not suite.attrib["hostname"]:
            raise V32EvidenceError("JUnit suite hostname is invalid")
        ...
        if any(item.tag != "testcase" for item in children):
            raise V32EvidenceError("JUnit suite contains an unexpected child")
        for testcase in children:
            if set(testcase.attrib) != {"classname", "name", "time"}:
                raise V32EvidenceError("JUnit testcase attribute schema is invalid")
            if not testcase.attrib["classname"] or not testcase.attrib["name"]:
                raise V32EvidenceError("JUnit testcase identity is empty")
            ...
            if len(outcome_children) > 1 or any(item.tag not in {"failure", "error", "skipped"} for item in outcome_children):
                raise V32EvidenceError("JUnit testcase contains an unexpected child")
            for outcome in outcome_children:
                if set(outcome.attrib) - {"message", "type"} or list(outcome):
                    raise V32EvidenceError("JUnit outcome element schema is invalid")
        ...
        if attrs["failures"] != child_failed or attrs["errors"] != child_errors or attrs["skipped"] != child_skipped:
            raise V32EvidenceError("JUnit suite counters do not reconcile with testcase children")
        tests += attrs["tests"]
        failed += child_failed
        errors += child_errors
    ...
    if delta < 0:
        raise V32EvidenceError("JUnit reports fewer tests than testcase children")
    if delta:
        if subtest_report is None or len(subtest_report["subtests"]) != delta:
            raise V32EvidenceError("JUnit synthetic count delta lacks validated subtest evidence")
        bases = subtest_report["base_tests"]
        if len(bases) != testcase_count:
            raise V32EvidenceError("JUnit testcase count does not match base-test evidence")
        ...
        if len(xml_nodeids) != testcase_count or xml_nodeids != report_nodeids:
            raise V32EvidenceError("JUnit testcase identities do not match base-test evidence")
        if failed or errors or skipped or any(item["outcome"] != "passed" for item in bases):
            raise V32EvidenceError("JUnit outcomes do not match all-pass base/subtest evidence")
...
def _static_path(path: Path, *, frozen_gate: str = "FROZEN_PROOF") -> dict[str, int]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if not separator or not value.isdigit():
            raise V32EvidenceError("static log is invalid")
    ...
    if set(result) != expected or any(result.values()):
        raise V32EvidenceError("static gate failed")
    return result
...
def _canonical_json(relative: str) -> dict[str, object]:
    ...
    try:
        ...
        raise V32EvidenceError(f"structured evidence JSON is invalid: {relative}") from exc
    if not isinstance(value, dict) or raw != json.dumps(value, sort_keys=True, separators=(",", ":")) + "\n":
        raise V32EvidenceError(f"structured evidence JSON is not canonical: {relative}")
    return value
...
def _projection_mapping() -> dict[str, dict[str, str]]:
    ...
    if (
        set(value) != {"contract", "entries", "path_integrity_contract"}
        or value.get("contract") != "Phase53CapabilityNexusMutableProjections.v32"
        or value.get("path_integrity_contract") != PATH_INTEGRITY_CONTRACT
    ):
        raise V32EvidenceError("mutable projection mapping contract is invalid")
    entries = value.get("entries")
    if not isinstance(entries, list) or len(entries) != len(MUTABLE_PROJECTIONS):
        raise V32EvidenceError("mutable projection mapping cardinality is invalid")
    result: dict[str, dict[str, str]] = {}
    for entry in entries:
        if not isinstance(entry, dict) or set(entry) != {
            "path", "snapshot", "snapshot_sha256", "source_sha256",
        } or not all(isinstance(item, str) for item in entry.values()):
            raise V32EvidenceError("mutable projection mapping entry is invalid")
        path = entry["path"]
        ...
        if (
            ...
            raise V32EvidenceError("mutable projection mapping is not exact")
        _assert_no_reparse(snapshot)
        if _sha(PROJECT / snapshot) != expected_digest:
            raise V32EvidenceError("mutable projection snapshot hash mismatch")
        result[path] = entry
    if tuple(result) != MUTABLE_PROJECTIONS or tuple(
        entry["snapshot"] for entry in result.values()
    ) != MUTABLE_PROJECTION_SNAPSHOTS:
        raise V32EvidenceError("mutable projection allowlist is missing, extra, or reordered")
    return result
...
def _sum_counts(items: list[dict[str, int]]) -> dict[str, int]:
    ...
    if not items or any(set(item) != keys for item in items):
        raise V32EvidenceError("JUnit count schema is invalid")
...
def _assert_exact_evidence(label: str, actual: object, expected: object) -> None:
    if type(actual) is not type(expected):
        raise V32EvidenceError(f"{label} evidence type drift")
    if isinstance(expected, dict):
        if set(actual) != set(expected):  # type: ignore[arg-type]
            raise V32EvidenceError(f"{label} evidence field drift")
    ...
    if isinstance(expected, list):
        if len(actual) != len(expected):  # type: ignore[arg-type]
            raise V32EvidenceError(f"{label} evidence list drift")
        for index, value in enumerate(expected):
            _assert_exact_evidence(f"{label}[{index}]", actual[index], value)  # type: ignore[index]
        return
    if actual != expected:
        raise V32EvidenceError(f"{label} evidence value drift")
...
def _checkpoint() -> None:
    ...
    if not required.issubset(set(text.splitlines())):
        raise V32EvidenceError("checkpoint status or structured-evidence claim is invalid")
    ...
    if unchecked.search(text):
        raise V32EvidenceError("checkpoint contains an unchecked quantitative claim")
...
def _validate_metrics(
    ...
    expected_keys = {
        ...
        "path_integrity", "regressions", "runtime", "static", "tamper",
    }
    if set(metrics) != expected_keys or metrics.get("contract") != "Phase53CapabilityNexusMetrics.v32":
        raise V32EvidenceError("structured evidence top-level schema is invalid")
    ...
    if regression_junit.get("passed") != 323:
        raise V32EvidenceError("structured regression JUnit semantic count drift")
    ...
    _assert_exact_evidence(
        "path_integrity", metrics.get("path_integrity"), PATH_INTEGRITY_CONTRACT
    )
    ...
    if not isinstance(runtime, dict) or set(runtime) != {
        ...
        raise V32EvidenceError("structured runtime evidence schema is invalid")
    if (
        ...
        or runtime.get("total_timeout_seconds") != 300
        ...
        raise V32EvidenceError("structured runtime timeout evidence drift")
    measured = runtime.get("measured_full_verifier_seconds")
    if type(measured) not in {int, float} or not 0 < measured <= 300:
        raise V32EvidenceError("structured measured verifier runtime is invalid")
    return metrics
...
def _validate_frozen_v14_proof() -> dict[str, object]:
    ...
    if root != expected_root:
        raise V32EvidenceError("frozen V14 root manifest edge is invalid")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V14 artifact manifest closure is invalid")
    for digest, relative in artifacts:
    ...
    if (
        ...
        raise V32EvidenceError("frozen V14 bundle contract/status is invalid")
    files = bundle.get("files")
    expected_files = set(artifact_paths) - {V14_BUNDLE}
    if not isinstance(files, dict) or set(files) != expected_files:
        raise V32EvidenceError("frozen V14 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V14 bundle file hash mismatch: {relative}")
...
def _validate_frozen_v15_proof() -> dict[str, object]:
    ...
    if root != expected_root:
        raise V32EvidenceError("frozen V15 root manifest edge is invalid")
    ...
    if len(artifacts) != 179 or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)) or not required.issubset(paths):
        raise V32EvidenceError("frozen V15 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V32EvidenceError("frozen V15 bundle contract/status is invalid")
    files = bundle.get("files")
    if not isinstance(files, dict) or set(files) != set(paths) - {V15_BUNDLE}:
        raise V32EvidenceError("frozen V15 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V15 bundle file hash mismatch: {relative}")
    expected_history = {
        f"phase53_v{version}": {"decision": "rejected", "hashes": history}
        for version, history in enumerate(
    ...
    if bundle.get("history") != expected_history:
        raise V32EvidenceError("frozen V15 rejected-history contract is invalid")
    junit = _junit_path(PROJECT / V15_JUNIT)
    if junit != {"errors": 0, "failed": 0, "passed": 1798, "skipped": 0} or bundle.get("counts") != junit:
        raise V32EvidenceError("frozen V15 JUnit evidence is invalid")
    static = _static_path(PROJECT / V15_STATIC)
    if bundle.get("static_gates") != static:
        raise V32EvidenceError("frozen V15 static evidence is invalid")
    ...
    if live_entries != (
        ...
        raise V32EvidenceError("frozen V15 live manifest anchors are invalid")
    if bundle.get("live_scan") != {"count": 94, "manifest_sha256": _sha(PROJECT / V15_LIVE)}:
        raise V32EvidenceError("frozen V15 live evidence is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V15_EVIDENCE_OK" not in verifier:
        raise V32EvidenceError("frozen V15 checkpoint/marker contract is invalid")
    return {
...
def _validate_frozen_v16_proof() -> dict[str, object]:
    ...
    if root != ((V16_HISTORY[V16_ARTIFACT_MANIFEST], V16_ARTIFACT_MANIFEST),):
        raise V32EvidenceError("frozen V16 root manifest edge is invalid")
    ...
    if len(artifacts) != 197 or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)) or not required.issubset(paths):
        raise V32EvidenceError("frozen V16 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V32EvidenceError("frozen V16 bundle contract/status is invalid")
    files = bundle.get("files")
    if not isinstance(files, dict) or set(files) != set(paths) - {V16_BUNDLE}:
        raise V32EvidenceError("frozen V16 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V16 bundle file hash mismatch: {relative}")
    junit = _junit_path(PROJECT / V16_JUNIT)
    if junit != {"errors": 0, "failed": 0, "passed": 1826, "skipped": 0} or bundle.get("counts") != junit:
        raise V32EvidenceError("frozen V16 JUnit evidence is invalid")
    static = _static_path(PROJECT / V16_STATIC)
    if bundle.get("static_gates") != static:
        raise V32EvidenceError("frozen V16 static evidence is invalid")
    ...
    if live_entries != (
        ...
        raise V32EvidenceError("frozen V16 live evidence is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V16_EVIDENCE_OK" not in verifier:
        raise V32EvidenceError("frozen V16 checkpoint/marker contract is invalid")
    return {
        "artifact_entries": len(artifacts), "bundle_status": bundle["status"],
        "junit_passed": junit["passed"], "live_scan_count": bundle["live_scan"]["count"],
        "marker_contract_bound": True, "static_gates": len(static),
...
def _validate_frozen_v17_proof() -> dict[str, object]:
    ...
    if root != ((V17_HISTORY[V17_ARTIFACT_MANIFEST], V17_ARTIFACT_MANIFEST),):
        raise V32EvidenceError("frozen V17 root manifest edge is invalid")
    ...
    if len(artifacts) != 215 or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)) or not required.issubset(paths):
        raise V32EvidenceError("frozen V17 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V32EvidenceError("frozen V17 bundle contract/status is invalid")
    files = bundle.get("files")
    if not isinstance(files, dict) or set(files) != set(paths) - {V17_BUNDLE}:
        raise V32EvidenceError("frozen V17 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V17 bundle file hash mismatch: {relative}")
    junit = _junit_path(PROJECT / V17_JUNIT)
    if junit != {"errors": 0, "failed": 0, "passed": 1847, "skipped": 0} or bundle.get("counts") != junit:
        raise V32EvidenceError("frozen V17 JUnit evidence is invalid")
    static = _static_path(PROJECT / V17_STATIC)
    if bundle.get("static_gates") != static:
        raise V32EvidenceError("frozen V17 static evidence is invalid")
    ...
    if live_entries != (
        ...
        raise V32EvidenceError("frozen V17 live evidence is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V17_EVIDENCE_OK" not in verifier:
        raise V32EvidenceError("frozen V17 checkpoint/marker contract is invalid")
    return {
        "artifact_entries": len(artifacts), "bundle_status": bundle["status"],
        "junit_passed": junit["passed"], "live_scan_count": bundle["live_scan"]["count"],
        "marker_contract_bound": True, "static_gates": len(static),
...
def _validate_frozen_v18_proof() -> dict[str, object]:
    ...
    if root != ((V18_HISTORY[V18_ARTIFACT_MANIFEST], V18_ARTIFACT_MANIFEST),):
        raise V32EvidenceError("frozen V18 root manifest edge is invalid")
    ...
    if len(artifacts) != 234 or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)) or not required.issubset(paths):
        raise V32EvidenceError("frozen V18 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V32EvidenceError("frozen V18 bundle contract/status is invalid")
    files = bundle.get("files")
    if not isinstance(files, dict) or set(files) != set(paths) - {V18_BUNDLE}:
        raise V32EvidenceError("frozen V18 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V18 bundle file hash mismatch: {relative}")
    junit = _junit_path(PROJECT / V18_JUNIT)
    if junit != {"errors": 0, "failed": 0, "passed": 1875, "skipped": 0} or bundle.get("counts") != junit:
        raise V32EvidenceError("frozen V18 JUnit evidence is invalid")
    static = _static_path(PROJECT / V18_STATIC)
    if bundle.get("static_gates") != static:
        raise V32EvidenceError("frozen V18 static evidence is invalid")
    ...
    if live_entries != (
        ...
        raise V32EvidenceError("frozen V18 live evidence is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V18_EVIDENCE_OK" not in verifier:
        raise V32EvidenceError("frozen V18 checkpoint/marker contract is invalid")
    return {
        "artifact_entries": len(artifacts), "bundle_status": bundle["status"],
        "junit_passed": junit["passed"], "live_scan_count": bundle["live_scan"]["count"],
        "marker_contract_bound": True, "static_gates": len(static),
...
def _validate_frozen_v19_proof() -> dict[str, object]:
    ...
    if root != ((V19_HISTORY[V19_ARTIFACT_MANIFEST], V19_ARTIFACT_MANIFEST),):
        raise V32EvidenceError("frozen V19 root manifest edge is invalid")
    ...
    if len(artifacts) != 253 or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)) or not required.issubset(paths):
        raise V32EvidenceError("frozen V19 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if bundle.get("contract") != "Phase53CapabilityNexusEvidence.v19" or bundle.get("status") != "candidate-default-off-shadow-only-not-accepted":
        raise V32EvidenceError("frozen V19 bundle contract/status is invalid")
    files = bundle.get("files")
    if not isinstance(files, dict) or set(files) != set(paths) - {V19_BUNDLE}:
        raise V32EvidenceError("frozen V19 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V19 bundle file hash mismatch: {relative}")
    junit = _junit_path(PROJECT / V19_JUNIT)
    if junit != {"errors": 0, "failed": 0, "passed": 1893, "skipped": 0} or bundle.get("counts") != junit:
        raise V32EvidenceError("frozen V19 JUnit evidence is invalid")
    static = _static_path(PROJECT / V19_STATIC)
    if bundle.get("static_gates") != static:
        raise V32EvidenceError("frozen V19 static evidence is invalid")
    ...
    if live_entries != (
        ...
        raise V32EvidenceError("frozen V19 live evidence is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V19_EVIDENCE_OK" not in verifier:
        raise V32EvidenceError("frozen V19 checkpoint/marker contract is invalid")
    return {
        "artifact_entries": len(artifacts), "bundle_status": bundle["status"],
        "junit_passed": junit["passed"], "live_scan_count": bundle["live_scan"]["count"],
        "marker_contract_bound": True, "static_gates": len(static),
...
def _validate_frozen_v20_proof() -> dict[str, object]:
    ...
    if root != ((V20_HISTORY[V20_ARTIFACT_MANIFEST], V20_ARTIFACT_MANIFEST),):
        raise V32EvidenceError("frozen V20 root manifest edge is invalid")
    ...
    if len(artifacts) != 271 or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)) or not required.issubset(paths):
        raise V32EvidenceError("frozen V20 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if bundle.get("contract") != "Phase53CapabilityNexusEvidence.v20" or bundle.get("status") != "candidate-default-off-shadow-only-not-accepted":
        raise V32EvidenceError("frozen V20 bundle contract/status is invalid")
    files = bundle.get("files")
    if not isinstance(files, dict) or set(files) != set(paths) - {V20_BUNDLE}:
        raise V32EvidenceError("frozen V20 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V20 bundle file hash mismatch: {relative}")
    junit = _junit_path(PROJECT / V20_JUNIT)
    if junit != {"errors": 0, "failed": 0, "passed": 1903, "skipped": 0} or bundle.get("counts") != junit:
        raise V32EvidenceError("frozen V20 JUnit evidence is invalid")
    static = _static_path(PROJECT / V20_STATIC)
    if bundle.get("static_gates") != static:
        raise V32EvidenceError("frozen V20 static evidence is invalid")
    ...
    if live_entries != (
        ...
        raise V32EvidenceError("frozen V20 live evidence is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V20_EVIDENCE_OK" not in verifier:
        raise V32EvidenceError("frozen V20 checkpoint/marker contract is invalid")
    return {
        "artifact_entries": len(artifacts), "bundle_status": bundle["status"],
        "junit_passed": junit["passed"], "live_scan_count": bundle["live_scan"]["count"],
        "marker_contract_bound": True, "static_gates": len(static),
...
def _validate_frozen_v21_proof() -> dict[str, object]:
    ...
    if root != ((V21_HISTORY[V21_ARTIFACT_MANIFEST], V21_ARTIFACT_MANIFEST),):
        raise V32EvidenceError("frozen V21 root manifest edge is invalid")
    ...
    if len(artifacts) != 289 or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)) or not required.issubset(paths):
        raise V32EvidenceError("frozen V21 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if bundle.get("contract") != "Phase53CapabilityNexusEvidence.v21" or bundle.get("status") != "candidate-default-off-shadow-only-not-accepted":
        raise V32EvidenceError("frozen V21 bundle contract/status is invalid")
    files = bundle.get("files")
    if not isinstance(files, dict) or set(files) != set(paths) - {V21_BUNDLE}:
        raise V32EvidenceError("frozen V21 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V21 bundle file hash mismatch: {relative}")
    junit = _junit_path(PROJECT / V21_JUNIT)
    if junit != {"errors": 0, "failed": 0, "passed": 1916, "skipped": 0} or bundle.get("counts") != junit:
        raise V32EvidenceError("frozen V21 JUnit evidence is invalid")
    static = _static_path(PROJECT / V21_STATIC)
    if bundle.get("static_gates") != static:
        raise V32EvidenceError("frozen V21 static evidence is invalid")
    ...
    if live_entries != (
        ...
        raise V32EvidenceError("frozen V21 live evidence is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V21_EVIDENCE_OK" not in verifier:
        raise V32EvidenceError("frozen V21 checkpoint/marker contract is invalid")
    return {
        "artifact_entries": len(artifacts), "bundle_status": bundle["status"],
        "junit_passed": junit["passed"], "live_scan_count": bundle["live_scan"]["count"],
        "marker_contract_bound": True, "static_gates": len(static),
...
def _validate_frozen_v22_proof() -> dict[str, object]:
    ...
    if root != ((V22_HISTORY[V22_ARTIFACT_MANIFEST], V22_ARTIFACT_MANIFEST),):
        raise V32EvidenceError("frozen V22 root manifest edge is invalid")
    ...
    if len(artifacts) != 307 or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)) or not required.issubset(paths):
        raise V32EvidenceError("frozen V22 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if bundle.get("contract") != "Phase53CapabilityNexusEvidence.v22" or not isinstance(files, dict) or set(files) != set(paths) - {V22_BUNDLE}:
        raise V32EvidenceError("frozen V22 bundle is invalid")
    ...
    if junit != {"errors": 0, "failed": 0, "passed": 1927, "skipped": 0} or bundle.get("counts") != junit or bundle.get("static_gates") != static:
        raise V32EvidenceError("frozen V22 quantitative proof is invalid")
...
def _validate_frozen_v23_proof() -> dict[str, object]:
    ...
    if root != expected_root:
        raise V32EvidenceError("frozen V23 root manifest edge is invalid")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V23 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V32EvidenceError("frozen V23 bundle contract is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V23 bundle file hash mismatch: {relative}")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V23 quantitative proof is invalid")
    ...
    if live_entries != (
        ...
        raise V32EvidenceError("frozen V23 live proof is invalid")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V23 checkpoint/marker contract is invalid")
    return {
...
def _validate_frozen_v24_proof() -> dict[str, object]:
    ...
    if root != ((V24_HISTORY[V24_ARTIFACT_MANIFEST], V24_ARTIFACT_MANIFEST),):
        raise V32EvidenceError("frozen V24 root manifest edge is invalid")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V24 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V32EvidenceError("frozen V24 bundle contract is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V24 bundle file hash mismatch: {relative}")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V24 quantitative proof is invalid")
    ...
    if live_entries != (
        ...
        raise V32EvidenceError("frozen V24 live proof is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V24_EVIDENCE_OK" not in verifier:
        raise V32EvidenceError("frozen V24 checkpoint/marker contract is invalid")
    return {
...
def _validate_frozen_v25_proof() -> dict[str, object]:
    ...
    if root != ((V25_HISTORY[V25_ARTIFACT_MANIFEST], V25_ARTIFACT_MANIFEST),):
        raise V32EvidenceError("frozen V25 root manifest edge is invalid")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V25 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V32EvidenceError("frozen V25 bundle contract is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V25 bundle file hash mismatch: {relative}")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V25 quantitative proof is invalid")
    ...
    if live_entries != (
        ...
        raise V32EvidenceError("frozen V25 live proof is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V25_EVIDENCE_OK" not in verifier:
        raise V32EvidenceError("frozen V25 checkpoint/marker contract is invalid")
    return {
...
def _validate_frozen_v26_proof() -> dict[str, object]:
    ...
    if root != ((V26_HISTORY[V26_ARTIFACT_MANIFEST], V26_ARTIFACT_MANIFEST),):
        raise V32EvidenceError("frozen V26 root manifest edge is invalid")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V26 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V32EvidenceError("frozen V26 bundle contract is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V26 bundle file hash mismatch: {relative}")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V26 quantitative proof is invalid")
    ...
    if live_entries != (
        ...
        raise V32EvidenceError("frozen V26 live proof is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V26_EVIDENCE_OK" not in verifier:
        raise V32EvidenceError("frozen V26 checkpoint/marker contract is invalid")
    return {
...
def _validate_frozen_v27_proof() -> dict[str, object]:
    ...
    if root != ((V27_HISTORY[V27_ARTIFACT_MANIFEST], V27_ARTIFACT_MANIFEST),):
        raise V32EvidenceError("frozen V27 root manifest edge is invalid")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V27 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V32EvidenceError("frozen V27 bundle contract is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V27 bundle file hash mismatch: {relative}")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V27 quantitative proof is invalid")
    ...
    if live_entries != (
        ...
        raise V32EvidenceError("frozen V27 live proof is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V27_EVIDENCE_OK" not in verifier:
        raise V32EvidenceError("frozen V27 checkpoint/marker contract is invalid")
    return {
...
def _validate_frozen_v28_proof() -> dict[str, object]:
    ...
    if root != ((V28_HISTORY[V28_ARTIFACT_MANIFEST], V28_ARTIFACT_MANIFEST),):
        raise V32EvidenceError("frozen V28 root manifest edge is invalid")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V28 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V32EvidenceError("frozen V28 bundle contract is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V28 bundle file hash mismatch: {relative}")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V28 quantitative proof is invalid")
    ...
    if live_entries != (
        ...
        raise V32EvidenceError("frozen V28 live proof is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V28_EVIDENCE_OK" not in verifier:
        raise V32EvidenceError("frozen V28 checkpoint/marker contract is invalid")
    return {
...
def _validate_frozen_v29_proof() -> dict[str, object]:
    ...
    if root != ((V29_HISTORY[V29_ARTIFACT_MANIFEST], V29_ARTIFACT_MANIFEST),):
        raise V32EvidenceError("frozen V29 root manifest edge is invalid")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V29 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V32EvidenceError("frozen V29 bundle contract is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V29 bundle file hash mismatch: {relative}")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V29 quantitative proof is invalid")
    ...
    if live_entries != (
        ...
        raise V32EvidenceError("frozen V29 live proof is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V29_EVIDENCE_OK" not in verifier:
        raise V32EvidenceError("frozen V29 checkpoint/marker contract is invalid")
    return {
...
def _validate_frozen_v30_proof() -> dict[str, object]:
    ...
    if root != ((V30_HISTORY[V30_ARTIFACT_MANIFEST], V30_ARTIFACT_MANIFEST),):
        raise V32EvidenceError("frozen V30 root manifest edge is invalid")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V30 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V32EvidenceError("frozen V30 bundle contract is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V30 bundle file hash mismatch: {relative}")
    ...
    if (
        ...
        raise V32EvidenceError("frozen V30 quantitative proof is invalid")
    ...
    if live_entries != (
        ...
        raise V32EvidenceError("frozen V30 live proof is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V30_EVIDENCE_OK" not in verifier:
        raise V32EvidenceError("frozen V30 checkpoint/marker contract is invalid")
    return {
...
def _validate_frozen_v31_proof() -> dict[str, object]:
    root = _historical_manifest(V31_ROOT_MANIFEST)
    if root != ((V31_HISTORY[V31_ARTIFACT_MANIFEST], V31_ARTIFACT_MANIFEST),):
        raise V32EvidenceError("frozen V31 root manifest edge is invalid")
    artifacts = _historical_manifest(V31_ARTIFACT_MANIFEST)
    paths = tuple(path for _, path in artifacts)
    required = {
        "core/capability_nexus_v31.py", "tests/test_capability_nexus_v31.py",
        V31_VERIFIER, "scripts/verify_phase5_capability_nexus_v31_worker.py",
        "scripts/phase5_capability_nexus_v31_subtest_reporter.py",
        "scripts/check_phase5_capability_nexus_v31_whitespace.py",
        V31_BUNDLE, V31_JUNIT, V31_STATIC, V31_LIVE, V30_ROOT_MANIFEST,
        f"{V31_EVIDENCE}/phase5-capability-nexus-v31.mutable-projections.json",
    }
    if (
        len(artifacts) != 475
        or paths != tuple(sorted(paths))
        ...
        or not required.issubset(paths)
        or set(MUTABLE_PROJECTIONS) & set(paths)
        or V31_ARTIFACT_MANIFEST in paths
        or V31_ROOT_MANIFEST in paths
    ):
        raise V32EvidenceError("frozen V31 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    bundle = _canonical_json(V31_BUNDLE)
    files = bundle.get("files")
    if (
        bundle.get("contract") != "Phase53CapabilityNexusEvidence.v31"
        or bundle.get("status") != "candidate-default-off-shadow-only-not-accepted"
        or bundle.get("external_e6")
        != {"accepted": False, "required_before_acceptance": True, "status": "pending-external-review"}
        or bundle.get("feature_gate")
        != {"dispatch_enabled": False, "nexus_enabled": False, "shadow_mode": True}
        or not isinstance(files, dict)
        or set(files) != set(paths) - {V31_BUNDLE}
    ):
        raise V32EvidenceError("frozen V31 bundle contract is invalid")
    for relative, digest in files.items():
        if digest != _historical_sha(relative):
            raise V32EvidenceError(f"frozen V31 bundle file hash mismatch: {relative}")
    junit = _junit_path(PROJECT / V31_JUNIT)
    static = _static_path(PROJECT / V31_STATIC)
    if (
        junit != {"errors": 0, "failed": 0, "passed": 2006, "skipped": 0}
        or bundle.get("counts") != junit
        or bundle.get("static_gates") != static
    ):
        raise V32EvidenceError("frozen V31 quantitative proof is invalid")
    live_entries = _historical_manifest(V31_LIVE)
    if live_entries != (
        (V30_HISTORY["core/capability_nexus_v30.py"], "core/capability_nexus_v30.py"),
        ("b1aae06677985f5ae00cb028bcade0e7de26bba6615eb7bca734cf23783133c8", V30_LIVE),
    ) or bundle.get("live_scan") != {"count": 110, "manifest_sha256": _sha(PROJECT / V31_LIVE)}:
        raise V32EvidenceError("frozen V31 live proof is invalid")
    checkpoint = _read_text(V31_CHECKPOINT)
    verifier = _read_text(V31_VERIFIER)
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V31_EVIDENCE_OK" not in verifier:
        raise V32EvidenceError("frozen V31 checkpoint/marker contract is invalid")
    return {
        "artifact_entries": 475, "bundle_status": bundle["status"],
        "junit_passed": 2006, "live_scan_count": 110,
        "marker_contract_bound": True, "static_gates": 8,
    }
...
def _history_and_tamper(bundle: dict[str, object]) -> tuple[dict[str, object], dict[str, object]]:
    expected = {
        ...
        "phase53_v31": {"decision": "rejected", "hashes": V31_HISTORY},
    }
    if bundle.get("history") != expected:
        raise V32EvidenceError("V1/V2/V3 history is not exact or not rejected")
    for version, history in (
        ...
        for relative, digest in history.items():
            if _historical_sha(relative) != digest:
                raise V32EvidenceError(f"{version} historical byte drift: {relative}")
    ...
    v31_proof = _validate_frozen_v31_proof()
    ...
    if not set(HISTORICAL_ROOTS).issubset(historical_manifests):
        raise V32EvidenceError("frozen V1-V31 root closure is incomplete")
    targets = [
    ...
    with tempfile.TemporaryDirectory(prefix="onyx-p53-history-tamper-") as directory:
        ...
        for relative in targets:
            ...
            if expected_digest is None:
                raise V32EvidenceError(f"historical tamper leaf is unbound: {relative}")
            copied = base / relative
            copied.parent.mkdir(parents=True, exist_ok=True)
            copied.write_bytes(_read_bytes(relative))
            if _sha(copied) != expected_digest:
                raise V32EvidenceError(f"historical pristine copy mismatch: {relative}")
            copied.write_bytes(copied.read_bytes() + b"\n# disposable tamper\n")
            if _sha(copied) == expected_digest:
                raise V32EvidenceError(f"historical tamper fixture was not rejected: {relative}")
    history_metrics = {
        "frozen_candidates_cryptographically_validated": 31,
...
def _verify_manifest_tree(
    ...
    if depth > 32 or len(verified) >= 256 or len(leaves) > 4096:
        raise V32EvidenceError("historical manifest traversal budget exceeded")
    if relative in active:
        raise V32EvidenceError(f"historical manifest cycle detected: {relative}")
    if relative in verified:
        return
    active.add(relative)
    entries = _historical_manifest(relative)
    if len(entries) > 4096:
        raise V32EvidenceError("historical manifest leaf budget exceeded")
...
def _record_historical_leaf(leaves: dict[str, str], relative: str, digest: str) -> None:
    ...
    if prior is None:
        ...
        raise V32EvidenceError(f"historical recursive digest conflict: {relative}")
    if len(leaves) > 4096:
        raise V32EvidenceError("historical manifest leaf budget exceeded")
...
def _live(bundle: dict[str, object]) -> dict[str, object]:
    ...
    expanded = _manifest(V1_LIVE) + (
        ...
        (V29_HISTORY["core/capability_nexus_v29.py"], "core/capability_nexus_v29.py"),
        (V30_HISTORY["core/capability_nexus_v30.py"], "core/capability_nexus_v30.py"),
        (V31_HISTORY["core/capability_nexus_v31.py"], "core/capability_nexus_v31.py"),
    )
    if bundle.get("live_scan") != {"count": len(expanded), "manifest_sha256": _sha(PROJECT / LIVE)}:
        raise V32EvidenceError("live scan summary drift")
    for digest, relative in expanded:
        path = PROJECT / relative
        if _sha(path) != digest or "capability_nexus_v32" in _read_text(relative):
            raise V32EvidenceError(f"live boundary drift/wiring: {relative}")
    return {
...
def _assignment(path: Path, name: str) -> object:
    ...
    raise V32EvidenceError(f"missing assignment: {name}")
...
def _semantics(bundle: dict[str, object]) -> None:
    ...
    for relative in (CORE, TESTS, SELF, WORKER, REPORTER, WHITESPACE, CHECKPOINT, METRICS, BUNDLE):
        ...
        if any(term in source for term in forbidden_overclaims):
            raise V32EvidenceError(f"same-process runtime overclaim is forbidden: {relative}")
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import) and {item.name.split(".", 1)[0] for item in node.names} & forbidden:
            raise V32EvidenceError("candidate imports a forbidden runtime/provider surface")
        if isinstance(node, ast.ImportFrom) and (node.module or "").split(".", 1)[0] in forbidden:
            raise V32EvidenceError("candidate imports a forbidden runtime/provider surface")
    sys.path.insert(0, str(PROJECT))
    try:
        from core.capability_nexus_v32 import (  # noqa: PLC0415
            CapabilityNexusV32,
            NexusFeatureGateV32,
            _METADATA_ENUMS,
            _safe_metadata,
        ...
        sys.path.pop(0)
    nexus = CapabilityNexusV32(NexusFeatureGateV32())
    if hasattr(nexus, "dispatch") or hasattr(nexus, "execute"):
        raise V32EvidenceError("registry exposes dispatch")
    ...
    if len(built.descriptors) != 26 or dict(built.policy_mapping()) != policies:
        raise V32EvidenceError("legacy parity failed")
    ...
    if dict(_METADATA_ENUMS) != expected_metadata_enums:
        raise V32EvidenceError("closed metadata provenance contract drift")
    for key, value in (
        ...
        try:
            ...
            raise V32EvidenceError("closed metadata provenance contract accepted an unknown value")
    ...
    if called & {"_secret_canary", "_raw_credential_canary", "_validate_content_decoding_closure"}:
        raise V32EvidenceError("typed metadata still uses free-text heuristic inference")
    ...
    expected = {
        ...
        "recursive_v1_to_v31_history_and_component_reparse_rejection",
        ...
        "path_gate_is_stable_state_metadata_precheck_only",
        "path_gate_is_not_atomic_concurrent_writer_toctou_boundary",
        "deterministic_toctou_limitation_reproduction_bound",
    }
    if not isinstance(claims, dict) or set(claims) != expected or not all(claims.values()):
        raise V32EvidenceError("semantic claim set is invalid")
    test_source = _read_text(TESTS)
    race_proof_tokens = {
    ...
    if any(token not in test_source for token in race_proof_tokens):
        raise V32EvidenceError("deterministic TOCTOU limitation proof is missing")
...
def _environment(bundle: dict[str, object]) -> None:
    ...
    if not isinstance(recorded, dict) or any(recorded.get(key) != value for key, value in expected.items()):
        raise V32EvidenceError("environment drift")
    commit = bundle.get("base_commit")
    if not isinstance(commit, str) or re.fullmatch(r"[0-9a-f]{40}", commit) is None:
        raise V32EvidenceError("base commit evidence is invalid")
...
def verify(fresh_focused_junit: Path) -> dict[str, object]:
    ...
    if focused["errors"] or focused["failed"] or focused["skipped"] or bundle.get("counts") != focused:
        raise V32EvidenceError("JUnit counts drift")
    if not fresh_focused_junit.is_file() or _junit_path(fresh_focused_junit) != focused:
        raise V32EvidenceError("direct focused pytest evidence does not match frozen JUnit")
    static = _static()
    if bundle.get("static_gates") != static:
        raise V32EvidenceError("static evidence drift")
    ...
    if bundle.get("historical_recursive_evidence") != history:
        raise V32EvidenceError("bundle recursive history evidence drift")
    if bundle.get("regression_evidence") != metrics["regressions"]:
        raise V32EvidenceError("bundle regression evidence drift")
    if bundle.get("artifact_entries") != metrics["artifact"]:
        raise V32EvidenceError("bundle artifact entry evidence drift")
    if bundle.get("structured_evidence") != {
        ...
        raise V32EvidenceError("bundle structured evidence binding drift")
    _checkpoint()
    _semantics(bundle)
    _environment(bundle)
...
def _validated_fresh_focused_junit(path: Path) -> Path:
    if path.name != "focused.junit.xml" or re.fullmatch(r"[0-9a-f]{64}", path.parent.name) is None:
        raise V32EvidenceError("fresh focused JUnit path is not nonce-scoped")
    try:
        base = _PRIVATE_BASE.resolve(strict=True)
        directory = path.parent.resolve(strict=True)
        resolved = path.resolve(strict=True)
    except OSError as exc:
        raise V32EvidenceError("fresh focused JUnit private realpath is invalid") from exc
    ...
    if base != expected_base or directory.parent != base or resolved.parent != directory:
        raise V32EvidenceError("fresh focused JUnit escapes its fixed private base")
    for component in (_PRIVATE_BASE, path.parent, path.parent / _PRIVATE_SENTINEL, path):
        ...
        if stat.S_ISLNK(info.st_mode) or getattr(info, "st_file_attributes", 0) & getattr(stat, "FILE_ATTRIBUTE_REPARSE_POINT", 0x400):
            raise V32EvidenceError("fresh focused JUnit path contains a reparse component")
    sentinel = path.parent / _PRIVATE_SENTINEL
    if sentinel.read_text(encoding="ascii") != f"{_PRIVATE_CONTRACT}:{path.parent.name}\n":
        raise V32EvidenceError("fresh focused JUnit sentinel is invalid")
    return resolved


def main() -> int:
    if len(sys.argv) != 3 or sys.argv[1] != "--fresh-focused-junit":
        raise V32EvidenceError("pure worker requires exactly one fresh focused JUnit path")
    fresh = _validated_fresh_focused_junit(Path(sys.argv[2]))
    print("P53_CAPABILITY_NEXUS_V32_EVIDENCE_OK " + json.dumps(verify(fresh), sort_keys=True))
    return 0
