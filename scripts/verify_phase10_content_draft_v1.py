"""Reproduce the Phase 10 Content Draft V1 candidate."""
...
from core import phase10_content_draft_v1 as candidate  # noqa: E402
from scripts import (  # noqa: E402
    verify_phase10_provider_connector_v1_acceptance as provider,
)

BASE = PROJECT / "docs/onyx/checkpoints/phase10-content-draft-v1"
MANIFEST = BASE / "manifest.json"
SELECTION = BASE / "cumulative-selection.json"
MARKER = "P10_CONTENT_DRAFT_V1_OK"
ACCEPTED_PROVIDER_ROOT = (
    "7aba66ad246c75b1660c70c00dbf012526de72e1592892bee0fd1ea363a35711"
)
ARTIFACT_PATHS = (
    "core/phase10_content_draft_v1.py",
    "tests/test_phase10_content_draft_v1.py",
    "scripts/verify_phase10_content_draft_v1.py",
    "docs/onyx/adrs/ADR-0052-phase10-content-draft-v1.md",
    "docs/onyx/research/PHASE10_CONTENT_DRAFT_V1_SOURCES.md",
    "docs/onyx/checkpoints/phase10-content-draft-v1/"
    "PHASE10_CONTENT_DRAFT_V1_CHECKPOINT.md",
    "docs/onyx/checkpoints/phase10-content-draft-v1/CAPABILITY_DELTA_SNAPSHOT.md",
    "docs/onyx/checkpoints/phase10-content-draft-v1/cumulative-selection.json",
)
...
def verify(*, run_tests: bool = True) -> dict[str, object]:
    ...
    if (
        manifest.get("schema") != "onyx.phase10.content-draft.v1"
        or manifest.get("candidate") != "phase10-content-draft-v1"
    ...
    provider_record = provider.verify(run_tests=False)
    if (
        provider_record.get("decision") != "accepted"
        or provider_record.get("candidate_artifact_root_sha256")
        != ACCEPTED_PROVIDER_ROOT
    ):
        raise VerificationError("accepted provider-connector entry drift")

    if candidate.ContentDraftFeatureGateV1.from_environ({}).enabled is not False:
        raise VerificationError("feature gate is not default-off")
    candidate._verify_entry(PROJECT)
    try:
        candidate.create_content_draft_set_v1(PROJECT / "missing")
    except candidate.ContentDraftV1Denied:
        pass
    else:
    ...
    source = (PROJECT / "core/phase10_content_draft_v1.py").read_text(encoding="utf-8")
    for required in (
        'FEATURE_FLAG: Final = "ONYX_PHASE10_CONTENT_DRAFT_V1"',
        "class ContentDraftSetV1",
        "RIGHTS_REQUIRED_SOURCES",
        "def is_ready_for_calendar",
    ...
    if run_tests:
        temporary = (
            PROJECT
            / "runtime/test-tmp/phase10-content-draft-v1-verify"
    ...
    return {
        "candidate": "phase10-content-draft-v1",
