...
from scripts import verify_phase5_approval_inbox_v14_worker as process_guard  # noqa: E402

CHECKPOINT = ROOT / "docs/onyx/checkpoints/phase5-approval-inbox-v14"
...
def main() -> int:
    ...
    focused_junit = CHECKPOINT / "phase5-approval-inbox-v14.junit.xml"
    combined_junit = CHECKPOINT / "phase5-approval-inbox-v14.combined.junit.xml"
    regressions_junit = CHECKPOINT / "phase5-approval-inbox-v14.regressions.junit.xml"
    ...
    write(CHECKPOINT / "phase5-approval-inbox-v14.raw.log", focused_output)
    write(CHECKPOINT / "phase5-approval-inbox-v14.combined.log", combined_output)
    write(CHECKPOINT / "phase5-approval-inbox-v14.regressions.log", regression_output)
    ...
    write(
        CHECKPOINT / "phase5-approval-inbox-v14.static.log",
        "\n\n".join(static_lines) + "\n",
    )
    ...
    write(
        CHECKPOINT / "phase5-approval-inbox-v14.live-scan.sha256",
    ...
    checkpoint_text = f"""# Phase 5.2 Approval Inbox V14 checkpoint

Status: **candidate — default-off, read-only, external acceptance pending**

V14 is a clean-room review projection. It preserves V1-V12 as rejected historical
candidates and does not import their core implementation, Session Grants or
Capability Nexus. The
host owns a bootstrap-only `ONYX_APPROVAL_INBOX_V14` gate, flag epoch, exact
source and monotonic clock. No live/startup/UI/dashboard path imports V14.
...
- Combined Approval Inbox V1-V14: {combined["passed"]} passed in {combined_seconds:.3f}s.
- Frozen V2-V7 R11 fixtures resolve exactly two mutable documentation paths
  directly from exact V9 content-addressed snapshots bound as V14 artifacts.
  ...
  direct V14 authorities with fixed digests.
...
- Live reachability scan: {len(live_entries)} files, no V14 flag/module reference.
...
- The digest-bound ZIP closes pytest, its protected import namespaces and direct
  runtime dependencies. Conventional package directories remain available
  after the ZIP only for unrelated application dependencies used by regression
  tests; protected pytest namespaces are required to resolve from the ZIP and
  are revalidated after the run.
- Internal Git compatibility implements only the frozen V13/V14 HEAD identity,
...
V14 remains unaccepted and default-off until three independent external reviews
pass. This checkpoint grants no Phase 5 exit or live activation.
"""
    checkpoint_md = CHECKPOINT / "PHASE5_2_APPROVAL_INBOX_V14_CHECKPOINT.md"
    ...
    v14_files = [
        ...
        "docs/onyx/checkpoints/phase5-approval-inbox-v14/phase5-approval-inbox-v14.raw.log",
        "docs/onyx/checkpoints/phase5-approval-inbox-v14/phase5-approval-inbox-v14.combined.log",
        "docs/onyx/checkpoints/phase5-approval-inbox-v14/phase5-approval-inbox-v14.regressions.log",
        "docs/onyx/checkpoints/phase5-approval-inbox-v14/phase5-approval-inbox-v14.static.log",
        "docs/onyx/checkpoints/phase5-approval-inbox-v14/phase5-approval-inbox-v14.live-scan.sha256",
        "docs/onyx/checkpoints/phase5-approval-inbox-v14/mutable-projections.json",
        PRIVATE_RUNTIME.relative_to(ROOT).as_posix(),
        *(entry["snapshot"] for entry in projections),
        *HISTORICAL_PROJECTIONS,
        *HISTORICAL_ROOT_AUTHORITIES,
    ]
    files = sorted(set(historical + v14_files))
    ...
    bundle = {
        "contract": "Phase52ApprovalInboxEvidence.v14",
        "status": "candidate-default-off-external-acceptance-pending",
        "base_commit": head,
        "feature_flag": {
            "name": "ONYX_APPROVAL_INBOX_V14",
            "default": False,
            "bootstrap_only": True,
        },
        "counts": {
    ...
    bundle_path = CHECKPOINT / "phase5-approval-inbox-v14.bundle.json"
