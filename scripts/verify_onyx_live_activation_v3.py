"""Frozen bundle verifier for Onyx Live Activation V3."""
...
def fail(message: str) -> None:
    raise SystemExit("ONYX_LIVE_ACTIVATION_V3_VERIFY_FAIL: " + message)
...
def main() -> None:
    ...
    if type(payload) is not dict or payload.get("schema") != "onyx.live-activation.v3":
    ...
    if frozenset(indexed) != V1 | V2 | V3 | HOST | COMPONENTS | E6:
    ...
    core = (ROOT / "core" / "onyx_live_activation_v3.py").read_text(
        encoding="utf-8"
    )
    launcher = (ROOT / "scripts" / "launch_onyx_live_v3.pyw").read_text(
        encoding="utf-8"
    )
    host_gate = (
        ROOT / "scripts" / "verify_onyx_live_activation_v3_host.py"
    ).read_text(encoding="utf-8")
    qt_gate = (
        ROOT / "scripts" / "verify_onyx_live_activation_v3_qt.py"
    ).read_text(encoding="utf-8")
    if "onyx_live_activation_v1" in core or "onyx_live_activation_v2" in core:
        fail("V3 reaches rejected candidate")
    ...
    refusal = launcher.find("ONYX_LIVE_V3_PREIMPORT_REFUSAL")
    ...
    if "range(1, 11)" not in host_gate or "range(1, 65)" not in host_gate:
    ...
    tree = ast.parse(core, filename="core/onyx_live_activation_v3.py")
    install = next(
        node
    ...
    print("ONYX_LIVE_ACTIVATION_V3_OK")
