...
from scripts import (  # noqa: E402
    phase5_approval_inbox_v12_historical_projection as historical_guard,
)
from scripts import verify_phase5_approval_inbox_v12_worker as process_guard  # noqa: E402

CHECKPOINT = ROOT / "docs/onyx/checkpoints/phase5-approval-inbox-v12"
...
def run(command: list[str]) -> tuple[str, float]:
    if command and command[0] == "git":
        historical_guard._validated_repository_git_directory()
...
def main() -> int:
    ...
    focused_junit = CHECKPOINT / "phase5-approval-inbox-v12.junit.xml"
    combined_junit = CHECKPOINT / "phase5-approval-inbox-v12.combined.junit.xml"
    regressions_junit = CHECKPOINT / "phase5-approval-inbox-v12.regressions.junit.xml"
    ...
    write(CHECKPOINT / "phase5-approval-inbox-v12.raw.log", focused_output)
    write(CHECKPOINT / "phase5-approval-inbox-v12.combined.log", combined_output)
    write(CHECKPOINT / "phase5-approval-inbox-v12.regressions.log", regression_output)
    ...
    write(
        CHECKPOINT / "phase5-approval-inbox-v12.static.log",
        "\n\n".join(static_lines) + "\n",
    )
    ...
    write(
        CHECKPOINT / "phase5-approval-inbox-v12.live-scan.sha256",
    ...
    checkpoint_text = f"""# Phase 5.2 Approval Inbox V12 checkpoint
...
- Combined Approval Inbox V1-V12: {combined["passed"]} passed in {combined_seconds:.3f}s.
...
- Live reachability scan: {len(live_entries)} files, no V12 flag/module reference.
...
"""
    checkpoint_md = CHECKPOINT / "PHASE5_2_APPROVAL_INBOX_V12_CHECKPOINT.md"
    ...
    v12_files = [
        ...
        "docs/onyx/checkpoints/phase5-approval-inbox-v12/phase5-approval-inbox-v12.raw.log",
        "docs/onyx/checkpoints/phase5-approval-inbox-v12/phase5-approval-inbox-v12.combined.log",
        "docs/onyx/checkpoints/phase5-approval-inbox-v12/phase5-approval-inbox-v12.regressions.log",
        "docs/onyx/checkpoints/phase5-approval-inbox-v12/phase5-approval-inbox-v12.static.log",
        "docs/onyx/checkpoints/phase5-approval-inbox-v12/phase5-approval-inbox-v12.live-scan.sha256",
        "docs/onyx/checkpoints/phase5-approval-inbox-v12/mutable-projections.json",
        *(entry["snapshot"] for entry in projections),
        *HISTORICAL_PROJECTIONS,
        *HISTORICAL_ROOT_AUTHORITIES,
    ]
    files = sorted(set(historical + v12_files))
    ...
    bundle = {
        ...
        "claims": {
            ...
            "repository_git_directory_guarded": True,
            "combined_process_sources_identity_bound": True,
            "child_import_sources_regated": True,
    ...
    bundle_path = CHECKPOINT / "phase5-approval-inbox-v12.bundle.json"
