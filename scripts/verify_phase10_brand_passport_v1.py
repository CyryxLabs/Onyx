"""Reproduce the Phase 10 Brand Passport V1 candidate."""
...
from core import phase10_brand_passport_v1 as candidate  # noqa: E402
from scripts import (  # noqa: E402
    verify_phase9_exit_candidate_v1_acceptance as phase9_exit,
)

BASE = PROJECT / "docs/onyx/checkpoints/phase10-brand-passport-v1"
MANIFEST = BASE / "manifest.json"
SELECTION = BASE / "cumulative-selection.json"
MARKER = "P10_BRAND_PASSPORT_V1_OK"
ACCEPTED_PHASE9_EXIT_ROOT = (
    "90d1947576ebaf430cd179b1624c74974789a1426552afb42eab53b180f81fc6"
)
ARTIFACT_PATHS = (
    "core/phase10_brand_passport_v1.py",
    "tests/test_phase10_brand_passport_v1.py",
    "scripts/verify_phase10_brand_passport_v1.py",
    "docs/onyx/adrs/ADR-0049-phase10-brand-passport-v1.md",
    "docs/onyx/research/PHASE10_BRAND_PASSPORT_V1_SOURCES.md",
    "docs/onyx/checkpoints/phase10-brand-passport-v1/"
    "PHASE10_BRAND_PASSPORT_V1_CHECKPOINT.md",
    "docs/onyx/checkpoints/phase10-brand-passport-v1/CAPABILITY_DELTA_SNAPSHOT.md",
    "docs/onyx/checkpoints/phase10-brand-passport-v1/cumulative-selection.json",
)
...
def verify(*, run_tests: bool = True) -> dict[str, object]:
    ...
    if (
        manifest.get("schema") != "onyx.phase10.brand-passport.v1"
        or manifest.get("candidate") != "phase10-brand-passport-v1"
    ...
    # Default-off + entry-bound: the gate is off with no env, and construction is
    # denied unless the accepted Phase 9 exit evidence is byte-exact on disk.
    if candidate.BrandPassportFeatureGateV1.from_environ({}).enabled is not False:
        raise VerificationError("feature gate is not default-off")
    candidate._verify_entry(PROJECT)
    try:
        candidate.create_brand_registry_v1(PROJECT / "missing")
    except candidate.BrandPassportV1Denied:
        pass
    else:
    ...
    source = (PROJECT / "core/phase10_brand_passport_v1.py").read_text(
        encoding="utf-8"
    )
    for required in (
    ...
    # Publishing must be structurally absent: can_publish returns a literal False.
    ...
    if run_tests:
        temporary = (
            PROJECT
            / "runtime/test-tmp/phase10-brand-passport-v1-verify"
    ...
    return {
        "candidate": "phase10-brand-passport-v1",
