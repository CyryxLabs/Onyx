"""Run the phase-only, fail-closed Capability Nexus V31 verifier."""
...
PRIVATE_BASE = Path(tempfile.gettempdir()) / "onyx-p53-v31-private"
SENTINEL = ".phase53-v31-private"
TOTAL_TIMEOUT_SECONDS = 180
POST_KILL_DRAIN_SECONDS = 2
MARKER = "P53_CAPABILITY_NEXUS_V31_EVIDENCE_OK"
...
class V31ParentError(RuntimeError):
    ...
    __slots__ = (
        "_v31_attachment_failures",
        "_v31_cleanup_failures",
        "_v31_cleanup_overflow",
        "_v31_primary_failure",
    )
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            raise TypeError("V31 cleanup snapshot is not canonical")
        object.__setattr__(
            self,
            "_v31_cleanup_failures",
            None
    ...
    def _trusted_cleanup_payload(self) -> tuple[dict[str, str], ...] | None:
        records = object.__getattribute__(self, "_v31_cleanup_failures")
        if records is None:
            return None
        return tuple(
            {
    ...
    def cleanup_failures(self):
        value = object.__getattribute__(self, "_v31_cleanup_failures")
        if value is None:
    ...
    def cleanup_overflow(self):
        value = object.__getattribute__(self, "_v31_cleanup_overflow")
        if value is None:
    ...
    def primary_failure(self):
        return object.__getattribute__(self, "_v31_primary_failure")

    @property
    def attachment_failures(self):
        return object.__getattribute__(self, "_v31_attachment_failures")

    def __setattr__(self, name: str, value: object) -> None:
        if type(name) is not str:
            raise TypeError("V31 attribute name must be exact built-in str")
        raise AttributeError("V31 parent error is API read-only after construction")
...
def _kernel32():
    if os.name != "nt":
        raise V31ParentError("Windows Job APIs requested on a non-Windows host")
    if ctypes.sizeof(wintypes.HANDLE) != ctypes.sizeof(ctypes.c_void_p):
        raise V31ParentError("Windows HANDLE width does not match pointer width")
...
def _validated_handle(raw: object) -> wintypes.HANDLE:
    ...
    if (
        ...
        raise V31ParentError("Windows handle is invalid or truncated")
...
def _handle_value(handle: wintypes.HANDLE) -> int:
    ...
    if not isinstance(value, int) or value <= 0:
        raise V31ParentError("Windows handle ownership record is invalid")
    return value


def _win_failure(operation: str) -> V31ParentError:
    return V31ParentError(
...
class _WindowsJob:
    def __init__(self):
        ...
        if not self.api.SetInformationJobObject(
            ...
            try:
                self.close()
            except V31ParentError as close_failure:
                raise V31ParentError(
...
def _wait_job_empty(job: _WindowsJob, label: str) -> None:
    ...
    while job.active() and time.monotonic() < deadline:
        time.sleep(0.01)
    if job.active():
        raise V31ParentError(f"{label} process tree survived termination")
...
def _assert_fixed_phase_target(relative: str) -> None:
    ...
    if (
        ...
        raise V31ParentError("fixed phase target path is not canonical")
    reparse_flag = getattr(stat, "FILE_ATTRIBUTE_REPARSE_POINT", 0x400)
    try:
        root_info = PROJECT.lstat()
        root_resolved = PROJECT.resolve(strict=True)
    except OSError as exc:
        raise V31ParentError("fixed phase workspace root is missing") from exc
    if (
        stat.S_ISLNK(root_info.st_mode)
        or getattr(root_info, "st_file_attributes", 0) & reparse_flag
        or str(PROJECT.absolute()) != str(root_resolved)
    ):
        raise V31ParentError("fixed phase workspace root is aliased or reparsed")
    current = PROJECT
    for component in pure.parts:
        ...
        try:
            ...
            raise V31ParentError("fixed phase target is missing") from exc
        if stat.S_ISLNK(info.st_mode) or getattr(info, "st_file_attributes", 0) & reparse_flag:
            raise V31ParentError("fixed phase target contains a reparse component")
        if resolved.name != component:
            raise V31ParentError("fixed phase target uses a case or name alias")
    try:
        current.resolve(strict=True).relative_to(root_resolved)
    except (OSError, ValueError) as exc:
        raise V31ParentError("fixed phase target escapes workspace") from exc


def _phase_command(phase: str, junit: Path) -> list[str]:
    if not isinstance(phase, str):
        raise V31ParentError("phase id must be focused or worker")
    if phase == "focused":
...
def _bounded_output(path: Path, label: str) -> str:
    ...
    if size > _OUTPUT_LIMIT_BYTES:
        raise V31ParentError(f"{label} output exceeds bounded evidence limit")
    ...
    *,
    ...
    try:
        ...
        if wait == _WAIT_TIMEOUT:
            raise V31ParentError(f"{label} timed out after {timeout:.3f}s")
    ...
    if not resumed or result is None:
        raise V31ParentError(f"{label} suspended process was not safely completed")
    return result
...
def _wait_posix_group_empty(pgid: int, label: str) -> None:
    ...
    if _posix_group_exists(pgid):
        raise V31ParentError(f"{label} POSIX process group survived cleanup")
...
class _CleanupFailures:
    ...
    def _read_accessor(
        ...
        *,
        ...
        if type(failure) is V31ParentError and field == "cleanup_failures":
            try:
                trusted = V31ParentError._trusted_cleanup_payload(failure)
...
def _direct_child_cleanup(
    ...
    try:
        if process.poll() is None:
            failures.add(
                "direct.poll.reap",
                V31ParentError(f"{label} direct child was not reaped"),
            )
    except BaseException as exc:
    ...
    try:
        if _pid_exists(process.pid):
            failures.add(
                "direct.pid.proof",
                V31ParentError(f"{label} direct child PID survived cleanup"),
            )
    except BaseException as exc:
    ...
    try:
        ...
        if failure is None:
            try:
                ...
                failure = V31ParentError(f"{label} timed out after {timeout:.3f}s")
...
def _validated_junit_path(evidence_nonce: str, *, require_file: bool) -> Path:
    ...
    if (
        ...
        raise V31ParentError("private evidence sentinel is invalid")
    ...
    if require_file:
        ...
        raise V31ParentError("fresh focused JUnit already exists")
    return junit
...
def _run_phase(
    ...
    *,
    ...
    if (
        ...
        raise V31ParentError(f"{label} timeout is invalid")
    if stdout_mode not in {"pipe", "devnull"}:
        raise V31ParentError(f"{label} stdout mode is invalid")
...
def _remaining(started: float, total_timeout: int) -> float:
    if type(total_timeout) is not int or total_timeout <= POST_KILL_DRAIN_SECONDS + 1:
        raise V31ParentError("total timeout is invalid")
    remaining = total_timeout - (time.monotonic() - started)
    if remaining <= POST_KILL_DRAIN_SECONDS + 1:
        raise V31ParentError(f"total verifier deadline exceeded after {total_timeout}s")
    return remaining
...
def _parse_worker_marker(stdout: str) -> dict[str, object]:
    ...
    try:
        ...
        raise V31ParentError("pure worker marker payload is invalid") from exc
    if not isinstance(payload, dict) or set(payload) != {
        ...
        raise V31ParentError("pure worker marker payload schema is invalid")
    return payload
...
def _run_parent(*, total_timeout: int = TOTAL_TIMEOUT_SECONDS) -> dict[str, object]:
    ...
    with _PrivateEvidence() as evidence_nonce:
        ...
        if focused.returncode:
            raise V31ParentError("fixed focused pytest failed")
        worker = _run_phase(
            "worker",
            evidence_nonce=evidence_nonce,
        ...
        if worker.returncode:
            raise V31ParentError("fixed pure artifact worker failed")
