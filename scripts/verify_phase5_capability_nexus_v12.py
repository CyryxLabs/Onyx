"""Verify the frozen Phase 5.3 Capability Nexus V12 candidate."""
...
ARTIFACT_MANIFEST = "docs/onyx/VE-ARTIFACTS-P53-CAPABILITY-NEXUS-V12-001.sha256"
ROOT_MANIFEST = "docs/onyx/VE-SOURCE-P53-CAPABILITY-NEXUS-V12-001.sha256"
EVIDENCE = "docs/onyx/checkpoints/phase5-capability-nexus-v12"
BUNDLE = f"{EVIDENCE}/phase5-capability-nexus-v12.bundle.json"
JUNIT = f"{EVIDENCE}/phase5-capability-nexus-v12.junit.xml"
RAW = f"{EVIDENCE}/phase5-capability-nexus-v12.raw.log"
STATIC = f"{EVIDENCE}/phase5-capability-nexus-v12.static.log"
LIVE = f"{EVIDENCE}/phase5-capability-nexus-v12.live-scan.sha256"
...
CHECKPOINT = f"{EVIDENCE}/PHASE5_3_CAPABILITY_NEXUS_V12_CHECKPOINT.md"
CORE = "core/capability_nexus_v12.py"
TESTS = "tests/test_capability_nexus_v12.py"
WHITESPACE = "scripts/check_phase5_capability_nexus_v12_whitespace.py"
SELF = "scripts/verify_phase5_capability_nexus_v12.py"
V1_HISTORY = {
...
class V12EvidenceError(RuntimeError):
    pass
...
def _manifest(relative: str) -> tuple[tuple[str, str], ...]:
    ...
    if b"\r" in raw or not raw.endswith(b"\n"):
        raise V12EvidenceError(f"manifest line endings are invalid: {relative}")
    entries = []
    for line in raw.decode().splitlines():
        match = _LINE.fullmatch(line)
        if match is None:
            raise V12EvidenceError(f"manifest line is invalid: {relative}")
        digest, path = match.groups()
        if path.startswith("/") or ".." in Path(path).parts or "\\" in path:
            raise V12EvidenceError("manifest path is unsafe")
        entries.append((digest, path))
    paths = [path for _, path in entries]
    if not entries or paths != sorted(paths) or len(paths) != len(set(paths)):
        raise V12EvidenceError(f"manifest set is invalid: {relative}")
...
def _historical_manifest(relative: str) -> tuple[tuple[str, str], ...]:
    ...
    try:
        ...
        raise V12EvidenceError(f"historical manifest encoding is invalid: {relative}") from exc
    entries = []
    for line in lines:
        match = _LINE.fullmatch(line)
        if match is None:
            raise V12EvidenceError(f"historical manifest line is invalid: {relative}")
        digest, path = match.groups()
        if path.startswith("/") or ".." in Path(path).parts or "\\" in path:
            raise V12EvidenceError("historical manifest path is unsafe")
        entries.append((digest, path))
    paths = [path for _, path in entries]
    if not entries or len(paths) != len(set(paths)):
        raise V12EvidenceError(f"historical manifest set is invalid: {relative}")
    return tuple(entries)
...
def _dag() -> tuple[str, ...]:
    ...
    if root != ((_sha(PROJECT / ARTIFACT_MANIFEST), ARTIFACT_MANIFEST),):
        raise V12EvidenceError("root manifest edge is invalid")
    ...
    required = {
        ...
        STATIC, LIVE, V1_LIVE, V2_LIVE, V3_LIVE, V4_LIVE, V5_LIVE, V6_LIVE, V7_LIVE, V8_LIVE, V9_LIVE, V10_LIVE, V11_LIVE,
        *V1_HISTORY, *V2_HISTORY, *V3_HISTORY, *V4_HISTORY, *V5_HISTORY, *V6_HISTORY, *V7_HISTORY, *V8_HISTORY, *V9_HISTORY, *V10_HISTORY, *V11_HISTORY,
    }
    if not required.issubset(paths) or ARTIFACT_MANIFEST in paths or ROOT_MANIFEST in paths:
        raise V12EvidenceError("artifact manifest closure is invalid")
    for digest, relative in artifacts:
        if not (PROJECT / relative).is_file() or _sha(PROJECT / relative) != digest:
            raise V12EvidenceError(f"artifact hash mismatch: {relative}")
    return paths
...
def _bundle(paths: tuple[str, ...]) -> dict[str, object]:
    ...
    try:
        ...
        raise V12EvidenceError("bundle JSON is invalid") from exc
    if raw != json.dumps(bundle, sort_keys=True, separators=(",", ":")) + "\n":
        raise V12EvidenceError("bundle JSON is not canonical")
    if bundle.get("contract") != "Phase53CapabilityNexusEvidence.v12" or bundle.get("status") != "candidate-default-off-shadow-only-not-accepted":
        raise V12EvidenceError("bundle contract/status is invalid")
    if bundle.get("external_e6") != {"accepted": False, "required_before_acceptance": True, "status": "pending-external-review"}:
        raise V12EvidenceError("bundle overstates acceptance")
    if bundle.get("feature_gate") != {"dispatch_enabled": False, "nexus_enabled": False, "shadow_mode": True}:
        raise V12EvidenceError("bundle feature gate is invalid")
    files = bundle.get("files")
    expected = set(paths) - {BUNDLE}
    if not isinstance(files, dict) or set(files) != expected:
        raise V12EvidenceError("bundle file set is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V12EvidenceError(f"bundle file hash mismatch: {relative}")
    return bundle
...
def _static() -> dict[str, int]:
    ...
    for line in (PROJECT / STATIC).read_text(encoding="utf-8").splitlines():
        ...
        if not separator or not value.isdigit():
            raise V12EvidenceError("static log is invalid")
    ...
    if set(result) != expected or any(result.values()):
        raise V12EvidenceError("static gate failed")
    return result
...
def _history_and_tamper(bundle: dict[str, object]) -> None:
    ...
    if bundle.get("history") != expected:
        raise V12EvidenceError("V1/V2/V3 history is not exact or not rejected")
    for version, history in (
        ...
        for relative, digest in history.items():
            if _sha(PROJECT / relative) != digest:
                raise V12EvidenceError(f"{version} historical byte drift: {relative}")
    ...
    for root in HISTORICAL_ROOTS:
        ...
        for relative, digest in leaves.items():
            ...
            if prior != digest:
                raise V12EvidenceError(f"historical leaf digest conflict: {relative}")
    targets = [
    ...
    with tempfile.TemporaryDirectory(prefix="onyx-p53-history-tamper-") as directory:
        ...
        for relative in targets:
            ...
            if expected_digest is None:
                raise V12EvidenceError(f"historical tamper leaf is unbound: {relative}")
            copied = base / relative
            copied.parent.mkdir(parents=True, exist_ok=True)
            copied.write_bytes((PROJECT / relative).read_bytes())
            if _sha(copied) != expected_digest:
                raise V12EvidenceError(f"historical pristine copy mismatch: {relative}")
            copied.write_bytes(copied.read_bytes() + b"\n# disposable tamper\n")
            if _sha(copied) == expected_digest:
                raise V12EvidenceError(f"historical tamper fixture was not rejected: {relative}")
...
def _verify_manifest_tree(
    ...
    if depth > 32 or len(verified) >= 256 or len(leaves) > 4096:
        raise V12EvidenceError("historical manifest traversal budget exceeded")
    if relative in active:
        raise V12EvidenceError(f"historical manifest cycle detected: {relative}")
    if relative in verified:
        return
    active.add(relative)
    entries = _historical_manifest(relative)
    if len(entries) > 4096:
        raise V12EvidenceError("historical manifest leaf budget exceeded")
...
def _record_historical_leaf(leaves: dict[str, str], relative: str, digest: str) -> None:
    ...
    if prior is None:
        ...
        raise V12EvidenceError(f"historical recursive digest conflict: {relative}")
    if len(leaves) > 4096:
        raise V12EvidenceError("historical manifest leaf budget exceeded")
...
def _verify_historical_path(relative: str, digest: str) -> None:
    ...
    try:
        ...
        raise V12EvidenceError(f"historical path escapes or is missing: {relative}") from exc
    if not path.is_file() or _sha(path) != digest:
        raise V12EvidenceError(f"historical recursive leaf mismatch: {relative}")


def _assert_no_reparse(relative: str) -> None:
    parts = Path(relative).parts
    if not parts or Path(relative).is_absolute() or ".." in parts:
        raise V12EvidenceError(f"historical path is unsafe: {relative}")
    current = PROJECT
    ...
    for component in (None, *parts):
        ...
        if stat.S_ISLNK(info.st_mode) or attributes & reparse_flag:
            raise V12EvidenceError(f"historical path contains a reparse component: {relative}")


def _run_frozen_verifiers() -> None:
    if int(os.environ.get("ONYX_P53_V12_HISTORY_VERIFY_DEPTH", "0")) != 0:
        raise V12EvidenceError("historical verifier recursion detected")
    env = os.environ.copy()
    env["ONYX_P53_V12_HISTORY_VERIFY_DEPTH"] = "1"
    env["PYTHONDONTWRITEBYTECODE"] = "1"
    # Frozen V11 executes and binds the complete frozen V1-V10 verifier closure.
    for version in (11,):
...
def _live(bundle: dict[str, object]) -> None:
    ...
    if bundle.get("live_scan") != {"count": len(expanded), "manifest_sha256": _sha(PROJECT / LIVE)}:
        raise V12EvidenceError("live scan summary drift")
    for digest, relative in expanded:
        path = PROJECT / relative
        if _sha(path) != digest or "capability_nexus_v12" in path.read_text(encoding="utf-8"):
            raise V12EvidenceError(f"live boundary drift/wiring: {relative}")
...
def _assignment(path: Path, name: str) -> object:
    ...
    raise V12EvidenceError(f"missing assignment: {name}")
...
def _semantics(bundle: dict[str, object]) -> None:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import) and {item.name.split(".", 1)[0] for item in node.names} & forbidden:
            raise V12EvidenceError("candidate imports a forbidden runtime/provider surface")
        if isinstance(node, ast.ImportFrom) and (node.module or "").split(".", 1)[0] in forbidden:
            raise V12EvidenceError("candidate imports a forbidden runtime/provider surface")
    sys.path.insert(0, str(PROJECT))
    try:
        from core.capability_nexus_v12 import (  # noqa: PLC0415
            CapabilityNexusV12,
            NexusFeatureGateV12,
        ...
        sys.path.pop(0)
    nexus = CapabilityNexusV12(NexusFeatureGateV12())
    if hasattr(nexus, "dispatch") or hasattr(nexus, "execute"):
        raise V12EvidenceError("registry exposes dispatch")
    ...
    if len(built.descriptors) != 26 or dict(built.policy_mapping()) != policies:
        raise V12EvidenceError("legacy parity failed")
    ...
    expected = {
        ...
        "recursive_v1_to_v11_history_and_component_reparse_rejection",
    }
    if not isinstance(claims, dict) or set(claims) != expected or not all(claims.values()):
        raise V12EvidenceError("semantic claim set is invalid")
...
def _environment(bundle: dict[str, object]) -> None:
    ...
    if not isinstance(recorded, dict) or any(recorded.get(key) != value for key, value in expected.items()):
        raise V12EvidenceError("environment drift")
    ...
    if bundle.get("base_commit") != commit:
        raise V12EvidenceError("base commit drift")
...
def _fresh() -> None:
    ...
    if completed.returncode or "1441 passed" not in completed.stdout:
        raise V12EvidenceError("fresh focused suite failed")
...
def verify() -> dict[str, object]:
    ...
    counts = _junit()
    if counts != {"errors": 0, "failed": 0, "passed": 1441, "skipped": 0} or bundle.get("counts") != counts:
        raise V12EvidenceError("JUnit counts drift")
    if bundle.get("static_gates") != _static():
        raise V12EvidenceError("static evidence drift")
    _history_and_tamper(bundle)
    _live(bundle)
    _semantics(bundle)
    _environment(bundle)
    _fresh()
    return {"artifacts": len(paths), "focused_passed": 1441, "live_scan_files": 91,
            "root_sha256": _sha(PROJECT / ROOT_MANIFEST)}


def main() -> int:
    print("P53_CAPABILITY_NEXUS_V12_EVIDENCE_OK " + json.dumps(verify(), sort_keys=True))
    return 0
