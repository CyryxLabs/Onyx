...
V14 never reads either mutable live documentation path while materializing the
...
class HistoricalProjectionPathError(RuntimeError):
    """A historical authority or output path crossed the V14 boundary."""
...
def _load_exact_authoritative_module(
    ...
    private_name = f"_onyx_p52_v14_{label}_{secrets.token_hex(16)}"
...
def _private_head(root: Path) -> str:
    ...
    if absolute == ROOT.absolute():
        ...
        # executing at the authoritative V14 root. Return the frozen authority
...
class _HermeticSubprocessProxy:
    ...
    def run(self, command: object, *args: object, **kwargs: object) -> object:
        ...
        if executable in {"git", "git.exe"}:
            return _internal_git(normalized, cwd)
        ...
        kwargs["env"] = _sanitized_environment(supplied)
        return _stdlib_subprocess.run(normalized, *args, **kwargs)


_HERMETIC_SUBPROCESS = _HermeticSubprocessProxy()
...
def _materialize_r14(module: ModuleType, destination: Path) -> tuple[str, ...]:
    ...
    _write_new(root, ".git/HEAD", b"ref: refs/heads/onyx-v14\n")
    _write_new(root, ".git/refs/heads/onyx-v14", FROZEN_BASE_COMMIT.encode() + b"\n")
    _write_new(
        root,
        ".git/config",
...
def _install_projection_wrapper(module: ModuleType) -> None:
    ...
    if getattr(original, "_onyx_v14_frozen_projections", False):
        return
    ...
    materialize_r11._onyx_v14_frozen_projections = True  # type: ignore[attr-defined]
    ...
    module.subprocess = _HERMETIC_SUBPROCESS
...
def pytest_collection_modifyitems(items: list[object]) -> None:
    ...
    for item in items:
        ...
        if path.name in {
            ...
            test_module = getattr(item, "module", None)
            if test_module is not None:
                test_module.subprocess = _HERMETIC_SUBPROCESS
