"""Canonical launcher for metadata-only operational events in Onyx Live V23."""
...
LOG_PATH = data_root() / "runtime/logs/onyx-live-v23-startup.log"


def _v22_contract() -> dict[str, object]:
    namespace = runpy.run_path(
        str(ROOT / "scripts" / "launch_onyx_live_v22.pyw"),
        run_name="onyx_v22_launcher_contract_for_v23",
    )
    if not callable(namespace.get("run")) or not callable(namespace.get("_v21_contract")):
        raise RuntimeError("Onyx V22 launcher contract is unavailable")
    return namespace


@contextmanager
def _diagnostic_resources():
    v22_contract = _v22_contract()
    v21_contract = v22_contract["_v21_contract"]()
...
def _run_operational_events_diagnostic() -> dict[str, object]:
    ...
    with _diagnostic_resources() as (v19, resources), external_call_boundary_v1(counters=calls):
        ...
        from core.onyx_live_activation_v23 import activate_main
        ...
        with v19["_diagnostic_main_aliases_v19"](imported_main, locations) as original_aliases:
            ...
            try:
                ...
                if (
                    payload["constructor_owned"] is not True
                    or payload["content_captured"] is not False
                    or any(
                        payload[key] != 0
                        for key in (
                            "background_workers",
                    ...
                    raise RuntimeError("V23 operational-events diagnostic evidence diverged")
                ...
                if cleanup_failures:
                    ...
                    if active_error is not None:
                        active_error.add_note(f"V23 diagnostic cleanup failures: {detail}")
                    else:
                        raise RuntimeError(f"V23 diagnostic cleanup failed: {detail}")
...
def run() -> None:
    ...
    if sys.argv[1:] not in accepted:
        raise RuntimeError("Onyx Live V23 launcher arguments are invalid")
    legacy = _v22_contract()
    if sys.argv[1:] == ["--operational-events-smoke-test"]:
        _run_operational_events_diagnostic()
        return
    if sys.argv[1:]:
        if sys.argv[1:] == ["--native-startup-smoke-test"]:
            ...
            with LOG_PATH.open("a", encoding="utf-8", buffering=1) as log:
                print(
                    ...
                    "Onyx native startup smoke entering stable V23 launcher",
                    file=log,
                )
                legacy["run"]()
    ...
    v21_contract = legacy["_v21_contract"]()
    v20_contract = v21_contract["_v20_contract"]()
    v19 = v20_contract["_v19_contract"]()
    ...
    try:
        ...
        with LOG_PATH.open("a", encoding="utf-8", buffering=1) as log:
            sys.stdout = log
            sys.stderr = log
            print(f"\n[{datetime.now(timezone.utc).isoformat()}] Onyx Live V23 starting")
            print(f"Operational events: {controller.operational_events_capability}")
            try:
                onyx_main.main()
