"""Verify the frozen Phase 5.3 Capability Nexus V29 candidate."""
...
ARTIFACT_MANIFEST = "docs/onyx/VE-ARTIFACTS-P53-CAPABILITY-NEXUS-V29-001.sha256"
ROOT_MANIFEST = "docs/onyx/VE-SOURCE-P53-CAPABILITY-NEXUS-V29-001.sha256"
EVIDENCE = "docs/onyx/checkpoints/phase5-capability-nexus-v29"
BUNDLE = f"{EVIDENCE}/phase5-capability-nexus-v29.bundle.json"
JUNIT = f"{EVIDENCE}/phase5-capability-nexus-v29.junit.xml"
RAW = f"{EVIDENCE}/phase5-capability-nexus-v29.raw.log"
STATIC = f"{EVIDENCE}/phase5-capability-nexus-v29.static.log"
LIVE = f"{EVIDENCE}/phase5-capability-nexus-v29.live-scan.sha256"
METRICS = f"{EVIDENCE}/phase5-capability-nexus-v29.metrics.json"
COMBINED_JUNIT = f"{EVIDENCE}/phase5-capability-nexus-v29.combined.junit.xml"
REGRESSION_JUNIT = f"{EVIDENCE}/phase5-capability-nexus-v29.regressions.junit.xml"
REGRESSION_REPORT = f"{EVIDENCE}/phase5-capability-nexus-v29.regressions.json"
...
CHECKPOINT = f"{EVIDENCE}/PHASE5_3_CAPABILITY_NEXUS_V29_CHECKPOINT.md"
CORE = "core/capability_nexus_v29.py"
TESTS = "tests/test_capability_nexus_v29.py"
WHITESPACE = "scripts/check_phase5_capability_nexus_v29_whitespace.py"
SELF = "scripts/verify_phase5_capability_nexus_v29.py"
WORKER = "scripts/verify_phase5_capability_nexus_v29_worker.py"
REPORTER = "scripts/phase5_capability_nexus_v29_subtest_reporter.py"
COMBINED_SUITES = tuple(f"tests/test_capability_nexus_v{version}.py" for version in range(1, 30))
...
_PRIVATE_BASE = Path(tempfile.gettempdir()) / "onyx-p53-v29-private"
_PRIVATE_SENTINEL = ".phase53-v29-private"
_PRIVATE_CONTRACT = "Phase53CapabilityNexusPrivate.v29"


class V29EvidenceError(RuntimeError):
    pass
...
def _manifest(relative: str) -> tuple[tuple[str, str], ...]:
    ...
    if b"\r" in raw or not raw.endswith(b"\n"):
        raise V29EvidenceError(f"manifest line endings are invalid: {relative}")
    entries = []
    for line in raw.decode().splitlines():
        match = _LINE.fullmatch(line)
        if match is None:
            raise V29EvidenceError(f"manifest line is invalid: {relative}")
        digest, path = match.groups()
        if path.startswith("/") or ".." in Path(path).parts or "\\" in path:
            raise V29EvidenceError("manifest path is unsafe")
        entries.append((digest, path))
    paths = [path for _, path in entries]
    if not entries or paths != sorted(paths) or len(paths) != len(set(paths)):
        raise V29EvidenceError(f"manifest set is invalid: {relative}")
...
def _historical_manifest(relative: str) -> tuple[tuple[str, str], ...]:
    ...
    try:
        ...
        raise V29EvidenceError(f"historical manifest encoding is invalid: {relative}") from exc
    entries = []
    for line in lines:
        match = _LINE.fullmatch(line)
        if match is None:
            raise V29EvidenceError(f"historical manifest line is invalid: {relative}")
        digest, path = match.groups()
        if path.startswith("/") or ".." in Path(path).parts or "\\" in path:
            raise V29EvidenceError("historical manifest path is unsafe")
        entries.append((digest, path))
    paths = [path for _, path in entries]
    if not entries or len(paths) != len(set(paths)):
        raise V29EvidenceError(f"historical manifest set is invalid: {relative}")
    return tuple(entries)
...
def _dag() -> tuple[str, ...]:
    ...
    if root != ((_sha(PROJECT / ARTIFACT_MANIFEST), ARTIFACT_MANIFEST),):
        raise V29EvidenceError("root manifest edge is invalid")
    ...
    required = {
        ...
        V8_LIVE, V9_LIVE, V10_LIVE, V11_LIVE, V12_LIVE, V13_LIVE, V14_LIVE, V15_LIVE, V16_LIVE, V17_LIVE, V18_LIVE, V19_LIVE, V20_LIVE, V21_LIVE, V22_LIVE, V23_LIVE, V24_LIVE, V25_LIVE, V26_LIVE, V27_LIVE, V28_LIVE,
        *V1_HISTORY, *V2_HISTORY, *V3_HISTORY, *V4_HISTORY, *V5_HISTORY,
        *V6_HISTORY, *V7_HISTORY, *V8_HISTORY, *V9_HISTORY, *V10_HISTORY,
        *V11_HISTORY, *V12_HISTORY, *V13_HISTORY, *V14_HISTORY, *V15_HISTORY, *V16_HISTORY, *V17_HISTORY, *V18_HISTORY, *V19_HISTORY, *V20_HISTORY, *V21_HISTORY, *V22_HISTORY, *V23_HISTORY, *V24_HISTORY, *V25_HISTORY, *V26_HISTORY, *V27_HISTORY, *V28_HISTORY,
    }
    if not required.issubset(paths) or ARTIFACT_MANIFEST in paths or ROOT_MANIFEST in paths:
        raise V29EvidenceError("artifact manifest closure is invalid")
    for digest, relative in artifacts:
        if not (PROJECT / relative).is_file() or _sha(PROJECT / relative) != digest:
            raise V29EvidenceError(f"artifact hash mismatch: {relative}")
    return paths
...
def _bundle(paths: tuple[str, ...]) -> dict[str, object]:
    ...
    try:
        ...
        raise V29EvidenceError("bundle JSON is invalid") from exc
    if raw != json.dumps(bundle, sort_keys=True, separators=(",", ":")) + "\n":
        raise V29EvidenceError("bundle JSON is not canonical")
    if bundle.get("contract") != "Phase53CapabilityNexusEvidence.v29" or bundle.get("status") != "candidate-default-off-shadow-only-not-accepted":
        raise V29EvidenceError("bundle contract/status is invalid")
    if bundle.get("external_e6") != {"accepted": False, "required_before_acceptance": True, "status": "pending-external-review"}:
        raise V29EvidenceError("bundle overstates acceptance")
    if bundle.get("feature_gate") != {"dispatch_enabled": False, "nexus_enabled": False, "shadow_mode": True}:
        raise V29EvidenceError("bundle feature gate is invalid")
    files = bundle.get("files")
    expected = set(paths) - {BUNDLE}
    if not isinstance(files, dict) or set(files) != expected:
        raise V29EvidenceError("bundle file set is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V29EvidenceError(f"bundle file hash mismatch: {relative}")
    return bundle
...
def _validate_regression_report(report: dict[str, object]) -> None:
    if set(report) != {"base_tests", "contract", "exit_status", "subtests"}:
        raise V29EvidenceError("regression report schema is invalid")
    if report.get("contract") != "Phase53CapabilityNexusRegressionReport.v29" or report.get("exit_status") != 0:
        raise V29EvidenceError("regression report contract/status is invalid")
    ...
    if not isinstance(bases, list) or not isinstance(subtests, list) or len(bases) != 102 or len(subtests) != 221:
        raise V29EvidenceError("regression report semantic counts are invalid")
    ...
    for base in bases:
        if not isinstance(base, dict) or set(base) != {"identity", "nodeid", "outcome"} or base.get("outcome") != "passed":
            raise V29EvidenceError("regression base record is invalid")
        ...
        if not isinstance(nodeid, str) or identity != expected or nodeid in base_nodeids or identity in base_identities:
            raise V29EvidenceError("regression base identity is invalid or duplicated")
    ...
    for subtest in subtests:
        ...
        if not isinstance(subtest, dict) or set(subtest) != expected_keys or subtest.get("outcome") != "passed":
            raise V29EvidenceError("regression subtest record is invalid")
        ...
        if (
            ...
            raise V29EvidenceError("regression subtest parent/context is invalid")
        ...
        if (
            ...
            raise V29EvidenceError("regression subtest identity is invalid or duplicated")
    ...
    if any(sorted(values) != list(range(len(values))) for values in ordinals.values()):
        raise V29EvidenceError("regression subtest ordinals are not contiguous")
...
def _junit_nodeid(testcase: ET.Element) -> str:
    ...
    if not classname.startswith(prefix) or not name:
        raise V29EvidenceError("regression JUnit testcase identity is invalid")
...
def _require_whitespace_only(value: str | None, label: str) -> None:
    if value is not None and value.strip():
        raise V29EvidenceError(f"JUnit {label} contains unexpected text")


def _canonical_counter(value: str, label: str) -> int:
    if _UINT_LEXICAL.fullmatch(value) is None:
        raise V29EvidenceError(f"JUnit {label} counter lexical form is invalid")
    return int(value)


def _canonical_time(value: str, label: str) -> float:
    if _TIME_LEXICAL.fullmatch(value) is None:
        raise V29EvidenceError(f"JUnit {label} duration lexical form is invalid")
    duration = float(value)
    if not math.isfinite(duration) or duration < 0:
        raise V29EvidenceError(f"JUnit {label} duration is invalid")
    return duration
...
def _junit_path(path: Path, *, subtest_report: dict[str, object] | None = None) -> dict[str, int]:
    ...
    if document_body.startswith(b"<?xml"):
        ...
        if declaration_end < 0:
            raise V29EvidenceError("JUnit XML declaration is malformed")
        document_body = document_body[declaration_end + 2:]
    if b"<!--" in raw or b"<?" in document_body:
        raise V29EvidenceError("JUnit comments and processing instructions are forbidden")
    try:
        root = ET.fromstring(raw)
    except ET.ParseError as exc:
        raise V29EvidenceError("JUnit XML is malformed") from exc
    if root.tag != "testsuites" or root.attrib != {"name": "pytest tests"}:
        raise V29EvidenceError("JUnit root schema is invalid")
    ...
    if len(suites) != 1 or suites[0].tag != "testsuite":
        raise V29EvidenceError("JUnit must contain exactly one suite")
    ...
    for suite in suites:
        ...
        if set(suite.attrib) != allowed_suite or suite.attrib.get("name") != "pytest":
            raise V29EvidenceError("JUnit suite attribute schema is invalid")
        _canonical_time(suite.attrib["time"], "suite")
        try:
            datetime.fromisoformat(suite.attrib["timestamp"])
        except (ValueError, TypeError) as exc:
            raise V29EvidenceError("JUnit suite timestamp is invalid") from exc
        if not suite.attrib["hostname"]:
            raise V29EvidenceError("JUnit suite hostname is invalid")
        ...
        if any(item.tag != "testcase" for item in children):
            raise V29EvidenceError("JUnit suite contains an unexpected child")
        for testcase in children:
            if set(testcase.attrib) != {"classname", "name", "time"}:
                raise V29EvidenceError("JUnit testcase attribute schema is invalid")
            if not testcase.attrib["classname"] or not testcase.attrib["name"]:
                raise V29EvidenceError("JUnit testcase identity is empty")
            ...
            if len(outcome_children) > 1 or any(item.tag not in {"failure", "error", "skipped"} for item in outcome_children):
                raise V29EvidenceError("JUnit testcase contains an unexpected child")
            for outcome in outcome_children:
                if set(outcome.attrib) - {"message", "type"} or list(outcome):
                    raise V29EvidenceError("JUnit outcome element schema is invalid")
        ...
        if attrs["failures"] != child_failed or attrs["errors"] != child_errors or attrs["skipped"] != child_skipped:
            raise V29EvidenceError("JUnit suite counters do not reconcile with testcase children")
        tests += attrs["tests"]
        failed += child_failed
        errors += child_errors
    ...
    if delta < 0:
        raise V29EvidenceError("JUnit reports fewer tests than testcase children")
    if delta:
        if subtest_report is None or len(subtest_report["subtests"]) != delta:
            raise V29EvidenceError("JUnit synthetic count delta lacks validated subtest evidence")
        bases = subtest_report["base_tests"]
        if len(bases) != testcase_count:
            raise V29EvidenceError("JUnit testcase count does not match base-test evidence")
        ...
        if len(xml_nodeids) != testcase_count or xml_nodeids != report_nodeids:
            raise V29EvidenceError("JUnit testcase identities do not match base-test evidence")
        if failed or errors or skipped or any(item["outcome"] != "passed" for item in bases):
            raise V29EvidenceError("JUnit outcomes do not match all-pass base/subtest evidence")
...
def _static_path(path: Path, *, frozen_gate: str = "FROZEN_PROOF") -> dict[str, int]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if not separator or not value.isdigit():
            raise V29EvidenceError("static log is invalid")
    ...
    if set(result) != expected or any(result.values()):
        raise V29EvidenceError("static gate failed")
    return result
...
def _canonical_json(relative: str) -> dict[str, object]:
    ...
    try:
        ...
        raise V29EvidenceError(f"structured evidence JSON is invalid: {relative}") from exc
    if not isinstance(value, dict) or raw != json.dumps(value, sort_keys=True, separators=(",", ":")) + "\n":
        raise V29EvidenceError(f"structured evidence JSON is not canonical: {relative}")
    return value
...
def _sum_counts(items: list[dict[str, int]]) -> dict[str, int]:
    ...
    if not items or any(set(item) != keys for item in items):
        raise V29EvidenceError("JUnit count schema is invalid")
...
def _assert_exact_evidence(label: str, actual: object, expected: object) -> None:
    if type(actual) is not type(expected):
        raise V29EvidenceError(f"{label} evidence type drift")
    if isinstance(expected, dict):
        if set(actual) != set(expected):  # type: ignore[arg-type]
            raise V29EvidenceError(f"{label} evidence field drift")
    ...
    if isinstance(expected, list):
        if len(actual) != len(expected):  # type: ignore[arg-type]
            raise V29EvidenceError(f"{label} evidence list drift")
        for index, value in enumerate(expected):
            _assert_exact_evidence(f"{label}[{index}]", actual[index], value)  # type: ignore[index]
        return
    if actual != expected:
        raise V29EvidenceError(f"{label} evidence value drift")
...
def _checkpoint() -> None:
    ...
    if not required.issubset(set(text.splitlines())):
        raise V29EvidenceError("checkpoint status or structured-evidence claim is invalid")
    ...
    if unchecked.search(text):
        raise V29EvidenceError("checkpoint contains an unchecked quantitative claim")
...
def _validate_metrics(
    ...
    if set(metrics) != expected_keys or metrics.get("contract") != "Phase53CapabilityNexusMetrics.v29":
        raise V29EvidenceError("structured evidence top-level schema is invalid")
    ...
    expected_regressions = {
        ...
        "junit_schema": "strict_single_pytest_suite_lexical_v29",
    ...
    if regression_junit.get("passed") != 323:
        raise V29EvidenceError("structured regression JUnit semantic count drift")
    ...
    if not isinstance(runtime, dict) or set(runtime) != {
        ...
        raise V29EvidenceError("structured runtime evidence schema is invalid")
    if (
        ...
        raise V29EvidenceError("structured runtime timeout evidence drift")
    measured = runtime.get("measured_full_verifier_seconds")
    if type(measured) not in {int, float} or not 0 < measured <= 180:
        raise V29EvidenceError("structured measured verifier runtime is invalid")
    return metrics
...
def _validate_frozen_v14_proof() -> dict[str, object]:
    ...
    if root != expected_root:
        raise V29EvidenceError("frozen V14 root manifest edge is invalid")
    ...
    if (
        ...
        raise V29EvidenceError("frozen V14 artifact manifest closure is invalid")
    for digest, relative in artifacts:
    ...
    if (
        ...
        raise V29EvidenceError("frozen V14 bundle contract/status is invalid")
    files = bundle.get("files")
    expected_files = set(artifact_paths) - {V14_BUNDLE}
    if not isinstance(files, dict) or set(files) != expected_files:
        raise V29EvidenceError("frozen V14 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V29EvidenceError(f"frozen V14 bundle file hash mismatch: {relative}")
    ...
    if live_entries != (
        ...
        raise V29EvidenceError("frozen V14 live manifest anchors are invalid")
    for digest, relative in live_entries:
        _verify_historical_path(relative, digest)
    if bundle.get("live_scan") != {"count": 93, "manifest_sha256": _sha(PROJECT / V14_LIVE)}:
        raise V29EvidenceError("frozen V14 live evidence is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "external" not in checkpoint.lower():
        raise V29EvidenceError("frozen V14 checkpoint status is invalid")
    verifier = (PROJECT / V14_VERIFIER).read_text(encoding="utf-8")
    if "P53_CAPABILITY_NEXUS_V14_EVIDENCE_OK" not in verifier:
        raise V29EvidenceError("frozen V14 verifier marker contract is missing")
    return {
...
def _validate_frozen_v15_proof() -> dict[str, object]:
    ...
    if root != expected_root:
        raise V29EvidenceError("frozen V15 root manifest edge is invalid")
    ...
    if len(artifacts) != 179 or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)) or not required.issubset(paths):
        raise V29EvidenceError("frozen V15 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V29EvidenceError("frozen V15 bundle contract/status is invalid")
    files = bundle.get("files")
    if not isinstance(files, dict) or set(files) != set(paths) - {V15_BUNDLE}:
        raise V29EvidenceError("frozen V15 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V29EvidenceError(f"frozen V15 bundle file hash mismatch: {relative}")
    expected_history = {
        f"phase53_v{version}": {"decision": "rejected", "hashes": history}
        for version, history in enumerate(
    ...
    if bundle.get("history") != expected_history:
        raise V29EvidenceError("frozen V15 rejected-history contract is invalid")
    junit = _junit_path(PROJECT / V15_JUNIT)
    if junit != {"errors": 0, "failed": 0, "passed": 1798, "skipped": 0} or bundle.get("counts") != junit:
        raise V29EvidenceError("frozen V15 JUnit evidence is invalid")
    static = _static_path(PROJECT / V15_STATIC)
    if bundle.get("static_gates") != static:
        raise V29EvidenceError("frozen V15 static evidence is invalid")
    ...
    if live_entries != (
        ...
        raise V29EvidenceError("frozen V15 live manifest anchors are invalid")
    if bundle.get("live_scan") != {"count": 94, "manifest_sha256": _sha(PROJECT / V15_LIVE)}:
        raise V29EvidenceError("frozen V15 live evidence is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V15_EVIDENCE_OK" not in verifier:
        raise V29EvidenceError("frozen V15 checkpoint/marker contract is invalid")
    return {
...
def _validate_frozen_v16_proof() -> dict[str, object]:
    ...
    if root != ((V16_HISTORY[V16_ARTIFACT_MANIFEST], V16_ARTIFACT_MANIFEST),):
        raise V29EvidenceError("frozen V16 root manifest edge is invalid")
    ...
    if len(artifacts) != 197 or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)) or not required.issubset(paths):
        raise V29EvidenceError("frozen V16 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V29EvidenceError("frozen V16 bundle contract/status is invalid")
    files = bundle.get("files")
    if not isinstance(files, dict) or set(files) != set(paths) - {V16_BUNDLE}:
        raise V29EvidenceError("frozen V16 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V29EvidenceError(f"frozen V16 bundle file hash mismatch: {relative}")
    junit = _junit_path(PROJECT / V16_JUNIT)
    if junit != {"errors": 0, "failed": 0, "passed": 1826, "skipped": 0} or bundle.get("counts") != junit:
        raise V29EvidenceError("frozen V16 JUnit evidence is invalid")
    static = _static_path(PROJECT / V16_STATIC)
    if bundle.get("static_gates") != static:
        raise V29EvidenceError("frozen V16 static evidence is invalid")
    ...
    if live_entries != (
        ...
        raise V29EvidenceError("frozen V16 live evidence is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V16_EVIDENCE_OK" not in verifier:
        raise V29EvidenceError("frozen V16 checkpoint/marker contract is invalid")
    return {
        "artifact_entries": len(artifacts), "bundle_status": bundle["status"],
        "junit_passed": junit["passed"], "live_scan_count": bundle["live_scan"]["count"],
        "marker_contract_bound": True, "static_gates": len(static),
...
def _validate_frozen_v17_proof() -> dict[str, object]:
    ...
    if root != ((V17_HISTORY[V17_ARTIFACT_MANIFEST], V17_ARTIFACT_MANIFEST),):
        raise V29EvidenceError("frozen V17 root manifest edge is invalid")
    ...
    if len(artifacts) != 215 or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)) or not required.issubset(paths):
        raise V29EvidenceError("frozen V17 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V29EvidenceError("frozen V17 bundle contract/status is invalid")
    files = bundle.get("files")
    if not isinstance(files, dict) or set(files) != set(paths) - {V17_BUNDLE}:
        raise V29EvidenceError("frozen V17 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V29EvidenceError(f"frozen V17 bundle file hash mismatch: {relative}")
    junit = _junit_path(PROJECT / V17_JUNIT)
    if junit != {"errors": 0, "failed": 0, "passed": 1847, "skipped": 0} or bundle.get("counts") != junit:
        raise V29EvidenceError("frozen V17 JUnit evidence is invalid")
    static = _static_path(PROJECT / V17_STATIC)
    if bundle.get("static_gates") != static:
        raise V29EvidenceError("frozen V17 static evidence is invalid")
    ...
    if live_entries != (
        ...
        raise V29EvidenceError("frozen V17 live evidence is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V17_EVIDENCE_OK" not in verifier:
        raise V29EvidenceError("frozen V17 checkpoint/marker contract is invalid")
    return {
        "artifact_entries": len(artifacts), "bundle_status": bundle["status"],
        "junit_passed": junit["passed"], "live_scan_count": bundle["live_scan"]["count"],
        "marker_contract_bound": True, "static_gates": len(static),
...
def _validate_frozen_v18_proof() -> dict[str, object]:
    ...
    if root != ((V18_HISTORY[V18_ARTIFACT_MANIFEST], V18_ARTIFACT_MANIFEST),):
        raise V29EvidenceError("frozen V18 root manifest edge is invalid")
    ...
    if len(artifacts) != 234 or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)) or not required.issubset(paths):
        raise V29EvidenceError("frozen V18 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V29EvidenceError("frozen V18 bundle contract/status is invalid")
    files = bundle.get("files")
    if not isinstance(files, dict) or set(files) != set(paths) - {V18_BUNDLE}:
        raise V29EvidenceError("frozen V18 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V29EvidenceError(f"frozen V18 bundle file hash mismatch: {relative}")
    junit = _junit_path(PROJECT / V18_JUNIT)
    if junit != {"errors": 0, "failed": 0, "passed": 1875, "skipped": 0} or bundle.get("counts") != junit:
        raise V29EvidenceError("frozen V18 JUnit evidence is invalid")
    static = _static_path(PROJECT / V18_STATIC)
    if bundle.get("static_gates") != static:
        raise V29EvidenceError("frozen V18 static evidence is invalid")
    ...
    if live_entries != (
        ...
        raise V29EvidenceError("frozen V18 live evidence is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V18_EVIDENCE_OK" not in verifier:
        raise V29EvidenceError("frozen V18 checkpoint/marker contract is invalid")
    return {
        "artifact_entries": len(artifacts), "bundle_status": bundle["status"],
        "junit_passed": junit["passed"], "live_scan_count": bundle["live_scan"]["count"],
        "marker_contract_bound": True, "static_gates": len(static),
...
def _validate_frozen_v19_proof() -> dict[str, object]:
    ...
    if root != ((V19_HISTORY[V19_ARTIFACT_MANIFEST], V19_ARTIFACT_MANIFEST),):
        raise V29EvidenceError("frozen V19 root manifest edge is invalid")
    ...
    if len(artifacts) != 253 or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)) or not required.issubset(paths):
        raise V29EvidenceError("frozen V19 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if bundle.get("contract") != "Phase53CapabilityNexusEvidence.v19" or bundle.get("status") != "candidate-default-off-shadow-only-not-accepted":
        raise V29EvidenceError("frozen V19 bundle contract/status is invalid")
    files = bundle.get("files")
    if not isinstance(files, dict) or set(files) != set(paths) - {V19_BUNDLE}:
        raise V29EvidenceError("frozen V19 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V29EvidenceError(f"frozen V19 bundle file hash mismatch: {relative}")
    junit = _junit_path(PROJECT / V19_JUNIT)
    if junit != {"errors": 0, "failed": 0, "passed": 1893, "skipped": 0} or bundle.get("counts") != junit:
        raise V29EvidenceError("frozen V19 JUnit evidence is invalid")
    static = _static_path(PROJECT / V19_STATIC)
    if bundle.get("static_gates") != static:
        raise V29EvidenceError("frozen V19 static evidence is invalid")
    ...
    if live_entries != (
        ...
        raise V29EvidenceError("frozen V19 live evidence is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V19_EVIDENCE_OK" not in verifier:
        raise V29EvidenceError("frozen V19 checkpoint/marker contract is invalid")
    return {
        "artifact_entries": len(artifacts), "bundle_status": bundle["status"],
        "junit_passed": junit["passed"], "live_scan_count": bundle["live_scan"]["count"],
        "marker_contract_bound": True, "static_gates": len(static),
...
def _validate_frozen_v20_proof() -> dict[str, object]:
    ...
    if root != ((V20_HISTORY[V20_ARTIFACT_MANIFEST], V20_ARTIFACT_MANIFEST),):
        raise V29EvidenceError("frozen V20 root manifest edge is invalid")
    ...
    if len(artifacts) != 271 or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)) or not required.issubset(paths):
        raise V29EvidenceError("frozen V20 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if bundle.get("contract") != "Phase53CapabilityNexusEvidence.v20" or bundle.get("status") != "candidate-default-off-shadow-only-not-accepted":
        raise V29EvidenceError("frozen V20 bundle contract/status is invalid")
    files = bundle.get("files")
    if not isinstance(files, dict) or set(files) != set(paths) - {V20_BUNDLE}:
        raise V29EvidenceError("frozen V20 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V29EvidenceError(f"frozen V20 bundle file hash mismatch: {relative}")
    junit = _junit_path(PROJECT / V20_JUNIT)
    if junit != {"errors": 0, "failed": 0, "passed": 1903, "skipped": 0} or bundle.get("counts") != junit:
        raise V29EvidenceError("frozen V20 JUnit evidence is invalid")
    static = _static_path(PROJECT / V20_STATIC)
    if bundle.get("static_gates") != static:
        raise V29EvidenceError("frozen V20 static evidence is invalid")
    ...
    if live_entries != (
        ...
        raise V29EvidenceError("frozen V20 live evidence is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V20_EVIDENCE_OK" not in verifier:
        raise V29EvidenceError("frozen V20 checkpoint/marker contract is invalid")
    return {
        "artifact_entries": len(artifacts), "bundle_status": bundle["status"],
        "junit_passed": junit["passed"], "live_scan_count": bundle["live_scan"]["count"],
        "marker_contract_bound": True, "static_gates": len(static),
...
def _validate_frozen_v21_proof() -> dict[str, object]:
    ...
    if root != ((V21_HISTORY[V21_ARTIFACT_MANIFEST], V21_ARTIFACT_MANIFEST),):
        raise V29EvidenceError("frozen V21 root manifest edge is invalid")
    ...
    if len(artifacts) != 289 or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)) or not required.issubset(paths):
        raise V29EvidenceError("frozen V21 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if bundle.get("contract") != "Phase53CapabilityNexusEvidence.v21" or bundle.get("status") != "candidate-default-off-shadow-only-not-accepted":
        raise V29EvidenceError("frozen V21 bundle contract/status is invalid")
    files = bundle.get("files")
    if not isinstance(files, dict) or set(files) != set(paths) - {V21_BUNDLE}:
        raise V29EvidenceError("frozen V21 bundle file closure is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V29EvidenceError(f"frozen V21 bundle file hash mismatch: {relative}")
    junit = _junit_path(PROJECT / V21_JUNIT)
    if junit != {"errors": 0, "failed": 0, "passed": 1916, "skipped": 0} or bundle.get("counts") != junit:
        raise V29EvidenceError("frozen V21 JUnit evidence is invalid")
    static = _static_path(PROJECT / V21_STATIC)
    if bundle.get("static_gates") != static:
        raise V29EvidenceError("frozen V21 static evidence is invalid")
    ...
    if live_entries != (
        ...
        raise V29EvidenceError("frozen V21 live evidence is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V21_EVIDENCE_OK" not in verifier:
        raise V29EvidenceError("frozen V21 checkpoint/marker contract is invalid")
    return {
        "artifact_entries": len(artifacts), "bundle_status": bundle["status"],
        "junit_passed": junit["passed"], "live_scan_count": bundle["live_scan"]["count"],
        "marker_contract_bound": True, "static_gates": len(static),
...
def _validate_frozen_v22_proof() -> dict[str, object]:
    ...
    if root != ((V22_HISTORY[V22_ARTIFACT_MANIFEST], V22_ARTIFACT_MANIFEST),):
        raise V29EvidenceError("frozen V22 root manifest edge is invalid")
    ...
    if len(artifacts) != 307 or paths != tuple(sorted(paths)) or len(paths) != len(set(paths)) or not required.issubset(paths):
        raise V29EvidenceError("frozen V22 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if bundle.get("contract") != "Phase53CapabilityNexusEvidence.v22" or not isinstance(files, dict) or set(files) != set(paths) - {V22_BUNDLE}:
        raise V29EvidenceError("frozen V22 bundle is invalid")
    ...
    if junit != {"errors": 0, "failed": 0, "passed": 1927, "skipped": 0} or bundle.get("counts") != junit or bundle.get("static_gates") != static:
        raise V29EvidenceError("frozen V22 quantitative proof is invalid")
...
def _validate_frozen_v23_proof() -> dict[str, object]:
    ...
    if root != expected_root:
        raise V29EvidenceError("frozen V23 root manifest edge is invalid")
    ...
    if (
        ...
        raise V29EvidenceError("frozen V23 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V29EvidenceError("frozen V23 bundle contract is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V29EvidenceError(f"frozen V23 bundle file hash mismatch: {relative}")
    ...
    if (
        ...
        raise V29EvidenceError("frozen V23 quantitative proof is invalid")
    ...
    if live_entries != (
        ...
        raise V29EvidenceError("frozen V23 live proof is invalid")
    ...
    if (
        ...
        raise V29EvidenceError("frozen V23 checkpoint/marker contract is invalid")
    return {
...
def _validate_frozen_v24_proof() -> dict[str, object]:
    ...
    if root != ((V24_HISTORY[V24_ARTIFACT_MANIFEST], V24_ARTIFACT_MANIFEST),):
        raise V29EvidenceError("frozen V24 root manifest edge is invalid")
    ...
    if (
        ...
        raise V29EvidenceError("frozen V24 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V29EvidenceError("frozen V24 bundle contract is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V29EvidenceError(f"frozen V24 bundle file hash mismatch: {relative}")
    ...
    if (
        ...
        raise V29EvidenceError("frozen V24 quantitative proof is invalid")
    ...
    if live_entries != (
        ...
        raise V29EvidenceError("frozen V24 live proof is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V24_EVIDENCE_OK" not in verifier:
        raise V29EvidenceError("frozen V24 checkpoint/marker contract is invalid")
    return {
...
def _validate_frozen_v25_proof() -> dict[str, object]:
    ...
    if root != ((V25_HISTORY[V25_ARTIFACT_MANIFEST], V25_ARTIFACT_MANIFEST),):
        raise V29EvidenceError("frozen V25 root manifest edge is invalid")
    ...
    if (
        ...
        raise V29EvidenceError("frozen V25 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V29EvidenceError("frozen V25 bundle contract is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V29EvidenceError(f"frozen V25 bundle file hash mismatch: {relative}")
    ...
    if (
        ...
        raise V29EvidenceError("frozen V25 quantitative proof is invalid")
    ...
    if live_entries != (
        ...
        raise V29EvidenceError("frozen V25 live proof is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V25_EVIDENCE_OK" not in verifier:
        raise V29EvidenceError("frozen V25 checkpoint/marker contract is invalid")
    return {
...
def _validate_frozen_v26_proof() -> dict[str, object]:
    ...
    if root != ((V26_HISTORY[V26_ARTIFACT_MANIFEST], V26_ARTIFACT_MANIFEST),):
        raise V29EvidenceError("frozen V26 root manifest edge is invalid")
    ...
    if (
        ...
        raise V29EvidenceError("frozen V26 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V29EvidenceError("frozen V26 bundle contract is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V29EvidenceError(f"frozen V26 bundle file hash mismatch: {relative}")
    ...
    if (
        ...
        raise V29EvidenceError("frozen V26 quantitative proof is invalid")
    ...
    if live_entries != (
        ...
        raise V29EvidenceError("frozen V26 live proof is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V26_EVIDENCE_OK" not in verifier:
        raise V29EvidenceError("frozen V26 checkpoint/marker contract is invalid")
    return {
...
def _validate_frozen_v27_proof() -> dict[str, object]:
    ...
    if root != ((V27_HISTORY[V27_ARTIFACT_MANIFEST], V27_ARTIFACT_MANIFEST),):
        raise V29EvidenceError("frozen V27 root manifest edge is invalid")
    ...
    if (
        ...
        raise V29EvidenceError("frozen V27 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V29EvidenceError("frozen V27 bundle contract is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V29EvidenceError(f"frozen V27 bundle file hash mismatch: {relative}")
    ...
    if (
        ...
        raise V29EvidenceError("frozen V27 quantitative proof is invalid")
    ...
    if live_entries != (
        ...
        raise V29EvidenceError("frozen V27 live proof is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V27_EVIDENCE_OK" not in verifier:
        raise V29EvidenceError("frozen V27 checkpoint/marker contract is invalid")
    return {
...
def _validate_frozen_v28_proof() -> dict[str, object]:
    ...
    if root != ((V28_HISTORY[V28_ARTIFACT_MANIFEST], V28_ARTIFACT_MANIFEST),):
        raise V29EvidenceError("frozen V28 root manifest edge is invalid")
    ...
    if (
        ...
        raise V29EvidenceError("frozen V28 artifact manifest closure is invalid")
    for digest, relative in artifacts:
        _verify_historical_path(relative, digest)
    ...
    if (
        ...
        raise V29EvidenceError("frozen V28 bundle contract is invalid")
    for relative, digest in files.items():
        if digest != _sha(PROJECT / relative):
            raise V29EvidenceError(f"frozen V28 bundle file hash mismatch: {relative}")
    ...
    if (
        ...
        raise V29EvidenceError("frozen V28 quantitative proof is invalid")
    ...
    if live_entries != (
        ...
        raise V29EvidenceError("frozen V28 live proof is invalid")
    ...
    if "Status: **candidate" not in checkpoint or "P53_CAPABILITY_NEXUS_V28_EVIDENCE_OK" not in verifier:
        raise V29EvidenceError("frozen V28 checkpoint/marker contract is invalid")
    return {
...
def _history_and_tamper(bundle: dict[str, object]) -> tuple[dict[str, object], dict[str, object]]:
    ...
    if bundle.get("history") != expected:
        raise V29EvidenceError("V1/V2/V3 history is not exact or not rejected")
    for version, history in (
        ...
        for relative, digest in history.items():
            if _sha(PROJECT / relative) != digest:
                raise V29EvidenceError(f"{version} historical byte drift: {relative}")
    ...
    if not set(HISTORICAL_ROOTS).issubset(historical_manifests):
        raise V29EvidenceError("frozen V1-V28 root closure is incomplete")
    targets = [
    ...
    with tempfile.TemporaryDirectory(prefix="onyx-p53-history-tamper-") as directory:
        ...
        for relative in targets:
            ...
            if expected_digest is None:
                raise V29EvidenceError(f"historical tamper leaf is unbound: {relative}")
            copied = base / relative
            copied.parent.mkdir(parents=True, exist_ok=True)
            copied.write_bytes((PROJECT / relative).read_bytes())
            if _sha(copied) != expected_digest:
                raise V29EvidenceError(f"historical pristine copy mismatch: {relative}")
            copied.write_bytes(copied.read_bytes() + b"\n# disposable tamper\n")
            if _sha(copied) == expected_digest:
                raise V29EvidenceError(f"historical tamper fixture was not rejected: {relative}")
    history_metrics = {
        "frozen_candidates_cryptographically_validated": 28,
...
def _verify_manifest_tree(
    ...
    if depth > 32 or len(verified) >= 256 or len(leaves) > 4096:
        raise V29EvidenceError("historical manifest traversal budget exceeded")
    if relative in active:
        raise V29EvidenceError(f"historical manifest cycle detected: {relative}")
    if relative in verified:
        return
    active.add(relative)
    entries = _historical_manifest(relative)
    if len(entries) > 4096:
        raise V29EvidenceError("historical manifest leaf budget exceeded")
...
def _record_historical_leaf(leaves: dict[str, str], relative: str, digest: str) -> None:
    ...
    if prior is None:
        ...
        raise V29EvidenceError(f"historical recursive digest conflict: {relative}")
    if len(leaves) > 4096:
        raise V29EvidenceError("historical manifest leaf budget exceeded")
...
def _verify_historical_path(relative: str, digest: str) -> None:
    ...
    try:
        ...
        raise V29EvidenceError(f"historical path escapes or is missing: {relative}") from exc
    if not path.is_file() or _sha(path) != digest:
        raise V29EvidenceError(f"historical recursive leaf mismatch: {relative}")


def _assert_no_reparse(relative: str) -> None:
    parts = Path(relative).parts
    if not parts or Path(relative).is_absolute() or ".." in parts:
        raise V29EvidenceError(f"historical path is unsafe: {relative}")
    current = PROJECT
...
def _live(bundle: dict[str, object]) -> dict[str, object]:
    ...
    if bundle.get("live_scan") != {"count": len(expanded), "manifest_sha256": _sha(PROJECT / LIVE)}:
        raise V29EvidenceError("live scan summary drift")
    for digest, relative in expanded:
        path = PROJECT / relative
        if _sha(path) != digest or "capability_nexus_v29" in path.read_text(encoding="utf-8"):
            raise V29EvidenceError(f"live boundary drift/wiring: {relative}")
    return {
...
def _assignment(path: Path, name: str) -> object:
    ...
    raise V29EvidenceError(f"missing assignment: {name}")
...
def _semantics(bundle: dict[str, object]) -> None:
    ...
    for relative in (CORE, TESTS, SELF, WORKER, REPORTER, WHITESPACE, CHECKPOINT, METRICS, BUNDLE):
        ...
        if any(term in source for term in forbidden_overclaims):
            raise V29EvidenceError(f"same-process runtime overclaim is forbidden: {relative}")
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import) and {item.name.split(".", 1)[0] for item in node.names} & forbidden:
            raise V29EvidenceError("candidate imports a forbidden runtime/provider surface")
        if isinstance(node, ast.ImportFrom) and (node.module or "").split(".", 1)[0] in forbidden:
            raise V29EvidenceError("candidate imports a forbidden runtime/provider surface")
    sys.path.insert(0, str(PROJECT))
    try:
        from core.capability_nexus_v29 import (  # noqa: PLC0415
            CapabilityNexusV29,
            NexusFeatureGateV29,
            _METADATA_ENUMS,
            _safe_metadata,
            build_legacy_descriptors_v29,
        )
    finally:
        sys.path.pop(0)
    nexus = CapabilityNexusV29(NexusFeatureGateV29())
    if hasattr(nexus, "dispatch") or hasattr(nexus, "execute"):
        raise V29EvidenceError("registry exposes dispatch")
    ...
    built = build_legacy_descriptors_v29(
    ...
    if len(built.descriptors) != 26 or dict(built.policy_mapping()) != policies:
        raise V29EvidenceError("legacy parity failed")
    ...
    if dict(_METADATA_ENUMS) != expected_metadata_enums:
        raise V29EvidenceError("closed metadata provenance contract drift")
    for key, value in (
        ...
        try:
            ...
            raise V29EvidenceError("closed metadata provenance contract accepted an unknown value")
    ...
    if called & {"_secret_canary", "_raw_credential_canary", "_validate_content_decoding_closure"}:
        raise V29EvidenceError("typed metadata still uses free-text heuristic inference")
    ...
    expected = {
        ...
        "recursive_v1_to_v28_history_and_component_reparse_rejection",
    ...
    if not isinstance(claims, dict) or set(claims) != expected or not all(claims.values()):
        raise V29EvidenceError("semantic claim set is invalid")
...
def _environment(bundle: dict[str, object]) -> None:
    ...
    if not isinstance(recorded, dict) or any(recorded.get(key) != value for key, value in expected.items()):
        raise V29EvidenceError("environment drift")
    commit = bundle.get("base_commit")
    if not isinstance(commit, str) or re.fullmatch(r"[0-9a-f]{40}", commit) is None:
        raise V29EvidenceError("base commit evidence is invalid")
...
def verify(fresh_focused_junit: Path) -> dict[str, object]:
    ...
    if focused["errors"] or focused["failed"] or focused["skipped"] or bundle.get("counts") != focused:
        raise V29EvidenceError("JUnit counts drift")
    if not fresh_focused_junit.is_file() or _junit_path(fresh_focused_junit) != focused:
        raise V29EvidenceError("direct focused pytest evidence does not match frozen JUnit")
    static = _static()
    if bundle.get("static_gates") != static:
        raise V29EvidenceError("static evidence drift")
    ...
    if bundle.get("historical_recursive_evidence") != history:
        raise V29EvidenceError("bundle recursive history evidence drift")
    if bundle.get("regression_evidence") != metrics["regressions"]:
        raise V29EvidenceError("bundle regression evidence drift")
    if bundle.get("artifact_entries") != metrics["artifact"]:
        raise V29EvidenceError("bundle artifact entry evidence drift")
    if bundle.get("structured_evidence") != {
        ...
        raise V29EvidenceError("bundle structured evidence binding drift")
    _checkpoint()
    _semantics(bundle)
    _environment(bundle)
...
def _validated_fresh_focused_junit(path: Path) -> Path:
    if path.name != "focused.junit.xml" or re.fullmatch(r"[0-9a-f]{64}", path.parent.name) is None:
        raise V29EvidenceError("fresh focused JUnit path is not nonce-scoped")
    try:
        base = _PRIVATE_BASE.resolve(strict=True)
        directory = path.parent.resolve(strict=True)
        resolved = path.resolve(strict=True)
    except OSError as exc:
        raise V29EvidenceError("fresh focused JUnit private realpath is invalid") from exc
    ...
    if base != expected_base or directory.parent != base or resolved.parent != directory:
        raise V29EvidenceError("fresh focused JUnit escapes its fixed private base")
    for component in (_PRIVATE_BASE, path.parent, path.parent / _PRIVATE_SENTINEL, path):
        ...
        if stat.S_ISLNK(info.st_mode) or getattr(info, "st_file_attributes", 0) & getattr(stat, "FILE_ATTRIBUTE_REPARSE_POINT", 0x400):
            raise V29EvidenceError("fresh focused JUnit path contains a reparse component")
    sentinel = path.parent / _PRIVATE_SENTINEL
    if sentinel.read_text(encoding="ascii") != f"{_PRIVATE_CONTRACT}:{path.parent.name}\n":
        raise V29EvidenceError("fresh focused JUnit sentinel is invalid")
    return resolved


def main() -> int:
    if len(sys.argv) != 3 or sys.argv[1] != "--fresh-focused-junit":
        raise V29EvidenceError("pure worker requires exactly one fresh focused JUnit path")
    fresh = _validated_fresh_focused_junit(Path(sys.argv[2]))
    print("P53_CAPABILITY_NEXUS_V29_EVIDENCE_OK " + json.dumps(verify(fresh), sort_keys=True))
    return 0
