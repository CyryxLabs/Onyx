"""Desktop bootstrap establishing the exact canonical V10 environment."""
...
LAUNCHER = ROOT / "scripts/launch_onyx_live_v10.pyw"
V9_ACTIVE = {
...
ACTIVE = {
    **V9_ACTIVE,
    "ONYX_LIVE_ACTIVATION_V10": "1",
    "ONYX_PHASE6_LIVE_WIRING_V1": "true",
    "ONYX_HUD_V7_LIVE": "1",
}
ALIASES = (
    "ONYX_PRINCIPAL_ID",
...
def run() -> None:
    if sys.argv[1:] not in ([], ["--preflight-only"]):
        raise RuntimeError("Onyx Live V10 bootstrap arguments are invalid")
    if str(ROOT) not in sys.path:
        sys.path.insert(0, str(ROOT))
    from core.onyx_live_activation_v10 import _verify_runtime_bundle
