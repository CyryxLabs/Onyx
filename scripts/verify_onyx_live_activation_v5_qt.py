"""Real offscreen Qt gate for V5's single setup projection."""
...
def main_gate():
    ...
    try:
        ...
        assert window._v5_host is not None
        assert not window._v5_host._loaded
        ...
        # Reconfiguration signals and repeated requests must raise the existing
        # setup surface, never create a second center panel.
        window._show_setup()
        window._show_setup()
        app.processEvents()
        ...
        app.processEvents()
        assert window._v5_host._loaded
        assert window._hud_v5_live
        assert not any(
            item.isVisible() for item in window.findChildren(ui.SetupOverlay)
        )

        print("ONYX_LIVE_ACTIVATION_V5_QT_OK")
        print(
