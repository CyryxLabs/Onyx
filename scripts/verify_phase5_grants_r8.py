"""Semantic, live-surface, and byte-exact verifier for Phase 5.1 R8."""
...
from scripts import verify_phase5_grants_r7 as r7


TOP_MANIFEST = "docs/onyx/VE-SOURCE-P51-GRANTS-R8-001.sha256"
ARTIFACT_MANIFEST = "docs/onyx/VE-ARTIFACTS-P51-GRANTS-R8-001.sha256"
EVIDENCE_DIR = "docs/onyx/checkpoints/phase5-grants-r8"
BUNDLE = f"{EVIDENCE_DIR}/phase5-grants-r8.bundle.json"
JUNIT = f"{EVIDENCE_DIR}/phase5-grants-r8.junit.xml"
RAW_LOG = f"{EVIDENCE_DIR}/phase5-grants-r8.raw.log"
STATIC_LOG = f"{EVIDENCE_DIR}/phase5-grants-r8.static.log"
LIVE_SCAN_MANIFEST = f"{EVIDENCE_DIR}/phase5-grants-r8.live-scan.sha256"
CHECKPOINT = f"{EVIDENCE_DIR}/PHASE5_1_GRANTS_SHADOW_R8_CHECKPOINT.md"
R7_ROOT = r7.TOP_MANIFEST
R7_ARTIFACT = r7.ARTIFACT_MANIFEST
...
NORMATIVE_SEEDS = r7.NORMATIVE_SEEDS
...
LIVE_FORBIDDEN_TOKENS = ("ONYX_GRANT_EVALUATOR", "SessionGrantShadowStore", "session_grants_v8")
...
_REQUIRED_PREFIX_COUNTS = {"test_released_identity_retries_only_identical_action_fingerprint[": 2}


class Phase5GrantR8EvidenceError(RuntimeError):
    pass
...
def _path(root: Path, relative: str) -> Path:
    ...
    if pure.is_absolute() or "." in pure.parts or ".." in pure.parts:
        raise Phase5GrantR8EvidenceError(f"unsafe evidence path: {relative}")
...
def derive_normative_graph(root: Path) -> dict[str, list[str]]:
    ...
    while queue:
        ...
        if not _path(root, relative).is_file():
            raise Phase5GrantR8EvidenceError(f"normative node missing: {relative}")
...
def local_import_closure(root: Path) -> tuple[str, ...]:
    try:
        return r7.r6.local_import_closure(root, SOURCE_SEEDS)
    except Exception as exc:
        raise Phase5GrantR8EvidenceError(str(exc)) from exc
...
def artifact_paths(root: Path) -> tuple[str, ...]:
    ...
    return tuple(sorted(set((
        *closure, *graph, R7_ARTIFACT, R7_ROOT, CHECKPOINT, BUNDLE, JUNIT, RAW_LOG,
...
def _parse_manifest(path: Path, expected: tuple[str, ...] | None = None):
    try:
        return r7._parse_manifest(path, expected)
    except Exception as exc:
        raise Phase5GrantR8EvidenceError(str(exc)) from exc
...
def _verify_entries(root: Path, entries) -> None:
    try:
        r7._verify_entries(root, entries)
    except Exception as exc:
        raise Phase5GrantR8EvidenceError(str(exc)) from exc
...
def _read_bundle(root: Path) -> dict[str, object]:
    ...
    try:
        ...
        raise Phase5GrantR8EvidenceError("bundle is not valid JSON") from exc
    canonical = (json.dumps(value, sort_keys=True, separators=(",", ":"), ensure_ascii=True) + "\n").encode()
    if raw != canonical or not isinstance(value, dict) or frozenset(value) != _BUNDLE_KEYS:
        raise Phase5GrantR8EvidenceError("bundle shape/serialization is noncanonical")
    return value
...
def _junit(root: Path) -> tuple[dict[str, int], str, set[str]]:
    try:
        ...
        raise Phase5GrantR8EvidenceError("JUnit is invalid") from exc
    suites = [document] if document.tag == "testsuite" else document.findall("testsuite")
    if len(suites) != 1:
        raise Phase5GrantR8EvidenceError("JUnit must contain exactly one suite")
    suite = suites[0]
    ...
    if actual != declared or not names or "" in names:
        raise Phase5GrantR8EvidenceError("JUnit declarations or names are invalid")
...
def _kv_log(root: Path, relative: str) -> dict[str, str]:
    ...
    if b"\r" in raw or not raw.endswith(b"\n"):
        raise Phase5GrantR8EvidenceError(f"log is noncanonical: {relative}")
    values: dict[str, str] = {}
    for line in raw.decode().splitlines():
        if not line or line.startswith("#"):
            continue
        if "=" not in line:
            raise Phase5GrantR8EvidenceError(f"log line is invalid: {relative}")
        key, value = line.split("=", 1)
        if not re.fullmatch(r"[a-z][a-z0-9_]*", key) or key in values:
            raise Phase5GrantR8EvidenceError(f"log key is invalid: {relative}")
        values[key] = value
    return values
        ...
        if isinstance(node, ast.ClassDef) and node.name == "SessionGrantShadowStore":
            for item in node.body:
                if isinstance(item, ast.FunctionDef) and item.name == name:
                    return item
    raise Phase5GrantR8EvidenceError(f"required method is missing: {name}")


def _validate_code_claims(root: Path) -> None:
    source = _path(root, "core/session_grants_v8.py").read_text(encoding="utf-8")
    ...
    if identity is None:
        raise Phase5GrantR8EvidenceError("external mutation identity function is missing")
    identity_literals = {node.value for node in ast.walk(identity) if isinstance(node, ast.Constant) and isinstance(node.value, str)}
    required_identity = {"session_id", "workspace_id", "provider", "provider_namespace", "account", "tool", "operation", "idempotency_key", "ExternalMutationIdentity.v8"}
    ...
    if not required_identity <= identity_literals or identity_literals & forbidden_identity:
        raise Phase5GrantR8EvidenceError("external mutation identity namespace is unsafe")
    ...
    if action_fingerprint is None:
        raise Phase5GrantR8EvidenceError("immutable external action fingerprint is missing")
    ...
    required_action = {"provider", "provider_namespace", "account", "operation", "tool", "target_identity", "payload_digest", "binding_digest", "action_contract_digest", "ImmutableExternalActionFingerprint.v8"}
    if not required_action <= action_literals:
        raise Phase5GrantR8EvidenceError("external action fingerprint omits material action context")
    required_services = {"verify_receipt", "reconcile", "resolve_outcome"}
    if not required_services <= _class_fields(tree, "HostServices"):
        raise Phase5GrantR8EvidenceError("pinned outcome/receipt/reconciliation services are incomplete")
    ...
    if not context <= uncertain or not context <= receipt:
        raise Phase5GrantR8EvidenceError("immutable mutation records are not self-sufficient")
    ...
    if not {"identity_digest", "action_fingerprint", "state", "reconciliation_count", "reconciliation_history_root", "latest_reconciliation_sequence"} <= ledger_fields:
        raise Phase5GrantR8EvidenceError("mutation ledger omits immutable binding or compact history")
    if not {"revision", "reconciliation_count", "reconciliation_history_root", "latest_reconciliation_sequence"} <= uncertain:
        raise Phase5GrantR8EvidenceError("uncertain record omits ABA revision history")
    ...
    if not {"_call_outcome_resolver", "_quarantine_reservation_locked", "_call_receipt_verifier", "_verification_is_exact"} <= outcome_calls:
        raise Phase5GrantR8EvidenceError("dispatch uncertainty path is incomplete")
    ...
    if not quarantine_lines or not snapshot_lines or min(quarantine_lines) >= min(snapshot_lines):
        raise Phase5GrantR8EvidenceError("returned outcome material is not quarantined before the fallible post-snapshot")
    ...
    if "_quarantine_reservation_locked" not in dispatch_calls:
        raise Phase5GrantR8EvidenceError("pre-dispatch uncertainty transition is missing")
    ...
    if not {"_call_reconciliation", "_verification_is_exact", "_receipt_replayed_locked", "_set_uncertain_ledger_state_locked", "_advance_reconciliation_locked"} <= reconcile_calls:
        raise Phase5GrantR8EvidenceError("reconciliation path is incomplete")
    reconcile_names = {node.id for node in ast.walk(reconcile) if isinstance(node, ast.Name)}
    if "MAX_RECONCILIATIONS" in reconcile_names or "grant_evaluator_enabled" not in reconcile_names:
        raise Phase5GrantR8EvidenceError("reconciliation is capacity-blocked or not strict-default-off")
    ...
    if not {"_roll_reconciliation_history", "replace"} <= advance_calls:
        raise Phase5GrantR8EvidenceError("reconciliation revision does not roll immutable history")
    for method_name in ("request_session_grant", "reserve", "mark_dispatch_attempted", "record_outcome", "reconcile_uncertain", "revoke", "kill", "mark_audit_unhealthy", "end_session"):
        method_names = {node.id for node in ast.walk(_method(tree, method_name)) if isinstance(node, ast.Name)}
        if "grant_evaluator_enabled" not in method_names:
            raise Phase5GrantR8EvidenceError(f"public operational method is not default-off: {method_name}")
    ...
    if "_enum" not in revoke_calls or '"reconciled-effect"' in source:
        raise Phase5GrantR8EvidenceError("revocation reasons are not closed and validated")
    ...
    if cleanup_attrs & {"_mutation_ledger", "_uncertain_records", "_receipts"}:
        raise Phase5GrantR8EvidenceError("grant cleanup erases durable mutation evidence")
    if "_global_generation" not in source or "_grant_generations" not in source:
        raise Phase5GrantR8EvidenceError("scoped generation barriers are missing")
    if "uncertain_needs_reconciliation" not in source or "consumed_verified" not in source:
        raise Phase5GrantR8EvidenceError("required mutation lifecycle states are missing")


def _constants(root: Path) -> dict[str, int]:
    tree = ast.parse(_path(root, "core/session_grants_v8.py").read_text(encoding="utf-8"))
    found: dict[str, int] = {}
    ...
    for node in tree.body:
        if isinstance(node, ast.Assign) and len(node.targets) == 1 and isinstance(node.targets[0], ast.Name) and node.targets[0].id in wanted:
            ...
            if type(value) is not int:
                raise Phase5GrantR8EvidenceError(f"limit is invalid: {node.targets[0].id}")
            found[node.targets[0].id] = value
    if set(found) != wanted:
        raise Phase5GrantR8EvidenceError("material limit set is incomplete")
    return {key: found[name] for key, name in _LIMIT_CONSTANTS.items()}


def _run_history(root: Path) -> dict[str, object]:
    try:
        history = r7._run_history(root)
        top = _parse_manifest(_path(root, R7_ROOT), (R7_ARTIFACT,))
        artifacts = _parse_manifest(_path(root, R7_ARTIFACT))
        _verify_entries(root, top + artifacts)
        result = r7.verify_evidence(root, current_base_commit=_current_commit(root), current_environment=_environment())
    except Exception as exc:
        raise Phase5GrantR8EvidenceError("V1-R7 historical verification failed") from exc
    expected = {"artifact_files": 38, "dependencies": 10, "live_scan": 69, "normative": 20, "root_files": 1, "tests": 21}
    if any(result.get(key) != value for key, value in expected.items()):
        raise Phase5GrantR8EvidenceError("V7 historical result is incomplete")
    history["v7"] = {
        "artifact_manifest": R7_ARTIFACT,
        "artifact_sha256": _sha(_path(root, R7_ARTIFACT)),
        "artifacts": len(artifacts),
        "root": R7_ROOT,
        "root_sha256": _sha(_path(root, R7_ROOT)),
    }
    return history
...
def _current_commit(root: Path) -> str:
    ...
    if result.returncode or not _HEX40.fullmatch(value):
        raise Phase5GrantR8EvidenceError("current git base commit is unavailable")
    return value
...
def _run_static_gates(root: Path, live_paths: tuple[str, ...]) -> dict[str, object]:
    ...
    for relative in compile_scope:
        try:
            ...
            raise Phase5GrantR8EvidenceError(f"live compile gate failed: {relative}") from exc
    ...
    if not ruff:
        raise Phase5GrantR8EvidenceError("live Ruff gate is unavailable")
    ...
    if ruff_result.returncode:
        raise Phase5GrantR8EvidenceError(f"live Ruff gate failed: {ruff_result.stdout}{ruff_result.stderr}")
    whitespace = subprocess.run([sys.executable, str(_path(root, "scripts/check_phase5_grants_r8_whitespace.py"))], cwd=root, capture_output=True, text=True, timeout=60, check=False, env={**os.environ, "PYTHONDONTWRITEBYTECODE": "1"})
    if whitespace.returncode or whitespace.stdout.strip() != "P51_GRANTS_R8_WHITESPACE_OK files=4":
        raise Phase5GrantR8EvidenceError("live whitespace gate failed")
    for relative in live_paths:
        ...
        if any(token in text for token in LIVE_FORBIDDEN_TOKENS):
            raise Phase5GrantR8EvidenceError(f"R8 is wired into live surface: {relative}")
    return {
        "compile": 0,
...
def _require_equal(claim: object, actual: object, label: str) -> None:
    if claim != actual:
        raise Phase5GrantR8EvidenceError(f"{label} binding is false or incomplete")
...
def verify_evidence(root: Path = PROJECT, *, current_base_commit: str | None = None, current_environment: dict[str, str] | None = None) -> dict[str, object]:
    ...
    if bundle.get("contract") != "Phase5SessionGrantShadowEvidence.v8" or bundle.get("status") != "candidate-default-off-not-accepted":
        raise Phase5GrantR8EvidenceError("bundle contract/status is invalid")
    ...
    if not isinstance(files, dict) or tuple(sorted(files)) != tuple(sorted(leaf_paths)):
        raise Phase5GrantR8EvidenceError("bundle leaf set is not exact")
    for relative, digest in files.items():
        if type(digest) is not str or not _HEX64.fullmatch(digest) or _sha(_path(root, relative)) != digest:
            raise Phase5GrantR8EvidenceError(f"bundle leaf hash mismatch: {relative}")
    closure = local_import_closure(root)
    _require_equal(bundle.get("dependency_closure"), list(closure), "dependency closure")
    graph = derive_normative_graph(root)
    if "readme.md" not in graph:
        raise Phase5GrantR8EvidenceError("bare root readme normative fallback was not resolved recursively")
    ...
    if counts["failed"] or counts["errors"] or not _REQUIRED_TESTS <= names:
        raise Phase5GrantR8EvidenceError("focused semantic JUnit evidence is incomplete")
    for prefix, expected in _REQUIRED_PREFIX_COUNTS.items():
        if sum(name.startswith(prefix) for name in names) != expected:
            raise Phase5GrantR8EvidenceError(f"focused parametrized JUnit evidence differs: {prefix}")
    ...
    if raw.get("timestamp") != junit_timestamp or raw.get("exit_code") != "0" or raw.get("result") != f"{counts['passed']} passed" or any(raw.get(key) != str(counts[key]) for key in ("passed", "failed", "errors", "skipped")):
        raise Phase5GrantR8EvidenceError("raw/JUnit evidence differs")
    static = _kv_log(root, STATIC_LOG)
    expected_static = {
        "compile_mode": "recursive-live-in-memory-built-in-compile",
        "compile_exit_code": "0",
        "diff_check_command": "python scripts/check_phase5_grants_r8_whitespace.py",
    ...
    if not _HEX40.fullmatch(base) or bundle.get("base_commit") != base:
        raise Phase5GrantR8EvidenceError("bundle base commit is invalid or stale")
    ...
    if type(timestamp) is not str or timestamp != junit_timestamp:
        raise Phase5GrantR8EvidenceError("bundle timestamp differs from JUnit")
    try:
        parsed = datetime.fromisoformat(timestamp)
    except ValueError as exc:
        raise Phase5GrantR8EvidenceError("bundle timestamp is not ISO-8601") from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise Phase5GrantR8EvidenceError("bundle timestamp lacks timezone")
    return {
...
def main() -> int:
    ...
    print(
        f"P51_GRANTS_R8_EVIDENCE_OK root={result['root_files']} artifacts={result['artifact_files']} "
