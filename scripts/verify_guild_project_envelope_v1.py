...
from core import guild_project_envelope_v1 as candidate  # noqa: E402
from scripts import (  # noqa: E402
    verify_guild_execution_intent_v1_acceptance as guild_intent,
)

BASE = PROJECT / "docs/onyx/checkpoints/guild-project-envelope-v1"
MANIFEST = BASE / "manifest.json"
SELECTION = BASE / "cumulative-selection.json"
MARKER = "GUILD_PROJECT_ENVELOPE_V1_OK"
ACCEPTED_GUILD_INTENT_ROOT = (
    "68a4fc775dbdccb0741c0bf69aa5ff786966f4f828f2220ac35ec21efde64e83"
)
ARTIFACT_PATHS = (
    "core/guild_project_envelope_v1.py",
    "tests/test_guild_project_envelope_v1.py",
    "scripts/verify_guild_project_envelope_v1.py",
    "docs/onyx/adrs/ADR-0060-guild-project-envelope-v1.md",
    "docs/onyx/research/GUILD_PROJECT_ENVELOPE_V1_SOURCES.md",
    "docs/onyx/checkpoints/guild-project-envelope-v1/"
    "GUILD_PROJECT_ENVELOPE_V1_CHECKPOINT.md",
    "docs/onyx/checkpoints/guild-project-envelope-v1/CAPABILITY_DELTA_SNAPSHOT.md",
    "docs/onyx/checkpoints/guild-project-envelope-v1/cumulative-selection.json",
)
...
def verify(*, run_tests: bool = True) -> dict[str, object]:
    ...
    if (
        manifest.get("schema") != "onyx.guild.project-envelope.v1"
        or manifest.get("candidate") != "guild-project-envelope-v1"
    ...
    intent_record = guild_intent.verify(run_tests=False)
    if (
        intent_record.get("decision") != "accepted"
        or intent_record.get("candidate_artifact_root_sha256")
        != ACCEPTED_GUILD_INTENT_ROOT
    ):
        raise VerificationError("accepted execution-intent entry drift")

    if (
        candidate.GuildProjectEnvelopeFeatureGateV1.from_environ({}).enabled
        is not False
    ):
        raise VerificationError("feature gate is not default-off")
    ledger = candidate.create_guild_project_ledger_v1(PROJECT)
    if ledger.is_active("verification-probe") is not False:
        raise VerificationError("project activity is not structurally false")
    try:
        candidate.create_guild_project_ledger_v1(PROJECT / "missing")
    except candidate.GuildProjectEnvelopeV1Denied:
        pass
    else:
        raise VerificationError("entry-bind did not deny missing evidence")

    # The owner's termination rule must remain a closed, single-outcome
    # vocabulary in V1: no softer fate may become representable by drift.
    if candidate.DECOMMISSION_OUTCOMES != ("revoke_and_decommission",):
        raise VerificationError("decommission outcome vocabulary drift")
    if candidate.MIN_BUDGET_MICRO < 1:
        raise VerificationError("budget floor drift")
    ...
    source = (PROJECT / "core/guild_project_envelope_v1.py").read_text(
        encoding="utf-8"
    )
    for required in (
    ...
    for forbidden in (
        ...
        "def activate",
        "def spend",
        # Executable decommission only. `decommission_outcome` is a pure
        # read of the declared policy and must stay allowed, so the token is
        # anchored on the call form.
        "def decommission(",
        "def schedule",
    ):
    ...
    if run_tests:
        temporary = (
            PROJECT
            / "runtime/test-tmp/guild-project-envelope-v1-verify"
    ...
    return {
        "candidate": "guild-project-envelope-v1",
