...
CHECKPOINT = ROOT / "docs/onyx/checkpoints/phase5-approval-inbox-v10"
ARTIFACTS = ROOT / "docs/onyx/VE-ARTIFACTS-P52-APPROVAL-INBOX-V10-001.sha256"
SOURCE = ROOT / "docs/onyx/VE-SOURCE-P52-APPROVAL-INBOX-V10-001.sha256"
FOCUSED = ("tests/test_approval_inbox_v10.py",)
COMBINED = tuple(f"tests/test_approval_inbox_v{version}.py" for version in range(1, 11))
REGRESSIONS = (
...
def main() -> int:
    ...
    focused_junit = CHECKPOINT / "phase5-approval-inbox-v10.junit.xml"
    combined_junit = CHECKPOINT / "phase5-approval-inbox-v10.combined.junit.xml"
    regressions_junit = CHECKPOINT / "phase5-approval-inbox-v10.regressions.junit.xml"
    ...
    combined_output, combined_seconds = run(
        [
            ...
            *COMBINED,
            ...
            "no:cacheprovider",
            "-p",
            "scripts.phase5_approval_inbox_v10_historical_projection",
    ...
    write(CHECKPOINT / "phase5-approval-inbox-v10.raw.log", focused_output)
    write(CHECKPOINT / "phase5-approval-inbox-v10.combined.log", combined_output)
    write(CHECKPOINT / "phase5-approval-inbox-v10.regressions.log", regression_output)
    ...
    static_commands = (
        ...
        [python, "scripts/check_phase5_approval_inbox_v10_whitespace.py"],
        [
            "git",
            "diff",
            "--check",
    ...
    write(
        CHECKPOINT / "phase5-approval-inbox-v10.static.log",
        "\n\n".join(static_lines) + "\n",
    )
    ...
    write(
        CHECKPOINT / "phase5-approval-inbox-v10.live-scan.sha256",
    ...
    checkpoint_text = f"""# Phase 5.2 Approval Inbox V10 checkpoint

Status: **candidate — default-off, read-only, external acceptance pending**

V10 is a clean-room review projection. It preserves V1-V9 as rejected historical
candidates and does not import them, Session Grants or Capability Nexus. The
host owns a bootstrap-only `ONYX_APPROVAL_INBOX_V10` gate, flag epoch, exact
source and monotonic clock. No live/startup/UI/dashboard path imports V10.
...
- Combined Approval Inbox V1-V10: {combined["passed"]} passed in {combined_seconds:.3f}s.
- Frozen V2-V7 R11 fixtures resolve their two mutable documentation paths from
  exact V9 content-addressed snapshots; no predecessor or live projection is edited.
...
- Live reachability scan: {len(live_entries)} files, no V10 flag/module reference.
- Parent and worker authoritative paths reject noncanonical spelling, case aliases, containment escapes and root/ancestor/final symlink, junction or reparse components before each read, hash or process launch; live cardinality is fixed at {len(live_entries)}.
...
V10 remains unaccepted and default-off until three independent external reviews
pass. This checkpoint grants no Phase 5 exit or live activation.
"""
    checkpoint_md = CHECKPOINT / "PHASE5_2_APPROVAL_INBOX_V10_CHECKPOINT.md"
    ...
    v10_files = [
        ...
        "docs/onyx/checkpoints/phase5-approval-inbox-v10/phase5-approval-inbox-v10.raw.log",
        "docs/onyx/checkpoints/phase5-approval-inbox-v10/phase5-approval-inbox-v10.combined.log",
        "docs/onyx/checkpoints/phase5-approval-inbox-v10/phase5-approval-inbox-v10.regressions.log",
        "docs/onyx/checkpoints/phase5-approval-inbox-v10/phase5-approval-inbox-v10.static.log",
        "docs/onyx/checkpoints/phase5-approval-inbox-v10/phase5-approval-inbox-v10.live-scan.sha256",
        "docs/onyx/checkpoints/phase5-approval-inbox-v10/mutable-projections.json",
        *(entry["snapshot"] for entry in projections),
    ]
    files = sorted(set(historical + v10_files))
    ...
    bundle = {
        ...
        "claims": {
            ...
            "no_live_wiring": True,
            "historical_v1_v9_preserved": True,
    ...
    bundle_path = CHECKPOINT / "phase5-approval-inbox-v10.bundle.json"
