"""Reproduce the Phase 8 Microsoft Graph OneDrive Read V1 candidate."""
...
from core import phase8_microsoft_graph_drive_v1 as candidate  # noqa: E402
from scripts import (  # noqa: E402
    verify_phase8_microsoft_graph_tasks_v1_acceptance as phase8_tasks,
)

MANIFEST = (
    PROJECT / "docs/onyx/checkpoints/phase8-microsoft-graph-drive-v1/manifest.json"
)
SELECTION = (
    PROJECT / "docs/onyx/checkpoints/phase8-microsoft-graph-drive-v1/"
    "cumulative-selection.json"
)
MARKER = "P8_MICROSOFT_GRAPH_DRIVE_V1_OK"
ARTIFACT_PATHS = (
    "core/phase8_microsoft_graph_drive_v1.py",
    "tests/test_phase8_microsoft_graph_drive_v1.py",
    "scripts/verify_phase8_microsoft_graph_drive_v1.py",
    "docs/onyx/adrs/ADR-0043-phase8-microsoft-graph-drive-v1.md",
    "docs/onyx/research/PHASE8_MICROSOFT_GRAPH_DRIVE_V1_SOURCES.md",
    "docs/onyx/checkpoints/phase8-microsoft-graph-drive-v1/"
    "PHASE8_MICROSOFT_GRAPH_DRIVE_V1_CHECKPOINT.md",
    "docs/onyx/checkpoints/phase8-microsoft-graph-drive-v1/"
...
def verify(*, run_tests: bool = True) -> dict[str, object]:
    ...
    if (
        manifest.get("schema") != "onyx.phase8.microsoft-graph-drive.v1"
        or manifest.get("candidate") != "phase8-microsoft-graph-drive-v1"
    ...
    tasks_result = phase8_tasks.verify(run_tests=False)
    if (
        tasks_result.get("decision") != "accepted"
        or tasks_result.get("candidate_artifact_root_sha256")
        != "984f32cf236bac4b9ef1c4ad0a63bcf9a1384c19a0fdc8feae5720d0334f6508"
    ):
        raise VerificationError("accepted Tasks V1 entry drift")
    candidate._verify_entry(PROJECT)
    if (
        candidate.create_microsoft_graph_drive_v1(
            gate=candidate.GraphDriveFeatureGateV1(False),
    ...
    source = (PROJECT / "core/phase8_microsoft_graph_drive_v1.py").read_text(
        encoding="utf-8"
    )
    for required in (
        'FEATURE_FLAG: Final = "ONYX_PHASE8_MICROSOFT_GRAPH_DRIVE_V1"',
        "cross-route nextLink denied",
        "files read scope was not granted",
        "def list_root",
        "def get_item",
    ):
    ...
    if run_tests:
        temporary = (
            PROJECT
            / "runtime/test-tmp/phase8-microsoft-graph-drive-v1-verify"
