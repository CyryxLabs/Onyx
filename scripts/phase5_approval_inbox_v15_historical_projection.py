...
V15 never reads either mutable live documentation path while materializing the
...
_GIT_VERSION = "git version 2.51.0.windows.1"
_RUFF_VERSION = "ruff 0.15.21"


class HistoricalProjectionPathError(RuntimeError):
    """A historical authority or output path crossed the V15 boundary."""
...
def _load_exact_authoritative_module(
    ...
    private_name = f"_onyx_p52_v15_{label}_{secrets.token_hex(16)}"
...
def _completed(
    ...
    return _stdlib_subprocess.CompletedProcess(command, returncode, stdout, stderr)


def _private_head(root: Path) -> str:
    absolute, _ = _safe_root(root)
    if absolute == ROOT.absolute():
        # Historical semantic tests ask for their creation-time base while
        # executing at the authoritative V15 root. Return the frozen authority
        # directly; never consult the live repository metadata.
        return FROZEN_BASE_COMMIT
    try:
        absolute.relative_to(ROOT.absolute())
    except ValueError as error:
        raise HistoricalProjectionPathError(
            "hermetic-root-outside-workspace"
        ) from error
    head = _safe_existing(absolute, ".git/HEAD").read_text(encoding="utf-8")
    if head not in {
        "ref: refs/heads/onyx-v13\n",
        "ref: refs/heads/onyx-v14\n",
        "ref: refs/heads/onyx-v15\n",
    }:
        raise HistoricalProjectionPathError("hermetic-head-drift")
    reference = head.removeprefix("ref: ").strip()
    value = _safe_existing(absolute, f".git/{reference}").read_text(encoding="utf-8")
    if value != FROZEN_BASE_COMMIT + "\n":
        raise HistoricalProjectionPathError("hermetic-ref-drift")
    return FROZEN_BASE_COMMIT


def _is_materialized_authority(root: Path) -> bool:
    try:
        marker = _safe_existing(root, "_onyx_v15_materialized_manifest.json")
        payload = __import__("json").loads(marker.read_text(encoding="utf-8"))
    except (OSError, ValueError, HistoricalProjectionPathError):
        return False
    return payload.get("contract") == "OnyxMaterializedAuthority.v15"
...
def _internal_git(
    ...
    *,
    ...
    if any(
        ...
        raise HistoricalProjectionPathError("git-override-argument")
    ...
    if arguments == ["--version"]:
        return _completed(command, 0, _GIT_VERSION + "\n")
    if arguments == ["rev-parse", "HEAD"]:
        absolute, _ = _safe_root(root)
        if absolute != ROOT.absolute():
            checker = owned_checker or _require_owned_root
            checker(absolute)  # type: ignore[operator]
        elif not _is_materialized_authority(absolute):
            raise HistoricalProjectionPathError("git-root-not-materialized")
        return _completed(command, 0, _private_head(root) + "\n")
    if len(arguments) >= 3 and arguments[:3] == ["diff", "--check", "--"]:
        absolute, _ = _safe_root(root)
        if absolute != ROOT.absolute():
            checker = owned_checker or _require_owned_root
            checker(root)  # type: ignore[operator]
        elif not _is_materialized_authority(absolute):
            raise HistoricalProjectionPathError("git-root-not-materialized")
        supplied = tuple(arguments[3:])
        if supplied != source_seeds:
            raise HistoricalProjectionPathError("git-diff-source-set-drift")
        for relative in supplied:
            data = _safe_existing(absolute, relative).read_bytes()
            if hashlib.sha256(data).hexdigest() != source_hashes.get(relative):
                raise HistoricalProjectionPathError(
                    f"git-diff-source-drift:{relative}"
                )
            if b"\r" in data or any(
                line.endswith((b" ", b"\t")) for line in data.splitlines()
            ):
                return _completed(command, 2, stderr=f"whitespace:{relative}\n")
        return _completed(command, 0)
    raise HistoricalProjectionPathError("git-command-not-authorized")


def _validated_system32_executable(name: str) -> Path:
    if os.name != "nt" or name.casefold() not in {"cmd.exe", "taskkill.exe"}:
        raise HistoricalProjectionPathError("system32-command-not-authorized")
    buffer = ctypes.create_unicode_buffer(32768)
    length = ctypes.windll.kernel32.GetWindowsDirectoryW(buffer, len(buffer))
    if not length or length >= len(buffer):
        raise HistoricalProjectionPathError("windows-root-unavailable")
    return _safe_existing(Path(buffer.value), f"System32/{name}")


class _HermeticSubprocessProxy:
    """Delegate non-Git tools with a clean environment; emulate Git internally."""

    CompletedProcess = _stdlib_subprocess.CompletedProcess
    PIPE = _stdlib_subprocess.PIPE
    STDOUT = _stdlib_subprocess.STDOUT
    DEVNULL = _stdlib_subprocess.DEVNULL
    ...
    def __init__(
        ...
        *,
        ...
        seeds = tuple(getattr(module, "SOURCE_SEEDS", ()))
        self._source_seeds = seeds
        self._source_hashes = {
            relative: hashlib.sha256(_safe_existing(ROOT, relative).read_bytes()).hexdigest()
            for relative in seeds
        }
        self._owned_checker = getattr(owner_module, "_require_owned_root", None)

    def run(self, command: object, *args: object, **kwargs: object) -> object:
        if not isinstance(command, (list, tuple)) or not command:
            raise HistoricalProjectionPathError("noncanonical-process-command")
        ...
        if executable in {"git", "git.exe"}:
            return _internal_git(
                normalized,
                cwd,
                source_seeds=self._source_seeds,
                source_hashes=self._source_hashes,
                owned_checker=self._owned_checker,
            )
        if (
            Path(normalized[0]).absolute() == Path(sys.executable).absolute()
            and normalized[1:] == ["-m", "ruff", "--version"]
        ):
            return _completed(normalized, 0, _RUFF_VERSION + "\n")
        if (
            os.name == "nt"
            and len(normalized) == 6
            and executable in {"cmd", "cmd.exe"}
            and [value.casefold() for value in normalized[1:4]]
            == ["/c", "mklink", "/j"]
        ):
            link = Path(normalized[4]).absolute()
            target = Path(normalized[5]).absolute()
            root, _ = _safe_root(ROOT)
            try:
                link_parent = link.parent.resolve(strict=True)
                target_resolved = target.resolve(strict=True)
                inside_root = (
                    link_parent == root or link_parent.is_relative_to(root)
                ) and target_resolved.is_relative_to(root)
                sibling_fixture = (
                    link_parent == target_resolved.parent
                    and link_parent.parent == root.parent
                    and link_parent.name.startswith("tmp")
                )
                if not inside_root and not sibling_fixture:
                    raise ValueError("outside authorized fixture")
                if link.exists() or link.is_symlink() or not target_resolved.is_dir():
                    raise ValueError("invalid junction fixture")
            except (OSError, ValueError) as error:
                raise HistoricalProjectionPathError("junction-outside-materialized-root") from error
            cmd = _validated_system32_executable("cmd.exe")
            if args:
                raise HistoricalProjectionPathError("process-positional-options-not-authorized")
            allowed_kwargs = {
                "capture_output",
                "check",
                "env",
            ...
            if set(kwargs) - allowed_kwargs:
                raise HistoricalProjectionPathError("process-options-not-authorized")
            if kwargs.get("check", False) is not False:
                raise HistoricalProjectionPathError("process-check-option-not-authorized")
            text_mode = kwargs.get("text", False)
            if type(text_mode) is not bool:
                raise HistoricalProjectionPathError("process-text-option-not-authorized")
            capture_output = kwargs.get("capture_output", False)
            if type(capture_output) is not bool:
                raise HistoricalProjectionPathError("process-capture-option-not-authorized")
            stdout = kwargs.get("stdout")
            stderr = kwargs.get("stderr")
            if capture_output and (stdout is not None or stderr is not None):
                raise HistoricalProjectionPathError("process-output-options-conflict")
            if (
                stdout is not None
                and stdout is not _stdlib_subprocess.PIPE
            ) or (
                stderr is not None
                and stderr is not _stdlib_subprocess.STDOUT
            ):
                raise HistoricalProjectionPathError("process-output-option-not-authorized")
            supplied = kwargs.get("env")
            if supplied is not None and type(supplied) is not dict:
                raise HistoricalProjectionPathError("noncanonical-process-environment")
            run_options: dict[str, object] = {
                "check": False,
                "env": _sanitized_environment(supplied),
                "timeout": 10,
            }
            if text_mode:
            ...
            if capture_output:
                ...
                if stdout is not None:
                    run_options["stdout"] = stdout
                if stderr is not None:
                    run_options["stderr"] = stderr
            return _stdlib_subprocess.run(
                [str(cmd), *normalized[1:]], **run_options
            )
        raise HistoricalProjectionPathError("process-command-not-authorized")
...
def _materialize_r14(module: ModuleType, destination: Path) -> tuple[str, ...]:
    ...
    _write_new(root, ".git/HEAD", b"ref: refs/heads/onyx-v15\n")
    _write_new(root, ".git/refs/heads/onyx-v15", FROZEN_BASE_COMMIT.encode() + b"\n")
    _write_new(
        root,
        ".git/config",
...
def _install_projection_wrapper(module: ModuleType) -> None:
    ...
    if getattr(original, "_onyx_v15_frozen_projections", False):
        return

    @functools.wraps(original)
    def materialize_r11(destination: Path) -> tuple[str, ...]:
        return _materialize_r14(module, Path(destination))

    materialize_r11._onyx_v15_frozen_projections = True  # type: ignore[attr-defined]
    ...
    module.subprocess = _HermeticSubprocessProxy(module)
...
def pytest_collection_modifyitems(items: list[object]) -> None:
    ...
    for item in items:
        ...
        if path.name in {
            "test_approval_inbox_v13.py",
            "test_approval_inbox_v14.py",
            "test_approval_inbox_v15.py",
        }:
            test_module = getattr(item, "module", None)
            if test_module is not None:
                conventional = getattr(test_module, "historical", None)
                test_module.subprocess = _HermeticSubprocessProxy(
                    owner_module=conventional
                )
