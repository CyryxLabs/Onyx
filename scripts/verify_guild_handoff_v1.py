"""Reproduce the Guild Handoff V1 candidate."""
...
from core import guild_handoff_v1 as candidate  # noqa: E402
from scripts import verify_guild_profiles_v1_acceptance as guild_profiles  # noqa: E402

BASE = PROJECT / "docs/onyx/checkpoints/guild-handoff-v1"
MANIFEST = BASE / "manifest.json"
SELECTION = BASE / "cumulative-selection.json"
MARKER = "GUILD_HANDOFF_V1_OK"
ACCEPTED_GUILD_PROFILES_ROOT = (
    "818196ac6bd2048bbfe5218d0e576b392b07aa1005816b6f4b867f3ee9bbc09d"
)
ARTIFACT_PATHS = (
    "core/guild_handoff_v1.py",
    "tests/test_guild_handoff_v1.py",
    "scripts/verify_guild_handoff_v1.py",
    "docs/onyx/adrs/ADR-0055-guild-handoff-v1.md",
    "docs/onyx/research/GUILD_HANDOFF_V1_SOURCES.md",
    "docs/onyx/checkpoints/guild-handoff-v1/GUILD_HANDOFF_V1_CHECKPOINT.md",
    "docs/onyx/checkpoints/guild-handoff-v1/CAPABILITY_DELTA_SNAPSHOT.md",
    "docs/onyx/checkpoints/guild-handoff-v1/cumulative-selection.json",
)
...
def verify(*, run_tests: bool = True) -> dict[str, object]:
    ...
    if (
        manifest.get("schema") != "onyx.guild.handoff.v1"
        or manifest.get("candidate") != "guild-handoff-v1"
    ...
    profiles_record = guild_profiles.verify(run_tests=False)
    if (
        profiles_record.get("decision") != "accepted"
        or profiles_record.get("candidate_artifact_root_sha256")
        != ACCEPTED_GUILD_PROFILES_ROOT
    ):
        raise VerificationError("accepted guild-profiles entry drift")

    if candidate.GuildHandoffFeatureGateV1.from_environ({}).enabled is not False:
        raise VerificationError("feature gate is not default-off")
    candidate._verify_entry(PROJECT)
    try:
        candidate.create_guild_story_ledger_v1(PROJECT / "missing")
    except candidate.GuildHandoffV1Denied:
        pass
    else:
        raise VerificationError("entry-bind did not deny missing evidence")

    if ("in_review", "in_progress") not in candidate.ALLOWED_TRANSITIONS:
        raise VerificationError("QA loop edge is missing")
    if ("draft", "done") in candidate.ALLOWED_TRANSITIONS:
        raise VerificationError("lifecycle allows a skipped path")
    if candidate.MAX_QA_REJECTS != 5:
        raise VerificationError("QA reject cap drift")
    ...
    source = (PROJECT / "core/guild_handoff_v1.py").read_text(encoding="utf-8")
    for required in (
    ...
    if run_tests:
        temporary = (
            PROJECT
            / "runtime/test-tmp/guild-handoff-v1-verify"
