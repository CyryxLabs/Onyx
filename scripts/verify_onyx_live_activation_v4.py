"""Frozen bundle verifier for Onyx Live Activation V4."""
...
V4 = frozenset(
    {
        ...
        "docs/onyx/checkpoints/onyx-live-activation-v4/ONYX_LIVE_ACTIVATION_V4_CHECKPOINT.md",
    }
)
HOST = frozenset(
    {
...
def fail(message: str) -> None:
    raise SystemExit("ONYX_LIVE_ACTIVATION_V4_VERIFY_FAIL: " + message)
...
def main() -> None:
    ...
    if type(payload) is not dict or payload.get("schema") != "onyx.live-activation.v4":
    ...
    if frozenset(indexed) != V1 | V2 | V3 | V4 | HOST | COMPONENTS | E6:
        fail("closure mismatch")
    if any(indexed[path]["role"] != "rejected-v1" for path in V1):
        fail("V1 rejection role")
    if any(indexed[path]["role"] != "rejected-v2" for path in V2):
        fail("V2 rejection role")
    if any(indexed[path]["role"] != "rejected-v3" for path in V3):
        fail("V3 rejection role")
    if any(indexed[path]["role"] != "accepted-e6-record" for path in E6):
        fail("E6 role")

    core = (ROOT / "core" / "onyx_live_activation_v4.py").read_text(
        encoding="utf-8"
    )
    launcher = (ROOT / "scripts" / "launch_onyx_live_v4.pyw").read_text(
        encoding="utf-8"
    )
    host_gate = (
        ROOT / "scripts" / "verify_onyx_live_activation_v4_host.py"
    ).read_text(encoding="utf-8")
    qt_gate = (
        ROOT / "scripts" / "verify_onyx_live_activation_v4_qt.py"
    ).read_text(encoding="utf-8")
    if any(
        token in core
        for token in (
            "onyx_live_activation_v1",
            "onyx_live_activation_v2",
            "onyx_live_activation_v3",
        )
    ):
        fail("V4 reaches rejected candidate")
    ...
    for token in (
        ...
        if token not in core:
            fail("Qt transaction contract: " + token)
    if "value != \"1\"" not in launcher:
        fail("canonical truth comparison")
    refusal = launcher.find("ONYX_LIVE_V4_PREIMPORT_REFUSAL")
    ...
    if "range(1, 12)" not in host_gate or "range(1, 65)" not in host_gate:
        fail("host failpoint/reconnect evidence")
    for token in (
        "QT_QPA_PLATFORM",
        "set/correct/forget",
        'failure_scenario(app, "noop")',
        'failure_scenario(app, "wrong")',
        'failure_scenario(app, "getter_throws")',
        '"rollback_noop"',
        'failure_scenario(app, "destroy")',
        "direct64_ms=",
    ):
        if token not in qt_gate:
            fail("offscreen Qt evidence: " + token)
    if any(token in core for token in ("requests.", "httpx.", "socket.", "google.genai")):
        fail("network/provider dependency")

    tree = ast.parse(core, filename="core/onyx_live_activation_v4.py")
    install = next(
        node
    ...
    if len(seam_patches) != 11:
        fail("transactional seam count")
    activate = next(
        node
    ...
    print("ONYX_LIVE_ACTIVATION_V4_OK")
