...
V11 never reads either mutable live documentation path while materializing the
...
class HistoricalProjectionPathError(RuntimeError):
    """A historical authority or output path crossed the V11 boundary."""
...
def _materialize_r11(module: ModuleType, destination: Path) -> tuple[str, ...]:
    ...
    git_data = f"gitdir: {(ROOT / '.git').resolve(strict=True).as_posix()}\n".encode()
...
def _install_projection_wrapper(module: ModuleType) -> None:
    ...
    if getattr(original, "_onyx_v11_frozen_projections", False):
        return

    @functools.wraps(original)
    def materialize_r11(destination: Path) -> tuple[str, ...]:
        return _materialize_r11(module, Path(destination))

    materialize_r11._onyx_v11_frozen_projections = True  # type: ignore[attr-defined]
