"""Fresh-process real-host non-GUI gate for Onyx Live Activation V3."""
...
def main_gate():
    ...
    for failpoint in range(1, 11):
        failed = live.OnyxLiveActivationV3(
    ...
    controller = live.OnyxLiveActivationV3(
        live.ActivationFlagsV3.from_canonical_environ(),
        contract,
        authority_factory=factory,
    )
    controller.install()
    assert calls == []
    assert controller.start() is live.ActivationV3State.READY
    ...
    assert seams(contract) == baseline
    print("ONYX_LIVE_ACTIVATION_V3_REAL_HOST_OK")
    print("seam_failpoints=10 reconnects=64 catalog_reads=64 session_trace_ids=128")
