"""Reproduce the Phase 8 Microsoft Graph Read V1 candidate."""
...
from core import phase8_microsoft_graph_read_v1 as candidate  # noqa: E402
from scripts import verify_phase7_exit_candidate_v1_acceptance as phase7  # noqa: E402

MANIFEST = (
    PROJECT / "docs/onyx/checkpoints/phase8-microsoft-graph-read-v1/manifest.json"
)
SELECTION = (
    PROJECT / "docs/onyx/checkpoints/phase8-microsoft-graph-read-v1/"
    "cumulative-selection.json"
)
MARKER = "P8_MICROSOFT_GRAPH_READ_V1_OK"
ARTIFACT_PATHS = (
    "core/phase8_microsoft_graph_read_v1.py",
    "tests/test_phase8_microsoft_graph_read_v1.py",
    "scripts/verify_phase8_microsoft_graph_read_v1.py",
    "docs/onyx/adrs/ADR-0036-phase8-microsoft-graph-read-v1.md",
    "docs/onyx/research/PHASE8_MICROSOFT_GRAPH_READ_V1_SOURCES.md",
    "docs/onyx/checkpoints/phase8-microsoft-graph-read-v1/"
    "PHASE8_MICROSOFT_GRAPH_READ_V1_CHECKPOINT.md",
    "docs/onyx/checkpoints/phase8-microsoft-graph-read-v1/"
    "CAPABILITY_DELTA_SNAPSHOT.md",
    "docs/onyx/checkpoints/phase8-microsoft-graph-read-v1/" "cumulative-selection.json",
)
...
def verify(*, run_tests: bool = True) -> dict[str, object]:
    ...
    if (
        manifest.get("schema") != "onyx.phase8.microsoft-graph-read.v1"
        or manifest.get("candidate") != "phase8-microsoft-graph-read-v1"
    ...
    phase7_result = phase7.verify(run_tests=False)
    if (
        phase7_result.get("decision") != "accepted"
        or phase7_result.get("candidate_artifact_root_sha256")
        != "0512b1af1ea38392ca62b37933404119c80b173a3d6e8e059d0e0f5418a7bfcb"
    ):
        raise VerificationError("accepted Phase 7 entry drift")
    candidate._verify_entry(PROJECT)
    if (
        candidate.create_microsoft_graph_read_adapter_v1(
            gate=candidate.GraphReadFeatureGateV1(False),
    ...
    source = (PROJECT / "core/phase8_microsoft_graph_read_v1.py").read_text(
        encoding="utf-8"
    )
    for required in (
        ...
        "mail body scope unavailable",
        "cross-route nextLink denied",
    ):
    ...
    if run_tests:
        temporary = PROJECT / "runtime/test-tmp/phase8-microsoft-graph-read-v1-verify"
