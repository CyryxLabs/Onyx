...
Item {
    ...
    readonly property color signal: uiProjection.animationRunning ? "#19C7C0" : "#8C949E"
    ...
    Column {
        ...
        Text {
            anchors.right: parent.right; text: root.uiProjection.stateLabel
    ...
    Column {
        ...
        Text {
            text: root.uiProjection.contentVisible ? root.uiProjection.contentTitle : root.uiProjection.transcriptTitle
        ...
        Text {
            text: root.uiProjection.contentVisible ? root.uiProjection.contentText : root.uiProjection.transcriptText
        ...
        Text {
            text: root.uiProjection.logText; width: parent.width
        ...
        Row {
            spacing: 8
            OrbAction { label: "HISTORY"; accessibleName: "Show Onyx activity history"; onTriggered: root.uiProjection.requestHistory() }
            OrbAction { label: "ACCESS"; accessibleName: "Show Onyx permission controls"; onTriggered: root.uiProjection.requestPermissions() }
        }
    }
    ...
    Text {
        ...
        y: root.height * .63
        text: root.uiProjection.actionStatus === "READY" ? root.uiProjection.stateLabel : root.uiProjection.actionStatus
    ...
    Item {
        ...
        TextInput {
            ...
            onAccepted: if (root.uiProjection.submitCommand(text)) text = ""
        }
        OrbAction {
            ...
            onTriggered: if (root.uiProjection.submitCommand(commandInput.text)) commandInput.text = ""
        }
        Flow {
            anchors.left: parent.left; anchors.right: parent.right; anchors.bottom: parent.bottom
            spacing: root.compact ? 5 : 8
            layoutDirection: Qt.LeftToRight
            OrbAction { label: "FILE"; accessibleName: "Attach a file"; onTriggered: root.uiProjection.requestFile() }
            OrbAction { label: "INTERRUPT"; accessibleName: "Interrupt Onyx"; onTriggered: root.uiProjection.requestInterrupt() }
            OrbAction { label: root.uiProjection.muted ? "UNMUTE" : "MUTE"; accessibleName: "Toggle microphone"; onTriggered: root.uiProjection.requestMuteToggle() }
            OrbAction { label: root.uiProjection.autonomyEnabled ? "AUTONOMY ON" : "AUTONOMY"; emphasized: root.uiProjection.autonomyEnabled; accessibleName: "Toggle owner autonomy"; onTriggered: root.uiProjection.requestAutonomy() }
            OrbAction { label: "REMOTE"; accessibleName: "Open remote access"; onTriggered: root.uiProjection.requestRemote() }
            OrbAction { label: root.uiProjection.cameraActive ? "STOP CAM" : "CAMERA"; accessibleName: "Toggle camera feed"; onTriggered: root.uiProjection.requestCamera() }
            OrbAction { label: "SETUP"; accessibleName: "Configure Onyx"; onTriggered: root.uiProjection.requestSetup() }
            OrbAction { label: "FULLSCREEN"; accessibleName: "Toggle fullscreen"; onTriggered: root.uiProjection.requestFullscreen() }
            OrbAction { label: "CLOSE"; accessibleName: "Close Onyx"; onTriggered: root.uiProjection.requestClose() }
        }
    }
