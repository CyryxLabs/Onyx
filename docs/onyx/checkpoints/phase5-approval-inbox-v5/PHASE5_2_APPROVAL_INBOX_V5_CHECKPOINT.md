# Phase 5.2 Approval Inbox V5 checkpoint

Status: **candidate — default-off, read-only, external acceptance pending**

V5 supersedes the rejected V1, V2, V3 and V4 candidates without modifying a
historical byte. It is an isolated review projection behind strict
`ONYX_APPROVAL_INBOX_V5`; no startup,
...
them and V5 exposes no approve, deny, revoke, dispatch, persistence, or write
operation.
...
The V5 verifier materializes and executes the complete accepted Phase 5.1 R11
verifier recursively, which in turn verifies R1-R10. It also materializes and
executes the frozen V3 verifier from a manifest-only pristine materialization;
V3 in turn binds its earlier historical closure. It also executes frozen V2 and
V1 verification. A transitive R11/R10, V3, or V2 drift therefore fails V5;
adversarial fixtures modify `core/session_grants_v10.py`,
...
`runpy.run_module` loads, and rejects unprovable nonliteral uses of those
loaders. It rejects direct or successor-indirected reachability to any approval
inbox. The broad source manifest excludes only the exact frozen V1-V5 module
paths, so a future `approval_inbox_vN.py` cannot disappear behind a filename
pattern.

The verifier derives the exact `MAX_*` limit set and values from V5 source. It
...
- V5 remains a default-off candidate pending independent external E6 review.
