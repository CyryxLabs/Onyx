# Phase 5.3 Capability Nexus V27 checkpoint
...
V27 changes evidence attachment only. Cleanup records and overflow evidence are
first materialized as canonical bounded values. Each attribute and note attach
operation is then attempted and verified independently. Malformed note state,
hostile descriptors, custom attribute interception and note overrides cannot
discard evidence or escape as the terminal error.

If any attachment fails, V27 returns a fresh trusted parent wrapper. That wrapper
carries immutable cleanup evidence, the bounded overflow record, a safely
rendered primary record and every canonical attachment-failure record. The
original primary may be chained as cause, but authoritative evidence never
depends on cause access. Any partial mutation of the hostile primary is therefore
irrelevant. V26 introspection, rendering, exact-type, traversal-budget, DAG,
...
Every phase owns its complete process tree. V27 has no bootstrap file, process,
...
Quantitative evidence is exclusively recorded in `docs/onyx/checkpoints/phase5-capability-nexus-v27/phase5-capability-nexus-v27.metrics.json`.
No quantitative result in this checkpoint is authoritative outside that structured artifact.

V27 remains isolated, default-off, shadow-only, not live and externally unaccepted.
