# Phase 5.2 Approval Inbox V7 checkpoint

Status: **candidate — default-off, read-only, external acceptance pending**

V7 supersedes the rejected V1, V2, V3, V4, V5 and V6 candidates without modifying a
historical byte. It is an isolated review projection behind strict
`ONYX_APPROVAL_INBOX_V7`; no startup,
...
them and V7 exposes no approve, deny, revoke, dispatch, persistence, or write
operation.
...
Reopening an identical snapshot/query/page-size view preserves the existing
returned-review proof map and therefore cannot invalidate a review token that
was already displayed. A digest collision, mismatched registered view, missing
proof map, or orphan proof map permanently latches integrity failure and clears
all cached review state.
...
The V7 verifier materializes and executes the complete frozen V6 verifier from
its manifest-only pristine materialization. V6 recursively executes V5, R11 and
V1-V3, while binding V4 and every prior exact source/evidence anchor. A
transitive V6, V5, R11/R10, V3, or V2 drift therefore fails V7; adversarial fixtures
modify `core/approval_inbox_v6.py`, `core/approval_inbox_v5.py`, `core/session_grants_v10.py`,
`core/approval_inbox_v3.py`, and `core/approval_inbox_v2.py` in disposable
materializations and prove rejection before acceptance.

The live boundary uses an AST local-import walk from `main.py`, `ui.py`,
`scripts/launch_onyx.pyw` and discovered launch/run/start entrypoints. It resolves
static imports plus literal `importlib.import_module`, `__import__`, and
`runpy.run_module` loads. Relative importlib names are resolved with Python's
package semantics for literal positional or keyword packages, including bound
and module aliases. Missing, nonliteral, malformed, over-level, or otherwise
unresolvable relative context fails closed. The bounded intra-module analysis
also follows assignment chains, conservative rebindings, `getattr` loaders,
`builtins.__import__`, and bound builtins aliases. Reachable `eval`, `exec`, or
executable `compile` flows fail closed without executing analyzed code. Direct and successor-indirected
reachability to any approval inbox is rejected. The broad source manifest
excludes only the exact frozen V1-V7 module paths, so a future
`approval_inbox_vN.py` cannot disappear behind a filename pattern.

The verifier derives the exact `MAX_*` limit set and values from V7 source. It
...
- Phase 5.2 V4 remains historical and rejected, byte-exact.
- Phase 5.2 V5 remains historical and rejected, byte-exact.
- Phase 5.2 V6 remains historical and rejected, byte-exact.
- V7 remains a default-off candidate pending independent external E6 review.
