# Phase 5.3 Capability Nexus V30 checkpoint

Status: **candidate — default-off, shadow-only, external acceptance pending**

V30 preserves rejected V1–V29 byte-for-byte.

V30 separates mutable external projection documents from frozen historical
...
V30 always constructs a fresh parent wrapper when cleanup records or overflow
...
Every phase owns its complete process tree. V30 has no bootstrap file, process,
...
Quantitative evidence is exclusively recorded in `docs/onyx/checkpoints/phase5-capability-nexus-v30/phase5-capability-nexus-v30.metrics.json`.
No quantitative result in this checkpoint is accepted outside that structured artifact.

V30 remains isolated, default-off, shadow-only, not live and externally unaccepted.
