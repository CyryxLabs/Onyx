# Phase 5.2 Approval Inbox V6 checkpoint

Status: **candidate — default-off, read-only, external acceptance pending**

V6 supersedes the rejected V1, V2, V3, V4 and V5 candidates without modifying a
historical byte. It is an isolated review projection behind strict
`ONYX_APPROVAL_INBOX_V6`; no startup,
...
them and V6 exposes no approve, deny, revoke, dispatch, persistence, or write
operation.
...
The V6 verifier materializes and executes the complete frozen V5 verifier from
its manifest-only pristine materialization. V5 recursively executes R11 and
V1-V3, while binding V4 and every prior exact source/evidence anchor. A
transitive V5, R11/R10, V3, or V2 drift therefore fails V6; adversarial fixtures
modify `core/approval_inbox_v5.py`, `core/session_grants_v10.py`,
...
unresolvable relative context fails closed. Direct and successor-indirected
reachability to any approval inbox is rejected. The broad source manifest
excludes only the exact frozen V1-V6 module paths, so a future
`approval_inbox_vN.py` cannot disappear behind a filename pattern.

The verifier derives the exact `MAX_*` limit set and values from V6 source. It
...
- V6 remains a default-off candidate pending independent external E6 review.
