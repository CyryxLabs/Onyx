...
from scripts import verify_phase5_approval_inbox_v10 as parent_verifier
from scripts import verify_phase5_approval_inbox_v10_worker as evidence_worker

from core.approval_inbox_v10 import (
    MAX_SNAPSHOT_AGE_MS,
...
def item(number: int, **changes: object) -> HostInboxItemV10:
    ...
    return HostInboxItemV10(**values)


class Harness:
    def __init__(
        self,
        items: tuple[HostInboxItemV10, ...] | None = None,
        *,
        enabled: str = "true",
    ):
        self.flag_epoch = 7
        ...
        self.clock = DeterministicInboxClockV10(1_000)
        self.gate = ApprovalInboxFeatureGateV10(
            environ={"ONYX_APPROVAL_INBOX_V10": enabled},
            epoch_reader=lambda: self.flag_epoch,
        )
        self.source = HostInboxSourceV10(
            principal_id="owner-primary",
            session_id="session-local",
            capture_callback=self.capture,
        )
        self.projection = ApprovalInboxProjectionV10(
            gate=self.gate,
            source=self.source,
            clock=self.clock,
        )
    ...
    def capture(self) -> HostInboxSnapshotV10:
        ...
        return HostInboxSnapshotV10(self.source_epoch, 55_000, self.items)
...
class EpochInterlock:
    ...
    def __call__(self) -> int:
        ...
        with self._lock:
            ...
            should_block = (
                threading.current_thread().name == "v10-interleaving"
...
def _thread_call(callable_):
    ...
    thread = threading.Thread(target=run, name="v10-interleaving")
    thread.start()
...
def test_exact_enable_values(value: str) -> None:
    ...
    assert harness.projection.state is ApprovalInboxV10State.READY
...
def test_default_off_never_calls_source(value: str) -> None:
    harness = Harness(enabled=value)
    assert harness.projection.state is ApprovalInboxV10State.DISABLED
    with pytest.raises(ApprovalInboxV10Unavailable, match="disabled"):
...
def test_flag_is_read_once_but_epoch_change_permanently_revokes() -> None:
    environment = {"ONYX_APPROVAL_INBOX_V10": "true"}
    epoch = [4]
    gate = ApprovalInboxFeatureGateV10(
        environ=environment, epoch_reader=lambda: epoch[0]
    )
    environment["ONYX_APPROVAL_INBOX_V10"] = "false"
    assert gate.state() is ApprovalInboxV10State.READY
    epoch[0] = 5
    assert gate.state() is ApprovalInboxV10State.REVOKED
    epoch[0] = 4
    assert gate.state() is ApprovalInboxV10State.REVOKED
...
def test_gate_honors_completed_revocation_after_stale_epoch_callback() -> None:
    ...
    gate = ApprovalInboxFeatureGateV10(
        environ={"ONYX_APPROVAL_INBOX_V10": "true"}, epoch_reader=interlock
    )
    interlock.arm(1)
    ...
    assert not failures
    assert results == [ApprovalInboxV10State.REVOKED]
...
def test_projection_state_cannot_publish_ready_after_completed_revocation() -> None:
    ...
    projection = ApprovalInboxProjectionV10(
        gate=gate,
        source=HostInboxSourceV10(
            principal_id="owner-primary",
            session_id="session-local",
            capture_callback=lambda: HostInboxSnapshotV10(1, 1, (item(1),)),
        ),
        clock=DeterministicInboxClockV10(),
    )
    interlock.arm(1)
...
def test_page_contains_complete_safe_review_and_zero_authority() -> None:
    ...
    assert page.state is ApprovalInboxV10State.READY
...
def test_pagination_is_stable_and_cursor_is_exact() -> None:
    ...
    with pytest.raises(ApprovalInboxV10ContractError):
...
def test_snapshot_reuse_never_extends_ttl() -> None:
    ...
    with pytest.raises(ApprovalInboxV10Unavailable, match="snapshot-stale"):
        harness.projection.open_page()
    assert harness.projection.state is ApprovalInboxV10State.STALE
...
def test_clock_rollback_integrity_latches() -> None:
    ...
    with pytest.raises(ApprovalInboxV10Unavailable, match="clock-integrity-latched"):
        harness.projection.open_page()
    assert harness.projection.state is ApprovalInboxV10State.INTEGRITY_LATCHED
...
def test_source_epoch_rollback_and_equivocation_latch() -> None:
    ...
    with pytest.raises(ApprovalInboxV10Unavailable, match="source-integrity-latched"):
        rollback.projection.open_page()
    assert rollback.projection.state is ApprovalInboxV10State.INTEGRITY_LATCHED
    ...
    with pytest.raises(ApprovalInboxV10Unavailable, match="source-integrity-latched"):
        equivocation.projection.open_page()
    assert equivocation.projection.state is ApprovalInboxV10State.INTEGRITY_LATCHED
...
def test_new_source_epoch_invalidates_all_old_cursors_and_review_proofs() -> None:
    ...
    with pytest.raises(ApprovalInboxV10Unavailable, match="cursor-state-unavailable"):
        harness.projection.continue_page(old.next_cursor)
    with pytest.raises(
        ApprovalInboxV10Unavailable, match="selection-state-unavailable"
    ):
...
def test_feature_epoch_change_during_capture_rejects_callback_output() -> None:
    ...
    def rotate() -> HostInboxSnapshotV10:
        epoch[0] = 2
        return HostInboxSnapshotV10(1, 1, (item(1),))

    gate = ApprovalInboxFeatureGateV10(
        environ={"ONYX_APPROVAL_INBOX_V10": "true"}, epoch_reader=lambda: epoch[0]
    )
    projection = ApprovalInboxProjectionV10(
        gate=gate,
        source=HostInboxSourceV10(
    ...
    with pytest.raises(ApprovalInboxV10Unavailable, match="feature-epoch-revoked"):
        projection.open_page()
    assert projection.state is ApprovalInboxV10State.REVOKED
...
def test_open_page_final_publication_rejects_completed_revocation() -> None:
    ...
    projection = ApprovalInboxProjectionV10(
        ...
        source=HostInboxSourceV10(
            principal_id="owner-primary",
            session_id="session-local",
            capture_callback=lambda: HostInboxSnapshotV10(1, 1, (item(1), item(2))),
        ),
        clock=DeterministicInboxClockV10(),
    )
    interlock.arm(4)
    ...
    assert isinstance(failures[0], ApprovalInboxV10Unavailable)
...
def test_newer_source_commit_suppresses_older_page_and_cache_repopulation() -> None:
    ...
    def capture() -> HostInboxSnapshotV10:
        if threading.current_thread().name == "v10-interleaving":
            return HostInboxSnapshotV10(1, 1, (item(1),))
        return HostInboxSnapshotV10(2, 2, (item(9),))
    ...
    assert isinstance(old_failures[0], ApprovalInboxV10Unavailable)
...
def test_authoritative_core_junction_with_identical_bytes_is_rejected(
    ...
    source = Path(__file__).resolve().parents[1] / "core/approval_inbox_v10.py"
    base = source.parents[1] / f".p52-v10-junction-{secrets.token_hex(8)}"
    clone = base / "clone"
    external_core = base / "external-core"
    external_file = external_core / "approval_inbox_v10.py"
    junction = clone / "core"
    base.mkdir()
    clone.mkdir()
    external_core.mkdir()
    try:
        ...
        with pytest.raises(verifier.PathIntegrityError, match="reparse-component"):
            verifier._safe_path_at(clone, "core/approval_inbox_v10.py")
    finally:
...
def test_source_exception_is_collapsed_without_secret_cause() -> None:
    def fail() -> HostInboxSnapshotV10:
        raise RuntimeError("password=do-not-leak")

    gate = ApprovalInboxFeatureGateV10(
        environ={"ONYX_APPROVAL_INBOX_V10": "1"}, epoch_reader=lambda: 1
    )
    source = HostInboxSourceV10(
        principal_id="owner-primary", session_id="session-local", capture_callback=fail
    )
    projection = ApprovalInboxProjectionV10(
        gate=gate, source=source, clock=DeterministicInboxClockV10()
    )
    with pytest.raises(ApprovalInboxV10Unavailable) as caught:
        projection.open_page()
...
def test_single_flight_callback_is_bounded_and_outside_projection_lock() -> None:
    ...
    def blocked_capture() -> HostInboxSnapshotV10:
        entered.set()
        assert release.wait(2)
        return HostInboxSnapshotV10(1, 1, (item(1),))

    gate = ApprovalInboxFeatureGateV10(
        environ={"ONYX_APPROVAL_INBOX_V10": "1"}, epoch_reader=lambda: epoch[0]
    )
    source = HostInboxSourceV10(
        principal_id="owner-primary",
        session_id="session-local",
    ...
    # the projection lock.
    assert projection.state is ApprovalInboxV10State.READY
    with pytest.raises(ApprovalInboxV10Unavailable, match="source-single-flight-busy"):
        projection.open_page()
    release.set()
    worker.join(2)
...
def test_calm_batch_is_order_independent_idempotent_and_non_authoritative() -> None:
    ...
    page = harness.projection.open_page(query=InboxQueryV10(batch_only=True))
    selections = _all_selections(page)
...
def test_ineligible_items_never_enter_calm_batch(changes: dict[str, object]) -> None:
    ...
    page = harness.projection.open_page()
    with pytest.raises(ApprovalInboxV10ContractError, match="not calm-batch eligible"):
        harness.projection.preview_calm_batch(
            page.snapshot_id, page.view_digest, _all_selections(page)
        )
    filtered = harness.projection.open_page(query=InboxQueryV10(batch_only=True))
...
def test_hidden_unreturned_cross_view_and_tampered_proofs_fail() -> None:
    ...
    second = harness.projection.open_page(
        query=InboxQueryV10(workspace_id="workspace-main"), page_size=1
    )
    selection = _all_selections(first)
    with pytest.raises(
        ApprovalInboxV10ContractError, match="not reviewed in this view"
    ):
        harness.projection.preview_calm_batch(
...
def test_duplicate_item_request_fingerprint_and_idempotency_identity_reject() -> None:
    ...
    with pytest.raises(ApprovalInboxV10ContractError, match="duplicate item_id"):
    ...
    with pytest.raises(
        ApprovalInboxV10ContractError, match="duplicate action_request_id"
    ):
    ...
    with pytest.raises(
        ApprovalInboxV10ContractError, match="duplicate action_fingerprint"
    ):
    ...
    with pytest.raises(
        ApprovalInboxV10ContractError, match="duplicate idempotency_identity"
    ):
...
def test_public_surface_has_no_authority_or_mutation_methods() -> None:
    public = {
        name for name in dir(ApprovalInboxProjectionV10) if not name.startswith("_")
    }
    assert public == {
        "continue_page",
        "handoff_calm_batch",
