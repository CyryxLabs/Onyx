...
import core.phase5_runtime_v6 as runtime


def _binding(**changes: object) -> runtime.RuntimeBindingV6:
    ...
    return runtime.RuntimeBindingV6(**values)  # type: ignore[arg-type]


def _flags(**changes: object) -> runtime.RuntimeFlagsV6:
    values: dict[str, object] = {
        "runtime": True,
        "grant_shadow": False,
    ...
    values.update(changes)
    return runtime.RuntimeFlagsV6(**values)  # type: ignore[arg-type]


def _action(
    binding: runtime.RuntimeBindingV6 | None = None, **changes: object
) -> runtime.ActionRequestV6:
    values: dict[str, object] = {
    ...
    values.update(changes)
    return runtime.ActionRequestV6(**values)  # type: ignore[arg-type]
...
def _core(
    *,
    flags: runtime.RuntimeFlagsV6 | None = None,
    components: runtime.RuntimeComponentsV6 | None = None,
    termination_callback_timeout_seconds: float = 1.0,
) -> runtime.Phase5RuntimeV6:
    return runtime.Phase5RuntimeV6(
        binding=_binding(),
...
def test_flags_are_independent_exact_and_default_off() -> None:
    ...
    assert runtime.RuntimeFlagsV6.from_environ({}) == runtime.RuntimeFlagsV6()
    for name in names:
        values = {item: "false" for item in names}
        values[name] = "true"
        assert (
            sum(dict(runtime.RuntimeFlagsV6.from_environ(values).payload()).values())
            == 1
        )
    for field in runtime.RuntimeFlagsV6.__dataclass_fields__:
        with pytest.raises(runtime.Phase5RuntimeV6ContractError):
            runtime.RuntimeFlagsV6(**{field: 1})
...
def test_default_off_is_observationally_pure() -> None:
    ...
    core = _core(
        ...
        components=runtime.RuntimeComponentsV6(
...
def test_only_exact_local_catalog_read_can_be_allowed(
    ...
    assert (
        ...
        is runtime.RuntimeDispositionV6.EXPLICIT
    )
...
def test_always_explicit_classes(action_class: str) -> None:
    ...
    assert decision.disposition is runtime.RuntimeDispositionV6.EXPLICIT
...
def test_decision_requires_exact_request_and_is_single_use() -> None:
    ...
    assert decision.disposition is runtime.RuntimeDispositionV6.ALLOW_LOCAL_CATALOG_READ
...
def test_closed_capability_schema_projects_only_runtime_binding() -> None:
    ...
    core = _core(
        ...
        components=runtime.RuntimeComponentsV6(
            nexus_projector=lambda binding: runtime.ProjectionBatchV6((supplied,)),
        ),
    )
    ...
    for field in runtime.RuntimeBindingV6.__dataclass_fields__:
...
def test_capability_rejects_provider_authority_secret_and_arbitrary_fields(
    ...
    core = _core(
        ...
        components=runtime.RuntimeComponentsV6(
            nexus_projector=lambda _binding: runtime.ProjectionBatchV6((raw,))
        ),
    )
...
def test_central_dlp_redacts_sensitive_values_from_all_dashboard_projection() -> None:
    ...
    core = _core(
        flags=_flags(nexus_projection=True, dashboard_projection=True),
        components=runtime.RuntimeComponentsV6(
            nexus_projector=lambda _binding: runtime.ProjectionBatchV6(
...
def test_closed_inbox_schema_and_runtime_scope_projection() -> None:
    class Reader:
        def read_page(self, **_kwargs: object) -> runtime.ProjectionBatchV6:
            return runtime.ProjectionBatchV6(
                (_item(),), source_cursor=None, next_cursor=None
            )

    core = _core(
        flags=_flags(approval_inbox=True, dashboard_projection=True),
        components=runtime.RuntimeComponentsV6(inbox_factory=lambda _binding: Reader()),
    )
...
def test_inbox_rejects_cross_scope_secret_and_arbitrary_fields(field: str) -> None:
    ...
    class Reader:
        def read_page(self, **_kwargs: object) -> runtime.ProjectionBatchV6:
            return runtime.ProjectionBatchV6((raw,), source_cursor=None)
...
def test_json_budget_rejects_cycle_shared_dag_depth_fanout_and_million_expansion_early() -> (
    ...
    for value in (cycle, dag, deep, million):
        with pytest.raises(runtime.Phase5RuntimeV6ContractError):
            runtime.ProjectionBatchV6(({"capability_id": "cap", "payload": value},))
    repeated_item = _cap()
    with pytest.raises(runtime.Phase5RuntimeV6ContractError):
        runtime.ProjectionBatchV6((repeated_item, repeated_item))
...
def test_json_budget_counts_incrementally_and_does_not_mutate_input() -> None:
    ...
    batch = runtime.ProjectionBatchV6((raw,))
...
def test_concurrent_component_failures_are_both_recorded_regardless_of_finish_order() -> (
    ...
    def nexus(_binding: runtime.RuntimeBindingV6) -> runtime.ProjectionBatchV6:
        barrier.wait()
        release_nexus.wait()
    ...
    assert core.status().state is runtime.RuntimeStateV6.DEGRADED
...
def test_partial_component_recovery_never_becomes_ready_or_allow() -> None:
    ...
    def nexus(_binding: runtime.RuntimeBindingV6) -> runtime.ProjectionBatchV6:
        if not state["nexus"]:
            raise RuntimeError
        return runtime.ProjectionBatchV6((_cap(),))

    class Reader:
        def read_page(self, **_kwargs: object) -> runtime.ProjectionBatchV6:
            if not state["inbox"]:
                raise RuntimeError
            return runtime.ProjectionBatchV6((_item(),), source_cursor=None)
    ...
    assert core.status().state is runtime.RuntimeStateV6.DEGRADED
    assert core.evaluate(_action()).disposition is runtime.RuntimeDispositionV6.EXPLICIT
    state["inbox"] = True
    core.recover_component("inbox")
    assert (
        core.status().component_failures == ()
        and core.status().state is runtime.RuntimeStateV6.READY
    )
...
def test_concurrent_failure_after_ordinary_success_remains_until_explicit_recovery() -> (
    ...
    core = _core(
        flags=_flags(nexus_projection=True),
        components=runtime.RuntimeComponentsV6(nexus_projector=nexus),
    )
    with ThreadPoolExecutor(max_workers=2) as pool:
...
def test_terminal_boundary_is_nonblocking_single_flight_and_clears_authority() -> None:
    ...
    core = _core(components=runtime.RuntimeComponentsV6(grant_terminator=terminator))
    action = _action()
    decision = core.evaluate(action)
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        assert all(
            result.state is runtime.RuntimeStateV6.TERMINATION_PENDING
            for result in results
        )
        release.set()
    assert core.await_termination(1.0).state is runtime.RuntimeStateV6.TERMINATED
    assert calls == ["kill"]
    assert not core.consume_decision(decision, action)
...
def test_two_hundred_noop_reconfigurations_after_failure_are_observationally_pure() -> (
    ...
    def failing(_binding: runtime.RuntimeBindingV6) -> runtime.ProjectionBatchV6:
        raise RuntimeError("failure")

    components = runtime.RuntimeComponentsV6(nexus_projector=failing)
    flags = _flags(nexus_projection=True, dashboard_projection=True)
    binding = _binding()
...
def test_inflight_failure_survives_noop_and_unrelated_real_reconfiguration() -> None:
    ...
    def projector(_binding: runtime.RuntimeBindingV6) -> runtime.ProjectionBatchV6:
        if should_fail:
            entered.set()
            release.wait()
    ...
    components = runtime.RuntimeComponentsV6(nexus_projector=projector)
    flags = _flags(nexus_projection=True)
    core = _core(flags=flags, components=components)
    assert core.recover_component("nexus").component_failures == ()
    should_fail = True
...
def test_old_generation_failure_cannot_poison_recovered_replacement() -> None:
    ...
    def old(_binding: runtime.RuntimeBindingV6) -> runtime.ProjectionBatchV6:
        entered.set()
        release.wait()
        raise RuntimeError("old generation")

    def current(_binding: runtime.RuntimeBindingV6) -> runtime.ProjectionBatchV6:
        return runtime.ProjectionBatchV6((_cap("current"),))

    core = _core(
        flags=_flags(nexus_projection=True),
        components=runtime.RuntimeComponentsV6(nexus_projector=old),
    )
    with ThreadPoolExecutor(max_workers=2) as pool:
        stale = pool.submit(core.refresh_capabilities)
        entered.wait()
        core.replace_components(runtime.RuntimeComponentsV6(nexus_projector=current))
        assert core.recover_component("nexus").component_failures == ()
        release.set()
        stale.result()
...
def test_enabling_missing_or_new_components_is_degraded_until_explicit_probe() -> None:
    ...
    assert status.state is runtime.RuntimeStateV6.DEGRADED
    ...
    components = runtime.RuntimeComponentsV6(
        nexus_projector=lambda _binding: runtime.ProjectionBatchV6((_cap(),)),
    ...
    assert core.evaluate(_action()).disposition is runtime.RuntimeDispositionV6.EXPLICIT
    assert core.recover_component("inbox").component_failures == ()
    assert core.status().state is runtime.RuntimeStateV6.READY
...
def test_recovery_clears_only_after_successful_probe_of_current_generation() -> None:
    ...
    def old(_binding: runtime.RuntimeBindingV6) -> runtime.ProjectionBatchV6:
        old_entered.set()
        release_old.wait()
        return runtime.ProjectionBatchV6((_cap("old"),))
...
def test_live_modules_do_not_import_v5_candidate() -> None:
    ...
    for relative in (
        ...
        assert "phase5_runtime_v6" not in (root / relative).read_text(encoding="utf-8")
...
def test_decision_advisory_value_dlp_is_canonical_and_redacted() -> None:
    decision = runtime.RuntimeDecisionV6(
        runtime.RuntimeDispositionV6.EXPLICIT,
        "explicit_required",
        "trace-one",
        1,
...
def test_two_hundred_recovery_before_failure_races_never_erase_later_failure() -> None:
    ...
    def projector(_binding: runtime.RuntimeBindingV6) -> runtime.ProjectionBatchV6:
        if threading.current_thread().name.startswith("v6-recovery"):
            probe_entered.set()
            release_probe.wait()
            return runtime.ProjectionBatchV6((_cap(),))
...
def test_callback_nested_same_thread_termination_is_bounded_and_final() -> None:
    nested: list[runtime.RuntimeStatusV6] = []
    nested_done = threading.Event()
    core: runtime.Phase5RuntimeV6
    ...
    core = _core(
        components=runtime.RuntimeComponentsV6(grant_terminator=callback),
        termination_callback_timeout_seconds=0.05,
    )
    started = time.perf_counter()
    result = core.kill()
    assert time.perf_counter() - started < 0.5
    assert result.state is runtime.RuntimeStateV6.TERMINATION_PENDING
    assert nested_done.wait(1.0)
    assert nested[0].state is runtime.RuntimeStateV6.TERMINATION_PENDING
    assert core.await_termination(1.0).state is runtime.RuntimeStateV6.TERMINATED
...
def test_callback_cross_thread_join_without_timeout_cannot_deadlock_host() -> None:
    ...
    core = _core(
        components=runtime.RuntimeComponentsV6(nexus_terminator=callback),
        termination_callback_timeout_seconds=0.05,
    )
    started = time.perf_counter()
...
def test_callback_failure_and_timeout_are_bounded_and_attributed() -> None:
    ...
    assert result.state is runtime.RuntimeStateV6.TERMINATION_PENDING
    timed = core.await_termination(0.01)
    assert timed.state is runtime.RuntimeStateV6.TERMINATION_PENDING
    release.set()
    final = core.await_termination(1.0)
    assert final.state is runtime.RuntimeStateV6.TERMINATED
    assert set(final.termination_failures) == {"grant"}
    assert "inbox" in final.termination_timeouts
...
def test_many_normal_terminal_callers_get_final_single_flight_outcome() -> None:
    ...
    assert all(
        result.state
        in {
            runtime.RuntimeStateV6.TERMINATION_PENDING,
            runtime.RuntimeStateV6.TERMINATED,
        }
        for result in results
    )
    assert core.await_termination(1.0).state is runtime.RuntimeStateV6.TERMINATED
...
def test_normal_termination_leaves_no_phase5_worker_threads() -> None:
    ...
    assert core.kill().state is runtime.RuntimeStateV6.TERMINATION_PENDING
    assert core.await_termination(1.0).state is runtime.RuntimeStateV6.TERMINATED
    assert calls == ["kill"] * 4
    assert not any(
        thread.name.startswith("phase5-v6-terminate-") and thread.is_alive()
...
def test_contextual_id_fields_accept_uuid_ulid_trace_and_hash_ids(
    identifier: str,
) -> None:
    binding = _binding(
        trace_id=identifier,
    ...
    assert binding.workspace_id == identifier
    batch = runtime.ProjectionBatchV6((_cap(identifier),))
    core = runtime.Phase5RuntimeV6(
        binding=binding,
        flags=_flags(nexus_projection=True),
        components=runtime.RuntimeComponentsV6(
            nexus_projector=lambda _binding: batch
        ),
    )
    assert core.refresh_capabilities().items[0].item_id == identifier
...
@pytest.mark.parametrize(
    ...
    (
        ...
        "mfrggzdfmztwq2lknnwg23tpoi234567",
        "mF_9x2Qa7vN1kR8pL4sT6uW0yZ3bC5dE",
        "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.c2lnbmF0dXJl",
        "authorization was 'Abcd-1234'",
        "auth code=Abcd-1234",
        "cookie Abcd-1234",
        "passport-Abcd-1234",
        "private_key_Abcd-1234",
        "session cookie: Abcd-1234",
        "access is Abcd-1234",
        "refresh=Abcd-1234",
    ),
)
def test_contextual_content_dlp_catches_secret_families_case_independently(
    canary: str,
) -> None:
    class Reader:
        def read_page(self, **_kwargs: object) -> runtime.ProjectionBatchV6:
            return runtime.ProjectionBatchV6(
                (_item(summary=f"request contains {canary}"),),
                source_cursor=None,
            )
    ...
    core.refresh_capabilities()
    core.refresh_inbox()
    rendered = repr((core.capabilities(), core.inbox()))
    assert canary not in rendered
    assert "[REDACTED]" in rendered


def test_newer_nexus_refresh_wins_inverse_completion_order_two_hundred_times() -> None:
    entered = threading.Event()
    release = threading.Event()
    calls = 0
    iteration = 0
    ...
    def projector(_binding: runtime.RuntimeBindingV6) -> runtime.ProjectionBatchV6:
        ...
        if call % 2:
            ...
            label = f"old-{iteration}"
        else:
            label = f"new-{iteration}"
        return runtime.ProjectionBatchV6((_cap(label),))
    ...
    for iteration in range(200):
        ...
        release.clear()
        with ThreadPoolExecutor(max_workers=2) as pool:
            old = pool.submit(core.refresh_capabilities)
            assert entered.wait(1.0)
            new = pool.submit(core.refresh_capabilities)
            new.result(timeout=1.0)
            release.set()
            old.result(timeout=1.0)
        assert core.capabilities().items[0].item_id == f"new-{iteration}"


def test_newer_nexus_recovery_wins_inverse_completion_order_two_hundred_times() -> None:
    entered = threading.Event()
    release = threading.Event()
    calls = 0
    iteration = 0
    ...
    def projector(_binding: runtime.RuntimeBindingV6) -> runtime.ProjectionBatchV6:
        ...
        if call % 2:
            entered.set()
            assert release.wait(1.0)
            label = f"old-recovery-{iteration}"
        else:
            label = f"new-recovery-{iteration}"
        return runtime.ProjectionBatchV6((_cap(label),))
    ...
    for iteration in range(200):
        with core._lock:  # noqa: SLF001
            core._set_failure_locked("nexus", True)  # noqa: SLF001
        entered.clear()
        release.clear()
        with ThreadPoolExecutor(max_workers=2) as pool:
            old = pool.submit(core.recover_component, "nexus")
            assert entered.wait(1.0)
            new = pool.submit(core.recover_component, "nexus")
            new.result(timeout=1.0)
            release.set()
            old.result(timeout=1.0)
        assert core.capabilities().items[0].item_id == f"new-recovery-{iteration}"
        assert core.status().component_failures == ()


def test_newer_inbox_page_wins_inverse_completion_order_two_hundred_times() -> None:
    entered = threading.Event()
    release = threading.Event()
    calls = 0
    iteration = 0
    ...
    class Reader:
        def read_page(self, **_kwargs: object) -> runtime.ProjectionBatchV6:
            ...
            if call % 2:
                entered.set()
                assert release.wait(1.0)
                label = f"old-page-{iteration}"
            else:
                label = f"new-page-{iteration}"
            return runtime.ProjectionBatchV6((_item(label),), source_cursor=None)
    ...
    for iteration in range(200):
        ...
        release.clear()
        with ThreadPoolExecutor(max_workers=2) as pool:
            old = pool.submit(core.refresh_inbox)
            assert entered.wait(1.0)
            new = pool.submit(core.refresh_inbox)
            new.result(timeout=1.0)
            release.set()
            old.result(timeout=1.0)
        assert core.inbox().items[0].item_id == f"new-page-{iteration}"


def test_newer_inbox_recovery_wins_inverse_completion_order_two_hundred_times() -> None:
    entered = threading.Event()
    release = threading.Event()
    calls = 0
    iteration = 0
    ...
    class Reader:
        def read_page(self, **_kwargs: object) -> runtime.ProjectionBatchV6:
            ...
            if call % 2:
                entered.set()
                assert release.wait(1.0)
                label = f"old-inbox-recovery-{iteration}"
            else:
                label = f"new-inbox-recovery-{iteration}"
            return runtime.ProjectionBatchV6((_item(label),), source_cursor=None)
    ...
    for iteration in range(200):
        with core._lock:  # noqa: SLF001
            core._set_failure_locked("inbox", True)  # noqa: SLF001
        entered.clear()
        release.clear()
        with ThreadPoolExecutor(max_workers=2) as pool:
            old = pool.submit(core.recover_component, "inbox")
            assert entered.wait(1.0)
            new = pool.submit(core.recover_component, "inbox")
            new.result(timeout=1.0)
            release.set()
            old.result(timeout=1.0)
        assert core.inbox().items[0].item_id == f"new-inbox-recovery-{iteration}"
        assert core.status().component_failures == ()


def test_stale_nexus_failure_cannot_overwrite_newer_success() -> None:
    entered = threading.Event()
    release = threading.Event()
    calls = 0
    mode = "recover"
    ...
    def projector(_binding: runtime.RuntimeBindingV6) -> runtime.ProjectionBatchV6:
        nonlocal calls
        calls += 1
        if mode == "race" and calls == 2:
            entered.set()
            assert release.wait(1.0)
            raise RuntimeError("stale failure")
        return runtime.ProjectionBatchV6((_cap(f"cap-{calls}"),))
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        assert entered.wait(1.0)
        fresh = pool.submit(core.refresh_capabilities)
        fresh.result(timeout=1.0)
        release.set()
        stale.result(timeout=1.0)
    assert core.status().component_failures == ()
    assert dict(core.status().failure_observations)["nexus"] == before


def test_cursor_rejections_are_audited() -> None:
    class Reader:
        def read_page(self, **_kwargs: object) -> runtime.ProjectionBatchV6:
            return runtime.ProjectionBatchV6(
                (_item(),), source_cursor=None, next_cursor="next-one"
            )
    ...
    with pytest.raises(runtime.Phase5RuntimeV6ContractError):
        core.refresh_inbox(cursor="wrong")
    assert any(event.outcome == "cursor_rejected" for event in core.events())


def test_sixty_four_runtimes_use_only_global_bounded_supervisor_threads() -> None:
    release = threading.Event()

    def blocked(_reason: str) -> None:
        release.wait()

    cores = [
        _core(components=runtime.RuntimeComponentsV6(grant_terminator=blocked))
        for _ in range(64)
    ]
    statuses = [core.kill() for core in cores]
    assert all(
        status.state is runtime.RuntimeStateV6.TERMINATION_PENDING
        for status in statuses
    )
    assert all(core.evaluate(_action()).reason == "runtime_terminated" for core in cores)
    supervisors = [
        thread
        for thread in threading.enumerate()
        if thread.name.startswith("phase5-v6-termination-supervisor-")
        and thread.is_alive()
    ]
    assert len(supervisors) <= runtime.MAX_TERMINATION_WORKERS
    release.set()
    assert all(
        core.await_termination(3.0).state is runtime.RuntimeStateV6.TERMINATED
        for core in cores
    )


def test_nested_await_returns_pending_without_deadlock() -> None:
    nested: list[runtime.RuntimeStatusV6] = []
    core: runtime.Phase5RuntimeV6

    def callback(_reason: str) -> None:
        nested.append(core.await_termination(10.0))
...
def test_supervisor_queue_saturation_is_pending_fail_closed_and_retryable(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    original = runtime._TERMINATION_SUPERVISOR.submit  # noqa: SLF001
    attempts = 0

    def reject_once(job: object) -> bool:
        nonlocal attempts
        attempts += 1
        if attempts == 1:
            return False
    ...
    monkeypatch.setattr(runtime._TERMINATION_SUPERVISOR, "submit", reject_once)  # noqa: SLF001
    calls: list[str] = []
    core = _core(
        components=runtime.RuntimeComponentsV6(
            grant_terminator=lambda reason: calls.append(reason)
        )
    )
    pending = core.kill()
    assert pending.state is runtime.RuntimeStateV6.TERMINATION_PENDING
    assert pending.termination_failures == ("supervisor_queue_saturated",)
    assert core.evaluate(_action()).reason == "runtime_terminated"
    final = core.await_termination(1.0)
    assert final.state is runtime.RuntimeStateV6.TERMINATED
    assert calls == ["kill"] and attempts == 2
