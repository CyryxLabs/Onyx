...
V41 = ROOT / "tests/fixtures/phase5_current_successor_transition_v41.json"


def test_v41_is_the_immutable_predecessor_of_current_v42() -> None:
    retirement.load_current_successor_transition.cache_clear()
    transition = retirement.load_current_successor_transition()
    assert hashlib.sha256(V41.read_bytes()).hexdigest() == (
        retirement.CURRENT_TRANSITION_V41_SHA256
    )
    assert transition["schema"] == "onyx.phase5-current-successor-transition.v42"
    assert transition["predecessor"] == {
        "path": "tests/fixtures/phase5_current_successor_transition_v41.json",
        "sha256": retirement.CURRENT_TRANSITION_V41_SHA256,
    }


def test_v42_authenticates_current_package_hygiene() -> None:
    retirement.load_current_successor_transition.cache_clear()
    transition = retirement.load_current_successor_transition()
    successors = {
