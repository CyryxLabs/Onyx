...
import ui
from core.onyx_hud_current_acceptance_v22 import (
    ARTIFACT_ROOT_SHA256,
    CURRENT_RUNTIME_INPUTS,
    CurrentHudAcceptanceV22Error,
    MANIFEST_RELATIVE,
...
def test_current_v22_closure_is_exact() -> None:
    assert verify_current_hud_acceptance(ROOT) == {
        "candidate": "onyx-hud-v10-v36-current-pinned-004",
...
def test_every_current_input_tamper_fails_closed(tmp_path: Path, relative: Path) -> None:
    ...
    with pytest.raises(CurrentHudAcceptanceV22Error, match="input drift"):
...
def test_every_v21_predecessor_tamper_fails_closed(tmp_path: Path, relative: Path) -> None:
    _copy_acceptance(tmp_path)
    with (tmp_path / relative).open("ab") as handle:
        handle.write(b"tamper")
    with pytest.raises(CurrentHudAcceptanceV22Error, match="predecessor drift"):
        verify_current_hud_acceptance(tmp_path)
...
def test_current_package_selection_is_v22_hud_and_stable_v24() -> None:
    ...
    assert "core.onyx_hud_current_acceptance_v22" in build
    ...
    assert "VE-HUD-CURRENT-V22-E6-001.manifest.json" in hygiene
    assert '"core.onyx_hud_current_acceptance_v22"' in spec
