from __future__ import annotations

import hashlib
import json
from pathlib import Path

from scripts import verify_phase5_exit_retirement_v1 as retirement


ROOT = Path(__file__).resolve().parents[1]
V13 = ROOT / "tests/fixtures/phase5_current_successor_transition_v13.json"
V14 = ROOT / "tests/fixtures/phase5_current_successor_transition_v14.json"


def _clear() -> None:
    retirement.load_record.cache_clear()
    retirement.load_current_successor_transition.cache_clear()
    retirement.load_successor_transition.cache_clear()
    retirement.load_release_workflow_transition.cache_clear()


def test_v14_authenticates_exact_v13_predecessor_and_domain_root() -> None:
    _clear()
    transition = json.loads(V14.read_text(encoding="utf-8"))
    assert hashlib.sha256(V14.read_bytes()).hexdigest() == (
        "da0f6d07b7dadd22709147cdfb667bbb7d205949d276f82042f1e27a39225190"
    )
    assert transition["schema"] == "onyx.phase5-current-successor-transition.v14"
    assert transition["issued_at"] == "2026-08-03T19:37:00-04:00"
    assert transition["predecessor"] == {
        "path": "tests/fixtures/phase5_current_successor_transition_v13.json",
        "sha256": retirement.CURRENT_TRANSITION_V13_SHA256,
    }
    assert transition["current_root_sha256"] == (
        "a456192a7ef7f05fc1b48ec093d717c2fadfa09e157fe93da8b1abcf2c22df52"
    )


def test_v13_is_exact_immutable_predecessor() -> None:
    assert hashlib.sha256(V13.read_bytes()).hexdigest() == (
        retirement.CURRENT_TRANSITION_V13_SHA256
    )
    assert V13 == retirement.CURRENT_TRANSITION_V13


def test_v14_preserves_identity_and_records_only_current_ui_delta() -> None:
    v13 = json.loads(V13.read_text(encoding="utf-8"))
    v14 = json.loads(V14.read_text(encoding="utf-8"))
    assert [
        (
            item["path"],
            item["historical_sha256"],
            item["state"],
            item["successor"],
        )
        for item in v14["historical_bindings"]
    ] == [
        (
            item["path"],
            item["historical_sha256"],
            item["state"],
            item["successor"],
        )
        for item in v13["historical_bindings"]
    ]
    assert [
        (item["path"], item["predecessor_sha256"])
        for item in v14["named_successors"]
    ] == [
        (item["path"], item["predecessor_sha256"])
        for item in v13["named_successors"]
    ]
    historical_deltas = {
        current["path"]
        for current, prior in zip(
            v14["historical_bindings"], v13["historical_bindings"], strict=True
        )
        if current["current_sha256"] != prior["current_sha256"]
    }
    successor_deltas = {
        current["path"]
        for current, prior in zip(
            v14["named_successors"], v13["named_successors"], strict=True
        )
        if current["current_sha256"] != prior["current_sha256"]
    }
    assert historical_deltas == {"ui.py"}
    assert successor_deltas == set()


def test_v14_changes_no_runtime_authority_policy() -> None:
    transition = json.loads(V14.read_text(encoding="utf-8"))
    assert transition["policy"] == {
        "historical_bindings_are_rewritten": False,
        "historical_hashes_are_rebound": False,
        "predecessor_root_must_remain_reproducible": True,
        "current_paths_are_sha256_bound": True,
        "named_successors_are_sha256_bound": True,
        "runtime_authority_changes": False,
    }
