...
import ui
from core.onyx_hud_current_acceptance_v23 import (
    ARTIFACT_ROOT_SHA256,
    CURRENT_RUNTIME_INPUTS,
    CurrentHudAcceptanceV23Error,
    MANIFEST_RELATIVE,
...
def test_current_v23_closure_is_exact() -> None:
    assert verify_current_hud_acceptance(ROOT) == {
        "candidate": "onyx-hud-v10-v36-runtime-final-pinned-005",
...
def test_every_current_input_tamper_fails_closed(tmp_path: Path, relative: Path) -> None:
    ...
    with pytest.raises(CurrentHudAcceptanceV23Error, match="input drift"):
...
def test_every_v22_predecessor_tamper_fails_closed(tmp_path: Path, relative: Path) -> None:
    _copy_acceptance(tmp_path)
    with (tmp_path / relative).open("ab") as handle:
        handle.write(b"tamper")
    with pytest.raises(CurrentHudAcceptanceV23Error, match="predecessor drift"):
        verify_current_hud_acceptance(tmp_path)
...
def test_current_package_selection_is_v23_hud_and_stable_v24() -> None:
    ...
    assert "core.onyx_hud_current_acceptance_v23" in build
    assert "from core.onyx_live_activation_v24 import (" in build
    assert "Packaged V24 preflight" in build
    assert "range(8, 25)" in hygiene
    assert "VE-HUD-CURRENT-V23-E6-001.manifest.json" in hygiene
    assert '"core.onyx_hud_current_acceptance_v23"' in spec
    assert 'bootstrap_onyx_live_v24.pyw"' in bootstrap
