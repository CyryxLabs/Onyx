...
import core.onyx_hud_orb_v14 as hud
import ui
...
def test_v14_authenticates_exact_v13_predecessor() -> None:
    predecessor = ROOT / hud.V13_MODULE_RELATIVE_PATH
    assert hashlib.sha256(predecessor.read_bytes()).hexdigest() == hud.V13_MODULE_SHA256


def test_v14_loads_one_humanoid_shell_root() -> None:
    source = (ROOT / hud.QML_RELATIVE_PATH).read_text(encoding="utf-8")
    assert hud.ROOT_OBJECT == "onyxLiveShellV13Root"
    assert 'objectName: "onyxLiveShellV13Root"' in source
    assert "OnyxHumanoidEntityV10" in source
    assert 'root.findObject(root, "onyxOrbLiquidMetalV9Root")' in source
    assert "predecessorOrb.visible = false" in source


def test_v14_remains_explicit_and_reversible() -> None:
    source = Path(hud.__file__).read_text(encoding="utf-8")
    assert 'return "qml-v14-humanoid-presence"' in source
...
def test_v14_renders_living_humanoid_and_hides_sphere(monkeypatch) -> None:
    app = QApplication.instance() or QApplication(["onyx-v14-humanoid-test"])
    ...
    try:
        ...
        humanoid = root.findChild(QQuickItem, "onyxHumanoidPresenceV10Root")
        webgl = root.findChild(QQuickItem, "onyxHumanoidThreeWebGLV1")
        fallback = root.findChild(QQuickItem, "onyxHumanoidWebGLFallbackV1")
        sphere = root.findChild(QQuickItem, "onyxOrbLiquidMetalV9Root")
        assert root.objectName() == "onyxLiveShellV13Root"
        assert humanoid is not None and humanoid.isVisible()
        assert sphere is not None and not sphere.isVisible()
        assert window._v5_host.renderer_mode == "qml-v14-humanoid-presence"
