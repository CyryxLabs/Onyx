...
from scripts import verify_phase5_approval_inbox_v15 as parent_verifier
from scripts import verify_phase5_approval_inbox_v15_worker as evidence_worker
from scripts import phase5_approval_inbox_v15_historical_projection as historical
from scripts import phase5_approval_inbox_v15_child as isolated_child
...
from core.approval_inbox_v15 import (
    ...
    ApprovalInboxV15State,
    ApprovalInboxV15Unavailable,
...
def item(number: int, **changes: object) -> HostInboxItemV15:
    ...
    return HostInboxItemV15(**values)


class Harness:
    def __init__(
        self,
        items: tuple[HostInboxItemV15, ...] | None = None,
        *,
        enabled: str = "true",
    ):
        self.flag_epoch = 7
        ...
        self.clock = DeterministicInboxClockV15(1_000)
        self.gate = ApprovalInboxFeatureGateV15(
            environ={"ONYX_APPROVAL_INBOX_V15": enabled},
            epoch_reader=lambda: self.flag_epoch,
        )
        self.source = HostInboxSourceV15(
            principal_id="owner-primary",
            session_id="session-local",
            capture_callback=self.capture,
        )
        self.projection = ApprovalInboxProjectionV15(
            gate=self.gate,
            source=self.source,
            clock=self.clock,
        )

    def capture(self) -> HostInboxSnapshotV15:
        self.capture_count += 1
        return HostInboxSnapshotV15(self.source_epoch, 55_000, self.items)
...
class EpochInterlock:
    ...
    def __call__(self) -> int:
        ...
        with self._lock:
            ...
            should_block = (
                threading.current_thread().name == "v15-interleaving"
...
def _thread_call(callable_):
    ...
    thread = threading.Thread(target=run, name="v15-interleaving")
    thread.start()
...
def test_exact_enable_values(value: str) -> None:
    ...
    assert harness.projection.state is ApprovalInboxV15State.READY
...
def test_default_off_never_calls_source(value: str) -> None:
    harness = Harness(enabled=value)
    assert harness.projection.state is ApprovalInboxV15State.DISABLED
    with pytest.raises(ApprovalInboxV15Unavailable, match="disabled"):
...
def test_flag_is_read_once_but_epoch_change_permanently_revokes() -> None:
    environment = {"ONYX_APPROVAL_INBOX_V15": "true"}
    epoch = [4]
    gate = ApprovalInboxFeatureGateV15(
        environ=environment, epoch_reader=lambda: epoch[0]
    )
    environment["ONYX_APPROVAL_INBOX_V15"] = "false"
    assert gate.state() is ApprovalInboxV15State.READY
    epoch[0] = 5
    assert gate.state() is ApprovalInboxV15State.REVOKED
    epoch[0] = 4
    assert gate.state() is ApprovalInboxV15State.REVOKED
...
def test_gate_honors_completed_revocation_after_stale_epoch_callback() -> None:
    ...
    gate = ApprovalInboxFeatureGateV15(
        environ={"ONYX_APPROVAL_INBOX_V15": "true"}, epoch_reader=interlock
    )
    interlock.arm(1)
    ...
    assert not failures
    assert results == [ApprovalInboxV15State.REVOKED]
...
def test_projection_state_cannot_publish_ready_after_completed_revocation() -> None:
    ...
    projection = ApprovalInboxProjectionV15(
        ...
        source=HostInboxSourceV15(
            principal_id="owner-primary",
            session_id="session-local",
            capture_callback=lambda: HostInboxSnapshotV15(1, 1, (item(1),)),
        ),
        clock=DeterministicInboxClockV15(),
    )
    interlock.arm(1)
...
def test_page_contains_complete_safe_review_and_zero_authority() -> None:
    ...
    assert page.state is ApprovalInboxV15State.READY
...
def test_snapshot_reuse_never_extends_ttl() -> None:
    ...
    with pytest.raises(ApprovalInboxV15Unavailable, match="snapshot-stale"):
        harness.projection.open_page()
    assert harness.projection.state is ApprovalInboxV15State.STALE
...
def test_clock_rollback_integrity_latches() -> None:
    ...
    with pytest.raises(ApprovalInboxV15Unavailable, match="clock-integrity-latched"):
        harness.projection.open_page()
    assert harness.projection.state is ApprovalInboxV15State.INTEGRITY_LATCHED
...
def test_source_epoch_rollback_and_equivocation_latch() -> None:
    ...
    with pytest.raises(ApprovalInboxV15Unavailable, match="source-integrity-latched"):
        rollback.projection.open_page()
    assert rollback.projection.state is ApprovalInboxV15State.INTEGRITY_LATCHED
    ...
    with pytest.raises(ApprovalInboxV15Unavailable, match="source-integrity-latched"):
        equivocation.projection.open_page()
    assert equivocation.projection.state is ApprovalInboxV15State.INTEGRITY_LATCHED
...
def test_new_source_epoch_invalidates_all_old_cursors_and_review_proofs() -> None:
    ...
    with pytest.raises(ApprovalInboxV15Unavailable, match="cursor-state-unavailable"):
        harness.projection.continue_page(old.next_cursor)
    with pytest.raises(
        ApprovalInboxV15Unavailable, match="selection-state-unavailable"
    ):
...
def test_feature_epoch_change_during_capture_rejects_callback_output() -> None:
    ...
    def rotate() -> HostInboxSnapshotV15:
        epoch[0] = 2
        return HostInboxSnapshotV15(1, 1, (item(1),))

    gate = ApprovalInboxFeatureGateV15(
        environ={"ONYX_APPROVAL_INBOX_V15": "true"}, epoch_reader=lambda: epoch[0]
    )
    projection = ApprovalInboxProjectionV15(
        gate=gate,
        source=HostInboxSourceV15(
    ...
    with pytest.raises(ApprovalInboxV15Unavailable, match="feature-epoch-revoked"):
        projection.open_page()
    assert projection.state is ApprovalInboxV15State.REVOKED
...
def test_open_page_final_publication_rejects_completed_revocation() -> None:
    ...
    projection = ApprovalInboxProjectionV15(
        ...
        source=HostInboxSourceV15(
            principal_id="owner-primary",
            session_id="session-local",
            capture_callback=lambda: HostInboxSnapshotV15(1, 1, (item(1), item(2))),
        ),
        clock=DeterministicInboxClockV15(),
    )
    interlock.arm(4)
    ...
    assert isinstance(failures[0], ApprovalInboxV15Unavailable)
...
def test_newer_source_commit_suppresses_older_page_and_cache_repopulation() -> None:
    ...
    def capture() -> HostInboxSnapshotV15:
        if threading.current_thread().name == "v15-interleaving":
            return HostInboxSnapshotV15(1, 1, (item(1),))
        return HostInboxSnapshotV15(2, 2, (item(9),))
    ...
    assert isinstance(old_failures[0], ApprovalInboxV15Unavailable)
...
def test_authoritative_core_junction_with_identical_bytes_is_rejected(
    ...
    source = Path(__file__).resolve().parents[1] / "core/approval_inbox_v15.py"
    base = source.parents[1] / f".p52-v15-junction-{secrets.token_hex(8)}"
    clone = base / "clone"
    external_core = base / "external-core"
    external_file = external_core / "approval_inbox_v15.py"
    junction = clone / "core"
    base.mkdir()
    clone.mkdir()
    external_core.mkdir()
    try:
        ...
        with pytest.raises(verifier.PathIntegrityError, match="reparse-component"):
            verifier._safe_path_at(clone, "core/approval_inbox_v15.py")
    finally:
...
def test_source_exception_is_collapsed_without_secret_cause() -> None:
    def fail() -> HostInboxSnapshotV15:
        raise RuntimeError("password=do-not-leak")

    gate = ApprovalInboxFeatureGateV15(
        environ={"ONYX_APPROVAL_INBOX_V15": "1"}, epoch_reader=lambda: 1
    )
    source = HostInboxSourceV15(
        principal_id="owner-primary", session_id="session-local", capture_callback=fail
    )
    projection = ApprovalInboxProjectionV15(
        gate=gate, source=source, clock=DeterministicInboxClockV15()
    )
    with pytest.raises(ApprovalInboxV15Unavailable) as caught:
        projection.open_page()
...
def test_single_flight_callback_is_bounded_and_outside_projection_lock() -> None:
    ...
    def blocked_capture() -> HostInboxSnapshotV15:
        entered.set()
        assert release.wait(2)
        return HostInboxSnapshotV15(1, 1, (item(1),))

    gate = ApprovalInboxFeatureGateV15(
        environ={"ONYX_APPROVAL_INBOX_V15": "1"}, epoch_reader=lambda: epoch[0]
    )
    source = HostInboxSourceV15(
        principal_id="owner-primary",
        session_id="session-local",
    ...
    # the projection lock.
    assert projection.state is ApprovalInboxV15State.READY
    with pytest.raises(ApprovalInboxV15Unavailable, match="source-single-flight-busy"):
        projection.open_page()
    release.set()
    worker.join(2)
...
def test_calm_batch_is_order_independent_idempotent_and_non_authoritative() -> None:
    ...
    page = harness.projection.open_page(query=InboxQueryV15(batch_only=True))
    selections = _all_selections(page)
...
def test_ineligible_items_never_enter_calm_batch(changes: dict[str, object]) -> None:
    ...
    page = harness.projection.open_page()
    with pytest.raises(ApprovalInboxV15ContractError, match="not calm-batch eligible"):
        harness.projection.preview_calm_batch(
            page.snapshot_id, page.view_digest, _all_selections(page)
        )
    filtered = harness.projection.open_page(query=InboxQueryV15(batch_only=True))
...
def test_hidden_unreturned_cross_view_and_tampered_proofs_fail() -> None:
    ...
    second = harness.projection.open_page(
        query=InboxQueryV15(workspace_id="workspace-main"), page_size=1
    )
    selection = _all_selections(first)
    with pytest.raises(
        ApprovalInboxV15ContractError, match="not reviewed in this view"
    ):
        harness.projection.preview_calm_batch(
...
def test_duplicate_item_request_fingerprint_and_idempotency_identity_reject() -> None:
    ...
    with pytest.raises(ApprovalInboxV15ContractError, match="duplicate item_id"):
    ...
    with pytest.raises(
        ApprovalInboxV15ContractError, match="duplicate action_request_id"
    ):
    ...
    with pytest.raises(
        ApprovalInboxV15ContractError, match="duplicate action_fingerprint"
    ):
    ...
    with pytest.raises(
        ApprovalInboxV15ContractError, match="duplicate idempotency_identity"
    ):
...
def test_public_surface_has_no_authority_or_mutation_methods() -> None:
    public = {
        name for name in dir(ApprovalInboxProjectionV15) if not name.startswith("_")
    }
    assert public == {
        "continue_page",
        "handoff_calm_batch",
...
def _owned_root():
    return historical._OwnedTemporaryDirectory(
        prefix=".p52-v15-owned-", dir=historical.ROOT
    )
...
def test_historical_projection_binds_exactly_two_direct_v9_snapshots() -> None:
    ...
    with _owned_root() as directory:
        root = Path(directory)
        copied = historical._materialize_r14(
            __import__(
...
def test_unrelated_historical_drift_still_fails_closed() -> None:
    ...
    with _owned_root() as directory:
        root = Path(directory)
        historical._materialize_r14(module, root)
...
def test_identical_external_snapshot_file_symlink_rejects_before_read() -> None:
    ...
    base = historical.ROOT / f".p52-v15-file-link-{secrets.token_hex(8)}"
    external = historical.ROOT.parent / f".p52-v15-external-{secrets.token_hex(8)}"
    base.mkdir()
...
def test_authority_directory_junction_and_aliased_root_reject() -> None:
    base = historical.ROOT / f".p52-v15-junction-{secrets.token_hex(8)}"
    actual = base / "actual"
    link = base / "linked"
...
def test_output_target_link_and_preexisting_target_reject_before_write() -> None:
    with _owned_root() as directory:
        ...
        external = historical.ROOT.parent / f".p52-v15-target-{secrets.token_hex(8)}"
...
def test_output_ancestor_junction_and_unowned_root_reject() -> None:
    with _owned_root() as directory:
        root = Path(directory)
        external = historical.ROOT / f".p52-v15-target-dir-{secrets.token_hex(8)}"
        external.mkdir()
        link = root / "linked"
        try:
            if sys.platform == "win32":
    ...
    unowned = historical.ROOT / f".p52-v15-unowned-{secrets.token_hex(8)}"
    unowned.mkdir()
    try:
        with pytest.raises(
            historical.HistoricalProjectionPathError,
...
def test_materialized_git_is_private_minimal_and_has_no_live_inputs() -> None:
    ...
    with _owned_root() as directory:
        root = Path(directory)
        historical._materialize_r14(module, root)
        assert (root / ".git/HEAD").read_text() == "ref: refs/heads/onyx-v15\n"
        assert (
            root / ".git/refs/heads/onyx-v15"
...
def test_historical_loader_ignores_forged_sys_modules(
    ...
    try:
        ...
        assert loaded.__name__.startswith("_onyx_p52_v15_historical_verifier_v4_")
...
def test_private_module_cache_collision_is_removed_and_rejected(
    ...
    private = "_onyx_p52_v15_historical_verifier_v4_" + "a" * 32
...
def test_exact_child_plugin_ignores_conventional_preloaded_module() -> None:
    ...
    conventional = "scripts.phase5_approval_inbox_v15_historical_projection"
    ...
    assert loaded.__name__ == "_onyx_p52_v15_exact_historical_plugin"
    assert loaded.__name__ not in sys.modules
...
def test_exact_child_plugin_rejects_and_removes_private_cache_collision() -> None:
    ...
    private = "_onyx_p52_v15_exact_historical_plugin"
    sys.modules[private] = type(sys)(private)
    with pytest.raises(
...
def test_global_deadline_terminates_descendant_tree(
    ...
    with tempfile.TemporaryDirectory(dir=historical.ROOT) as directory:
        ...
        with pytest.raises(SystemExit, match="deadline-tree-terminated"):
            evidence_worker.run(
                [sys.executable, "-c", code], cwd=historical.ROOT
            )
...
def test_child_rejects_root_symlink_before_file_lookup(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with tempfile.TemporaryDirectory(dir=historical.ROOT.parent) as directory:
        ...
        monkeypatch.setattr(isolated_child, "ROOT", link)
        with pytest.raises(
            isolated_child.ChildBoundaryError, match="reparse-or-aliased-root"
        ):
            isolated_child._safe_root()


@pytest.mark.skipif(os.name != "nt", reason="Windows junction contract")
def test_child_rejects_root_junction_before_file_lookup(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with tempfile.TemporaryDirectory(dir=historical.ROOT.parent) as directory:
        parent = Path(directory)
        ...
        monkeypatch.setattr(isolated_child, "ROOT", junction)
        with pytest.raises(
            isolated_child.ChildBoundaryError, match="reparse-or-aliased-root"
        ):
            isolated_child._safe_root()
...
def test_child_rejects_pytest_shadow_inside_root(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with tempfile.TemporaryDirectory(dir=historical.ROOT.parent) as directory:
        root = Path(directory)
        (root / "pytest.py").write_text("raise RuntimeError('shadow')\n")
        monkeypatch.setattr(isolated_child, "ROOT", root)
        monkeypatch.setenv("PYTEST_DISABLE_PLUGIN_AUTOLOAD", "1")
        for name in (
            "PYTEST_ADDOPTS",
        ...
        with pytest.raises(
            isolated_child.ChildBoundaryError,
            match="prohibited-ambient-input:pytest.py",
        ):
            isolated_child._verify_ambient_boundary()


def test_private_pytest_runtime_is_bound_and_rejects_substitution() -> None:
    runtime = isolated_child.ROOT / isolated_child.RUNTIME_RELATIVE
    digest = hashlib.sha256(runtime.read_bytes()).hexdigest()
    with zipfile.ZipFile(runtime) as archive:
        manifest = archive.read(isolated_child.RUNTIME_MANIFEST_MEMBER)
    manifest_digest = hashlib.sha256(manifest).hexdigest()
    path, identity = isolated_child._validate_runtime_archive(digest, manifest_digest)
    assert path == runtime
    assert isolated_child._file_identity(path) == identity
    with pytest.raises(
        isolated_child.ChildBoundaryError, match="pytest-runtime-digest-drift"
    ):
        isolated_child._validate_runtime_archive("0" * 64, manifest_digest)
...
def test_git_environment_overrides_are_removed(name: str) -> None:
    cleaned = historical._sanitized_environment({name: "attacker", "SAFE": "yes"})
    assert name not in cleaned
    assert cleaned == {"SAFE": "yes"}


def test_historical_git_queries_use_internal_private_tree_only() -> None:
    module = historical._load_guarded_historical_verifier(4)
    historical._install_projection_wrapper(module)
    with _owned_root() as directory:
        root = Path(directory)
        historical._materialize_r14(module, root)
        result = module.subprocess.run(
            ...
            capture_output=True,
            text=True,
            env={"GIT_DIR": str(historical.ROOT / ".git")},
        )
        assert result.returncode == 0
        assert result.stdout.strip() == historical.FROZEN_BASE_COMMIT
        with pytest.raises(
            historical.HistoricalProjectionPathError,
            match="git-command-not-authorized",
        ):
            module.subprocess.run(["git", "status"], cwd=root)


def test_parent_verifier_requires_full_isolated_python_flags() -> None:
    result = evidence_worker.subprocess.run(
        [sys.executable, "-B", str(Path(parent_verifier.__file__).absolute())],
        cwd=parent_verifier.ROOT,
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
        env=evidence_worker._sanitize_process_environment(dict(os.environ)),
        check=False,
    )
    assert result.returncode != 0
    assert "python-not-isolated-use--I--S--B" in result.stdout


def test_worker_environment_drops_path_python_git_and_startup_poisoning() -> None:
    poisoned = {
        "PATH": r"C:\attacker",
        ...
        "GIT_CONFIG_GLOBAL": r"C:\attacker\.gitconfig",
        "SystemRoot": os.environ.get("SystemRoot", r"C:\Windows"),
    }
    cleaned = evidence_worker._sanitize_process_environment(poisoned)
    assert cleaned["PATH"] == ""
    assert cleaned["PYTEST_DISABLE_PLUGIN_AUTOLOAD"] == "1"
    assert cleaned["PYTHONDONTWRITEBYTECODE"] == "1"
    assert cleaned["SystemRoot"] == poisoned["SystemRoot"]
    assert not any(
        name in cleaned
        for name in (
            "PYTHONPATH",
...
@pytest.mark.parametrize(
    "relative",
    ["module.pyc", "module.pyo", "pkg/__pycache__/module.cpython-313.pyc"],
)
def test_worker_rejects_bytecode_from_materialization_authority(
    relative: str,
) -> None:
    with pytest.raises(SystemExit, match="bytecode-prohibited"):
        evidence_worker._reject_bytecode_relative(relative)


def _write_test_materialized_manifest(root: Path, files: dict[str, str]) -> str:
    payload = (
        json.dumps(
            {"contract": "OnyxMaterializedAuthority.v15", "files": files},
            sort_keys=True,
            separators=(",", ":"),
        )
        + "\n"
    ).encode()
    path = root / isolated_child.MATERIALIZED_MANIFEST
    path.write_bytes(payload)
    return hashlib.sha256(payload).hexdigest()


def test_child_manifest_rejects_timestamp_bytecode_even_when_hash_bound(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with tempfile.TemporaryDirectory(dir=historical.ROOT.parent) as directory:
        root = Path(directory)
        bytecode = root / "poison.cpython-313.pyc"
        bytecode.write_bytes(b"timestamp-pyc-poison")
        digest = hashlib.sha256(bytecode.read_bytes()).hexdigest()
        manifest_digest = _write_test_materialized_manifest(
            root, {bytecode.name: digest}
        )
        monkeypatch.setattr(isolated_child, "ROOT", root)
        with pytest.raises(
            isolated_child.ChildBoundaryError, match="materialized-manifest-path"
        ):
            isolated_child._load_materialized_manifest(manifest_digest)


def test_child_manifest_rejects_unbound_package_initializer_poisoning(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with tempfile.TemporaryDirectory(dir=historical.ROOT.parent) as directory:
        root = Path(directory)
        ...
        package = root / "poisoned_package"
        package.mkdir()
        (package / "__init__.py").write_text(
            "raise RuntimeError('ambient package initializer executed')\n",
            encoding="utf-8",
        )
        manifest_digest = _write_test_materialized_manifest(
            root, {"bound.txt": hashlib.sha256(bound.read_bytes()).hexdigest()}
        )
        monkeypatch.setattr(isolated_child, "ROOT", root)
        with pytest.raises(
            isolated_child.ChildBoundaryError, match="materialized-extra-files"
        ):
            isolated_child._load_materialized_manifest(manifest_digest)


def test_child_postrun_scan_rejects_new_bytecode(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with tempfile.TemporaryDirectory(dir=historical.ROOT.parent) as directory:
        root = Path(directory)
        (root / "late.pyc").write_bytes(b"forbidden-runtime-output")
        monkeypatch.setattr(isolated_child, "ROOT", root)
        with pytest.raises(isolated_child.ChildBoundaryError, match="bytecode-present"):
            isolated_child._validate_no_bytecode({})


def test_dependency_copy_rejects_hash_substitution_before_materialization() -> None:
    with tempfile.TemporaryDirectory(dir=historical.ROOT.parent) as directory:
        root = Path(directory)
        source = root / "dependency.py"
        target = root / "clean" / "dependency.py"
        source.write_bytes(b"replacement")
        with pytest.raises(SystemExit, match="materialization-source-drift"):
            evidence_worker._exclusive_copy(source, target, "0" * 64)
        assert not target.exists()


def test_child_rejects_arbitrary_pytest_options_and_external_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with tempfile.TemporaryDirectory(dir=historical.ROOT.parent) as directory:
        ...
        with pytest.raises(
            isolated_child.ChildBoundaryError,
            match="pytest-arguments-not-authorized",
        ):
            isolated_child._validated_pytest_args(
                [
                    *isolated_child.FOCUSED,
                    "-q",
                    "-p",
                    "attacker",
                    f"--rootdir={historical.ROOT}",
                ]
            )
        assert isolated_child._validated_pytest_args(
            [
                *isolated_child.FOCUSED,
                "-q",
                "-p",
...
def test_child_junit_output_is_limited_to_owned_sandbox(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with tempfile.TemporaryDirectory(dir=historical.ROOT.parent) as directory:
        root = Path(directory) / "root"
        root.mkdir()
        monkeypatch.setattr(isolated_child, "ROOT", root)
        allowed = str(root.parent / "home" / "junit" / "focused.xml")
        fixed = [
            *isolated_child.FOCUSED,
            "-q",
            "-p",
        ...
        assert isolated_child._validated_pytest_args(
            [*fixed, f"--junitxml={allowed}"]
        )[-1] == f"--junitxml={allowed}"
        with pytest.raises(
            isolated_child.ChildBoundaryError,
            match="pytest-arguments-not-authorized",
        ):
            isolated_child._validated_pytest_args(
                [*fixed, f"--junitxml={historical.ROOT / 'escape.xml'}"]
            )


def test_stdlib_precedes_project_and_dependency_shadow_candidates(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with tempfile.TemporaryDirectory(dir=historical.ROOT.parent) as directory:
        root = Path(directory) / "root"
        dependency = root / isolated_child.DEPENDENCY_ROOT
        dependency.mkdir(parents=True)
        poison = dependency / "json.py"
        poison.write_text("raise RuntimeError('stdlib shadowed')\n", encoding="utf-8")
        runtime = root / "runtime.zip"
        runtime.write_bytes(b"not-consulted-before-stdlib")
        monkeypatch.setattr(isolated_child, "ROOT", root)
        stdlib = str(Path(json.__file__).resolve().parents[1])
        authoritative = isolated_child._authoritative_sys_path(
            runtime, dependency, [stdlib]
        )
        assert authoritative[-3:] == [str(runtime), str(root), str(dependency)]
        assert str(Path(sys.base_prefix).resolve()) not in authoritative
        spec = importlib.machinery.PathFinder.find_spec("json", authoritative)
        assert spec is not None
        assert spec.origin is not None
        assert Path(spec.origin).resolve() != poison.resolve()
        assert "site-packages" not in {
            part.casefold()
            for entry in authoritative[:-3]
            for part in Path(entry).parts
        }


def test_real_base_prefix_shadow_entry_is_rejected(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    base_prefix = str(Path(sys.base_prefix).resolve())
    monkeypatch.setattr(sys, "path", [base_prefix, *sys.path])
    assert base_prefix in {
        str(Path(value).absolute()) for value in sys.path if value
    }
    with pytest.raises(
        isolated_child.ChildBoundaryError,
        match="stdlib-entry-base-prefix",
    ):
        isolated_child._validated_stdlib_entries(
            [[base_prefix, "directory"]]
        )


def test_worker_requires_exact_semantic_stdlib_entry_set() -> None:
    executable = Path(sys.executable).absolute()
    roots: list[str] = []
    for name in ("stdlib", "platstdlib"):
        value = str(
            Path(evidence_worker.sysconfig.get_paths()[name]).resolve(strict=True)
        )
        if value not in roots:
            roots.append(value)
    entries = evidence_worker._expected_python_stdlib_entries()
    base_prefix = str(Path(sys.base_prefix).resolve())
    assert base_prefix not in {entry["path"] for entry in entries}
    poisoned = {
        "python_host_tcb": {
            ...
            "stdlib_entries": [
                *entries,
                {"path": base_prefix, "kind": "directory"},
            ],
        }
    }
    with pytest.raises(SystemExit, match="python-tcb-stdlib-entries"):
        evidence_worker._validate_python_tcb(poisoned)


@pytest.mark.parametrize(
    "relative",
    ["core/__init__.py", "actions/tool.py", "tests/test_poison.py", "main.py"],
)
def test_dependency_namespaces_cannot_collide_with_workspace_authority(
    relative: str,
) -> None:
    with pytest.raises(SystemExit, match="dependency-namespace-collision"):
        evidence_worker._reject_dependency_namespace_collision(relative)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"shell": True},
        {"cwd": str(historical.ROOT.parent)},
        {"creationflags": 0},
    ],
)
def test_junction_fixture_rejects_process_option_injection(
    poisoned_option: dict[str, object],
) -> None:
    with _owned_root() as directory:
        root = Path(directory)
        target = root / "target"
        target.mkdir()
        link = root / "link"
        proxy = historical._HermeticSubprocessProxy(owner_module=historical)
        with pytest.raises(
            historical.HistoricalProjectionPathError,
            match="process-options-not-authorized",
        ):
            proxy.run(
...
def _minimal_materialization_bundle(root: Path) -> dict[str, object]:
    workspace = root / "workspace.txt"
    workspace.write_bytes(b"workspace-authority")
    dependencies = root / "host-dependencies"
    dependencies.mkdir()
    dependency = dependencies / "dependency.py"
    dependency.write_bytes(b"VALUE = 1\n")
    return {
        "materialized_tree": {
            "contract": "OnyxCleanMaterializedTree.v15",
            "files": {
                "workspace.txt": hashlib.sha256(workspace.read_bytes()).hexdigest()
            },
        },
        "host_dependency_tcb": {
            "contract": "OnyxHostDependencyTCB.v15",
            "source_roots": {"root_0": str(dependencies)},
            "files": {
                "dependency.py": {
                    "root": "root_0",
...
def test_clean_materialized_sandbox_is_removed_after_success_and_failure(
    monkeypatch: pytest.MonkeyPatch, raise_inside: bool
) -> None:
    with tempfile.TemporaryDirectory(dir=historical.ROOT.parent) as directory:
        authority = Path(directory) / "authority"
        authority.mkdir()
        bundle = _minimal_materialization_bundle(authority)
        monkeypatch.setattr(evidence_worker, "ROOT", authority)
        sandbox: Path | None = None
        if raise_inside:
            with pytest.raises(RuntimeError, match="intentional-materialized-failure"):
                with evidence_worker.materialized_tree(bundle) as clean:
                    sandbox = clean.parent
                    raise RuntimeError("intentional-materialized-failure")
        else:
            with evidence_worker.materialized_tree(bundle) as clean:
                sandbox = clean.parent
                assert (clean / "workspace.txt").read_bytes() == b"workspace-authority"
        assert sandbox is not None
        assert not sandbox.exists()


def test_clean_materialized_sandbox_is_removed_after_process_timeout(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with tempfile.TemporaryDirectory(dir=historical.ROOT.parent) as directory:
        authority = Path(directory) / "authority"
        ...
        with evidence_worker.materialized_tree(bundle) as clean:
            sandbox = clean.parent
            monkeypatch.setattr(
                evidence_worker, "_GLOBAL_DEADLINE", time.monotonic() + 0.5
            )
            with pytest.raises(SystemExit, match="deadline-tree-terminated"):
                evidence_worker.run(
