...
from core.capability_nexus_v15 import (
    CATALOG_CONTRACT_VERSION_V15,
    LOCK_ORDER_V15,
    CancellationTokenV15,
    CapabilityDescriptorV15,
    CapabilityNexusV15,
    CapabilityProjectionV15,
    CapabilityStatusV15,
    CapabilityV15ContractError,
    CapabilityV15DeniedError,
    CatalogItemV15,
    CatalogReadRequestV15,
    LocalCatalogReadAdapterV15,
    NexusFeatureGateV15,
    OperationDescriptorV15,
    OperationKindV15,
    ReadFailureClassV15,
    ReadStateV15,
    TransportKindV15,
    build_legacy_descriptors_v15,
)
import core.capability_nexus_v15 as nexus_v15_module
import scripts.verify_phase5_capability_nexus_v15 as verifier_v15
...
def _operation(**changes) -> OperationDescriptorV15:
    values = {
        "operation_id": "catalog_read", "kind": OperationKindV15.READ,
    ...
    return OperationDescriptorV15(**values)


def _descriptor(**changes) -> CapabilityDescriptorV15:
    values = {
        "capability_id": "local.catalog", "capability_version": "v15", "provider": "onyx_local",
        "transport": TransportKindV15.LOCAL, "api_name": "local_catalog",
        "api_version": CATALOG_CONTRACT_VERSION_V15, "workspace_id": WORKSPACE,
        "account_id": ACCOUNT, "profile_id": PROFILE, "credential_alias": None,
        "operations": (_operation(),), "status": CapabilityStatusV15.DISABLED,
        "status_reason": "candidate_not_activated", "limitations": ("no_dispatch",),
    }
    values.update(changes)
    return CapabilityDescriptorV15(**values)


def _items(count: int = 5) -> tuple[CatalogItemV15, ...]:
    return tuple(CatalogItemV15(f"item-{i:02d}", f"Item {i}", "fixture", "v15", ("safe",)) for i in range(count))


def _adapter(hook=lambda: None, **changes) -> LocalCatalogReadAdapterV15:
    values = {
        "items": _items(), "workspace_id": WORKSPACE, "account_id": ACCOUNT,
        "profile_id": PROFILE, "cursor_signing_key": KEY, "read_hook": hook,
        "status": CapabilityStatusV15.AVAILABLE_READ_ONLY, "status_reason": "healthy",
    }
    values.update(changes)
    return LocalCatalogReadAdapterV15(**values)


def _request(**changes) -> CatalogReadRequestV15:
    values = {
        "workspace_id": WORKSPACE, "account_id": ACCOUNT, "profile_id": PROFILE,
        "target": "local_catalog", "correlation_id": "corr-001", "page_size": 2,
    }
    values.update(changes)
    return CatalogReadRequestV15(**values)
...
def test_registry_requires_profile_and_binds_discovery_exactly():
    nexus = CapabilityNexusV15()
    descriptor = _descriptor()
    nexus.register(descriptor)
    with pytest.raises(TypeError):
        nexus.discover(
            descriptor.capability_id, workspace_id=WORKSPACE, account_id=ACCOUNT,
            expected_version="v15", expected_digest=descriptor.digest,
        )
    for profile in ("profile-other", "PROFILE-alpha"):
        with pytest.raises(CapabilityV15DeniedError):
            nexus.discover(
                descriptor.capability_id, workspace_id=WORKSPACE, account_id=ACCOUNT,
                profile_id=profile, expected_version="v15", expected_digest=descriptor.digest,
            )
...
def test_projection_constructor_rejects_cross_profile_and_authority():
    descriptor = _descriptor()
    with pytest.raises(CapabilityV15ContractError):
        CapabilityProjectionV15(
            descriptor, descriptor.digest, WORKSPACE, ACCOUNT, "profile-other",
            CapabilityStatusV15.DISABLED, "disabled",
        )
    with pytest.raises(CapabilityV15ContractError):
        CapabilityProjectionV15(
            descriptor, descriptor.digest, WORKSPACE, ACCOUNT, PROFILE,
            CapabilityStatusV15.DISABLED, "disabled", authority_granted=True,
        )


class _GateSubclass(NexusFeatureGateV15):
    pass
...
def test_registry_rejects_gate_subclasses_fakes_and_duck_types(gate):
    with pytest.raises(CapabilityV15ContractError, match="exact concrete"):
        CapabilityNexusV15(gate)  # type: ignore[arg-type]


def test_exact_gate_is_frozen_default_off_and_lock_order_is_declared():
    gate = NexusFeatureGateV15()
    nexus = CapabilityNexusV15(gate)
    assert nexus.gate is gate and nexus.lock_order == LOCK_ORDER_V15
    with pytest.raises(FrozenInstanceError):
        gate.shadow_mode = False  # type: ignore[misc]
    for values in ({"nexus_enabled": True}, {"shadow_mode": False}, {"dispatch_enabled": True}):
        with pytest.raises(CapabilityV15ContractError):
            NexusFeatureGateV15(**values)
...
@pytest.mark.parametrize("prefix", sorted(nexus_v15_module._SECRET_NAMES))
...
@pytest.mark.parametrize("name", sorted(nexus_v15_module._SECRET_NAMES))
...
@pytest.mark.parametrize("name", sorted(nexus_v15_module._SECRET_NAMES | nexus_v15_module._CREDENTIAL_OPAQUE_NAMES))
@pytest.mark.parametrize("position", ["leading", "middle", "trailing"])
...
def test_embedded_ngram_model_accepts_held_out_natural_boundary_corpus(word):
    assert nexus_v15_module._english_word_score(word) >= nexus_v15_module._NATURAL_SCORE_THRESHOLD
...
def test_schema_rejects_raw_credentials_in_nested_defaults_examples_and_descriptions(value):
    for field in ("default", "example", "description"):
        ...
        encoded = nexus_v15_module.json.dumps(schema, sort_keys=True, separators=(",", ":"))
        with pytest.raises(CapabilityV15ContractError):
...
def test_recursive_schema_cycle_depth_node_and_scalar_budgets_fail_closed():
    cycle = {}
    cycle["safe"] = cycle
    with pytest.raises(CapabilityV15ContractError, match="cycle"):
        nexus_v15_module._reject_json_secrets(cycle, "fixture")
    deep = current = {}
    for index in range(26):
        current["safe"] = {}
        current = current["safe"]
    with pytest.raises(CapabilityV15ContractError):
        nexus_v15_module._reject_json_secrets(deep, "fixture")
    with pytest.raises(CapabilityV15ContractError, match="node"):
        nexus_v15_module._reject_json_secrets(0, "fixture", _budget=[4096, 0])
    with pytest.raises(CapabilityV15ContractError, match="scalar byte"):
        nexus_v15_module._reject_json_secrets(["a" * 4096] * 17, "fixture")
...
def test_every_non_hash_metadata_key_is_a_closed_enum(key, value):
    with pytest.raises(CapabilityV15ContractError, match="closed enum"):
        _descriptor(metadata=((key, value),))
...
def test_metadata_enum_table_is_immutable_and_future_values_require_source_change():
    assert dict(nexus_v15_module._METADATA_ENUMS) == {
    ...
    with pytest.raises(TypeError):
        nexus_v15_module._METADATA_ENUMS["data_source"] = frozenset({"future_catalog"})
    with pytest.raises(AttributeError):
        nexus_v15_module._METADATA_ENUMS["data_source"].add("future_catalog")
...
def test_benign_human_text_remains_valid_only_in_description_context(description):
    ...
    encoded = nexus_v15_module.json.dumps(schema, sort_keys=True, separators=(",", ":"), ensure_ascii=False)
...
def test_content_decoding_cycle_detection_fails_closed(monkeypatch):
    ...
    monkeypatch.setattr(
        nexus_v15_module, "_canonical_percent_decode", lambda value: transitions.get(value)
    )
    with pytest.raises(CapabilityV15ContractError, match="cycle"):
        nexus_v15_module._validate_content_decoding_closure("cycle_a")
...
def test_adapter_defaults_disabled_and_has_no_dispatch_or_mutation():
    adapter = LocalCatalogReadAdapterV15(
        _items(), workspace_id=WORKSPACE, account_id=ACCOUNT, profile_id=PROFILE,
        cursor_signing_key=KEY, read_hook=lambda: None,
    )
    assert adapter.health().status is CapabilityStatusV15.DISABLED
    assert adapter.read_page(_request()).state is ReadStateV15.DENIED
...
def test_hook_runs_without_adapter_lock_and_can_reenter_health():
    ...
    assert result.state is ReadStateV15.COMPLETED
    assert observed == [CapabilityStatusV15.AVAILABLE_READ_ONLY]
...
def test_same_correlation_reentry_is_denied_before_second_hook_or_reservation():
    ...
    def hook():
        calls.append(1)
        with pytest.raises(CapabilityV15DeniedError, match="reentrancy"):
            holder["adapter"].read_page(request)

    holder["adapter"] = _adapter(hook)
    assert holder["adapter"].read_page(request).state is ReadStateV15.COMPLETED
...
def test_new_correlation_and_indirect_hook_reentry_are_denied_without_quota_drift():
    ...
    assert holder["adapter"].read_page(_request()).state is ReadStateV15.COMPLETED
    limited = holder["adapter"].read_page(_request(correlation_id="corr-after"))
    assert limited.state is ReadStateV15.RATE_LIMITED
...
def test_hook_exception_cleans_reentrancy_guard_for_later_correlation():
    ...
    assert adapter.read_page(_request()).state is ReadStateV15.UNCERTAIN
    assert adapter.read_page(_request(correlation_id="corr-002")).state is ReadStateV15.COMPLETED
    assert calls == [1, 1]
...
def test_reentrancy_guard_rejects_hook_spawned_thread_before_reservation():
    ...
    def hook():
        ...
        with ThreadPoolExecutor(max_workers=1) as pool:
            ...
            except CapabilityV15DeniedError as exc:
                nested_errors.append(str(exc))

    holder["adapter"] = _adapter(hook)
    outer = holder["adapter"].read_page(_request())
    assert outer.state is ReadStateV15.COMPLETED
    assert nested_errors == ["catalog read reentrancy is denied"]
    assert len(calls) == 1 and holder["adapter"].health().quota_remaining == 99
    assert holder["adapter"].read_page(_request(correlation_id="corr-after")).state is ReadStateV15.COMPLETED
...
def test_hook_guard_is_per_adapter_and_does_not_hold_lock_during_callback():
    ...
    assert first.read_page(_request()).state is ReadStateV15.COMPLETED
    assert observed == [ReadStateV15.COMPLETED]


def test_historical_leaf_budget_exact_boundary_and_overflow():
    leaves = {f"leaf-{index}": "a" * 64 for index in range(4095)}
    verifier_v15._record_historical_leaf(leaves, "boundary", "b" * 64)
    assert len(leaves) == 4096
    with pytest.raises(verifier_v15.V15EvidenceError, match="leaf budget"):
        verifier_v15._record_historical_leaf(leaves, "overflow", "c" * 64)
    with pytest.raises(verifier_v15.V15EvidenceError, match="digest conflict"):
        verifier_v15._record_historical_leaf(leaves, "boundary", "d" * 64)
...
def test_wide_historical_manifest_and_cycle_fail_before_expansion(monkeypatch):
    ...
    monkeypatch.setattr(verifier_v15, "_historical_manifest", lambda relative: wide)
    monkeypatch.setattr(verifier_v15, "_verify_historical_path", lambda relative, digest: None)
    with pytest.raises(verifier_v15.V15EvidenceError, match="leaf budget"):
        verifier_v15._verify_manifest_tree("wide.sha256", set(), set(), {}, 0)

    graph = {
        "a.sha256": (("a" * 64, "b.sha256"),),
        "b.sha256": (("b" * 64, "a.sha256"),),
    }
    monkeypatch.setattr(verifier_v15, "_historical_manifest", graph.__getitem__)
    with pytest.raises(verifier_v15.V15EvidenceError, match="cycle"):
        verifier_v15._verify_manifest_tree("a.sha256", set(), set(), {}, 0)


def test_exact_4096_leaf_manifest_is_accepted(monkeypatch):
    entries = tuple(("a" * 64, f"leaf-{index}") for index in range(4096))
    monkeypatch.setattr(verifier_v15, "_historical_manifest", lambda relative: entries)
    monkeypatch.setattr(verifier_v15, "_verify_historical_path", lambda relative, digest: None)
    leaves = {}
    verifier_v15._verify_manifest_tree("boundary.sha256", set(), set(), leaves, 0)
    assert len(leaves) == 4096


def test_historical_manifest_rejects_path_traversal_and_symlink(monkeypatch):
    with tempfile.TemporaryDirectory(prefix="p53-v15-path-", dir=ROOT) as directory:
        base = Path(directory)
        monkeypatch.setattr(verifier_v15, "PROJECT", base)
        manifest = base / "bad.sha256"
        manifest.write_text("" + "a" * 64 + "  ../escape\n", encoding="utf-8")
        with pytest.raises(verifier_v15.V15EvidenceError, match="unsafe"):
            verifier_v15._historical_manifest("bad.sha256")
        ...
        verifier_v15._verify_historical_path(
        ...
        with pytest.raises(verifier_v15.V15EvidenceError, match="reparse"):
            verifier_v15._verify_historical_path("link.txt", hashlib.sha256(target.read_bytes()).hexdigest())
...
def test_concurrent_duplicate_is_pending_then_completed_and_hook_once():
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        release.set()
        assert future.result().state is ReadStateV15.COMPLETED
...
def test_validation_exception_releases_admission_claim():
    ...
    assert adapter.read_page(_request()).state is ReadStateV15.COMPLETED
...
def test_same_correlation_payload_drift_denies_without_hook_or_quota():
    ...
    with pytest.raises(CapabilityV15DeniedError, match="drift"):
...
def test_hook_exception_becomes_uncertain_and_never_replays_or_leaks_exception():
    ...
    assert result.state is ReadStateV15.UNCERTAIN
    assert result.failure_class is ReadFailureClassV15.HOOK_EXCEPTION
...
def test_cancelled_before_hook_rolls_back_quota_and_rate_and_never_calls_hook():
    ...
    assert result.state is ReadStateV15.CANCELLED_BEFORE_HOOK
...
def test_timeout_after_hook_is_uncertain_and_not_replayed():
    ...
    result = adapter.read_page(request)
    assert result.state is ReadStateV15.UNCERTAIN
    assert result.failure_class is ReadFailureClassV15.TIMEOUT
    assert adapter.read_page(request) == result and calls == [1]


@pytest.mark.parametrize(("action", "failure"), [("kill", ReadFailureClassV15.KILL), ("revoke", ReadFailureClassV15.REVOKED)])
def test_kill_and_revoke_during_hook_latch_uncertain(action, failure):
    ...
    result = holder["adapter"].read_page(_request())
    assert result.state is ReadStateV15.UNCERTAIN and result.failure_class is failure
...
def test_pending_expiry_becomes_uncertain_and_late_hook_cannot_commit():
    ...
    with ThreadPoolExecutor(max_workers=1) as pool:
        ...
        assert reconciled.state is ReadStateV15.UNCERTAIN
        assert reconciled.failure_class is ReadFailureClassV15.EXPIRED_PENDING
        release.set()
        assert future.result().state is ReadStateV15.UNCERTAIN
...
def test_uncertain_expiry_keeps_no_replay_tombstone_and_frees_uncertain_capacity():
    ...
    assert adapter.read_page(first_request).state is ReadStateV15.UNCERTAIN
    with pytest.raises(CapabilityV15DeniedError, match="pending/uncertain"):
        adapter.read_page(_request(correlation_id="corr-002"))
    now[0] = 2.0
    expired = adapter.reconcile(first_request)
    assert expired.state is ReadStateV15.EXPIRED_UNCERTAIN
    assert expired.failure_class is ReadFailureClassV15.UNCERTAIN_EXPIRED
    assert adapter.read_page(first_request) == expired and calls == [1]
    assert adapter.read_page(_request(correlation_id="corr-002")).state is ReadStateV15.UNCERTAIN
...
def test_ledger_capacity_is_bounded_and_does_not_evict_for_replay():
    ...
    request = _request()
    assert adapter.read_page(request).state is ReadStateV15.COMPLETED
    with pytest.raises(CapabilityV15DeniedError, match="capacity"):
        adapter.read_page(_request(correlation_id="corr-002"))
    assert adapter.read_page(request).state is ReadStateV15.COMPLETED
...
def test_cursor_snapshot_and_signature_tamper_fail_closed():
    ...
    changed = LocalCatalogReadAdapterV15(
        _items(6), workspace_id=WORKSPACE, account_id=ACCOUNT, profile_id=PROFILE,
        cursor_signing_key=KEY, read_hook=lambda: None, status=CapabilityStatusV15.AVAILABLE_READ_ONLY,
...
def test_rate_limit_and_quota_are_reserved_once_and_truthful():
    ...
    assert limited.state is ReadStateV15.RATE_LIMITED and limited.retry_after_ms == 1000
...
def test_unavailable_auth_scope_disabled_degraded_and_unknown_reconcile():
    assert _adapter(auth_available=False).read_page(_request()).failure_class is ReadFailureClassV15.AUTH
    assert _adapter(granted_scopes=()).read_page(_request()).failure_class is ReadFailureClassV15.SCOPE
    assert _adapter(status=CapabilityStatusV15.DISABLED, status_reason="operator_disabled").read_page(_request()).state is ReadStateV15.DENIED
    degraded = _adapter(status=CapabilityStatusV15.DEGRADED, status_reason="partial_fixture")
    assert degraded.read_page(_request()).state is ReadStateV15.COMPLETED
    unknown = degraded.reconcile(_request(correlation_id="unknown-corr"))
    assert unknown.state is ReadStateV15.UNKNOWN_CORRELATION
...
def test_legacy_builder_preserves_current_32_declarations_and_policies_without_import():
    ...
    policies = _assignment(ROOT / "core" / "permission_broker.py", "MODEL_TOOL_POLICIES")
    built = build_legacy_descriptors_v15(
        declarations, policies, workspace_id=WORKSPACE, account_id=ACCOUNT, profile_id=PROFILE,
    )
    ...
    assert all(item.status is CapabilityStatusV15.DISABLED for item in built.descriptors)
...
def test_no_live_runtime_source_references_v15():
    live = [ROOT / "main.py", ROOT / "ui.py", ROOT / "scripts" / "launch_onyx.pyw"]
    live += list((ROOT / "actions").rglob("*.py")) + list((ROOT / "dashboard").rglob("*.py"))
    live += [path for path in (ROOT / "core").glob("*.py") if path.name != "capability_nexus_v15.py"]
    assert all("capability_nexus_v15" not in path.read_text(encoding="utf-8", errors="ignore") for path in live)


def test_bounded_subprocess_timeout_is_fail_closed_and_never_retried(monkeypatch):
    calls = []
    ...
    monkeypatch.setattr(verifier_v15.subprocess, "run", expire)
    with pytest.raises(verifier_v15.V15EvidenceError, match="timed out after 7s"):
        verifier_v15._run_bounded(["fixture"], label="fixture subprocess", timeout=7)
    assert len(calls) == 1 and calls[0][1]["timeout"] == 7 and calls[0][1]["check"] is False


def test_invalid_subprocess_timeout_fails_before_process_start(monkeypatch):
    called = False
    ...
    monkeypatch.setattr(verifier_v15.subprocess, "run", unexpected)
    with pytest.raises(verifier_v15.V15EvidenceError, match="timeout is invalid"):
        verifier_v15._run_bounded(["fixture"], label="fixture subprocess", timeout=0)
    assert called is False


def test_every_verifier_subprocess_is_centralized_and_every_call_has_explicit_timeout():
    tree = ast.parse((ROOT / "scripts/verify_phase5_capability_nexus_v15.py").read_text(encoding="utf-8"))
    raw_calls = [
    ...
    bounded_calls = [
        node for node in ast.walk(tree)
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id == "_run_bounded"
    ]
...
def test_frozen_history_proof_never_spawns_a_recursive_verifier(monkeypatch):
    def forbidden(*args, **kwargs):
        raise AssertionError("frozen proof must not spawn a subprocess")

    monkeypatch.setattr(verifier_v15, "_run_bounded", forbidden)
    proof = verifier_v15._validate_frozen_v14_proof()
    source = (ROOT / "scripts/verify_phase5_capability_nexus_v15.py").read_text(encoding="utf-8")
    assert proof["marker_contract_bound"] is True
    assert "_run_frozen_verifiers" not in source
    assert "ONYX_P53_V15_HISTORY_VERIFY_DEPTH" not in source


def test_frozen_v14_proof_rejects_a_historical_hash_tamper(monkeypatch):
    original = verifier_v15._sha

    def tampered(path):
        if path == ROOT / verifier_v15.V14_ARTIFACT_MANIFEST:
            return "0" * 64
        return original(path)

    monkeypatch.setattr(verifier_v15, "_sha", tampered)
    with pytest.raises(verifier_v15.V15EvidenceError, match="historical recursive leaf mismatch"):
        verifier_v15._validate_frozen_v14_proof()


def test_total_verifier_deadline_is_monotonic_bounded_and_fail_closed(monkeypatch):
    monkeypatch.setattr(verifier_v15.time, "monotonic", lambda: 178.1)
    assert verifier_v15._remaining_timeout(0.0, 120) == 1
    monkeypatch.setattr(verifier_v15.time, "monotonic", lambda: 180.0)
    with pytest.raises(verifier_v15.V15EvidenceError, match="total verifier deadline exceeded"):
        verifier_v15._remaining_timeout(0.0, 120)
...
_EVIDENCE_TAMPER_PATHS = (
    ...
    ("regressions", "junit_sha256"), ("regressions", "selected_suites"),
    ...
    ("live", "anchors"),
    ("runtime", "fresh_focused_timeout_seconds"), ("runtime", "fresh_regression_timeout_seconds"),
    ("runtime", "git_timeout_seconds"), ("runtime", "measured_full_verifier_seconds"),
...
def _evidence_fixture():
    return {
        ...
        "regressions": {
            ...
            "junit_sha256": "b" * 64, "selected_suites": ["tests/test_regressions.py"],
            "subtests_passed": 221, "unique_suites": True,
        },
        "history": {
            "frozen_candidates_cryptographically_validated": 14, "manifests_verified": 69,
            "roots": ["root.sha256"], "unique_leaves_verified": 319,
            "v14_proof": {
        ...
        "live": {"anchors": ["core.py"], "count": 94, "manifest_sha256": "c" * 64, "unique": True},
        "runtime": {
            "fresh_focused_timeout_seconds": 120, "fresh_regression_timeout_seconds": 120,
            "git_timeout_seconds": 10, "measured_full_verifier_seconds": 90.0,
...
def test_every_structured_evidence_field_tamper_fails_closed(path):
    ...
    with pytest.raises(verifier_v15.V15EvidenceError):
        verifier_v15._assert_exact_evidence("fixture", tampered, expected)
