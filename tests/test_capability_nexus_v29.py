...
from core.capability_nexus_v29 import (
    CATALOG_CONTRACT_VERSION_V29,
    LOCK_ORDER_V29,
    CancellationTokenV29,
    CapabilityDescriptorV29,
    CapabilityNexusV29,
    CapabilityProjectionV29,
    CapabilityStatusV29,
    CapabilityV29ContractError,
    CapabilityV29DeniedError,
    CatalogItemV29,
    CatalogReadRequestV29,
    LocalCatalogReadAdapterV29,
    NexusFeatureGateV29,
    OperationDescriptorV29,
    OperationKindV29,
    ReadFailureClassV29,
    ReadStateV29,
    TransportKindV29,
...
import core.capability_nexus_v29 as nexus_v29_module
import scripts.verify_phase5_capability_nexus_v29 as parent_v29
import scripts.verify_phase5_capability_nexus_v29_worker as verifier_v29
...
def _operation(**changes) -> OperationDescriptorV29:
    values = {
        "operation_id": "catalog_read",
        "kind": OperationKindV29.READ,
    ...
    return OperationDescriptorV29(**values)


def _descriptor(**changes) -> CapabilityDescriptorV29:
    values = {
        "capability_id": "local.catalog",
        "capability_version": "v29",
        "provider": "onyx_local",
        "transport": TransportKindV29.LOCAL,
        "api_name": "local_catalog",
        "api_version": CATALOG_CONTRACT_VERSION_V29,
        "workspace_id": WORKSPACE,
        "account_id": ACCOUNT,
        "profile_id": PROFILE,
        "credential_alias": None,
        "operations": (_operation(),),
        "status": CapabilityStatusV29.DISABLED,
        "status_reason": "candidate_not_activated",
        "limitations": ("no_dispatch",),
    }
    values.update(changes)
    return CapabilityDescriptorV29(**values)


def _items(count: int = 5) -> tuple[CatalogItemV29, ...]:
    return tuple(
        CatalogItemV29(f"item-{i:02d}", f"Item {i}", "fixture", "v29", ("safe",))
        for i in range(count)
    )


def _adapter(hook=lambda: None, **changes) -> LocalCatalogReadAdapterV29:
    values = {
        "items": _items(),
        "workspace_id": WORKSPACE,
        "account_id": ACCOUNT,
        ...
        "read_hook": hook,
        "status": CapabilityStatusV29.AVAILABLE_READ_ONLY,
        "status_reason": "healthy",
    }
    values.update(changes)
    return LocalCatalogReadAdapterV29(**values)


def _request(**changes) -> CatalogReadRequestV29:
    values = {
        "workspace_id": WORKSPACE,
        "account_id": ACCOUNT,
        "profile_id": PROFILE,
    ...
    values.update(changes)
    return CatalogReadRequestV29(**values)
...
def test_v20_windows_suspended_job_path_is_exact_after_version_normalization():
    ...
    current = ast.parse(
        (ROOT / "scripts/verify_phase5_capability_nexus_v29.py").read_text(
            encoding="utf-8"
        )
    )
    names = {
    ...
    normalized = {
        name: source.replace("V29", "V20").replace("v29", "v20")
...
def test_projection_constructor_rejects_cross_profile_and_authority():
    descriptor = _descriptor()
    with pytest.raises(CapabilityV29ContractError):
        CapabilityProjectionV29(
            descriptor,
            descriptor.digest,
            WORKSPACE,
...
class _GateSubclass(NexusFeatureGateV29):
    pass
...
def test_registry_rejects_gate_subclasses_fakes_and_duck_types(gate):
    with pytest.raises(CapabilityV29ContractError, match="exact concrete"):
        CapabilityNexusV29(gate)  # type: ignore[arg-type]


def test_exact_gate_is_frozen_default_off_and_lock_order_is_declared():
    gate = NexusFeatureGateV29()
    nexus = CapabilityNexusV29(gate)
    assert nexus.gate is gate and nexus.lock_order == LOCK_ORDER_V29
    with pytest.raises(FrozenInstanceError):
...
@pytest.mark.parametrize("prefix", sorted(nexus_v29_module._SECRET_NAMES))
...
@pytest.mark.parametrize("name", sorted(nexus_v29_module._SECRET_NAMES))
...
@pytest.mark.parametrize(
    "name",
    sorted(nexus_v29_module._SECRET_NAMES | nexus_v29_module._CREDENTIAL_OPAQUE_NAMES),
)
@pytest.mark.parametrize("position", ["leading", "middle", "trailing"])
...
def test_embedded_ngram_model_accepts_held_out_natural_boundary_corpus(word):
    assert (
        nexus_v29_module._english_word_score(word)
        >= nexus_v29_module._NATURAL_SCORE_THRESHOLD
    )
...
def test_schema_rejects_raw_credentials_in_nested_defaults_examples_and_descriptions(
    ...
    for field in ("default", "example", "description"):
        ...
        encoded = nexus_v29_module.json.dumps(
...
def test_recursive_schema_cycle_depth_node_and_scalar_budgets_fail_closed():
    cycle = {}
    cycle["safe"] = cycle
    with pytest.raises(CapabilityV29ContractError, match="cycle"):
        nexus_v29_module._reject_json_secrets(cycle, "fixture")
    deep = current = {}
    for index in range(26):
        current["safe"] = {}
        current = current["safe"]
    with pytest.raises(CapabilityV29ContractError):
        nexus_v29_module._reject_json_secrets(deep, "fixture")
    with pytest.raises(CapabilityV29ContractError, match="node"):
        nexus_v29_module._reject_json_secrets(0, "fixture", _budget=[4096, 0])
    with pytest.raises(CapabilityV29ContractError, match="scalar byte"):
        nexus_v29_module._reject_json_secrets(["a" * 4096] * 17, "fixture")
...
def test_every_non_hash_metadata_key_is_a_closed_enum(key, value):
    with pytest.raises(CapabilityV29ContractError, match="closed enum"):
        _descriptor(metadata=((key, value),))
...
def test_metadata_enum_table_is_read_only_and_future_values_require_source_change():
    assert dict(nexus_v29_module._METADATA_ENUMS) == {
    ...
    with pytest.raises(TypeError):
        nexus_v29_module._METADATA_ENUMS["data_source"] = frozenset({"future_catalog"})
    with pytest.raises(AttributeError):
        nexus_v29_module._METADATA_ENUMS["data_source"].add("future_catalog")
...
def test_content_decoding_cycle_detection_fails_closed(monkeypatch):
    ...
    with pytest.raises(CapabilityV29ContractError, match="cycle"):
        nexus_v29_module._validate_content_decoding_closure("cycle_a")
...
def test_adapter_defaults_disabled_and_has_no_dispatch_or_mutation():
    adapter = LocalCatalogReadAdapterV29(
        _items(),
        workspace_id=WORKSPACE,
        account_id=ACCOUNT,
        profile_id=PROFILE,
    ...
    assert adapter.health().status is CapabilityStatusV29.DISABLED
    assert adapter.read_page(_request()).state is ReadStateV29.DENIED
...
def test_hook_runs_without_adapter_lock_and_can_reenter_health():
    ...
    assert result.state is ReadStateV29.COMPLETED
    assert observed == [CapabilityStatusV29.AVAILABLE_READ_ONLY]
...
def test_same_correlation_reentry_is_denied_before_second_hook_or_reservation():
    ...
    def hook():
        calls.append(1)
        with pytest.raises(CapabilityV29DeniedError, match="reentrancy"):
            holder["adapter"].read_page(request)

    holder["adapter"] = _adapter(hook)
    assert holder["adapter"].read_page(request).state is ReadStateV29.COMPLETED
...
def test_new_correlation_and_indirect_hook_reentry_are_denied_without_quota_drift():
    ...
    assert holder["adapter"].read_page(_request()).state is ReadStateV29.COMPLETED
    limited = holder["adapter"].read_page(_request(correlation_id="corr-after"))
    assert limited.state is ReadStateV29.RATE_LIMITED
...
def test_hook_exception_cleans_reentrancy_guard_for_later_correlation():
    ...
    assert adapter.read_page(_request()).state is ReadStateV29.UNCERTAIN
    assert (
        adapter.read_page(_request(correlation_id="corr-002")).state
        is ReadStateV29.COMPLETED
    )
    assert calls == [1, 1]
...
def test_reentrancy_guard_rejects_hook_spawned_thread_before_reservation():
    ...
    def hook():
        ...
        with ThreadPoolExecutor(max_workers=1) as pool:
            ...
            except CapabilityV29DeniedError as exc:
                nested_errors.append(str(exc))

    holder["adapter"] = _adapter(hook)
    outer = holder["adapter"].read_page(_request())
    assert outer.state is ReadStateV29.COMPLETED
...
def test_hook_guard_is_per_adapter_and_does_not_hold_lock_during_callback():
    ...
    assert first.read_page(_request()).state is ReadStateV29.COMPLETED
    assert observed == [ReadStateV29.COMPLETED]


def test_historical_leaf_budget_exact_boundary_and_overflow():
    leaves = {f"leaf-{index}": "a" * 64 for index in range(4095)}
    verifier_v29._record_historical_leaf(leaves, "boundary", "b" * 64)
    assert len(leaves) == 4096
    with pytest.raises(verifier_v29.V29EvidenceError, match="leaf budget"):
        verifier_v29._record_historical_leaf(leaves, "overflow", "c" * 64)
    with pytest.raises(verifier_v29.V29EvidenceError, match="digest conflict"):
        verifier_v29._record_historical_leaf(leaves, "boundary", "d" * 64)
...
def test_wide_historical_manifest_and_cycle_fail_before_expansion(monkeypatch):
    ...
    monkeypatch.setattr(verifier_v29, "_historical_manifest", lambda relative: wide)
    monkeypatch.setattr(
        verifier_v29, "_verify_historical_path", lambda relative, digest: None
    )
    with pytest.raises(verifier_v29.V29EvidenceError, match="leaf budget"):
        verifier_v29._verify_manifest_tree("wide.sha256", set(), set(), {}, 0)

    graph = {
        "a.sha256": (("a" * 64, "b.sha256"),),
        "b.sha256": (("b" * 64, "a.sha256"),),
    }
    monkeypatch.setattr(verifier_v29, "_historical_manifest", graph.__getitem__)
    with pytest.raises(verifier_v29.V29EvidenceError, match="cycle"):
        verifier_v29._verify_manifest_tree("a.sha256", set(), set(), {}, 0)


def test_exact_4096_leaf_manifest_is_accepted(monkeypatch):
    entries = tuple(("a" * 64, f"leaf-{index}") for index in range(4096))
    monkeypatch.setattr(verifier_v29, "_historical_manifest", lambda relative: entries)
    monkeypatch.setattr(
        verifier_v29, "_verify_historical_path", lambda relative, digest: None
    )
    leaves = {}
    verifier_v29._verify_manifest_tree("boundary.sha256", set(), set(), leaves, 0)
    assert len(leaves) == 4096


def test_historical_manifest_rejects_path_traversal_and_symlink(monkeypatch):
    with tempfile.TemporaryDirectory(prefix="p53-v29-path-", dir=ROOT) as directory:
        base = Path(directory)
        monkeypatch.setattr(verifier_v29, "PROJECT", base)
        manifest = base / "bad.sha256"
        manifest.write_text("" + "a" * 64 + "  ../escape\n", encoding="utf-8")
        with pytest.raises(verifier_v29.V29EvidenceError, match="unsafe"):
            verifier_v29._historical_manifest("bad.sha256")
        ...
        verifier_v29._verify_historical_path(
        ...
        with pytest.raises(verifier_v29.V29EvidenceError, match="reparse"):
            verifier_v29._verify_historical_path(
...
def test_concurrent_duplicate_is_pending_then_completed_and_hook_once():
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        release.set()
        assert future.result().state is ReadStateV29.COMPLETED
...
def test_validation_exception_releases_admission_claim():
    ...
    assert adapter.read_page(_request()).state is ReadStateV29.COMPLETED
...
def test_same_correlation_payload_drift_denies_without_hook_or_quota():
    ...
    with pytest.raises(CapabilityV29DeniedError, match="drift"):
...
def test_hook_exception_becomes_uncertain_and_never_replays_or_leaks_exception():
    ...
    assert result.state is ReadStateV29.UNCERTAIN
    assert result.failure_class is ReadFailureClassV29.HOOK_EXCEPTION
...
def test_cancelled_before_hook_rolls_back_quota_and_rate_and_never_calls_hook():
    ...
    assert result.state is ReadStateV29.CANCELLED_BEFORE_HOOK
...
def test_timeout_after_hook_is_uncertain_and_not_replayed():
    ...
    result = adapter.read_page(request)
    assert result.state is ReadStateV29.UNCERTAIN
    assert result.failure_class is ReadFailureClassV29.TIMEOUT
    assert adapter.read_page(request) == result and calls == [1]


@pytest.mark.parametrize(
    ("action", "failure"),
    [("kill", ReadFailureClassV29.KILL), ("revoke", ReadFailureClassV29.REVOKED)],
)
def test_kill_and_revoke_during_hook_latch_uncertain(action, failure):
    ...
    result = holder["adapter"].read_page(_request())
    assert result.state is ReadStateV29.UNCERTAIN and result.failure_class is failure
...
def test_pending_expiry_becomes_uncertain_and_late_hook_cannot_commit():
    ...
    with ThreadPoolExecutor(max_workers=1) as pool:
        ...
        assert reconciled.state is ReadStateV29.UNCERTAIN
        assert reconciled.failure_class is ReadFailureClassV29.EXPIRED_PENDING
        release.set()
        assert future.result().state is ReadStateV29.UNCERTAIN
...
def test_uncertain_expiry_keeps_no_replay_tombstone_and_frees_uncertain_capacity():
    ...
    assert adapter.read_page(first_request).state is ReadStateV29.UNCERTAIN
    with pytest.raises(CapabilityV29DeniedError, match="pending/uncertain"):
        adapter.read_page(_request(correlation_id="corr-002"))
    now[0] = 2.0
    expired = adapter.reconcile(first_request)
    assert expired.state is ReadStateV29.EXPIRED_UNCERTAIN
    assert expired.failure_class is ReadFailureClassV29.UNCERTAIN_EXPIRED
    assert adapter.read_page(first_request) == expired and calls == [1]
    assert (
        adapter.read_page(_request(correlation_id="corr-002")).state
        is ReadStateV29.UNCERTAIN
    )
...
def test_ledger_capacity_is_bounded_and_does_not_evict_for_replay():
    ...
    request = _request()
    assert adapter.read_page(request).state is ReadStateV29.COMPLETED
    with pytest.raises(CapabilityV29DeniedError, match="capacity"):
        adapter.read_page(_request(correlation_id="corr-002"))
    assert adapter.read_page(request).state is ReadStateV29.COMPLETED
...
def test_cursor_snapshot_and_signature_tamper_fail_closed():
    ...
    changed = LocalCatalogReadAdapterV29(
        _items(6),
        workspace_id=WORKSPACE,
        account_id=ACCOUNT,
        profile_id=PROFILE,
...
def test_rate_limit_and_quota_are_reserved_once_and_truthful():
    ...
    assert limited.state is ReadStateV29.RATE_LIMITED and limited.retry_after_ms == 1000
...
def test_unavailable_auth_scope_disabled_degraded_and_unknown_reconcile():
    assert (
        ...
        is ReadFailureClassV29.AUTH
    )
    assert (
        _adapter(granted_scopes=()).read_page(_request()).failure_class
        is ReadFailureClassV29.SCOPE
    )
    assert (
        _adapter(status=CapabilityStatusV29.DISABLED, status_reason="operator_disabled")
        .read_page(_request())
        .state
        is ReadStateV29.DENIED
    )
    degraded = _adapter(
        status=CapabilityStatusV29.DEGRADED, status_reason="partial_fixture"
    )
    assert degraded.read_page(_request()).state is ReadStateV29.COMPLETED
    unknown = degraded.reconcile(_request(correlation_id="unknown-corr"))
    assert unknown.state is ReadStateV29.UNKNOWN_CORRELATION
...
def test_legacy_builder_preserves_current_32_declarations_and_policies_without_import():
    ...
    assert all(
        item.status is CapabilityStatusV29.DISABLED for item in built.descriptors
    )
...
def test_no_live_runtime_source_references_v29():
    ...
    live += [
        ...
        if path.name != "capability_nexus_v29.py"
    ]
    assert all(
        "capability_nexus_v29" not in path.read_text(encoding="utf-8", errors="ignore")
        for path in live
    )


def test_phase_runner_rejects_every_arbitrary_command_surface():
    with pytest.raises(parent_v29.V29ParentError, match="phase id"):
        parent_v29._run_phase(
    ...
    with pytest.raises(parent_v29.V29ParentError, match="phase id"):
        parent_v29._phase_command("arbitrary", Path("focused.junit.xml"))
...
def test_parent_has_two_direct_bounded_phases_and_pure_worker_has_no_subprocess_surface():
    ...
    worker_tree = ast.parse(
        (ROOT / "scripts/verify_phase5_capability_nexus_v29_worker.py").read_text(
            encoding="utf-8"
        )
    )
    popen_calls = [
    ...
    source = (ROOT / "scripts/verify_phase5_capability_nexus_v29.py").read_text(
        encoding="utf-8"
    )
    assert all(
        token in source
    ...
    assert not (ROOT / "scripts/phase5_capability_nexus_v29_bootstrap.py").exists()
...
def test_frozen_history_proof_never_spawns_a_recursive_verifier():
    v14 = verifier_v29._validate_frozen_v14_proof()
    v15 = verifier_v29._validate_frozen_v15_proof()
    v16 = verifier_v29._validate_frozen_v16_proof()
    v17 = verifier_v29._validate_frozen_v17_proof()
    v18 = verifier_v29._validate_frozen_v18_proof()
    v19 = verifier_v29._validate_frozen_v19_proof()
    v20 = verifier_v29._validate_frozen_v20_proof()
    v21 = verifier_v29._validate_frozen_v21_proof()
    v22 = verifier_v29._validate_frozen_v22_proof()
    v23 = verifier_v29._validate_frozen_v23_proof()
    v24 = verifier_v29._validate_frozen_v24_proof()
    v25 = verifier_v29._validate_frozen_v25_proof()
    v26 = verifier_v29._validate_frozen_v26_proof()
    v27 = verifier_v29._validate_frozen_v27_proof()
    v28 = verifier_v29._validate_frozen_v28_proof()
    source = (ROOT / "scripts/verify_phase5_capability_nexus_v29_worker.py").read_text(
        encoding="utf-8"
    )
    assert all(
...
def test_frozen_v14_proof_rejects_a_historical_hash_tamper(monkeypatch):
    original = verifier_v29._sha

    def tampered(path):
        if path == ROOT / verifier_v29.V14_ARTIFACT_MANIFEST:
            return "0" * 64
        return original(path)

    monkeypatch.setattr(verifier_v29, "_sha", tampered)
    with pytest.raises(
        verifier_v29.V29EvidenceError, match="historical recursive leaf mismatch"
    ):
        verifier_v29._validate_frozen_v14_proof()


@pytest.mark.skipif(sys.platform != "win32", reason="Windows Job ABI")
def test_kernel32_job_abi_is_explicit_and_pointer_safe():
    api = parent_v29._kernel32()
    assert ctypes.sizeof(parent_v29.wintypes.HANDLE) == ctypes.sizeof(ctypes.c_void_p)
    for name in (
        "CreateJobObjectW",
...
def test_invalid_or_truncated_windows_handles_fail_closed(raw):
    with pytest.raises(parent_v29.V29ParentError, match="invalid or truncated"):
        parent_v29._validated_handle(raw)


def test_old_v19_bootstrap_invocation_path_is_absent_in_v29():
    source = (ROOT / "scripts/verify_phase5_capability_nexus_v29.py").read_text(
        encoding="utf-8"
    )
    assert "BOOTSTRAP" not in source
    assert "HELLO" not in source and "READY" not in source and "GO " not in source
    assert "ONYX_P53_V29" not in source


def test_worker_rejects_arbitrary_fresh_junit_path():
    with tempfile.TemporaryDirectory(prefix="onyx-v29-arbitrary-junit-") as directory:
        path = Path(directory) / "focused.junit.xml"
        path.write_text("fixture", encoding="utf-8")
        with pytest.raises(verifier_v29.V29EvidenceError, match="nonce-scoped"):
            verifier_v29._validated_fresh_focused_junit(path)
...
def test_delayed_job_assignment_cannot_execute_target_before_resume(monkeypatch):
    ...
    original = parent_v29._WindowsJob.assign_handle
    with tempfile.TemporaryDirectory(prefix="onyx-v29-delayed-assign-") as directory:
        ...
        monkeypatch.setattr(parent_v29._WindowsJob, "assign_handle", delayed)
        with (
            parent_v29._PrivateEvidence() as evidence_nonce,
            ThreadPoolExecutor(max_workers=1) as pool,
        ):
            future = pool.submit(
                parent_v29._windows_phase,
                "focused",
...
def test_assign_failure_terminates_suspended_target_before_execution(monkeypatch):
    with tempfile.TemporaryDirectory(prefix="onyx-v29-assign-failure-") as directory:
        marker = Path(directory) / "TARGET_RAN"
        target = Path(directory) / "target.py"
        target.write_text(
            "from pathlib import Path\nimport sys\nPath(sys.argv[1]).write_text('ran')\n"
        ...
        def rejected(_job, _process_handle):
            raise parent_v29.V29ParentError("injected AssignProcessToJobObject failure")

        monkeypatch.setattr(parent_v29._WindowsJob, "assign_handle", rejected)
        with parent_v29._PrivateEvidence() as evidence_nonce:
            with pytest.raises(parent_v29.V29ParentError, match="injected Assign"):
                parent_v29._windows_phase(
                    "focused",
                    evidence_nonce=evidence_nonce,
...
def test_resume_failure_terminates_target_before_execution(monkeypatch):
    with tempfile.TemporaryDirectory(prefix="onyx-v29-resume-failure-") as directory:
        marker = Path(directory) / "TARGET_RAN"
        target = Path(directory) / "target.py"
        target.write_text(
            "from pathlib import Path\nimport sys\nPath(sys.argv[1]).write_text('ran')\n"
        ...
        real_job = parent_v29._WindowsJob()
        ...
        class ApiProxy:
            ...
            def ResumeThread(self, _thread):
                return parent_v29._INFINITE_RESUME_FAILURE

        real_job.api = ApiProxy()
        monkeypatch.setattr(parent_v29, "_WindowsJob", lambda: real_job)
        with parent_v29._PrivateEvidence() as evidence_nonce:
            with pytest.raises(parent_v29.V29ParentError, match="ResumeThread"):
                parent_v29._windows_phase(
                    "focused",
                    evidence_nonce=evidence_nonce,
...
def test_createprocess_failure_closes_precreated_resources(monkeypatch):
    ...
    monkeypatch.setattr(parent_v29, "_WindowsJob", lambda: real_job)
    with parent_v29._PrivateEvidence() as evidence_nonce:
        with pytest.raises(parent_v29.V29ParentError, match="CreateProcessW"):
            parent_v29._windows_phase(
                "focused",
                evidence_nonce=evidence_nonce,
    ...
    assert not parent_v29._RETAINED_HANDLES


def test_closehandle_failure_retains_ownership_until_eventual_cleanup():
    parent_v29._RETAINED_HANDLES.clear()
    ...
    job = object.__new__(parent_v29._WindowsJob)
    job.api = Api()
    job.handle = parent_v29.wintypes.HANDLE(321)
    with pytest.raises(parent_v29.V29ParentError, match=r"CloseHandle\(Job\)"):
        job.close()
    assert job.handle.value == 321 and 321 in parent_v29._RETAINED_HANDLES
    parent_v29._cleanup_retained_handles()
    assert not parent_v29._RETAINED_HANDLES
    job.handle = parent_v29.wintypes.HANDLE()
...
def test_pid_exists_closehandle_failure_propagates_and_is_retained(monkeypatch):
    ...
    class Api:
        ...
        def GetExitCodeProcess(self, _handle, pointer):
            ctypes.cast(
                pointer, ctypes.POINTER(parent_v29.wintypes.DWORD)
            ).contents.value = 0
            return True
    ...
    api = Api()
    monkeypatch.setattr(parent_v29, "_kernel32", lambda: api)
    monkeypatch.setattr(parent_v29.os, "name", "nt")
    with pytest.raises(parent_v29.V29ParentError, match=r"CloseHandle\(Process\)"):
        parent_v29._pid_exists(123)
    assert 456 in parent_v29._RETAINED_HANDLES
...
def test_posix_cleanup_state_machine_covers_every_communicate_path(
    ...
    class Process:
        ...
        def communicate(self, timeout=None):
            if outcome == "timeout" and timeout != parent_v29.POST_KILL_DRAIN_SECONDS:
                raise subprocess.TimeoutExpired(["fixed"], timeout)
            if outcome == "exception" and timeout != parent_v29.POST_KILL_DRAIN_SECONDS:
                raise RuntimeError("communicate exploded")
            return "ok", ""

    process = Process()
    monkeypatch.setattr(parent_v29.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(parent_v29.os, "getpgid", lambda _pid: 999, raising=False)
    exists = iter(([True, False, False] if outcome == "residual" else [False, False]))
    monkeypatch.setattr(
        parent_v29, "_posix_group_exists", lambda _pgid: next(exists, False)
    )
    monkeypatch.setattr(
        parent_v29, "_kill_posix_group", lambda pgid: events.append(("kill", pgid))
    )
    monkeypatch.setattr(
        parent_v29, "_drain", lambda _process: events.append(("drain", 999)) or ("", "")
    )
    with parent_v29._PrivateEvidence() as evidence_nonce:
        if outcome == "normal":
            assert (
                parent_v29._posix_phase(
                    "focused",
                    evidence_nonce=evidence_nonce,
                    label="posix",
                    timeout=1,
            ...
            with pytest.raises((parent_v29.V29ParentError, RuntimeError)):
                parent_v29._posix_phase(
                    "focused",
                    evidence_nonce=evidence_nonce,
                    label="posix",
...
def test_getpgid_failure_after_spawn_kills_reaps_and_never_accepts_pass(
    ...
    monkeypatch.setattr(parent_v29, "_pid_exists", lambda _pid: False)
    with parent_v29._PrivateEvidence() as evidence_nonce:
...
def test_unexpected_getpgid_cleans_only_direct_child_and_fails_closed(monkeypatch):
    ...
    monkeypatch.setattr(parent_v29.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(
        parent_v29.os, "getpgid", lambda _pid: process.pid + 1, raising=False
    )
    monkeypatch.setattr(parent_v29, "_pid_exists", lambda _pid: False)
    monkeypatch.setattr(
        parent_v29, "_kill_posix_group", lambda _pgid: pytest.fail("wrong group killed")
    )
    with parent_v29._PrivateEvidence() as evidence_nonce:
        with pytest.raises(parent_v29.V29ParentError, match="ownership mismatch"):
            parent_v29._posix_phase(
                "focused",
                evidence_nonce=evidence_nonce,
                label="wrong group",
...
def test_successful_normal_posix_phase_retains_group_owned_path(monkeypatch):
    ...
    monkeypatch.setattr(parent_v29.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(
        parent_v29.os, "getpgid", lambda _pid: process.pid, raising=False
    )
    monkeypatch.setattr(parent_v29, "_posix_group_exists", lambda _pgid: False)
    with parent_v29._PrivateEvidence() as evidence_nonce:
        result = parent_v29._posix_phase(
            "focused",
            evidence_nonce=evidence_nonce,
            label="normal posix",
            timeout=1,
...
def test_cleanup_collector_order_and_exact_deduplication():
    failures = parent_v29._CleanupFailures()
...
def test_cleanup_collector_without_primary_raises_one_payload_error():
    ...
    assert isinstance(combined, parent_v29.V29ParentError)
...
def test_cleanup_collector_single_fault_returns_wrapper_snapshot():
    ...
    assert type(combined) is parent_v29.V29ParentError
...
def test_cleanup_cycle_is_explicit_and_finite():
    ...
    assert failures.records == [
        parent_v29.CleanupFailureRecord(
...
def test_cleanup_shared_dag_is_not_a_cycle_and_dedupes_exactly():
    ...
    failures.add("op", group)
    assert failures.records == [
        parent_v29.CleanupFailureRecord("op", "builtins.OSError", "shared")
    ]
...
def test_normal_posix_empty_cleanup_payload_proof_rejects_pass(monkeypatch, capsys):
    ...
    monkeypatch.setattr(parent_v29.subprocess, "Popen", lambda *a, **k: Process())
    monkeypatch.setattr(parent_v29.os, "getpgid", lambda pid: pid, raising=False)
    monkeypatch.setattr(
        parent_v29, "_posix_group_exists", lambda _pgid: (_ for _ in ()).throw(proof)
    )
    monkeypatch.setattr(parent_v29, "_kill_posix_group", lambda _pgid: None)
    monkeypatch.setattr(parent_v29, "_drain", lambda _process: ("", ""))
    monkeypatch.setattr(
        parent_v29, "_wait_posix_group_empty", lambda _pgid, _label: None
    )
    with parent_v29._PrivateEvidence() as nonce:
        with pytest.raises(parent_v29.V29ParentError):
            parent_v29._posix_phase(
                "focused",
...
def test_cleanup_duplicate_payload_bomb_is_bounded_and_dedupes_before_capacity():
    ...
    failure = RuntimeError("wrapper")
    failure.cleanup_failures = [item] * parent_v29._CleanupFailures.MAX_PAYLOAD_ITEMS
    failures = parent_v29._CleanupFailures()
    failures.add("proof", failure)
    assert failures.records == [
        parent_v29.CleanupFailureRecord("same", "builtins.OSError", "same")
    ]
    assert not failures.overflowed
...
def test_cleanup_record_capacity_dedup_boundary_and_129th_unique_fail_closed():
    failures = parent_v29._CleanupFailures()
    for index in range(parent_v29._CleanupFailures.MAX_RECORDS):
        failure = RuntimeError("wrapper")
...
def test_cleanup_failures_getter_bomb_is_captured_with_original_leaf():
    ...
    collector = parent_v29._CleanupFailures()
...
def test_v29_primary_is_record_and_cause_but_never_returned_or_attached():
    primary = RuntimeError("primary")
    collector = parent_v29._CleanupFailures()
...
def test_v29_primary_attachment_surfaces_are_never_invoked():
...
def test_v29_one_shot_cleanup_failures_descriptor_cannot_create_toctou():
...
def test_v29_one_shot_notes_accessor_cannot_create_toctou():
...
def test_v29_unprintable_primary_record_uses_pointer_free_fallback():
    class Primary(RuntimeError):
        def __str__(self):
...
def test_v29_no_primary_builds_wrapper_without_attachment_failure():
    collector = parent_v29._CleanupFailures()
    collector.add("direct.kill", OSError("kill failed"))
...
def test_v29_snapshot_rejects_ordinary_assignment_and_delete():
    collector = parent_v29._CleanupFailures()
    collector.add("direct.kill", OSError("kill failed"))
    ...
    with pytest.raises(AttributeError):
        combined._v29_cleanup_failures = ()
    with pytest.raises(AttributeError):
        del combined.cleanup_failures
    with pytest.raises(AttributeError):
        del combined._v29_cleanup_failures
...
def test_v29_str_subclass_attribute_names_reject_without_running_subclass_code():
    class S(str):
        calls = 0
    ...
    hostile_name = S("_v29_cleanup_failures")
...
def test_v29_non_string_and_all_exact_string_names_reject_after_construction():
    collector = parent_v29._CleanupFailures()
    collector.add("direct.kill", OSError("kill failed"))
    combined = collector.finish(None)
...
def test_v29_reflection_is_documented_outside_runtime_object_contract():
    collector = parent_v29._CleanupFailures()
    collector.add("direct.kill", OSError("kill failed"))
    disposable = collector.finish(None)
    object.__setattr__(disposable, "_v29_cleanup_failures", ())
    assert disposable.cleanup_failures == ()
    assert parent_v29.V29ParentError.__doc__ is not None
    assert "not a same-process reflection security boundary" in parent_v29.V29ParentError.__doc__


def test_v29_current_candidate_contains_no_same_process_overclaim():
    paths = [
        ROOT / "core/capability_nexus_v29.py",
        ROOT / "tests/test_capability_nexus_v29.py",
        ROOT / "scripts/verify_phase5_capability_nexus_v29.py",
        ROOT / "scripts/verify_phase5_capability_nexus_v29_worker.py",
        ROOT / "scripts/phase5_capability_nexus_v29_subtest_reporter.py",
        ROOT / "scripts/check_phase5_capability_nexus_v29_whitespace.py",
    ]
    evidence = ROOT / "docs/onyx/checkpoints/phase5-capability-nexus-v29"
    paths.extend(
        path
        for path in (
            evidence / "PHASE5_3_CAPABILITY_NEXUS_V29_CHECKPOINT.md",
            evidence / "phase5-capability-nexus-v29.metrics.json",
            evidence / "phase5-capability-nexus-v29.bundle.json",
        )
        if path.is_file()
    )
    forbidden = (
...
def test_v29_manifests_supply_durable_integrity_not_runtime_wrapper():
    source = (ROOT / "scripts/verify_phase5_capability_nexus_v29_worker.py").read_text(
        encoding="utf-8"
    )
    ...
    assert verifier_v29.V27_ROOT_MANIFEST in verifier_v29.HISTORICAL_ROOTS
    root_manifest = ROOT / verifier_v29.V27_ROOT_MANIFEST
...
def test_hanging_child_and_grandchild_tree_is_killed_without_retry_or_marker(
    ...
    with tempfile.TemporaryDirectory(prefix="onyx-v29-process-tree-") as directory:
        ...
        with parent_v29._PrivateEvidence() as evidence_nonce:
            if hang_phase == "worker":
                (
                    parent_v29.PRIVATE_BASE / evidence_nonce / "focused.junit.xml"
                ).write_text("fixture")
            with pytest.raises(parent_v29.V29ParentError, match="timed out"):
                parent_v29._run_phase(
                    hang_phase,
                    evidence_nonce=evidence_nonce,
                    label="tree fixture",
        ...
        while (
            parent_v29._pid_exists(direct_pid) or parent_v29._pid_exists(grandchild_pid)
        ) and time.monotonic() < deadline:
            time.sleep(0.02)
        assert not parent_v29._pid_exists(direct_pid)
        assert not parent_v29._pid_exists(grandchild_pid)
        assert parent_v29.MARKER not in capsys.readouterr().out
...
def test_regression_report_semantic_tamper_fails_closed(tamper):
    report = copy.deepcopy(verifier_v29._regression_report())
    ...
    with pytest.raises(verifier_v29.V29EvidenceError):
        verifier_v29._validate_regression_report(report)
...
def test_regression_junit_attribute_and_child_tamper_fails_closed(tamper):
    report = verifier_v29._regression_report()
    tree = ET.parse(ROOT / verifier_v29.REGRESSION_JUNIT)
    ...
    with tempfile.TemporaryDirectory(prefix="onyx-v29-junit-tamper-") as directory:
        path = Path(directory) / "tampered.junit.xml"
        tree.write(path, encoding="utf-8", xml_declaration=True)
        with pytest.raises(verifier_v29.V29EvidenceError):
            verifier_v29._junit_path(path, subtest_report=report)


def test_synthetic_junit_counter_requires_explicit_validated_subtest_report():
    with tempfile.TemporaryDirectory(prefix="onyx-v29-junit-synthetic-") as directory:
        ...
        with pytest.raises(
            verifier_v29.V29EvidenceError, match="lacks validated subtest evidence"
        ):
            verifier_v29._junit_path(path)
...
def test_junit_lexical_text_tail_and_markup_tamper_fails_closed(tamper):
    ...
    with tempfile.TemporaryDirectory(prefix="onyx-v29-junit-lexical-") as directory:
        path = Path(directory) / "tampered.junit.xml"
...
def test_every_structured_evidence_field_tamper_fails_closed(path):
    ...
    with pytest.raises(verifier_v29.V29EvidenceError):
        verifier_v29._assert_exact_evidence("fixture", tampered, expected)
