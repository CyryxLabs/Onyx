...
from core import approval_inbox_v5 as inbox
from scripts import verify_phase5_approval_inbox_v5 as evidence
...
def test_exact_source_output_types_only_no_subclass_or_custom_mapping() -> None:
    ...
    for result in ({"items": ()}, subclass, object()):
        ...
        with pytest.raises(inbox.ApprovalInboxV5ContractError, match="concrete type"):
...
def test_bounds_are_checked_before_any_item_traversal_or_copy_hook() -> None:
    ...
    with pytest.raises(inbox.ApprovalInboxV5ContractError, match="exceeds"):
        service.open_page(environ=ON)
    assert hooks == {"iter": 0, "deepcopy": 0, "attribute": 0}

    custom = Hook()
    service, _source, _clock = projection(lambda: host_snapshot(custom))
    with pytest.raises(inbox.ApprovalInboxV5ContractError, match="exact tuple"):
        service.open_page(environ=ON)
    assert hooks == {"iter": 0, "deepcopy": 0, "attribute": 0}
...
def test_raw_callback_exception_is_not_chained_or_recoverable() -> None:
    ...
    with pytest.raises(inbox.ApprovalInboxV5Unavailable) as caught:
        service.open_page(environ=ON)
    error = caught.value
...
def test_every_recoverable_display_field_rejects_secrets(field: str) -> None:
    ...
    with pytest.raises(inbox.ApprovalInboxV5ContractError, match="secret-like"):
        service.open_page(environ=ON)
...
def test_projection_owned_deadline_expires_without_source_callback_and_never_resurrects() -> None:
    ...
    with pytest.raises(inbox.ApprovalInboxV5Stale, match="deadline"):
    ...
    with pytest.raises(inbox.ApprovalInboxV5Stale):
        service.continue_page(page.next_cursor, environ=ON)
...
def test_monotonic_clock_rollback_latches_and_cannot_resurrect() -> None:
    ...
    with pytest.raises(inbox.ApprovalInboxV5Unavailable, match="rollback"):
        service.continue_page(page.next_cursor, environ=ON)
...
def test_same_epoch_equivocation_latches_and_restore_never_resurrects() -> None:
    ...
    with pytest.raises(inbox.ApprovalInboxV5Stale, match="without-epoch"):
        service.continue_page(page.next_cursor, environ=ON)
    state["snapshot"] = original
    for operation in (
        ...
        with pytest.raises(inbox.ApprovalInboxV5Unavailable, match="integrity-failed"):
            operation()
...
def test_epoch_rollback_latches_and_restore_never_resurrects() -> None:
    ...
    with pytest.raises(inbox.ApprovalInboxV5Stale, match="rollback"):
        service.open_page(environ=ON)
    state["snapshot"] = current
    with pytest.raises(inbox.ApprovalInboxV5Unavailable, match="integrity-failed"):
        service.open_page(environ=ON)
...
def test_integrity_latch_is_nonresurrecting_across_concurrent_callers() -> None:
    ...
    def equivocate() -> None:
        ...
        try:
            service.continue_page(page.next_cursor, environ=ON)
        except inbox.ApprovalInboxV5Stale:
    ...
    def restore() -> None:
        ...
        try:
            service.continue_page(page.next_cursor, environ=ON)
        except inbox.ApprovalInboxV5Unavailable:
...
def test_batch_cannot_select_hidden_workspace_or_filtered_item() -> None:
    ...
    for hidden in ("approval-hidden", "approval-filtered"):
        with pytest.raises(inbox.ApprovalInboxV5ContractError, match="outside"):
            service.preview_calm_batch(
                page.snapshot_token,
...
def test_page_size_one_requires_second_item_to_be_returned_before_batch() -> None:
    ...
    with pytest.raises(inbox.ApprovalInboxV5ContractError, match="not returned"):
        service.preview_calm_batch(
...
def test_reopening_conflicting_registered_view_state_latches_fail_closed(
    ...
    with pytest.raises(inbox.ApprovalInboxV5Stale, match="registered-view-proof-conflict"):
        service.open_page(environ=ON)
    with pytest.raises(inbox.ApprovalInboxV5Unavailable, match="integrity"):
        service.open_page(environ=ON)
...
def test_high_critical_always_explicit_or_ineligible_never_enters_batch(
    ...
    page = service.open_page(environ=ON)
    with pytest.raises(inbox.ApprovalInboxV5ContractError, match="ineligible"):
        service.preview_calm_batch(
...
def test_host_cannot_mislabel_high_or_always_explicit_as_batch_eligible() -> None:
    for risk, explicit in (("high", False), ("critical", False), ("low", True)):
        ...
        with pytest.raises(inbox.ApprovalInboxV5ContractError, match="cannot be batch"):
            service.open_page(environ=ON)
...
def test_batch_forbids_wildcards_categories_duplicates_and_unbounded_sequences() -> None:
    ...
    for selected in ((), ("*",), ("all-low-risk",), ("approval-*",), ("approval-unknown",)):
        with pytest.raises(inbox.ApprovalInboxV5ContractError):
            service.preview_calm_batch(page.snapshot_token, selected, environ=ON)
    with pytest.raises(inbox.ApprovalInboxV5ContractError, match="duplicates"):
        service.preview_calm_batch(
            page.snapshot_token,
...
def test_outer_token_validation_occurs_before_hmac_and_normalizes_all_malformed_inputs(
    ...
    for token in malformed:
        before = calls
        with pytest.raises(inbox.ApprovalInboxV5Stale) as caught:
...
def test_source_concurrency_is_capped_at_one_and_excess_fails_closed() -> None:
    ...
    with pytest.raises(inbox.ApprovalInboxV5Unavailable, match="busy") as caught:
        service.open_page(environ=ON)
    assert caught.value.__cause__ is None
    assert caught.value.__context__ is None
    release.set()
...
def test_result_constructors_reject_forged_or_contradictory_inputs() -> None:
    ...
    view = service._views[page.view_digest]
    with pytest.raises(inbox.ApprovalInboxV5ContractError, match="next cursor"):
    ...
    with pytest.raises(inbox.ApprovalInboxV5ContractError, match="not canonical"):
        inbox._make_page_result(
            service,
            status="ready",
            snapshot=snapshot,
    ...
    with pytest.raises(inbox.ApprovalInboxV5Stale, match="invalid-authenticated"):
        inbox._make_page_result(
            service,
            status="ready",
            snapshot=snapshot,
...
def test_unknown_identity_safety_and_schema_state_fail_closed(field: str, value: object) -> None:
    ...
    with pytest.raises((inbox.ApprovalInboxV5ContractError, inbox.ApprovalInboxV5Unavailable)):
        service.open_page(environ=ON)


def test_limits_and_bounded_snapshot_view_caches() -> None:
    service, _source, _clock = projection(lambda: host_snapshot())
    with pytest.raises(inbox.ApprovalInboxV5ContractError, match="page_size"):
    ...
    with pytest.raises(inbox.ApprovalInboxV5ContractError, match="reason"):
...
def test_no_mutation_grant_import_persistence_or_live_wiring() -> None:
    ...
    for relative in (
        ...
        assert "approval_inbox_v5" not in text
        assert inbox.APPROVAL_INBOX_FLAG not in text


def test_v1_v2_v3_v4_and_r11_frozen_anchor_bytes_remain_exact() -> None:
    root = Path(__file__).resolve().parents[1]
    expected = {
...
def test_r11_transitive_tamper_fixture_rejects_v10_before_recursive_execution() -> None:
    with tempfile.TemporaryDirectory(
        prefix=".p52-v5-r11-tamper-", dir=evidence.PROJECT
    ) as directory:
        ...
        with pytest.raises(evidence.Phase52V5EvidenceError, match="session_grants_v10.py"):
...
def test_v2_transitive_tamper_fixture_rejects_core_v2() -> None:
    with tempfile.TemporaryDirectory(
        prefix=".p52-v5-v2-tamper-", dir=evidence.PROJECT
    ) as directory:
        frozen = Path(directory)
        ...
        with pytest.raises(evidence.Phase52V5EvidenceError, match="approval_inbox_v2.py"):
...
def test_v3_transitive_tamper_fixture_rejects_core_v3() -> None:
    with tempfile.TemporaryDirectory(
        prefix=".p52-v5-v3-tamper-", dir=evidence.PROJECT
    ) as directory:
        frozen = Path(directory)
        ...
        with pytest.raises(evidence.Phase52V5EvidenceError, match="approval_inbox_v3.py"):
...
def test_future_approval_successor_import_indirection_is_rejected() -> None:
    with tempfile.TemporaryDirectory(
        prefix=".p52-v5-import-walk-", dir=evidence.PROJECT
    ) as directory:
        root = Path(directory)
        (root / "core").mkdir()
        (root / "scripts").mkdir()
        ...
        (root / "core" / "approval_inbox_v5.py").write_text(
            "\n", encoding="utf-8", newline="\n"
        )
        (root / "core" / "approval_inbox_v6.py").write_text(
            "import importlib\n"
            "importlib.import_module('core.approval_inbox_v5')\n",
            encoding="utf-8",
            newline="\n",
        )
        with pytest.raises(evidence.Phase52V5EvidenceError, match="imports frozen approval"):
...
def test_literal_dynamic_approval_imports_are_rejected(loader: str) -> None:
    with tempfile.TemporaryDirectory(
        prefix=".p52-v5-dynamic-import-", dir=evidence.PROJECT
    ) as directory:
        root = Path(directory)
        (root / "core").mkdir()
        (root / "scripts").mkdir()
        ...
        with pytest.raises(evidence.Phase52V5EvidenceError, match="approval inbox"):
            evidence.live_scan_paths(root)
...
def test_regenerated_hashes_cannot_bypass_semantic_metadata(field: str) -> None:
    ...
    with tempfile.TemporaryDirectory(
        prefix=".p52-v5-semantic-tamper-", dir=evidence.PROJECT
    ) as directory:
        root = Path(directory)
        ...
        with pytest.raises(evidence.Phase52V5EvidenceError):
