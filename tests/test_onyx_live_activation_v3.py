...
from core import onyx_live_activation_v3 as live
...
LAUNCHER = ROOT / "scripts" / "launch_onyx_live_v3.pyw"
HOST_GATE = ROOT / "scripts" / "verify_onyx_live_activation_v3_host.py"
QT_GATE = ROOT / "scripts" / "verify_onyx_live_activation_v3_qt.py"


def launcher_api():
    return runpy.run_path(str(LAUNCHER), run_name="onyx_live_v3_launcher_test")
...
def test_invalid_launcher_refuses_before_main_or_ui_import_in_fresh_process():
    ...
    assert "ONYX_LIVE_V3_PREIMPORT_REFUSAL CLEAN" in result.stderr
...
def test_actual_main_preflight_in_fresh_subprocess_without_provisioning():
    ...
    assert "ONYX_LIVE_V3_HOST_PREFLIGHT_OK" in result.stdout


def test_real_onyxlive_ten_seams_reconnects_and_catalog_e2e():
    result = subprocess.run(
    ...
    assert "ONYX_LIVE_ACTIVATION_V3_REAL_HOST_OK" in result.stdout
    assert "seam_failpoints=10 reconnects=64 catalog_reads=64" in result.stdout
...
def test_real_offscreen_qt_affinity_atomicity_timeout_and_teardown():
    ...
    assert result.returncode == 0, result.stderr + result.stdout
    assert "ONYX_LIVE_ACTIVATION_V3_QT_OK" in result.stdout
    assert "atomic_rollback=mutate-then-raise timeout=fail-closed" in result.stdout
...
def test_v1_v2_are_frozen_rejected_v3_is_independent_and_host_has_no_drift():
    frozen = {
    ...
    source = (ROOT / "core" / "onyx_live_activation_v3.py").read_text(
        encoding="utf-8"
    )
...
def test_activation_flags_reject_noncanonical_truth(value):
    ...
    with pytest.raises(live.ActivationV3Error, match="canonical"):
        live.ActivationFlagsV3.from_canonical_environ(environment)
