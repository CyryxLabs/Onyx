from __future__ import annotations

import hashlib
import json
from pathlib import Path

from scripts import verify_phase5_exit_retirement_v1 as retirement


ROOT = Path(__file__).resolve().parents[1]
V10 = ROOT / "tests/fixtures/release_workflow_transition_v10.json"
V11 = ROOT / "tests/fixtures/release_workflow_transition_v11.json"


def _clear() -> None:
    retirement.load_current_successor_transition.cache_clear()
    retirement.load_release_workflow_transition.cache_clear()


def test_release_v11_authenticates_v10_and_binds_v24_package_correction() -> None:
    _clear()
    release = json.loads(V11.read_text(encoding="utf-8"))

    assert hashlib.sha256(V11.read_bytes()).hexdigest() == (
        retirement.RELEASE_TRANSITION_V11_SHA256
    )
    assert hashlib.sha256(V10.read_bytes()).hexdigest() == (
        retirement.RELEASE_TRANSITION_V10_SHA256
    )
    assert release["schema"] == "onyx.release-workflow-transition.v11"
    assert release["predecessor"] == {
        "path": "tests/fixtures/release_workflow_transition_v10.json",
        "sha256": retirement.RELEASE_TRANSITION_V10_SHA256,
    }
    paths = {entry["path"] for entry in release["current_release_paths"]}
    assert {
        "tests/fixtures/phase5_current_successor_transition_v24.json",
        "tests/test_phase5_current_successor_transition_v24.py",
        "tests/test_package_hygiene_v1.py",
    }.issubset(paths)
    assert release["current_root_sha256"] == (
        "f25120019fff6ef8f85a253be5ff79add7876d6ec70fa2d65f399176017ddb5e"
    )
