...
def test_v47_authenticates_v46_and_current_security_successor() -> None:
    ...
    assert transition["schema"] == "onyx.phase5-current-successor-transition.v47"
    assert transition["predecessor"] == {
        "path": "tests/fixtures/phase5_current_successor_transition_v46.json",
        "sha256": retirement.CURRENT_TRANSITION_V46_SHA256,
    }
    ...
    assert historical[".github/workflows/release-packages.yml"] == hashlib.sha256(
