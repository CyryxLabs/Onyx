...
from core.capability_nexus_v31 import (
    CATALOG_CONTRACT_VERSION_V31,
    LOCK_ORDER_V31,
    CancellationTokenV31,
    CapabilityDescriptorV31,
    CapabilityNexusV31,
    CapabilityProjectionV31,
    CapabilityStatusV31,
    CapabilityV31ContractError,
    CapabilityV31DeniedError,
    CatalogItemV31,
    CatalogReadRequestV31,
    LocalCatalogReadAdapterV31,
    NexusFeatureGateV31,
    OperationDescriptorV31,
    OperationKindV31,
    ReadFailureClassV31,
    ReadStateV31,
    TransportKindV31,
...
import core.capability_nexus_v31 as nexus_v31_module
import scripts.verify_phase5_capability_nexus_v31 as parent_v31
import scripts.verify_phase5_capability_nexus_v31_worker as verifier_v31
...
def _operation(**changes) -> OperationDescriptorV31:
    values = {
        "operation_id": "catalog_read",
        "kind": OperationKindV31.READ,
    ...
    return OperationDescriptorV31(**values)


def _descriptor(**changes) -> CapabilityDescriptorV31:
    values = {
        "capability_id": "local.catalog",
        "capability_version": "v31",
        "provider": "onyx_local",
        "transport": TransportKindV31.LOCAL,
        "api_name": "local_catalog",
        "api_version": CATALOG_CONTRACT_VERSION_V31,
        "workspace_id": WORKSPACE,
        "account_id": ACCOUNT,
        "profile_id": PROFILE,
        "credential_alias": None,
        "operations": (_operation(),),
        "status": CapabilityStatusV31.DISABLED,
        "status_reason": "candidate_not_activated",
        "limitations": ("no_dispatch",),
    }
    values.update(changes)
    return CapabilityDescriptorV31(**values)


def _items(count: int = 5) -> tuple[CatalogItemV31, ...]:
    return tuple(
        CatalogItemV31(f"item-{i:02d}", f"Item {i}", "fixture", "v31", ("safe",))
        for i in range(count)
    )


def _adapter(hook=lambda: None, **changes) -> LocalCatalogReadAdapterV31:
    values = {
        "items": _items(),
        "workspace_id": WORKSPACE,
        "account_id": ACCOUNT,
        ...
        "read_hook": hook,
        "status": CapabilityStatusV31.AVAILABLE_READ_ONLY,
        "status_reason": "healthy",
    }
    values.update(changes)
    return LocalCatalogReadAdapterV31(**values)


def _request(**changes) -> CatalogReadRequestV31:
    values = {
        "workspace_id": WORKSPACE,
        "account_id": ACCOUNT,
        "profile_id": PROFILE,
    ...
    values.update(changes)
    return CatalogReadRequestV31(**values)
...
def test_v31_core_and_metadata_behavior_are_exact_version_normalization():
    prior = (ROOT / "core/capability_nexus_v30.py").read_text(encoding="utf-8")
    current = (ROOT / "core/capability_nexus_v31.py").read_text(encoding="utf-8")
    assert prior.replace("V30", "V31").replace("v30", "v31") == current
...
def test_v20_windows_suspended_job_path_is_exact_after_version_normalization():
    ...
    current = ast.parse(
        (ROOT / "scripts/verify_phase5_capability_nexus_v31.py").read_text(
            encoding="utf-8"
        )
    )
    names = {
    ...
    normalized = {
        name: source.replace("V31", "V20").replace("v31", "v20")
...
def test_projection_constructor_rejects_cross_profile_and_authority():
    descriptor = _descriptor()
    with pytest.raises(CapabilityV31ContractError):
        CapabilityProjectionV31(
            descriptor,
            descriptor.digest,
            WORKSPACE,
...
class _GateSubclass(NexusFeatureGateV31):
    pass
...
def test_registry_rejects_gate_subclasses_fakes_and_duck_types(gate):
    with pytest.raises(CapabilityV31ContractError, match="exact concrete"):
        CapabilityNexusV31(gate)  # type: ignore[arg-type]


def test_exact_gate_is_frozen_default_off_and_lock_order_is_declared():
    gate = NexusFeatureGateV31()
    nexus = CapabilityNexusV31(gate)
    assert nexus.gate is gate and nexus.lock_order == LOCK_ORDER_V31
    with pytest.raises(FrozenInstanceError):
...
@pytest.mark.parametrize("prefix", sorted(nexus_v31_module._SECRET_NAMES))
...
@pytest.mark.parametrize("name", sorted(nexus_v31_module._SECRET_NAMES))
...
@pytest.mark.parametrize(
    "name",
    sorted(nexus_v31_module._SECRET_NAMES | nexus_v31_module._CREDENTIAL_OPAQUE_NAMES),
)
@pytest.mark.parametrize("position", ["leading", "middle", "trailing"])
...
def test_embedded_ngram_model_accepts_held_out_natural_boundary_corpus(word):
    assert (
        nexus_v31_module._english_word_score(word)
        >= nexus_v31_module._NATURAL_SCORE_THRESHOLD
    )
...
def test_schema_rejects_raw_credentials_in_nested_defaults_examples_and_descriptions(
    ...
    for field in ("default", "example", "description"):
        ...
        encoded = nexus_v31_module.json.dumps(
...
def test_recursive_schema_cycle_depth_node_and_scalar_budgets_fail_closed():
    cycle = {}
    cycle["safe"] = cycle
    with pytest.raises(CapabilityV31ContractError, match="cycle"):
        nexus_v31_module._reject_json_secrets(cycle, "fixture")
    deep = current = {}
    for index in range(26):
        current["safe"] = {}
        current = current["safe"]
    with pytest.raises(CapabilityV31ContractError):
        nexus_v31_module._reject_json_secrets(deep, "fixture")
    with pytest.raises(CapabilityV31ContractError, match="node"):
        nexus_v31_module._reject_json_secrets(0, "fixture", _budget=[4096, 0])
    with pytest.raises(CapabilityV31ContractError, match="scalar byte"):
        nexus_v31_module._reject_json_secrets(["a" * 4096] * 17, "fixture")
...
def test_every_non_hash_metadata_key_is_a_closed_enum(key, value):
    with pytest.raises(CapabilityV31ContractError, match="closed enum"):
        _descriptor(metadata=((key, value),))
...
def test_metadata_enum_table_is_read_only_and_future_values_require_source_change():
    assert dict(nexus_v31_module._METADATA_ENUMS) == {
    ...
    with pytest.raises(TypeError):
        nexus_v31_module._METADATA_ENUMS["data_source"] = frozenset({"future_catalog"})
    with pytest.raises(AttributeError):
        nexus_v31_module._METADATA_ENUMS["data_source"].add("future_catalog")
...
def test_content_decoding_cycle_detection_fails_closed(monkeypatch):
    ...
    with pytest.raises(CapabilityV31ContractError, match="cycle"):
        nexus_v31_module._validate_content_decoding_closure("cycle_a")
...
def test_adapter_defaults_disabled_and_has_no_dispatch_or_mutation():
    adapter = LocalCatalogReadAdapterV31(
        _items(),
        workspace_id=WORKSPACE,
        account_id=ACCOUNT,
        profile_id=PROFILE,
    ...
    assert adapter.health().status is CapabilityStatusV31.DISABLED
    assert adapter.read_page(_request()).state is ReadStateV31.DENIED
...
def test_hook_runs_without_adapter_lock_and_can_reenter_health():
    ...
    assert result.state is ReadStateV31.COMPLETED
    assert observed == [CapabilityStatusV31.AVAILABLE_READ_ONLY]
...
def test_same_correlation_reentry_is_denied_before_second_hook_or_reservation():
    ...
    def hook():
        calls.append(1)
        with pytest.raises(CapabilityV31DeniedError, match="reentrancy"):
            holder["adapter"].read_page(request)

    holder["adapter"] = _adapter(hook)
    assert holder["adapter"].read_page(request).state is ReadStateV31.COMPLETED
...
def test_new_correlation_and_indirect_hook_reentry_are_denied_without_quota_drift():
    ...
    assert holder["adapter"].read_page(_request()).state is ReadStateV31.COMPLETED
    limited = holder["adapter"].read_page(_request(correlation_id="corr-after"))
    assert limited.state is ReadStateV31.RATE_LIMITED
...
def test_hook_exception_cleans_reentrancy_guard_for_later_correlation():
    ...
    assert adapter.read_page(_request()).state is ReadStateV31.UNCERTAIN
    assert (
        adapter.read_page(_request(correlation_id="corr-002")).state
        is ReadStateV31.COMPLETED
    )
    assert calls == [1, 1]
...
def test_reentrancy_guard_rejects_hook_spawned_thread_before_reservation():
    ...
    def hook():
        ...
        with ThreadPoolExecutor(max_workers=1) as pool:
            ...
            except CapabilityV31DeniedError as exc:
                nested_errors.append(str(exc))

    holder["adapter"] = _adapter(hook)
    outer = holder["adapter"].read_page(_request())
    assert outer.state is ReadStateV31.COMPLETED
...
def test_hook_guard_is_per_adapter_and_does_not_hold_lock_during_callback():
    ...
    assert first.read_page(_request()).state is ReadStateV31.COMPLETED
    assert observed == [ReadStateV31.COMPLETED]


def test_historical_leaf_budget_exact_boundary_and_overflow():
    leaves = {f"leaf-{index}": "a" * 64 for index in range(4095)}
    verifier_v31._record_historical_leaf(leaves, "boundary", "b" * 64)
    assert len(leaves) == 4096
    with pytest.raises(verifier_v31.V31EvidenceError, match="leaf budget"):
        verifier_v31._record_historical_leaf(leaves, "overflow", "c" * 64)
    with pytest.raises(verifier_v31.V31EvidenceError, match="digest conflict"):
        verifier_v31._record_historical_leaf(leaves, "boundary", "d" * 64)
...
def test_wide_historical_manifest_and_cycle_fail_before_expansion(monkeypatch):
    ...
    monkeypatch.setattr(verifier_v31, "_historical_manifest", lambda relative: wide)
    monkeypatch.setattr(
        verifier_v31, "_verify_historical_path", lambda relative, digest: None
    )
    with pytest.raises(verifier_v31.V31EvidenceError, match="leaf budget"):
        verifier_v31._verify_manifest_tree("wide.sha256", set(), set(), {}, 0)

    graph = {
        "a.sha256": (("a" * 64, "b.sha256"),),
        "b.sha256": (("b" * 64, "a.sha256"),),
    }
    monkeypatch.setattr(verifier_v31, "_historical_manifest", graph.__getitem__)
    with pytest.raises(verifier_v31.V31EvidenceError, match="cycle"):
        verifier_v31._verify_manifest_tree("a.sha256", set(), set(), {}, 0)


def test_exact_4096_leaf_manifest_is_accepted(monkeypatch):
    entries = tuple(("a" * 64, f"leaf-{index}") for index in range(4096))
    monkeypatch.setattr(verifier_v31, "_historical_manifest", lambda relative: entries)
    monkeypatch.setattr(
        verifier_v31, "_verify_historical_path", lambda relative, digest: None
    )
    leaves = {}
    verifier_v31._verify_manifest_tree("boundary.sha256", set(), set(), leaves, 0)
    assert len(leaves) == 4096


def test_historical_manifest_rejects_path_traversal_and_symlink(monkeypatch):
    with tempfile.TemporaryDirectory(prefix="p53-v31-path-", dir=ROOT) as directory:
        base = Path(directory)
        monkeypatch.setattr(verifier_v31, "PROJECT", base)
        manifest = base / "bad.sha256"
        manifest.write_text("" + "a" * 64 + "  ../escape\n", encoding="utf-8")
        with pytest.raises(verifier_v31.V31EvidenceError, match="unsafe"):
            verifier_v31._historical_manifest("bad.sha256")
        ...
        verifier_v31._verify_historical_path(
        ...
        with pytest.raises(verifier_v31.V31EvidenceError, match="reparse"):
            verifier_v31._verify_historical_path(
...
def test_concurrent_duplicate_is_pending_then_completed_and_hook_once():
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        release.set()
        assert future.result().state is ReadStateV31.COMPLETED
...
def test_validation_exception_releases_admission_claim():
    ...
    assert adapter.read_page(_request()).state is ReadStateV31.COMPLETED
...
def test_same_correlation_payload_drift_denies_without_hook_or_quota():
    ...
    with pytest.raises(CapabilityV31DeniedError, match="drift"):
...
def test_hook_exception_becomes_uncertain_and_never_replays_or_leaks_exception():
    ...
    assert result.state is ReadStateV31.UNCERTAIN
    assert result.failure_class is ReadFailureClassV31.HOOK_EXCEPTION
...
def test_cancelled_before_hook_rolls_back_quota_and_rate_and_never_calls_hook():
    ...
    assert result.state is ReadStateV31.CANCELLED_BEFORE_HOOK
...
def test_timeout_after_hook_is_uncertain_and_not_replayed():
    ...
    result = adapter.read_page(request)
    assert result.state is ReadStateV31.UNCERTAIN
    assert result.failure_class is ReadFailureClassV31.TIMEOUT
    assert adapter.read_page(request) == result and calls == [1]


@pytest.mark.parametrize(
    ("action", "failure"),
    [("kill", ReadFailureClassV31.KILL), ("revoke", ReadFailureClassV31.REVOKED)],
)
def test_kill_and_revoke_during_hook_latch_uncertain(action, failure):
    ...
    result = holder["adapter"].read_page(_request())
    assert result.state is ReadStateV31.UNCERTAIN and result.failure_class is failure
...
def test_pending_expiry_becomes_uncertain_and_late_hook_cannot_commit():
    ...
    with ThreadPoolExecutor(max_workers=1) as pool:
        ...
        assert reconciled.state is ReadStateV31.UNCERTAIN
        assert reconciled.failure_class is ReadFailureClassV31.EXPIRED_PENDING
        release.set()
        assert future.result().state is ReadStateV31.UNCERTAIN
...
def test_uncertain_expiry_keeps_no_replay_tombstone_and_frees_uncertain_capacity():
    ...
    assert adapter.read_page(first_request).state is ReadStateV31.UNCERTAIN
    with pytest.raises(CapabilityV31DeniedError, match="pending/uncertain"):
        adapter.read_page(_request(correlation_id="corr-002"))
    now[0] = 2.0
    expired = adapter.reconcile(first_request)
    assert expired.state is ReadStateV31.EXPIRED_UNCERTAIN
    assert expired.failure_class is ReadFailureClassV31.UNCERTAIN_EXPIRED
    assert adapter.read_page(first_request) == expired and calls == [1]
    assert (
        adapter.read_page(_request(correlation_id="corr-002")).state
        is ReadStateV31.UNCERTAIN
    )
...
def test_ledger_capacity_is_bounded_and_does_not_evict_for_replay():
    ...
    request = _request()
    assert adapter.read_page(request).state is ReadStateV31.COMPLETED
    with pytest.raises(CapabilityV31DeniedError, match="capacity"):
        adapter.read_page(_request(correlation_id="corr-002"))
    assert adapter.read_page(request).state is ReadStateV31.COMPLETED
...
def test_cursor_snapshot_and_signature_tamper_fail_closed():
    ...
    changed = LocalCatalogReadAdapterV31(
        _items(6),
        workspace_id=WORKSPACE,
        account_id=ACCOUNT,
        profile_id=PROFILE,
...
def test_rate_limit_and_quota_are_reserved_once_and_truthful():
    ...
    assert limited.state is ReadStateV31.RATE_LIMITED and limited.retry_after_ms == 1000
...
def test_unavailable_auth_scope_disabled_degraded_and_unknown_reconcile():
    assert (
        ...
        is ReadFailureClassV31.AUTH
    )
    assert (
        _adapter(granted_scopes=()).read_page(_request()).failure_class
        is ReadFailureClassV31.SCOPE
    )
    assert (
        _adapter(status=CapabilityStatusV31.DISABLED, status_reason="operator_disabled")
        .read_page(_request())
        .state
        is ReadStateV31.DENIED
    )
    degraded = _adapter(
        status=CapabilityStatusV31.DEGRADED, status_reason="partial_fixture"
    )
    assert degraded.read_page(_request()).state is ReadStateV31.COMPLETED
    unknown = degraded.reconcile(_request(correlation_id="unknown-corr"))
    assert unknown.state is ReadStateV31.UNKNOWN_CORRELATION
...
def test_legacy_builder_preserves_current_32_declarations_and_policies_without_import():
    ...
    assert all(
        item.status is CapabilityStatusV31.DISABLED for item in built.descriptors
    )
...
def test_no_live_runtime_source_references_v31():
    ...
    live += [
        ...
        if path.name != "capability_nexus_v31.py"
    ]
    assert all(
        "capability_nexus_v31" not in path.read_text(encoding="utf-8", errors="ignore")
        for path in live
    )


def test_phase_runner_rejects_every_arbitrary_command_surface():
    with pytest.raises(parent_v31.V31ParentError, match="phase id"):
        parent_v31._run_phase(
    ...
    with pytest.raises(parent_v31.V31ParentError, match="phase id"):
        parent_v31._phase_command("arbitrary", Path("focused.junit.xml"))
...
def test_parent_has_two_direct_bounded_phases_and_pure_worker_has_no_subprocess_surface():
    ...
    worker_tree = ast.parse(
        (ROOT / "scripts/verify_phase5_capability_nexus_v31_worker.py").read_text(
            encoding="utf-8"
        )
    )
    popen_calls = [
    ...
    source = (ROOT / "scripts/verify_phase5_capability_nexus_v31.py").read_text(
        encoding="utf-8"
    )
    assert all(
        token in source
    ...
    assert not (ROOT / "scripts/phase5_capability_nexus_v31_bootstrap.py").exists()
...
def _copy_projection_fixture(destination: Path) -> None:
    for relative in (
        verifier_v31.MUTABLE_PROJECTION_MAP,
        *verifier_v31.MUTABLE_PROJECTION_SNAPSHOTS,
        *verifier_v31.MUTABLE_PROJECTIONS,
    ):
...
def test_v31_mutable_projection_mapping_is_exact_and_not_authority():
    mapping = verifier_v31._projection_mapping()
    assert tuple(mapping) == verifier_v31.MUTABLE_PROJECTIONS
    assert verifier_v31._mutable_projection_evidence() == {
        "authority": "content_addressed_frozen_snapshots_not_external_projection_paths",
        "entries": [mapping[path] for path in verifier_v31.MUTABLE_PROJECTIONS],
        "mapping_sha256": hashlib.sha256(
            (ROOT / verifier_v31.MUTABLE_PROJECTION_MAP).read_bytes()
        ).hexdigest(),
        "projection_paths_are_authority": False,
    }


def test_v31_history_uses_snapshot_after_current_projection_mutation(
    monkeypatch,
):
    with tempfile.TemporaryDirectory(prefix="onyx-v31-projection-current-") as directory:
        project = Path(directory)
        _copy_projection_fixture(project)
        for relative in verifier_v31.MUTABLE_PROJECTIONS:
            (project / relative).write_bytes(b"external projection changed after freeze\n")
        monkeypatch.setattr(verifier_v31, "PROJECT", project)
        for relative, digest in verifier_v31.MUTABLE_PROJECTION_HASHES.items():
            verifier_v31._verify_historical_path(relative, digest)


def test_v31_projection_snapshot_tamper_fails_closed(monkeypatch):
    with tempfile.TemporaryDirectory(prefix="onyx-v31-projection-snapshot-") as directory:
        project = Path(directory)
        _copy_projection_fixture(project)
        snapshot = project / verifier_v31.MUTABLE_PROJECTION_SNAPSHOTS[0]
        snapshot.write_bytes(snapshot.read_bytes() + b"tamper")
        monkeypatch.setattr(verifier_v31, "PROJECT", project)
        with pytest.raises(verifier_v31.V31EvidenceError, match="snapshot hash mismatch"):
            verifier_v31._projection_mapping()


def test_v31_projection_mapping_extra_missing_and_substitution_fail_closed(
    monkeypatch,
):
    with tempfile.TemporaryDirectory(prefix="onyx-v31-projection-map-") as directory:
        project = Path(directory)
        _copy_projection_fixture(project)
        mapping_path = project / verifier_v31.MUTABLE_PROJECTION_MAP
        ...
        for value in variants:
            ...
            with pytest.raises(verifier_v31.V31EvidenceError):
                verifier_v31._projection_mapping()
...
def _authoritative_reader(kind: str, project: Path, relative: str) -> None:
    ...
    if kind in {"source_root_manifest", "artifact_manifest", "live_manifest"}:
        verifier_v31._manifest(relative)
    elif kind == "history_manifest":
        verifier_v31._historical_manifest(relative)
    elif kind in {"bundle", "metrics", "regression_report"}:
        verifier_v31._canonical_json(relative)
    elif kind in {"focused_junit", "combined_junit", "regression_junit"}:
        verifier_v31._junit_path(path)
    elif kind == "static":
        verifier_v31._static_path(path)
    elif kind in {"checkpoint", "tamper_fixture"}:
        verifier_v31._read_text(relative)
    elif kind in {"snapshot", "generic_artifact"}:
        verifier_v31._sha(path)
...
@pytest.mark.parametrize(
    ("kind", "relative"),
    [
        ("source_root_manifest", verifier_v31.ROOT_MANIFEST),
        ("artifact_manifest", verifier_v31.ARTIFACT_MANIFEST),
        ("bundle", verifier_v31.BUNDLE),
        ("metrics", verifier_v31.METRICS),
        ("checkpoint", verifier_v31.CHECKPOINT),
        ("focused_junit", verifier_v31.JUNIT),
        ("combined_junit", verifier_v31.COMBINED_JUNIT),
        ("regression_junit", verifier_v31.REGRESSION_JUNIT),
        ("regression_report", verifier_v31.REGRESSION_REPORT),
        ("static", verifier_v31.STATIC),
        ("live_manifest", verifier_v31.LIVE),
        ("history_manifest", verifier_v31.V30_ROOT_MANIFEST),
        ("tamper_fixture", "history/tamper-fixture.log"),
        ("snapshot", verifier_v31.MUTABLE_PROJECTION_SNAPSHOTS[0]),
        ("generic_artifact", "artifacts/generic.bin"),
    ],
)
def test_v31_every_authoritative_evidence_class_rejects_identical_symlink(
    monkeypatch,
    kind,
    relative,
):
    with tempfile.TemporaryDirectory(prefix="onyx-v31-authoritative-class-") as directory:
        base = Path(directory)
...
def test_v31_reviewer_mapping_symlink_repro_rejects_external_identical_map(
    monkeypatch,
):
    with tempfile.TemporaryDirectory(prefix="onyx-v31-reviewer-map-") as directory:
        base = Path(directory)
        project = base / "project"
        mapping = project / verifier_v31.MUTABLE_PROJECTION_MAP
        mapping.parent.mkdir(parents=True, exist_ok=True)
        mapping.write_bytes((ROOT / verifier_v31.MUTABLE_PROJECTION_MAP).read_bytes())
...
def test_v31_parent_rejects_fixed_phase_target_symlink_before_launch(monkeypatch):
    with tempfile.TemporaryDirectory(prefix="onyx-v31-parent-target-") as directory:
        base = Path(directory)
        project = base / "project"
        ...
        monkeypatch.setattr(parent_v31, "PROJECT", project)
        monkeypatch.setattr(parent_v31, "TESTS", "tests/focused.py")
        with pytest.raises(parent_v31.V31ParentError, match="reparse"):
            parent_v31._phase_command("focused", Path("focused.junit.xml"))


def _v31_dag_required_paths() -> set[str]:
    histories = [getattr(verifier_v31, f"V{version}_HISTORY") for version in range(1, 31)]
    lives = [getattr(verifier_v31, f"V{version}_LIVE") for version in range(1, 31)]
    return {
        verifier_v31.CORE,
        verifier_v31.TESTS,
        verifier_v31.SELF,
        verifier_v31.WORKER,
        verifier_v31.REPORTER,
        verifier_v31.WHITESPACE,
        verifier_v31.CHECKPOINT,
        verifier_v31.BUNDLE,
        verifier_v31.JUNIT,
        verifier_v31.RAW,
        verifier_v31.STATIC,
        verifier_v31.LIVE,
        verifier_v31.METRICS,
        verifier_v31.COMBINED_JUNIT,
        verifier_v31.REGRESSION_JUNIT,
        verifier_v31.REGRESSION_REPORT,
        verifier_v31.MUTABLE_PROJECTION_MAP,
        *verifier_v31.MUTABLE_PROJECTION_SNAPSHOTS,
        *lives,
...
def test_v31_dag_rejects_identical_generic_artifact_symlink_before_hash(
    monkeypatch,
):
    with tempfile.TemporaryDirectory(prefix="onyx-v31-dag-symlink-") as directory:
        base = Path(directory)
        project = base / "project"
        generic = "artifacts/reviewer-identical.bin"
        artifact_paths = _v31_dag_required_paths() | {generic}
        ...
        artifact = project / verifier_v31.ARTIFACT_MANIFEST
        ...
        root = project / verifier_v31.ROOT_MANIFEST
        root.write_text(
            f"{hashlib.sha256(artifact.read_bytes()).hexdigest()}  {verifier_v31.ARTIFACT_MANIFEST}\n",
            encoding="utf-8",
            newline="\n",
        )
        monkeypatch.setattr(verifier_v31, "PROJECT", project)
...
def test_v31_authoritative_path_rejects_aliases_case_and_outside_workspace(
    monkeypatch,
):
    with tempfile.TemporaryDirectory(prefix="onyx-v31-path-alias-") as directory:
        base = Path(directory)
        project = base / "project"
        ...
        monkeypatch.setattr(verifier_v31, "PROJECT", project)
        verifier_v31._assert_no_reparse("Evidence/Record.json")
        for alias in (
            ...
            with pytest.raises(verifier_v31.V31EvidenceError):
                verifier_v31._assert_no_reparse(alias)


def test_v31_authoritative_path_rejects_symlinked_ancestor_and_workspace_root(
    monkeypatch,
):
    with tempfile.TemporaryDirectory(prefix="onyx-v31-ancestor-link-") as directory:
        base = Path(directory)
        ...
        monkeypatch.setattr(verifier_v31, "PROJECT", project)
        with pytest.raises(verifier_v31.V31EvidenceError, match="reparse"):
            verifier_v31._read_text("linked/evidence.json")

        workspace_alias = base / "workspace-alias"
        workspace_alias.symlink_to(project, target_is_directory=True)
        monkeypatch.setattr(verifier_v31, "PROJECT", workspace_alias)
        with pytest.raises(verifier_v31.V31EvidenceError, match="workspace root"):
            verifier_v31._assert_no_reparse("linked/evidence.json")


@pytest.mark.skipif(os.name != "nt", reason="Windows junction semantics only")
def test_v31_authoritative_path_rejects_windows_junction_ancestor(
    monkeypatch,
):
    with tempfile.TemporaryDirectory(prefix="onyx-v31-junction-") as directory:
        base = Path(directory)
        ...
        monkeypatch.setattr(verifier_v31, "PROJECT", project)
        with pytest.raises(verifier_v31.V31EvidenceError, match="reparse"):
            verifier_v31._read_text("junction/evidence.json")


def test_frozen_history_proof_never_spawns_a_recursive_verifier():
    v14 = verifier_v31._validate_frozen_v14_proof()
    v15 = verifier_v31._validate_frozen_v15_proof()
    v16 = verifier_v31._validate_frozen_v16_proof()
    v17 = verifier_v31._validate_frozen_v17_proof()
    v18 = verifier_v31._validate_frozen_v18_proof()
    v19 = verifier_v31._validate_frozen_v19_proof()
    v20 = verifier_v31._validate_frozen_v20_proof()
    v21 = verifier_v31._validate_frozen_v21_proof()
    v22 = verifier_v31._validate_frozen_v22_proof()
    v23 = verifier_v31._validate_frozen_v23_proof()
    v24 = verifier_v31._validate_frozen_v24_proof()
    v25 = verifier_v31._validate_frozen_v25_proof()
    v26 = verifier_v31._validate_frozen_v26_proof()
    v27 = verifier_v31._validate_frozen_v27_proof()
    v28 = verifier_v31._validate_frozen_v28_proof()
    v29 = verifier_v31._validate_frozen_v29_proof()
    v30 = verifier_v31._validate_frozen_v30_proof()
    source = (ROOT / "scripts/verify_phase5_capability_nexus_v31_worker.py").read_text(
        encoding="utf-8"
    )
    assert all(
...
def test_frozen_v14_proof_rejects_a_historical_hash_tamper(monkeypatch):
    original = verifier_v31._sha

    def tampered(path):
        if path == ROOT / verifier_v31.V14_ARTIFACT_MANIFEST:
            return "0" * 64
        return original(path)

    monkeypatch.setattr(verifier_v31, "_sha", tampered)
    with pytest.raises(
        verifier_v31.V31EvidenceError, match="historical recursive leaf mismatch"
    ):
        verifier_v31._validate_frozen_v14_proof()


@pytest.mark.skipif(sys.platform != "win32", reason="Windows Job ABI")
def test_kernel32_job_abi_is_explicit_and_pointer_safe():
    api = parent_v31._kernel32()
    assert ctypes.sizeof(parent_v31.wintypes.HANDLE) == ctypes.sizeof(ctypes.c_void_p)
    for name in (
        "CreateJobObjectW",
...
def test_invalid_or_truncated_windows_handles_fail_closed(raw):
    with pytest.raises(parent_v31.V31ParentError, match="invalid or truncated"):
        parent_v31._validated_handle(raw)


def test_old_v19_bootstrap_invocation_path_is_absent_in_v31():
    source = (ROOT / "scripts/verify_phase5_capability_nexus_v31.py").read_text(
        encoding="utf-8"
    )
    assert "BOOTSTRAP" not in source
    assert "HELLO" not in source and "READY" not in source and "GO " not in source
    assert "ONYX_P53_V31" not in source


def test_worker_rejects_arbitrary_fresh_junit_path():
    with tempfile.TemporaryDirectory(prefix="onyx-v31-arbitrary-junit-") as directory:
        path = Path(directory) / "focused.junit.xml"
        path.write_text("fixture", encoding="utf-8")
        with pytest.raises(verifier_v31.V31EvidenceError, match="nonce-scoped"):
            verifier_v31._validated_fresh_focused_junit(path)
...
def test_delayed_job_assignment_cannot_execute_target_before_resume(monkeypatch):
    ...
    original = parent_v31._WindowsJob.assign_handle
    with tempfile.TemporaryDirectory(prefix="onyx-v31-delayed-assign-") as directory:
        ...
        monkeypatch.setattr(parent_v31._WindowsJob, "assign_handle", delayed)
        with (
            parent_v31._PrivateEvidence() as evidence_nonce,
            ThreadPoolExecutor(max_workers=1) as pool,
        ):
            future = pool.submit(
                parent_v31._windows_phase,
                "focused",
...
def test_assign_failure_terminates_suspended_target_before_execution(monkeypatch):
    with tempfile.TemporaryDirectory(prefix="onyx-v31-assign-failure-") as directory:
        marker = Path(directory) / "TARGET_RAN"
        target = Path(directory) / "target.py"
        target.write_text(
            "from pathlib import Path\nimport sys\nPath(sys.argv[1]).write_text('ran')\n"
        ...
        def rejected(_job, _process_handle):
            raise parent_v31.V31ParentError("injected AssignProcessToJobObject failure")

        monkeypatch.setattr(parent_v31._WindowsJob, "assign_handle", rejected)
        with parent_v31._PrivateEvidence() as evidence_nonce:
            with pytest.raises(parent_v31.V31ParentError, match="injected Assign"):
                parent_v31._windows_phase(
                    "focused",
                    evidence_nonce=evidence_nonce,
...
def test_resume_failure_terminates_target_before_execution(monkeypatch):
    with tempfile.TemporaryDirectory(prefix="onyx-v31-resume-failure-") as directory:
        marker = Path(directory) / "TARGET_RAN"
        target = Path(directory) / "target.py"
        target.write_text(
            "from pathlib import Path\nimport sys\nPath(sys.argv[1]).write_text('ran')\n"
        ...
        real_job = parent_v31._WindowsJob()
        ...
        class ApiProxy:
            ...
            def ResumeThread(self, _thread):
                return parent_v31._INFINITE_RESUME_FAILURE

        real_job.api = ApiProxy()
        monkeypatch.setattr(parent_v31, "_WindowsJob", lambda: real_job)
        with parent_v31._PrivateEvidence() as evidence_nonce:
            with pytest.raises(parent_v31.V31ParentError, match="ResumeThread"):
                parent_v31._windows_phase(
                    "focused",
                    evidence_nonce=evidence_nonce,
...
def test_createprocess_failure_closes_precreated_resources(monkeypatch):
    ...
    monkeypatch.setattr(parent_v31, "_WindowsJob", lambda: real_job)
    with parent_v31._PrivateEvidence() as evidence_nonce:
        with pytest.raises(parent_v31.V31ParentError, match="CreateProcessW"):
            parent_v31._windows_phase(
                "focused",
                evidence_nonce=evidence_nonce,
    ...
    assert not parent_v31._RETAINED_HANDLES


def test_closehandle_failure_retains_ownership_until_eventual_cleanup():
    parent_v31._RETAINED_HANDLES.clear()
    ...
    job = object.__new__(parent_v31._WindowsJob)
    job.api = Api()
    job.handle = parent_v31.wintypes.HANDLE(321)
    with pytest.raises(parent_v31.V31ParentError, match=r"CloseHandle\(Job\)"):
        job.close()
    assert job.handle.value == 321 and 321 in parent_v31._RETAINED_HANDLES
    parent_v31._cleanup_retained_handles()
    assert not parent_v31._RETAINED_HANDLES
    job.handle = parent_v31.wintypes.HANDLE()
...
def test_pid_exists_closehandle_failure_propagates_and_is_retained(monkeypatch):
    ...
    class Api:
        ...
        def GetExitCodeProcess(self, _handle, pointer):
            ctypes.cast(
                pointer, ctypes.POINTER(parent_v31.wintypes.DWORD)
            ).contents.value = 0
            return True
    ...
    api = Api()
    monkeypatch.setattr(parent_v31, "_kernel32", lambda: api)
    monkeypatch.setattr(parent_v31.os, "name", "nt")
    with pytest.raises(parent_v31.V31ParentError, match=r"CloseHandle\(Process\)"):
        parent_v31._pid_exists(123)
    assert 456 in parent_v31._RETAINED_HANDLES
...
def test_posix_cleanup_state_machine_covers_every_communicate_path(
    ...
    class Process:
        ...
        def communicate(self, timeout=None):
            if outcome == "timeout" and timeout != parent_v31.POST_KILL_DRAIN_SECONDS:
                raise subprocess.TimeoutExpired(["fixed"], timeout)
            if outcome == "exception" and timeout != parent_v31.POST_KILL_DRAIN_SECONDS:
                raise RuntimeError("communicate exploded")
            return "ok", ""

    process = Process()
    monkeypatch.setattr(parent_v31.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(parent_v31.os, "getpgid", lambda _pid: 999, raising=False)
    exists = iter(([True, False, False] if outcome == "residual" else [False, False]))
    monkeypatch.setattr(
        parent_v31, "_posix_group_exists", lambda _pgid: next(exists, False)
    )
    monkeypatch.setattr(
        parent_v31, "_kill_posix_group", lambda pgid: events.append(("kill", pgid))
    )
    monkeypatch.setattr(
        parent_v31, "_drain", lambda _process: events.append(("drain", 999)) or ("", "")
    )
    with parent_v31._PrivateEvidence() as evidence_nonce:
        if outcome == "normal":
            assert (
                parent_v31._posix_phase(
                    "focused",
                    evidence_nonce=evidence_nonce,
                    label="posix",
                    timeout=1,
            ...
            with pytest.raises((parent_v31.V31ParentError, RuntimeError)):
                parent_v31._posix_phase(
                    "focused",
                    evidence_nonce=evidence_nonce,
                    label="posix",
...
def test_getpgid_failure_after_spawn_kills_reaps_and_never_accepts_pass(
    ...
    monkeypatch.setattr(parent_v31, "_pid_exists", lambda _pid: False)
    with parent_v31._PrivateEvidence() as evidence_nonce:
...
def test_unexpected_getpgid_cleans_only_direct_child_and_fails_closed(monkeypatch):
    ...
    monkeypatch.setattr(parent_v31.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(
        parent_v31.os, "getpgid", lambda _pid: process.pid + 1, raising=False
    )
    monkeypatch.setattr(parent_v31, "_pid_exists", lambda _pid: False)
    monkeypatch.setattr(
        parent_v31, "_kill_posix_group", lambda _pgid: pytest.fail("wrong group killed")
    )
    with parent_v31._PrivateEvidence() as evidence_nonce:
        with pytest.raises(parent_v31.V31ParentError, match="ownership mismatch"):
            parent_v31._posix_phase(
                "focused",
                evidence_nonce=evidence_nonce,
                label="wrong group",
...
def test_successful_normal_posix_phase_retains_group_owned_path(monkeypatch):
    ...
    monkeypatch.setattr(parent_v31.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(
        parent_v31.os, "getpgid", lambda _pid: process.pid, raising=False
    )
    monkeypatch.setattr(parent_v31, "_posix_group_exists", lambda _pgid: False)
    with parent_v31._PrivateEvidence() as evidence_nonce:
        result = parent_v31._posix_phase(
            "focused",
            evidence_nonce=evidence_nonce,
            label="normal posix",
            timeout=1,
...
def test_cleanup_collector_order_and_exact_deduplication():
    failures = parent_v31._CleanupFailures()
...
def test_cleanup_collector_without_primary_raises_one_payload_error():
    ...
    assert isinstance(combined, parent_v31.V31ParentError)
...
def test_cleanup_collector_single_fault_returns_wrapper_snapshot():
    ...
    assert type(combined) is parent_v31.V31ParentError
...
def test_cleanup_cycle_is_explicit_and_finite():
    ...
    assert failures.records == [
        parent_v31.CleanupFailureRecord(
...
def test_cleanup_shared_dag_is_not_a_cycle_and_dedupes_exactly():
    ...
    failures.add("op", group)
    assert failures.records == [
        parent_v31.CleanupFailureRecord("op", "builtins.OSError", "shared")
    ]
...
def test_normal_posix_empty_cleanup_payload_proof_rejects_pass(monkeypatch, capsys):
    ...
    monkeypatch.setattr(parent_v31.subprocess, "Popen", lambda *a, **k: Process())
    monkeypatch.setattr(parent_v31.os, "getpgid", lambda pid: pid, raising=False)
    monkeypatch.setattr(
        parent_v31, "_posix_group_exists", lambda _pgid: (_ for _ in ()).throw(proof)
    )
    monkeypatch.setattr(parent_v31, "_kill_posix_group", lambda _pgid: None)
    monkeypatch.setattr(parent_v31, "_drain", lambda _process: ("", ""))
    monkeypatch.setattr(
        parent_v31, "_wait_posix_group_empty", lambda _pgid, _label: None
    )
    with parent_v31._PrivateEvidence() as nonce:
        with pytest.raises(parent_v31.V31ParentError):
            parent_v31._posix_phase(
                "focused",
...
def test_cleanup_duplicate_payload_bomb_is_bounded_and_dedupes_before_capacity():
    ...
    failure = RuntimeError("wrapper")
    failure.cleanup_failures = [item] * parent_v31._CleanupFailures.MAX_PAYLOAD_ITEMS
    failures = parent_v31._CleanupFailures()
    failures.add("proof", failure)
    assert failures.records == [
        parent_v31.CleanupFailureRecord("same", "builtins.OSError", "same")
    ]
    assert not failures.overflowed
...
def test_cleanup_record_capacity_dedup_boundary_and_129th_unique_fail_closed():
    failures = parent_v31._CleanupFailures()
    for index in range(parent_v31._CleanupFailures.MAX_RECORDS):
        failure = RuntimeError("wrapper")
...
def test_cleanup_failures_getter_bomb_is_captured_with_original_leaf():
    ...
    collector = parent_v31._CleanupFailures()
...
def test_v31_primary_is_record_and_cause_but_never_returned_or_attached():
    primary = RuntimeError("primary")
    collector = parent_v31._CleanupFailures()
...
def test_v31_primary_attachment_surfaces_are_never_invoked():
...
def test_v31_one_shot_cleanup_failures_descriptor_cannot_create_toctou():
...
def test_v31_one_shot_notes_accessor_cannot_create_toctou():
...
def test_v31_unprintable_primary_record_uses_pointer_free_fallback():
    class Primary(RuntimeError):
        def __str__(self):
...
def test_v31_no_primary_builds_wrapper_without_attachment_failure():
    collector = parent_v31._CleanupFailures()
    collector.add("direct.kill", OSError("kill failed"))
...
def test_v31_snapshot_rejects_ordinary_assignment_and_delete():
    collector = parent_v31._CleanupFailures()
    collector.add("direct.kill", OSError("kill failed"))
    ...
    with pytest.raises(AttributeError):
        combined._v31_cleanup_failures = ()
    with pytest.raises(AttributeError):
        del combined.cleanup_failures
    with pytest.raises(AttributeError):
        del combined._v31_cleanup_failures
...
def test_v31_str_subclass_attribute_names_reject_without_running_subclass_code():
    class S(str):
        calls = 0
    ...
    hostile_name = S("_v31_cleanup_failures")
...
def test_v31_non_string_and_all_exact_string_names_reject_after_construction():
    collector = parent_v31._CleanupFailures()
    collector.add("direct.kill", OSError("kill failed"))
    combined = collector.finish(None)
...
def test_v31_reflection_is_documented_outside_runtime_object_contract():
    collector = parent_v31._CleanupFailures()
    collector.add("direct.kill", OSError("kill failed"))
    disposable = collector.finish(None)
    object.__setattr__(disposable, "_v31_cleanup_failures", ())
    assert disposable.cleanup_failures == ()
    assert parent_v31.V31ParentError.__doc__ is not None
    assert "not a same-process reflection security boundary" in parent_v31.V31ParentError.__doc__


def test_v31_current_candidate_contains_no_same_process_overclaim():
    paths = [
        ROOT / "core/capability_nexus_v31.py",
        ROOT / "tests/test_capability_nexus_v31.py",
        ROOT / "scripts/verify_phase5_capability_nexus_v31.py",
        ROOT / "scripts/verify_phase5_capability_nexus_v31_worker.py",
        ROOT / "scripts/phase5_capability_nexus_v31_subtest_reporter.py",
        ROOT / "scripts/check_phase5_capability_nexus_v31_whitespace.py",
    ]
    evidence = ROOT / "docs/onyx/checkpoints/phase5-capability-nexus-v31"
    paths.extend(
        path
        for path in (
            evidence / "PHASE5_3_CAPABILITY_NEXUS_V31_CHECKPOINT.md",
            evidence / "phase5-capability-nexus-v31.metrics.json",
            evidence / "phase5-capability-nexus-v31.bundle.json",
        )
        if path.is_file()
    )
    forbidden = (
...
def test_v31_manifests_supply_durable_integrity_not_runtime_wrapper():
    source = (ROOT / "scripts/verify_phase5_capability_nexus_v31_worker.py").read_text(
        encoding="utf-8"
    )
    ...
    assert verifier_v31.V30_ROOT_MANIFEST in verifier_v31.HISTORICAL_ROOTS
    root_manifest = ROOT / verifier_v31.V30_ROOT_MANIFEST
...
def test_hanging_child_and_grandchild_tree_is_killed_without_retry_or_marker(
    ...
    with tempfile.TemporaryDirectory(prefix="onyx-v31-process-tree-") as directory:
        ...
        with parent_v31._PrivateEvidence() as evidence_nonce:
            if hang_phase == "worker":
                (
                    parent_v31.PRIVATE_BASE / evidence_nonce / "focused.junit.xml"
                ).write_text("fixture")
            with pytest.raises(parent_v31.V31ParentError, match="timed out"):
                parent_v31._run_phase(
                    hang_phase,
                    evidence_nonce=evidence_nonce,
                    label="tree fixture",
        ...
        while (
            parent_v31._pid_exists(direct_pid) or parent_v31._pid_exists(grandchild_pid)
        ) and time.monotonic() < deadline:
            time.sleep(0.02)
        assert not parent_v31._pid_exists(direct_pid)
        assert not parent_v31._pid_exists(grandchild_pid)
        assert parent_v31.MARKER not in capsys.readouterr().out
...
def test_regression_report_semantic_tamper_fails_closed(tamper):
    report = copy.deepcopy(verifier_v31._regression_report())
    ...
    with pytest.raises(verifier_v31.V31EvidenceError):
        verifier_v31._validate_regression_report(report)
...
def test_regression_junit_attribute_and_child_tamper_fails_closed(tamper):
    report = verifier_v31._regression_report()
    tree = ET.parse(ROOT / verifier_v31.REGRESSION_JUNIT)
    ...
    with tempfile.TemporaryDirectory(prefix="onyx-v31-junit-tamper-") as directory:
        path = Path(directory) / "tampered.junit.xml"
        tree.write(path, encoding="utf-8", xml_declaration=True)
        with pytest.raises(verifier_v31.V31EvidenceError):
            verifier_v31._junit_path(path, subtest_report=report)


def test_synthetic_junit_counter_requires_explicit_validated_subtest_report():
    with tempfile.TemporaryDirectory(prefix="onyx-v31-junit-synthetic-") as directory:
        ...
        with pytest.raises(
            verifier_v31.V31EvidenceError, match="lacks validated subtest evidence"
        ):
            verifier_v31._junit_path(path)
...
def test_junit_lexical_text_tail_and_markup_tamper_fails_closed(tamper):
    ...
    with tempfile.TemporaryDirectory(prefix="onyx-v31-junit-lexical-") as directory:
        path = Path(directory) / "tampered.junit.xml"
...
def test_every_structured_evidence_field_tamper_fails_closed(path):
    ...
    with pytest.raises(verifier_v31.V31EvidenceError):
        verifier_v31._assert_exact_evidence("fixture", tampered, expected)
