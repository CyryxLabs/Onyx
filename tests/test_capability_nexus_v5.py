...
from core.capability_nexus_v5 import (
    CATALOG_CONTRACT_VERSION_V5,
    LOCK_ORDER_V5,
    CancellationTokenV5,
    ...
    CapabilityStatusV5,
    CapabilityV5ContractError,
    CapabilityV5DeniedError,
    CatalogItemV5,
    CatalogReadRequestV5,
    LocalCatalogReadAdapterV5,
    NexusFeatureGateV5,
    OperationDescriptorV5,
    OperationKindV5,
    ReadFailureClassV5,
    ReadStateV5,
    TransportKindV5,
    build_legacy_descriptors_v5,
)
import core.capability_nexus_v5 as nexus_v5_module
import scripts.verify_phase5_capability_nexus_v5 as verifier_v5
...
def _operation(**changes) -> OperationDescriptorV5:
    values = {
        "operation_id": "catalog_read", "kind": OperationKindV5.READ,
    ...
    return OperationDescriptorV5(**values)


def _descriptor(**changes) -> CapabilityDescriptorV5:
    values = {
        "capability_id": "local.catalog", "capability_version": "v5", "provider": "onyx_local",
        "transport": TransportKindV5.LOCAL, "api_name": "local_catalog",
        "api_version": CATALOG_CONTRACT_VERSION_V5, "workspace_id": WORKSPACE,
        "account_id": ACCOUNT, "profile_id": PROFILE, "credential_alias": None,
        "operations": (_operation(),), "status": CapabilityStatusV5.DISABLED,
        "status_reason": "candidate_not_activated", "limitations": ("no_dispatch",),
    }
    values.update(changes)
    return CapabilityDescriptorV5(**values)


def _items(count: int = 5) -> tuple[CatalogItemV5, ...]:
    return tuple(CatalogItemV5(f"item-{i:02d}", f"Item {i}", "fixture", "v5", ("safe",)) for i in range(count))


def _adapter(hook=lambda: None, **changes) -> LocalCatalogReadAdapterV5:
    values = {
        "items": _items(), "workspace_id": WORKSPACE, "account_id": ACCOUNT,
        "profile_id": PROFILE, "cursor_signing_key": KEY, "read_hook": hook,
        "status": CapabilityStatusV5.AVAILABLE_READ_ONLY, "status_reason": "healthy",
    }
    values.update(changes)
    return LocalCatalogReadAdapterV5(**values)


def _request(**changes) -> CatalogReadRequestV5:
    values = {
        "workspace_id": WORKSPACE, "account_id": ACCOUNT, "profile_id": PROFILE,
        "target": "local_catalog", "correlation_id": "corr-001", "page_size": 2,
    }
    values.update(changes)
    return CatalogReadRequestV5(**values)
...
def test_registry_requires_profile_and_binds_discovery_exactly():
    nexus = CapabilityNexusV5()
    descriptor = _descriptor()
    nexus.register(descriptor)
    with pytest.raises(TypeError):
        nexus.discover(
            descriptor.capability_id, workspace_id=WORKSPACE, account_id=ACCOUNT,
            expected_version="v5", expected_digest=descriptor.digest,
        )
    for profile in ("profile-other", "PROFILE-alpha"):
        with pytest.raises(CapabilityV5DeniedError):
            nexus.discover(
                descriptor.capability_id, workspace_id=WORKSPACE, account_id=ACCOUNT,
                profile_id=profile, expected_version="v5", expected_digest=descriptor.digest,
            )
...
def test_projection_constructor_rejects_cross_profile_and_authority():
    descriptor = _descriptor()
    with pytest.raises(CapabilityV5ContractError):
        CapabilityProjectionV5(
            descriptor, descriptor.digest, WORKSPACE, ACCOUNT, "profile-other",
            CapabilityStatusV5.DISABLED, "disabled",
        )
    with pytest.raises(CapabilityV5ContractError):
        CapabilityProjectionV5(
            descriptor, descriptor.digest, WORKSPACE, ACCOUNT, PROFILE,
            CapabilityStatusV5.DISABLED, "disabled", authority_granted=True,
        )


class _GateSubclass(NexusFeatureGateV5):
    pass
...
def test_registry_rejects_gate_subclasses_fakes_and_duck_types(gate):
    with pytest.raises(CapabilityV5ContractError, match="exact concrete"):
        CapabilityNexusV5(gate)  # type: ignore[arg-type]


def test_exact_gate_is_frozen_default_off_and_lock_order_is_declared():
    gate = NexusFeatureGateV5()
    nexus = CapabilityNexusV5(gate)
    assert nexus.gate is gate and nexus.lock_order == LOCK_ORDER_V5
    with pytest.raises(FrozenInstanceError):
        gate.shadow_mode = False  # type: ignore[misc]
    for values in ({"nexus_enabled": True}, {"shadow_mode": False}, {"dispatch_enabled": True}):
        with pytest.raises(CapabilityV5ContractError):
            NexusFeatureGateV5(**values)
...
def test_metadata_rejects_bounded_raw_credential_families(value):
    with pytest.raises(CapabilityV5ContractError, match="secret"):
        _descriptor(metadata=(("data_source", value),))
...
def test_schema_rejects_raw_credentials_in_nested_defaults_examples_and_descriptions(value):
    for field in ("default", "example", "description"):
        ...
        encoded = nexus_v5_module.json.dumps(schema, sort_keys=True, separators=(",", ":"))
        with pytest.raises(CapabilityV5ContractError):
...
def test_recursive_schema_cycle_depth_node_and_scalar_budgets_fail_closed():
    cycle = {}
    cycle["safe"] = cycle
    with pytest.raises(CapabilityV5ContractError, match="cycle"):
        nexus_v5_module._reject_json_secrets(cycle, "fixture")
    deep = current = {}
    for index in range(26):
        current["safe"] = {}
        current = current["safe"]
    with pytest.raises(CapabilityV5ContractError, match="depth"):
        nexus_v5_module._reject_json_secrets(deep, "fixture")
    with pytest.raises(CapabilityV5ContractError, match="node"):
        nexus_v5_module._reject_json_secrets(0, "fixture", _budget=[4096, 0])
    with pytest.raises(CapabilityV5ContractError, match="scalar byte"):
        nexus_v5_module._reject_json_secrets(["a" * 4096] * 17, "fixture")
...
def test_metadata_rejects_depth_and_expansion_bombs():
    ...
    expansion = "\ufdfa" * 500
    with pytest.raises(CapabilityV5ContractError, match="expansion"):
...
def test_metadata_cycle_detection_fails_closed(monkeypatch):
    ...
    monkeypatch.setattr(
        nexus_v5_module, "_canonical_percent_decode", lambda value: transitions.get(value)
    )
    with pytest.raises(CapabilityV5ContractError, match="cycle"):
        nexus_v5_module._validate_metadata_decoding_closure("cycle_a")
...
def test_adapter_defaults_disabled_and_has_no_dispatch_or_mutation():
    adapter = LocalCatalogReadAdapterV5(
        _items(), workspace_id=WORKSPACE, account_id=ACCOUNT, profile_id=PROFILE,
        cursor_signing_key=KEY, read_hook=lambda: None,
    )
    assert adapter.health().status is CapabilityStatusV5.DISABLED
    assert adapter.read_page(_request()).state is ReadStateV5.DENIED
...
def test_hook_runs_without_adapter_lock_and_can_reenter_health():
    ...
    assert result.state is ReadStateV5.COMPLETED
    assert observed == [CapabilityStatusV5.AVAILABLE_READ_ONLY]
...
def test_same_correlation_reentry_is_denied_before_second_hook_or_reservation():
    ...
    def hook():
        calls.append(1)
        with pytest.raises(CapabilityV5DeniedError, match="reentrancy"):
            holder["adapter"].read_page(request)

    holder["adapter"] = _adapter(hook)
    assert holder["adapter"].read_page(request).state is ReadStateV5.COMPLETED
...
def test_new_correlation_and_indirect_hook_reentry_are_denied_without_quota_drift():
    ...
    assert holder["adapter"].read_page(_request()).state is ReadStateV5.COMPLETED
    limited = holder["adapter"].read_page(_request(correlation_id="corr-after"))
    assert limited.state is ReadStateV5.RATE_LIMITED
...
def test_hook_exception_cleans_reentrancy_guard_for_later_correlation():
    ...
    assert adapter.read_page(_request()).state is ReadStateV5.UNCERTAIN
    assert adapter.read_page(_request(correlation_id="corr-002")).state is ReadStateV5.COMPLETED
    assert calls == [1, 1]
...
def test_reentrancy_guard_rejects_hook_spawned_thread_before_reservation():
    ...
    def hook():
        ...
        with ThreadPoolExecutor(max_workers=1) as pool:
            ...
            except CapabilityV5DeniedError as exc:
                nested_errors.append(str(exc))

    holder["adapter"] = _adapter(hook)
    outer = holder["adapter"].read_page(_request())
    assert outer.state is ReadStateV5.COMPLETED
    assert nested_errors == ["catalog read reentrancy is denied"]
    assert len(calls) == 1 and holder["adapter"].health().quota_remaining == 99
    assert holder["adapter"].read_page(_request(correlation_id="corr-after")).state is ReadStateV5.COMPLETED
...
def test_hook_guard_is_per_adapter_and_does_not_hold_lock_during_callback():
    ...
    assert first.read_page(_request()).state is ReadStateV5.COMPLETED
    assert observed == [ReadStateV5.COMPLETED]


def test_historical_leaf_budget_exact_boundary_and_overflow():
    leaves = {f"leaf-{index}": "a" * 64 for index in range(4095)}
    verifier_v5._record_historical_leaf(leaves, "boundary", "b" * 64)
    assert len(leaves) == 4096
    with pytest.raises(verifier_v5.V5EvidenceError, match="leaf budget"):
        verifier_v5._record_historical_leaf(leaves, "overflow", "c" * 64)
    with pytest.raises(verifier_v5.V5EvidenceError, match="digest conflict"):
        verifier_v5._record_historical_leaf(leaves, "boundary", "d" * 64)
...
def test_wide_historical_manifest_and_cycle_fail_before_expansion(monkeypatch):
    ...
    monkeypatch.setattr(verifier_v5, "_historical_manifest", lambda relative: wide)
    monkeypatch.setattr(verifier_v5, "_verify_historical_path", lambda relative, digest: None)
    with pytest.raises(verifier_v5.V5EvidenceError, match="leaf budget"):
        verifier_v5._verify_manifest_tree("wide.sha256", set(), set(), {}, 0)

    graph = {
        "a.sha256": (("a" * 64, "b.sha256"),),
        "b.sha256": (("b" * 64, "a.sha256"),),
    }
    monkeypatch.setattr(verifier_v5, "_historical_manifest", graph.__getitem__)
    with pytest.raises(verifier_v5.V5EvidenceError, match="cycle"):
        verifier_v5._verify_manifest_tree("a.sha256", set(), set(), {}, 0)


def test_exact_4096_leaf_manifest_is_accepted(monkeypatch):
    entries = tuple(("a" * 64, f"leaf-{index}") for index in range(4096))
    monkeypatch.setattr(verifier_v5, "_historical_manifest", lambda relative: entries)
    monkeypatch.setattr(verifier_v5, "_verify_historical_path", lambda relative, digest: None)
    leaves = {}
    verifier_v5._verify_manifest_tree("boundary.sha256", set(), set(), leaves, 0)
    assert len(leaves) == 4096


def test_historical_manifest_rejects_path_traversal_and_symlink(monkeypatch):
    with tempfile.TemporaryDirectory(prefix="p53-v5-path-", dir=ROOT) as directory:
        base = Path(directory)
        monkeypatch.setattr(verifier_v5, "PROJECT", base)
        manifest = base / "bad.sha256"
        manifest.write_text("" + "a" * 64 + "  ../escape\n", encoding="utf-8")
        with pytest.raises(verifier_v5.V5EvidenceError, match="unsafe"):
            verifier_v5._historical_manifest("bad.sha256")
        ...
        with pytest.raises(verifier_v5.V5EvidenceError, match="mismatch"):
            verifier_v5._verify_historical_path("link.txt", hashlib.sha256(target.read_bytes()).hexdigest())
...
def test_concurrent_duplicate_is_pending_then_completed_and_hook_once():
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        release.set()
        assert future.result().state is ReadStateV5.COMPLETED
...
def test_same_correlation_payload_drift_denies_without_hook_or_quota():
    ...
    with pytest.raises(CapabilityV5DeniedError, match="drift"):
...
def test_hook_exception_becomes_uncertain_and_never_replays_or_leaks_exception():
    ...
    assert result.state is ReadStateV5.UNCERTAIN
    assert result.failure_class is ReadFailureClassV5.HOOK_EXCEPTION
...
def test_cancelled_before_hook_rolls_back_quota_and_rate_and_never_calls_hook():
    ...
    assert result.state is ReadStateV5.CANCELLED_BEFORE_HOOK
...
def test_timeout_after_hook_is_uncertain_and_not_replayed():
    ...
    result = adapter.read_page(request)
    assert result.state is ReadStateV5.UNCERTAIN
    assert result.failure_class is ReadFailureClassV5.TIMEOUT
    assert adapter.read_page(request) == result and calls == [1]


@pytest.mark.parametrize(("action", "failure"), [("kill", ReadFailureClassV5.KILL), ("revoke", ReadFailureClassV5.REVOKED)])
def test_kill_and_revoke_during_hook_latch_uncertain(action, failure):
    ...
    result = holder["adapter"].read_page(_request())
    assert result.state is ReadStateV5.UNCERTAIN and result.failure_class is failure
...
def test_pending_expiry_becomes_uncertain_and_late_hook_cannot_commit():
    ...
    with ThreadPoolExecutor(max_workers=1) as pool:
        ...
        assert reconciled.state is ReadStateV5.UNCERTAIN
        assert reconciled.failure_class is ReadFailureClassV5.EXPIRED_PENDING
        release.set()
        assert future.result().state is ReadStateV5.UNCERTAIN
...
def test_uncertain_expiry_keeps_no_replay_tombstone_and_frees_uncertain_capacity():
    ...
    assert adapter.read_page(first_request).state is ReadStateV5.UNCERTAIN
    with pytest.raises(CapabilityV5DeniedError, match="pending/uncertain"):
        adapter.read_page(_request(correlation_id="corr-002"))
    now[0] = 2.0
    expired = adapter.reconcile(first_request)
    assert expired.state is ReadStateV5.EXPIRED_UNCERTAIN
    assert expired.failure_class is ReadFailureClassV5.UNCERTAIN_EXPIRED
    assert adapter.read_page(first_request) == expired and calls == [1]
    assert adapter.read_page(_request(correlation_id="corr-002")).state is ReadStateV5.UNCERTAIN
...
def test_ledger_capacity_is_bounded_and_does_not_evict_for_replay():
    ...
    request = _request()
    assert adapter.read_page(request).state is ReadStateV5.COMPLETED
    with pytest.raises(CapabilityV5DeniedError, match="capacity"):
        adapter.read_page(_request(correlation_id="corr-002"))
    assert adapter.read_page(request).state is ReadStateV5.COMPLETED
...
def test_cursor_snapshot_and_signature_tamper_fail_closed():
    ...
    changed = LocalCatalogReadAdapterV5(
        _items(6), workspace_id=WORKSPACE, account_id=ACCOUNT, profile_id=PROFILE,
        cursor_signing_key=KEY, read_hook=lambda: None, status=CapabilityStatusV5.AVAILABLE_READ_ONLY,
...
def test_rate_limit_and_quota_are_reserved_once_and_truthful():
    ...
    assert adapter.read_page(_request()).state is ReadStateV5.COMPLETED
    limited = adapter.read_page(_request(correlation_id="corr-002"))
    assert limited.state is ReadStateV5.RATE_LIMITED and limited.retry_after_ms == 1000
...
def test_unavailable_auth_scope_disabled_degraded_and_unknown_reconcile():
    assert _adapter(auth_available=False).read_page(_request()).failure_class is ReadFailureClassV5.AUTH
    assert _adapter(granted_scopes=()).read_page(_request()).failure_class is ReadFailureClassV5.SCOPE
    assert _adapter(status=CapabilityStatusV5.DISABLED, status_reason="operator_disabled").read_page(_request()).state is ReadStateV5.DENIED
    degraded = _adapter(status=CapabilityStatusV5.DEGRADED, status_reason="partial_fixture")
    assert degraded.read_page(_request()).state is ReadStateV5.COMPLETED
    unknown = degraded.reconcile(_request(correlation_id="unknown-corr"))
    assert unknown.state is ReadStateV5.UNKNOWN_CORRELATION
...
def test_legacy_builder_preserves_current_32_declarations_and_policies_without_import():
    ...
    built = build_legacy_descriptors_v5(
    ...
    assert all(item.status is CapabilityStatusV5.DISABLED for item in built.descriptors)
...
def test_no_live_runtime_source_references_v5():
    live = [ROOT / "main.py", ROOT / "ui.py", ROOT / "scripts" / "launch_onyx.pyw"]
    live += list((ROOT / "actions").rglob("*.py")) + list((ROOT / "dashboard").rglob("*.py"))
    live += [path for path in (ROOT / "core").glob("*.py") if path.name != "capability_nexus_v5.py"]
    assert all("capability_nexus_v5" not in path.read_text(encoding="utf-8", errors="ignore") for path in live)
