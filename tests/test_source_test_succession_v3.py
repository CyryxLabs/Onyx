...
from scripts import source_test_succession_v3 as route
...
def test_replacement_rebuilds_explicit_fixtures_but_keeps_autouse_and_exact_node(tmp_path):
    ...
    (tmp_path / "conftest.py").write_text(
        ...
        "from scripts.source_test_succession_v3 import replace_collected_item\n"
...
def test_closed_routes_preserve_inheritance_and_specific_negative_coverage():
    ...
    assert broker["successors"] == [
        ...
        "tests/test_release_workflow_transition_v101.py::test_rejects_linked_permission_broker",
    ]
...
def test_current_source_rechecked_before_cached_success(monkeypatch):
    from scripts import verify_release_workflow_v101 as source

    record = route.load_record(ROOT)
    paths = {*record["bindings"], route.RECORD, route.HELPER, route.TEST}
    current = {
        "publishable": False, "formal_release_ready": False,
        "transition": {"logical_sequence": 101, "current_release_paths": [
    ...
    def verify(root):
        ...
        return current
    monkeypatch.setattr(source, "verify_release_workflow_v101", verify)
