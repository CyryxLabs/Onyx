...
from core.capability_nexus_v22 import (
    CATALOG_CONTRACT_VERSION_V22,
    LOCK_ORDER_V22,
    CancellationTokenV22,
    CapabilityDescriptorV22,
    CapabilityNexusV22,
    CapabilityProjectionV22,
    CapabilityStatusV22,
    CapabilityV22ContractError,
    CapabilityV22DeniedError,
    CatalogItemV22,
    CatalogReadRequestV22,
    LocalCatalogReadAdapterV22,
    NexusFeatureGateV22,
    OperationDescriptorV22,
    OperationKindV22,
    ReadFailureClassV22,
    ReadStateV22,
    TransportKindV22,
    build_legacy_descriptors_v22,
)
import core.capability_nexus_v22 as nexus_v22_module
import scripts.verify_phase5_capability_nexus_v22 as parent_v22
import scripts.verify_phase5_capability_nexus_v22_worker as verifier_v22
...
def _operation(**changes) -> OperationDescriptorV22:
    values = {
        "operation_id": "catalog_read", "kind": OperationKindV22.READ,
    ...
    return OperationDescriptorV22(**values)


def _descriptor(**changes) -> CapabilityDescriptorV22:
    values = {
        "capability_id": "local.catalog", "capability_version": "v22", "provider": "onyx_local",
        "transport": TransportKindV22.LOCAL, "api_name": "local_catalog",
        "api_version": CATALOG_CONTRACT_VERSION_V22, "workspace_id": WORKSPACE,
        "account_id": ACCOUNT, "profile_id": PROFILE, "credential_alias": None,
        "operations": (_operation(),), "status": CapabilityStatusV22.DISABLED,
        "status_reason": "candidate_not_activated", "limitations": ("no_dispatch",),
    }
    values.update(changes)
    return CapabilityDescriptorV22(**values)


def _items(count: int = 5) -> tuple[CatalogItemV22, ...]:
    return tuple(CatalogItemV22(f"item-{i:02d}", f"Item {i}", "fixture", "v22", ("safe",)) for i in range(count))


def _adapter(hook=lambda: None, **changes) -> LocalCatalogReadAdapterV22:
    values = {
        "items": _items(), "workspace_id": WORKSPACE, "account_id": ACCOUNT,
        "profile_id": PROFILE, "cursor_signing_key": KEY, "read_hook": hook,
        "status": CapabilityStatusV22.AVAILABLE_READ_ONLY, "status_reason": "healthy",
    }
    values.update(changes)
    return LocalCatalogReadAdapterV22(**values)


def _request(**changes) -> CatalogReadRequestV22:
    values = {
        "workspace_id": WORKSPACE, "account_id": ACCOUNT, "profile_id": PROFILE,
        "target": "local_catalog", "correlation_id": "corr-001", "page_size": 2,
    }
    values.update(changes)
    return CatalogReadRequestV22(**values)
...
def test_v20_windows_suspended_job_path_is_exact_after_version_normalization():
    prior = ast.parse((ROOT / "scripts/verify_phase5_capability_nexus_v20.py").read_text(encoding="utf-8"))
    current = ast.parse((ROOT / "scripts/verify_phase5_capability_nexus_v22.py").read_text(encoding="utf-8"))
    names = {
        "_kernel32", "_checked_close", "_WindowsJob", "_wait_job_empty",
        "_create_file_handle", "_bounded_output", "_windows_phase",
    }
    ...
    normalized = {
        name: source.replace("V22", "V20").replace("v22", "v20")
...
def test_registry_requires_profile_and_binds_discovery_exactly():
    nexus = CapabilityNexusV22()
    descriptor = _descriptor()
    nexus.register(descriptor)
    with pytest.raises(TypeError):
        nexus.discover(
            descriptor.capability_id, workspace_id=WORKSPACE, account_id=ACCOUNT,
            expected_version="v22", expected_digest=descriptor.digest,
        )
    for profile in ("profile-other", "PROFILE-alpha"):
        with pytest.raises(CapabilityV22DeniedError):
            nexus.discover(
                descriptor.capability_id, workspace_id=WORKSPACE, account_id=ACCOUNT,
                profile_id=profile, expected_version="v22", expected_digest=descriptor.digest,
            )
...
def test_projection_constructor_rejects_cross_profile_and_authority():
    descriptor = _descriptor()
    with pytest.raises(CapabilityV22ContractError):
        CapabilityProjectionV22(
            descriptor, descriptor.digest, WORKSPACE, ACCOUNT, "profile-other",
            CapabilityStatusV22.DISABLED, "disabled",
        )
    with pytest.raises(CapabilityV22ContractError):
        CapabilityProjectionV22(
            descriptor, descriptor.digest, WORKSPACE, ACCOUNT, PROFILE,
            CapabilityStatusV22.DISABLED, "disabled", authority_granted=True,
        )


class _GateSubclass(NexusFeatureGateV22):
    pass
...
def test_registry_rejects_gate_subclasses_fakes_and_duck_types(gate):
    with pytest.raises(CapabilityV22ContractError, match="exact concrete"):
        CapabilityNexusV22(gate)  # type: ignore[arg-type]


def test_exact_gate_is_frozen_default_off_and_lock_order_is_declared():
    gate = NexusFeatureGateV22()
    nexus = CapabilityNexusV22(gate)
    assert nexus.gate is gate and nexus.lock_order == LOCK_ORDER_V22
    with pytest.raises(FrozenInstanceError):
        gate.shadow_mode = False  # type: ignore[misc]
    for values in ({"nexus_enabled": True}, {"shadow_mode": False}, {"dispatch_enabled": True}):
        with pytest.raises(CapabilityV22ContractError):
            NexusFeatureGateV22(**values)
...
@pytest.mark.parametrize("prefix", sorted(nexus_v22_module._SECRET_NAMES))
...
@pytest.mark.parametrize("name", sorted(nexus_v22_module._SECRET_NAMES))
...
@pytest.mark.parametrize("name", sorted(nexus_v22_module._SECRET_NAMES | nexus_v22_module._CREDENTIAL_OPAQUE_NAMES))
@pytest.mark.parametrize("position", ["leading", "middle", "trailing"])
...
def test_natural_looking_suffix_never_blesses_keyboard_or_sequential_material(opaque, position):
    values = {
...
@pytest.mark.parametrize(
    "word",
    [
        "responsibilities", "telecommunications", "microarchitecture",
        "multidisciplinary", "observability", "extensibility", "portability",
        "maintainability", "deployment", "orchestration", "federation",
        "synchronization", "platform", "database", "service", "endpoint", "registry",
    ],
)
def test_embedded_ngram_model_accepts_held_out_natural_boundary_corpus(word):
    assert nexus_v22_module._english_word_score(word) >= nexus_v22_module._NATURAL_SCORE_THRESHOLD


@pytest.mark.parametrize(
    "word",
    [
        "qwertyuiopasdfghization", "abcdefghijklmnop", "bcdfghjklmnpqrst",
        "zxcvbnmasdfghjkl", "poiuytrewqlkjhgfment", "aaaaaaaaaaaaaaaa",
    ],
)
...
@pytest.mark.parametrize("protected", ["tóken", "to\u0301ken", "áuth", "a\u0301uth", "tóкen"])
@pytest.mark.parametrize("encoding", ["plain", "base64", "percent"])
@pytest.mark.parametrize("delimiter", ["|", "@", "•", "—"])
def test_composed_decomposed_diacritic_and_confusable_names_reject_every_encoding(protected, encoding, delimiter):
...
def test_schema_rejects_raw_credentials_in_nested_defaults_examples_and_descriptions(value):
    for field in ("default", "example", "description"):
        ...
        encoded = nexus_v22_module.json.dumps(schema, sort_keys=True, separators=(",", ":"))
        with pytest.raises(CapabilityV22ContractError):
...
def test_recursive_schema_cycle_depth_node_and_scalar_budgets_fail_closed():
    cycle = {}
    cycle["safe"] = cycle
    with pytest.raises(CapabilityV22ContractError, match="cycle"):
        nexus_v22_module._reject_json_secrets(cycle, "fixture")
    deep = current = {}
    for index in range(26):
        current["safe"] = {}
        current = current["safe"]
    with pytest.raises(CapabilityV22ContractError):
        nexus_v22_module._reject_json_secrets(deep, "fixture")
    with pytest.raises(CapabilityV22ContractError, match="node"):
        nexus_v22_module._reject_json_secrets(0, "fixture", _budget=[4096, 0])
    with pytest.raises(CapabilityV22ContractError, match="scalar byte"):
        nexus_v22_module._reject_json_secrets(["a" * 4096] * 17, "fixture")
...
def test_metadata_contract_requires_exact_case_ascii_type_and_canonical_form(key, value):
    with pytest.raises(CapabilityV22ContractError):
...
def test_every_non_hash_metadata_key_is_a_closed_enum(key, value):
    with pytest.raises(CapabilityV22ContractError, match="closed enum"):
        _descriptor(metadata=((key, value),))
...
def test_metadata_enum_table_is_immutable_and_future_values_require_source_change():
    assert dict(nexus_v22_module._METADATA_ENUMS) == {
    ...
    with pytest.raises(TypeError):
        nexus_v22_module._METADATA_ENUMS["data_source"] = frozenset({"future_catalog"})
    with pytest.raises(AttributeError):
        nexus_v22_module._METADATA_ENUMS["data_source"].add("future_catalog")
...
def test_benign_human_text_remains_valid_only_in_description_context(description):
    ...
    schema = {"properties": {"safe_field": {"description": description, "type": "STRING"}}, "type": "OBJECT"}
    encoded = nexus_v22_module.json.dumps(schema, sort_keys=True, separators=(",", ":"), ensure_ascii=False)
...
def test_description_context_preserves_decoded_credential_and_confusable_rejection(description):
    with pytest.raises(CapabilityV22ContractError):
        _operation(description=description)


def test_legacy_declaration_description_uses_the_same_content_scanner():
    declarations = [dict(item) for item in _assignment(ROOT / "main.py", "TOOL_DECLARATIONS")]
    declarations[0] = {
        **declarations[0],
...
def test_content_decoding_cycle_detection_fails_closed(monkeypatch):
    ...
    monkeypatch.setattr(
        nexus_v22_module, "_canonical_percent_decode", lambda value: transitions.get(value)
    )
    with pytest.raises(CapabilityV22ContractError, match="cycle"):
        nexus_v22_module._validate_content_decoding_closure("cycle_a")
...
def test_adapter_defaults_disabled_and_has_no_dispatch_or_mutation():
    adapter = LocalCatalogReadAdapterV22(
        _items(), workspace_id=WORKSPACE, account_id=ACCOUNT, profile_id=PROFILE,
        cursor_signing_key=KEY, read_hook=lambda: None,
    )
    assert adapter.health().status is CapabilityStatusV22.DISABLED
    assert adapter.read_page(_request()).state is ReadStateV22.DENIED
...
def test_hook_runs_without_adapter_lock_and_can_reenter_health():
    ...
    assert result.state is ReadStateV22.COMPLETED
    assert observed == [CapabilityStatusV22.AVAILABLE_READ_ONLY]
...
def test_same_correlation_reentry_is_denied_before_second_hook_or_reservation():
    ...
    def hook():
        calls.append(1)
        with pytest.raises(CapabilityV22DeniedError, match="reentrancy"):
            holder["adapter"].read_page(request)

    holder["adapter"] = _adapter(hook)
    assert holder["adapter"].read_page(request).state is ReadStateV22.COMPLETED
...
def test_new_correlation_and_indirect_hook_reentry_are_denied_without_quota_drift():
    ...
    assert holder["adapter"].read_page(_request()).state is ReadStateV22.COMPLETED
    limited = holder["adapter"].read_page(_request(correlation_id="corr-after"))
    assert limited.state is ReadStateV22.RATE_LIMITED
...
def test_hook_exception_cleans_reentrancy_guard_for_later_correlation():
    ...
    assert adapter.read_page(_request()).state is ReadStateV22.UNCERTAIN
    assert adapter.read_page(_request(correlation_id="corr-002")).state is ReadStateV22.COMPLETED
    assert calls == [1, 1]
...
def test_reentrancy_guard_rejects_hook_spawned_thread_before_reservation():
    ...
    def hook():
        ...
        with ThreadPoolExecutor(max_workers=1) as pool:
            ...
            except CapabilityV22DeniedError as exc:
                nested_errors.append(str(exc))

    holder["adapter"] = _adapter(hook)
    outer = holder["adapter"].read_page(_request())
    assert outer.state is ReadStateV22.COMPLETED
    assert nested_errors == ["catalog read reentrancy is denied"]
    assert len(calls) == 1 and holder["adapter"].health().quota_remaining == 99
    assert holder["adapter"].read_page(_request(correlation_id="corr-after")).state is ReadStateV22.COMPLETED
...
def test_hook_guard_is_per_adapter_and_does_not_hold_lock_during_callback():
    ...
    assert first.read_page(_request()).state is ReadStateV22.COMPLETED
    assert observed == [ReadStateV22.COMPLETED]


def test_historical_leaf_budget_exact_boundary_and_overflow():
    leaves = {f"leaf-{index}": "a" * 64 for index in range(4095)}
    verifier_v22._record_historical_leaf(leaves, "boundary", "b" * 64)
    assert len(leaves) == 4096
    with pytest.raises(verifier_v22.V22EvidenceError, match="leaf budget"):
        verifier_v22._record_historical_leaf(leaves, "overflow", "c" * 64)
    with pytest.raises(verifier_v22.V22EvidenceError, match="digest conflict"):
        verifier_v22._record_historical_leaf(leaves, "boundary", "d" * 64)
...
def test_wide_historical_manifest_and_cycle_fail_before_expansion(monkeypatch):
    ...
    monkeypatch.setattr(verifier_v22, "_historical_manifest", lambda relative: wide)
    monkeypatch.setattr(verifier_v22, "_verify_historical_path", lambda relative, digest: None)
    with pytest.raises(verifier_v22.V22EvidenceError, match="leaf budget"):
        verifier_v22._verify_manifest_tree("wide.sha256", set(), set(), {}, 0)

    graph = {
        "a.sha256": (("a" * 64, "b.sha256"),),
        "b.sha256": (("b" * 64, "a.sha256"),),
    }
    monkeypatch.setattr(verifier_v22, "_historical_manifest", graph.__getitem__)
    with pytest.raises(verifier_v22.V22EvidenceError, match="cycle"):
        verifier_v22._verify_manifest_tree("a.sha256", set(), set(), {}, 0)


def test_exact_4096_leaf_manifest_is_accepted(monkeypatch):
    entries = tuple(("a" * 64, f"leaf-{index}") for index in range(4096))
    monkeypatch.setattr(verifier_v22, "_historical_manifest", lambda relative: entries)
    monkeypatch.setattr(verifier_v22, "_verify_historical_path", lambda relative, digest: None)
    leaves = {}
    verifier_v22._verify_manifest_tree("boundary.sha256", set(), set(), leaves, 0)
    assert len(leaves) == 4096


def test_historical_manifest_rejects_path_traversal_and_symlink(monkeypatch):
    with tempfile.TemporaryDirectory(prefix="p53-v22-path-", dir=ROOT) as directory:
        base = Path(directory)
        monkeypatch.setattr(verifier_v22, "PROJECT", base)
        manifest = base / "bad.sha256"
        manifest.write_text("" + "a" * 64 + "  ../escape\n", encoding="utf-8")
        with pytest.raises(verifier_v22.V22EvidenceError, match="unsafe"):
            verifier_v22._historical_manifest("bad.sha256")
        ...
        verifier_v22._verify_historical_path(
        ...
        with pytest.raises(verifier_v22.V22EvidenceError, match="reparse"):
            verifier_v22._verify_historical_path("link.txt", hashlib.sha256(target.read_bytes()).hexdigest())
...
def test_concurrent_duplicate_is_pending_then_completed_and_hook_once():
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        release.set()
        assert future.result().state is ReadStateV22.COMPLETED
...
def test_validation_exception_releases_admission_claim():
    ...
    assert adapter.read_page(_request()).state is ReadStateV22.COMPLETED
...
def test_same_correlation_payload_drift_denies_without_hook_or_quota():
    ...
    with pytest.raises(CapabilityV22DeniedError, match="drift"):
...
def test_hook_exception_becomes_uncertain_and_never_replays_or_leaks_exception():
    ...
    assert result.state is ReadStateV22.UNCERTAIN
    assert result.failure_class is ReadFailureClassV22.HOOK_EXCEPTION
...
def test_cancelled_before_hook_rolls_back_quota_and_rate_and_never_calls_hook():
    ...
    assert result.state is ReadStateV22.CANCELLED_BEFORE_HOOK
...
def test_timeout_after_hook_is_uncertain_and_not_replayed():
    ...
    result = adapter.read_page(request)
    assert result.state is ReadStateV22.UNCERTAIN
    assert result.failure_class is ReadFailureClassV22.TIMEOUT
    assert adapter.read_page(request) == result and calls == [1]


@pytest.mark.parametrize(("action", "failure"), [("kill", ReadFailureClassV22.KILL), ("revoke", ReadFailureClassV22.REVOKED)])
def test_kill_and_revoke_during_hook_latch_uncertain(action, failure):
    ...
    result = holder["adapter"].read_page(_request())
    assert result.state is ReadStateV22.UNCERTAIN and result.failure_class is failure
...
def test_pending_expiry_becomes_uncertain_and_late_hook_cannot_commit():
    ...
    with ThreadPoolExecutor(max_workers=1) as pool:
        ...
        assert reconciled.state is ReadStateV22.UNCERTAIN
        assert reconciled.failure_class is ReadFailureClassV22.EXPIRED_PENDING
        release.set()
        assert future.result().state is ReadStateV22.UNCERTAIN
...
def test_uncertain_expiry_keeps_no_replay_tombstone_and_frees_uncertain_capacity():
    ...
    assert adapter.read_page(first_request).state is ReadStateV22.UNCERTAIN
    with pytest.raises(CapabilityV22DeniedError, match="pending/uncertain"):
        adapter.read_page(_request(correlation_id="corr-002"))
    now[0] = 2.0
    expired = adapter.reconcile(first_request)
    assert expired.state is ReadStateV22.EXPIRED_UNCERTAIN
    assert expired.failure_class is ReadFailureClassV22.UNCERTAIN_EXPIRED
    assert adapter.read_page(first_request) == expired and calls == [1]
    assert adapter.read_page(_request(correlation_id="corr-002")).state is ReadStateV22.UNCERTAIN
...
def test_ledger_capacity_is_bounded_and_does_not_evict_for_replay():
    ...
    request = _request()
    assert adapter.read_page(request).state is ReadStateV22.COMPLETED
    with pytest.raises(CapabilityV22DeniedError, match="capacity"):
        adapter.read_page(_request(correlation_id="corr-002"))
    assert adapter.read_page(request).state is ReadStateV22.COMPLETED
...
def test_cursor_snapshot_and_signature_tamper_fail_closed():
    ...
    changed = LocalCatalogReadAdapterV22(
        _items(6), workspace_id=WORKSPACE, account_id=ACCOUNT, profile_id=PROFILE,
        cursor_signing_key=KEY, read_hook=lambda: None, status=CapabilityStatusV22.AVAILABLE_READ_ONLY,
...
def test_rate_limit_and_quota_are_reserved_once_and_truthful():
    ...
    assert limited.state is ReadStateV22.RATE_LIMITED and limited.retry_after_ms == 1000
...
def test_unavailable_auth_scope_disabled_degraded_and_unknown_reconcile():
    assert _adapter(auth_available=False).read_page(_request()).failure_class is ReadFailureClassV22.AUTH
    assert _adapter(granted_scopes=()).read_page(_request()).failure_class is ReadFailureClassV22.SCOPE
    assert _adapter(status=CapabilityStatusV22.DISABLED, status_reason="operator_disabled").read_page(_request()).state is ReadStateV22.DENIED
    degraded = _adapter(status=CapabilityStatusV22.DEGRADED, status_reason="partial_fixture")
    assert degraded.read_page(_request()).state is ReadStateV22.COMPLETED
    unknown = degraded.reconcile(_request(correlation_id="unknown-corr"))
    assert unknown.state is ReadStateV22.UNKNOWN_CORRELATION
...
def test_legacy_builder_preserves_current_32_declarations_and_policies_without_import():
    ...
    policies = _assignment(ROOT / "core" / "permission_broker.py", "MODEL_TOOL_POLICIES")
    built = build_legacy_descriptors_v22(
        declarations, policies, workspace_id=WORKSPACE, account_id=ACCOUNT, profile_id=PROFILE,
    )
    ...
    assert all(item.status is CapabilityStatusV22.DISABLED for item in built.descriptors)
...
def test_no_live_runtime_source_references_v22():
    live = [ROOT / "main.py", ROOT / "ui.py", ROOT / "scripts" / "launch_onyx.pyw"]
    live += list((ROOT / "actions").rglob("*.py")) + list((ROOT / "dashboard").rglob("*.py"))
    live += [path for path in (ROOT / "core").glob("*.py") if path.name != "capability_nexus_v22.py"]
    assert all("capability_nexus_v22" not in path.read_text(encoding="utf-8", errors="ignore") for path in live)


def test_phase_runner_rejects_every_arbitrary_command_surface():
    with pytest.raises(parent_v22.V22ParentError, match="phase id"):
        parent_v22._run_phase(
            [sys.executable, "-c", "print('forbidden')"],
            evidence_nonce="a" * 64, label="x", timeout=1,
        )
    with pytest.raises(parent_v22.V22ParentError, match="phase id"):
        parent_v22._phase_command("arbitrary", Path("focused.junit.xml"))


def test_parent_has_two_direct_bounded_phases_and_pure_worker_has_no_subprocess_surface():
    parent_tree = ast.parse((ROOT / "scripts/verify_phase5_capability_nexus_v22.py").read_text(encoding="utf-8"))
    worker_tree = ast.parse((ROOT / "scripts/verify_phase5_capability_nexus_v22_worker.py").read_text(encoding="utf-8"))
    popen_calls = [
    ...
    bounded_calls = [
        node for node in ast.walk(parent_tree)
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id == "_run_phase"
    ]
    assert len(popen_calls) == 1
    assert len(bounded_calls) == 2
    assert all(any(keyword.arg == "timeout" for keyword in node.keywords) for node in bounded_calls)
    assert not any(isinstance(node, (ast.Import, ast.ImportFrom)) and "subprocess" in ast.unparse(node) for node in ast.walk(worker_tree))
    assert not any(
        isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute) and node.func.attr in {"run", "Popen"}
        and isinstance(node.func.value, ast.Name) and node.func.value.id == "subprocess"
        for node in ast.walk(worker_tree)
    )
    source = (ROOT / "scripts/verify_phase5_capability_nexus_v22.py").read_text(encoding="utf-8")
    assert all(token in source for token in (
        "_KILL_ON_JOB_CLOSE", "CreateJobObjectW", "AssignProcessToJobObject",
        "TerminateJobObject", "QueryInformationJobObject", "start_new_session", "os.killpg",
    ))
    assert not (ROOT / "scripts/phase5_capability_nexus_v22_bootstrap.py").exists()
    assert "bootstrap" not in source.lower()
    assert "CreateProcessW" in source and "_CREATE_SUSPENDED" in source and "ResumeThread" in source


def test_frozen_history_proof_never_spawns_a_recursive_verifier():
    v14 = verifier_v22._validate_frozen_v14_proof()
    v15 = verifier_v22._validate_frozen_v15_proof()
    v16 = verifier_v22._validate_frozen_v16_proof()
    v17 = verifier_v22._validate_frozen_v17_proof()
    v18 = verifier_v22._validate_frozen_v18_proof()
    v19 = verifier_v22._validate_frozen_v19_proof()
    v20 = verifier_v22._validate_frozen_v20_proof()
    v21 = verifier_v22._validate_frozen_v21_proof()
    source = (ROOT / "scripts/verify_phase5_capability_nexus_v22_worker.py").read_text(encoding="utf-8")
    assert all(item["marker_contract_bound"] is True for item in (v14, v15, v16, v17, v18, v19, v20, v21))
    assert "_run_frozen_verifiers" not in source
    assert "import subprocess" not in source


def test_frozen_v14_proof_rejects_a_historical_hash_tamper(monkeypatch):
    original = verifier_v22._sha

    def tampered(path):
        if path == ROOT / verifier_v22.V14_ARTIFACT_MANIFEST:
            return "0" * 64
        return original(path)

    monkeypatch.setattr(verifier_v22, "_sha", tampered)
    with pytest.raises(verifier_v22.V22EvidenceError, match="historical recursive leaf mismatch"):
        verifier_v22._validate_frozen_v14_proof()


@pytest.mark.skipif(sys.platform != "win32", reason="Windows Job ABI")
def test_kernel32_job_abi_is_explicit_and_pointer_safe():
    api = parent_v22._kernel32()
    assert ctypes.sizeof(parent_v22.wintypes.HANDLE) == ctypes.sizeof(ctypes.c_void_p)
    for name in (
        "CreateJobObjectW", "SetInformationJobObject", "AssignProcessToJobObject",
        "TerminateJobObject", "QueryInformationJobObject", "CloseHandle",
        "OpenProcess", "GetExitCodeProcess", "CreateFileW", "CreateProcessW",
        "ResumeThread", "WaitForSingleObject", "TerminateProcess",
    ):
...
@pytest.mark.parametrize("raw", [True, 0, -1, 1 << (ctypes.sizeof(ctypes.c_void_p) * 8)])
def test_invalid_or_truncated_windows_handles_fail_closed(raw):
    with pytest.raises(parent_v22.V22ParentError, match="invalid or truncated"):
        parent_v22._validated_handle(raw)


def test_old_v19_bootstrap_invocation_path_is_absent_in_v22():
    source = (ROOT / "scripts/verify_phase5_capability_nexus_v22.py").read_text(encoding="utf-8")
    assert "BOOTSTRAP" not in source
    assert "HELLO" not in source and "READY" not in source and "GO " not in source
    assert "ONYX_P53_V22" not in source


def test_worker_rejects_arbitrary_fresh_junit_path():
    with tempfile.TemporaryDirectory(prefix="onyx-v22-arbitrary-junit-") as directory:
        path = Path(directory) / "focused.junit.xml"
        path.write_text("fixture", encoding="utf-8")
        with pytest.raises(verifier_v22.V22EvidenceError, match="nonce-scoped"):
            verifier_v22._validated_fresh_focused_junit(path)
...
def test_delayed_job_assignment_cannot_execute_target_before_resume(monkeypatch):
    ...
    original = parent_v22._WindowsJob.assign_handle
    with tempfile.TemporaryDirectory(prefix="onyx-v22-delayed-assign-") as directory:
        ...
        monkeypatch.setattr(parent_v22._WindowsJob, "assign_handle", delayed)
        with parent_v22._PrivateEvidence() as evidence_nonce, ThreadPoolExecutor(max_workers=1) as pool:
            future = pool.submit(
                parent_v22._windows_phase, "focused", evidence_nonce=evidence_nonce,
                label="delayed assignment", timeout=10, env=None, stdout_mode="pipe",
            )
...
def test_assign_failure_terminates_suspended_target_before_execution(monkeypatch):
    with tempfile.TemporaryDirectory(prefix="onyx-v22-assign-failure-") as directory:
        marker = Path(directory) / "TARGET_RAN"
        target = Path(directory) / "target.py"
        target.write_text("from pathlib import Path\nimport sys\nPath(sys.argv[1]).write_text('ran')\n")
        monkeypatch.setattr(
        ...
        def rejected(_job, _process_handle):
            raise parent_v22.V22ParentError("injected AssignProcessToJobObject failure")

        monkeypatch.setattr(parent_v22._WindowsJob, "assign_handle", rejected)
        with parent_v22._PrivateEvidence() as evidence_nonce:
            with pytest.raises(parent_v22.V22ParentError, match="injected Assign"):
                parent_v22._windows_phase(
                    "focused", evidence_nonce=evidence_nonce, label="assign failure",
                    timeout=5, env=None, stdout_mode="pipe",
                )
        assert not marker.exists()
...
def test_resume_failure_terminates_target_before_execution(monkeypatch):
    with tempfile.TemporaryDirectory(prefix="onyx-v22-resume-failure-") as directory:
        marker = Path(directory) / "TARGET_RAN"
        target = Path(directory) / "target.py"
        target.write_text("from pathlib import Path\nimport sys\nPath(sys.argv[1]).write_text('ran')\n")
        monkeypatch.setattr(
        ...
        real_job = parent_v22._WindowsJob()
        real_api = real_job.api

        class ApiProxy:
            def __getattr__(self, name): return getattr(real_api, name)
            def ResumeThread(self, _thread): return parent_v22._INFINITE_RESUME_FAILURE

        real_job.api = ApiProxy()
        monkeypatch.setattr(parent_v22, "_WindowsJob", lambda: real_job)
        with parent_v22._PrivateEvidence() as evidence_nonce:
            with pytest.raises(parent_v22.V22ParentError, match="ResumeThread"):
                parent_v22._windows_phase(
                    "focused", evidence_nonce=evidence_nonce, label="resume failure",
                    timeout=5, env=None, stdout_mode="pipe",
                )
        assert not marker.exists()
...
def test_createprocess_failure_closes_precreated_resources(monkeypatch):
    ...
    class ApiProxy:
        def __getattr__(self, name): return getattr(real_api, name)
        def CreateProcessW(self, *_args): return False

    real_job.api = ApiProxy()
    monkeypatch.setattr(parent_v22, "_WindowsJob", lambda: real_job)
    with parent_v22._PrivateEvidence() as evidence_nonce:
        with pytest.raises(parent_v22.V22ParentError, match="CreateProcessW"):
            parent_v22._windows_phase(
                "focused", evidence_nonce=evidence_nonce, label="create failure",
                timeout=5, env=None, stdout_mode="pipe",
            )
    assert not parent_v22._RETAINED_HANDLES


def test_closehandle_failure_retains_ownership_until_eventual_cleanup():
    parent_v22._RETAINED_HANDLES.clear()
    ...
    job = object.__new__(parent_v22._WindowsJob)
    job.api = Api()
    job.handle = parent_v22.wintypes.HANDLE(321)
    with pytest.raises(parent_v22.V22ParentError, match=r"CloseHandle\(Job\)"):
        job.close()
    assert job.handle.value == 321 and 321 in parent_v22._RETAINED_HANDLES
    parent_v22._cleanup_retained_handles()
    assert not parent_v22._RETAINED_HANDLES
    job.handle = parent_v22.wintypes.HANDLE()
...
def test_pid_exists_closehandle_failure_propagates_and_is_retained(monkeypatch):
    ...
    class Api:
        allow_close = False
        def OpenProcess(self, *_args): return 456
        def GetExitCodeProcess(self, _handle, pointer):
            ctypes.cast(pointer, ctypes.POINTER(parent_v22.wintypes.DWORD)).contents.value = 0
            return True
        def CloseHandle(self, _handle): return self.allow_close

    api = Api()
    monkeypatch.setattr(parent_v22, "_kernel32", lambda: api)
    monkeypatch.setattr(parent_v22.os, "name", "nt")
    with pytest.raises(parent_v22.V22ParentError, match=r"CloseHandle\(Process\)"):
        parent_v22._pid_exists(123)
    assert 456 in parent_v22._RETAINED_HANDLES
...
def test_posix_cleanup_state_machine_covers_every_communicate_path(monkeypatch, outcome):
    events = []
    ...
    class Process:
        ...
        def communicate(self, timeout=None):
            if outcome == "timeout" and timeout != parent_v22.POST_KILL_DRAIN_SECONDS:
                raise subprocess.TimeoutExpired(["fixed"], timeout)
            if outcome == "exception" and timeout != parent_v22.POST_KILL_DRAIN_SECONDS:
                raise RuntimeError("communicate exploded")
            return "ok", ""

    process = Process()
    monkeypatch.setattr(parent_v22.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(parent_v22.os, "getpgid", lambda _pid: 999, raising=False)
    exists = iter(([True, False, False] if outcome == "residual" else [False, False]))
    monkeypatch.setattr(parent_v22, "_posix_group_exists", lambda _pgid: next(exists, False))
    monkeypatch.setattr(parent_v22, "_kill_posix_group", lambda pgid: events.append(("kill", pgid)))
    monkeypatch.setattr(parent_v22, "_drain", lambda _process: events.append(("drain", 999)) or ("", ""))
    with parent_v22._PrivateEvidence() as evidence_nonce:
        if outcome == "normal":
            assert parent_v22._posix_phase(
                "focused", evidence_nonce=evidence_nonce, label="posix",
                timeout=1, env=None, stdout_mode="pipe",
            ).returncode == 0
            assert events == []
        else:
            with pytest.raises((parent_v22.V22ParentError, RuntimeError)):
                parent_v22._posix_phase(
                    "focused", evidence_nonce=evidence_nonce, label="posix",
                    timeout=1, env=None, stdout_mode="pipe",
                )
...
def test_getpgid_failure_after_spawn_kills_reaps_and_never_accepts_pass(monkeypatch, capsys):
    events = []
    process = _unowned_posix_process(events)
    monkeypatch.setattr(parent_v22.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(parent_v22.os, "getpgid", lambda _pid: (_ for _ in ()).throw(RuntimeError("getpgid exploded")), raising=False)
    monkeypatch.setattr(parent_v22, "_pid_exists", lambda _pid: False)
    with parent_v22._PrivateEvidence() as evidence_nonce:
        with pytest.raises(RuntimeError, match="getpgid exploded"):
            parent_v22._posix_phase(
                "focused", evidence_nonce=evidence_nonce, label="reviewer reproducer",
                timeout=1, env=None, stdout_mode="pipe",
            )
    assert "kill" in events
    assert sum(event[0] == "communicate" for event in events if isinstance(event, tuple)) >= 1
...
def test_unexpected_getpgid_cleans_only_direct_child_and_fails_closed(monkeypatch):
    ...
    process = _unowned_posix_process(events)
    monkeypatch.setattr(parent_v22.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(parent_v22.os, "getpgid", lambda _pid: process.pid + 1, raising=False)
    monkeypatch.setattr(parent_v22, "_pid_exists", lambda _pid: False)
    monkeypatch.setattr(parent_v22, "_kill_posix_group", lambda _pgid: pytest.fail("wrong group killed"))
    with parent_v22._PrivateEvidence() as evidence_nonce:
        with pytest.raises(parent_v22.V22ParentError, match="ownership mismatch"):
            parent_v22._posix_phase(
                "focused", evidence_nonce=evidence_nonce, label="wrong group",
                timeout=1, env=None, stdout_mode="pipe",
            )
    assert "kill" in events and any(isinstance(event, tuple) and event[0] == "communicate" for event in events)
...
def test_getpgid_failure_preserves_original_when_direct_kill_also_fails(monkeypatch):
    ...
    monkeypatch.setattr(parent_v22.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(parent_v22.os, "getpgid", lambda _pid: (_ for _ in ()).throw(RuntimeError("getpgid original")), raising=False)
    monkeypatch.setattr(parent_v22, "_pid_exists", lambda _pid: False)
    with parent_v22._PrivateEvidence() as evidence_nonce:
        with pytest.raises(RuntimeError, match="getpgid original") as caught:
            parent_v22._posix_phase(
                "focused", evidence_nonce=evidence_nonce, label="kill failure",
                timeout=1, env=None, stdout_mode="pipe",
            )
    assert "kill" in events
    assert any("direct kill exploded" in note for note in getattr(caught.value, "__notes__", ()))
...
def test_getpgid_failure_preserves_original_when_direct_communicate_fails(monkeypatch):
    ...
    with parent_v22._PrivateEvidence() as evidence_nonce:
        with pytest.raises(RuntimeError, match="getpgid original") as caught:
            parent_v22._posix_phase(
                "focused", evidence_nonce=evidence_nonce, label="communicate failure",
                timeout=1, env=None, stdout_mode="pipe",
            )
    assert any(isinstance(event, tuple) and event[0] == "communicate" for event in events)
    assert any("output decode exploded" in note for note in getattr(caught.value, "__notes__", ()))
...
def test_successful_normal_posix_phase_retains_group_owned_path(monkeypatch):
    ...
    process = _unowned_posix_process(events)
    monkeypatch.setattr(parent_v22.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(parent_v22.os, "getpgid", lambda _pid: process.pid, raising=False)
    monkeypatch.setattr(parent_v22, "_posix_group_exists", lambda _pgid: False)
    with parent_v22._PrivateEvidence() as evidence_nonce:
        result = parent_v22._posix_phase(
            "focused", evidence_nonce=evidence_nonce, label="normal posix",
            timeout=1, env=None, stdout_mode="pipe",
        )
...
def test_reviewer_multifault_cleanup_preserves_every_flat_record(monkeypatch, capsys):
    ...
    monkeypatch.setattr(parent_v22.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(parent_v22.os, "getpgid", lambda _pid: (_ for _ in ()).throw(RuntimeError("getpgid exploded")), raising=False)
    monkeypatch.setattr(parent_v22, "_pid_exists", lambda _pid: (_ for _ in ()).throw(PermissionError("PID proof exploded")))
    with parent_v22._PrivateEvidence() as evidence_nonce:
        with pytest.raises(RuntimeError, match="getpgid exploded") as caught:
            parent_v22._posix_phase(
                "focused", evidence_nonce=evidence_nonce, label="reviewer multifault",
                timeout=1, env=None, stdout_mode="pipe",
            )
    records = caught.value.cleanup_failures
    assert [item["operation"] for item in records] == [
        "direct.poll.initial", "direct.kill", "direct.communicate",
        "direct.poll.reap", "direct.pid.proof",
    ]
    assert [item["message"] for item in records] == [
        "poll exploded", "kill exploded", "decode exploded",
        "poll exploded", "PID proof exploded",
    ]
    assert len(caught.value.__notes__) == 5
    assert all(message in "\n".join(caught.value.__notes__) for message in (
        "poll exploded", "kill exploded", "decode exploded", "PID proof exploded",
    ))
    assert "PASS" not in capsys.readouterr().out


def test_cleanup_collector_order_and_exact_deduplication():
    failures = parent_v22._CleanupFailures()
    failures.add("second", RuntimeError("same"))
    failures.add("first", ValueError("different"))
    failures.add("second", RuntimeError("same"))
    failures.add("second", ValueError("same"))
    assert [(item.operation, item.exception_type, item.message) for item in failures.records] == [
...
def test_cleanup_collector_without_primary_raises_one_payload_error():
    ...
    assert isinstance(combined, parent_v22.V22ParentError)
    assert str(combined) == "cleanup failed closed"
    assert [item["operation"] for item in combined.cleanup_failures] == ["group.kill", "group.drain"]
...
def test_cleanup_collector_single_fault_remains_direct_note_compatible():
    ...
    assert primary.cleanup_failures == ({"operation": "direct.kill", "type": "builtins.OSError", "message": "single cleanup"},)
    assert primary.__notes__ == ["cleanup_failure operation=direct.kill type=builtins.OSError message=single cleanup"]
...
def test_hanging_child_and_grandchild_tree_is_killed_without_retry_or_marker(
    hang_phase, stdout_mode, capsys, monkeypatch,
):
    with tempfile.TemporaryDirectory(prefix="onyx-v22-process-tree-") as directory:
        ...
        monkeypatch.setattr(parent_v22, "_phase_command", lambda _phase, _junit: [sys.executable, str(script)])
        started = time.monotonic()
        with parent_v22._PrivateEvidence() as evidence_nonce:
            if hang_phase == "worker":
                (parent_v22.PRIVATE_BASE / evidence_nonce / "focused.junit.xml").write_text("fixture")
            with pytest.raises(parent_v22.V22ParentError, match="timed out"):
                parent_v22._run_phase(
                    hang_phase, evidence_nonce=evidence_nonce, label="tree fixture",
                    timeout=3, stdout_mode=stdout_mode,
                )
        assert time.monotonic() - started < 7
        assert pid_file.is_file()
        direct_pid, grandchild_pid = [int(value) for value in pid_file.read_text(encoding="utf-8").splitlines()]
        deadline = time.monotonic() + 2
        while (parent_v22._pid_exists(direct_pid) or parent_v22._pid_exists(grandchild_pid)) and time.monotonic() < deadline:
            time.sleep(0.02)
        assert not parent_v22._pid_exists(direct_pid)
        assert not parent_v22._pid_exists(grandchild_pid)
        assert parent_v22.MARKER not in capsys.readouterr().out


@pytest.mark.parametrize(
    "tamper",
    ["base_count", "base_identity", "base_outcome", "base_duplicate",
     "subtest_count", "subtest_identity", "subtest_outcome", "subtest_duplicate",
     "subtest_parent", "subtest_context", "subtest_ordinal"],
)
def test_regression_report_semantic_tamper_fails_closed(tamper):
    report = copy.deepcopy(verifier_v22._regression_report())
    ...
    with pytest.raises(verifier_v22.V22EvidenceError):
        verifier_v22._validate_regression_report(report)


@pytest.mark.parametrize(
    "tamper",
    ["suite_tests", "suite_failures", "missing_case", "extra_case", "case_identity", "case_outcome",
     "suite_extra_attr", "case_extra_attr", "case_time", "case_extra_child", "suite_extra_child", "root_extra_child"],
)
def test_regression_junit_attribute_and_child_tamper_fails_closed(tamper):
    report = verifier_v22._regression_report()
    tree = ET.parse(ROOT / verifier_v22.REGRESSION_JUNIT)
    ...
    with tempfile.TemporaryDirectory(prefix="onyx-v22-junit-tamper-") as directory:
        path = Path(directory) / "tampered.junit.xml"
        tree.write(path, encoding="utf-8", xml_declaration=True)
        with pytest.raises(verifier_v22.V22EvidenceError):
            verifier_v22._junit_path(path, subtest_report=report)


def test_synthetic_junit_counter_requires_explicit_validated_subtest_report():
    with tempfile.TemporaryDirectory(prefix="onyx-v22-junit-synthetic-") as directory:
        ...
        with pytest.raises(verifier_v22.V22EvidenceError, match="lacks validated subtest evidence"):
            verifier_v22._junit_path(path)


@pytest.mark.parametrize(
    "tamper",
    ["counter_0323", "time_exponent", "time_leading_zero", "root_text", "root_tail",
     "suite_tail", "case_tail", "outcome_text", "comment", "processing_instruction", "namespace"],
)
def test_junit_lexical_text_tail_and_markup_tamper_fails_closed(tamper):
    ...
    if tamper == "counter_0323":
        ...
        raw = base.replace('<testsuites name="pytest tests">', '<testsuites name="pytest tests">tamper')
    elif tamper == "root_tail":
        raw = base + "tamper"
    elif tamper == "suite_tail":
        raw = base.replace('</testsuite></testsuites>', '</testsuite>tamper</testsuites>')
        ...
        raw = base.replace('failures="0"', 'failures="1"').replace(
            ' time="0.000" />', ' time="0.000"><failure>tamper</failure></testcase>',
        )
    elif tamper == "comment":
    ...
    with tempfile.TemporaryDirectory(prefix="onyx-v22-junit-lexical-") as directory:
        path = Path(directory) / "tampered.junit.xml"
...
_EVIDENCE_TAMPER_PATHS = (
    ("artifact", "entries"), ("artifact", "unique"),
    ("combined", "counts", "passed"), ("combined", "exit_code"),
    ("combined", "junit_sha256"), ("combined", "selected_suites"),
    ("combined", "unique_suites"),
    ("regressions", "base_tests_passed"), ("regressions", "subtests_passed"),
    ("regressions", "junit_counts", "passed"), ("regressions", "exit_code"),
    ("regressions", "junit_sha256"), ("regressions", "junit_testcase_elements"),
    ("regressions", "junit_duration_policy"), ("regressions", "junit_schema"),
    ("regressions", "report_sha256"), ("regressions", "subtest_records"), ("regressions", "selected_suites"),
    ("regressions", "unique_suites"),
    ("history", "frozen_candidates_cryptographically_validated"), ("history", "manifests_verified"),
    ("history", "unique_leaves_verified"), ("history", "roots"),
    ...
    ("tamper", "rejected"), ("tamper", "total"), ("tamper", "unique"),
    ("tamper", "fixtures"),
    ("static", "exit_codes", "RUFF"), ("static", "selected_gates"),
    ("live", "count"), ("live", "manifest_sha256"), ("live", "unique"),
    ("live", "anchors"),
    ("runtime", "architecture"), ("runtime", "parent_direct_phases"),
    ("runtime", "arbitrary_command_surfaces"), ("runtime", "phase_ids"),
    ("runtime", "windows_launch"),
    ("runtime", "closehandle_fail_closed"), ("runtime", "fresh_junit_scope"),
    ("runtime", "parent_owned_tree_roots"),
    ("runtime", "post_kill_drain_seconds"), ("runtime", "tree_ownership"),
    ("runtime", "tree_ownership", "windows"), ("runtime", "tree_ownership", "posix"),
    ("runtime", "worker_subprocesses"), ("runtime", "measured_full_verifier_seconds"),
    ("runtime", "total_timeout_seconds"), ("runtime", "windows_processes_per_phase"),
)
...
def _evidence_fixture():
    return {
        ...
        "combined": {
            ...
            "exit_code": 0, "junit_sha256": "a" * 64,
            "selected_suites": ["a.py", "b.py"], "unique_suites": True,
        },
        "regressions": {
            "base_tests_passed": 102, "exit_code": 0,
            ...
            "junit_schema": "strict_single_pytest_suite_lexical_v22",
            "junit_sha256": "b" * 64, "junit_testcase_elements": 102,
            "report_sha256": "d" * 64, "selected_suites": ["tests/test_regressions.py"],
            "subtest_records": 221, "subtests_passed": 221, "unique_suites": True,
        },
        "history": {
            "frozen_candidates_cryptographically_validated": 21, "manifests_verified": 90,
            "roots": ["root.sha256"], "unique_leaves_verified": 425,
            "v14_proof": {
        ...
        "live": {"anchors": ["core.py"], "count": 101, "manifest_sha256": "c" * 64, "unique": True},
        "runtime": {
            ...
            "total_timeout_seconds": 180, "windows_processes_per_phase": 1, "worker_subprocesses": 0,
        },
    }
...
def test_every_structured_evidence_field_tamper_fails_closed(path):
    ...
    with pytest.raises(verifier_v22.V22EvidenceError):
        verifier_v22._assert_exact_evidence("fixture", tampered, expected)
