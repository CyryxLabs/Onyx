...
import core.phase5_runtime_v9 as runtime


def _binding(**changes: object) -> runtime.RuntimeBindingV9:
    ...
    return runtime.RuntimeBindingV9(**values)  # type: ignore[arg-type]


def _flags(**changes: object) -> runtime.RuntimeFlagsV9:
    values: dict[str, object] = {
        "runtime": True,
        "grant_shadow": False,
    ...
    values.update(changes)
    return runtime.RuntimeFlagsV9(**values)  # type: ignore[arg-type]


def _action(
    binding: runtime.RuntimeBindingV9 | None = None, **changes: object
) -> runtime.ActionRequestV9:
    values: dict[str, object] = {
    ...
    values.update(changes)
    return runtime.ActionRequestV9(**values)  # type: ignore[arg-type]
...
def _core(
    *,
    flags: runtime.RuntimeFlagsV9 | None = None,
    components: runtime.RuntimeComponentsV9 | None = None,
) -> runtime.Phase5RuntimeV9:
    return runtime.Phase5RuntimeV9(
...
class _Inbox:
    ...
    def read_page(
        self,
        *,
        binding: runtime.RuntimeBindingV9,
        page_size: int,
        cursor: str | None,
    ) -> runtime.ProjectionBatchV9:
        del binding, page_size
        return runtime.ProjectionBatchV9(
            (self.row,), replace=cursor is None, source_cursor=cursor
        )


def test_v9_flags_are_independent_and_default_off() -> None:
    assert runtime.RuntimeFlagsV9.from_environ({}) == runtime.RuntimeFlagsV9()
    names = [
    ...
    for selected in names:
        ...
        assert (
            sum(
                dict(
                    runtime.RuntimeFlagsV9.from_environ(environment).payload()
                ).values()
            )
            == 1
        )
...
def test_v9_default_off_is_observationally_pure() -> None:
    touched: list[str] = []
    core = _core(
        flags=_flags(runtime=False, grant_shadow=True, nexus_projection=True),
        components=runtime.RuntimeComponentsV9(
...
def test_allow_decision_is_exact_request_single_use_commit() -> None:
    ...
    assert decision.disposition is runtime.RuntimeDispositionV9.ALLOW_LOCAL_CATALOG_READ
...
def test_only_exact_local_catalog_read_is_allowed(field: str, value: object) -> None:
    assert (
        ...
        is runtime.RuntimeDispositionV9.EXPLICIT
    )
...
def test_unicode_dlp_rejects_nfkc_combining_and_separator_confusables(
    ...
    with pytest.raises(runtime.Phase5RuntimeV9ContractError):
        runtime.RuntimeBindingV9(
            trace_id="trace-one",
            session_id="session-one",
            workspace_id="workspace-one",
            account_id="account-one",
...
def test_ids_validate_after_nfkc_and_normal_language_is_not_base32_secret() -> None:
    ...
    for value in (
        ...
        core = _core(
            ...
            components=runtime.RuntimeComponentsV9(
                nexus_projector=lambda binding, value=value: runtime.ProjectionBatchV9(
...
def test_dlp_covers_binding_capability_inbox_event_and_advisory_surfaces() -> None:
    ...
    nexus = _core(
        flags=_flags(nexus_projection=True),
        components=runtime.RuntimeComponentsV9(
            nexus_projector=lambda binding: runtime.ProjectionBatchV9(
                (_cap(limitation=secret),)
            )
        ),
    )
    assert nexus.recover_component("nexus").state is runtime.RuntimeStateV9.READY
    ...
    assert inbox.recover_component("inbox").state is runtime.RuntimeStateV9.READY
    ...
    assert grant.recover_component("grant_shadow").state is runtime.RuntimeStateV9.READY
    event_core = _core()
...
def test_projection_reads_are_closed_bounded_and_context_bound() -> None:
    core = _core(
        ...
        components=runtime.RuntimeComponentsV9(
            nexus_projector=lambda binding: runtime.ProjectionBatchV9((_cap(),)),
            inbox_factory=lambda binding: _Inbox(_item()),
        ),
    )
    assert core.recover_component("nexus").state is runtime.RuntimeStateV9.DEGRADED
    assert core.recover_component("inbox").state is runtime.RuntimeStateV9.READY
...
def _all_termination_components() -> runtime.RuntimeComponentsV9:
    return runtime.RuntimeComponentsV9(
...
def test_termination_is_host_owned_bounded_and_never_invokes_callbacks() -> None:
    ...
    status = core.kill()
    assert status.state is runtime.RuntimeStateV9.TERMINATION_PENDING
    plan = core.termination_plan()
    assert isinstance(plan, runtime.TerminationPlanV9)
...
def test_default_termination_still_has_complete_constant_plan_and_is_stable() -> None:
    ...
    first = core.shutdown()
    plan = core.termination_plan()
    assert first.state is runtime.RuntimeStateV9.TERMINATION_PENDING
    assert isinstance(plan, runtime.TerminationPlanV9)
    ...
    assert core.status().state is runtime.RuntimeStateV9.TERMINATED


def test_ack_tokens_are_bound_single_use_and_failures_are_fail_closed() -> None:
    core = _core(components=_all_termination_components())
    assert core.revoke().state is runtime.RuntimeStateV9.TERMINATION_PENDING
    ...
    for item in actions[:-1]:
        assert (
            ...
            is runtime.RuntimeStateV9.TERMINATION_PENDING
        )
    last = actions[-1]
    final = core.ack_termination_action(
        last.token, success=False, error="Host rollback failed safely"
    )
    assert final.state is runtime.RuntimeStateV9.TERMINATED
...
def test_repeated_and_concurrent_termination_get_same_plan_without_blocking() -> None:
    ...
    assert {status.state for status in statuses} == {
        runtime.RuntimeStateV9.TERMINATION_PENDING
    }
    plan = core.termination_plan()
...
def test_132_runtimes_create_no_threads_or_queues_and_do_not_starve() -> None:
    ...
    for core in cores:
        assert core.kill().state is runtime.RuntimeStateV9.TERMINATION_PENDING
...
def test_late_grant_failure_cannot_overwrite_newer_recovery() -> None:
    ...
    core = _core(
        flags=_flags(grant_shadow=True),
        components=runtime.RuntimeComponentsV9(grant_evaluator=evaluator),
    )
    assert core.recover_component("grant_shadow").state is runtime.RuntimeStateV9.READY
    result: list[runtime.RuntimeDecisionV9] = []
    ...
    assert core.status().state is runtime.RuntimeStateV9.READY
...
def test_v9_identifier_grammars_accept_uuid_ulid_hash_and_normal_ids() -> None:
    binding = runtime.RuntimeBindingV9(
...
def test_v9_ids_reject_credential_labels_and_unicode_spoofing(unsafe: str) -> None:
    with pytest.raises(runtime.Phase5RuntimeV9ContractError):
        _binding(trace_id=unsafe)
...
def test_cross_field_dlp_scans_long_values_and_collapsed_semantic_roles() -> None:
    ...
    assert core.recover_component("nexus").state is runtime.RuntimeStateV9.READY
    ...
    with pytest.raises(runtime.Phase5RuntimeV9ContractError, match="sensitive"):
...
def test_termination_transition_uses_only_precomputed_immutable_material(
    ...
    # replacement classes.  V9 has already materialized exact immutable plans.
    monkeypatch.setattr(runtime, "_build_termination_action_v9", forbidden, raising=False)
    monkeypatch.setattr(
        runtime, "_build_fallback_termination_action_v9", forbidden, raising=False
    )
    ...
    monkeypatch.setattr(runtime, "TerminationActionV9", object)
    monkeypatch.setattr(runtime, "TerminationPlanV9", object)
    status = core.kill()
    plan = core.termination_plan()
    assert status.state is runtime.RuntimeStateV9.TERMINATION_PENDING
    assert plan is not None
...
def test_unexpected_post_revoke_fault_keeps_complete_host_readable_plan(
    ...
    status = core.kill()
    plan = core.termination_plan()
    assert status.state is runtime.RuntimeStateV9.TERMINATION_ERROR
...
def test_termination_actions_cannot_move_after_precomputation() -> None:
    core = _core(components=_all_termination_components())
    with pytest.raises(
        runtime.Phase5RuntimeV9ContractError,
        match="immutable after initialization",
    ):
        core.replace_components(runtime.RuntimeComponentsV9(termination_actions=("grant",)))
