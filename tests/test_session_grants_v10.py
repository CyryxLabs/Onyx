...
import core.session_grants_v10 as grants
...
class Host:
    ...
    def feature_gate(self) -> grants.FeatureGateSnapshot:
        return self.gate.snapshot()

    def services(self) -> grants.HostServices:
        return grants.HostServices(
            "desktop-root", self.feature_gate, self.resolve_grant, self.resolve_action, self.resolve_outcome,
...
def test_dispatch_attempt_is_quarantined_and_cannot_retry_across_successor_grant(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(grants.GrantV10Denied, match="idempotency-uncertain"):
...
def test_same_external_namespace_key_cannot_reserve_across_distinct_bindings(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(grants.GrantV10Denied, match="idempotency-action-mismatch"):
        store.reserve("other")
...
def test_reconciliation_after_terminal_resolves_once_and_replay_is_denied(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(grants.GrantV10Denied, match="already reconciled"):
...
def test_zero_cost_zero_aggregate_direct_and_reconciled_commits_do_not_exhaust_aggregate(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(grants.GrantV10ContractError, match="revocation_reason"):
...
def test_default_off_reconciliation_preserves_uncertainty_without_callback(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(grants.GrantV10Disabled):
        store.reconcile_uncertain(identity)
...
def test_verified_effect_post_snapshot_failure_preserves_material_for_reconciliation(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(grants.GrantV10ContractError, match="snapshot callback failed"):
        store.record_outcome("done")
...
def test_epoch_aba_after_outcome_stops_callbacks_and_preserves_uncertainty(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    before = store._uncertain_records[identity]
    with pytest.raises(grants.GrantV10Disabled, match="feature epoch changed"):
        store.record_outcome("done")
    uncertain = store._uncertain_records[identity]
...
class _GateSequence:
    def __init__(self, *values: grants.FeatureGateSnapshot) -> None:
        self._values = list(values)

    def snapshot(self) -> grants.FeatureGateSnapshot:
...
@pytest.mark.parametrize("method_name", ["kill", "mark_audit_unhealthy", "end_session", "revoke", "snapshot_counts"])
def test_public_finalization_epoch_change_applies_only_mandatory_revocation(
    monkeypatch: pytest.MonkeyPatch,
    method_name: str,
) -> None:
    host, store, grant_id = _ready(monkeypatch)
    before_flags = (store._killed, store._audit_unhealthy, store._session_ended, store._global_generation)
    host.gate = _GateSequence(grants.FeatureGateSnapshot(True, 1), grants.FeatureGateSnapshot(False, 2))  # type: ignore[assignment]
    args = (grant_id,) if method_name == "revoke" else ()
    with pytest.raises(grants.GrantV10Disabled, match="feature epoch changed"):
        getattr(store, method_name)(*args)
    assert (store._killed, store._audit_unhealthy, store._session_ended, store._global_generation) == before_flags
...
def test_raw_environment_is_bootstrap_only_and_strict_default_off(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    host.store = store
    monkeypatch.setenv(grants.GRANT_EVALUATOR_FLAG, "true")
    with pytest.raises(grants.GrantV10Disabled, match="bootstrap"):
        store.request_session_grant("grant")
...
def test_uncertain_action_holds_provisional_use_and_cost_until_no_effect(
    ...
    with pytest.raises(grants.GrantV10Denied, match=expected_reason):
        store.reserve("other")
    host.reconciliation_outcome = "confirmed-no-effect"
    assert store.reconcile_uncertain(identity).outcome == "confirmed-no-effect"
...
def test_concurrent_distinct_bindings_cannot_overbook_one_provisional_use(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def attempt(reference: str) -> str:
        ...
        except grants.GrantV10Denied as exc:
            return f"denied:{exc}"
