...
from core.onyx_hud_current_acceptance_v30 import (
...
def test_v30_authenticates_projection_safe_qml_lifecycle() -> None:
    result = verify_current_hud_acceptance(ROOT)
    assert result["candidate"] == "onyx-hud-v11-guarded-successor-012"
...
def test_v30_rejects_runtime_and_predecessor_tamper(tmp_path: Path) -> None:
    _copy_closure(tmp_path)
    with (tmp_path / "qml/OnyxLiveShellGuardedV1.qml").open("ab") as handle:
        handle.write(b"tamper")
    with pytest.raises(CurrentHudAcceptanceV30Error, match="source input drift"):
    ...
    with (tmp_path / PREDECESSOR_MANIFEST_RELATIVE).open("ab") as handle:
        handle.write(b"tamper")
    with pytest.raises(
        CurrentHudAcceptanceV30Error, match="source input drift|predecessor drift"
    ):
        verify_current_hud_acceptance(tmp_path)
...
def test_current_qml_loads_and_tears_down_without_projection_property_errors() -> None:
    ...
    view = QQuickView()
    try:
        view.setSource(QUrl.fromLocalFile(str(ROOT / "qml/OnyxLiveShellV11.qml")))
        app.processEvents()
