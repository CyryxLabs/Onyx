...
def test_current_v19_manifest_and_semantics_are_exact() -> None:
    result = verify_current_hud_acceptance(ROOT)
    assert result == {
        "candidate": "onyx-hud-v9-current-pinned-001",
...
def test_every_current_runtime_qml_tamper_fails_closed(
    ...
    with (tmp_path / relative).open("ab") as handle:
        handle.write(b"\n// tamper")
    with pytest.raises(CurrentHudAcceptanceV19Error, match="input drift"):
