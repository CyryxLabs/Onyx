...
from scripts import generate_advanced_operations_source_acceptance_v24 as generator
from scripts import verify_advanced_operations_source_acceptance_v24 as verifier


ROOT = Path(__file__).resolve().parents[1]


def test_v24_is_reproducible_exact_and_append_only() -> None:
    manifest = verifier.verify(ROOT)
    assert manifest == generator.build(ROOT)
    assert manifest["predecessor"]["path"].endswith("V23-001.manifest.json")
...
def test_v24_rejects_selection_scope_reduction(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        verifier.AdvancedOperationsSourceAcceptanceV24Error,
