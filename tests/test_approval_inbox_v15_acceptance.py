...
from scripts import verify_phase5_approval_inbox_v15_acceptance as acceptance
...
COPY_PATHS = (
    ...
    acceptance.BUNDLE,
    acceptance.V14_ROOT_MANIFEST,
    acceptance.V14_ARTIFACT_MANIFEST,
...
def test_external_acceptance_validates_without_rerunning_parent() -> None:
    result = acceptance.verify(run_parent=False)
    assert result == {
        "acceptance_id": acceptance.ACCEPTANCE_ID,
        "record_sha256": acceptance._digest(
            acceptance.PROJECT, acceptance.ACCEPTANCE_RECORD
        ),
...
def test_record_manifest_and_candidate_anchor_tamper_fail_closed(
    tmp_path: Path, relative: str
) -> None:
    project = _copy_acceptance_fixture(tmp_path)
    target = project / relative
    target.write_bytes(target.read_bytes() + b"tamper")
    with pytest.raises(acceptance.ApprovalInboxV15AcceptanceError):
        acceptance.verify(project, run_parent=False)


def test_mutable_projections_are_required_but_excluded_from_frozen_v15_dag() -> None:
    ...
    assert acceptance.ACCEPTANCE_ID in acceptance._text(
        acceptance.PROJECT, acceptance.CAPABILITY_MATRIX
    )
    assert acceptance.ACCEPTANCE_ID in acceptance._text(
        acceptance.PROJECT, acceptance.VERIFICATION_EVIDENCE
    )


def test_default_verification_requires_exact_proportional_parent_marker(
    ...
    monkeypatch.setattr(acceptance, "_run_manifest_parent", fake_parent)
