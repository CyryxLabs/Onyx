...
from core.capability_nexus_v21 import (
    CATALOG_CONTRACT_VERSION_V21,
    LOCK_ORDER_V21,
    CancellationTokenV21,
    CapabilityDescriptorV21,
    CapabilityNexusV21,
    CapabilityProjectionV21,
    CapabilityStatusV21,
    CapabilityV21ContractError,
    CapabilityV21DeniedError,
    CatalogItemV21,
    CatalogReadRequestV21,
    LocalCatalogReadAdapterV21,
    NexusFeatureGateV21,
    OperationDescriptorV21,
    OperationKindV21,
    ReadFailureClassV21,
    ReadStateV21,
    TransportKindV21,
    build_legacy_descriptors_v21,
)
import core.capability_nexus_v21 as nexus_v21_module
import scripts.verify_phase5_capability_nexus_v21 as parent_v21
import scripts.verify_phase5_capability_nexus_v21_worker as verifier_v21
...
def _operation(**changes) -> OperationDescriptorV21:
    values = {
        "operation_id": "catalog_read", "kind": OperationKindV21.READ,
    ...
    return OperationDescriptorV21(**values)


def _descriptor(**changes) -> CapabilityDescriptorV21:
    values = {
        "capability_id": "local.catalog", "capability_version": "v21", "provider": "onyx_local",
        "transport": TransportKindV21.LOCAL, "api_name": "local_catalog",
        "api_version": CATALOG_CONTRACT_VERSION_V21, "workspace_id": WORKSPACE,
        "account_id": ACCOUNT, "profile_id": PROFILE, "credential_alias": None,
        "operations": (_operation(),), "status": CapabilityStatusV21.DISABLED,
        "status_reason": "candidate_not_activated", "limitations": ("no_dispatch",),
    }
    values.update(changes)
    return CapabilityDescriptorV21(**values)


def _items(count: int = 5) -> tuple[CatalogItemV21, ...]:
    return tuple(CatalogItemV21(f"item-{i:02d}", f"Item {i}", "fixture", "v21", ("safe",)) for i in range(count))


def _adapter(hook=lambda: None, **changes) -> LocalCatalogReadAdapterV21:
    values = {
        "items": _items(), "workspace_id": WORKSPACE, "account_id": ACCOUNT,
        "profile_id": PROFILE, "cursor_signing_key": KEY, "read_hook": hook,
        "status": CapabilityStatusV21.AVAILABLE_READ_ONLY, "status_reason": "healthy",
    }
    values.update(changes)
    return LocalCatalogReadAdapterV21(**values)


def _request(**changes) -> CatalogReadRequestV21:
    values = {
        "workspace_id": WORKSPACE, "account_id": ACCOUNT, "profile_id": PROFILE,
        "target": "local_catalog", "correlation_id": "corr-001", "page_size": 2,
    }
    values.update(changes)
    return CatalogReadRequestV21(**values)
...
def test_v20_windows_suspended_job_path_is_exact_after_version_normalization():
    ...
    current = ast.parse((ROOT / "scripts/verify_phase5_capability_nexus_v21.py").read_text(encoding="utf-8"))
    names = {
    ...
    normalized = {
        name: source.replace("V21", "V20").replace("v21", "v20")
...
def test_registry_requires_profile_and_binds_discovery_exactly():
    nexus = CapabilityNexusV21()
    descriptor = _descriptor()
    nexus.register(descriptor)
    with pytest.raises(TypeError):
        nexus.discover(
            descriptor.capability_id, workspace_id=WORKSPACE, account_id=ACCOUNT,
            expected_version="v21", expected_digest=descriptor.digest,
        )
    for profile in ("profile-other", "PROFILE-alpha"):
        with pytest.raises(CapabilityV21DeniedError):
            nexus.discover(
                descriptor.capability_id, workspace_id=WORKSPACE, account_id=ACCOUNT,
                profile_id=profile, expected_version="v21", expected_digest=descriptor.digest,
            )
...
def test_projection_constructor_rejects_cross_profile_and_authority():
    descriptor = _descriptor()
    with pytest.raises(CapabilityV21ContractError):
        CapabilityProjectionV21(
            descriptor, descriptor.digest, WORKSPACE, ACCOUNT, "profile-other",
            CapabilityStatusV21.DISABLED, "disabled",
        )
    with pytest.raises(CapabilityV21ContractError):
        CapabilityProjectionV21(
            descriptor, descriptor.digest, WORKSPACE, ACCOUNT, PROFILE,
            CapabilityStatusV21.DISABLED, "disabled", authority_granted=True,
        )


class _GateSubclass(NexusFeatureGateV21):
    pass
...
def test_registry_rejects_gate_subclasses_fakes_and_duck_types(gate):
    with pytest.raises(CapabilityV21ContractError, match="exact concrete"):
        CapabilityNexusV21(gate)  # type: ignore[arg-type]


def test_exact_gate_is_frozen_default_off_and_lock_order_is_declared():
    gate = NexusFeatureGateV21()
    nexus = CapabilityNexusV21(gate)
    assert nexus.gate is gate and nexus.lock_order == LOCK_ORDER_V21
    with pytest.raises(FrozenInstanceError):
        gate.shadow_mode = False  # type: ignore[misc]
    for values in ({"nexus_enabled": True}, {"shadow_mode": False}, {"dispatch_enabled": True}):
        with pytest.raises(CapabilityV21ContractError):
            NexusFeatureGateV21(**values)
...
@pytest.mark.parametrize("prefix", sorted(nexus_v21_module._SECRET_NAMES))
...
@pytest.mark.parametrize("name", sorted(nexus_v21_module._SECRET_NAMES))
...
@pytest.mark.parametrize("name", sorted(nexus_v21_module._SECRET_NAMES | nexus_v21_module._CREDENTIAL_OPAQUE_NAMES))
@pytest.mark.parametrize("position", ["leading", "middle", "trailing"])
...
def test_embedded_ngram_model_accepts_held_out_natural_boundary_corpus(word):
    assert nexus_v21_module._english_word_score(word) >= nexus_v21_module._NATURAL_SCORE_THRESHOLD
...
def test_schema_rejects_raw_credentials_in_nested_defaults_examples_and_descriptions(value):
    for field in ("default", "example", "description"):
        ...
        encoded = nexus_v21_module.json.dumps(schema, sort_keys=True, separators=(",", ":"))
        with pytest.raises(CapabilityV21ContractError):
...
def test_recursive_schema_cycle_depth_node_and_scalar_budgets_fail_closed():
    cycle = {}
    cycle["safe"] = cycle
    with pytest.raises(CapabilityV21ContractError, match="cycle"):
        nexus_v21_module._reject_json_secrets(cycle, "fixture")
    deep = current = {}
    for index in range(26):
        current["safe"] = {}
        current = current["safe"]
    with pytest.raises(CapabilityV21ContractError):
        nexus_v21_module._reject_json_secrets(deep, "fixture")
    with pytest.raises(CapabilityV21ContractError, match="node"):
        nexus_v21_module._reject_json_secrets(0, "fixture", _budget=[4096, 0])
    with pytest.raises(CapabilityV21ContractError, match="scalar byte"):
        nexus_v21_module._reject_json_secrets(["a" * 4096] * 17, "fixture")
...
def test_every_non_hash_metadata_key_is_a_closed_enum(key, value):
    with pytest.raises(CapabilityV21ContractError, match="closed enum"):
        _descriptor(metadata=((key, value),))
...
def test_metadata_enum_table_is_immutable_and_future_values_require_source_change():
    assert dict(nexus_v21_module._METADATA_ENUMS) == {
    ...
    with pytest.raises(TypeError):
        nexus_v21_module._METADATA_ENUMS["data_source"] = frozenset({"future_catalog"})
    with pytest.raises(AttributeError):
        nexus_v21_module._METADATA_ENUMS["data_source"].add("future_catalog")
...
def test_benign_human_text_remains_valid_only_in_description_context(description):
    ...
    encoded = nexus_v21_module.json.dumps(schema, sort_keys=True, separators=(",", ":"), ensure_ascii=False)
...
def test_content_decoding_cycle_detection_fails_closed(monkeypatch):
    ...
    monkeypatch.setattr(
        nexus_v21_module, "_canonical_percent_decode", lambda value: transitions.get(value)
    )
    with pytest.raises(CapabilityV21ContractError, match="cycle"):
        nexus_v21_module._validate_content_decoding_closure("cycle_a")
...
def test_adapter_defaults_disabled_and_has_no_dispatch_or_mutation():
    adapter = LocalCatalogReadAdapterV21(
        _items(), workspace_id=WORKSPACE, account_id=ACCOUNT, profile_id=PROFILE,
        cursor_signing_key=KEY, read_hook=lambda: None,
    )
    assert adapter.health().status is CapabilityStatusV21.DISABLED
    assert adapter.read_page(_request()).state is ReadStateV21.DENIED
...
def test_hook_runs_without_adapter_lock_and_can_reenter_health():
    ...
    assert result.state is ReadStateV21.COMPLETED
    assert observed == [CapabilityStatusV21.AVAILABLE_READ_ONLY]
...
def test_same_correlation_reentry_is_denied_before_second_hook_or_reservation():
    ...
    def hook():
        calls.append(1)
        with pytest.raises(CapabilityV21DeniedError, match="reentrancy"):
            holder["adapter"].read_page(request)

    holder["adapter"] = _adapter(hook)
    assert holder["adapter"].read_page(request).state is ReadStateV21.COMPLETED
...
def test_new_correlation_and_indirect_hook_reentry_are_denied_without_quota_drift():
    ...
    assert holder["adapter"].read_page(_request()).state is ReadStateV21.COMPLETED
    limited = holder["adapter"].read_page(_request(correlation_id="corr-after"))
    assert limited.state is ReadStateV21.RATE_LIMITED
...
def test_hook_exception_cleans_reentrancy_guard_for_later_correlation():
    ...
    assert adapter.read_page(_request()).state is ReadStateV21.UNCERTAIN
    assert adapter.read_page(_request(correlation_id="corr-002")).state is ReadStateV21.COMPLETED
    assert calls == [1, 1]
...
def test_reentrancy_guard_rejects_hook_spawned_thread_before_reservation():
    ...
    def hook():
        ...
        with ThreadPoolExecutor(max_workers=1) as pool:
            ...
            except CapabilityV21DeniedError as exc:
                nested_errors.append(str(exc))

    holder["adapter"] = _adapter(hook)
    outer = holder["adapter"].read_page(_request())
    assert outer.state is ReadStateV21.COMPLETED
    assert nested_errors == ["catalog read reentrancy is denied"]
    assert len(calls) == 1 and holder["adapter"].health().quota_remaining == 99
    assert holder["adapter"].read_page(_request(correlation_id="corr-after")).state is ReadStateV21.COMPLETED
...
def test_hook_guard_is_per_adapter_and_does_not_hold_lock_during_callback():
    ...
    assert first.read_page(_request()).state is ReadStateV21.COMPLETED
    assert observed == [ReadStateV21.COMPLETED]


def test_historical_leaf_budget_exact_boundary_and_overflow():
    leaves = {f"leaf-{index}": "a" * 64 for index in range(4095)}
    verifier_v21._record_historical_leaf(leaves, "boundary", "b" * 64)
    assert len(leaves) == 4096
    with pytest.raises(verifier_v21.V21EvidenceError, match="leaf budget"):
        verifier_v21._record_historical_leaf(leaves, "overflow", "c" * 64)
    with pytest.raises(verifier_v21.V21EvidenceError, match="digest conflict"):
        verifier_v21._record_historical_leaf(leaves, "boundary", "d" * 64)
...
def test_wide_historical_manifest_and_cycle_fail_before_expansion(monkeypatch):
    ...
    monkeypatch.setattr(verifier_v21, "_historical_manifest", lambda relative: wide)
    monkeypatch.setattr(verifier_v21, "_verify_historical_path", lambda relative, digest: None)
    with pytest.raises(verifier_v21.V21EvidenceError, match="leaf budget"):
        verifier_v21._verify_manifest_tree("wide.sha256", set(), set(), {}, 0)

    graph = {
        "a.sha256": (("a" * 64, "b.sha256"),),
        "b.sha256": (("b" * 64, "a.sha256"),),
    }
    monkeypatch.setattr(verifier_v21, "_historical_manifest", graph.__getitem__)
    with pytest.raises(verifier_v21.V21EvidenceError, match="cycle"):
        verifier_v21._verify_manifest_tree("a.sha256", set(), set(), {}, 0)


def test_exact_4096_leaf_manifest_is_accepted(monkeypatch):
    entries = tuple(("a" * 64, f"leaf-{index}") for index in range(4096))
    monkeypatch.setattr(verifier_v21, "_historical_manifest", lambda relative: entries)
    monkeypatch.setattr(verifier_v21, "_verify_historical_path", lambda relative, digest: None)
    leaves = {}
    verifier_v21._verify_manifest_tree("boundary.sha256", set(), set(), leaves, 0)
    assert len(leaves) == 4096


def test_historical_manifest_rejects_path_traversal_and_symlink(monkeypatch):
    with tempfile.TemporaryDirectory(prefix="p53-v21-path-", dir=ROOT) as directory:
        base = Path(directory)
        monkeypatch.setattr(verifier_v21, "PROJECT", base)
        manifest = base / "bad.sha256"
        manifest.write_text("" + "a" * 64 + "  ../escape\n", encoding="utf-8")
        with pytest.raises(verifier_v21.V21EvidenceError, match="unsafe"):
            verifier_v21._historical_manifest("bad.sha256")
        ...
        verifier_v21._verify_historical_path(
        ...
        with pytest.raises(verifier_v21.V21EvidenceError, match="reparse"):
            verifier_v21._verify_historical_path("link.txt", hashlib.sha256(target.read_bytes()).hexdigest())
...
def test_concurrent_duplicate_is_pending_then_completed_and_hook_once():
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        release.set()
        assert future.result().state is ReadStateV21.COMPLETED
...
def test_validation_exception_releases_admission_claim():
    ...
    assert adapter.read_page(_request()).state is ReadStateV21.COMPLETED
...
def test_same_correlation_payload_drift_denies_without_hook_or_quota():
    ...
    with pytest.raises(CapabilityV21DeniedError, match="drift"):
...
def test_hook_exception_becomes_uncertain_and_never_replays_or_leaks_exception():
    ...
    assert result.state is ReadStateV21.UNCERTAIN
    assert result.failure_class is ReadFailureClassV21.HOOK_EXCEPTION
...
def test_cancelled_before_hook_rolls_back_quota_and_rate_and_never_calls_hook():
    ...
    assert result.state is ReadStateV21.CANCELLED_BEFORE_HOOK
...
def test_timeout_after_hook_is_uncertain_and_not_replayed():
    ...
    result = adapter.read_page(request)
    assert result.state is ReadStateV21.UNCERTAIN
    assert result.failure_class is ReadFailureClassV21.TIMEOUT
    assert adapter.read_page(request) == result and calls == [1]


@pytest.mark.parametrize(("action", "failure"), [("kill", ReadFailureClassV21.KILL), ("revoke", ReadFailureClassV21.REVOKED)])
def test_kill_and_revoke_during_hook_latch_uncertain(action, failure):
    ...
    result = holder["adapter"].read_page(_request())
    assert result.state is ReadStateV21.UNCERTAIN and result.failure_class is failure
...
def test_pending_expiry_becomes_uncertain_and_late_hook_cannot_commit():
    ...
    with ThreadPoolExecutor(max_workers=1) as pool:
        ...
        assert reconciled.state is ReadStateV21.UNCERTAIN
        assert reconciled.failure_class is ReadFailureClassV21.EXPIRED_PENDING
        release.set()
        assert future.result().state is ReadStateV21.UNCERTAIN
...
def test_uncertain_expiry_keeps_no_replay_tombstone_and_frees_uncertain_capacity():
    ...
    assert adapter.read_page(first_request).state is ReadStateV21.UNCERTAIN
    with pytest.raises(CapabilityV21DeniedError, match="pending/uncertain"):
        adapter.read_page(_request(correlation_id="corr-002"))
    now[0] = 2.0
    expired = adapter.reconcile(first_request)
    assert expired.state is ReadStateV21.EXPIRED_UNCERTAIN
    assert expired.failure_class is ReadFailureClassV21.UNCERTAIN_EXPIRED
    assert adapter.read_page(first_request) == expired and calls == [1]
    assert adapter.read_page(_request(correlation_id="corr-002")).state is ReadStateV21.UNCERTAIN
...
def test_ledger_capacity_is_bounded_and_does_not_evict_for_replay():
    ...
    request = _request()
    assert adapter.read_page(request).state is ReadStateV21.COMPLETED
    with pytest.raises(CapabilityV21DeniedError, match="capacity"):
        adapter.read_page(_request(correlation_id="corr-002"))
    assert adapter.read_page(request).state is ReadStateV21.COMPLETED
...
def test_cursor_snapshot_and_signature_tamper_fail_closed():
    ...
    changed = LocalCatalogReadAdapterV21(
        _items(6), workspace_id=WORKSPACE, account_id=ACCOUNT, profile_id=PROFILE,
        cursor_signing_key=KEY, read_hook=lambda: None, status=CapabilityStatusV21.AVAILABLE_READ_ONLY,
...
def test_rate_limit_and_quota_are_reserved_once_and_truthful():
    ...
    assert limited.state is ReadStateV21.RATE_LIMITED and limited.retry_after_ms == 1000
...
def test_unavailable_auth_scope_disabled_degraded_and_unknown_reconcile():
    assert _adapter(auth_available=False).read_page(_request()).failure_class is ReadFailureClassV21.AUTH
    assert _adapter(granted_scopes=()).read_page(_request()).failure_class is ReadFailureClassV21.SCOPE
    assert _adapter(status=CapabilityStatusV21.DISABLED, status_reason="operator_disabled").read_page(_request()).state is ReadStateV21.DENIED
    degraded = _adapter(status=CapabilityStatusV21.DEGRADED, status_reason="partial_fixture")
    assert degraded.read_page(_request()).state is ReadStateV21.COMPLETED
    unknown = degraded.reconcile(_request(correlation_id="unknown-corr"))
    assert unknown.state is ReadStateV21.UNKNOWN_CORRELATION
...
def test_legacy_builder_preserves_current_32_declarations_and_policies_without_import():
    ...
    policies = _assignment(ROOT / "core" / "permission_broker.py", "MODEL_TOOL_POLICIES")
    built = build_legacy_descriptors_v21(
        declarations, policies, workspace_id=WORKSPACE, account_id=ACCOUNT, profile_id=PROFILE,
    )
    ...
    assert all(item.status is CapabilityStatusV21.DISABLED for item in built.descriptors)
...
def test_no_live_runtime_source_references_v21():
    live = [ROOT / "main.py", ROOT / "ui.py", ROOT / "scripts" / "launch_onyx.pyw"]
    live += list((ROOT / "actions").rglob("*.py")) + list((ROOT / "dashboard").rglob("*.py"))
    live += [path for path in (ROOT / "core").glob("*.py") if path.name != "capability_nexus_v21.py"]
    assert all("capability_nexus_v21" not in path.read_text(encoding="utf-8", errors="ignore") for path in live)


def test_phase_runner_rejects_every_arbitrary_command_surface():
    with pytest.raises(parent_v21.V21ParentError, match="phase id"):
        parent_v21._run_phase(
            [sys.executable, "-c", "print('forbidden')"],
            evidence_nonce="a" * 64, label="x", timeout=1,
        )
    with pytest.raises(parent_v21.V21ParentError, match="phase id"):
        parent_v21._phase_command("arbitrary", Path("focused.junit.xml"))


def test_parent_has_two_direct_bounded_phases_and_pure_worker_has_no_subprocess_surface():
    parent_tree = ast.parse((ROOT / "scripts/verify_phase5_capability_nexus_v21.py").read_text(encoding="utf-8"))
    worker_tree = ast.parse((ROOT / "scripts/verify_phase5_capability_nexus_v21_worker.py").read_text(encoding="utf-8"))
    popen_calls = [
    ...
    source = (ROOT / "scripts/verify_phase5_capability_nexus_v21.py").read_text(encoding="utf-8")
    ...
    assert not (ROOT / "scripts/phase5_capability_nexus_v21_bootstrap.py").exists()
...
def test_frozen_history_proof_never_spawns_a_recursive_verifier():
    v14 = verifier_v21._validate_frozen_v14_proof()
    v15 = verifier_v21._validate_frozen_v15_proof()
    v16 = verifier_v21._validate_frozen_v16_proof()
    v17 = verifier_v21._validate_frozen_v17_proof()
    v18 = verifier_v21._validate_frozen_v18_proof()
    v19 = verifier_v21._validate_frozen_v19_proof()
    v20 = verifier_v21._validate_frozen_v20_proof()
    source = (ROOT / "scripts/verify_phase5_capability_nexus_v21_worker.py").read_text(encoding="utf-8")
    assert all(item["marker_contract_bound"] is True for item in (v14, v15, v16, v17, v18, v19, v20))
    assert "_run_frozen_verifiers" not in source
    assert "import subprocess" not in source


def test_frozen_v14_proof_rejects_a_historical_hash_tamper(monkeypatch):
    original = verifier_v21._sha

    def tampered(path):
        if path == ROOT / verifier_v21.V14_ARTIFACT_MANIFEST:
            return "0" * 64
        return original(path)

    monkeypatch.setattr(verifier_v21, "_sha", tampered)
    with pytest.raises(verifier_v21.V21EvidenceError, match="historical recursive leaf mismatch"):
        verifier_v21._validate_frozen_v14_proof()


@pytest.mark.skipif(sys.platform != "win32", reason="Windows Job ABI")
def test_kernel32_job_abi_is_explicit_and_pointer_safe():
    api = parent_v21._kernel32()
    assert ctypes.sizeof(parent_v21.wintypes.HANDLE) == ctypes.sizeof(ctypes.c_void_p)
    for name in (
...
def test_invalid_or_truncated_windows_handles_fail_closed(raw):
    with pytest.raises(parent_v21.V21ParentError, match="invalid or truncated"):
        parent_v21._validated_handle(raw)


def test_old_v19_bootstrap_invocation_path_is_absent_in_v21():
    source = (ROOT / "scripts/verify_phase5_capability_nexus_v21.py").read_text(encoding="utf-8")
    assert "BOOTSTRAP" not in source
    assert "HELLO" not in source and "READY" not in source and "GO " not in source
    assert "ONYX_P53_V21" not in source


def test_worker_rejects_arbitrary_fresh_junit_path():
    with tempfile.TemporaryDirectory(prefix="onyx-v21-arbitrary-junit-") as directory:
        path = Path(directory) / "focused.junit.xml"
        path.write_text("fixture", encoding="utf-8")
        with pytest.raises(verifier_v21.V21EvidenceError, match="nonce-scoped"):
            verifier_v21._validated_fresh_focused_junit(path)
...
def test_delayed_job_assignment_cannot_execute_target_before_resume(monkeypatch):
    ...
    original = parent_v21._WindowsJob.assign_handle
    with tempfile.TemporaryDirectory(prefix="onyx-v21-delayed-assign-") as directory:
        ...
        monkeypatch.setattr(parent_v21._WindowsJob, "assign_handle", delayed)
        with parent_v21._PrivateEvidence() as evidence_nonce, ThreadPoolExecutor(max_workers=1) as pool:
            future = pool.submit(
                parent_v21._windows_phase, "focused", evidence_nonce=evidence_nonce,
...
def test_assign_failure_terminates_suspended_target_before_execution(monkeypatch):
    with tempfile.TemporaryDirectory(prefix="onyx-v21-assign-failure-") as directory:
        marker = Path(directory) / "TARGET_RAN"
        target = Path(directory) / "target.py"
        target.write_text("from pathlib import Path\nimport sys\nPath(sys.argv[1]).write_text('ran')\n")
        monkeypatch.setattr(
        ...
        def rejected(_job, _process_handle):
            raise parent_v21.V21ParentError("injected AssignProcessToJobObject failure")

        monkeypatch.setattr(parent_v21._WindowsJob, "assign_handle", rejected)
        with parent_v21._PrivateEvidence() as evidence_nonce:
            with pytest.raises(parent_v21.V21ParentError, match="injected Assign"):
                parent_v21._windows_phase(
...
def test_resume_failure_terminates_target_before_execution(monkeypatch):
    with tempfile.TemporaryDirectory(prefix="onyx-v21-resume-failure-") as directory:
        marker = Path(directory) / "TARGET_RAN"
        target = Path(directory) / "target.py"
        target.write_text("from pathlib import Path\nimport sys\nPath(sys.argv[1]).write_text('ran')\n")
        monkeypatch.setattr(
        ...
        real_job = parent_v21._WindowsJob()
        real_api = real_job.api

        class ApiProxy:
            def __getattr__(self, name): return getattr(real_api, name)
            def ResumeThread(self, _thread): return parent_v21._INFINITE_RESUME_FAILURE

        real_job.api = ApiProxy()
        monkeypatch.setattr(parent_v21, "_WindowsJob", lambda: real_job)
        with parent_v21._PrivateEvidence() as evidence_nonce:
            with pytest.raises(parent_v21.V21ParentError, match="ResumeThread"):
                parent_v21._windows_phase(
...
def test_createprocess_failure_closes_precreated_resources(monkeypatch):
    ...
    monkeypatch.setattr(parent_v21, "_WindowsJob", lambda: real_job)
    with parent_v21._PrivateEvidence() as evidence_nonce:
        with pytest.raises(parent_v21.V21ParentError, match="CreateProcessW"):
            parent_v21._windows_phase(
                "focused", evidence_nonce=evidence_nonce, label="create failure",
                timeout=5, env=None, stdout_mode="pipe",
            )
    assert not parent_v21._RETAINED_HANDLES


def test_closehandle_failure_retains_ownership_until_eventual_cleanup():
    parent_v21._RETAINED_HANDLES.clear()
    ...
    job = object.__new__(parent_v21._WindowsJob)
    job.api = Api()
    job.handle = parent_v21.wintypes.HANDLE(321)
    with pytest.raises(parent_v21.V21ParentError, match=r"CloseHandle\(Job\)"):
        job.close()
    assert job.handle.value == 321 and 321 in parent_v21._RETAINED_HANDLES
    parent_v21._cleanup_retained_handles()
    assert not parent_v21._RETAINED_HANDLES
    job.handle = parent_v21.wintypes.HANDLE()
...
def test_pid_exists_closehandle_failure_propagates_and_is_retained(monkeypatch):
    ...
    class Api:
        ...
        def GetExitCodeProcess(self, _handle, pointer):
            ctypes.cast(pointer, ctypes.POINTER(parent_v21.wintypes.DWORD)).contents.value = 0
            return True
        def CloseHandle(self, _handle): return self.allow_close

    api = Api()
    monkeypatch.setattr(parent_v21, "_kernel32", lambda: api)
    monkeypatch.setattr(parent_v21.os, "name", "nt")
    with pytest.raises(parent_v21.V21ParentError, match=r"CloseHandle\(Process\)"):
        parent_v21._pid_exists(123)
    assert 456 in parent_v21._RETAINED_HANDLES
...
def test_posix_cleanup_state_machine_covers_every_communicate_path(monkeypatch, outcome):
    ...
    class Process:
        ...
        def communicate(self, timeout=None):
            if outcome == "timeout" and timeout != parent_v21.POST_KILL_DRAIN_SECONDS:
                raise subprocess.TimeoutExpired(["fixed"], timeout)
            if outcome == "exception" and timeout != parent_v21.POST_KILL_DRAIN_SECONDS:
                raise RuntimeError("communicate exploded")
            return "ok", ""

    process = Process()
    monkeypatch.setattr(parent_v21.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(parent_v21.os, "getpgid", lambda _pid: 999, raising=False)
    exists = iter(([True, False, False] if outcome == "residual" else [False, False]))
    monkeypatch.setattr(parent_v21, "_posix_group_exists", lambda _pgid: next(exists, False))
    monkeypatch.setattr(parent_v21, "_kill_posix_group", lambda pgid: events.append(("kill", pgid)))
    monkeypatch.setattr(parent_v21, "_drain", lambda _process: events.append(("drain", 999)) or ("", ""))
    with parent_v21._PrivateEvidence() as evidence_nonce:
        if outcome == "normal":
            assert parent_v21._posix_phase(
                "focused", evidence_nonce=evidence_nonce, label="posix",
                timeout=1, env=None, stdout_mode="pipe",
            ).returncode == 0
            assert events == []
        else:
            with pytest.raises((parent_v21.V21ParentError, RuntimeError)):
                parent_v21._posix_phase(
                    "focused", evidence_nonce=evidence_nonce, label="posix",
                    timeout=1, env=None, stdout_mode="pipe",
                )
...
def test_getpgid_failure_after_spawn_kills_reaps_and_never_accepts_pass(monkeypatch, capsys):
    ...
    monkeypatch.setattr(parent_v21.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(parent_v21.os, "getpgid", lambda _pid: (_ for _ in ()).throw(RuntimeError("getpgid exploded")), raising=False)
    monkeypatch.setattr(parent_v21, "_pid_exists", lambda _pid: False)
    with parent_v21._PrivateEvidence() as evidence_nonce:
...
def test_unexpected_getpgid_cleans_only_direct_child_and_fails_closed(monkeypatch):
    ...
    process = _unowned_posix_process(events)
    monkeypatch.setattr(parent_v21.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(parent_v21.os, "getpgid", lambda _pid: process.pid + 1, raising=False)
    monkeypatch.setattr(parent_v21, "_pid_exists", lambda _pid: False)
    monkeypatch.setattr(parent_v21, "_kill_posix_group", lambda _pgid: pytest.fail("wrong group killed"))
    with parent_v21._PrivateEvidence() as evidence_nonce:
        with pytest.raises(parent_v21.V21ParentError, match="ownership mismatch"):
            parent_v21._posix_phase(
...
def test_getpgid_failure_preserves_original_when_direct_kill_also_fails(monkeypatch):
    ...
    monkeypatch.setattr(parent_v21.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(parent_v21.os, "getpgid", lambda _pid: (_ for _ in ()).throw(RuntimeError("getpgid original")), raising=False)
    monkeypatch.setattr(parent_v21, "_pid_exists", lambda _pid: False)
    with parent_v21._PrivateEvidence() as evidence_nonce:
...
def test_successful_normal_posix_phase_retains_group_owned_path(monkeypatch):
    ...
    process = _unowned_posix_process(events)
    monkeypatch.setattr(parent_v21.subprocess, "Popen", lambda *args, **kwargs: process)
    monkeypatch.setattr(parent_v21.os, "getpgid", lambda _pid: process.pid, raising=False)
    monkeypatch.setattr(parent_v21, "_posix_group_exists", lambda _pgid: False)
    with parent_v21._PrivateEvidence() as evidence_nonce:
        result = parent_v21._posix_phase(
...
def test_hanging_child_and_grandchild_tree_is_killed_without_retry_or_marker(
    ...
    with tempfile.TemporaryDirectory(prefix="onyx-v21-process-tree-") as directory:
        ...
        monkeypatch.setattr(parent_v21, "_phase_command", lambda _phase, _junit: [sys.executable, str(script)])
        started = time.monotonic()
        with parent_v21._PrivateEvidence() as evidence_nonce:
            if hang_phase == "worker":
                (parent_v21.PRIVATE_BASE / evidence_nonce / "focused.junit.xml").write_text("fixture")
            with pytest.raises(parent_v21.V21ParentError, match="timed out"):
                parent_v21._run_phase(
        ...
        while (parent_v21._pid_exists(direct_pid) or parent_v21._pid_exists(grandchild_pid)) and time.monotonic() < deadline:
            time.sleep(0.02)
        assert not parent_v21._pid_exists(direct_pid)
        assert not parent_v21._pid_exists(grandchild_pid)
        assert parent_v21.MARKER not in capsys.readouterr().out
...
def test_regression_report_semantic_tamper_fails_closed(tamper):
    report = copy.deepcopy(verifier_v21._regression_report())
    ...
    with pytest.raises(verifier_v21.V21EvidenceError):
        verifier_v21._validate_regression_report(report)
...
def test_regression_junit_attribute_and_child_tamper_fails_closed(tamper):
    report = verifier_v21._regression_report()
    tree = ET.parse(ROOT / verifier_v21.REGRESSION_JUNIT)
    ...
    with tempfile.TemporaryDirectory(prefix="onyx-v21-junit-tamper-") as directory:
        path = Path(directory) / "tampered.junit.xml"
        tree.write(path, encoding="utf-8", xml_declaration=True)
        with pytest.raises(verifier_v21.V21EvidenceError):
            verifier_v21._junit_path(path, subtest_report=report)


def test_synthetic_junit_counter_requires_explicit_validated_subtest_report():
    with tempfile.TemporaryDirectory(prefix="onyx-v21-junit-synthetic-") as directory:
        ...
        with pytest.raises(verifier_v21.V21EvidenceError, match="lacks validated subtest evidence"):
            verifier_v21._junit_path(path)
...
def test_junit_lexical_text_tail_and_markup_tamper_fails_closed(tamper):
    ...
    with tempfile.TemporaryDirectory(prefix="onyx-v21-junit-lexical-") as directory:
        path = Path(directory) / "tampered.junit.xml"
...
def _evidence_fixture():
    return {
        ...
        "regressions": {
            ...
            "junit_schema": "strict_single_pytest_suite_lexical_v21",
        ...
        "history": {
            "frozen_candidates_cryptographically_validated": 20, "manifests_verified": 87,
            "roots": ["root.sha256"], "unique_leaves_verified": 410,
            "v14_proof": {
        ...
        "live": {"anchors": ["core.py"], "count": 100, "manifest_sha256": "c" * 64, "unique": True},
        "runtime": {
            ...
            "tree_ownership": {
                "posix": "popen_immediate_ownership_direct_fallback_or_killpg_finally",
...
def test_every_structured_evidence_field_tamper_fails_closed(path):
    ...
    with pytest.raises(verifier_v21.V21EvidenceError):
        verifier_v21._assert_exact_evidence("fixture", tampered, expected)
