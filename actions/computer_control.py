...
from core.paths import config_file, memory_dir, resource_root

_RNG = secrets.SystemRandom()

def _load_pyautogui():
    """Load desktop control only when the current OS session can support it."""
    ...
    try:
        ...
        # Linux build/smoke hosts commonly have no DISPLAY.  Desktop actions
        # remain fail-closed, while non-desktop Onyx capabilities may start.
...
def _safe_screenshot_path(requested: str | None) -> Path:
    ...
    fallback = user_desktop_dir() / "onyx_screenshot.png"
    if not requested:
        return fallback
    try:
...
def _require_pyautogui():
    ...
    if not _PYAUTOGUI or pyautogui is None:
        ...
        raise RuntimeError(
            "PyAutoGUI is unavailable in the current desktop session" + detail
        )
    return pyautogui
...
def _random_data(data_type: str) -> str:
    ...
    if dt == "first_name":
        return _RNG.choice(_FIRST_NAMES)

    if dt == "last_name":
        return _RNG.choice(_LAST_NAMES)
    ...
    if dt == "email":
        ...
        num   = _RNG.randint(10, 999)
        return f"{first}.{last}{num}@{_RNG.choice(_DOMAINS)}"

    if dt == "username":
        return f"{_RNG.choice(_FIRST_NAMES).lower()}{_RNG.randint(100, 9999)}"
    ...
    if dt == "password":
        ...
        raw   = (
            ...
            + _RNG.choice("!@#$%")
            + "".join(_RNG.choices(chars, k=9))
        )
        return "".join(_RNG.sample(raw, len(raw)))

    if dt == "phone":
        return f"+1{_RNG.randint(200,999)}{_RNG.randint(1_000_000, 9_999_999)}"

    if dt == "birthday":
        y = _RNG.randint(1980, 2000)
        m = _RNG.randint(1, 12)
        d = _RNG.randint(1, 28)
    ...
    if dt == "address":
        ...
        street = _RNG.choice(["Main St", "Oak Ave", "Park Blvd", "Elm St", "Cedar Ln"])
        return f"{num} {street}"
    ...
    if dt == "city":
        return _RNG.choice(["New York", "Los Angeles", "Chicago", "Houston", "Phoenix"])

    return f"random_{data_type}_{_RNG.randint(1000, 9999)}"
