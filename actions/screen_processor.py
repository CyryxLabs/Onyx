...
from core.audio_contract import LIVE_VOICE, assert_original_live_voice
...
_SYSTEM_PROMPT = (
    "You are Onyx, a Cyryx Labs AI assistant using the configured cloud vision model. "
        ...
        while True:
            try:
                ...
                async with client.aio.live.connect(
                    model=resolve_live_model(_CONFIG_PATH), config=config
                ) as session:
            ...
            async for response in self._session.receive():
                ...
                if sc.turn_complete:
                    if transcript and self._player:
                        ...
                        if full:
                            self._player.write_log(f"Onyx: {full}")
                            print(f"[Vision] 💬 {full}")
                    transcript = []
                    # Auto-close camera ~2s after Onyx finishes speaking
