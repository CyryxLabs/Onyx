...
try:
    ...
    notification.notify(title="Onyx Reminder", message=message, timeout=15)
    notified = True
except Exception:
    pass
...
if not notified:
    try:
        ...
        ToastNotifier().show_toast("Onyx Reminder", message, duration=15, threaded=False)
        notified = True
    except Exception:
        pass
...
if not notified:
    try:
        import subprocess
        script = 'display notification "{{}}" with title "Onyx Reminder"'.format(
            message.replace('"', '')
        )
...
"""
    ...
    script_body = f"""# Auto-generated by Onyx reminder — do not edit
...
def _schedule_windows(target_dt: datetime, task_name: str,
    ...
    xml_content = (
        ...
        '  <RegistrationInfo><Description>Onyx Reminder</Description></RegistrationInfo>\n'
...
def _schedule_mac(target_dt: datetime, task_name: str,
    ...
    label     = f"com.cyryxlabs.onyx.reminder.{task_name}"
...
_TOPIC_STORE_NAME = "watched_topics.json"


def _topic_store_path() -> Path:
    return Path(data_root()) / _TOPIC_STORE_NAME


def _load_monitor():
    """Rebuild the monitor from disk so watches survive a restart."""
    from core.topic_monitor_v1 import TopicMonitorV1

    monitor = TopicMonitorV1()
    path = _topic_store_path()
    if not path.is_file():
        return monitor
    try:
        ...
        return monitor
    for entry in stored.get("topics", []) if isinstance(stored, dict) else []:
        try:
            monitor.watch(
                str(entry.get("topic", "")),
                interval_ms=int(entry.get("interval_ms", monitor.default_interval_ms)),
            )
            watch = monitor._watches[str(entry["topic"]).casefold()]
            watch.next_due_ms = int(entry.get("next_due_ms", 0))
            watch.seen = [str(item) for item in entry.get("seen", [])][-200:]
        except Exception:
            continue
    return monitor


def _save_monitor(monitor) -> None:
    payload = {
        "topics": [
            {
                "topic": watch.topic,
            ...
            for watch in monitor._watches.values()
        ]
    }
    path = _topic_store_path()
    try:
        ...
        print(f"[Reminder] could not persist watched topics: {error}")


def _topic_action(action: str, parameters: dict, player=None) -> str:
    from core.topic_monitor_v1 import TopicMonitorV1Error

    monitor = _load_monitor()
    topic = str(parameters.get("topic") or parameters.get("message") or "").strip()

    if action == "list_topics":
        names = monitor.topics
        return ("Watching: " + ", ".join(names)) if names else "No topics are being watched."

    if action == "watch_topic":
        try:
            interval = parameters.get("interval_ms")
            name = monitor.watch(
                topic,
                interval_ms=interval if interval is not None else None,
            )
        except TopicMonitorV1Error as error:
            return f"Could not watch that: {error}"
        _save_monitor(monitor)
        return f"Now watching {name}. I will mention anything new."

    if action == "unwatch_topic":
        try:
            removed = monitor.unwatch(topic)
        except TopicMonitorV1Error as error:
            return f"Could not stop watching that: {error}"
        _save_monitor(monitor)
        return f"Stopped watching {topic}." if removed else f"{topic} was not being watched."

    # check_topics
    if not monitor.topics:
        return "No topics are being watched."
    return (
        "Topic checks run through the runtime dispatcher so permission and "
        "audit controls cannot be bypassed."
    )
...
def reminder(
    ...
    # Topic watching shares this tool because both answer "tell me about this
    # later".  Absent an explicit action the original date/time contract is
    # untouched, so every existing caller behaves exactly as before.
    action = str(parameters.get("action") or "").strip().lower().replace("-", "_")
    if action in {"watch_topic", "unwatch_topic", "list_topics", "check_topics"}:
        return _topic_action(action, parameters, player)
    ...
    task_name  = f"OnyxReminder_{target_dt.strftime('%Y%m%d_%H%M%S')}"
