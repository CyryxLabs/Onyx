"""
file_processor.py — Onyx Universal File Processor
...
from xml.etree.ElementTree import ElementTree, indent, tostring
from pathlib import Path

from core.approved_execution import execute_materialized_source, materialize_source_request
...
def materialize_file_processor_request(parameters: dict | None) -> dict:
    """Resolve the exact input, including immutable identity for code execution."""
    request = copy.deepcopy(dict(parameters or {}))
    action = str(request.get("action", "")).lower().strip().replace("-", "_")
    request["action"] = action
    raw_path = str(request.get("file_path", "")).strip()
    if not raw_path:
        raise ValueError("an exact file path is required")
    path = Path(raw_path).expanduser().resolve(strict=action == "run")
    if action == "run" and not path.is_file():
        raise ValueError(f"not a regular file: {path}")
    request["file_path"] = str(path)
    if action == "run" and _detect_type(path) == "code":
...
def _generated_preview(
    ...
    """Return model-derived content without writing unseen bytes to disk.

    The ordinary tool approval happens before the model, OCR, or transcription
    result exists, so it cannot authorize those exact bytes.  ``save=True`` is
    consequently a fail-closed request until a separate immutable-output
    approval flow is added.  Deterministic conversions do not use this helper.
    """
    safe_action = re.sub(r"[^A-Za-z0-9_-]+", "_", action).strip("_") or "generated"
    proposed = _output_path(src, safe_action, new_ext)
    if params.get("save"):
        explanation = (
            "The requested save was refused because the generated bytes did not "
            "exist when the original action was approved."
        )
    else:
        explanation = "Generated content is preview-only by default."
    return (
        f"{result}\n\n"
        "[PREVIEW ONLY - NOT SAVED]\n"
        f"{explanation}\n"
        f"Proposed output path: {proposed}"
    )
...
def _process_image(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action in ("describe", "ocr", "analyze", "read", "extract_text"):
        try:
            ...
            return _generated_preview(path, f"{action}_result", result, params)
...
def _process_pdf(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action in ("summarize", "extract_text", "translate_hint", "analyze", "reformat"):
        ...
        try:
            ...
            result   = response.text.strip()
            return _generated_preview(path, action, result, params)
        except Exception as e:
...
def _process_data(path: Path, file_type: str, action: str,
    ...
    if action == "analyze":
        ...
        try:
            ...
            return _generated_preview(path, action, response.text.strip(), params)
        except Exception as e:
            return f"AI analysis failed: {e}"
...
def _process_xml(path: Path, action: str, params: dict, speak=None) -> str:
    """Validate, format, or analyze XML without pretending it is JSON."""
    action = action or "analyze"
    try:
    ...
    preview = tostring(root, encoding="unicode")[:8000]
    instruction = params.get("instruction") or f"{action.capitalize()} this XML document."
    try:
        response = _gemini_client().generate_content(f"{instruction}\n\nXML:\n{preview}")
        return _generated_preview(path, action, response.text.strip(), params)
    except Exception as e:
        return f"AI processing failed: {e}"
...
def _process_code(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action == "run":
        return execute_materialized_source(params)
    ...
    try:
        ...
        response = model.generate_content(prompt)
        result   = response.text.strip()
        if action in ("fix", "optimize", "document", "test"):
            return _generated_preview(path, action, result, params, path.suffix)
        return _generated_preview(path, action, result, params)
    except Exception as e:
        return f"AI processing failed: {e}"
...
def _process_audio(
    ...
    if action == "transcribe":
        try:
            ...
            result = response.text.strip()
            return _generated_preview(preview_source or path, "transcript", result, params)
        except Exception as e:
    ...
    if action == "extract":
        ...
        try:
            ...
            if archive == destination or archive.is_relative_to(destination):
                return "Extraction destination must not contain the archive itself."
            ...
            if zipfile.is_zipfile(path):
                with zipfile.ZipFile(path) as archive_file:
                    ...
                    if sum(item.file_size for item in members) > max_unpacked:
                        raise ValueError("Archive expands beyond the 2 GB safety limit.")
                    for item in members:
                        _safe_target(item.filename)
                ...
                with tarfile.open(path) as archive_file:
                    ...
                    if len(members) > max_files:
                        raise ValueError(f"Archive contains too many files ({len(members):,}).")
                    if sum(item.size for item in members) > max_unpacked:
                        raise ValueError("Archive expands beyond the 2 GB safety limit.")
                    for item in members:
                        _safe_target(item.name)
                    archive_file.extractall(destination, filter="data")
            else:
                return f"Unsupported archive format: {path.suffix.lower()}"
