        ...
        if month_name in lower:
            ...
            if day_match:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid flight date: {raw!r}") from exc

    raise ValueError(
        f"Unsupported flight date {raw!r}. Use YYYY-MM-DD, an unambiguous full date, "
        "today, or tomorrow."
    )
...
def _build_google_flights_url(
    ...
    cabin_label = cabin.lower() if cabin.lower() in _CABIN_CODE else "economy"
    passenger_label = f"{passengers} adult{'s' if passengers != 1 else ''}"
    if return_date:
        trip = (
            f"Flights from {origin} to {destination} on {date} returning {return_date}, "
            f"{passenger_label}, {cabin_label} class"
        )
    else:
        trip = (
    ...
    return f"{base}?{urlencode({'q': trip, 'curr': 'USD', 'hl': 'en'})}"
...
def flight_finder(parameters: dict, player=None, speak=None) -> str:
    ...
    try:
        passengers = min(9, max(1, int(params.get("passengers", 1))))
    except (TypeError, ValueError):
        return "Passengers must be a whole number from 1 to 9, sir."
    ...
    if return_date and return_date < date:
        return "Return date cannot be earlier than the departure date, sir."
