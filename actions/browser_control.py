...
ONYX_PROFILES_DIR = Path.home() / ".onyx_profiles"


def _managed_profile_dir(name: str) -> Path:
    """Return a persistent browser profile owned by Onyx."""
    primary = ONYX_PROFILES_DIR / name
...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = _managed_profile_dir(browser)
...
class _BrowserSession:
    ...
    async def _launch(self):
        """
        Launches the browser using the real user profile.
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(_managed_profile_dir("firefox"))
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
                "viewport":    None,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using Onyx profile")
                onyx_profile = str(_managed_profile_dir("firefox_onyx"))
        ...
        if engine_name == "webkit":
            safari_profile = str(_managed_profile_dir("safari"))
            kwargs = {
                "headless":    False,
                "slow_mo":     0,
                "viewport":    None,
        ...
        onyx_profile = str(_managed_profile_dir(self.browser_name))
        print(f"[Browser] Retrying with Onyx profile: {onyx_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(onyx_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with Onyx profile")
    ...
    async def screenshot(self, path: str = None) -> str:
        ...
        try:
            ...
            save_path = path or str(user_desktop_dir() / "onyx_screenshot.png")
