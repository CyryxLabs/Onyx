...
ONYX_GAME_UPDATER_TASK = "Onyx_GameUpdater"
ONYX_GAME_UPDATER_PLIST = "labs.cyryx.onyx.gameupdater.plist"
ONYX_GAME_UPDATER_LABEL = "labs.cyryx.onyx.gameupdater"
ONYX_GAME_UPDATER_CRON_MARKER = "# Onyx_GameUpdater"
        ...
        try:
            for ctrl in dialog.descendants(control_type=control_type):
                if target in ctrl.window_text().upper():
                    ctrl.click_input()
                    print(
                        f"[GameUpdater] ✅ Sürücü seçildi ({control_type}): {ctrl.window_text()}"
                    )
                    return True
        except Exception:
            continue
...
def _handle_install_dialog_pyautogui(game_name: str, best_drive: dict) -> str:
    ...
    return (
        f"Attempted drive {drive_label} selection and Install click for '{game_name}'."
    )
...
def _handle_install_dialog(game_name: str) -> str:
    ...
    print(
        f"[GameUpdater] 🏆 Hedef sürücü: {drive_label} ({best_drive['free_gb']:.1f} GB boş)"
    )
                ...
                for hwnd in findwindows.find_windows(
                    ...
                    try:
                        ...
                        if (
                            ...
                            if any(
                                x in all_text
                                for x in ("C:", "D:", "E:", "F:", "INSTALL", "YÜKLE")
                            ):
                                dialog = win
                                break
                    except Exception:
...
def _search_steam_appid(game_name: str) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})  # noqa: S310 - fixed HTTPS Steam API
        with urllib.request.urlopen(req, timeout=6) as resp:  # noqa: S310 - fixed HTTPS Steam API
...
def _update_steam_games(steam_path: Path, game_name: str = None) -> str:
    ...
    if update_started:
        ...
        suffix = (
            f" and {len(update_started) - 3} more" if len(update_started) > 3 else ""
        )
...
def _install_steam_game(
    ...
    if app_id:
        ...
        name_lower = game_name.lower()
        already = next(
            (g for g in installed_games if name_lower in g["name"].lower()), None
        )
    else:
    ...
    try:
        ...
        return (
            f"Install started for '{game_name}'. Steam will open the download dialog."
        )
    except Exception as e:
...
def _schedule_windows(hour: int, minute: int) -> str:
    task_name = ONYX_GAME_UPDATER_TASK
...
def _schedule_mac(hour: int, minute: int) -> str:
    ...
    plist_path = plist_dir / ONYX_GAME_UPDATER_PLIST
    script_path = Path(__file__).resolve()
    plist_content = f"""<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN"
  "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
    <key>Label</key><string>{ONYX_GAME_UPDATER_LABEL}</string>
    ...
    try:
        ...
        if result.returncode == 0:
            return (
                f"Daily game update scheduled at {hour:02d}:{minute:02d} via launchd."
            )
        return f"Scheduling failed: {result.stderr.strip()}"
    except Exception as e:
...
def _schedule_linux(hour: int, minute: int) -> str:
    script_path = Path(__file__).resolve()
    marker = ONYX_GAME_UPDATER_CRON_MARKER
    cron_entry = (
...
def _cancel_scheduled_update() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Delete", "/TN", ONYX_GAME_UPDATER_TASK, "/F"],
            capture_output=True,
            text=True,
            **_CNW,
        )
    ...
    if is_mac():
        ...
        plist_path = launch_agents / ONYX_GAME_UPDATER_PLIST
    ...
    try:
        ...
        lines = [
            line
            for line in existing.stdout.splitlines()
            if ONYX_GAME_UPDATER_CRON_MARKER not in line
        ]
...
def _get_schedule_status() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Query", "/TN", ONYX_GAME_UPDATER_TASK, "/FO", "LIST"],
            capture_output=True,
            text=True,
            **_CNW,
        )
        ...
        for line in result.stdout.strip().splitlines():
            if any(
                k in line
                for k in ("Next Run", "Sonraki", "Prochaine", "Próxima", "Nächste")
            ):
                return f"Game update scheduled. {line.strip()}"
        return "Game update is scheduled."
    if is_mac():
        launch_agents = Path.home() / "Library" / "LaunchAgents"
        candidate = launch_agents / ONYX_GAME_UPDATER_PLIST
        return (
...
def game_updater(parameters: dict, player=None, speak=None) -> str:
    ...
    if not 0 <= hour <= 23 or not 0 <= minute <= 59:
        return "Schedule time must use hour 0-23 and minute 0-59."
    ...
    if action == "download_status":
        if platform in ("steam", "both"):
            steam_path = _find_steam_path()
            results.append(
                _get_download_status(steam_path)
                if steam_path
